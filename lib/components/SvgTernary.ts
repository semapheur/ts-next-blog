...
import { betaPDF } from "lib/utils/probability"
...
export default class SVGTernaryPlot {
  ...
  constructor(
    ...
    initialState: TernaryState = {
      point: new DOMPoint(THIRD, THIRD, THIRD),
      director: 0.5,
    },
    size?: number,
    margin?: number,
    ...
    addChildElement(this.frameGroup, "g", {
      id: `${this.idPrefix}-crosshair-group`,
    })
    addChildElement(this.frameGroup, "g", {
      id: `${this.idPrefix}-director-group`,
    })
  ...
  private transformMatrix() {
    const matrix = [1, 0, 0, -1, this.translate.x, this.translate.y]
    return `matrix(${matrix.join(",")})`
  }

  private setTransform(size?: number) {
    if (!size) size = Number.parseFloat(this.svgElement.getAttribute("width")!)
  ...
  private drawTriangle(stroke = "rgb(var(--color-text))") {
    ...
    const attr = {
      ...
      points: triangle,
      fill: `url(#${this.idPrefix}-grid-pattern)`,
      stroke: stroke,
      "stroke-width": "2px",
  ...
  public grid(division = 10) {
    ...
    // Pattern
    const attr = {
      id: `${this.idPrefix}-grid-pattern`,
      patternUnits: "userSpaceOnUse",
      //patternTransform: this.transformMatrix(),
      width: `${gridUnit}`,
      height: `${2 * SIN60 * gridUnit}`,
    }
  ...
  public axis() {
    ...
    let line = {
      x1: `${(3 / 4) * this.side}`,
      y1: `${(this.side * SIN60) / 2}`,
      x2: "0",
      y2: "0",
      stroke: this.colors.axis.a,
      "stroke-width": "1",
    }
  ...
  public point(newPoint?: DOMPoint, store?: TernaryStore) {
    ...
    const line = {
      id: `${this.idPrefix}-plot-projector`,
      x1: `${svgPos.x}`,
      y1: `${svgPos.y}`,
      x2: `${this.projector() * this.side}`,
      y2: "0",
      stroke: this.colors.point,
      "stroke-width": "1",
      "stroke-dasharray": "5",
    }
    ...
    function onDrag(store: TernaryStore | undefined, event: PointerEvent) {
      ...
      const projector = document.getElementById(
        `${this.idPrefix}-plot-projector`,
      )
  ...
  public director(newDirector?: number, store?: TernaryStore) {
    ...
    const attr = {
      id: `${this.idPrefix}-director-point`,
      cx: `${this.side * this.state.director}`,
      cy: "0",
      r: "0.5rem",
    ...
    const line = {
      id: `${this.idPrefix}-director-line`,
      x1: `${this.side / 2}`,
      y1: `${this.side * SIN60}`,
      x2: `${this.side * this.state.director}`,
      y2: "0",
      stroke: this.colors.director,
      "stroke-width": "1",
      "stroke-dasharray": "5",
    }
  ...
  private inTriangle(svgPos: DOMPoint | null): boolean {
    ...
    if (svgPos.y < 0 || svgPos.y > this.side * SIN60) return false

    if (svgPos.x < svgPos.y * TAN30 || svgPos.x > this.side - svgPos.y * TAN30)
      return false
  ...
  private crosshair() {
    ...
    const path = {
      id: `${this.idPrefix}-crosshair-line-a`,
      d: "",
      stroke: this.colors.axis.a,
      "stroke-width": "1px",
    ...
    function onPointerMove(event: PointerEvent) {
      ...
      if (this.inTriangle(svgPos)) {
        ...
        text.innerHTML = `(${coord.x.toFixed(2)}, ${coord.y.toFixed(
          2,
        )}, ${coord.z.toFixed(2)})`

        aLine.setAttribute(
          "d",
          `M${((1 - coord.x) * this.side) / 2},${
            (1 - coord.x) * this.side * SIN60
          }L${printPoint(svgPos)}`,
        )
        bLine.setAttribute(
          "d",
          `M${coord.y * this.side},0L${printPoint(svgPos)}`,
        )
        cLine.setAttribute(
          "d",
          `M${this.side - svgPos.y * TAN30},${svgPos.y}L${printPoint(svgPos)}`,
        )
      } else {
        text.setAttribute("x", "")
        text.setAttribute("y", "")
