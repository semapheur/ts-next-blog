...
import { SvgPlot2d } from "lib/components/SvgPlot";
...
interface PlotInputProps {
  index: number;
  values: PlotFields;
  handleAddPlot: (i: number, e: SubmitEvent<HTMLFormElement>) => void;
...
function Panel() {
  ...
  const addPlot = (i: number, e: SubmitEvent<HTMLFormElement>) => {
    e.preventDefault();
...
export default function InteractiveSvgPlotApp() {
  ...
  useEffect(() => {
    ...
    if (!svgRef.current && wrap) {
      svgRef.current = new SvgPlot2d(wrap);
      svgRef.current.axes();
      svgRef.current.grid();
