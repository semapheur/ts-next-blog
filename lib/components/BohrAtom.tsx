...
import useResizeObserver, { Size } from 'lib/hooks/useResizeObserver'
...
export default function BohrAtom({ number, symbol, shells, ...props }: Props) {
  ...
  useEffect(() => {
    ...
    svg.setAttribute(
      'width',
      width > height ? height.toString() : width.toString(),
    )
  }, [wrapSize])

  return (
    <div ref={wrapRef} {...props}>
          ...
          <g key={`g.${index}`}>
            ...
            {[...Array(electrons).keys()].map((electron) => (
              <circle
                ...
                r='0.3'
                cx={
                  (index + 3) * Math.cos((2 * Math.PI * electron) / electrons)
                }
                cy={
                  (index + 3) * Math.sin((2 * Math.PI * electron) / electrons)
                }
