...
export class InstancedArrow extends THREE.Object3D {
  ...
  private shaftMaterial: THREE.MeshBasicNodeMaterial;
  private headMaterial: THREE.MeshBasicNodeMaterial;
  ...
  constructor(
    ...
    const opacityNode = attribute("opacity", "float");

    const finalColorNode = vec4(instanceColorNode, opacityNode);

    this.shaftMaterial = new THREE.MeshBasicNodeMaterial({
      transparent: true,
      colorNode: finalColorNode,
    });
    this.headMaterial = new THREE.MeshBasicNodeMaterial({
      transparent: true,
      colorNode: finalColorNode,
    });
  ...
  setArrow(
    ...
    if (length <= minLength) {
      ...
      this.instancedShaft.count = Math.max(
        this.instancedShaft.count,
        index + 1,
      );
...
export function gridSphere(
  ...
  const longitudeLines = createLongitudeLines(
    radius,
    lonSegments,
    lineMaterial,
  );
