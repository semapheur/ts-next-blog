...
$$
  C = \bigcup_{T, L \in\N_+} \bigcup_{l_1,\dots,l_L \in\N_+} \bigcup_{(c_{k,t})_{(k,t) \in [L]\times [T]} \subseteq\N} \left(\bigtimes_{k=1}^L ((\R^{c_{k,1} \times\cdots\times c_{k,T}})^{l_k \times l_{k-1}} \times \R^{l_k}) \right)
$$
...
- $\Phi = (((W_{k,n,m})_{(n,m) \in [l_k ]\times [l_{k-1}]}, (B_{k,n})_{n\in [l_k]}))_{k\in [L]} \in \bigtimes_{k=1}^L ((\R^{c_{k,1} \times\cdots\times c_{k,T}})^{l_k \times l_{k-1}} \times \R^{l_k}) \subseteq C$
