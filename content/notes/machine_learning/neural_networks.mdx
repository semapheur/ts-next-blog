...
Let $\mathcal{F}_1$ be a set of functions from $X$ to $Z$ and $\mathcal{F}_2$ a set of functions from $Z$ to $Y$. Let $\mathcal{H} = \mathcal{F_2}\circ\mathcal{F_1}$ be the composition class. That is, for every $f_1\in\mathcal{F}_1$ and $f_2\in\mathcal{F}_2$, there exists $h\in\mathcal{H}$ with $h(\mathbf{x}) = f_2(f_1(\mathbf{x}))$. If 
...
where $\mathcal{H}_C$ is the restriction of $\mathcal{H}$ to functions from $C$ to $\Set{0,1}$. We can naturally extend the definition for a set of functions from $X$ to som finite set $Y$, by letting $\mathcal{H}_C$ be the restriction of $\mathcal{H}$ to functions from $C$ to $Y$, and keeping the definition of $\tau_\mathcal{H}(m)$ intact.

Let $V_0,\dots,V_T$ be the layers of a neural network. Fix some $t\in[T]$. By assigning different weights on the edges between $V_{t-1}$ and $V_t$, we obtain different functions from $\R^{|V_{t-1}|} \to\set{\pm 1}^{|V_t|}$. Let $\mathcal{H}_t$ be the class of all possible mappings $\R^{|V_{t-1}|}\to\set{\pm 1}^{|V_t|}$. Then $\mathcal{H}$ can be written as a composition $\mathcal{H} = \mathcal{H}_T \circ\cdots\circ\mathcal{H}_1$. It can shown that the growth function of a composition of hypothesis classes is bounded by the product of the growth function of the individual classes, i.e.

$$
  \tau_\mathcal{H} (m) \leq \prod_{t=1}^T \tau_{\mathcal{H}_t} (m)
$$

In addition, each $\mathcal{H}_t$ can be written as a product of function classes, $\mathcal{H}_t = \mathcal{H}_{t,1} \times\cdots\times \mathcal{H}_{t,|V_t|}$, where each $\mathcal{H}_{t,j}$ is all function from layer $t - 1$ to $\set{\pm 1}$ that the $j$th neuron of layer $t$ can implement. It can also be shown that

$$
  \tau_{\mathcal{H}_t} (m) \leq \prod_{i=1}^{|V_t|} \tau_{\mathcal{H}_{t,i}} (m)
$$
