...
$$
  \boldsymbol{\theta}_{t+1} = \argmin_{\boldsymbol{\theta}} \frac{1}{2}\lVert \boldsymbol{\theta} - \boldsymbol{\theta}_t \rVert^2 + \eta\left(\mathcal{L}(\boldsymbol{\theta}_t) + \langle \boldsymbol{\theta} - \boldsymbol{\theta}_t, \nabla\mathcal{L}(\boldsymbol{\theta}_t)\rangle \right)
$$
...
$$
\begin{equation}
  \sum_{t=1}^\tau \langle\mathbf{w}_t - \mathbf{w}^*, \mathbf{v}_t \rangle \leq \frac{1}{2\eta}\lVert\mathbf{w}^* \rVert^2 + \frac{\eta}{2} \sum_{t=1}^\tau \lVert\mathbf{v}_t \rVert^2 \tag{\label{equation-9}}
\end{equation}
$$

In particular, for every $B, \rho > 0$, if we have that $\lVert\mathbf{v}_t \rVert\leq\rho$ for all $t$ and if we set $\eta = \sqrt{B^2 / (\rho^2 t)}$, then for every $\mathbf{w}^*$ with $\lVert\mathbf{w}^* \rVert \leq B$ we have
...
\begin{align*}
  ...
  =& \frac{1}{2\eta} \left(-\lVert \mathbf{w}_t - \mathbf{w}^* - \eta\mathbf{w}_t \rVert^2 + \lVert\mathbf{w}_t - \mathbf{w}^* \rVert^2 + \eta^2 \lVert\mathbf{v}_t \rVert^2 \right) \\
  =& \frac{1}{2\eta} \left(-\lVert\mathbf{w}_{t+1} - \mathbf{w}^* \rVert^2 + \lVert\mathbf{w}_t - \mathbf{w}^* \rVert^2 \right) + \frac{\eta}{2}\lVert\mathbf{v}_t \rVert^2
\end{align*}
$$
...
\begin{equation}
\begin{split}
  \sum_{t=1}^\tau \langle\mathbf{w}_t - \mathbf{w}^*, \mathbf{v}_t \rangle =& \frac{1}{2\eta} \sum_{t=1}^\tau \left(-\lVert \mathbf{w}_{t+1} - \mathbf{w}^* \rVert^2 + \lVert \mathbf{w}_t - \mathbf{w}^* \rVert^2 \right) \\
  &+ \frac{2}{\eta} \sum_{t=1}^T \lVert\mathbf{v}_t \rVert^2
...
$$
  \lVert \mathbf{w}_0 - \mathbf{w}^* \rVert^2 - \lVert \mathbf{w}_{\tau + 1} - \mathbf{w}^* \rVert^2
$$
...
$$
\begin{align*}
  \sum_{t=1}^\tau \langle\mathbf{w}_t - \mathbf{w}^*, \mathbf{v}_t \rangle =& \frac{1}{2\eta} (\lVert\mathbf{w}_0 - \mathbf{w}^* \rVert^2 - \lVert \mathbf{w}_{\tau+1} - \lVert \mathbf{w}_{\tau+1} - \mathbf{w}^* \rVert^2) \\
  &+ \frac{\eta}{2} \sum_{t=1}^\tau \lVert\mathbf{v}_t \rVert^2 \\
  \leq& \frac{1}{2\eta}\lVert \mathbf{w}_0 - \mathbf{w}^* \rVert + \frac{\eta}{2}\sum_{t=1}^\tau \lVert\mathbf{v}_t \rVert^2 \\
  =& \frac{1}{2\eta} \lVert\mathbf{w}^* \rVert^2 + \frac{\eta}{2} \sum_{t=1}^\tau \sum_{t=1}^\tau \lVert\mathbf{v}_t \rVert^2
\end{align*}
$$

This proves $\eqref{equation-9}$. The second inequality follows by upper bounding $\lVert\mathbf{w}^* \rVert$ by $B$, $\lVert\mathbf{v}_t \rVert$ by $\rho$, dividing by $\tau$, and plugging in the value of $\eta$.
</details>
</MathBox>

Consider a loss function $\mathcal{L}$ that is convex-Lipschitz and let $\boldsymbol{\theta}^* \in\R^p$ be any vector with upper bound $\lVert\boldsymbol{\theta}^* \rVert\leq B$. We would like to obtain an upper bound on the suboptimality of our solution with respect to $\boldsymbol{\theta^*}$, i.e. $\mathcal{L}(\bar{\boldsymbol{\theta}}) - \mathcal{L}(\boldsymbol{\theta}^*)$ where $\bar{\boldsymbol{\theta}} = \frac{1}{\tau} \sum_{t=1}^\tau \boldsymbol{\theta}_t$. From the definition of $\bar{\boldsymbol{\theta}}$ and Jensen's inequality, we have that
...
Since $\mathcal{L}$ is $\rho$-Lipschitz, it follows that $\lVert\nabla\mathcal{L}(\boldsymbol{\theta}_t)\rVert \leq\rho$. Applying $\ref{lemma-1}$, we obtain
...
<MathBox title='' boxType='lemma'>
Let $S\subseteq\R^n$ be a convex open set and let $f:A\to\R$ be a convex function. Then, $f$ is $\rho$-Lipschitz over $A$ if and only if for all $\mathbf{w}\in A$ and $\mathbf{v}\in \partial f(\mathbf{w})$ we have that $\lVert\mathbf{v}\rVert\leq\rho$.


<details>
<summary>Proof</summary>

Assume that for all $\mathbf{v}\in\partial f(\mathbf{w})$ we have that $\lVert\mathbf{v}\rVert\leq\rho$. Since $\mathbf{v}\in\partial f(\mathbf{w})$ we have
...
$$
  f(\mathbf{w}) - f(\mathbf{u}) \leq \langle\mathbf{v},\mathbf{w} - \mathbf{u}\rangle \leq \lVert\mathbf{v}\rVert\cdot\lVert\mathbf{w} - \mathbf{v}\rVert \leq \rho\lVert\mathbf{w} - \mathbf{u}\rVert
$$

An analogous argument can show that $f(\mathbf{u}) - f(\mathbf{w}) \leq\rho\lVert\mathbf{w} - \mathbf{u}\rVert$. Hence $f$ is $\rho$-Lipschitz.

Now assume that $f$ is $\rho$-Lipschitz. Choose some $\mathbf{w}\in A$ and $\mathbf{v}\in\partial f(\mathbf{w})$. Since $A$ is open, there exists $\epsilon > 0$ such that $\mathbf{u} = \mathbf{w} + \epsilon\mathbf{v}/\lVert\mathbf{v}\rVert$ belongs to $A$. Thus, $\langle\mathbf{u} - \mathbf{w}, \mathbf{v}\rangle = \epsilon\lVert\mathbf{v}\rVert$ and $\lVert\mathbf{u} - \mathbf{w}\rVert = \epsilon$. From the definition of the subgradient (Definition $\ref{definition-1}$) we get

$$
  f(\mathbf{u}) - f(\mathbf{w}) \geq \langle\mathbf{v}, \mathbf{u} - \mathbf{w} \rangle = \epsilon\lVert\mathbf{v}\rVert
$$

On the other hand, from the Lipschitzness of $f$ we have

$$
  \rho\epsilon = \rho\lVert\mathbf{u} - \mathbf{w}\rVert \geq f(\mathbf{u}) - f(\mathbf{w})
$$

Combining these two inequalities we conclude that $\lVert\mathbf{v}\rVert\leq\rho$.
</details>
</MathBox>
...
$$
  \mathcal{L}(\boldsymbol{\theta}) = \frac{1}{n} \sum_{i=1}^n (\mathbf{x}_i^\top \boldsymbol{\theta} - y_i)^2 = \frac{1}{n}\lVert\mathbf{X}\boldsymbol{\theta} - \mathbf{y}\rVert_2^2
$$
