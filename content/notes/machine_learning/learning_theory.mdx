...
Let $\mathcal{S} = \Set{(\mathbf{x}_n, \mathbf{y}_n)}_{n=1}^N$ be a sample and $\Theta$ a parameter space. In frequentist decision theory, we are interested in finding an estimator $\hat{\boldsymbol{\theta}} = \delta(\mathcal{S})$ miniziming the risk

$$
  R(\delta, \boldsymbol{\theta}^*) = \mathbb{E}_{\Pr (\mathcal{D}|\boldsymbol{\theta}^*)} [\ell(\boldsymbol{\theta}^*, \delta(\mathcal{S}))]
$$

where $\ell$ is a loss function. In supervised learning, the estimator $\delta$ is a prediction function $\hat{\mathbf{y}} = f(\mathbf{x})$, and the state of nature is the true distribution $\Pr^* (\mathbf{x}, \mathbf{y})$. The risk of an estimator $f$ is given by

$$
  R(f, \Pr^*) = R(f) := \mathbb{E}_{\Pr^* (\mathbf{x})\Pr^* (\mathbf{y}|\mathbf{x})} [\ell(\mathbf{y}, f(\mathbf{x}))]
$$

This is called the *population risk*, since the expectation are taken with respect to the true joint distribution $\Pr^* (\mathbf{x}, \mathbf{y})$. Because $\Pr^*$ is unknown, we can approximate it using the empirical distribution with $N$ samples:

$$
  \Pr_\mathcal{S} (\mathbf{x}, \mathbf{y}|\mathcal{S}) := \frac{1}{|\mathcal{S}|} \sum_{(\mathbf{x}_n, \mathbf{y}_n \in\mathcal{S})} \delta(\mathbf{x} - \mathbf{x}_n) \delta(\mathbf{y} - \mathbf{y}_n)
$$

Plugging this in gives the *empirical risk*

$$
  R(f,\mathcal{S}) := \mathbb{E}_{\Pr_\mathcal{S} (\mathbf{x}, \mathbf{y})} [\ell(\mathbf{y}, f(\mathbf{x}))] = \frac{1}{N} \sum_{n=1}^N \ell(\mathbf{y}_n, f(\mathbf{x}_n))
$$

Note that $R(f,\mathcal{S})$ is a random variable, since it depends on the training set. An optimal predictor is given by

$$
  \hat{f}_{\text{ERM}} = \argmin_{f\in\mathcal{H}} R(f,\mathcal{S}) = \argmin_{f\in\mathcal{H}} \frac{1}{N} \sum_{n=1}^N \ell(\mathbf{y}_n, f(\mathbf{x}_n))
$$

where $\mathcal{H}$ is a *hypothesis space* of functions. This is called *empirical risk minization* (ERM).

## Approximation and estimation error

Let $f^{**} = \argmin_f R(f)$ be the predictor that achieves the minimal possible population risk, where we optimize over all possible function. Since we cannot consider all possible functions, we can restrict the optimization to a hypothesis space $\mathcal{H}$ giving the predictor $f_\mathcal{H}^* = \argmin_{f\in\mathcal{H}} R(f)$. Unfortunately, we cannot compute the population risk $R(f)$, and we are thus limited to optimize the empirical risk in $\mathcal{H}$ given a fixed training set $\mathcal{S}$:

$$
  f_\mathcal{S}^* = \argmin_{f\in\mathcal{H}} R(f,\mathcal{S})
$$

This is a random function since $\mathcal{S}\sim \Pr^*$ is random. By adding and subtracting the deterministic population risks $R(f^**)$ and $R(f_\mathcal{H}^*)$, the expected risk of our chosen predictor compared to the best possible predictor can be decomposed into two terms as follows

$$
  \mathbb{E}_{\mathcal{S}\sim\Pr^*} [R(f_\mathcal{S}^*) - R(f^{**})] = \underbrace{R(f_\mathcal{H}^*) - R(f^{**})}_{\varepsilon_{\text{a}}(\mathcal{H})} + \underbrace{\mathbb{E}_{\mathcal{S}\sim\Pr^*} [R(f_{\mathcal{S}}^*) - R(f_\mathcal{H}^*)]}_{\varepsilon_{\text{e}}(\mathcal{H}, N)}
$$

where
- $\varepsilon_{\text{a}}(\mathcal{H})$ is the *approximation error* measuring how closely $f_\mathcal{H}^*$ can model the true optimal function $f^{**}$
- $\varepsilon_{\text{e}}(\mathcal{H}, N)$ is the *estimation error*, which measures the error in finding the best function in the class, due to having a finite training set of size $N$ to evaluate performance

We can decrease the approximation error by using a more complex set of functions $\mathcal{H}$. However, this usually increases overfitting, which increases the estimation error. We can quantify the degree of overfitting of any model $f$ by computing the *generalization gap*, defined as

$$
  \operatorname{GenGap}(f) = R(f) - R(f, \mathcal{S}_{\text{train}}) \approx R(f, \mathcal{S}_{\text{test}}) - R(f, \mathcal{D}_{train})
$$

Thus we need to find model that tradeoff approximation error and estimation error.
...
$$
  R_\lambda (f,\mathcal{S}) = R(f,\mathcal{D}) + \lambda C(f)
$$

where $C(f)$ measures the complexity of the predictor $f(\mathbf{x},\boldsymbol{\theta})$ and $\lambda\geq 0$, which is known as a *hyperparameter* controls the strength of the complexity penalty. In practive, we usually work with parametric functions, and apply the regularizer to the parameters themselves. This yields the following form of the risk:

$$
  R_\lambda (\boldsymbol{\theta},\mathcal{S}) = R(\boldsymbol{\theta},\mathcal{S}) + \lambda C(\boldsymbol{\theta})
$$

Note that, if the loss function is logarithmic loss, and the regularizer is a negative log prior, the regularized risk is given by

$$
  R_\lambda (\boldsymbol{\theta}, \mathcal{S}) = -\frac{1}{N} \sum_{n=1}^N \ln[\Pr(\mathbf{y}_n | \mathbf{x}_n, \boldsymbol{\theta})] - \ln[\Pr(\boldsymbol{\theta})]
$$

Minimizing this is equivalent to a maximum aposteriori estimation.

## Structural risk

A natural way to estimate the hyperparameters is to minimize for the lowest achievable empirical risk, i.e.

$$
  \hat{\lambda} = \argmin_\lambda \min_{\boldsymbol{\theta}} R_\lambda (\boldsymbol{\theta}, \mathcal{S})
$$

This is an example of *bilevel optimization*, or *nested information*. Unfortunately, this technique will not work, since it always pick the least amount of regularization, i.e. $\hat{\lambda} = 0$. To see this, note that

$$
  \argmin_\lambda \min_{\mathbf{\theta}} R_\lambda (\boldsymbol{\theta},\mathcal{S}) = \argmin_\lambda \min_{\boldsymbol{\theta}} R(\boldsymbol{\theta},\mathcal{S}) + \lambda C(\lambda)
$$

which is minimized for $\lambda = 0$. The problem is that the empirical risk underestimates the population risk, resulting in overfitting when we choose $\lambda$. This is called *optimism of the training error*.

If we knew the regularized population risk $R_\lambda (\boldsymbol{\theta})$, instead of the regularized empirical risk $R_\lambda (\boldsymbol{\theta},\mathcal{S})$, we could use it to pick a model of the right complexity. This is know as *structural risk minimization*. There are two main ways to estimate the population risk for a given model: cross-validation and statistical learning theory.

## Cross-validation

One way of estimating the population risk for supervised models is to partition the dataset into two parts: one part used for training the model, and a second part, called the *validation/holdout set*, used for assessing the risk. We can fit the model in the training set, and use its performance on the validation set as an approximation to the population risk.

To explain this method, we write out the regularized empirical risk as

$$
  R_\lambda (\boldsymbol{\theta}, \mathcal{S}) = \frac{1}{|\mathcal{S}|} \sum_{(\mathbf{x},\mathbf{y})\in\mathcal{S}} \ell(\mathbf{y}, f(\mathbf{x},\boldsymbol{\theta})) + \lambda C(\boldsymbol{\theta}) 
$$

Let us also define $\hat{\boldsymbol{\theta}}_\lambda (\mathcal{S}) = \argmin_{\boldsymbol{\theta}} R_\lambda (\mathcal{S}, \boldsymbol{\theta})$. Finally, let $\mathcal{S} = \mathcal{S}_{\text{train}} \sqcup \mathcal{S}_{\text{valid}}$ be a partition of $\mathcal{S}$ into a training set and validation set.

For each model $\lambda$, we fit it to the training set to get $\hat{\boldsymbol{\theta}}_\lambda (\mathcal{S}_{\text{train}})$. We then use the unregularized empirical risk on the validation set as an estimate of the population risk, i.e.

$$
  R_\lambda^{\text{val}} := R_0 (\hat{\boldsymbol{\theta}}_\lambda (\mathcal{S}_{\text{train}}), \mathcal{S}_{\text{valid}})
$$

which is called the validation risk.

For small datasets, the number of training cases is insufficient for the model to learn the underlying patterns. The number of validation cases is also to small to estimate the performance of the model. One way to overcome lacking data is to use *cross-validation* (CV). This works by splitting the training data into $K$ folds. For each fold $k\in\set{1,\dots,K}$, we train on all the the folds but the $k$th, and test on the $k$th folds, in a round-robin fashion. Formally, we have the *cross-validation risk*

$$
  R_\lambda^{\text{cv}} := \frac{1}{K} \sum_{k=1}^K R_0 (\hat{\boldsymbol{\theta}}(\mathcal{S}_k^c))
$$

where $\mathcal{S}_k$ is the $k$th fold and $\mathcal{S}_k^c = \mathcal{S} \setminus \mathcal{S}_k$ are the remaining folds. For $K = N$, we get a method known as *leave-one-out cross-validation*, since we always train on $N - 1$ items and test on the remaining one.

We can use the CV estimate as an objective inside of an optimization routine to pick the optimal hyperparameter, $\hat{\lambda} = \argmin_\lambda R_\lambda^{\text{cv}}$. Finally we combine all the available data and re-estimate the model parameters using $\hat{\boldsymbol{\theta}} = \argmin_{\boldsymbol{\theta}} R_{\hat{\lambda}} (\boldsymbol{\theta}, \mathcal{S})$.
...
<MathBox title='Generalization bound (single hypothesis)' boxType="corollary" tag="2">
...
<MathBox title='Rademacher complexity' boxType="definition" tag="1">
...
<MathBox title='' boxType="theorem" tag="1">
Let $G$ be a collection of functions mapping from $Z$ to $[0,1]$. For any $\delta > 0$, with probability at least $1 - \delta$, each of the following holds for all $g\in G$:
...
<MathBox title="Massart's lemma" boxType="theorem" tag="2">
Let $A\subset\R^m$ be a finite set, with $r = \max_{\mathbf{x}\in A} \norm{\mathbf{x}}_2$, then the following holds
...
<MathBox title='Growth function generalization bounds' boxType="corollary" tag="2">
...
<MathBox title="Sauer's lemma" boxType="theorem" tag="3">
...
Now consider the following collections over $S' = \set{x_1,\dots,x_{m-1}}$. We define $G_1 = G_{|S'}$ as the set of concepts $H$ includes by restriction to $S'$. Next, by identifying each concept as the set of points (i $S$ or $S'$) for which it is non-zero, we can define $G_2$ as
...
<MathBox title='VC-dimension generalization bounds' boxType="corollary" tag="4">
...
$$
  R(h) \leq \hat{R}(h) + O\left(\sqrt{\frac{\ln(m/d)}{(m/d)}}\right)
$$
...
<MathBox title='Lower bound (realizable case)' boxType="theorem" tag="4">
...
<MathBox title='Lower bound (nonrealizable case)' boxType="theorem" tag="5">
...
$$
\begin{align*}
  &\underset{\substack{\boldsymbol{\sigma}\sim\mathcal{U}^m \\ S\sim\mathcal{D}_{\boldsymbol{\sigma}}}}{\mathbb{E}} \left(\frac{1}{\alpha}[R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_S) - R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_{\boldsymbol{D}_{\boldsymbol{\sigma}}}^*)] \right) \\
  &= \frac{1}{d} \sum_{\mathbf{x}\in X'} \underset{\substack{\boldsymbol{\sigma}\sim\mathcal{U}^m \\ S\sim\mathcal{D}_{\boldsymbol{\sigma}}}}{\mathbb{E}} \left(\mathbf{1}_{h_S (\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}}^* (\mathbf{x})} \right) \\
...
<MathBox title="Hoeffding's lemma" boxType="lemma" tag="4">
