...
A concept class $C$ is PAC-learnable if there exists an algorithm $\mathcal{A}$ and a polynomial function $p(\cdot,\cdot,\cdot,\cdot)$ such that for any $\varepsilon > 0$ and $\delta > 0$, for all distributions $D$ on $X$ and for any target concept $c\in C$, the following holds for any sample size $n \geq p\left(\frac{1}{\varepsilon}, \frac{1}{\delta}, m, s(c) \right)$
...
$$
\begin{align*}
  \hat{\mathcal{R}}_S (G) =& \mathbb{E}_{\boldsymbol{\sigma}} \left(\sup_{h\in H} \frac{1}{m} \sum_{i=1}^m \sigma_i 1_{h(x_i) \neq y} \right) \\
...
where we use the fact that $1_{h(x_i) \neq y} = (1 - y_i h(x_i))/2$ and the fact that for a fixed $y_i \in\set{\pm 1}$, then $\sigma_i$ and $-y_i \sigma$ are distributed in the same way.
</details>
</MathBox>

<MathBox title='Rademacher complexity bounds for binary classification' boxType='theorem'>
Let $H$ be a famliy of functions taking values in $\set{\pm 1}$ and let $\mathcal{D}$ be the distribution over the input space $X$. Then, for any $\delta > 0$, with probability at least $1 - \delta$ over a sample $S$ of size $m$ drawn according to $D$, each of the following holds for any $h\in H$
...
Since $\eqref{equation-17}$ holds for all $S\in\mathcal{S}$, it also holds in expectation over all $S\in\mathcal{S}$, i.e. $\mathbb{E}_{S\in\mathcal{S}} [\mathbb{E}_{f\sim U} (R_\mathcal{D} (h_S, f))] \geq 2\epsilon$. By Fubini's theorem, the expectations can be permuted, this

$$
  \mathbb{E}_{f\sim U}[\mathbb{E}_{S\in\mathcal{S}} (R_\mathcal{D} (h_S, f))] \geq 2\epsilon
$$
...
\begin{align*}
  ...
  \leq& \Pr_\mathcal{D} (X' - \set{\mathbf{x}_0}) \Pr_{S\in\mathcal{S}} [R_\mathcal{D}(h_S, f_0) \geq\epsilon] + \epsilon\Pr_{S\in\mathcal{S}} [R_\mathcal{D} (h_S, f_0) < \epsilon] \\
...
Clearly, the Bayes classifier $h_{\mathcal{D}_{\boldsymbol{\sigma}}}^*$ is defined by $h_{\mathcal{D}_{\boldsymbol{\sigma}}} (\mathbf{x}_i) = \argmax_{y\in\set{0,1}} \Pr(y|\mathbf{x}_i) = 1_{sigma_i \geq 0}$ for all $i\in [1,d]$. Also, $h_{\mathcal{D}_{\boldsymbol{\sigma}}}^*$ is in $H$ since $X'$ is fully shattered. For all $h\in H$

$$
\begin{equation}
\begin{split}
  R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h) - R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_{\mathcal{D}_{\boldsymbol{\sigma}}}^*) =& \frac{1}{d} \sum_{\mathbf{x}\in X'} \left(\frac{\alpha}{2} + \frac{\alpha}{2} \right) 1_{h(\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}}^*(\mathbf{x})} \\
  =& \frac{\alpha}{d} \sum_{x\in X} 1_{h(\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}}^*(\mathbf{x})} \\
...
\begin{align*}
  ...
  =& \frac{1}{d} \sum_{\mathbf{x}\in X'} \underset{\begin{subarray}{l} \boldsymbol{\sigma}\sim\mathcal{U}^m \\ S\sim\mathcal{D}_{\boldsymbol{\sigma}} \end{subarray}}{\mathbb{E}} \left(1_{h_S (\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}}^* (\mathbf{x})} \right) \\
  =& \frac{1}{d} \sum_{x\in X'} \underset{\boldsymbol{\sigma}\sim\mathcal{U}^m}{\mathbb{E}} \left(\underset{S\sim\mathcal{D}_{\boldsymbol{\sigma}}^m}{\Pr} \left[h_S (\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}} (\mathbf{x}) \right] \right) \\
  =& \frac{1}{d} \sum_{\mathbf{x}\in X'} \sum_{n=1}^m \underset{\boldsymbol{\sigma}\sim\mathcal{U}^m}{\mathbb{E}} \left(\underset{S\sim\mathcal{D}_{\boldsymbol{\sigma}}^m}{\Pr} \left[h_S (\mathbf{x}) \neq h_{\mathcal{D}_{\boldsymbol{\sigma}}}(\mathbf{x}) | |S|_\mathbf{x} = n \right] \Pr(|S|_\mathbf{x} = n) \right) \\
...
$$
  \underset{S\sim\mathcal{D}_{\boldsymbol{\sigma}}^m}{\Pr}\left(\frac{1}{\alpha}\left[R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_S) - R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_{\mathcal{D}_{\boldsymbol{\sigma}}^*}) \right] > \gamma u \right) > (1 - \gamma)u
$$

where $u = \Phi(m/d + 1, \alpha)$. Selecting $\delta$ and $\epsilon$ such that $\delta \leq (1 - \gamma)u$ and $\epsilon \leq \gamma\alpha u$ gives

$$
  \underset{S\sim\mathcal{D}_{\boldsymbol{\sigma}}^m}{\Pr}\left(\frac{1}{\alpha}\left[R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_S) - R_{\mathcal{D}_{\boldsymbol{\sigma}}} (h_{\mathcal{D}_{\boldsymbol{\sigma}}^*}) \right]  > \epsilon \right) > \delta
$$
...
$$
  \iemga = \left(\frac{7}{64}\right)^2 \ln\left(\frac{4}{3}\right) - \left(\frac{1}{64}\right)^2 \left(\ln\left(\frac{4}{3}) + 1 \right) \geq 1/320
$$
...
Using Chernoff boundin and Lemma $ref{lemma-4}$, we can write
