...
where $\boldsymbol{\theta} = (w_0, \mathbf{w}, \sigma^2)$ are all the parameters of the models. The vector of parameters $\mathbf{w}\in\R^d$ are known as the *weights* or *regression coefficients*. Each coefficient $w_i$ for $i=1,\dots,d$ specifies the change in the output we expect if we change the corresponing input feature $x_n$ by one unit. The term $w_0$ is the *offset* or *bias* term, and specifies the output value if all the inputs are $0$. This captures the unconditional mean of the response, $w_0 = \mathbb{E}(y)$, and acts as a baseline.
...
If $\mathbf{X}$ is tall and skinny, i.e. $N \gg D$, it can be quicker to use QR decomposition to solve the normal equation. For $\mathbf{X} = \mathbf{QR}$, where $\mathbf{Q}^\top \mathbf{Q} = \mathbf{I}$, we can write the system of linear equations $\mathbf{Xw} = \mathbf{y}$ as
...
Hence, the MAP estimate is given by the normal equation
...
If $\mathbf{R} = \mathbf{US}$ is an $N\times N$ matrix, we can show that

$$
  \hat{\mathbf{w}}_\text{MAP} = \mathbf{V}(\mathbf{R}^\top \mathbf{R} + \lambda \mathbf{I}_N)^{-1} \mathbf{R}^\top \mathbf{y}
$$

in other words, we can replace the $D$-dimensional vectors $\mathbf{x}_i$ with $N$-dimensional vectors $\mathbf{r}_i$, and perform our penalized fit as before. The time complexity for this algorithm is $\mathcal{O}(DN^2)$, which is less than $\mathcal{O}(D^3)$ if $D > N$.
...
$$
  p(\mathbf{w}|\lambda) = \prod_{d=1}^D \operatorname{Laplace}(w_d|0, 1/\lambda) \propto \prod_{d=1}^D e^{-\lambda|w_d|}
$$
...
where $\lVert\mathbf{w}\rVert = \sum_{d=1}^D |w_d|$ is the $\ell_1$ norm of $\mathbf{w}$. This method is called *LASSO regression*, which stands for "least absolute shrinkage and selection operator".
...
$$
\begin{align*}
  \frac{\partial}{\partial w_d} \operatorname{NLL}(\mathbf{w}) =& a_d w_d - c_d \\
  a_d =& \sum_{n=1}^N x_{nd}^2 \\
  c_d =& \sum_{n=1}^N x_{nd} (y_n - \mathbf{w}_{-d}^\top \mathbf{x}_{n,-d})
\end{align*}
$$

where $\mathbf{w}_{-d}$ is $\mathbf{w}$ without the $d$th component, and similarly $\mathbf{x}_{n,-d}$ is $\mathbf{x}_n$ withouth the $d$th component. We see that $c_d$ is proportional to the correlation between $d$th column of features $\mathbf{x}_{:,d}$ and the residual error obtained by predicting using all the other features, $\mathbf{r}_{-d} = \mathbf{y} - \mathbf{X}_{:,-d} \mathbf{w}_{-d}$. Hence the magnitude of $c_d$ is an indication of how relevant feature $d$ is for predictin $\mathbf{y}$, relative to the other features and the current parameters. Setting the gradient to $0$ gives the optimal update for $w_d$, keeping all other weights fixed

$$
  w_d = c_d / a_d = \frac{\mathbf{x}_{:,d}^\top \mathbf{r}_{-d}}{\lVert\mathbf{x}_{:,d}\rVert_2^2}
$$

The corresponding new prediction for $\mathbf{r}_{-d}$ becomes $\hat{\mathbf{r}}_{-d} = w_d \mathbf{x}_{:,d}$, which is the orthogonal projection of the residual onto the column vector $\mathbf{x}_{:,d}$.

Unfortunately, the $\lVert\mathbf{w}\rVert_1$ term is not differentiable whenever $w_d = 0$. However, we can compute a subgradient at this point

$$
\begin{align*}
  \frac{\partial}{\partial w_d} \operatorname{NLL}(\mathbf{w}) + \lambda\lVert\mathbf{w}\rVert_1 = (a_d w_d - c_d) + \lambda \partial_{w_d} \lVert\mathbf{w}\rVert_1 \\
  =& \begin{cases}
    a_d w_d - c_d - \lambda, \quad& w_d < 0 \\
    [-c_d - \lambda, -c_d + \lambda] \quad& w_d = 0 \\
    a_d w_d - c_d + \lambda \quad& w_d > 0
  \end{cases}
\end{align*}
$$

Depending on the value of $c_d$, the solutino to $\partial_{w_d} \operatorname{NLL}(\mathbf{w}) + \lambda\lVert\mathbf{w}\rVert_1 = 0$ can occur at $3$ different values of $w_d$

1. If $c_d < -\lambda$, the feature is strongly negatively correlated with the residual, such that the subgradient is zero at $\hat{w}_d = (c_d + \lambda)/a_d < 0$
2. If $c_d \in [-\lambda, \lambda]$, the feature is only weakly correlated with the residual, such that the subgradient is zero at $\hat{w}_d = 0$
3. If $c_d > \lambda$, the feature is strongly positively correlated with the residual, such that the subgradient is zero at $\hat{w}_d = (c_d - \lambda)/a_d > 0$

In summary, we have

$$
  \hat{w}_d (c_d) = \begin{cases}
    (c_d + \lambda)/a_d,\quad& c_d < -\lambda \\
    0, \quad& c_d \in [-\lambda, \lambda] \\
    (c_d - \lambda)/a_d,\quad& c_d > \lambda
  \end{cases}
$$
...
$$
  \hat{w}_d = \operatorname{SoftThreshold}\left(\frac{c_d}{a_d}, \frac{\lambda}{a_d} \right)
$$

where

$$
  \operatorname{SoftThreshold}(x,\delta) := \sign(x) (|x| - \delta)_+
$$

and $x_+ = \max(x, 0)$ is the positive part of $x$. This is called *soft thresholding*. In contrast, *hard thresholding* sets values of $w_d$ to $0$ if $-\lambda \leq c_d \leq \lambda$, but it does not shrink the values of $w_d$ outside this interval. The slope of the soft thresholding line does does not coincide with the diagonal, which means that even large coefficients are shrunk towards zero. Consequently, LASSO regression is a biased estimator.