...
Suppose $u \in C^{1,2}([0,T] \times \R^d, \R)$ has partial derivatives of at most polynomial growth, satisfying the partial differential equation
...
1. There exists a unique continuous function $u: [a,b]^d \to\R$ such that
$$
\begin{equation*}
...
Then
1. $\varphi:\R^d \to\R$ is twice continuously differentiable with at most polynomially growing derivatives
...
3. $U(\mathbf{x}) = u(T, \mathbf{x})$ for every $\mathbf{x}\in [a,b]^d$
