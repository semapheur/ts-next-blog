...
<MathBox title="Parallelization of fully connected feedforward neural networks" boxType="definition">
...
\begin{align*}
  ...
  \leq& \frac{1}{2} \left(\sum_{i=1}^n \right)^2
\end{align*}
$$
...
<MathBox title='Realization of parallelization of fully connected feedfoward neural networks of unequal length' boxType='definition'>
...
- the realization of $\mathbf{I}_j$ is the identity map, i.e.
$$
  (\mathbf{F}_a^\mathcal{N} (\mathbf{I}_j))(\mathbf{x}) = \mathbf{x}$ for $\mathbf{x}\in\R^{\mathcal{O}(\boldsymbol{\Phi}_j)}
$$
