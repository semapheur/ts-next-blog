...
$$
  \mathfrak{i}_i \cric \mathbf{A}_{\alpha,\beta} = ((\alpha, \beta), (1,0))
$$ 
...
<MathBox title="Rectified linear unit representation for linear interpolations" boxType="proposition" tag="proposition-15">
...
3. **Realization under ReLU:** Let $r:\R\to [0,\infty]$ be the rectified linear unit
$$
  r(x) := \max\set{x, 0}
$$

Then the realization $\mathbf{F}_\mathcal{N} (\boldsymbol{\Lambda}) \in C(\R)$ is equal to the linear interpolation operator $\mathrm{L}_{r_0,\dots,r_K}^{f_0,\dots,f_K} \in C(\R)$, i.e.

$$
  (\mathbf{F}_\mathcal{N} (\boldsymbol{\Lambda}))(x) := \mathrm{L}_{r_0,\dots,r_K}^{f_0,\dots,f_K} (x)
$$
...
---
- $\ref{proposition-15-2}$

From item $\ref{proposition-15-1}$ it follows that
