...
A fully-connected feedforward neural network (FFNN) is any element $\boldsymbol{\Phi}\in\mathcal{N}$ in the form of a finite sequence of weight-bias pairs

$$
  \boldsymbol{\Phi} = \left((\mathbf{W}_1, \mathbf{B}_1), \dots, (\mathbf{W}_L, \mathbf{B}_L) \right)
$$

where for each layer $n \in\set{1,\dots,L}$
- $\mathbf{W}_n \in\R^{\ell_n \times\ell_{n+1}}$ is the weight matrix
- $\mathbf{B}_n \in\R^{\ell_n}$ is the bias matrix
...
The structure of an FFNN $\boldsymbol{\Phi}\in\mathcal{N}$ is characterized by the following quantities:
...
- Number of hidden layers: $\mathcal{H}(\boldsymbol{\Phi}) = L - 1 \in\N_0$
- Total number of parameters: $\mathcal{P}(\boldsymbol{\Phi}) = \sum_{k=1}^L \ell_k (\ell_{k+1} + 1) \in\N_+$

Let $a:\R\to\R$ be an activation function. The realization of an FFNN $\boldsymbol{\Phi} \in\mathcal{N}$ with activation $a$ is the function $\mathcal{R}_a^\mathcal{N}: \R^{\mathcal{I}(\boldsymbol{\Phi})} \to \R^{\mathcal{O}(\boldsymbol{\Phi})}$ defined recursively as follows.

Given an input $\mathbf{x}_0 \in\R^{d_0 (\boldsymbol{\Phi})}$, define for $k = 1,\dots,\mathcal{L}(\boldsymbol{\Phi})$

$$
  \mathbf{x}_k = \begin{cases}
    \mathbf{M}_{a,d_k (\boldsymbol{\Phi})} (\mathcal{W}_k (\boldsymbol{\Phi}) \mathbf{x}_{k-1} + \mathcal{B}_k (\boldsymbol{\Phi}),\quad& k < \mathcal{L}(\boldsymbol{\Phi}) \\
    \mathcal{W}_k (\boldsymbol{\Phi}) \mathbf{x}_{k-1} + \mathcal{B}_k (\boldsymbol{\Phi}),\quad& k=\mathcal{L}(\boldsymbol{\Phi})
  \end{cases}
$$

where $\mathbf{M}_{a,d}$ is the componentwise extension of $a$ to $\R^d$. Finally set

$$
  (\mathcal{R}_a^{\mathcal{N}} (\boldsymbol{\Phi}))(\mathbf{x}_0) := \mathbf{x}_{\mathcal{L}(\boldsymbol{\Phi})}
$$
</MathBox>
...
$$
  \mathcal{R}_a^{\mathcal{N}} (\boldsymbol{\Phi}) = \begin{cases}
...
Then $(\mathcal{R}_a^\mathcal{N} (\boldsymbol{\Phi}))(\mathbf{x}_0) = \mathbf{x}_L$. Equivalently, for $L > 1$
...
The space of all possible feedforward convolutional network architectures is defined by

$$
  C = \bigcup_{T, L \in\N_+} \bigcup_{l_1,\dots,l_L \in\N_+} \bigcup_{(c_{k,t})_{(k,t) \in [L]\times [T]} \subseteq\N} \left(\prod_{k=1}^L ((\R^{c_{k,1} \times\cdots\times c_{k,T}})^{l_k \times l_{k-1}} \times \R^{l_k}) \right)
$$

where
- $T$ is the tensor dimension
- $L$ is the number of layers
- $l_k$ is the number of channels at layer $k$
- $c_{k,t}$ is the kernel size along dimension $t$ in layer $k$

Each layer $k$ is characterized by convolutional weights $W_{k,n,m} \in \R^{c_{k,1} \times\cdots\times c_{k,T}}$ and biases $B_{k,n} \in\R$. A function $\Phi$ is a feedforward convolutional neural network if and only if $\Phi\in C$.

## Realization of feedforward convolutional networks

<MathBox title='Identity tensor' boxType='defintion'>
...
- $\Phi = (((W_{k,n,m})_{(n,m) \in [l_k ]\times [l_{k-1}]}, (B_{k,n})_{n\in [l_k]}))_{k\in [L]} \in \prod_{k=1}^L ((\R^{c_{k,1} \times\cdots\times c_{k,T}})^{l_k \times l_{k-1}} \times \R^{l_k}) \subseteq C$

and let $a: \R\to\R$ denote an activation function. Define the realization map

$$
  \mathcal{R}_a^C (\Phi): \left(\bigcup_{\substack{d_1,\dots,d_T \in\N \\ \forall t\in [T]: d_t - \sum_{k=1}^L (c_{k,1} - 1) \geq 1}} (\R^{d_1 \times\cdots\times d_T})^{l_0} \right) \to \left(\bigcup_{d_1,\dots,d_T \in\N_+} (\R^{d_1 \times\cdots\times d_T})^{l_L} \right)
$$

recursively as follows. Let $x_0 = (x_{0,1}, \dots, x_{0, l_0})$ denote the input feature maps, and for each $k \in [L]$ and $t \in [T]$ define

$$
  \mathfrak{d}_{k,t} = \mathfrak{d}_{k-1, t} - c_{k,t} + 1
$$

so that the spatial dimensions shrink consistently under valid convolution. Then for each $k \in [L]$ and $n\in [l_k]$

$$
  x_{k,n} = \mathfrak{M}_{\psi_k, \mathfrak{d}_{k,1},\dots,\mathfrak{d}_{k,T}} (B_{k,n} \mathbf{I}^{\mathfrak{d}_{k,1}, \cdots,\mathfrak{d}_{k,T}} + \sum_{m=1}^{l_{k-1}} x_{k-1, m} * W_{k,n,m})
$$
...
and $\mathfrak{M}_{\psi_k, \mathfrak{d}_{k,1},\dots,\mathfrak{d}_{k,T}}$ denoted the componentwise application of $\psi_k$ to a tensor of the given dimension. Defining $(\mathcal{R}_a^C (\Phi))(x_0) = x_L$, the function $\mathcal{R}_a^C (\phi)$ is the realization function of the feedforward convolutional neural network $\Phi$ with activation function $a$.
