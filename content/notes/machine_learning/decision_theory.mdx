...
$$
  D_{\text{KL}} (p \lVert q) := \sum_{y\in Y} p(y) \ln\frac{p(y)}{q(y)}
$$
...
$$
\begin{align*}
  D_{KL} (p \lVert q) =& \sum_{y\in Y} p(y) \ln[p(y)] - \sum_{y\in Y} p(y) \ln[q(y)] \\
