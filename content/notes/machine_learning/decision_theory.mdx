...
$$
\begin{equation}
  \pi^* (\mathbf{x}) = \argmin_{a\in A} \mathbb{E}_{p(h|\mathbf{x})} [\ell(h,a)] \tag{\label{equation-1}}
\end{equation}
$$
...
$$
  \pi^* (\mathbf{x}) = \argmax_{a\in A} \mathrm{E}_h [U(h,a)]
$$
...
\begin{array}{cc|c}
  ...
  \text{Truth} & \begin{array}{c} 0 \\ 1 \end{array} & \begin{array}{cc}
    \frac{\operatorname{TN}}{N} = \operatorname{TNR} & \frac{\operatorname{FP}}{N} = \operatorname{FPR} \\
    \frac{\operatorname{FN}}{P} = \operatorname{FNR} & \frac{\operatorname{TP}}{P} = \operatorname{TPR}
  \end{array}
\end{array}
$$
</TableFigure>
...
$$
\begin{align*}
  \frac{1}{F_1} = \frac{1}{2}\left(\mathbf{1}{\mathcal{P}} + \frac{1}{\mathcal{R}}\right) \\
...
The optimal estimate is the *posterior median*, i.e. value such that $p(h < a|\mathbf{x}) = p(h\geq a | \mathbf{x}) = 0.5$
...
If we use $0$-$1$ loss, we can think of the hypothesis test as a binary classification problem, where $H\in\set{0,1}$ represents the identity of the true model. Given a sample $\mathcal{S}$, the optimal decision is to pick the alternative hypothesis if and only if $p(M_1|\mathcal{S}) > p(M_0|\mathcal{S})$, or equivalently, if $p(M_1 | \mathcal{S}) / p(M_0 | \mathcal{S}) > 1$. If we use a uniform prior, $p(M_0) = p(M_1) = 0.5$, the optimal decision rule is to accept $M_1$ if and only if $p(\mathcal{S}|M_1)/p(\mathbb{S}|M_0) > 1$. This is called a likelihood ratio test.

If the parameters for the null and alternative hypotheses are fully specified, the hypothesis test is called *simple*. A hypothesis that does specify all its parameters is known as a *compound hypothesis*. In Bayesian hypothesis testing, we compare compound hypotheses by integrating out their unknown parameters. This allows us to compare models of different complexity, due to the Bayesian Occam's razor effect. In this case the ratio of marginal likelihoods is known as the *Bayes factor*

$$
  \operatorname{BF}(1,0) := \frac{p(\mathcal{S}|M_0)}{p(\mathcal{S}|M_1)}
$$

If $\operatorname{BF}(1,0) > 1$, then we prefer model $1$, otherwise we prefer model $0$. An alternative approach is to maximize out the parameters, giving the maximum likelihood ratio

$$
  \frac{p(M_1|\mathcal{S})}{p(M_0|\mathcal{S})} = \frac{\int_{\boldsymbol{\theta\in M_1}} p(\boldsymbol{\theta}) p_{\boldsymbol{\theta}} (\mathcal{S})}{\int_{\boldsymbol{\theta}\in M_0} p(\boldsymbol{\theta}) p_{\boldsymbol{\theta}} (\mathcal{S})} \approx \frac{\max_{\boldsymbol{\theta}\in M_1} p_{\boldsymbol{\theta}}(\mathcal{S})}{\max_{\boldsymbol{\theta}\in M_0} p_{\boldsymbol{\theta}}(\mathcal{S})}
$$

<TableFigure caption="Jeffrey scale of evidence for interpreting Bayes factor">
| Bayes factor $\operatorname{BF}(1,0)$ | Intepretation |
| :-: | --- |
| $\operatorname{BF} < 1/100$ | Decisive evidence for $M_0$ |
| $\operatorname{BF} < 1/10$ | Strong evidence for $M_0$ |
| $1/10 < \operatorname{BF} < 1/3$ | Moderate evidence for $M_0$ |
| $1/3 < \operatorname{BF} < 1$ | Weak evidence for $M_0$ |
| $1 < \operatorname{BF} < 3$ | Weak evidence for $M_1$ |
| $3 < \operatorname{BF} < 10$ | Moderate evidence for $M_1$ |
| $\operatorname{BF} > 10$ | Strong evidence for $M_1$ |
| $\operatorname{BF} > 100$ | Decisive evidence for $M_1$ |
</TableFigure>

<MathBox title='Testing if a coin is fair' boxType="example">
Let $\theta \in [0,1]$ represent the bias of a coin and $\mathcal{S}$ denote the outcome of $N$ coin tosses. To test if a coin is fair with $\theta = 0.5$, we can formulate two hypothesis. The null hypothesis $M_0$ for $\theta = 0.5$ has marginal likelihood

$$
  p(\mathcal{S}|M_0) = \left(\frac{1}{2}\right)^N
$$

Using a Beta prior, the marginal likelihood for the alternative hypothesis $M_1$ with $\theta\neq 0.5$ is

$$
  p(\mathcal{S}|M_1) = \int p(\mathcal{S}|\theta)p(\theta)\;\d\theta = \frac{\operatorname{Beta}(\alpha_1 + N, \alpha_0 + N_0)}{\operatorname{Beta}(\alpha_1, \alpha_0)}
$$

where $N_1$ and $N_0$ are the number of heads and tails, respectively. 

If $N_1 / N \approx 0.5$, the unbiased coiin hypothesis $M_0$ is more likely than $M_1$, since $M_0$ is a simpler model with no free parameters. However as the head counts become more extreme, we favor the biased coin hypothesis.
</MathBox>

<MathBox title='Comparing Gaussian means' boxType="example">
Suppose we are interested in testing whether some data comes from a Gaussian with mean $\mu_0$ or $\mu_1$, where we assume a known shared variance $\sigma^2$. Given observed data $\mathcal{S} = \set{x_n}_{n=1}^N$, the likelihood ratio is given by 

$$
\begin{align*}
  \frac{p[\mathcal{S}(M_1)]}{p[\mathcal{S}(M_0)]} =& \frac{\exp\left(-\frac{1}{2\sigma^2} \sum_{n=1}^N (x_n - \mu_1)^2 \right)}{-\frac{1}{2\sigma^2} \sum_{n=1}^N (x_n - \mu_0)^2} \\ \\
  =& \exp\left(\frac{1}{2\sigma^2} (2N\bar{x}(\mu_1 - \mu_0) + N\mu_0^2 - N\mu_1^2) \right)
\end{align*}
$$

We see that this ratio only depend on the observed data via its mean $\bar{x}$. In general $p(\mathcal{S}|M_1) > p(\mathcal{S}|M_0) > 1$ if and only if $\bar{x} < x^*$, where $x^*$ is the point where the two probability density functions interact.

The type I error rate $\alpha$ for false positive is given by

$$
\begin{align*}
  \alpha(\mu_0) =& p(\text{type I error}) = p(\text{reject $H_0$}|\text{$H_0$ is true}) \\
  =& p(\bar{X}(\tilde{\mathcal{S}}) > x^* | \tilde{\mathcal{S}}\sim H_0) \\
  =& p\left(\frac{\bar{X} - \mu_0}{\sigma/\sqrt{N}} > \frac{x^* - \mu_0}{\sigma/\sqrt{N}} \right)
\end{align*}
$$

Hence $x^* = z_\alpha \sigma/\sqrt{N} + \mu_0$, where $z_\alpha$ is the upper $\alpha$ quantile of the standard normal distribution. The type II error rate $\beta$ for false negatives is given by

$$
\begin{align*}
  \beta(\mu_1) =& p(\text{type II error}) = p(\text{accept $M_0$}|\text{$M_1$ is true}) \\
  =& p(\bar{X}(\tilde{\mathcal{S}}) < x^* | \tilde{\mathcal{S}}\sim M_1)
\end{align*}
$$

The power of the hypothesis test given by $1 - \beta(\mu_1)$ is the probability that we reject $M_0$ given that $M_1$ is true, also known as the true positive rate. Clearly, the least power occurs if $\mu_1 = \mu_0$ giving $1 - \beta(\mu_1) = \alpha(\mu_0)$. As $\mu_1$ and $\mu_0$ becomes further apart, the power approaches $1$.
</MathBox>

### Null hypothesis significance testing (NHST)

Another way of testing a hypothesis is to compare it to some test statistic in a method known as null hypothesis significance testing (NHST). In this method, we define a test statistic $t(\mathcal{S})$ of an unknown distribution $T$ and compare its observed value to value we would expect if the null hypothesis $M_0$ were true. If the observed value is unexpected given $M_0$, we reject the null hypothesis. This is quantified by the $P$-value, which is defined as the smallest significance level $\alpha$ (type I error for false positives) for which the null hypothesis $M_0$ would be rejected, given the observed value of the test statistic.

Depending on the test statistic distribution $T$, the $P$-value is given by:
- $P = p(T \geq t | M_0)$ for a one-sided right-tail test statistic distribution
- $P = p(T \leq t | M_0)$ for a one-sided left-tail test statistic distribution
- $P = 2\min\Set{p(T \geq t : M_0), p(T \leq t : M_0)}$ for a two-sided test-statistic distribution. If the distribution of $T$ is symmetric about zero, then $P = p(|T| \geq |t| : M_0)$
...
- the null hypothesis $H_0 : \Delta\in R$, which says both methods are practically the same (which is a more realistic assumption than $H_0: \Delta = 0$)
...
$$
  \operatorname{MSE}(\hat{\theta}|\theta^*) = \operatorname{var}(\hat{\theta}) + \operatorname{bias}^2 (\hat{\theta})
$$
...
Let $\mathbf{X} = (X_n)_{n=1}^N$ with $X_n \sim\mathcal{N}(\theta_n, 1)$ be independent Gaussian variables with unit variance. Assuming a quadratic loss function $\ell_2 (\hat{\boldsymbol{\theta}} - \boldsymbol{\theta}) = \norm{\hat{\boldsymbol{\theta}} - \boldsymbol{\theta}}^2$, the corresponding risk function $R(\hat{\boldsymbol{\theta}}, \boldsymbol{\theta}) = \mathbb{E}[\ell(\hat{\boldsymbol{\theta}}, \boldsymbol{\theta})]$ is the mean squared error. It can be shown that the unbiased ordinary estimator $\hat{\boldsymbol{\theta}}_0 (\mathbf{X}) = \mathbf{X}$ with $R(\hat{\boldsymbol{\theta}}_0, \boldsymbol{\theta}) = N$ is admissible for squared error loss when $N = \Set{1,2}$. However, for $N \geq 3$, the James-Stein estimator 

$$
  \hat{\boldsymbol{\theta}}_{\text{JS}}(\mathbf{X}) = \left(1 - \frac{N - 2}{\norm{\mathbf{X}}^2} \right)\mathbf{X}
$$
...
$$
\begin{align*}
  R(\hat{\boldsymbol{\theta}}_{\text{JS}}, \boldsymbol{\theta}) =& \mathbb{E}\left(\Norm{ \left(1 - \frac{N - 2}{\norm{\mathbf{X}}^2} \right)\mathbf{X} - \boldsymbol{\theta} }^2 \right) \\
  =& \mathbb{E}\left(\Norm{ \mathbf{X} - \boldsymbol{\theta} - \frac{(N - 2)\mathbf{X}}{\norm{\mathbf{X}}^2} }^2 \right) \\
  =& \mathbb{E}(\norm{\mathbf{X} - \boldsymbol{\theta}}^2) + \mathbb{E}\left(\Norm{ \frac{(N - 2)\mathbf{X}}{\norm{\mathbf{X}}^2} }^2 \right) \\
  &- \mathbb{E}\left(2(N - 2) \sum_{n=1}^N \frac{(X_n - \theta_n)X_n}{\norm{\mathbf{X}}^2} \right) \\
  =& N + (N - 2)^2 \mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2}\right) - 2(N - 2) \sum_{n=1}^N \mathbb{E}\left(\frac{(X_n - \theta_n)X_n}{\norm{\mathbf{X}}^2}\right)
\end{align*}
$$
...
$$
\begin{align*}
  \mathbb{E}\left(\frac{(X_1 - \theta_1)X_1}{\norm{\mathbf{X}}^2} \right) =& \int_{-\infty}^\infty \cdots \int_{-\infty}^\infty \frac{(x_1 - \theta_1)x_1}{\norm{\mathbf{x}}^2} \frac{1}{(2\pi)^{N/2}}e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2} \;\d x_1\cdots\d x_N
\end{align*}
$$
...
$$
  \frac{\partial}{\partial x_1} \frac{1}{(2\pi)^{N/2}}e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2} = -\frac{x_1 -\theta_1}{(2\pi)^{N/2}}e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2}
$$
...
$$
\begin{align*}
  \mathbb{E}\left(\frac{(X_1 - \theta_1)X_1}{\norm{\mathbf{X}}^2} \right) =& \frac{1}{(2\pi)^{N/2}} \int_{-\infty}^\infty \cdots \bigg(\overbrace{\left[-\frac{x_1}{\norm{\mathbf{x}}^2} e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2} \right]_{-\infty}^\infty}^{=0} \\
  &+ \left. \int_{-\infty}^\infty e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2} \d\left(\frac{x_1}{\norm{\mathbf{x}}^2}\right) \right) \cdots\d x_N \\
  =& \int_{-\infty}^\infty \cdots \int_{-\infty}^\infty \frac{\norm{\mathbf{x}}^2 - 2x_1^2}{\norm{\mathbf{x}}^4} \frac{1}{(2\pi)^{N/2}} e^{-\norm{\mathbf{x} - \boldsymbol{\theta}}^2 / 2} \;\d x_1 \cdots\d x_N \\
  =& \mathbb{E}\left(\frac{\norm{\mathbf{X}}^2 - 2X_1^2}{\norm{\mathbf{X}}^4}\right)
\end{align*}
$$
...
$$
\frac{\d}{\d x_1} \left(\frac{x_1}{\norm{\mathbf{x}}^2}\right) = \frac{\norm{\mathbf{x}}^2 - 2x_1^2}{\norm{\mathbf{x}}^4}
$$  
...
$$
\begin{align*}
  \sum_{n=1}^N \mathbb{E}\left(\frac{(X_n - \theta_n)X_n}{\norm{\mathbf{X}}^2}\right) =& \mathbb{E}\left(\sum_{n=1}^N \frac{\norm{\mathbf{X}}^2 - 2X_i^2}{\norm{\mathbf{X}}^4} \right) \\
  =& \mathbb{E}\left(\frac{N\norm{\mathbf{X}}^2 - 2\norm{\mathbf{X}}^2}{\norm{\mathbf{X}}^4}\right) \\
  =& (N - 2)\mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2} \right)
\end{align*}
$$
...
$$
\begin{align*}
  R(\hat{\boldsymbol{\theta}}_{\text{JS}}, \boldsymbol{\theta}) =& N + (N - 2)^2 \mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2} \right) - 2(N - 2)^2 \mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2} \right) \\
  =& N - (N - 2)^2 \mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2} \right)
\end{align*}
$$
...
$$
  R(\hat{\boldsymbol{\theta}}_{\text{JS}}, \boldsymbol{\theta}) = N - (N - 2)\mathbb{E}\left(\frac{1}{\norm{\mathbf{X}}^2} \right) < N = R(\hat{\boldsymbol{\theta}}_0, \boldsymbol{\theta})
$$
