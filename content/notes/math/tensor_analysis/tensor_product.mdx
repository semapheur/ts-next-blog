...
\begin{align*}
  \varepsilon_{a_1 a_2\dots a_n} &:= \begin{cases}
    ...
    0 &\textrm{ otherwise}
  \end{cases} \\
  &= \prod_{1\leq i < j \leq n} \operatorname{sgn}(a_j - a_i)
\end{align*}
$$
</MathBox>
...
Let $U$, $V$ and $W$ be vector spaces over $\mathbb{F}$. A function $\mathrm{F}:U\times V\to W$, where $U\times V$ is the cartesian product of sets, is bilinear if it is linear in both variables separately

$$
\begin{align*}
  \mathrm{F}(\alpha\mathbf{u}_1 + \beta\mathbf{u}_2, \mathbf{v}) =& \alpha \mathrm{F}(\mathbf{u}_1,\mathbf{v}) + \beta \mathrm{F}(\mathbf{u}_2, \mathbf{v}) \\
  \mathrm{F}(\mathbf{u}, \alpha\mathbf{v}_1 + \beta\mathbf{v_2}) =& \alpha \mathrm{F}(\mathbf{u},\mathbf{v}_1) + \beta \mathrm{F}(\mathbf{u}, \mathbf{v}_2) 
\end{align*}
$$

The set of all bilinear functions from $U\times V$ to $W$ is denoted $\hom_{\mathbb{F}}(U\times V, W)$. A bilinear function $\mathrm{F}:U\times V\to\mathbb{F}$ taking values in the base field $\mathbb{F}$ is called a *bilinear form* on $U\times V$.
</MathBox>

Note the difference between bilinear functions $\hom_{\mathbb{F}}(V\times V;W)$, mapping from Cartesian products of vector spaces, and linear functions $\mathcal{L}(V\times V,W)$, mapping from direct products of vector spaces. Unlike direct products, Cartesian products of sets are not associated with any algebraic structure.

In category theory, the tensor product can be defined as a universal mapping for bilinearity. Let $U$ and $V$ be vector spaces over $\mathbb{F}$, and consider the functor

$$
  \mathbf{Bilin}(U,V;-): \mathbf{Vect}_{\mathbb{F}}\to\mathbf{Set} \\
  W \mapsto \hom_{\mathbb{F}}(U\times V, W)
$$

mapping a vector space $W$ to the set of bilinear maps $U\times V\to W$. It can be shown that this functor is representable, meaning there exists a vector space $T$ such that $\mathbf{Bilin}(U,V;W)\cong\mathbf{Vect}_{\mathbb{F}}(T,W) = \mathcal{L}(T,W)$ naturally in $W$. 

In other words, there exists a universal pair $(T, \mathrm{B}:U\times V \to T)$ of a vector space $T$ and a bilinear map $\mathrm{B}$, such that for any pair $(W, \mathrm{F}:U\times V \to W)$ of a vector space $W$ and a bilinear map $F$ there is a unique linear map $\mathrm{L}:T\to W$ with $\mathrm{F} = \mathrm{L} \circ \mathrm{B}$ according to the diagram below.

<TikzFigure caption='Universal property for bilinearity'>
{String.raw`\begin{tikzcd}[
  ...
  U\times V \arrow[r, "\mathrm{B}"] \arrow[dr, "\forall \mathrm{F}" swap] & 
  T \arrow[d, "\exists!\mathrm{L}", dashed] \\
  & \forall W
\end{tikzcd}`}
</TikzFigure>

This property, called the universal property, uniquely determines $(T, \mathrm{B})$ up to isomorphism. The vector space $T$ is called the tensor product of $U$ and $V$, denoted $U \otimes V$ and the bilinear map $\mathrm{B}$ is written
...
<MathBox title='Tensor product of vector spaces' boxType="definition">
The tensor product of two vector spaces $U$ and $V$ is a vector space denoted $U\otimes V$ together with a bilinear map $\otimes:U\times V\to U\otimes V$ given by $(\mathbf{u},\mathbf{v})\mapsto \mathbf{u}\otimes \mathbf{v}$ satisfying the universal property. That is, for every for every bilinear map $\mathrm{F}:U\times V\to W$ there is a unique linear map $\mathrm{L}:U\otimes V\to W$ such that $\mathrm{F} = \mathrm{L} \circ\otimes$, i.e. $\mathrm{F}(\mathbf{u},\mathbf{v}) = \mathrm{L}(\mathbf{u}\otimes\mathbf{v})$ for every $\mathbf{u}\in U$ and $\mathbf{v}\in V$.

<TikzFigure caption='Universal property for the tensor product of vector spaces'>
{String.raw`\begin{tikzcd}[
  every label/.append style={text=black},
  every arrow/.append style={draw=black},
  every cell/.append style={text=white}
]
  U\times V \arrow[r, "\otimes"] \arrow[dr, "\forall \mathrm{F}" swap] & 
...
Let $U$ and $V$ be vector spaces with respective bases $B = \Set{\mathbf{b}_j }_{j\in J}$ and $C = \Set{\mathbf{c}_j }_{j\in J}$ for an index set $J\subseteq\N$. The tensor product from $U\otimes V$ can be constructed as vector space $U\otimes V$ with basis $D = \Set{ \mathbf{b}_i \otimes \mathbf{c}_j | \mathbf{b}_i \in B, \mathbf{c}_j \in C }$ where $\otimes:U\times V \to U\otimes V$ maps $(\mathbf{b}_i, \mathbf{c}_j)\mapsto \mathbf{b}_i \otimes\mathbf{c}_i$.

For any bilinear function $\mathrm{F}:U\times V \to W$, then the universal condition $\mathrm{F} = \mathrm{L}\circ\otimes$ is equivalent to

$$
  \mathrm{L}(\mathbf{b}_i \otimes \mathbf{c}_j) = \mathrm{F}(\mathbf{b}_i, \mathbf{c}_i)
$$

which uniquely defines a linear map $\mathrm{L}: U\otimes V\to W$. Hence, $(U\otimes V, \otimes)$ has the universal property for bilinearity.

If $\mathbf{u} = \sum_{i\in J}^n \alpha_i \mathbf{b}_i \in U$ and $\mathbf{v} = \sum_{j\in J} \beta\mathbf{c}_i \in V$ for $\alpha_i, \beta_j\in \mathbb{F}$ then the elements of $U\otimes V$, called tensors take the form
...
where $\alpha, \beta\in\mathbb{F}$. The quotient space $U\otimes V = \mathbb{F}_{U\times V}/S$ is also called the *tensor product* of $U$ and $V$. The elements of the tensor product have the form
...
<TikzFigure caption='Tensor product diagram'>
{String.raw`\begin{tikzcd}[
  every label/.append style={text=black},
  every arrow/.append style={draw=black},
  every cell/.append style={text=white}
...
Since $\otimes(\mathbf{u},\mathbf{v}) = \mathbf{u}\times\mathbf{v} = \mathrm{R}\circ \mathrm{G}(\mathbf{u}, \mathbf{v})$ it follows that $\otimes = \mathrm{R}\circ\mathrm{G}$. The universal property of vector spaces implies that there is a unique linear transformation $\mathrm{S}: L_{U\times V}\to W$ for which $\mathrm{S}\circ\mathrm{G} = \mathrm{F}$.
...
and similarly for the second coordinate. Hence by the universal property for linear transformations, there is a unique linear transformation $\mathrm{T}:U\otimes V\to W$ such that $\mathrm{T}\circ \mathrm{R} = \mathrm{S}$. Thus

$$
  \mathrm{T}\circ \otimes = \mathrm{T} \circ \mathrm{R}\circ h = \mathrm{S}\circ h \circ f
$$

As to uniqueness, if $\mathrm{T}' \circ \otimes = \mathrm{F}$, then $\mathrm{S} = \mathrm{T}' \circ\mathrm{R}$ satisfies

$$
  \mathrm{S}' \circ \mathrm{G} = \mathrm{T}' \circ \mathrm{R} = \mathrm{T}' \circ \otimes = \mathrm{F}
$$
...
To show that $D$ is linearly independent, suppose
...
where only a finite number of the scalars $\rho_{i,j}\in\mathbb{F}$ are nonzero. In fact, for a fixed $\mathbf{w}\in U\otimes V$, we can reindex the bases so that
...
Suppose $X = \Set{\mathbf{x}_i}_{i\in I}$ and $Y = \Set{\mathbf{y}_j}{j\in J}$ also are bases for $U$ and $V$, respectively and that
...
$$
  U_n = \operatorname{span}\Set{\mathbf{b}_i}_{i=1}^n
$$
...
Repeating the same process for $V$, where we reindex $C$ so that the subspace $V_m = \operatorname{span}\Set{\mathbf{v}_j}_{j=1}^m$ contains $\mathbf{y}_1,\dots,\mathbf{y}_d$ and extend to a basis $Y' = \Set{\mathbf{y}_1,\dots,\mathbf{y}_d,\mathbf{y}_{d+1},\dots,\mathbf{y}_m}$ for $V_m$. Then
...
$$
  \operatorname{rank}(\mathbf{R}) = \operatorname{rank}(\mathbf{R}_1) = \operatorname{rank}(\mathbf{S}_1) = \operatorname{rank}(\mathbf{S})
$$
...
where $\operatorname{rank}(\mathbf{A}_{c,d}^T)\geq\operatorname{rank}(\mathbf{R})$ and $\operatorname{rank}(\mathbf{B})\geq\operatorname{rank}(\mathbf{R})$.
...
The vectors $\Set{\mathbf{b}_i + \rho_i \mathbf{b}_n | 1\leq i \leq n-1}$ are linearly independent. This reduction can be repeated until the second coordinates are linearly independent. Also, the identity matrix $\mathbf{I}_n$ is a coordinate matrix for $\mathbf{w}$ giving $n = \operatorname{rank}(\mathbf{I}_n) = \operatorname{rank}(\mathbf{w})$.
</details>
</MathBox>
...
If $X$ an $Y$ are finite-dimensional vector spaces let $X\xhookrightarrow{\sim} Y$ denote that $X$ and $Y$ are isomorphic through an embedding. Consider the following special cases of the linear map for finite-dimensional vector spaces $U$ and $V$ over $\mathbb{F}$
...
$$
  U^* \otimes\mathcal{L}(V,V') \xhookrightarrow{\sim} \mathcal{L}(U\otimes V, V')
$$
...
$$
  U^* \otimes V^* \xhookrightarrow{\sim} (U\otimes V)^*
$$
...
$$
  \mathcal{L}(U,U') \otimes W \xhookrightarrow{\sim} \mathcal{L}(U, U'\otimes W)
$$
...
$$
  U^* \otimes W \xhookrightarrow{\sim} \mathcal{L}(U,W)
$$
...
$$
\begin{gather*}
  \bigotimes_{i=1}^n U_i^* \xhookrightarrow{\sim} \left(\bigotimes_{i=1}^n U_i \right)^* \\
...
$$
\begin{gather*}
  \left(\bigotimes_{i=1}^n U_i^* \right)\otimes V \xhookrightarrow{\sim} \mathcal{L}\left(\bigotimes_{i=1}^n U_i, V \right)^* \\
...
The special case $n=q$ gives a linear functional on $V^{\otimes q}$, i.e. each element of $(V^*)^{\otimes q}$ is a distinct member of $(V^{\otimes q})^*$, producing the embedding $(V^*)^{\otimes q} \xhookrightarrow{\sim} (V^{\otimes q})^*$.
...
$$
  \operatorname{sgn}(\pi) = \begin{cases}
    1,& \quad \pi\text{ has even parity} \\
    -1,& \quad \pi\text{ has odd parity}
  \end{cases}
$$

If $\operatorname{sgn}(\pi) = 1$, then $\pi$ is an *even permutation* and if $\operatorname{sgn}(\pi) = -1$, then $\pi$ is an *odd permutation*. The sign of $\pi$ is often written $(-1)^\pi$.
</MathBox>
...
$$
  F_\sigma (x_1,\dots,x_p) = x_{\sigma_1} \otimes\cdots\otimes x_{\sigma_p}
$$

determines a unique linear operator $\lambda_\sigma$ on $T^p (V)$ for which

$$
  \lambda_\sigma (x_1 \otimes\cdots\otimes x_p) = x_{\sigma_1} \otimes\cdots\otimes x_{\sigma_p}
$$
...
Thus, for an (anti)symmetric tensor
...
In either case, the monomials $\tau u_M$ are linearly independent for distinct (multi)sets $M$. Therefore, if $\tau\mathbf{v} = 0$ then $\alpha_M |G_M| = 0$ for all (multi)set $M$. Hence, if $\mathrm{char}(\mathbb{F}) = 0$, then $\alpha_M = 0$ and so $\mathbf{v} = 0$. This shows that the restricted maps $\tau|_{ST^p (V)}$ and $\tau|_{AT^p (V)}$ are isomorphisms.

The symmetric and antisymmetric tensor products can be realized as subspaces of the ordinary tensor product. Define in $\otimes^n V$ the linear maps $P_s$ and $P_a$ by linear extension of
...
- Positive definite: $g(\mathbf{v}, \mathbf{v}) = \norm{ v}^2 \geq 0 $
...
$$
\begin{align*}
  \frac{\d^2\mathbf{R}}{\d t^2} &= \frac{\d}{\d t} \left( \frac{\d\mathbf{R}}{\d t} \right) \\
  &= \frac{\d}{\d t} \left( \frac{\d x^i}{\d t} \frac{\partial \mathbf{R}}{\partial x^i} \right) \\
  &= \frac{\d^2x^i}{\d t^2} \frac{\partial \mathbf{R}}{\partial x^i} + \frac{\d x^i}{\d t} \left( \frac{\d}{\d t} \frac{\partial \mathbf{R}}{\partial x^i} \right) \\
  &= \frac{\d^2x^i}{\d t^2} \frac{\partial \mathbf{R}}{\partial x^i} + \frac{\d x^i}{\d t} \left[ \left( \frac{\d x^j}{\d t} \frac{\partial}{\partial x^j} \right) \frac{\partial \mathbf{R}}{\partial x^i} \right] \\
  &= \underbrace{\frac{\d^2x^i}{\d t^2} \frac{\partial \mathbf{R}}{\partial x^i}}_{\mathrm{tangential}} + \frac{\d x^i}{\d t} \frac{\d x^j}{\d t} \frac{\partial^2 \mathbf{R}}{\partial x^i \partial x^j}
\end{align*}
$$
...
$$
  \frac{\d^2\mathbf{R}}{\d t^2} = \underbrace{\left( \frac{\d^2x^k}{\d t^2} + \Gamma^k_{ij} \frac{\d x^i}{\d t} \frac{\d x^j}{\d t} \right) \frac{\partial \mathbf{R}}{\partial{x^k}}}_{\mathrm{tangential}} + \underbrace{L_{ij} \frac{\d x^i}{\d t}\frac{\d x^j}{\d t} \mathbf{\hat{n}}}_{\mathrm{normal}}
$$
...
$$
  \frac{\d^2x^k}{\d t^2} + \Gamma^k_{ij} \frac{\d x^i}{\d t} \frac{\d x^j}{\d t} = 0
$$
...
$$
  \nabla_{\frac{\d}{\d t}}\mathbf{v} = \frac{\d\mathbf{v}}{\d t} - \mathbf{n}
$$
...
\begin{align*}
  ...
  &= \frac{\d}{\d\lambda} \left( \frac{\d u^j}{\d\lambda} \boldsymbol{\partial}_j \right) \\
  &= \frac{\d}{\d\lambda} \left( \frac{\d u^j}{\d\lambda} \right) \boldsymbol{\partial}_j + \frac{\d u^j}{\d\lambda} \frac{\d}{\d\lambda} \boldsymbol{\partial}_j \\
  &= \frac{\d^2 u^j}{\d\lambda^2} \boldsymbol{\partial}_j + \frac{\d u^j}{\d\lambda} \left( \frac{\d u^i}{\d\lambda} \boldsymbol{\partial}_i  \right) \boldsymbol{\partial}_j \\
  &= \frac{\d^2 u^j}{\d\lambda^2} \boldsymbol{\partial}_j + \frac{\d u^i}{\d\lambda} \frac{\d u^j}{\d\lambda} \nabla_{\boldsymbol{\partial}_i} \boldsymbol{\partial}_j \\
  &= \frac{\d^2 u^j}{\d\lambda^2} \boldsymbol{\partial}_j + \frac{\d u^i}{\d\lambda} \frac{\d u^j}{\d\lambda} \Gamma_{ij}^k \boldsymbol{\partial}_k \\
  &= \left( \frac{\d^2 u^k}{\d\lambda^2} + \frac{\d u^i}{\d\lambda} \frac{\d u^j}{\d\lambda} \Gamma_{ij}^k \right) \boldsymbol{\partial}_k
\end{align*}
$$

Hence the curve is geodesic if $\nabla_{\frac{\d}{\d\lambda}} \frac{\d}{\d\lambda} = \mathbf{0}$
...
\begin{align*}
  ...
  &= \lim_{r,s \to 0} \underbrace{DC}_{=\operatorname{id}} \left( \nabla_{\mathbf{u}}\nabla_{\mathbf{v}}\mathbf{w} - \nabla_{\mathbf{v}}\nabla_{\mathbf{u}}\mathbf{w} \right) \\
  &= \nabla_{\mathbf{u}}\nabla_{\mathbf{v}}\mathbf{w} - \nabla_{\mathbf{v}}\nabla_{\mathbf{u}}\mathbf{w}
\end{align*}
$$
...
where the denominator is a normalization factor equaling the area of the parallellogram spaned by the vector fields, equivalent of $\norm{ \mathbf{s} \times \mathbf{v}}^2$. The sign of the nominator denotes wheter the flow curves described by $\mathbf{v}$ are converging/diverging.
