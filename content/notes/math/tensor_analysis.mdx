...
- Vector addition: $+: V \times V \to V$ (binary operation)
- Scalar multiplication: $\cdot: \mathbf{F} \times V \to V$
...
Let $V$ and $W$ be vector spaces over the same field $\mathbb{F}$. A function $f: V \to W$ is called a linear map if it preserve the operations of vector addition and scalar multiplication
...
$$
  \bar{T}_{j_1\dots j_n}^{i_1\dots i_m} = T_{p_1\dots p_n}^{q_1\dots r_m} \frac{\partial\bar{x}^{i_1}}{\partial x^{q_1}}\cdots\frac{\partial\bar{x}^{i_m}}{\partial x^{q_m}}\frac{\partial x^{p_1}}{\partial \bar{x}^{j_1}}\dots\frac{\partial x^{q_n}}{\partial \bar{x}^{j_n}}
$$
$$
$$
$$
