...
A vector space over a field $\mathbb{F}$ is a set $V$ equipped with the two operations
- $+: V \times V \to V$ (vector addition)
- $\cdot: \mathbb{F} \times V \to V$ (scalar multiplication)
satisfying the following properties 

- Vector addition forms an Abelian group $(V,+)$ with the following properties for all $\mathbf{u}, \mathbf{v}, \mathbf{w} \in V$
  - Associativity: $\mathbf{u} + (\mathbf{v} + \mathbf{w}) = (\mathbf{u} + \mathbf{v}) + \mathbf{w}$
  - Commutativity: $\mathbf{u} + \mathbf{v} = \mathbf{v} + \mathbf{u}$
  - Identity element: $\exists \mathbf{0} \in V : \mathbf{v} + \mathbf{0} = \mathbf{v}$
  - Additive inverse: $\forall \mathbf{v} \; \exists -\mathbf{v} : \mathbf{v} + (-\mathbf{v}) = \mathbf{0}$
- Scalar multiplication is compatible satisfying for all $\alpha, \beta\in\mathbf{F}$
  - $\alpha(\beta\mathbf{v}) = (\alpha\beta)\mathbf{v}$
  - $1\mathbf{v} = \mathbf{v}$
- Vector addition and scalar multiplication are distributive
  - $\alpha (\mathbf{u} + \mathbf{v}) = \alpha\mathbf{u} + \alpha\mathbf{v}$
  - $(\alpha + \beta)\mathbf{v} = \alpha\mathbf{v} + \beta\mathbf{v}$
</MathBox>

<MathBox title='Linear subspace' boxType='definition'>
A linear subspace of an $\mathbb{F}$-vector space $V$ is a set $U\subseteq V$ that is itself an $\mathbb{F}$-vector field satisfying
...
The conditions imply that $U$ is closed under linear combination, i.e. $\alpha u + \beta v \in U$ for all $\alpha,\beta\in\mathbb{R}$ and all $u, v \in U$.
</MathBox>
...
Let $V$ and $W$ be vector spaces over the same field $\mathbb{F}$. A function $T: V \to W$ is called a linear transformation if it preserve the operations of vector addition and scalar multiplication
