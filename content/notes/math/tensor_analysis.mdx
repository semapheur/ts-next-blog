...
Using category theory, the tensor product can be defined as a universal mapping for bilinearity. Let $U$ and $V$ be vector spaces over $\mathbb{F}$, and consider the functor
...
In other words, there is a universal pair $(T, \mathrm{B}:U\times V \to T)$ of a vector space $T$ and a bilinear map $\mathrm{B}$ such that for any pair $(W, \mathrm{F}:U\times V \to W)$ of a vector space $W$ and a bilinear map $F$ there is a unique linear map $\mathrm{L}:T\to W$ with $\mathrm{F} = \mathrm{L} \circ \mathrm{B}$ according to the diagram below.

<LatexFigure width={50} src='/fig/universal_property_bilinearity.svg' alt=''
