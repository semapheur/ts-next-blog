...
has kernel
$$
  \mathrm{ker}(\tau) = I_p = \langle \lambda_\sigma (t) - t | t\in B, \sigma\in S_p \rangle
$$
and so
$$
...
has kernel
$$
  \mathrm{ker}(\tau) = I_p = \langle (-1)^\sigma\lambda_\sigma (t) - t | t\in B, \sigma\in S_p \rangle
$$
and so
$$
  T^p (V) / I_p \cong \mathbb{F}_p^- [\mathbf{e}_i]_{i=1}^n
...
Since $\tau$ sends elements of different groups $G_M (\mathbf{v}) = \Set{t_i}_{i=1}^k$ to different monomials in $\mathbb{F}_p [\mathbf{e}_i]_{i=1}^n$ or $\mathbb{F}_p^- [\mathbf{e}_i]_{i=1}^n$, it follows that $\mathbf{v}\in\mathrm{ker}(\tau)$ if and only if $\tau(S_M (\mathbf{v})) = 0$ for all $M$, that is, if and only if $\alpha_{t_1} \tau t +\cdots+ \alpha_{t_k} \tau t_k = 0$.

In the symmetric case, $\tau$ is constant on $G_M (\mathbf{v})$ and so $\mathbf{v}\in\mathrm{ker}(\tau)$ if and only if $\alpha_{t_1} +\cdots+ = 0$. In the antisymmetric case, $t\tau_k = (-1)^{\sigma_{1,j}} \tau t_1$ where $\lambda_{\sigma_{i,j}} = t_j$ and so $\mathbf{v}\in\mathrm{ker}(\tau)$ if and only if
...
$$
  \mathrm{ker}(\tau) \subseteq I_p := \langle \lambda_\sigma (t) - t | t\in B, \sigma\in S_p \rangle
$$

and since $\tau(\lambda_\sigma(t) - t) = 0$, it follows that $\mathrm{ker}(\tau) = I_p$. In the antisymmetric case

$$
  \mathrm{ker}(\tau) \subseteq I_p := \langle (-1)^\sigma \lambda_\sigma (t) - t | t\in B, \sigma\in S_p \rangle
$$

and since $\tau((-1)^\sigma \lambda_\sigma (t) - 1) = 0$, it follows that $\mathrm{ker}(\tau) = I_p$.
</details>
</MathBox>
