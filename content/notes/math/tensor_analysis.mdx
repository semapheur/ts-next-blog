...
A vector space over a field $\mathbb{F} \in \{\mathbb{R}, \mathbb{C} \}$ is a set $V$ with the two operations

- Vector addition: $+: V \times V \mapsto V$ (binary operation)
- Scalar multiplication: $\cdot: \mathbf{F} \times V \mapsto V$

The operations are by definition closed such that
- $\forall \mathbf{u}, \mathbf{v}\in V \quad \mathbf{u} + \mathbf{v} \in V$
- $\forall \alpha \in \mathbb{F} \quad \alpha\mathbf{v} \in V$

Vector addition has the following properties $\forall \mathbf{u}, \mathbf{v}, \mathbf{w} \in V$
- Associativity: $\mathbf{u} + (\mathbf{v} + \mathbf{w}) = (\mathbf{u} + \mathbf{v}) + \mathbf{w}$
- Commutativity: $\mathbf{u} + \mathbf{v} = \mathbf{v} + \mathbf{u}$
- Identity element: $\exists \mathbf{0} \in V$ such that $\mathbf{v} + \mathbf{0} = \mathbf{v}$
- Additive inverse: $\forall \mathbf{v} \quad \exists -\mathbf{v}$ such that $\mathbf{v} + (-\mathbf{v}) = \mathbf{0}$

Scalar multiplication has the following properties $\forall \alpha, \beta \in \mathbf{F}$
...
Given a vector space $V$ over a field $\mathbb{F}$, the dual space $V^*$ is defined as the set of all linear functionals (covectors) $\phi : V \mapsto \mathbb{F}$.
...
$$
\begin{align*}
  \tilde{\mathbf{e}}_i &= \sum_{j=1}^n A_i^j\mathbf{e}_j \\
  \mathbf{e}_i &= \sum_{j=1}^n \tilde{A}_i^j \tilde{\mathbf{e}}_j
\end{align*}
$$
...
Let $V$ and $W$ be vector spaces over the same field $\mathbb{F}$. A function $f: V \mapsto W$ is called a linear map if it preserve the operations of vector addition and scalar multiplication

- $f(\mathbf{u} + \mathbf{v}) = f(\mathbf{u}) + f(\mathbf{v}), \quad \forall \mathbf{u}, \mathbf{v} \in V$
- $f(\alpha \mathbf{u}) = \alpha f(\mathbf{u}), \quad \forall \alpha \in \mathbb{F}$

An isomorphism is a structure-preserving mapping between two vector spaces that can be reversed by an inverse mapping. That is, for a linear map $f: V \mapsto W$ there exists an inverse map $g: W \mapsto V$ such that the compositions $f \circ g: W \mapsto W$ and $g \circ f: V \mapsto V$ are identity maps.

An endomorphism is a linear map $f: V \mapsto V$. An endomorphism that is also an isomorphism is called an automorphism.

For a vector space $V$, the outer product of its basis $B = \left\{ \mathbf{e}_i \right\}_{i \in I \subset \mathbb{N}}$ and its dual basis $B^* = \left\{ \mathbf{e}^i \right\}_{i \in I \subset \mathbb{N}}$spans all endomorphic linear transforms. Every linear transform $L : V \mapsto V$ can be writen as a linear combination of vector-covector pairs 
...
Every linear map between finite-dimensional vector spaces can be represented by a matrix. This can be shown as follows for a transform $L : V \subseteq \mathbb{F}^n \mapsto W \subseteq \mathbb{F}^m$. For simplicity, it is assumed that the bases $B^V = \{\hat{\mathbf{v}}_i\}_{i \in I \subseteq \mathbb{N}}$ and $B^W = \{\hat{\mathbf{w}}_i\}_{i \in I \subseteq \mathbb{N}}$ are orthonormal
...
Let $L$ be an endomorphic linear transformation $L: V \mapsto V$. A non-zero vector $\mathbf{v} \neq \mathbf{0} \in V$ is an eigenvector of $T$ if
...
A linear form is a linear map from a vector space to its field of scalars, $f : V \mapsto \mathbb{F}$ (1-form).

Linear functionals are represented as row vectors in $\mathbb{R}^n$.

### Bilinear form

A bilinear form is a map $B: V \times V \mapsto \mathbb{F}$ (2-form) that is linear in each argument separately $\forall \mathbf{u}, \mathbf{v}, \mathbf{w} \in V$ and $\lambda \in \mathbb{F}$
...
A tensor of rank $(m, n)$ is a multilinear function $T: \prod_{i=1}^n V_i \to \mathbb{R}$ where $V_i$ is a vector spaces.
</MathBox>

## Metric tensor

The metric tensor is a bilinear form $g : V \times V \mapsto \mathbb{R}$ given by

$$
  g(\mathbf{v}, \mathbf{w}) \mapsto g_{ij} v^i w^j 
$$
...
The metric tensor can thus act as a map $g : V \mapsto V*$ with the effect of lowering indices 
...
The inverse metric tensor (contravariant) can act as a map $\mathfrak{g}: V^* \mapsto V$ connecting a covector $\nu \in V^*$ to its vector pair
