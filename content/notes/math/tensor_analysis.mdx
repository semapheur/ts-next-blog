...
<LatexFigure width={50} src='/fig/universal_property_tensor_product.svg' alt=''
  caption='Universal property for the tensor product of vector spaces'
>
```latex
...
Let $U$ and $V$ be vector spaces with respective bases $B = \Set{\mathbf{b}_j }_{j\in J}$ and $C = \Set{\mathbf{c}_j }_{j\in J}$. The tensor product from $U\otimes V$ can be constructed as vector space $U\otimes V$ with basis $D = \Set{ \mathbf{b}_i \otimes \mathbf{c}_j | \mathbf{b}_i \in B, \mathbf{c}_j \in C }$ where $\otimes:U\times V \to U\otimes V$ maps $(\mathbf{b}_i, \mathbf{c}_j)\mapsto \mathbf{b}_i \otimes\mathbf{c}_i$.
...
<LatexFigure width={50} src='/fig/tensor_product_isomorphism.svg' alt=''
