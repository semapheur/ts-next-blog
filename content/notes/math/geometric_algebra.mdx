...
Let $\mathcal{B} = \set{\mathbf{e}_i}_{i=1}^{n}$ be an orthogonal basis of an $n$-dimensional vector space. Then by $\eqref{equation-32}$
...
The dimension of a universal Clifford algebra $\operatorname{Cl}(V, g)$ is $\sum_{p=0}^n \binom{n}{p}$, that is

$$
  \dim[\operatorname{Cl}(V, g)] = 2^{\dim(V)}
$$

There exists an isomorphism between the Clifford algebra $\operatorname{Cl}(V,g)$ and the exterior algebra $\Lambda(V)$ or the Grassmann algebra $\mathcal{G}(V)$. This isomorphism is not an algebra isomorphism, by a vector space isomorphism, 

$$
  \operatorname{Cl}(V, g) \underset{V}{\simeq} \Lambda(V)
$$

For the operations of the of the vector space structure, it is natural to use for the Clifford algebra $\operatorname{Cl}(V,g)$ the same notation used for the exterior algebra, although the subspace of the $p$-vectors will still be denoted by $\Lambda_p (V)$. Then the following equality must be realised as an equality of vector spaces: 

$$
  \operatorname{Cl}(V, g) = \bigoplus_{p=0}^n \Lambda_p (V)
$$

The projection operators are equivalently denoted by

$$
  \braket{}_p : \operatorname{Cl}(V,g) \to \Lambda_p (V)
$$

The projection operator into the scalar part $\braket{}_0$ has prominent importance in the formalism of Clifford algebras. It can be shown that, for $A,B\in \operatorname{Cl}(V,g)$, the scalar part is such that

$$
  \braket{\mathbf{AB}}_0 = \braket{\mathbf{BA}}_+
$$

Let $g$ be a symmetric bilinear form of signature $(p,q)$, where $p + q = n$, such that $\mathcal{B} = \set{\mathbf{e}_i}_{i=1}^n$ is an orthonormal basis. On any vector $\mathbf{v} = v^i \mathbf{e}_i \in V$, the symmetric bilinear form can be evaluated as

$$
  g(\mathbf{v}, \mathbf{v}) = (v^1)^2 + \cdots + (v^p)^2 - (v^{p+1})^2 - \cdots - (v^n)^2
$$

We denote this quadratic space by $\R^{p,q}$, and the corresponding Clifford algbra by 

$$
  \operatorname{Cl}_{p,q} = \operatorname{Cl}(\R^{p,q})
$$

The centrs of the Clifford algebra $\operatorname{Cl}_{p,q}$ is defined as being the set of elements in $\operatorname{Cl}_{p,q}$ that commute with all elements of $\operatorname{Cl}_{p,q}$

$$
  \operatorname{cen}(\operatorname{Cl}_{p,q}) = \set{a\in\operatorname{Cl}_{p,q} | ax = xa, \forall x\in\operatorname{Cl}_{p,q}}
$$

When a basis $\set{\mathbf{e}_i}_{i=1}^n$ is taken into account, every $\mathbf{e}_i$ either commutes or anti-commutes with $\mathbf{e}_{\mu_1} \cdots \mathbf{e}_{\mu_p}$. Thies property provides the elements of the centre in $\operatorname{Cl}(V,g)$. This property provides the elements fo the centre in $\operatorname{Cl}(V,g)$. It can be shown that
...
The grade involution, the reversion, and the conjugation preserve the ideal $\operatorname{I}_C$. Then, as in the interior algebras, such operations pass to the quotient. Hence, for the grade involution, it follows that
...
$$
\begin{equation*}
  (\widetilde{\mathbf{A}_{[p]} \mathbf{B}_{[q]}}) = \tilde{\mathbf{B}}_{[q]} \tilde{\mathbf{A}}_{[p]}
\tag{\label{equation-37}}
\end{equation*}
$$

with $\tilde{\mathbf{A}}_{[0]} = \mathbf{A}_{[0]}$ and $\tilde{\mathbf{A}}_{[1]} = \mathbf{A}_{[1]}$. This result implies that

$$
  \tilde{\mathbf{A}}_{[p]} = (-1)^{p(p-1)/2} \mathbf{A}_{[p]}
$$

and, for the conjugation

$$
  \widebar{\mathbf{A}}_{[p]} = \widetilde{\widehat{\mathbf{A}}}_{[p]} = \widehat{\widetilde{\mathbf{A}}}_{[p]}
$$

<details>
<summary>Proof</summary>
Equation $\eqref{equation-37}$ is straightforward to prove when $\mathbf{A}_{[p]}$ and $\mathbf{B}_{[q]}$ are products of vectors of the orthogonal basis $\set{\mathbf{e}_i}_{i=1}^n$. When some vectors $\mathbf{e}_i$ are factors both in $\mathbf{A}_{[p]}$ and $\mathbf{B}_{[q]}$, we can, without loss of generality, choose $\mathbf{e}_p$ as the common vector that composes $\mathbf{A}_{[p]}$ and $\mathbf{B}_{[q]}$. Up to a sign, we can write $\mathbf{A}_{[p]}$ and $\mathbf{B}_{[q]}$ as

$$
  \mathbf{A}_{[p]} = \bigwedge_{i=1}^p \mathbf{e}_i, \quad \mathbf{B}_{[q]} = \bigwedge_{j=p+1}^{p+q} \mathbf{e}_j
$$
...
$$
\begin{align*}
  \widetilde{\mathbf{A}_{[p]}\mathbf{B}_{[q]}} =& \left[\left(\bigwedge_{i=1}^p \mathbf{e}_i \right) \left(\bigwedge_{j=p}^{p+q} \mathbf{e}_j \right) \right]^\tilde \\
...
$$
\begin{align*}
  \widetilde{\mathbf{A}_{[p]}\mathbf{B}_{[q]}} =& \left[\left(\bigwedge_{i=1}^{p-1} \mathbf{e}_i \right) \mathbf{e}_p \left(\bigwedge_{j=p}^{p+q} \mathbf{e}_j \right) \right]^\tilde \\
  ...
  =& \tilde{\mathbf{B}}_{[q]} \tilde{\mathbf{A}}_{[p]}
\end{align*}
$$
