...
$$
\begin{gather*}
  0 = (\mathbf{v}\lfloor\alpha)\Omega_{V^*} - \alpha\wedge (\mathbf{v} \lfoor\Omega_{V^*}) \\
...
$$
\begin{equation*}
  \underline{\star}(\Psi^{[p]} \wedge \bar{\star}\mathbf{A}_{[p]}) = \tilde{\mathbf{A}}_{[p]} \floor\Psi^{[p]}
...
$$
  (\widetilde{\mathbf{A}_{[p]} \wedge \underline{\star} \Psi^{[p]}}) \lfloor\Omega_{V^*} = \tilde{\mathbf{A}}_{[p]} \lfloor\Psi^{[p]} (\tilde{\Omega}_{V^*} \lfloor\Omega_{V^*}) = \tilde{\mathbf{A}}_{[p]} \floor \Psi^{[p]}
$$
...
This mapping satisfies the Clifford relation
...
for all $\mathbf{u},\mathbf{v}\in\R^3$. Thus, $\gamma$ defined a Clifford map, and $\mathcal{A}\cong \operatorname{Cl}(\R^3, g)$ realizes the Clifford algebra of $\R^3$ with the Euclidean inner product.
</MathBox>
...
In terms of the quotient mapping $\pi:\operatorname{T}(V) \to \operatorname{T}(V)/\mathcal{I}_c \simeq \operatorname{Cl}(V,g)$, the product of the vectors in the quotient algebra $\operatorname{T}(V)/\mathcal{I}_c$ can be written as
...
$$
\begin{align*}
  \operatorname{alt}\left(\sum_{i=1}^n \mathbf{A}_i \otimes (\mathbf{v}\otimes\mathbf{v} - Q(\mathbf{v})\mathbf{1}) \otimes \mathbf{B}_i \right) =& \operatorname{alt}\left(\sum_{i=1}^n \mathbf{A}_i \otimes\mathbf{v}\otimes\mathbf{v} \otimes \mathbf{B}_i \right) \\
  &- \operatorname{alt}\left(\sum_{i=1}^n \mathbf{A}_i \otimes Q(\mathbf{v})\mathbf{1} \otimes \mathbf{B}_i \right) \\
  =& -Q(\mathbf{v}) \operatorname{alt}\left(\sum_{i=1}^n \mathbf{A}_i \otimes \mathbf{1} \otimes \mathbf{A}_i \right) = 0 \iff Q(\mathbf{v}) = 0
\end{align*}
$$
...
Moreover, $\Lambda(V)$ is identified with $\operatorname{Cl}(V,g)$. In fact, the quotient mapping $\pi:\operatorname{T}(V) \to\operatorname{Cl}(V,g)$ induces a mapping $\Lambda(V)\to\operatorname{Cl}(V,g)$, which by following the previous argument can be shown to be a bijection.
...
\begin{align*}
  ...
  =& \sum_{i=1}^p (-1)^i \mathbf{v}_i \otimes \left[\sum_{j=1}^{i-1} (-1)^{j+1} g(\mathbf{u},\mathbf{v}_j) \mathbf{A}_{[1\cdots\hat{j}\cdots\hat{i}\cdots p]} + \sum_{j=1+1}^p (-1)^j g(\mathbf{u},\mathbf{v}_j) \mathbf{A}_{1\cdots\hat{i}\cdots\hat{j}\cdots p} \right] \\
  ...
  =& \sum_{j=1}^p (-1)^{j+1} g(\mathbf{u},\mathbf{v}_j) \left[\sum_{i=1}^{j-1} (-1)^{i+1} \mathbf{v}_i \otimes\mathbf{A}_{[1\cdots\hat{i}\cdots\hat{j}\cdots p]} + \sum__{i=j+1}^p (-1)^i \mathbf{v}_i \otimes\mathbf{A}_{[1\cdots\hat{j}\cdots\hat{i}\cdots p]}\right] \\
