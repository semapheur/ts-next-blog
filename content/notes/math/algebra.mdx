...
- for each $A, B\in\mathrm{ob}(\mathcal{C})$ a collection $\mathrm{Hom}_{\mathcal{C}}(A,B)$ of *morphisms* (maps or arrows) from $A$ to $B$, shortened as $\mathcal{C}(A,B)$
...
<LatexFig width={50} src='/fig/natural_transformation.svg' alt=''
