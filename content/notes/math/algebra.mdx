...
<TikzFigure 
  ...
  packages="amsmath"
  fallbackImage={{src: "/fig/category.svg", alt:"", width: 50}}
>
{String.raw`\begin{tikzcd}[row sep=large, column sep=large, every label/.append style = {font = \scriptsize}]
  A \arrow[r, "f"] \arrow[dr, "g\circ f" swap] \arrow[in=120, out=150, loop, "\operatorname{id}_A"] &
...
<TikzFigure 
  ...
  fallbackImage={{src: "/fig/isomorphism.svg", alt:"", width: 50}}
>
{String.raw`\begin{tikzcd}[row sep=normal, column sep=normal, every label/.append style = {font = \scriptsize}]
...
<TikzFigure 
  ...
  fallbackImage={{src: "/fig/natural_transformation.svg", alt:"", width: 25}}
>
{String.raw`\begin{tikzcd}[cramped, sep=normal, every label/.append style = {font = \scriptsize}]
  \mathcal{A} 
...
<TikzFigure 
  ...
  fallbackImage={{src: "/fig/universal_property.svg", alt:"", width: 50}}
>
{String.raw`\begin{tikzcd}[row sep=large, column sep=large, every label/.append style = {font = \scriptsize}]
...
Let $N$ be an ideal ideal of $R$ such that $M\subset C \subset R$ and $N\neq R$. We have to prove that $M = N$ to conclude that $M$ is maximal.
