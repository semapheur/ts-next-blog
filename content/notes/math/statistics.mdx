...
Let $\theta\in \Theta\subseteq\mathbb{R}$ be an unknown real parameter. A real-valued statistic $U = u(\mathbf{X})$ that is used to estimate $\theta$ is called an estimator of $\theta$. The estimator is a random variable whose moments generally depend on $\theta$.
...
\begin{align*}
  ...
  F_2 (n, \nu) = \frac{1}{n}\sum_{i=1}^n \left( Y[N_n - (\nu - N_n)n + i] - \hat{Y}_\nu(t) right)^2,\quad \nu\in\{1,\dots,N_n},\quad \nu\in\{N_n + 1,\dots, 2N_n\}
\end{align*}
$$
