...
$$
  \hat{L}_{\boldsymbol{x}}(\boldsymbol{\lambda}) = \max\left\{ L_{\boldsymbol{x}}(\boldsymbol{\theta}) \mid \boldsymbol{\theta}\in h^{-1}(\boldsymbol{\lambda}) \right\},\quad \boldsymbol{\lambda}\in\Lambda
$$
...
Often, the critical region is defined in terms of a statistic $w(\mathbf{X})$, known as a *test statistic*, where $w:S\to T$ is a function from the sample space $S$ into another set $T$. We find an appropriate rejection region $R_T\subseteq T$ and reject $H_0$ when the observed value $w(\mathbf{x})\in R_T$. Thus, the rejection region in $S$ is $R = w^{-1}(R_T) = \{\mathbf{x}\in S \mid w(\mathbf{x})\in R_T\}$. The use of a statistic often allows significant data reduction when the dimension of the test statistic is much smaller than the dimension of the data vector.
