...
<MathBox title='Central limit theorem'>
...
Suppose that $\mathbf{X}\in S$ is an observable random variable for a statistical experiment, depending on an unknown parameter $\theta\in\Theta$. Let $f_\theta$ denote the probability density function of $\mathbf{X}$ for $\theta$. The likelihood function at $\mathbf{x}\in S$ is the function $L_\mathbf{x}:\Theta\to[0,\infty)$ given by

$$
  L_\mathbf{x} (\theta) = f_\theta(\mathbf{x})
$$
...
$$
  \ln L_\mathbf{x}(\theta) = \ln f_\theta(\mathbf{x})
$$
...
Suppose that the maximum value of $L_\mathbf{x}$ or $\ln L_\mathbf{x}$ occurs at $u(\mathbf{x})\in\Theta$ for each $\mathbf{x}\in S$. Then the statistic $u(\mathbf{X})$ is a *maximus likelihood estimator* of $\theta$.
</MathBox>
