...
$$
  S^n := \Set{\mathbf{x} \in\R^{n+1} | \lVert \mathbf{x} \rVert = 1}
$$
...
Conversely, suppose that $F$ is a smooth immersion, and let $\mathbf{p}\in M$. We show first that $\mathbf{p}$ has a neighbourhood on which $F$ is injective. If $F(\mathbf{p}) \notin\partial N$, then either the rank theorem (if $\mathbf{p}\notin\partial M$) or the local immersion theorem (if $\mathbf{p}\in\partial M$) implies that there is a neighborhood $U_1$ on $\mathbf{p}$ on which $F$ has a coordinate representation from the rank theorem. It follows from this formula that $F|_{U_1}$ is injective.
...
If $\pi:M\to N$ is any continuous map, a *section* of $\pi$ is a continuous right inverse for $\pi$, i.e. a continuous map $\sigma:N\to M$ such that $\pi\circ\simga = \mathrm{id}_N$.
