...
The vector space all $\binom{p}{q}$-tensors on $V$ is denoted
...
<MathBox title='' boxType='proposition'>
Suppose $M$ is a smooth manifold with or without boundary, $A\in\mathcal{T}_k (M)$, $B\in\mathcal{T}_l (M)$ and that $f\in C^\infty(M)$. Then $fA$ nad $A\otimes B$ are also smooth tensor fields, whose components in any smooth local coordinate chart are
