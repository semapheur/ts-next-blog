...
$$
  \operatorname{span}(\mathbf{e}_i|_\mathbf{p})_{i=1}^n = \operatorname{span}(X_i|_\mathbf{p})_{i=1}^n
$$
...
For eahc $j=1,\dots,n$ and each $\mathbf{p}\in U$, then $X_j|_\mathbf{p}\notin \operatorname{span}(E_1|_\mathbf{p},\dots,E_{j-1}|_\mathbf{p})$, so the denominator above is a nowhere-vanishing smooth function on $U$. Thus, this formula defines $(\mathbf{e}_i)_{i=1}^n$ as a smooth orthonormal frame on $U$.
</details>
</MathBox>
