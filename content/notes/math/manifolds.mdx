...
$$
\begin{align*}
  \operatorname{id}_{\R^n} =& \mathrm{D}(\operatorname{id}_{\varphi(U)})(\mathbf{x}) = \mathrm{D}(\tilde{F}^{-1}\circ\tilde{F})(\mathbf{x}) \\
  =& \mathrm{D}(\tilde{F}^{-1})(\tilde{F}(\mathbf{x}))\circ\mathrm{D}\tilde{F}(\mathbf{x})
\end{align*}
$$

Similarly, $\tilde{F}\circ\tilde{F}^{-1} = \operatorname{id}_{\psi(V)}$ implies that $\operatorname{id}_{\R^m} = \mathrm{D}F(\mathbf{x})\circ\mathrm{D}(\tilde{F}^{-1})(\tilde{F}(\mathbf{x}))$. This implies that $\mathrm{D}(\tilde{F}(\mathbf{x}))$ is invertible with inverse $\mathrm{D}(\tilde{F}^{-1})(\tilde{F}(\mathbf{x}))$, and hence $m = n$.
</details>
</MathBox>
...
3. $\mathrm{d}(\operatorname{id}_M)_\mathbf{p} = \operatorname{id}_{T_\mathbf{p} M}: T_\mathbf{p} M \to T_\mathbf{p} M$
...
2. $\mathrm{d}(\operatorname{id}_M) = \operatorname{id}_{TM}$ 
...
Conversely, assume each point of $M$ is in the image of a smooth local section. Given $\mathbf{p}\in M$, let $\sigma:U\to M$ be a smooth local section such that $\sigma(\mathbf{q}) = \mathbf{p}$, where $\mathbf{q} = \pi(\sigma(\mathbf{q})) = \pi(\mathbf{p})\in N$. The equation $\pi\circ\sigma = \operatorname{id}_U$ implies that $\mathrm{d}\pi_\mathbf{p} \circ \mathrm{d}\sigma_\mathbf{q} = \operatorname{id}_{T_\mathbf{q} N}$, which in turn implies that $\mathrm{d}\pi_\mathbf{p}$ is surjective.
</details>
</MathBox>
...
Both of the relations $\mathrm{d}(\operatorname{id}_G)_e = \operatorname{id}_{T_e G}$ and $\mathrm{d}(F_2 \circ F_1)_e = \mathrm{d}(F_2)_e \circ \mathrm{d}(F_1)_e$ hold for differentials. Since the induced homomorphism is determined by the differential at the identity, this proves **(1)** and **(2)**. If $F:G\to H$ is an isomorphism, **(1)** and **(2)** together imply that 
