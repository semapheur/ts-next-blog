...
Given a point $a\in\R^n$, the *geometric tangent space* to $\R^n$ at $a$, denoted $\R_a^n$, is the set 

$$
  \Set{a} \times \R^n = \Set{(a, \mathbf{v}) = \mathbf{v}_a | \mathbf{v}\in R^n}$
$$

A *geometric tangent vector* $\mathbf{v}$ in $\R^n$ is an element of $\R_a^n$ with initial point at $a\in\R^n$. The set $\R_a^n$ is a vector space.
</MathBox>

As a vector space, $\R_a^n$ is essentially the same as $\R^n$ itself; the only reason for the additional index $a$ is so that the geometric tangent spaces $\R_a^n$ and $\R_b^n$ at distinct points $a$ and $b$ will be disjoint sets.

The directional derivate of a geometric tangent vector $\mathbf{v}$ at $a$ is a map $\mathrm{D}_{\mathbf{v}_a}: C^\infty (\R^n) \to\R$ defined by

$$
  \mathrm{D}_{\mathbf{v}_a}(f) = \mathrm{D}_{\mathbf{v}}f(a) = \left\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t=0} f(a + t\mathbf{v})
$$

This operation is linear over $\R$ and satisfies the product rule

$$
  \mathrm{D}_{\mathbf{v}_a} (fg) = f(a)\mathrm{D}_{\mathbf{v}_a}(g) + g(a)\mathrm{D}_{\mathbf{v}_a}(f)
$$

If $\mathbf{v}_a = v^i \mathbf{e}_i|_a$ in terms of the standard basis, then by the rule, $\mathrm{D}_{\mathbf{v}|a}(f)$ can written as

$$
  \mathrm{D}_{\mathbf{v}_a}(f) = v^i \frac{\partial f}{\partial x^i}(a)
$$

<MathBox title='Derivation' boxType='definition'>
If $a$ is a point of $\R^n$, a function $C^\infty (\R^n)\to\R$ is called a $derivation$ at $a$ if it is linear over $\R$ and satisfies the following product rule
...
Suppose that $a\in\R^n$, $w\in T_a \R^n$ and $f,g\in C^\infty(\R)$.
...
**(1):** It suffices to consider the constant function $f_1 (x) \equiv 1$, for then $f(x) \equiv c$ implies $wf = w(cf_1) = cw f_1 = 0$ by linearity. For $f_1$, the product rule gives

$$
\begin{align*}
  wf_1 =& w(f_1 f_1) \\
  =& f_1 (a)wf_1 + f_1 (a)w f_1 = 2wf_1
\end{align*}
$$
...
<MathBox title='' boxType='proposition'>
Let $a\in\R^n$.
1. For each geometric tangent vector $\mathbf{v}_a \in\R_a^n$, the directional derivative $D_{\mathbf{v}_a}: C^\infty (\R^n) \to\R$ is a derivation at $a$.
2. The map $\mathbf{v}_a \mapsto \mathrm{D}_{\mathbf{v}_a}$ is an isomorphism from $\R_a^n$ onto $T_a\R^n$.
...
**(2):** It is easily seen that $\mathbf{v}_a \mapsto \mathrm{D}_{\mathbf{v}_a}$ is linear. To see that it is injective, suppose that $\mathbf{v}_a\in\R_a^n$ has the property that $\mathrm{D}_{\mathbf{v}_a}$ is the zero derivation. Writing $\mathbf{v}_a = v^i \mathbf{e}_i|_a$ in terms of the standard basis, and taking $f$ to be the $j$th coordinate function $x^j: \R^n\to\R$ thought of as a smooth function on $\R^n$, we obtained

$$
  0 = \mathrm{D}_{\mathbf{v}_a}(x^j) = v^i \left.\frac{\partial}{\partial x^i}(x^j)\right|_{x=a} = v^i \delta_i^j = v^j
$$

Since this is true for each $j$, it follows that $\mathbf{v}_a$ is the zero vector.

To prove surjectivity, let $w\in T_a \R^n$ be arbitrary and defined $\mathbf{v} = v^i \mathbf{e}_i$ where $v^i = w(x^i)$. We will show that $w = \mathrm{D}_{\mathbf{v}_a}$. Let $f:\R^n \to\R$ be any smooth function. By Taylor's theorem, we can write

$$
\begin{align*}
  f(x) =& f(a) + \sum_{i=1}^n \frac{\partial f}{\partial x_i}(a)(x^i - a^i) \\
  &+ \sum_{i,j=1}^n (x^i - a^i)(x^j - a^j) \int_0^1 (1-t)\frac{\partial^2 f}{\partial x^i \partial x^j}(a + t(x - a))\;\mathrm{d}t
\end{align*}
$$

Note that each term in the last sum above is a product of two smooth functions of $x$ that vanish at $x = a$: one is (x^i - a^i), and the other is $(x^j - a^j)$ times the integral. The derivation $w$ annihilates this sum by its properties, giving

$$
\begin{align*}
  wf =& w(f(a)) + \sum_{i=1}^n w\left(\frac{\partial f}{\partial x^i}(a)(x^i - a^i)\right) \\
  =& 0 + \sum_{i=1}^n \frac{\partial f}{\partial x^i}(x)(w(x^i) - w(a^i)) \\
  =& \sum_{i=1}^n \frac{\partial f}{\partial x^i} (a)v^i = \mathrm{D}_{\mathbf{v}_a}(f)
\end{align*}
$$
</details>
</MathBox>
...
For $a\in\R^n$, the set of $n$ derivations

$$
  \Set{\left.\frac{\partial}{\partial x^i}\right|_a }_{i=1}^n,\quad $\left.\frac{\partial}{\partial x^i}\right|_a f = \frac{\partial f}{\partial x^i}(a)$
$$

form a basis for $T_a \R^n$, which there has dimension $n$

<details>
<summary>Proof</summary>

Note that $\left.\frac{\partial}{\partial x^i}\right|_a = \mathrm{D}_{\mathbf{e}_i|_a}$.
</details>
</MathBox>
...
Let $M$ be a smooth manifold with or without boundary. If $p\in M$, a linear function $v: C^\infty(M)\to\R$ is called a *derivation* at $p$ if it satisfied the product rule

$$
  v(fg) = f(p)vg + g(p)vf,\quad \forall f,g\in C^\infty(M)
$$

The set of all derivations of $C^\infty (M)$ at $p$, denoted $T_p M$, is a vector space called the *tangent space* to $M$ at $p$. An element of $T_p M$ is called a *tangent vector* at $p$. 
</MathBox>

<MathBox title='Properties of tangent vectors on manifolds' boxType='definition'>
Suppose that $M$ is a smooth manifold with or without boundary, $p\in M$, $\mathbf{v} \in T_p M$ and $f, g\in C^\infty (M)$.
...
If $M$ and $N$ are smooth manifolds with or without boundary and $F:M\to N$ is a smooth map, the differential of $F$ at $p\in M$ is the map

$$
  \mathrm{d}F_p : T_p M \to T_{F(p)}N
$$

defined as follows. Given $v\in T_p M$, we let $\mathrm{d}F_p (v)$ be the derivation at $F(p)$ that acts on $f\in C^\infty$ by the rule

$$
  \mathrm{d}F_p (\mathbf{v}) = \mathbf{v}(f\circ F)
$$

<details>
<summary>Details</summary>

Note that if $f\in C^\infty (N$, then $f\circ F \in C^\infty (M)$, so $v(v\circ F)$ makes sense. The operator $\mathrm{d}F_p (v): C^\infty (N)\to\R$ is linear because $v$ is, and is a derivation at $F(p))$ because for any $f,g\in C^\infty (N)$ we have
...
1. $\mathrm{d}F_p: T_p M \to T_{F(p)}N$ is linear
2. $\mathrm{d}(G\circ F)_p = \mathrm{d}G_{F(p)} \circ\mathrm{d}F_p: T_p M \to T_{G\circ F(p)} P$
3. $\mathrm{d}(\mathrm{id}_M)_p = \mathrm{id}_{T_p M}: T_p M \to T_p M$
4. If $F$ is a diffeomorphism, then $\mathrm{d}F_p: T_p M \to T_{F(p)} N$ is an isomorphism and $(\mathrm{d}F_p)^{-1} = \mathrm{d}(F^{-1})_{F_(p)}$
</MathBox>

<MathBox title='' boxType='proposition'>
Let $M$ be a smooth manifold with or without boundary, $p\in M$ and $v\in T_p M$. If $f,g\in C^\infty(M)$ agree on some neighbourhood of $p$, then $vf = vg$.

<details>
<summary>Proof</summary>

Let $h = f - g$ so that $h$ is a smooth function that vanished in a neighbourhood of $p$. Let $\psi\in C^\infty (M)$ be a smooth bump function that is identically equal to $1$ on the support of $h$ and is supported in $M\setminus\Set{p}$. Because $\psi\equiv 1$ where $h$ is nonzero, the product $\psi h$ is identically equal to $h$. Since $h(p) = \psi(p) = 0$, it follows that $vh = v(\psi h) = 0$. By linearity, this implies $vf = vg$.
</details>
</MathBox>

<MathBox title='The tangent space to an open submanifold' boxType='proposition'>
Let $M$ be a smooth manifold with or without boundary, $U\subseteq M$ an open subset, and let $\iota: U \hookrightarrow M$ be the inclusion map. For every $p\in U$, the differential $\mathrm{d}\iota_p: T_p U \to T_p M$ is an isomorphism.

<details>
<summary>Proof</summary>

To show injectivity, suppose that $v\in T_p U$ and $\mathrm{d}\iota_p (v) = 0\in T_p M$. Let $B$ a neighbourhood of $p$ such that $\overline{B} \subseteq U$. If $f \in C^\infty (U)$ is arbitrary, the extension lemma for smooth function guarantees that there exists $\tilde{f}\in C^\infty(M)$ such taht $\tilde{f} \equiv f$ on $\overline{B}$. Then since $f$ and $\tilde{f}|_U$ are smooth function on $U$ that agree in a neighbourhood of $p$, it follows from the previous proposition that

$$
  vf = v(\tilde{f}|_U) = v(\tilde{f}\circ\iota) = \mathrm{d}\iota(v)_p \tilde{f} = 0
$$

Since this holds for every $f\in C^\infty (U)$, it follows that $v = 0$, so $\mathrm{d}\iota_p$ is injective.

To show surjectivity, suppose that $w\in T_p M$ is arbitrary. Define an operator $v: C^\infty(U) \to\R$ by setting $vf = w\tilde{f}$, where $\tilde{f}$ is any smooth function on all of $M$ that agrees with $f$ on $\overline{B}$. By the previous proposition, $vf$ is independent of the choice of $\tilde{f}$, so $v$ is well-defined, and it is easy to check that it is a derivation of $C^\inft (U)$ at $p$. For any $g\in C^\infty (M)$

$$
  \mathrm{d}\iota_p (v)g = v(g\circ\iota) = w(\tilde{g\circ\iota}) = wg
$$

where the last two equalities follow from the fact that $g\circ\iota$, $\tilde{g\circ\iota}$ and $g$ all agree on $B$. Thus, $\mathrm{d}\iota_p$ is also surjective.
</details>
</MathBox>

<MathBox title='Dimension of the tangent space' boxType='proposition'>
If $M$ is a an $n$-dimensional smooth manifold, then for each $p\in M$, the tangent space $T_p M$ is an $n$-dimensional vector space.

<details>
<summary>Proof</summary>

Given $p\in M$, let $(U,\varphi)$ be a smooth coordinate chart containing $p$. Because $\varphi$ is a diffeomorphism from $U$ onto an open subset $\tilde{U}\subseteq\R^n$, follows from the properties of differentials that $\mathrm{d}\varphi_p$ is an isomorphism from $T_p U \to T_{\varphi(p)}\tilde{U}$. Since the previous proposition guarantees that $T_p M \cong T_p U$ and $T_{\varphi(p)}\tilde{U} \cong T_{\varphi(p)} \R^n$ it follows that $\dim(T_p M) = \dim(T_{\varphi(p)}\R^n = n$.
</details>
</MathBox>
