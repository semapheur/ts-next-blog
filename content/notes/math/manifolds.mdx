...
To show that $\theta^{(\mathbf{p})}$ is an integral curve, assume that $t_0 \in D^{(\mathbf{p})}$. Because both $\mathbf{D}^{(\mathbf{p})}$ and $D^{(\theta_{t_0}(\mathbf{p}))}$ are open intervals containing $0$, there is $\varepsilon > 0$ such that $t + t_0\in D^{(\mathbf{p})}$ and $t\in D^{(\theta_{t_0})(\mathbf{p})}$ whenever $|t| < \varepsilon$, and then $\theta_t(\theta_{t_0}(\mathbf{p})) = \theta_{t+t_0}(\mathbf{p})$ by definition of a flow. The rest of the proof goes just as before.
</details>
</MathBox>
