...
$$
  \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p} :=& \left.\frac{\partial f}{\partial x^i}\right| := \frac{\partial (f\circ\varphi^{-1})}{\partial r^i} (\varphi(\mathbf{p})) \\
...
Let $(U,\varphi=(x^j)_{j=1}^n)$ and $(V,\psi=(y^i)_{i=1}^m)$ be overlapping charts on a smooth manifold $M$. The transition map $\psi\circ\varphi^{-1} : \varphi(U\vap V) \to \psi(U\cap V)$ is a diffeomorphism of open subsets of $\R^n$. The Jacobian matrix is given by 
...
Let $M$ be a smooth manifold with or without boundary. If $\mathbf{p}\in M$, a linear function $\mathbf{v}: C_\mathbf{p}^\infty(M)\to\R$ is called a *derivation* at $\mathbf{p}$ if it satisfied the product rule
...
$$
  \boldsymbol{\partial}_i |_\mathbf{p} := \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p} : C_\mathbf{p}^\inft (M)\to\R
$$ 
...
such that for $\mathbf{v}\in T_\mathbf{p} M$, we let $\mathrm{d}F_\mathbf{p} (\mathbf{v})$ be the derivation at $F(\mathbf{p})$ that acts on $f\in C_{F(\mathbf{p})}^\infty$ by the rule
...
<MathBox title='Properties of differentials' boxType='proposition' tag='proposition-39'>
Let $M$, $N$ and $P$ be smooth manifolds with or without boundary, $F:M\to N$ and $G:N\to P$ be smooth maps and let $p\in M$. The differentials of $F$ at $\mathbf{p}$ and $G$ at $F(\mathbf{p})$ are linear maps

$$
  T_\mathbf{p}N \xrightarrow{\mathrm{d}F_\mathbf{p}} T_{F(\mathbf{p})}M \xrightarrow{\mathrm{d}G_{F(\mathbf{p})}} T_{G(F(\mathbf{p}))}P
$$


1. $\mathrm{d}F_\mathbf{p}: T_\mathbf{p} M \to T_{F(\mathbf{p})}N$ is linear
2. **Chain rule:** $\mathrm{d}(G\circ F)_\mathbf{p} = \mathrm{d}G_{F(\mathbf{p})} \circ\mathrm{d}F_\mathbf{p}: T_\mathbf{p} M \to T_{G\circ F(\mathbf{p})} P$
...
$$
  (\mathrm{d}(G\circ F)_\mathbf{p} (\mathbf{v}))f = \mathbf{v}(f\circ G\circ f)
$$
...
$$
\begin{align*}
  (\mathrm{d}G_{F(\mathbf{p})} \circ \mathrm{d}F_\mathbf{p})(\mathbf{v})f =& \mathrm{d}G_{F(p)}(\mathrm{d}F_\mathbf{p}(\mathbf{v}))f \\
  =& (\mathrm{d}F_\mathbf{p}(\mathbf{v}))(f\circ G) \\
...
$$
  \mathrm{d}(\mathrm{id}_M)(\mathbf{v})f = \mathbf{v}(f\circ \operatorname{id}_M ) = \mathbf{v}(f)
$$
...
$$
\begin{align*}
  \mathrm{d}(G\circ F)_\mathbf{p} = \mathrm{d}G_{F(\mathbf{p})} \circ \mathrm{d}F_\mathbf{p} = \mathrm{d}(\operatorname{id}_N)_{F(\mathbf{p})} = \operatorname{id}_{T_\mathbf{p}M} \\
  \mathrm{d}(F\circ G)_{F(\mathbf{p})} = \mathrm{d}F_\mathbf{p} \circ \mathrm{d}G_{F(\mathbf{p})} = \mathrm{d}(\operatorname{id}_M)_\mathbf{p} = \operatorname{id}_{T_{F(\mathbf{p})}M} 
\end{align*}

Hence $\mathrm{d}F_\mathbf{p}$ and $\mathrm{d}G_\mathbf{p}$ are diffeomorphisms
$$
</details>
</MathBox>
...
Suppose $M$ is a smooth $n$-manifold without boundary, and let $(U,\varphi)$ be a smooth coordinate chart on $M$. Since $\varphi: U\to\tilde{U}\subseteq\R^n$ is a diffeomorphism onto its image, then by proposition $\ref{proposition-39}.4$  the differential 

$$
  \mathrm{d}\varphi_\mathbf{p}: T_\mathbf{p} U \to T_{\varphi(\mathbf{p})} \R^n
$$ 

is a vector space isomorphism. Furthermore, since $U$ is an open submanifold it follows that $T_\mathbf{p} U \cong T_\mathbf{p} M$ for every $\mathbf{p}\in M$. This implies that $\mathrm{d}\varphi_\mathbf{p}: T_\mathbf{p} M \to T_{\varphi(\mathbf{p})} \R^n$ is an isomorphism.

<MathBox title='The tangent space to a product manifold' boxType='proposition' tag='proposition-46'>
...
$$
  \mathrm{d}\varphi_\mathbf{p} \left(\left.\frac{\partial}{\partial x^i}\right) = \left.\frac{\partial}{\partial r^i}\right|_{\varphi(\mathbf{p})}
$$
...
$$
\begin{align*}
  \mathrm{d}\varphi_\mathbf{p} \left(\left.\frac{\partial}{\partial x^i}\right|_\mathbf{p} \right) =& \left\frac{\partial}{\partial x^i}\right|_\mathbf{p} (f\circ\varphi) \tag{definition of $\mathrm{d}\varphi$} \\
...
$$
  \Set{\boldsymbol{\partial}_i|_\mathbf{p} := \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}}_{i=1}^n
$$

<details>
<summary>Proof</summary>

An isomorphism of vector spaces maps a basis to a basis. By proposition $\ref{proposition-46}$, the isomorphism $\mathrm{d}\varphi_\mathbf{p}: T_\mathbf{p}M \to T_{\varphi(\mathbf{p})} \R^n$ maps $\Set{\left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}}_{i=1}^n$ to $\Set{\left.\frac{\partial}{\partial r^i}\right|_\mathbf{p}}_{i=1}^n$, which is a basis for $T_{\varphi(\mathbf{p})} \R^n$. Thus, $\Set{\left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}}_{i=1}^n$ is a basis for $T_\mathbf{p} M$.
...
Consider the more general case of a smooth map $F:M\to N$ between smooth manifolds with or without boundary. Choosing smooth coordinate charts $(U,\varphi)$ for $M$ containing $\mathbf{p}\in M$ and $(V,\psi)$ for $N$ containing $F(\mathbf{p})$, we obtain the coordinate representation $\tilde{F} = \psi\circ F\circ\varphi^{-1}: \varphi(U\cap F^{-1}(V)) \to\psi(V)$.
