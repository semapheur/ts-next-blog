...
or equivalently, $X_\mathbf{p}\in T_\mathbf{p}M$ for each $\mathbf{p}\in M$. If $X$ is smooth it is a *smooth vector field*, otherwise it is called a *rough vector field*. The support of $X$ is defined as
