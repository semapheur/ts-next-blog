...
A projective space $P^n (\R)$ is the set  of 1-dimensional subspaces of $\R^{n+1}$. It is defined as the quotient space $P^n := \mathbb{S}^n / \sim$ with the equivalence relation $x \sim y :\iff (x = y \textrm{ or } x = -y)$ and where 

$$
  \mathbf{S}^n := \Set{ x \in \R^{n+1} \,|\, \| x \| = 1 }
$$
...
Considering $\hat{U}:= q[u]$ where $q: \mathbb{S}^n \mapsto \mathbb{S}^n / \sim$ is the canonical projection. Taking the preimage we see that 
...
$$
  \mathbb{S}^n := \Set{\mathbf{x} \in\R^{n+1} | \| \mathbf{x} \| = 1}
$$
...
A point $p\in M$ is called an *interior point* of $M$, i.e. $p\in\mathrm{int}(M)$ if it is in the domain of some interior chart. It is a *boundary point* of $M$, i.e. $p\in\partial M$, if it is in the domain of a boundary chart that sends $p$ to $\partial \mathbb{H}^n$.
...
Suppose that $M$ is a smooth manifold with boundary and $p\in M$. If there is some smooth chart $(U,\varphi)$ for $M$ such that $\varphi(U)\subseteq \mathbb{H}^n$ and $\varphi(p)\in \partial \mathbb{H}^n$, then same is true for every smooth chart whose domain contains $p$.
...
<MathBox title='' boxType='lemma'>
Let $\iota:\mathbb{H}^n \to\R^n$ denote the inclusion map. For any $\mathbf{a}\in\partial \mathbb{H}^n$, the differential $\mathrm{d}\iota_\mathbf{a}: T_\mathbf{a} \mathbb{H}^n \to T_\mathbf{a} \R^n$ is an isomorphism.

<details>
<summary>Proof</summary>

To show that $\mathrm{d}\iota_\mathbf{a}$ is injective, assume that $\mathrm{d}\iota_\mathbf{a} (\mathbf{0}) = 0$. Suppose that $f:\mathbb{H}^n \to\R^n$ is smooth, and let $\tilde{f}$ be any extension of $f$ to a smooth function defined on all of $\R^n$. Then $\tilde{f}\circ\iota = f$ and
...
This is independent of the choice of $\tilde{f}$, because by continuity the derivatives of $\tilde{f}$ at $\mathbf{a}$ are determined by those of $f$ in $\mathbb{H}^n$. It is easy to check that $\mathbf{v}$ is a derivation at $\mathbf{a}$ and that $\mathbf{w} = \mathrm{d}\iota_\mathbf{a} (\mathbf{v})$, so $\mathrm{d}\iota_\mathbf{a}$ is surjective.
</details>
</MathBox>
...
If $M$ is a smooth manifold with boundary and $\mathbf{p}$ is an interior point, the discussion above applies verbatim. For $\mathbf{p}\in\partial M$, the only change that needs to be made is to substitute $\mathbb{H}^n$ for $\R^n$, with the understanding that the notation $\boldsymbol{\partial}_i|_{\varphi(\mathbf{p})}$ can be used interchangeably to denote either an element of $T_{\varphi(\mathbf{p})} \R^n$ or an element of $T_{\varphi(\mathbf{p})} \mathbb{H}^n$, where we consider the isomorphism $\mathrm{d}\iota_{\varphi(\mathbf{p})}: T_{\varphi(\mathbf{p})}\mathbb{H}^n \to T_{\varphi(\mathbf{p})}\R^n$ as an identification. The $n$th coordinte vector $\boldsymbol{\partial}_i|_\mathbf{p}$ should be interpreted as a one-sided derivative in this case. 
...
Suppose that $S\subseteq M$ is an open submanifold, and let $\iota:S\hookrightarrow M$ be the inclusion map. because $S$ is an open submanifold, it follows that it is a smooth manifold with same dimension as $M$ and there has codimension $0$. 
...
<MathBox title='' boxType='propositon'>
Suppose that $M$ is a smooth manifold and that $S\subseteq M$ is an embedded submanifold. If $\Phi:U\to N$ is any local defining map for $S$, then $T_\mathbf{p}S = \ker(\mathrm{d}\Phi_\mathbf{p}): T_\mathbf{p}M \to T_{\Phi(\mathbf{p})}N$ for each $\mathbf{p}\in S\cap U$.

<details>
<summary>Proof</summary>

Recall that we identify $T_\mathbf{p}S$ with the subspace $\mathrm{d}\iota_\mathbf{p}(T_\mathbf{p}S) \subseteq T_\mathbf{p}M$, where $\iota:S\hookrightarrow M$ is the inclusion map. Because $\Phi\circ\iota$ is constant on $S\cap U$, it follows that $\mathrm{d}\Phi_\mathbf{p}\circ\mathrm{d}\iota_\mathbf{p}$ is the zero map from $T_\mathbf{p}S$ to $T_{\Phi(\mathbf{p})}N$, and therefore $\mathrm{ran}(\mathbf{d}\iota_\mathbf{p}) \subseteq \ker(\mathrm{d}\Phi_\mathbf{p})$. On the other hand, $\mathrm{d}\Phi_\mathbf{p}$ is surjective by the definition of a defining map, so the rank-nullity theorem implies that

$$
\begin{align*}
  \dim(\ker(\mathrm{d}\Phi_\mathbf{p})) =& \dim(T_\mathbf{p}M) - \dim(T_{\Phi(\mathbf{p})}N) \\
  =& \dim(T_\mathbf{p}S) = \dim(\mathrm{ran}(\mathrm{d}\iota_\mathbf{p}))
\end{align*}
$$

which implies that $\mathrm{ran}(\mathrm{d}\iota_\mathbf{p}) = \ker(\mathrm{d}\Phi_\mathbf{p})$.
</details>
</MathBox>
...
First, we show that $\tilde{e}$ is an identity for multiplication in $\tilde{G}$. Consider the map $f:\tilde{G}\to\tilde{G}$ defined by $f(x) = \tilde{e}x$. Then the first commutative diagram implies that $\pi\circ f(x) = \pi(\tilde{e})\pi(x) = e\pi(x) = \pi(x)$, so $f$ is a lift of $\pi:\tilde{G}\to G$. The identity map $\mathrm{id}_{\tilde{G}}$ is another lift of $\pi$, and it agrees with $f$ at a point because $f(\tilde{e}) = \tilde{m}(\tilde{e},\tilde{e}) = \tilde{e}$, so the unique lifting property of covering maps implies that $f = \mathrm{id}_{\tilde{G}}$, or equivalently $\tilde{e}x = x$ for all $x\in\tilde{G}$. The same argument shows that $x\tilde{e} = x$.
...
Suppose that $(X_i)_{i=1}^n$ is a smooth local frame for $T\R^n$ over an open subset $U\subseteq \R^n$. Then there is a smooth orthonormal frame $(\mathbf{e}_i)_{i=1}^n$ over $U$ such that for each $\mathbf{p}\in U$

$$
  \mathrm{span}(\mathbf{e}_i|_\mathbf{p})_{i=1}^n = \mathrm{span}(X_i|_\mathbf{p})_{i=1}^n
$$

<details>
<summary>Proof</summary>

Applying the Gram-Schmidt algorithm to the vectors $(X_j|_\mathbf{p})$ at each $\mathbf{p}\in U$, we obtain an $n$-tuple of rough vector fields $(\mathbf{e}_i)_{i=1}^n$ given inductively by

$$
  E_j = \frac{X_j - \sum_{i=1}^{j-1} (X_j \cdot \mathbf{e}_i)\mathbf{e}_i}{|X_j - \sum_{i=1}^{j-1} (X_j \cdot \mathbf{e}_i)\mathbf{e}_i|},\; j=1,\dots,n
$$

For eahc $j=1,\dots,n$ and each $\mathbf{p}\in U$, then $X_j|_\mathbf{p}\notin \mathrm{span}(E_1|_\mathbf{p},\dots,E_{j-1}|_\mathbf{p})$, so the denominator above is a nowhere-vanishing smooth function on $U$. Thus, this formula defines $(\mathbf{e}_i)_{i=1}^n$ as a smooth orthonormal frame on $U$.
</details>
</MathBox>
...
Suppose that $M$ is a smooth $n$-manifold. A small local frame $(\mathbf{e}_i)_{i=1}^n$ for $M$ is called a *commuting frame* if $[\mathbf{e}_i, E_j] = 0$ for all $i$ and $j$. Commuting frames are also known as *holonomic frames*.
</MathBox>

<MathBox title='Canonical form for commuting vector fields' boxType='theorem'>
Let $M$ be a smooth $n$-manifold, and let $(V_i)_{i=1}^k$ be a linearly independent $k$-tuple of smooth commuting vector fields on an open subset $W\subseteq M$. For each $\mathbf{p}\in W$, there exists a smooth coordinate chart $(U, (\mathbb{S}^i))$ centered at $\mathbf{p}$ such that $V_i = \frac{\partial}{\partial \mathbb{S}^i}$ for $i = 1,\dots,k$. If $S\subseteq W$ is an embedded codimension-$k$ submanifold and $\mathbf{p}$ is a point of $S$ such that $T_\mathbf{p}S$ is complementary to the span of $(V_i|_\mathbf{p})_{i=1}^k$, then the coordinates can also be chosen such that $S\cap U$ is the slice defined by $\mathbb{S}^1 =\cdots=\mathbb{S}^k = 0$.
...
$$
\begin{align*}
  \mathrm{d}\Phi_{s_0}\left(\left.\frac{\partial}{\partial \mathbb{S}^i}\right|_{s_0}\right) =& \left.\frac{\partial}{\partial \mathbb{S}^i}\right|_{s_0} f(\Phi(\mathbb{S}^1,\dots,\mathbb{S}^n)) \\
...
$$
  \mathrm{d}\Phi_0 \left( \left.\frac{\partial}{\partial \mathbb{S}^i}\right|_0 \right) = V_i|_\mathbf{p},\; i=1,\dots,k
$$
...
<MathBox title='' boxType='theorem'>
Let $E$ and $E'$ be smooth vector bundles over a smooth manifold $M$, and let $F:E\to E'$ be a smooth bundle homomorphism over $M$. Define subsets $\ker(F \subseteq E)$ and $\mathrm{ran}(F \subseteq E')$ by
...
First suppose that $\gamma$ is smooth, and that $\varphi:[c,d] \to [a,b]$ is a diffeomorphism such that $\tilde{\gamma} = \gamma\circ\varphi$. The fact that $\varphi$ is a diffeomorphism implies that either $\varphi' > 0$ or $\varphi' < 0$ everywhere. Assuming that $\varphi' > 0$, then

$$
\begin{align*}
  L_g (\tilde{\gamma}) =& \int_c^d |\tilde{\gamma}'(t)|_g = \int_c^d \left| \frac{\mathrm{d}}{\mathrm{d}t} (\gamma\circ\varphi)(t) \right|_g \;\mathrm{d}t \\
  =& \int_c^d |\varphi'(t)\gamma'(\varphi(t))|_g \;\mathrm{d}t = \int_c^d |\gamma'(\varphi(t))|_g \varphi'(t)\;\mathrm{d}t \\
  =& \int_a^b |\gamma(t)|_g \;\mathrm{d}s = L_g (\gamma)
\end{align*}
$$

where we have used the substitution $s = \varphi(t)$ and $\mathrm{d}s = \varphi'(t) \mathrm{d}t$.
...
It is immediate from the definition that $d_g (\mathbf{p}, \mathbf{q}) \geq 0$. Because every constant curve segment has length zero, it follows that $d_g (\mathbf{p}, \mathbf{p}) = 0$. Furthermore, $\mathrm{d}_g (\mathbf{p}, \mathbf{q}) = d_g (\mathbf{q}, \mathbf{p})$ follows from the fact that any curve segment from $\mathbf{p}$ to $\mathbf{q}$ can be reparametrized to go from $\mathbf{q}$ to $\mathbf{p}$. Suppose that $\gamma_1$ and $\gamma_2$ are piecewise smooth curve segments from $\mathbf{p}$ to $\mathbf{q}$ and $\mathbf{q}$ to $\mathbf{r}$, respectively, and let $\gamma$ be a piecewise smooth curve segment that first follows $\gamma_1$ and then follows $\gamma_2$. Then
...
Suppose that $\gamma: [a,b]\to M$ is a piecewise smooth curve segment from $\mathbf{p}$ to $\mathbf{q}$. Let $t_0$ be the infimum of all $t\in [a, b]$ such that $\gamma(t) \notin \overline{V}$. It follows that $\gamma(t_0)\in\partial V$ by continuity, and $\gamma(t)\in\overline{V}$ for $a \leq t \leq t_0$. Thus
...
Taking the infimum over all such $\gamma$, we conclude taht $d_g (\mathbf{p}, \mathbf{q}) \geq c\varepsilon > 0$.
...
$$
  \hat{g}(X)(Y) = g(X, Y), \; X, Y\in\mathfrak{X}(M)
$$

Because $\hat{g}(X)(Y)$ is linear over $C^\infty (M)$ as a function of $Y$, it follows from the tensor characterization lemma that $\hat{g}(X)$ is a smooth covector field. Because $\hat{g}(X)$ is linear over $C^\infty (M)$ as a function of $X$, this defines $\hat{g}$ as a smooth bundle homomorphism by the bundle homomorphism characterization lemma. As usual, we use the same symbol for both the pointwise bundle homomorphism $\hat{g}: TM\to T^M$ and the linear map on sections $\hat{g}:\mathfrak{X}(M) \to \mathfrak{X}^* (M)$.
...
In any smooth coordinate $(x^i)$, we can write $g = g_{ij}\;\mathrm{d}x^i \;\mathrm{d}x^j$. Thus, if $X$ and $Y$ are smooth vector fields, we have

$$
  \hat{g}(X)(Y) = g_{ij} X^i Y^j
$$
...
We use the notation $X^\flat = \hat{g}(X)$ to indicate that $\hat{g}(X)$ is obtained from $X$ by lowering an index. This is in analogy to the musical "flat" symbol $\flat$ which signifies lowering of a tone.
...
The most important use of teh sharp operation is to reinstate the gradient as a vector field on Riemannian manifolds. For any smooth real-valued function $f$ on a Riemannian manofld $(M,g)$ with or without boundary, we define a vector field called the *gradient* of $f$ bY

$$
  \nabla f = \mathrm{grad}(f) = (\mathrm{d}f)^\sharp = \hat{g}^{-1}(\mathrm{d}f)
$$

Unraveling the definitions, we see that for any $X\in\mathfrak{X}(M)$, the gradient satisfies

$$
\begin{align*}
  \langle \nabla f, X\rangle_g = \hat{g}(\nabla f)(X) \\
  =& \mathrm{d}f(X) = Xf
\end{align*}
$$

Thus $\nabla f$ is the unique vector field that satisfies $\langle\nabla f, X\rangle_g = Xf$ for every vector field $X$, or equivalently 

$$
  \langle \nabla f, \cdot \rangle_g = \mathrm{d}f
$$

In smooth coordinates, $\nabla f$ ha the expression

$$
  \nabla f = g^{ij}\frac{\partial f}{\partial x^i} \frac{\partial}{\partial x^j}
$$

In particular, this shows that $\nabla f$ is smooth. On $\R^n$ with the Euclidean metric, this reduces to

$$
  \nabla f = \delta^{ij}\frac{\partial f}{\partial x^i}\frac{\partial}{\partial x^j} = \sum_{i=1}^n \frac{\partial f}{\partial x^i}\frac{\partial}{\partial x^i}
$$
...
Assume that $\alpha$ satisfies **(3)**. For any vectors $(\mathbf{v}_i)_{i=1}^k$, the hypothesis implies
...
Thus, $\alpha$ is alternating. On the other hand, if $(\mathbf{v}_i)_{i=1}^k$ is a linearly dependent $k$-tuple, then one of the $\mathbf{v}_i$ can be written as a linear combination of the others. For simplicity, assume that $\mathbf{v}_k = \sum_{j=1}^{k-1} a^j \mathbf{v}_j$. Then multilinearity of $\alpha$ implies
...
$$
  (\mathrm{alt}(\alpha))(\mathbf{v}_1,\dots,\mathbf{v}_k) = \frac{1}{k!}\sum_{\sigma\in S_k} \mathrm{sgn}(\sigma) \alpha(\mathbf{v}_{\sigma(1)},\dots,\mathbf{v}_{\sigma(k)})
$$
</MathBox>
...
Let $V$ be an $n$-dimensional vector space, and suppose that $(\varepsilon^i)_{i=1}^n$ is any basis for $V^*$. For each multi-index $I = (i_1,\dots,i_k)$ of length $k$ such that $1 \leq i_1, \dots, i_k \leq n$, define a covariant $k$-tensor $\varepsilon^I = \varepsilon^{i_1 \dots i_k}$ bY
...
Suppose that $V$ is a finite-dimensional vector space. Let $(\mathbf{e}_i)$ be a basis for $V$, let $(\varepsilon^i)$ be the dual basis $V^*$, and let $\varepsilon^I$ be an elementary alternating tensor.
1. If $I$ has a repeated index, then $\varepsilon^I = 0$
2. If $J = I_\sigma$ for some $\sigma\in S_k$, then $\varepsilon^I = \mathrm{sgn}(\sigma) \varepsilon^J$
...
To shows that $\mathrm{span}(\mathcal{E}) = \Lambda_k (V^*)$, let $\alpha\in\Lambda_k (V^*)$ be arbitrary. For each multi-index $I = (i_1,\dots,i_k)$, define a real number $\alpha_I$ bY

$$
  \alpha_I = \alpha(\mathbf{e}_{i_1},\dots,\mathbf{e}_{i_k})
$$

The fact that $\alpha$ is alternating implies $\alpha_I = 0$ if $I$ contains a repeated index, and $\alpha_J = \mathrm{sgn}(\sigma)\alpha_I$ if $J = I_\sigma$ for $\sigma\in S_k$. For any multi-index $J$, therefore, by the properties of elementary alternating tensors
...
Thus, $\sum'_I \alpha_I \varepsilon^I = \alpha$, implying that $\mathrm{span}(\mathcal{E}) = \Lambda_k (V^*)$.
...
Suppose that $V$ is an $n$-dimensional vector space and $\omega\in\Lambda_n (V^*)$. If $T:V\to V$ is any linear operator and $\mathbf{v}_1,\dots,\mathbf{v}_n$ are arbitrary vectors in $V$, then
...
Suppose that $V$ is a finite-dimensional vector space. If $\omega\in\Lambda_k (V^*)$ and $\eta\in\Lambda_l (V^*)$, their *wedge product* or *exterior product* is defined as a $(k+l)$-covector defined As
...
\begin{align*}
  ...
  =& \frac{1}{k!l!} \sum_{\sigma\in S_{k+l}} \mathrm{sgn}(\sigma) \varepsilon^I (\mathbf{e}_{p}_{\sigma(1)},\dots,\mathbf{e}_{p_{\sigma(k)}})\varepsilon^J (\mathbf{e}_{p_{\sigma(k+1)}},\dots,\mathbf{e}_{p_{\sigma(k+l)}})
\end{align*}
$$

By the properties of elementary alternating tensors, the only terms in the sum above that give nonzero values are those in which $\sigma$ permutes the first $k$ indices and the last $l$ indesc of $P$ separately. In other words, $\sigma$ must be of the form $\sigma = \tau\eta$, where $\tau\in S_k$ act by permuting $\Set{1,\dots,k}$ and $\eta\in S_l$ acts by permuting $\Set{k+1,\dots,k+l}$. Since $\mathrm{sgn}(\tau\eta) = \mathrm{sgn}(\tau)\mathrm{sgn}(\eta)$, we have

$$
\begin{align*}
  &\varepsilon^I \wedge \varepsilon^J (\mathbf{e}_{p_1},\dots,\mathbf{e}_{p_{k+l}}) \\
  =& \frac{1}{k!l!} \sum_{\begin{subarray}{l} \tau\in S_k \\ \eta\in S_l \end{subarray}} \mathrm{sgn}(\tau)\mathrm{sgn}(\eta) \varepsilon^I (\mathbf{e}_{p}_{\sigma(1)},\dots,\mathbf{e}_{p_{\sigma(k)}})\varepsilon^J (\mathbf{e}_{p_{\sigma(k+1)}},\dots,\mathbf{e}_{p_{\sigma(k+l)}}) \\
  =& \left(\frac{1}{k!}\sum_{\tau\in S_k} \mathrm{sgn}(\tau)\varepsilon^I (\mathbf{e}_{p_{\tau(1)}},\dots,\mathbf{e}_{p_{\tau(k)}}) \right) \\
  &\times \left(\frac{1}{l!}\sum_{\eta\in S_l} \mathrm{sgn}(\eta)\varepsilon^J (\mathbf{e}_{p_{k + \eta(1)}},\dots,\mathbf{e}_{p_{k + \eta(l)}}) \right) \\
...
Suppose that $\omega$, $\omega'$, $\eta$, $\eta'$ and $\xi$ are multicovectors on a finite-dimensional vector space $V$.
...
$$
\begin{align*}
  \varepsilon^I \wedge\varepsilon^J =& \varepsilon^{IJ} = \mathrm{sgn}(\tau)\varepsilon^{JI} \\
  =& \mathrm{sgn}(\tau)\varepsilon^J \wedge \varepsilon^I 
\end{align*}
$$

where $\tau$ is the permutation that sends $IJ$ to $JI$. It is easy to check that $\mathrm{sgn}(\tau) = (-1)^{kl}$, because $\tau$ can be decomposed as a composition of $kl$ transposition (each index of $I$ must be moved past each of the indices of $J$). Anticommutativity then follows from bilinearity.
...
Suppose that $M$ is a smooth $n$-manifold and let $T_k T^*M$ be a bundle of covariant $k$-tensors on $M$. The subset of $T_k T^* M$ consisting of alternating tensors is denoted by $\Lambda_k T^* M$:
...
$$
  \omega = \sum'_{I} \omega_I \;\mathrm{d}x^{i_1} \wedge\cdots\wedge\mathrm{d}x^{i_k} = \sum'_I \omega_I \mathrm{d}x^I
$$
...
$$
  \mathrm{d}x^{i_1} \wedge\cdots\wedge \mathrm{d}x^{i_k} \left( \frac{\partial}{\partial x^{j_i}},\dots,\frac{\partial}{\partial x^{j_k}} \right) = \delta_J^I
$$
...
$$
  (F^* \omega)_\mathbf{p} (\mathbf{v}_1,\dots,\mathbf{v}_k) = \omega_{F(\mathbf{p})} (\mathrm{d}F_\mathbf{p}(\mathbf{v}_1),\dots,\mathrm{d}F_\mathbf{p}(\mathbf{v}_k))
$$

<MathBox title='Properties of pullbacks of differential forms' boxType='lemma'>
Let $M$ and $N$ be smooth manifolds and suppose that $F:M\to N$ is smooth
1. $F^*: \Omega^k (N) \to \Omega^k (M)$ is linear over $\R$
2. $F^* (\omega\wedge\eta) = (F^* \omega)\wedge(F^* \eta)$
3. In any smooth chart
$$
  F^* \left( \sum'_I \omega_I \mathrm{d}y^{i_1} \wedge\cdots\wedge \mathrm{d}y^{i_k} \right) = \sum'_I (\omega_I \circ F) \mathrm{d}(y^{i_1}\circ F) \wedge\cdots\wedge \mathrm{d}(y^{i_k} \circ F)
$$
</MathBox>

<MathBox title='Pullback formula for top-degree forms' boxType='proposition'>
...
where $\mathrm{D}F$ represents the Jacobian matrix of $F$ in these coordinates. 

<details>
<summary>Proof</summary>

Because the fibre of $\Lambda_n T^* M$ is spanned by $\mathrm{d}x^1 \wedge\cdots\wedge \mathrm{d}x^n$ at each point, it suffices to show that both sides of the stated equation give the same result when evaluated on $\left(\frac{\partial}{\partial x^1},\dots,\frac{\partial}{\partial x^n} \right)$. From properties of pullbacks of differential form, we get

$$
  F^* (u\mathrm{d}y^1 \wedge\cdots\wedge \mathrm{d}y^n) = (u\circ F)\mathrm{d}F^1 \wedge\cdots\wedge \mathrm{d}F^n
$$
...
$$
\begin{align*}
  \mathrm{d}F^1 \wedge\cdots\wedge \mathrm{d}F^n \left( \frac{\partial}{\partial x^1},\dots,\frac{\partial}{\partial x^n} \right) =& \det\left(\mathrm{d}F^j \left(\frac{\partial}{\partial x^i}\right) \right) \\
  =& \det\left(\frac{\partial F^j}{\partial x^i}\right)
\end{align*}
$$

Thus, the left hand side of the stated equation gives $(u\circ F)\det(\mathrm{D}F)$ when applied to $\left(\frac{\partial}{\partial x^n},\dots,\frac{\partial}{\partial x^n} \right)$. On the other hand, the right-hand side gives the same thing because

$$
  \mathrm{d}x^1 \wedge\cdots\wedge \mathrm{d}x^n \left(\frac{\partial}{\partial x^1},\dots,\frac{\partial}{\partial x^n} \right) = 1
$$
</details>
</MathBox>
...
<details>
<summary>Proof</summary>
This follows from the previous theorem with $F$ equal to the identity map of $U\cap\tilde{U}$, but using coordinates $(x^i)$ in the domain and $(\tilde{x}^j)$ in the codomain.
</details>
</MathBox>

Interior multiplication also extends naturally to vector fields and differential form, by letting it act pointwise: if $X\in\mathfrak{X}(M)$ and $\omega\in\Omega^k (M)$, define a $(k-1)$-form $X \lrcorner\omega = i_X \omega$ bY

$$
  (X \lrcorner\omega)_\mathbf{p} = X_\mathbf{p} \lrcorner \omega_\mathbf{p}
$$

## Exterior derivatives

Recall that given a smooth $1$-form $\omega$, a necessary condition for the existence of a smooth function $f$ such that $\omega = \mathrm{d}f$ is that $\omega$ is *closed*, satisfying

$$
  \frac{\partial\omega_j}{\partial x^i} - \frac{\partial\omega_i}{\partial x^j} = 0
$$

in every smooth coordinate chart. Since the left-hand side is antisymmetric in the indices $i$ and $j$, it can be interpreted as the $ij$-component of an alternating tensor field, i.e. a $2$-form. We can define a $2$-form $\mathrm{d}\omega$ locally in each smooth chart by
...
It follows that $\omega$ is closed if and only if $\mathrm{d}\omega = 0$ in each chart. It turns out that $\mathrm{d}\omega$ is well defined globally, independently of the choice of coordinate chart, and this definition has a significant generalization to differential forms of all degrees.

For each smooth manifold $M$ with or without boundary, we can show that there is a differential operator $\mathrm{d}:\Omega^k (M) \to\Omega^{k+1}(M)$ satisfying $\mathrm{d}(\mathbf{d}\omega) = 0$ for all $\omega$. Thus, it will follow that a necessary condition for a smooth $k$-form $\omega$ to be equal to $\mathrm{d}\eta$ for some $(k-1)$-form $\eta$ is that $\mathrm{d}\omega = 0$.

<MathBox title='Exterior derivative on Euclidean space' boxType='definition'>
If $\omega = \sum'_J \omega_J \mathrm{d}x^J$ is a smooth $k$-form on an open subset $U\subseteq\R^n$ or $\mathbf{H}^n$, we define its *exterior derivative* $\mathrm{d}\omega$ to be the following $(k+l)$-form

$$
  \mathrm{d}\left( \sum'_J \omega_J \mathrm{d}x^J \right) = \sum'_J \mathrm{d}\omega_J \wedge \mathrm{d}x^J
$$

where $\mathrm{d}\omega_J$ is the differential of the function $\omega_J$. In more detail, this is

$$
  \mathrm{d}\left( \sum'_J \omega_J \bigwedge_{h=1}^k \mathrm{d}x^{j_h} \right) = \sum'_J \sum_i \frac{\partial\omega_J}{\partial x^i} \mathrm{d}x^i \wedge \left(\bigwedge_{h=1}^k \mathrm{d}x^{j_h} \right)
$$
...
$$
\begin{align*}
  \mathrm{d}(\omega_j \mathrm{d}x^j) =& \sum_{i,j} \frac{\partial\omega_j}{\partial x^i} \mathrm{d}x^i \wedge\mathrm{d}x^j \\
  =& \sum_{i<j} \frac{\partial\omega_j}{\partial x^i} \mathrm{d}x^i \wedge\mathrm{d}x^j + \sum_{i>j} \frac{\partial\omega_j}{\partial x^i} \mathrm{d}x^i \wedge\mathrm{d}x^j \\
...
$$
3. $\mathrm{d}\circ\mathrm{d}\equiv 0$
4. $d$ commutes with pullbacks: if $U$ is an open subset of $\R^n$ or $\mathbf{H}^n$, $V$ is an open subset of $\R^m$ or $\H^m$, $F:U\to V$ is a smooth map, and $\omega\in\Omega^k (V)$, then
...
**(2):** It suffices to consider terms of the form $\omega = u\mathrm{d}x^I \in\Omega^k (U)$ and $\eta = v\mathrm{d}x^J \in \Omega^l (U)$ for smooth real-valued functions $u$ and $v$. First we need to know that $\mathrm{d}$ satisfies $\mathrm{d}(u\mathrm{d}x^I) = \mathrm{d}u \wedge \mathrm{d}x^I$ for any multi-index $I$, not just increasing ones. If $I$ has repeated indices, then clearly $\mathrm{d}(u\;\mathrm{d}x^I) = 0 = \mathrm{d}u \wedge \mathrm{d}x^I$. If not, let $\sigma$ be the permutation sending $I$ to an increasin multi-index $J$. Then

$$
\begin{align*}
  \mathrm{d}(u\;\mathrm{d}x^I) = \mathrm{sgn}(\sigma)\mathrm{d}(u\;\mathrm{d}x^J) \\
  =& \mathrm{sgn}(\sigma) \mathrm{d}u \wedge \mathrm{d}x^J = \mathrm{d}u \wedge \mathrm{d}x^I
\end{align*}
$$
...
where the $(-1)^k$ comes from the fact that $\mathrm{d}v \wedge\mathrm{d}x^I = (-1)^k \;\mathrm{d}x^I \wedge\mathrm{d}v$ because $\mathrm{d}v$ is a $1$-form and $\mathrm{d}x^I$ is a $k$-form.

**(3):** In the case of a $0$-form, which is just a real-valued function, we get

$$
\begin{align*}
  \mathrm{d}(\mathrm{d}u) =& \mathrm{d}\left(\frac{\partial u}{\partial x^j} \mathrm{d}x^j \right) = \frac{\partial^2 u}{\partial x^i \partial x^j}\mathrm{d}x^i \wedge \mathrm{d}x^j \\
  =& \sum_{i < j} \left( \underbrace{\frac{\partial^2 u}{\partial x^i x^j} - \frac{\partial^2 u}{\partial x^j x^i}}{=0} \right) \mathrm{d}x^i \wedge \mathrm{d}x^j = 0
\end{align*}
$$
...
$$
\begin{align*}
  \mathrm{d}(\mathrm{d}\omega) =& \mathrm{d}\left( \sum'_J \mathrm{d}\omega_J \wedge \mathrm{d}x^{j_1} \wedge\cdots\wedge \mathrm{d}x^{j_k} \right) \\
  =& \sum'_J \mathrm{d}(\mathrm{d}\omega_J) \wedge \mathrm{d}x^{j_1} \wedge\cdots\wedge \mathrm{d}x^{j_k} \\
  &+ \sum'_J \sum_{i=1}^k (-1)^i \mathrm{d}\omega_J \wedge\mathrm{d}x^{j_1} \wedge\cdots\wedge \mathrm{d}(\mathrm{d}x^{j_i}) \wedge\cdots\wedge \mathrm{d}x^{j_k} \\
  =& 0
\end{align*}
$$

**(4):** It suffices to consider $\omega = u \;\mathrm{d}x^{i_1} \wedge\cdots\wedge \mathrm{d}x^{i_k}$. For such a form, the left-hand side of $F^* (\mathrm{d}\omega) = \mathrm{d}(F^* \omega)$ is

$$
\begin{align*}
  F^* (\mathrm{d}(u \mathrm{d}x^{i_1} \wedge\cdots\wedge \mathrm{d}x^{i_k})) =& F^* (\mathrm{d}u \wedge \mathrm{d}x^{i_1} \wedge\cdots\wedge \mathrm{d}x^{i_k}) \\
...
$$
  \mathrm{d}(F^* (u\;\mathrm{d}x^{i_1} \wedge\cdots\wedge\mathrm{d}x^{i_k})) =& \mathrm{d}((u\circ F)\mathrm{d}(x^{i_1}\circ F)\wedge\cdots\wedge\mathrm{d}(x^{ik}\circ F)) \\
  =& \mathrm{d}(u\circ F) \wedge \mathrm{d}(x^{i_1} \circ F)\wedge\cdots\wedge \mathrm{d}(x^{i_k}\circ F)
$$
</details>
</MathBox>
...
Suppose that $M$ is a smooth manifold with or without boundary. There are unique operators $\mathrm{d}:\Omega^k (M) \to\Omega^{k+1}(M)$ for all $k\in\N_+$, called *exterior differentiation*, satisfying the following properties
...
$$
3. $\mathrm{d}\circ\mathrm{d} \equiv 0$
4. For $f\in\Omega^0 (M) = C^\infty(M)$, $\mathrm{d}f$ is the differential of $f$, given by $\mathrm{d}f(X) = Xf$

<details>
<summary>Proof</summary>

First, we prove existence. Suppose that $\omega\in\Omega^k (M)$. We seek to define $\mathrm{d}\omega$ by means of the coordinate formula $\mathrm{d}\left( \sum'_J \omega_J \mathrm{d}x^J \right) = \sum'_J \mathrm{d}\omega_J \wedge \mathrm{d}x^J$ in each chart. More precisely, this means that for each smooth chart $(U,\varphi)$ for $M$, we seek to set

$$
  \mathrm{d}\omega = \varphi^* \mathrm{d}(\varphi^{-1}* \omega)
$$
...
$$
  (\varphi\circ\psi^{-1})^* \mathrm{d}(\varphi^{-1*}\omega) = \mathrm{d}((\varphi\circ\psi^{-1})^* \varphi^{-1*}\omega)
$$

Together with the fact that $(\varphi\circ\psi^{-1}) = \psi^{-1*}\circ\varphi^*$, this implies that $\varphi^* \mathrm{d}(\varphi^{-1*}\omega) = \psi^* \mathrm{d}(\psi^{-1*}\omega)$, so $\mathrm{d}\omega$ is well defined. It satisfies **(1)**-**(4)** by virtue of the properties for Euclidean exteriori derivative.

To prove uniqueness, suppose that $\mathrm{d}$ is any operator satisfying **(1)**-**(4)**. First, we need to show that $\mathrm{d}\omega$ is determined locally: if $\omega_1$ and $\omega_2$ are $k$-forms that agree on an open subset $U\subseteq M$, the $\mathrm{d}\omega_1 = \mathrm{d}\omega_2$ on $U$. To see this, let $\mathbf{p}\in U$ be arbitrary, let $\eta = \omega_1 - \omega_2$, and let $\psi\in C^\infty (M)$ be a bump function with $\psi\equiv 1$ on some neighbourhood of $\mathbf{p}$ and supported in $U$. Then $\psi\eta\equiv 0$, so **(1)**-**(4)** imply $0 = \mathrm{d}(\psi\eta) = \mathrm{d}\psi\wedge\eta + \psi\mathrm{d}\eta$. Evaluating this at $\mathbf{p}$ and using the facts that $\psi(\mathbf{p}) = 1$ and $\mathrm{d}\psi_\mathbf{p} = 0$, we conclude that $\mathrm{d}\omega_1|_\mathbf{p} - \mathrm{d}\omega_2|_\mathbf{p} = \mathrm{d}\eta_\mathbf{p} = 0$.

Let $\omega\in\Omega^k (M)$ be arbitrary, and let $(U,\varphi)$ be any smooth coordinate chart on $M$. We can write $\omega$ in coordinates as $\sum'_I \omega_I \mathrm{d}x^I$ on $U$. For any $\mathbf{p}\in U$, by means of a bump function we can construct global smooth functions $\tilde{omega}_I$ and $\tilde{x}^i$ on $M$ that agree with $\omega_I$ and $x^i$ in a neighbourhood of $\mathbf{p}$. By virtue of **(1)**-**(4)** together with the observation in the preceding paragraph, it follows that $\mathrm{d}\left( \sum'_J \omega_J \mathrm{d}x^J \right) = \sum'_J \mathrm{d}\omega_J \wedge \mathrm{d}x^J$ holds at $\mathbf{p}$. Since $\mathbf{p}$ was arbitrary, this $\mathrm{d}$ must be equal to the one we defined above.
</details>
</MathBox>
...
Suppose that $M$ and $N$ are smooth manifolds. If $F:M\to N$ is a smooth map, then for each $k$ the pullback map $F^*: \Omega^k (N) \to\Omega^k (M)$ commutes with $d$, i.e.

$$
  F^* (\mathrm{d}\omega) = \mathrm{d}(F^* \omega),\; \forall\omega\in\Omega^k (N)
$$

<details>
<summary>Proof</summary>

If $(U,\varphi)$ and $(V,\psi)$ are smooth charts for $M$ and $N$, respectively, we can apply the null property of the wedge product to the cordinate representation $\psi\circ F\circ\varphi^{-1}$. Using $\mathrm{d}\omega = \varphi^* \mathrm{d}(\varphi^{-1*}\omega)$ twice, we compute as follows on $U\cap F^{-1}(V)$

$$
\begin{align*}
  F^* (\mathrm{d}\omega) =& F^* \psi^* \mathrm{d}(\psi^{-1*}\omega) \\
  =& \varphi^* \circ (\psi\circ F\circ\varphi^{-1})^* \mathrm{d}(\psi^{-1*}\omega) \\
  =& \varphi^* \mathrm{d}((\psi\circ F \circ\varphi^{-1})^* \psi^{-1*}\omega) \\
  =& \varphi^* \mathrm{d}(\varphi^{-1*} F^* \omega) \\
  =& \mathrm{d}(F^* \omega)
\end{align*}
$$
</details>
...
A smooth differential form $\omega\in\Omega^k (M)$ is closed if $\mathrm{d}\omega$ and exact if there exists a smooth $(k-1)$-form $\eta$ on $M$ such that $\omega = \mathrm{d}\eta$. The fact that $\mathrm{d}\circ\mathrm{d}\equiv 0$ implies that every exact form is closed.
...
for some smooth functions $P, Q, R$. Using $\mathrm{d}\left( \sum'_J \omega_J \mathrm{d}x^J \right) = \sum'_J \mathrm{d}\omega_J \wedge \mathrm{d}x^J$ and the fact that any $1$-form with itself is zero, we compute
...
\begin{align*}
  ...
  &+ \left(\frac{\partial R}{\partial x}\mathrm{d}x + \frac{\partial R}{\partial y}\mathrm{d}y + \frac{\partial R}{\partial z}\mathrm{d}z \right)\wedge\mathrm{d}y \\
  =& \left(\frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y}\right)\mathrm{d}x \wedge\mathrm{d}y + \left( \frac{\partial R}{\partial x} - \frac{\partial P}{\partial z} \right)\mathrm{d}x \wedge\mathrm{d}z \\
  &+ \left( \frac{\partial R}{\partial y} - \frac{\partial Q}{\partial z} \right)\mathrm{d}y \wedge\mathbf{d}z
\end{align*}
$$
...
Note that the components of the $2$-form $\mathrm{d}\omega$ corresponds (execpt in ordering and sign) with the curl of a vector field with components $(P,Q,R)$. Similarly, there is a strong correspondence between the $3$-form $\mathrm{d}\eta$ and the divergence of a vector field. These analogies can be justified by defining the following maps.
...
$$
\begin{CD}
  C^\inft (\R^3) @>{\mathrm{grad}}>> \mathfrak{X}(\R^3) @>{\mathrm{curl}}>> \mathfrak{X}(\R^3) @>{\mathrm{div}}>> C^\infty (\R^3) \\
  @VV{\mathrm{id}}V @VV{\flat}V @VV{\beta}V @VV{*}V \\
  \Omega^0 (\R^3) @>>{\mathrm{d}}> \Omega^1 (\R^3) @>>{\mathrm{d}}> \Omega^2 (\R^3) @>>{\mathrm{d}}> \Omega^3 (\R^3)
\end{CD}
$$
...
Since any smooth $1$-form can be expressed locally as a sum of terms of the form $u\;\mathrm{d}v$ for smooth functions $u$ and $v$, it suffices to consider that case. Suppose that $\omega = u\;\mathrm{d}v$, and that $X$ and $Y$ are smooth vector fields. Then the left-hand side of the formula is

$$
\begin{align*}
  \mathrm{d}(u\;\mathrm{d}v)(X, Y) =& \mathrm{d}u \wedge \mathrm{d}v(X,Y) \\
  =& \mathrm{d}u(X) \mathrm{d}v(Y) - \mathrm{d}v(X)\mathrm{d}u(Y) \\
  =& XuYv - XvYu
\end{align*}
$$
...
$$
\begin{align*}
  &X(u\;\mathrm{d}v(Y)) - Y(u\;\mathrm{d}v(X)) - u\;\mathrm{d}v([X,Y]) \\
  =& X(uYv) - Y(uXv) - u[X,Y]v \\
...
$$
  \mathrm{d}\varepsilon^i = \sum_{j < k} b_{jk}^i \varepsilon^j \wedge\varepsilon^k,\; [E_j, E_k] = c_{jk}^i \mathbf{e}_i
$$
...
$$
  \mathrm{D}\omega(X_1,\dots,X_{k+1}) = I(X_1,\dots,X_{k+1}) + II(X_1,\dots,X_{k+1})
$$

where $I$ and $II$ denote the two sum on the right-hand side. Note that $\mathrm{D}\omega$ is obviously multilinear over $\R$. We begin by showing that it is actually multilinear over $C^\infty (M)$, which is to say that for $1\leq p \leq k+1$ and $f\in C^\infty (M)$

$$
\begin{align*}
  &\mathrm{D}\omega(X_1,\dots,fX_\mathbf{p},\dots,X_{k+1}) \\
  =& f\mathrm{D}\omega(X_1,\dots,X_p,\dots, X_{k+1})
\end{align*}
$$

In the expansion of $I(X_1,\dots,fX_p,\dots,X_{k+1})$, $f$ obviously factors out of the $i = p$ term. The other terms expand as follows

$$
\begin{align*}
  &\sum_{i\neq p} (-1)^{i-1} X_i(f\omega(X_1,\dots,\hat{X}_i,\dots,X_{k+1})) \\
  =& \sum_{i\neq p} (-1)^{i-1} (fX_i (\omega(X_1,\dots,\hat{X}_i,\dots,X_{k+1}))) \\
...
$$
\begin{align*}
  [fX_p, X_j] =& f[X_p, X_j] - (X_j f)X_p \\
  [X_i, fX_p] =& f[X_i, X_p] + (X_i f)X_p
\end{align*}
$$
</details>
</MathBox>
...
\begin{align*}
  ...
  &+ \sum_{p < j} (-1)^{p+j+1} (X_j f)\omega (X_p, X_1,\dots,\hat{X}_p,\dots,\hat{X}_j,\dots,X_{k+1}) \\
  &+ \sum_{i < p} (-1)^{i + p} (X_i f)\omega (X_p, X_1,\dots,\hat{X}_i,\dots,\hat{X}_p,\dots,X_{k+1})
\end{align*}
$$

Rearranging the arguments in these two sums so as put $X_p$ into its original position, we see that they exactly cancel the sum $I$. This completes the proof that $\mathrm{D}\omega$ is multilinear over $C^\infty (M)$, so it defines a smooth $(k+1)$-tensor field.

Since both $\mathrm{D}\oemga$ and $\mathrm{d}\omega$ are smooth tensor fields, we can verify the equation $\mathrm{D}\omega = \mathrm{d}\omega$ in any frame that is convenient. By multilinearity, it suffices to show that both sides give the same result when applied to an arbitrary sequence of basis vectors in some chosen local frame in a neighbourhood of each point. The computations are greatly simplified by working in a coordinate frame, for which all the Lie brackset vanish. Thus, let $(U,(x^i))$ be an arbitrary smooth chart on $M$. Because both $\mathrm{d}\omega$ and $\mathrm{D}\omega$ depend linearly on $\omega$, we may assume that $\omega = u\;\mathrm{d}x^I$ for some smooth function $u$ and some increasing multi-index $I = (i_1,\dots,i_k)$ so

$$
\begin{align*}
  \mathrm{d}\omega =& \mathrm{d}u\wedge\mathrm{d}x^I \\
  =& \sum_m \frac{\partial u}{\partial x^m}\mathrm{d}x^m \wedge\mathrm{d}x^I
\end{align*}
$$
...
$$
  \mathrm{d}\omega\left(\frac{\partial}{\partial x^{j_1}},\dots,\frac{\partial}{\partial x^{j_k + 1}} \right) = \sum_m \frac{\partial u}{\partial x^m} \delta_J^{mI}
$$
...
$$
  \mathrm{d}\omega\left(\frac{\partial}{\partial x^{j_1}},\dots,\frac{\partial}{\partial x^{j_k +1}} \right) = \sum_{1\leq p\leq k + 1} (-1)^{p-1}\frac{\partial u}{\partial x^{j_p}} \delta_{\hat{J}_p}^I
$$
...
$$
\begin{align*}
  &\mathrm{D}\omega\left(\frac{\partial}{\partial x^{j_1}},\dots,\frac{\partial}{\partial x^{j_k + 1}}\right) \\
  =& \sum_{1\leq p \leq k+1} (-1)^{p-1} \frac{\partial}{\partial x^{j_p}}\left(u\;\mathrm{d}x^I \left(\frac{\partial}{\partial x^{j_1}},\dots,\hat{\frac{\partial}{\partial x^{j_p}}},\dots,\frac{\partial}{\partial x^{j_{k+1}}} \right) \right) \\
...
<MathBox title='' boxType='proposition'>
Suppose that $M$ is a smooth manifold, $V\in\mathfrak{X}(M)$, and that $\omega,\eta\in\Omega^* (M)$. Then
...
$$
\begin{align*}
  V\lrcorner (\mathrm{d}f) + \mathrm{d}(V\lrcorner f) =& V\lrcorner\mathrm{d}f \\
  =& \mathrm{d}f(V) = Vf = \mathcal{L}_V f
\end{align*}
$$

which agrees with the formula.

Let $k\geq 1$, and suppose that Cartan's magic formula holds for forms of degrees less than $k$. Let $\omega$ be an arbitrary smooth $k$-form, written in smooth local coordinates as

$$
  \omega = \sum'_I \omega_I \;\mathrm{d}x^{i_1} \wedge\cdots\wedge \mathrm{d}x^{i_k}
$$

Writing $u = x^{i_1}$ and $\beta = \omega_I \mathrm{d}x^{i^2} \wedge\cdots\wedge \mathrm{d}x^{i_k}$, we see that each term in this sum can be written in the form $\mathrm{d}u \wedge\beta$, where $u$ is a smooth function and $\beta$ is a smooth $(k-1)$-form. Because $\mathcal{L}_V \mathrm{d}u = \mathrm{d}(\mathcal{L}_V u) = \mathrm{d}(Vu)$, the Lie product rule and the induction hypothesis imply

$$
\begin{align*}
  \mathcal{L}_V (\mathrm{d}u \wedge\beta) =& (\mathcal{L}_V \mathrm{d}u)\wedge\beta + \mathrm{d}u\wedge (\mathcal{L}_V \beta) \\
  =& \mathrm{d}(Vu)\wedge\beta + \mathrm{d}u\wedge (V\lrcorner \mathrm{d}\beta + \mathrm{d}(V \lrcorner \beta))
\end{align*}
$$

On the other hand, using the facts that both $\mathrm{d}$ and interior multiplication by $V$ are antiderivatives, and $V \lrcorner\mathrm{d}u = \mathrm{d}u(V) = Vu$, we get

$$
\begin{align*}
  &V\lrcorner \mathrm{d}(\mathrm{d}u\wedge\beta) + \mathrm{d}(V\lrcorner (\mathrm{d}u\wedge\beta)) \\
  =& V \lrcorner (-\mathrm{d}u \wedge \mathrm{d}\beta) + \mathrm{d}((Vu)\beta - \mathrm{d}u \wedge (V\lrcorner\beta)) \\
  =& -(Vu)\mathrm{d}\beta + \mathrm{d}u\wedge (V\lrcorner\mathrm{d}\beta) + \mathrm{d}(Vu)\wedge\beta \\
  &+ (Vu)\mathrm{d}\beta + \mathrm{d}u \wedge\mathrm{d}(V \lrcorner\beta) \\
  =& \mathrm{d}(Vu)\wedge\beta + \mathrm{d}u\wedge (V\lrcorner \mathrm{d}\beta + \mathrm{d}(V \lrcorner \beta))
\end{align*}
$$
</details>
...
<MathBox title='The Lie derivative commutes with $\mathrm{d}$' boxType='corollary'>
...
This follows from Cartan's formula and the fact that $\mathrm{d}\circ\mathrm{d} = 0$:
...
\begin{align*}
  ...
  \mathrm{d}(\mathcal{L}_V \omega) =& \mathrm{d}(V\lrcorner\mathrm{d}\omega) + \mathrm{d}(\mathrm{d}(V\lrcorner\omega)) = \mathrm{d}(V\lrcorner\mathrm{d}\omega)
\end{align*}
$$
</details>
</MathBox>
...
The $0$-dimensional case is immediate, since a nonzero element of $\Lambda_0 (V^*)$ is just a nonzero real number. Thus, we may assume that $n\geq 1$. Let $\omega\in\Lambda_n (V^*)$ be nonzero, and let $\mathcal{O}_\omega$ denote the set of ordered bases on which $\omega$ gives positive values. We need to show that $\mathcal{O}_\omega$ is exactly one equivalence class.

Suppose that $(\mathbf{e}_i)$ and $(\tilde{\mathbf{e}}_i)$ are any two ordered bases for $V$, and let $B:V\to V$ be the linear map sending $\mathbf{e}_j$ to $\tilde{\mathbf{e}}_j$. This means that $\tilde{\mathbf{e}}_j = B\mathbf{e}_j = B_j^i \mathbf{e}_i$, so $B$ is the transition matrix between the two bases such that
...
Suppose that $M$ is an oriented smooth manifold with or without boundary, and that $D\subseteq M$ is a smooth codimension-$0$ submanifold with or without boundary. Then the orientation of $M$ restricts to an orientation of $D$. If $\omega$ is an orientation form for $M$, then $\iota_D^* \omega$ is an orientation form for $D$.
</MathBox>

<MathBox title='Orientation-preserving and orientation-reversing maps' boxType='definition'>
Let $M$ and $N$ be oriented smooth manifolds with or without boundary, and suppose that $F:M\to N$ is a local diffeomorphism. If $M$ and $N$ are positive-dimensional, we say that $F$ is *orientation-preserving* if for each $\mathbf{p}\in M$, the isomorphism $\mathrm{d}F_\mathbf{p}$ takes oriented bases of $T_\mathbf{p}M$ to oriented bases of $T_{F(\mathbf{p})} N$, and *orientation-reversing* if it takes oriented bases of $T_\mathbf{p}M$ to negatively oriented bases of $T_{F(\mathbf{p})} N$. If $M$ and $N$ are $0$-manifolds, then $F$ is orientation-preserving if for every $\mathbf{p}\in M$, the points $\mathbf{p}$ and $F(\mathbf{p})$ have the same orientation; and it is orientation-reversing if they have the opposite orientation.
</MathBox>

<MathBox title='The pullback orientation' boxType='proposition'>
Suppose that $M$ and $N$ are smooth manifold with or without boundary. If $F:M\to N$ is a local diffeomorphism and $N$ is oriented, then $M$ has a unique orientation, called the *pullback orientation* induced by $F$, such that $F$ is orientation-presevering.
...
For each $\mathbf{p}\in M$, there is a unique orientation on $T_\mathbf{p}M$ that makes the isomorphism $\mathrm{d}F_\mathbf{p}:T_\mathbf{p}M \to T_{F(\mathbf{p})} N$ is orientation-preserving. This defines a pointwise orientation on $M$, and provided that it is continuous, it is the unique orientation on $M$ with respect to which $F$ is orientation-preserving. To see that it is continuous, just choose a smooth orientation form $\omega$ for $N$ and note that $F^* \omega$ is a smooth orientation form for $M$.
</details>
</MathBox>
...
<MathBox title='' boxType='proposition'>
Let $M$ be an oriented smooth manifold, and suppose that $S\subseteq M$ is a regular level set of a smooth function $f:M\to\R$. Then $S$ is orientable.

<details>
<summary>Proof</summary>

Choose any Riemannian metric on $M$, and let $N = \nabla f|_$. The hypotheses imply that $N$ is a nowhere tangent vector field along $S$, so the result follows from the previous proposition.
</details>
</MathBox>
...
<MathBox title='' boxType='lemma'>
Let $M$ be an oriented smooth $n$-manifold with boundary. Suppose that $U\subseteq\R^{n-1}$ is open, $a,b\in\R$ with $a<b$, and $F:(a,b]\times U\to M$ is a smooth embedding that restricts to an embedding of $\Set{b}\times U$ into $\partial M$. Then the parametrization $f:U\to\partial M$ given by $f(\mathbf{x}) = F(b,\mathbf{x})$ is orientation-preserving for $\partial M$ if and only if $F$ is orientation-preserving for $M$.

<details>
<summary>Proof</summary>

Let $\mathbf{x}\in U$ be arbitrary, and let $\mathbf{p} = f(\mathbf{x}) = F(b,\mathbf{x}) \in\partial M$. The hypothesis that $F$ is an embedding means that the linear map $\mathrm{d}F_{(b,\mathbf{x})}: (T_b \R \otimes T_\mathbf{x}\R^{n-1} \to T_\mathbf{p} M$ is bijective. Since the restriction of $\mathrm{d}F_{(b,\mathbf{x})}$ to $T_\mathbf{x}\R^{n-1}$ is equal to $\mathrm{d}f_\mathbf{x}: T_\mathbf{x}\R^{n-1} \to T_\mathbf{p}\partial M$, which is already injective, it follows that $\mathrm{d}F\left( \frac{\partial}{\partial s|_{(b,\mathbf{x})}} \right) \notin T_\mathbf{p}\partial M$ (where $s$ denoted the coordinate on $(a,b]$).
...
This curve satisfies $\gamma(0) = \mathbf{p}$ and $\gamma'(0) = -\mathrm{d}F\left(\frac{\partial}{\partial s|_{(b,\mathbf{x})}}\right)\notin T_\mathbf{p}\partial M$. It follows that $-\mathrm{d}F\left(\frac{\partial}{\partial s|_{(b,\mathbf{x})}}\right)$ is inward-pointing, and therefore $\mathrm{d}F\left(\frac{\partial}{\partial s|_{(b,\mathbf{x})}}\right)$ is outward-pointing.

The definition of the induced orientation yields the following equivalences
1. $F$ is orientation-preserving for $M$
2. $\left(\mathrm{d}F\left(\frac{\partial}{\partial s}, \mathrm{d}F\left(\frac{\partial}{\partial x^1}\right),\dots,\mathrm{d}F\left(\frac{\partial}{\partial x^{n-1}}\right)\right)\right)$ is oriented for $TM$
3. $\left(\mathrm{d}F\left(\frac{\partial}{\partial x^1}\right),\dots,\mathrm{d}F\left(\frac{\partial}{\partial x^{n-1}}\right)\right)\right)$ is oriented for $T\partial M$
4. $\left(\mathrm{d}f\left(\frac{\partial}{\partial x^1}\right),\dots,\mathrm{d}f\left(\frac{\partial}{\partial x^{n-1}}\right)\right)\right)$
...
To prove existence, we would like to define $\omega_g$ in a neighbourhood of each point by $\omega_g = \bigwedge_{i=1}^n \varepsilon^i$, so we need to check that this definition is independent of the choice of oriented orthonormal. If $(\tilde{\mathbf{e}}_1,\dots,\tilde{\mathbf{e}}_n)$ is another oriented frame, with dual coframe $(\tilde{\varepsilon}^i)$ let $\tilde{\omega}_g = \bigwedge_{i=1}^n \tilde{\varepsilon}^i$. We can write $\tilde{E}_i = A_i^j E_j$ for some matrix $A$ of smooth functions. The fact that both frames are orthonormal means that $(A_i^j (\mathbf{p}))\in O(n)$ for each $\mathbf{p}$, so $\det(A_i^j) = \pm 1$, and the fact that the two frames are consistently oriented forces the positive sign. We compute

$$
\begin{align*}
  \omega_g (\tilde{e}_1,\dots,\tilde{e}_n) =& \det(\varepsilon^j (\tilde{e}_i)) \\
  =& \det(A_i^j) = 1 \\
  =& \omega_g (\tilde{e}_1,\dots,\tilde{e}_n)
\end{align*}
$$
...
The last expression is the $(i,j)$-entry of the matrix product $A^T A$, where $A = (A_i^j)$. thus

$$
\begin{align*}
  \det(g_{ij}) =& \det(A^T A) = \det(A^T) \det(A) \\
  \det(A)^2
\end{align*}
$$

from which it follows that $f = \det(A) = \pm\sqrt{\det(g_{ij})}$. Since both frame $\left(\frac{\partial}{\partial x^i}\right)$ and $(\mathbf{e}_j)$ are oriented, the sign must be positive.
</details>
</MathBox>
...
$$
  \iota_S^* (N\lrcorner\omega_g) (\mathbf{e}_1,\dots,\mathbf{e}_{n-1}) = \omega_g (\mathbf{n}, \mathbf{e}_1,\dots,\mathbf{e}_{n-1}) =1
$$
...
<MathBox title='' boxType='proposition'>
Suppose that $M$ is any Riemannian manifold with boundary. There is a unique smooth outward-pointing unit normal vector field $N$ along $\partial M$.
...
To prove existence, let $f:M\to\R$ be a boundary defining function, and let $\mathbf{n}$ be the restriction to $\partial M$ of the unit vector field $-\frac{\nabla f}{|\nabla f|_g}$. Because $\mathrm{d}f \neq 0$ at points of $\partial M$, then $\mathbf{n}$ is well-defined and smooth on $\partial M$. Then $\mathbf{n}$ is normal to $\partial M$ and outward-pointing, because $\mathbf{n}f = -\frac{\langle \nabla f, nabla f \rangle_g}{|\nabla f|_g} = - |\nabla f|_g < 0$.
</details>
</MathBox>
...
<MathBox title='' boxType='theorem'>
Suppose that $M$ is a connected, oriented, smooth manifold with or without boundary, and $\pi:E\to M$ is a smooth normal covering map. THen $M$ is orientable if and only if the action of $\mathrm{aut}_\pi (E)$ on $E$ is orientation-preserving.

<details>
<summary>Proof</summary>

Let $\mathcal{O}_E$ denoted the given orientation on $E$. First suppose that $M$ is orientable, and let $\mathbf{q}\in E$ be arbitrary. Because $M$ is connected, it has exactly two orientations, and one of them has the property that $\mathrm{d}\pi_\mathbf{q}: T_\mathbf{q}E \to T_{\pi(\mathbf{q})} M$ is orientation-preserving. Call that orientation $\mathcal{O}_M$. The pullback orientation $\pi^* \mathcal{O}_M$ agrees with the given orientation at $\mathbf{q}$, so it must be equal to $\mathcal{O}_E$. Suppose that $\varphi\in\mathrm{aut}_\pi (E)$. The fact that $\pi\circ\varphi = \pi$ implies That
...
Suppose that $M$ is a connected, smooth, positive-dimensional manifold with or without boundary, and let $\hat{\pi}:\hat{M}\to M$ be its orientation covering. Then $\hat{M}$ can be given the structure of a smooth, oriented manifold with or without boundary, with the following properties:
...
Finally, to prove **(3)**, assume that $U\subseteq M$ is evenly covered and therefore orientable. Given any orientation $\mathcal{O}$ of $U$, define a section $\sigma:U\to\hat{M}$ by setting $\sigma(\mathbf{p}) = (\mathbf{p},\mathcal{O}_\mathbf{p})$. To see that $\sigma$ is continuous, suppose that $\hat{U}'_\mathcal{O}$ is any basis open subset of $\hat{M}$. Then for each component $V$ of $U\cap U'$, the restricted orientations $\mathcal{O}|_V$ and $\mathcal{O}'|_V$ must either agree or disagree on all of $V$, so $\sigma^{-1}(\hat{U}'_{\mathcal{O}'})$ is a union of such components and therefore open.
</details>
</MathBox>

<MathBox title='The orientation covering theorem' boxType='theorem'>
Suppose that $M$ is a connected smooth manifold with or without boundary, and let $\hat{\pi}:\hat{M}\to M$ be its orientation covering.
...
<MathBox title='' boxType='theorem'>
Let $M$ be a connected smooth manifold with or without boundary, and suppose that the fundamental group of $M$ has no subgroup of index $2$. Then $M$ is orientable. In particular, if $M$ is simply connected then it is orientable.

<details>
<summary>Proof</summary>

Suppose that $M$ is not orientable, and let $\hat{\pi}:\hat{M}\to M$ be its orientation covering, which is an honest covering map in this case. Choose any point $\mathbf{q}\in\hat{M}$, and let $\mathbf{p} = \hat{\pi}(\mathbf{q})\in M$. Let $\alpha:\hat{M}\to\hat{M}$ be the map that interchanges the two points in each fibre of $\hat{\pi}$. To prove that $\alpha$ is smooth, suppose that $U\subseteq M$ is any evenly covered open subset and $U_0, U_1\subseteq\hat{M}$ are the two components of $\hat{\pi}^{-1} (U)$. Since $\hat{\pi}$ restricts to a diffeomorphism from each component onto $U$, we can write $\alpha|_{U_0} = (\hat{\pi}|_{U_1})^{-1} \circ (\hat{\pi}|_{U_0})$, which is smooth. Similarly, $\alpha|_{U_1}$ is also smooth. Since the collection of all such sets $U_0, U_1$ is an open covering of $\hat{M}$, it follows that $\alpha$ is smooth, and it is a covering automorphism because it satisfies $\hat{\pi}\circ\alpha=\hat{\pi}$. In fact, since a covering automorphism is determined by what it does to one point, $\alpha$ is the unique nontrivial element of the automorphism group $\mathrm{aut}_{\hat{\pi}} (\hat{M})$, which is therefore equal to the two-element group $\Set{\mathrm{id}_{\hat{M}}, \alpha}$. Because the automorphism group acts transitively on fibers, $\hat{\pi}$ is a normal covering map. Let $H$ denote the subgroup $\hat{\pi}_* (\pi_1 (\hat{M},\mathbf{q}))$ of $\pi_1 (M,\mathbf{p})$. A fundamental result in the theory of covering spaces is that the quotient group $\pi_1 (M,\mathbf{p})/H$ is isomorphic to $\mathrm{aut}_{\hat{\pi}} (\hat{M})$. Therefore, $H$ has index $2$ in $\pi_1 (M,\mathbf{p})$.
</details>
</MathBox>

# Integrations on manifolds

## Integration of differential forms

<MathBox title='Integral of differential forms' boxType=''>
Let $D\subseteq\R^n$ be a bounded subset whose boundary has measure zero, and let $\omega$ be a (continuous) $n$-form on $\overline{D}$. Any such form can be written $\omega = f \bigwedge_{i=1}^n \mathrm{d}x^i$ for some continuous function $f:\overline{D}\to\R$. We define the integral of $\omega$ over $D$ as
...
<MathBox title='' boxType='proposition'>
Suppose that $D$ and $E$ are open domains of integration in $\R^n$ or $\mathbb{H}^n$, and that $G:\overline{D}\to\overline{E}$ is a smooth map that restricts to an orientation-preserving or orientation-reversing diffeomorphism from $D$ to $E$. If $\omega$ is an $n$-form on $\overline{E}$, then
...
Let $(y^i)_{i=1}^n$ denote the standard coordinates on $E$, and $(x^i)_{i=1}^n$ denote those on $D$. Suppose first that $G$ is orientation-preserving. With $\omega = f \bigwedge_{i=1}^n \mathrm{d}y^i$, the change of variables formula together formula for pullback of $n$-form yields

$$
\begin{align*}
  \int_E \omega =& \int_E f\mathrm{d}V = \int_D (f\circ G)|\det(DG)|\;\mathrm{d}V \\
  =& \int_D (f\circ G)\det(DG)\;\mathrm{d}V \\
  =& \int_D (f\circ G)\det(DG)\; \mathrm{d}x^1 \wedge\cdots\wedge \mathrm{d}x^n \\
  =& \int_D G^* \omega
\end{align*}
$$
...
<MathBox title='' boxType='lemma'>
Suppose that $U$ is an open subset of $\R^n$ or $\mathbb{H}^n$, and that $K$ is a compact subset of $U$. Then there is an open domain of integration $D$ such that $K\subseteq D\subseteq\overline{D}\subseteq K$.
...
<MathBox title='' boxType='proposition'>
Suppose that $U, V$ are open subsets of $\R^n$ or $\mathbb{H}^n$, and the $G:U\to V$ is an orientation-preserving or orientation-reversing diffeomorphism. If $\omega$ is a compactly supported $n$-form on $V$, then
...
Suppose that $\Set{\tilde{U}_j}$ is another finite open cover of $\mathrm{supp}(\omega)$ by domains of positively or negatively oriented smooth charts, and $\Set{\tilde{\psi}_j}$ is a subordinate smooth partition of unity. For each $i$, we compute
...
**(3):** Suppose that $\omega$ is a positively oriented orientation form for $M$. This means that if $(U,\varphi)$ is a positively oriented smooth chart, then $(\varphi^{-1})^* \omega$ is a positive function times $\mathrm{d}x^1 \wedge\cdots\wedge \mathrm{d}x^n$, and for a negatively oriented chart it is a negative functino times the same form. Thus, each term in the sum defining $\int_M$ is nonnegative, with at least one strictly positive term.

**(4):** It suffices to assume that $\omega$ is compactly supported in a single positively or negatively oriented smooth chart, because any compactly supported $n$-form on $M$ can be written as a finite sum of such forms by means of a partition of unity. Thus, suppose that $(U,\varphi)$ is a positively oriented smooth chart on $M$, whose domain contains the support of $\omega$. When $F$ is orientation-preserving, it is easy to check that $(F^{-1}(U), \varphi\circ F)$ is an oriented smooth chart no $N$ whose domain contains the support of $F^*\omega$, and the result follows. The cases in which the chart is negatively oriented or $F$ is orientation-reversing then follow from this result together with **(2)**.
</details>
</MathBox>

<MathBox title='Integration over parametrizations' boxType='proposition'>
Let $M$ be an oriented smooth $n$-manifold with or without boundary, and let $\omega$ be a compactly supported $n$-form on $M$. Suppose that $D_1,\dots,D_k$ are open domains of integration in $\R^n$, and for $i=1,\dots,k$ we are given smooth maps $F_i : \overline{D}_i \to M$ satisfying
...
It suffices to assume that $\omega$ is supported in the domain of a single oriented smooth chart $(U,\varphi)$. In fact, by restricting to sufficiently nice charts, we may assume that $U$ is precompact, $Y = \varphi(U)$ is a domain of integration in $\R^n$ or $\mathbb{H}^n$, and $\varphi$ extends to a diffeomorphism from $\overline{U}$ to $\overline{Y}$.
...
<MathBox title="Stoke's theorem" boxType='theorem'>
...
where $\partial M$ has the induced orientation such that $\iota_{\partial M}^* \omega$. If $\partial M = \emptyset$, then $\int_M \mathrm{d}\omega = 0$.
...
where the hat means that $\mathrm{d}x^i$ is omitted. Thus,

$$
\begin{align*}
  \mathrm{d}\omega =& \sum_{i=1}^n \mathrm{d}\omega_i \;\mathrm{d}x^1 \wedge\cdots\wedge \hat{\mathrm{d}x}^i \wedge\cdots\wedge\mathrm{d}x^n \\
  =& \sum_{i,j=1}^n \frac{\partial\omega_i}{\partial x^j} \mathrm{d}x^j \;\mathrm{d}x^1 \wedge\cdots\wedge \hat{\mathrm{d}x}^i \wedge\cdots\wedge\mathrm{d}x^n \\
...
$$
\begin{align*}
  \int_{\mathbb{H}^n} \mathrm{d}\omega =& \sum_{i=1}^n (-1)^{i-1} \int_A \frac{\partial\omega_i}{\partial x^i} \;\mathrm{d}x^1 \wedge\cdots\wedge\mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \int_{-r}^r \cdots \int_{-r}^r \frac{\partial\omega_i}{\partial x^i}(\mathbf{x}) \;\mathrm{d}x^1\cdots\mathrm{d}x^n
\end{align*}
$$
...
\begin{align*}
  &\sum_{i=1}^n (-1)^{i-1} \int_0^r \int_{-r}^r \cdots \int_{-r}^r \frac{\partial\omega_i}{\partial x^i}(\mathbf{x}) \;\mathrm{d}x^1\cdots\mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \int_{-r}^r \cdots \int_{-r}^r \frac{\partial\omega_i}{\partial x^i}(\mathbf{x}) \;\mathrm{d}x^i \mathrm{d}x^1\cdots \hat{\mathrm{d}x}^i \cdots \mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \int_{-r}^r \cdots \int_{-r}^r \left[\omega_i (\mathbf{x}) \right]_{x^i = -r}^{x^i = r} \; \mathrm{d}x^1\cdots \hat{\mathrm{d}x}^i \cdots \mathrm{d}x^n \\
  =& 0
\end{align*}
$$
...
$$
\begin{align*}
  \int_{\mathbb{H}^n} \mathrm{d}\omega =& (-1)^{n-1} \int_{-r}^r \cdots \int_{-r}^r \int_0^r \frac{\partial\omega_n}{\partial x^n}(\mathbf{x})\;\mathrm{d}x^n \mathrm{d}x^1 \cdots\mathrm{d}x^{n-1} \\
  =& (-1)^{n-1} \int_{-r}^r \cdots \int_{-r}^r \left[\omega_n (\mathbf{x})\right]_{x^n = 0}^{x^n = r} \;\mathrm{d}x^1 \cdots\mathrm{d}x^{n-1} \\
  =& (-1)^n \int_{-r}^r \cdots \int_{-r}^r \omega_n (x^1,\dots,x^{n-1}, 0) \;\mathrm{d}x^1 \cdots\mathrm{d}x^{n-1}
\end{align*}
$$
...
Since $x^n$ vanishes on $\partial\mathbb{H}^n$, the pullback of $\mathrm{d}x^n$ to the boundary is identically zero. Thus, the only term above that is nonzero is the one for which $i = n$, which becomes

$$
  \int_{\partiall\mathbb{H}^n} \omega = \int_{A\cap\partial\mathbb{H}^n} \omega^n (x^^1,\dots,x^{n-1},0) \bigwedge_{i=1}^n \mathrm{d}x^i
$$

Taking into account the fact that the coordinates $(x^1,\dots,x^{n-1})$ are positively oriented for $\partial\mathbb{H}^n$ when $n$ is even and negatively oriented when $n$ is odd, we find that $\int_{\mathbb{H}^n} \mathrm{d}\omega = \int_{\partial\mathbb{H}^n} \omega$.

**Case 2:** $M = \R^n$<br/> 
In this case, the support of $\omega$ is contained in a cube of the form $A = [-r,r]^n$. The same computations as for **Case 1** goes through, except that in this case the $i = n$ term vanishes like all others, such that $\int_\R \mathrm{d}\omega = 0$. Since $\partial\R = \emptyset$, then $\int_{\partial\R} \omega = 0$.
...
$$
  \int_M \mathrm{d}\omega = \int_{\mathbb{H}^n} (\varphi^{-1})^* \;\mathrm{d}\omega = \int_{\mathbb{H}^n} \mathrm{d}\left( (\varphi^{-1})^* \omega\right)
$$
...
\begin{align*}
  ...
  =& \sum_{i=1}^n \int_M \mathrm{d}(\psi_i \omega) \\
  =& \sum_{i=1}^n \int_M \mathrm{d}\psi_i \wedge\omega + \psi_i \;\mathrm{d}\omega \\
  =& \int_M \mathrm{d}\left( \sum_{i=1}^n \psi_i \right)\wedge\omega + \int_M \left( \sum_{i=1}^n \psi_i \right)\mathrm{d}\omega \\
  =& 0 + \int_M \mathrm{d}\omega
\end{align*}
$$
...
$$
  \partial M = \emptyset \implies \int_M \mathrm{d}\omega = 0
$$
</MathBox>

<MathBox title='Integrals of closed forms over boundaries' boxType='corollary'>
Suppose that $M$ is a compact oriented smooth manifold with boundary. If $\omega$ is a closed form on $M$, then the integral of $\omega$ over $\partial M$ is zero:

$$
  \mathrm{d}\omega \equiv 0 \implies \int_{\partial M} \omega = 0
$$
</MathBox>

<MathBox title='' boxType='corollary'>
Suppose that $M$ is a smooth manifold with or without boundary, $S\subseteq M$ is an oriented compact smooth $k$-dimensional submanifold (without boundary), and that $\omega$ is a closed $k$-form on $M$. If $\int_S \omega\neq 0$, then both of the following are true:
...
Suppose that $D$ is a compact regular domain in $\R^2$, and let $P$ and $Q$ be smooth real-valued functions on $D$. Then
...
<details>
<summary>Proof</summary>
This follows from applying Stoke's theorem to the $1$-form $P\;\mathrm{d}x + Q\;\mathrm{d}y$.
</details>
</MathBox>
...
Suppose that $(U,\varphi)$ and $(V,\psi)$ are two smooth charts with corners such that $\varphi(\mathbf{p})$ is a corner point $\psi(\mathbf{p})$ is not. To simplify notation, let us assume without loss of generality that $\varphi(\mathbf{p})$ has coordinates $(x^1,\dots,x^k,0,\dots,0)$ with $k\leq n-2$. Then $\psi(V)$ contains an open subset of some $(n-1)$-dimensional linear subspace $S\subseteq\R^n$, with $\psi(\mathbf{p})\in S$. If $\psi(\mathbf{p})\in \partial\overline{\R}_+^n$, take $S$ to be the unique subspace defined by an equation of the form $x^i = 0$ that contains $\psi(\mathbf{p})$. If $\psi(\mathbf{p})$ is an interior point, any $(n-1)$-dimensional subspace containing $\psi(\mathbf{p})$ will do.

Let $S' = S\cap\psi(U\cap V)$, and let $\alpha:S'\to\R^n$ be the restriction of $\varphi\circ\psi^{-1}$ to $S'$. Because $\varphi\circ\psi^{-1}$ is a diffeomorphism from $\psi(U\cap V)$ to $\psi(U\cap V)$, it follows that $\psi\circ\varphi^{-1}\circ\alpha$ is the identity of $S'$, and therefore $\mathrm{d}\alpha_{\psi(\mathbf{p})}$ is an injective linear map. Let $T = \mathrm{d}\alpha_{\psi(\mathbf{p})} (T_{\psi(\mathbf{p})}S) \subseteq\R^n$. Because $T$ is $(n-1)$-dimensional, it must contain a vector $\mathbf{v}$ such that one of the last two components, $v^{n-1}$ or $v^n$, is nonzero (otherwise, $T$ would be contained in a codimension-2 subspace). Renumbering the coordinates and replacing $\mathbf{v}$ by $-\mathbf{v}$ if necessary, we may assume that $v^n < 0$.

Let $\gamma:(-\varepsilon,\varepsilon)\to S$ be a smooth curve such that $\gamma(0) = \mathbf{p}$ and $\mathrm{d}\alpha(\gamma'(0)) = \mathbf{v}$. Then $\alpha\circ\gamma(t)$ has negative $x^n$ coordinate for small $t> 0$, which contradicts the fact that $\alpha$ takes it values in $\overline{R}_+^n$.
</details>
</MathBox>

<MathBox title='Integrals of differental forms on manifolds with corners' boxType='definition'>
Let $M$ be an oriented smooth $n$-manifold with corners, and suppose that $\omega$ is an $(n-1)$-form on $\partial M$ that is compactly supported in the domain of a single oriented smooth chart with corners $(U,\varphi)$. The integral of $\omega$ over $M$ is defined bY
...
$$
\begin{align*}
  \int_{\overline{\R}_+^n} \mathrm{d}\omega =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \cdots\int_0^r \frac{\partial\omega_i}{\partial x^i}(\mathbf{x})\;\mathrm{d}x^1\cdots\mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \cdots\int_0^r \frac{\partial\omega_i}{\partial x^i}(\mathbf{x})\;\mathrm{d}x^i \mathrm{d}x^1\cdots\hat{\mathrm{d}x}^i \cdots\mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^{i-1} \int_0^r \cdots\int_0^r \left[ \omega_i (\mathbf{x}) \right]_{x^i = 0}^{x^i = r} \mathrm{d}x^1\cdots\hat{\mathrm{d}x}^i \cdots\mathrm{d}x^n \\
  =& \sum_{i=1}^n (-1)^i \int_0^r \cdots\int_0^r \omega_i(x^1,\dots,0,\dots,x^n) \mathrm{d}x^1\cdots\hat{\mathrm{d}x}^i \cdots\mathrm{d}x^n \\
...
<MathBox title='' boxType='theorem'>
Suppose that $M$ is a smooth manifold and that $\gamma_0, \gamma_1:[a,b]\to M$ are path-homotopic piecewise smooth curve segments. For every closed $1$-form $\omega$ on $M$
...
$$
  \int_{I\times I} \mathrm{d}(H^* \omega) = \int_{I\times I} H^* \;\mathrm{d}\omega = 0
$$

On the other hand, $I\times I$ is a smooth manifold with corners, so Stoke's theorem implies That

$$
  0 = \int_{I\times I} \mathrm{d}(H^* \omega) = \int_{\partial(I\times I)} H^* \omega
$$
...
Suppose that $M$ is simply connected and that $\omega$ is a closed $1$-form on $M$. Since every piecewise smooth closed curve in $M$ is path-homotopic to a constant curve, the preceding theorem shows that the integral of $\omega$ over every such curve is equal to $0$. Thus, $\omega$ is conservative and therefore exact.
</details>
</MathBox>
...
Suppose that $(M, g)$ is an oriented Riemannian manifold with or without boundary, and let $\omega_g$ denote its Riemannian volume form. If $f$ is a compactly supported continuous real-valued function on $M$, then $f\omega_g$ is a compactly supported $n$-form, so we can define the integral of $f$ over $M$ to be
...
The Riemannian volume form is often denoted by $\mathrm{d}V_g$, or $\mathrm{d}A_g$ or $\mathrm{d}s_g$ in the 2-dimensional and $1$-dimensional case, respectively. Then the integral of $f$ over $M$ is written $\int_M f\mathrm{d}V_g$, and the volume of $M$ as $\mathrm{vol}(M) = \int_M \mathrm{d}V_g$. Note that this notation is just used for conveniance and does not imply that the volum form is the exterior derivative of an $(n-1)$-form.

<MathBox title='' boxType='proposition'>
Let $(M,g)$ be a nonempty oriented Riemannian $n$-manifold with or without boundary, and suppose that $f$ is a compactly supported continuous real-valued function on $M$ satisfying $f \geq 0$. Then $\int_M f \;\mathrm{d}V_g \geq 0$ with equality if and only if $f \equiv 0$.
...
$$
  \int_M f\;\mathrm{d}V_g = \int_{\varphi(U)} f(\mathbf{x}) \sqrt{\det(g_{ij})}\;\mathrm{d}x^1\cdots\mathrm{d}x^n \geq 0
$$

The same inequality holds in a negatively oriented chart because the negative sign from the chart cancels the negative sign in the expression for $\mathrm{d}V_g$. The general case follows from this oone, because $\int_M f\;\mathrm{d}V_g$ is equal to a sum of terms like $\int_M \psi_i f\;\mathrm{d}V_g$, where each integrand $\psi_i f$ is nonnegative and supported in a single smooth chart. If in addition $f$ is positive somewhere, then it is positivie on a nonempty open subset by continuity, so at least one of the integrals in this sum is positive. On the other hand, if $f$ is identically zero, then clearly $\int_M f\;\mathrm{d}V_g = 0$.
</details>
</MathBox>
...
<MathBox title='' boxType='proposition'>
Let $(M,g)$ be an oriented Riemannian manifold with or without boundary. Suppose that $S\subseteq M$ is an immersed hypersurface with the orientation determined by a unit normal vector field $N$, and $\tilde{g}$ is the induced metric on $S$. If $X$ is any vector field along $S$, then
...
$$
  \beta(X) = X \lrcorner \mathrm{d}V_g
$$
...
$$
  \beta(X) = X^\perp \lrcorner \mathrm{d}V_g + X^T \lrcorner \mathrm{d}V_g
$$
...
$$
\begin{align*}
  \iota_S^* (X^\perp \lrcorner \mathrm{d}V_g) =& \langle X, N \rangle_g \iota_S^* (N \lrcorner \mathrm{d}V_g) \\
  =& \langle X, N \rangle_g \mathrm{d}V_{\tilde{g}}
\end{align*}
$$

The stated equation will be proved if we can show that $\iota_S^* (X^T \lrcorner \mathrm{d}V_g) = 0$. If $X_1,\dots,X_{n-1}$ are any vectors tangent to $S$, then

$$
  (X^T \lrcorner\mathrm{d}V_g)(X_1,\dots,X_{n-1}) = \mathrm{d}V_g (X^T, X_1,\dots,X_{n-1}) = 0
$$
...
Let $(M, g)$ be an oriented Riemannian $n$-manifold with or without boundary. Multiplication by the Riemannian volume form defines a smooth bundle isomorphism $*: C^\infty (M) \to\Omega^n (M)$ by $*f = f\;\mathrm{d}V_g$. The *divergence operator* on $\mathfrak{X}(M)$, denoted $\div: \mathfrak{X}(M) \to C^\infty (M)$, by

$$
  \div(X) = *^{-1} \mathrm{d}(\beta(X))
$$

or equivalently

$$
  \mathrm{d}(X \lrcorner \mathrm{d}V_g) = (\div(X))\mathrm{d}V_g
$$
</MathBox>
...
$$
  \int_M \div(X) \mathrm{d}V_g = \int_{\partial M} \langle X, N \rangle_g \;\mathrm{d}V_{\tilde{g}}
$$
...
$$
  \int_M \div(X)\;\mathrm{d}V_g = \int_M \mathrm{d}(\beta(X)) = \int_{\partial M} \iota_S^* \beta(X)
$$
...
Let $M$ be an oriented Riemannian manifold, let $\mathfrak{X}(M)$, and let $\theta$ be the flow of $X$. Then $\theta$ is
1. *volume-preserving* if and only if $\div(X) = 0$ everywhere on $M$
2. *volume-nondecreasing* if and only if $\div(X) \geq 0$ everywhere on $M$
3. *volume-nonincreasing* if and only if $\div(X) \leq 0$ everywhere on $M$
4. *volume-increasing* if and only if $\div(X) > 0$ everywhere on $M$
5. *volume-decreasing* if and only if $\div(X) < 0$ everywhere on $M$
...
$$
  \mathrm{vol}(\theta_t (D)) = \int_{\theta_t (D)} \mathrm{d}V_g = \int_D \theta_t^* \mathrm{d}V_g
$$
...
$$
\begin{align*}
  \mathcal{L}_X \mathrm{d}V_g =& X \lrcorner \mathrm{d}(\mathrm{d}V_g) + \mathrm{d}(X \lrcorner \mathrm{d}V_g) \\
  =& \div(X)\;\mathrm{d}V_g
\end{align*}
$$

because $\mathrm{d}(\mathrm{d}V_g)$ is an $(n+1)$-form on an $n$-manifold. Then

$$
\begin{align*}
  \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t=t_0} \mathrm{vol}(\theta_t (D)) =& \int_D \left.\frac{\partial}{\partial t}\right|_{t=t_0} (\theta_t^* \;\mathrm{d}V_g) \\
  =& \int_D \theta_{t_0}^* (\mathcal{L}_X \;\mathrm{d}V_g) \\
  =& \int_D \theta_{t_0}^* (\div(X)\;\mathrm{d}V_g) \\
  =& \int_{\theta_{t_0}(D)} \div(X)\;\mathrm{d}V_g
\end{align*}
$$

Now we can prove the "if" parts of all five equivalences. If $\div(X)\equiv 0$, it follows that $\mathrm{vol}(\theta_t (D))$ is a constant function $t$ for every $D$, and thus $\theta$ is volume-preserving. Similarly, an inequality of the form $\div(X) \geq 0$ or $\div(X) \leq 0$ implies that $\mathrm{vol}(\theta_t (D))$ is nondecreasing or nonincreasing, respectively.

For **(4)**, suppose that $\div(X) > 0$ on a dense subset of $M$, and let $D$ be a compact regular domain in $M$. Then $\div(X) \geq 0$ everywhere by continuity, so $\mathrm{vol}(\theta_t (D))$ is nondecreasing by the argument above. Because $\mathrm{int}(D)$ is an open subset of $M$, it follows that $\mathrm{int}(\theta_t (D))$ is open for each $t$ such that $D\subseteq M_t$, and therefore by density there is a point in $\mathrm{int}(\theta_t (D))$ where $\div(X) > 0$. It follows that $\int_{\theta_t (D)} \div(X) \;\mathrm{d}V_g > 0$ and thus $\mathrm{vol}(\theta_t (D))$ is strictly increasing. A similar argument proves **(3)**.

To prove the converses, we prove their contrapositives. For **(2)**, if there is a point where $\div(X) < 0$, then by continuity there is an open subset $U\subseteq M$ on which $\div(X) < 0$. The argument in the first part of the proof shows that $X$ generates a volume-decreasing flow on $U$. In particular, for any regular coordinate ball $B$ such that $\overline{B}\subseteq U$ and any $t > 0$ small enough to ensure that $\theta_t (\overline{B}) \subseteq U$, we have $\mathrm{vol}(\theta_t (\overline{B})) < \mathrm{vol}(\overline{B})$, which implies that $\theta$ is not volume-nondecreasing. The same argument with inequalities reversed proves **(3)**. 

If $\div(X)$ is not identically zero, then there is an open subset on which it is either strictly positive of strictly negative, and then the argument above shows that it is not volume-preserving on that set, thus proving **(1)**.

For **(4)**, if the subset of $M$ where $\div(X) > 0$ is not dense, there is an open subset $U\subseteq M$ on which $\div(X) \leq 0$. Then **(3)** shows that $\theta$ is volume-nondecreasing on $U$, so it cannot be volume-increasing on $M$. The argument for **(5)** is similar.
</details>
</MathBox>
...
If $(M,g)$ is an oriented Riemannian $3$-manifold, the *curl operator*, denoted $\curl: \mathfrak{X}(M) \to\mathfrak{X}(M)$ by

$$
  \curl(X) = \beta^{-1} \;\mathrm{d}(X^\flat)
$$

where $\beta:\mathfrak{X}(M) \to\Omega^2 (M)$ is defiend by $\beta(X) = X \lrcorner \mathrm{d}A_g$. Unwinding the definitions, this is equivalent to
...
The operators $\grad$, $\curl$ and $\div$ on an oriented Riemannian $3$-manifold $M$ are related by the following commutative diagram.

$$
\begin{CD}
  C^\infty (M) @>{\mathrm{grad}}>> \mathfrak{X}(M) @>{\mathrm{curl}}>> \mathfrak{X}(M) @>{\mathrm{div}}>> C^\infty (M) \\
  @VV{\mathrm{id}}V @VV{\flat}V @VV{\beta}V @VV{*}V \\
  \Omega^0 (M) @>>{\mathrm{d}}> \Omega^1 (M) @>>{\mathrm{d}}> \Omega^2 (M) @>>{\mathrm{d}}> \Omega^3 (M) 
\end{CD}
$$

The indenties $\curl\circ\grad\equiv 0$ and $\div\circ\curl\equiv =$ follow from $\mathrm{d}\circ\mathrm{d}\equiv 0$ just as they do in the Euclidean case. The curl operative is defined only in dimension $3$ because it is only in that case that $\Lambda^2 T^* M$ is isomorphic to $TM$ (via the map $\beta: X\mapsto X\lrcorner \mathrm{d}V_g$).

<MathBox title='Surface integral' boxType='definition'>
Let $(M,g)$ be an oriented Riemannian $3$-manifold. Suppose that $S\subseteq M$ is a compact $2$-dimensional submanifold with or without boundary, and $N$ is a smooth unit normal vector field along $S$. Let $\mathrm{d}A$ denote the Riemannian volume form on $S$ with respect to the induced metric $\iota_S^* g$ and the orientation determined by $N$, so that $\mathrm{d}A = \iot_S^* (N \lrcorner \mathrm{d}V_g)$. For any smooth vector field $X$ on $M$, the *surface integral* of $X$ over $S$ is defined As
...
Suppose that $M$ is an oriented Riemannian $3$-manifold with or without boundary, and $S$ is a compact oriented $2$-dimensional smooth submanifold with boundary in $M$. For any smooth vector field $X$ on $M$

$$
  \int_S \langle \curl(X), N \rangle_g \;\mathrm{d}A = \int_{\partial S} \langle X, T \rangle_g \;\mathrm{d}s
$$

where $N$ is the smooth unit normal vector field along $S$ that determines its orientation, $\mathrm{d}s$ is the Riemannian volume form for $\partial S$ with respect to the metric and orientation induced from $S$, and $T$ is the unique positively oriented unit tangent vector field on $\partial S$.

<details>
<summary>Proof</summary>

The general version of Stokes' theorem applied to the $1$-form $X^\flat$ yields

$$
  \int_S \mathrm{d}(X^\flat) = \int_{\partial S} X^\flat
$$
...
$$
\begin{align*}
  \iota_S^* \mathrm{d}(X^\flat) =& \langle\curl(X), N\rangle_g \;\mathrm{d}A \\
  \iota_{\partial S}^* X^\flat =& \langle X, T\rangle_g \;\mathrm{d}s
\end{align*}
$$

The first identity is just the defining equation for the curl combined with the previous lemma. To prove the second identity, we note that $\iota_{\partial S}^* X^\flat$ is a smooth $1$-form on a $1$-manifold, and thus must be equal to $f\;\mathrm{d}s$ for some smooth function $f$ on $\partial S$. To evaluate $f$, we note that $\mathrm{d}s(T) = 1$, and so the definition of $X^\flat$ yields
...
**(1)** follows immediately from the definition. For **(2)**, suppose that $\mu_1$ and $\mu_2$ give the same value when applied to $(\mathbf{e}_i)_{i=1}^n$. If $(\mathbf{v}_i)_{i=1}^n$ are arbitrary vectors in $V$, let $T:V\to V$ be the unique linear map that takes $\mathbf{e}_i$ to $\mathbf{v}_i$ for $i=1,\dots,n$. It follows that
...
To prove **(4)**, suppose that $\omega$ is any nonzero element of $\Lambda^N (V^*)$. If $\mu$ is an arbitrary element of $\mathcal{D}(V)$, it suffices to show that $\mu = c|\omega|$ for some $c\in\R$. Let $(\mathbf{e}_i)$ be a basis for $V$, and define $a,b\inR$ by
...
We will construct local trivializations and use the vector bundle chart lemma. Let $(U, (x^i))$ be any smooth coordinate chart on $M$, and let $\omega = \bigwedge_{i=1}^n \mathrm{d}_x^i$. By density properties, $|\omega_\mathbf{p}|$ is a basis for $\mathcal{D}(T_\mathbf{p}M)$ at each point $\mathbf{p}\in U$. Thus, the map $\Phi: \pi^{-1} (U)\to U\times\R$ given bY
...
Suppose that $(\tilde{U}, (\tilde{x}^j))$ is another smooth chart with $U \cap \tilde{U} \neq\emptyset$. Let $\tilde{\omega} = \bigwedge_{j=1}^n \mathrm{d}\tilde{x}^j$, and define $\tilde{\Phi}:\pi^{-1} (\tilde{U}\to\tilde{U}\times\R$ correspondingly
...
$$
  (F^* \mu)_\mathbf{p} (\mathbf{v}_1,\dots,\mathbf{v}_n) = \mu_{F(\mathbf{p})}(\mathrm{d}F_\mathbf{p}(\mathbf{v}_1),\dots,\mathrm{d}F_\mathbf{p}(\mathbf{v}_n))
$$
</MathBox>
...
Suppose that $F:M\to N$ is a smooth map between $n$-manifolds with or without boundary. If $(x^i)$ and $(y^i)$ are smooth coordinates on open subsets $U\subseteq M$ and $V\subseteq N$, respectively, and $u$ is a continuous real-valued function on $V$, then the following holds on $U\cap F^{-1} (V)$

$$
  F^* \left(u\left| \bigwedge_{i=1}^n \mathrm{d}y^i \right|\right) = (u\circ F)|\det(\mathrm{d}F)| \left| \bigwedge_{i=1}^n \mathrm{d}y^i \right|
$$
...
$$
\begin{align*}
  F^* \left(u\left| \bigwedge_{i=1}^n \mathrm{d}y^i \right|\right) =& (u\circ F)F^* \left| \bigwedge_{i=1}^n \mathrm{d}y^i \right| \\
  =& (u\circ F) \left| F^* \bigwedge_{i=1}^n \mathrm{d}y^i \right| \\
  =& (u\circ F) \left|\det(\mathrm{D}F) \bigwedge_{i=1}^n \mathrm{d}y^i \right| \\
  =& (u\circ F) |\det(\mathrm{D}F)| \left| \bigwedge_{i=1}^n \mathrm{d}y^i \right|
\end{align*}
$$
</details>
</MathBox>
...
If $D\subseteq\R^n$ is a domain of integration and $\mu$ is a density on $\overline{D}$, we can write $\mu = f\left|\bigwedge_{i=1}^n \mathrm{d}x^i \right|$ for some uniquely determined continuous function $f:\overline{D}\to\R$. We define the *integral* of $\mu$ over $D$ by

$$
  \int_D \mu = \int_D f\;\mathrm{d}V
$$

or equivalently

$$
  \int_D f \left|mathrm{d}x^1 \wedge\cdots\wedge \mathrm{d}x^n \right| = \int_D f\;\mathrm{d}x^1\cdots\mathrm{d}x^n
$$
...
Suppose that $U$ and $V$ are open subsets of $\R^n$ or $\mathbb{H}^n$, and that $G:U\to V$ is a diffeomorphism. If $\mu$ is a compactly supported density on $V$, then
...
Suppose that $M$ and $N$ are smooth $n$-manifolds with or without boundary, and that $\mu$ and $\eta$ are compactly supported densities on $M$.
1. **(Linearity):** If $a,b\in\R$, Then
$$
...
Let $(M,g)$ be a Riemannian $n$-manifold with or without boundary, and with Riemannian density $\mu_g = \mathrm{d}V_g$. If $f:M\to\R$ is a compact supported continuous function, the integral of $f$ over $M$ is defined to because
...
Let $(M,g)$ be a nonorientable Riemannian $n$-manifold with boundary. For any compactly supported smooth vector field $X$ on $M$

$$
  \int_M \div(X)\mu_g = \int_{\partial M} \langle X, N \rangle_g \mu_{\tilde{g}}
$$
...
\begin{align*}
  ...
  =& 2\int_{\partial\hat{M}} \langle X, N \rangle_g \mu_{\tilde{g}}
\end{align*}
$$
</details>
</MathBox>