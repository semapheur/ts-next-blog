...
$$
  \mathbf{\mathbf{X_p}} f = \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_0 (f\circ\gamma)
$$
...
$$
  \mathbf{\mathbf{X_p}} f = \gamma'(0)f = \mathrm{d}\gamma_0 \left(\left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_0 \right)f = \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_0 (f\circ\gamma)
$$
</details>
</MathBox>
...
Let $M$ and $N$ be smooth manifolds with or without boundary. Suppose $F:M\to N$ is smooth and that $\mathbf{X}$ is a vector field on $M$. If there is a vector field $\mathbf{Y}$ on $N$ such that $\mathrm{d}F_\mathbf{p}(\mathbf{\mathbf{X_p}}) = \mathbf{Y}_{F(\mathbf{p})}$ for each $\mathbf{p}\in M$, we say that $\mathbf{X}$ and $\mathbf{Y}$ are $F$-related. 
</MathBox>
...
$$
  \mathbf{X}(f\circ F)(\mathbf{p}) = \mathbf{X_p}(f\circ F) = \mathrm{d}F_\mathbf{p}(\mathbf{\mathbf{X_p}})f
$$
...
Thus, the first expression is true for all $f$ if and only if $\mathrm{d}F_\mathbf{p}(\mathbf{\mathbf{X_p}}) = \mathbf{Y}_{F(\mathbf{p})}$ for all $\mathbf{p}\in M$, i.e. if and only if $\mathbf{X}$ and $\mathbf{Y}$ are $F$-related.
</details>
</MathBox>
...
For $Y\in\mathfrak{X}(N)$ to be $F$-related to $\mathbf{X}$ means that $\mathrm{d}F_\mathbf{p}(\mathbf{\mathbf{X_p}}) = \mathbf{Y}_{F(\mathbf{p})}$ for every $\mathbf{p}\in M$. If $F$ is a diffeomorphism, therefore, we define $\mathbf{Y}$ by
...
The fact that $\mathbf{Y}$ is tangent to $S$ means by definition that $\mathbf{Y_p}$ is in the image of $\mathrm{d}\iota_\mathbf{p}$ for each $\mathbf{p}\in S$. Thus, for each $\mathbf{p}$ there is a vector $\mathbf{\mathbf{X_p}} \in T_\mathbf{p}S$ such that $\mathbf{Y_p} = \mathrm{d}\iota_\mathbf{p}(\mathbf{\mathbf{X_p}})$. Since $\mathrm{d}\iota_\mathbf{p}$ is injective, $\mathbf{\mathbf{X_p}}$ is unique, so this defines $\mathbf{X}$ as a rough vector field on $S$. If we can show that $\mathbf{X}$ is smooth, it is the unique vector field that is $\iota$-related to $\mathbf{Y}$. It suffices to show that it is smooth in a neighbourhood of each point.
...
$$
  [\mathbf{X},\mathbf{Y}]f = X\mathbf{Y}f - YXf
$$
</MathBox>
...
It suffices to show that $[\mathbf{X},\mathbf{Y}]$ is a derivation of $C^\infty(M)$. For arbitrary $f,g\in C^\infty (M)$ we gets
...
\begin{align*}
  ...
  &- \bcancel{\mathbf{Y}f\mathbf{X}g} - fYXg - YgXf - \bcance{g\mathbf{Y}Xf} \\
...
Let $X = X^i \frac{\partial}{\partial x^i}$ and $Y = Y^j \frac{\partial}{\partial x^j}$ be the coordinate expression for $\mathbf{X}$ and $\mathbf{Y}$ in terms of some smooth local coordinates $(x^i)$ for $M$. Then $[\mathbf{X},\mathbf{Y}]$ has the following coordinate expression
...
$$
  [\mathbf{X},\mathbf{Y}] = (XY^j - YX^j)\frac{\partial}{\partial x^j}
$$

<details>
<summary>Proof</summary>

Since $[\mathbf{X},\mathbf{Y}]$ is a smooth vector field, its action on a function is determined locally: $([\mathbf{X},\mathbf{Y}]f)|_U = [\mathbf{X},\mathbf{Y}](f|_U)$. Thus it suffices to compute in a single smooth chart, where we have
...
It is clear from the definition that $\varepsilon$ is linear over $\R$. First, we show that $\varepsilon$ is injective. If $\varepsilon(X) = \mathbf{X}_e = 0$ for some $X\in\mathrm{Lie}(G)$, the left-invariance of $\mathbf{X}$ implies that $\mathbf{X}_g = \mathrm{d}(L_g)_e(\mathbf{X}_e) = 0$ for every $g\in G$, so $X = 0$.
...
If there is any vector field $\mathbf{Y}\in\mathfrak{h}$ that is $F$-related to $\mathbf{X}$, it must satisfy $\mathbf{Y}_e = \mathrm{d}F_e (\mathbf{X}_e)$, and thus it must be uniquely determined by

$$
  Y = (\mathrm{d}F_d (\mathbf{X}_e))^\mathrm{L}
$$
...
$$
\begin{align*}
  \mathrm{d}F(\mathbf{X}_g) =& \mathrm{d}F(\mathrm{d}(L_g)(\mathbf{X}_e)) \\
  =& \mathrm{d}(L_{F(g)})(\mathrm{d}F(\mathbf{X}_e)) \\
  =& \mathrm{d}(L_{F(g)})(\mathbf{Y}_e) = \mathbf{Y}_{F(g)}
\end{align*}
$$
...
\begin{align*}
  ...
  =& \mathrm{d}F_{\gamma(t)}(X_{\gamma(t)}) = \mathbf{Y}_{F(\gamma(t))} = \mathbf{Y}_{\sigma(t)}
\end{align*}
$$
...
$$
  \mathbf{Y}_{F(\mathbf{p})} = (F\circ\gamma)'(0) = \mathrm{d}F_\mathbf{p}(\gamma'(0)) = \mathrm{d}F_\mathbf{p}(\mathbf{\mathbf{X_p}})
$$
...
Define $H:M\times I \to M$ b\mathbf{Y}
...
$$
\begin{align*}
  (\mathcal{L}_\mathbf{V} \mathbf{W})_\mathbf{p} =& \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t=0} \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})} (\mathbf{W}_{\theta_t (\mathbf{p})}) \\
  =& \lim_{t\to 0} \frac{\mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})}) - \mathbf{W}_\mathbf{p}}{t}
\end{align*}
$$
...
For small $t\neq 0$, the difference quotient makes sense: $\theta_t$ is defined in a neighbourhood of $\mathbf{p}$ and $\theta_{-t}$ is the inverse of $\theta_t$, so both $\mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})})$ and $\mathbf{W}_\mathbf{p}$ are elements of $T_\mathbf{p}M$.
...
$$
  \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{x})}(\mathbf{W}_{\theta_t (\mathbf{x})}) = \frac{\partial\theta^i}{\partial x^j}(-t, \theta(t,\mathbf{x}))\mathbf{W}^j(\theta(t, \mathbf{x})) \left.\frac{\partial}{\partial x^i}\right|_\mathbf{x}
$$
...
$$
\begin{align*}
  \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{u})}(\mathbf{W}_{\theta_t (\mathbf{u})}) =& \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{x})}\left( \mathbf{W}^j (u^1 + t,u^2,\dots,u^n) \left.\frac{\partial}{\partial u^j}\right|_{\theta_t (\mathbf{u})} \right) \\
...
$$
\begin{align*}
  (\mathcal{L}_\mathbf{V} \mathbf{W}) =& \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t=0} \mathbf{W}^j (u^1 + t, u^2,\dots,u^n)\left.\frac{\partial}{\partial u^j}\right|_\mathbf{u} \\
...
In this case, $\mathbf{V}\equiv 0$ on a neighbourhood of $\mathbf{p}$. On the one hand, this implies that $\theta_t$ is equal to the identity map in a neighbourhood of $\mathbf{p}$ for all $t$, so $\mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})}) = \mathbf{W}_\mathbf{p}$, which implies that $(\mathcal{L}_\mathbf{V} \mathbf{W})_\mathbf{p} = 0$. On the other hand $[\mathbf{V},\mathbf{W}]_\mathbf{p} = 0$.
</details>
</MathBox>
...
$$
  \left.\frac{\mathrm{d}}{\mathrm{d}t}\right|_{t=t_0} \mathrm{d}(\theta_{-t})_{\theta_t (\mathbf{p})}(\mathbf{W}_{\theta_t (\mathbf{p})}) = \mathrm{d}(\theta_{-t_0})\left((\mathcal{L}_\mathbf{V} \mathbf{W})_{\theta_{t_0}(\mathbf{p})} \right)
$$

<details>
<summary>Proof</summary>

Let $\mathbf{p}\in M$ be arbitrary, let $\mathcal{D}^{(\mathbf{p})}\subseteq\R$ denote the domain of the integral curve $\theta^{(\mathbf{p})}$, and consider the map $X:\mathcal{D}^{(\mathbf{p})} \to T_\mathbf{p}M$ given by $X(t) = \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(W_{\theta_t(\mathbf{p})})$. The argument in the proof of the previous lemma shows that $\mathbf{X}$ is a smooth curve in the vector space $T_\mathbf{p}M$. Making the change of variables $t = t_0 + s$ we obtain

$$
\begin{align*}
  X'(t_0) =& \left.\frac{\mathrm{d}}{\mathrm{d}s}\right|_{s=0} X(t_0 + s) \\
  =& \left.\frac{\mathrm{d}}{\mathrm{d}s}\right|_{s=0} \mathrm{d}(\theta_{-t_0 - s})(\mathbf{W}_{\theta_{s + t_0}(\mathbf{p})}) \\
...
Suppose $\mathbf{V},\mathbf{W}\in\mathfrak{X}(M)$, and let $\theta$ denote the flow of $\mathbf{V}$. If **(2)** holds, then $\mathbf{W}_{\theta_t(\mathbf{p})} = \mathrm{d}(\theta_t)_\mathbf{p}(\mathbf{W}_\mathbf{p})$ whenever $(t,\mathbf{p})$ is in the domain of $\theta$. Applying $\mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}$ to both sides, we conclude that $\mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})}) = \mathbf{W}_\mathbf{p}$, which obviously implies $[\mathbf{V},\mathbf{W}] = \mathcal{L}_\mathbf{V} \mathbf{W} = 0$ directly from the definition of the Lie derivative. The same argument shows that **(3)** implies **(1)**.

To prove that **(1)** implies **(2)**, assume $[\mathbf{V},\mathbf{W}] = \mathcal{L}_\mathbf{V} \mathbf{W} = 0$. Let $\mathbf{p}\in M$ be arbitrary, and let $X(t) = \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})})$ for $t\in \mathcal{D}^{(\mathbf{p})}$. The previous proposition shows that $X'(t) \equiv 0$. Since $X(0) = \mathbf{W}_\mathbf{p}$, this implies that $X(t) = \mathbf{W}_\mathbf{p}$ for all $t\in \mathcal{D}^{(\mathbf{p})}$, and applying $\mathrm{d}(\theta_t)_\mathbf{p}$ to both sides yields the identity that says $\mathbf{W}$ is invariant under $\theta$. The same proof also shows that **(1)** implies **(2)**.
</details>
</MathBox>
...
\begin{align*}
  ...
  =& \mathrm{d}(\psi_s)(\mathbf{V}_{\theta^{(\mathbf{p})}(t)}) = \mathbf{V}_{\gamma(t)}
\end{align*}
$$
...
$$
\begin{align*}
  \mathbf{W}_{\theta_t(\mathbf{p})} =& \left.\frac{\mathrm{d}}{\mathbf{d}s}\right|_{s=0} \psi^{\theta_t(\mathbf{p})}(s) \\
  =& \left.\frac{\mathrm{d}}{\mathrm{d}s}\right|_{s=0}\theta_t (\psi^{(\mathbf{p})}(s)) \\
  =& \mathrm{d}(\theta_t)_\mathbf{p}(\mathbf{W}_\mathbf{p})
\end{align*}
$$
...
$$
  \mathrm{d}(\theta_{-t})_{\theta_t(\mathbf{p})}(\mathbf{W}_{\theta_t(\mathbf{p})}) = \mathbf{W}_\mathbf{p}
$$
...
and define $\Phi:(-\varepsilon,\varepsilon)^k \times\Omega\to U$ b\mathbf{Y}
...
$$
  \mathrm{d}\Phi_0 \left( \left.\frac{\partial}{\partial \mathbb{S}^i}\right|_0 \right) = \mathbf{V}_i|_\mathbf{p},\; i=1,\dots,k
$$
...
Consider the smooth vector field $\tilde{V}$ on $I\times M$ defined b\mathbf{Y}
...
Let $\mathcal{E}$ be the subset of $\R\times I\times M$ defined b\mathbf{Y}
...
Defined $\psi:\mathcal{E}\to M$ b\mathbf{Y}
...
The key is to conver the dependent variable $u$ to an additional independent variable. Define the characteristic vector field for the quasilinear PDE to be the vector field $\xi$ on $W\subseteq\R^{n+1}$ given b\mathbf{Y}
...
As in the case of the tangent bundle, smooth local coordinates for a smooth $n$-manifold $M$ yield smooth local coordinates for its cotangent bundle. If $(x^i)$ are smooth coordinates on an open subset $U\subseteq M$, then the map from $\pi^{-1}(U)$ to $\R^{2n}$ given b\mathbf{Y}
...
<MathBox title='Differential' boxType='definition'>
Let $f \in C^\infty (M)$ be a smooth real-valued function on a smooth manifold $M$ with or without boundary. The *differential* of $f$ is a covector $\mathrm{d}f$ defined b\mathbf{Y}
...
We construct the isomorphism $V_1 \otimes V_2 \otimes V_3 \cong (V_1 \otimes V_2) \cong V_3$; the other one is constructed similarly. The map $\alpha: V_1\times V_2 \times V_3 \to (V_1 \otimes V_2) \otimes V_3$ defined b\mathbf{Y}
...
First define a map $\Phi: \prod_{i=1}^k V_i^* \to\mathrm{L}(V_i,\dots,V_k;\R)$ b\mathbf{Y}
...
Note that $^\tau(^\tau \alpha) = ^{\tau\sigma}\alpha$, where $\tau\sigma$ represents the composition of $\tau$ and $\alpha$, i.e. $\tau\sigma(i) = \tau(\sigma(i))$. We define a projection $\mathrm{sym}: T_k (V^*) \to\sigma_k (V^*)$ called *symmetrization* b\mathbf{Y}
...
Let $V$ be a finite-dimensional real vector space. If $\alpha\in\Sigma_k (V^*)$ and $\beta\in\Sigma_l (V^*)$, their *symmetric product* is the $(k+l)$-tensor $\alpha\beta$ defined b\mathbf{Y}
...
The simple example of a Riemannian metric is the *Euclidean metric* $\overline{g}$ on $\R^n$, given in standard coordinates b\mathbf{Y}
...
In any smooth coordinate $(x^i)$, we can write $g = g_{ij}\;\mathrm{d}x^i \;\mathrm{d}x^j$. Thus, if $\mathbf{X}$ and $\mathbf{Y}$ are smooth vector fields, we have
...
The most important use of the sharp operation is to reinstate the gradient as a vector field on Riemannian manifolds. For any smooth real-valued function $f$ on a Riemannian manofld $(M,g)$ with or without boundary, we define a vector field called the *gradient* of $f$ b\mathbf{Y}
...
Let $V$ be an $n$-dimensional vector space, and suppose $(\varepsilon^i)_{i=1}^n$ is any basis for $V^*$. For each multi-index $I = (i_1,\dots,i_k)$ of length $k$ such that $1 \leq i_1, \dots, i_k \leq n$, define a covariant $k$-tensor $\varepsilon^I = \varepsilon^{i_1 \dots i_k}$ b\mathbf{Y}
...
To shows that $\operatorname{span}(\mathcal{E}) = \Lambda_k (V^*)$, let $\alpha\in\Lambda_k (V^*)$ be arbitrary. For each multi-index $I = (i_1,\dots,i_k)$, define a real number $\alpha_I$ b\mathbf{Y}
...
Thus, the component functions $\omega_I$ of $\omega$ are determined b\mathbf{Y}
...
Recall that the vector calculus operatior on $\R^n$, the gradient of function $f\in C^\infty (\R^n)$ and the divergence of a vector field $X\in\mathfrak{X}(\R^n)$, are defined b\mathbf{Y}
...
$$
  \mathrm{d}\omega(\mathbf{X},\mathbf{Y}) = \mathbf{X}(\omega(\mathbf{Y})) - \mathbf{Y}(\omega(\mathbf{X})) - \omega([\mathbf{X},\mathbf{Y}])
$$

<details>
<summary>Proof</summary>

Since any smooth $1$-form can be expressed locally as a sum of terms of the form $u\;\mathrm{d}v$ for smooth functions $u$ and $v$, it suffices to consider that case. Suppose $\omega = u\;\mathrm{d}v$, and that $\mathbf{X}$ and $\mathbf{Y}$ are smooth vector fields. Then the left-hand side of the formula is

$$
\begin{align*}
  \mathrm{d}(u\;\mathrm{d}v)(\mathbf{X}, \mathbf{Y}) =& \mathrm{d}u \wedge \mathrm{d}v(\mathbf{X}, \mathbf{Y}) \\
...
$$
\begin{align*}
  &X(u\;\mathrm{d}v(Y)) - Y(u\;\mathrm{d}v(X)) - u\;\mathrm{d}v([\mathbf{X},\mathbf{Y}]) \\
...
$$
\begin{align*}
  &\mathrm{D}\omega(X_1,\dots,f\mathbf{\mathbf{X_p}},\dots,X_{k+1}) \\
  =& f\mathrm{D}\omega(X_1,\dots,\mathbf{X_p},\dots, X_{k+1})
\end{align*}
$$
...
Rearranging the arguments in these two sums so as put $\mathbf{X_p}$ into its original position, we see that they exactly cancel the sum $I$. This completes the proof that $\mathrm{D}\omega$ is multilinear over $C^\infty (M)$, so it defines a smooth $(k+1)$-tensor field.
...
We first topologize $\hat{M}$ by defining a basis for it. For each pair $(U,\mathcal{O})$, where $U$ is an open subset of $M$ and $\mathcal{O}$ is an orientation on $U$, define a subset $\hat{U}_\mathcal{O} \subseteq \hat{M}$ b\mathbf{Y}
...
Let $M$ be an oriented smooth $n$-manifold with corners, and suppose $\omega$ is an $(n-1)$-form on $\partial M$ that is compactly supported in the domain of a single oriented smooth chart with corners $(U,\varphi)$. The integral of $\omega$ over $M$ is defined b\mathbf{Y}
...
$$
  \iota_S^* (\beta(\mathbf{X})) = \langle \mathbf{X}, \mathbf{N} \rangle_g \;\mathrm{d}V_{\tilde{g}}
$$

where $\beta: \mathfrak{X}(M) \to\Omega^{n-1} (M)$ is a smooth bundle isomorphism defined by

$$
  \beta(\mathbf{X}) = \mathbf{X} \lrcorner\,\mathrm{d}V_g
$$
...
$$
\begin{align*}
  \iota_S^* (X^\perp \lrcorner \mathrm{d}V_g) =& \langle\mathbf{X},\mathbf{N}\rangle_g \iota_S^* (N \lrcorner \mathrm{d}V_g) \\
  =& \langle\mathbf{X},\mathbf{N}\rangle_g \mathrm{d}V_{\tilde{g}}
\end{align*}
$$
...
$$
  \div(\mathbf{X}) = *^{-1} \mathrm{d}(\beta(\mathbf{X}))
$$

or equivalently

$$
  \mathrm{d}(\mathbf{X} \lrcorner\,\mathrm{d}V_g) = (\div(\mathbf{X}))\mathrm{d}V_g
$$
</MathBox>
...
$$
  \int_M \div(\mathbf{X}) \mathrm{d}V_g = \int_{\partial M} \langle\mathbf{X},\mathbf{N}\rangle_g \;\mathrm{d}V_{\tilde{g}}
$$
...
$$
  \int_M \div(\mathbf{X})\;\mathrm{d}V_g = \int_M \mathrm{d}(\beta(X)) = \int_{\partial M} \iota_S^* \beta(X)
$$
...
\begin{align*}
  ...
  =& \int_D \theta_{t_0}^* (\div(\mathbf{X})\;\mathrm{d}V_g) \\
  =& \int_{\theta_{t_0}(D)} \div(\mathbf{X})\;\mathrm{d}V_g
\end{align*}
$$
...
$$
  \operatorname{\curl}(\mathbf{X}) = \beta^{-1} \;\mathrm{d}(\mathbf{X}^\flat)
$$
...
$$
  \operatorname{\curl}(\mathbf{X}) \lrcorner \mathrm{d}V_g = \mathrm{d}(\mathbf{X}^\flat)
$$
</MathBox>
...
$$
  \int_S \langle \operatorname{\curl}(\mathbf{X}),\mathbf{N}\rangle_g \;\mathrm{d}A = \int_{\partial S} \langle\mathbf{X},\mathbf{T}\rangle_g \;\mathrm{d}s
$$

where $\mathbf{N}$ is the smooth unit normal vector field along $S$ that determines its orientation, $\mathrm{d}s$ is the Riemannian volume form for $\partial S$ with respect to the metric and orientation induced from $S$, and $\mathbf{T}$ is the unique positively oriented unit tangent vector field on $\partial S$.
...
$$
  \int_S \mathrm{d}(\mathbf{X}^\flat) = \int_{\partial S} \mathbf{X}^\flat
$$
...
We will construct local trivializations and use the vector bundle chart lemma. Let $(U, (x^i))$ be any smooth coordinate chart on $M$, and let $\omega = \bigwedge_{i=1}^n \mathrm{d}_x^i$. By density properties, $|\omega_\mathbf{p}|$ is a basis for $\mathcal{D}(T_\mathbf{p}M)$ at each point $\mathbf{p}\in U$. Thus, the map $\Phi: \pi^{-1} (U)\to U\times\R$ given b\mathbf{Y}
...
If $F:M\to N$ is a smooth map between $n$-manifolds with or without boundary and $\mu$ is a density on $N$, we define a denisty $F^* \mu$ on $M$ b\mathbf{Y}
