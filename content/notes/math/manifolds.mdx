...
$$
  U_{\pm i} := \Set{ x \in \R^{n+1} | \pm x_i > 0 }
$$
...
where $U_{\pm i} := \Set{ x \in \R^{n+1} | \pm x_i > 0 }$ are open hemispheres. Homeomorphic charts $h_i : V_i \rightarrow V'_i \subseteq \R^n$ can be formed by taking the slopes wrt the $i$-component
