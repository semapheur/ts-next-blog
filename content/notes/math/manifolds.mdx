...
<MathBox title='Integral of differential forms over oriented manifolds' boxType='proposition'>
...
with the positive sign for a positively oriented chart, and the negative sign otherwise. Since $(\varphi^{-1})^* \omega$ is a compactly supported $n$-form on the open subset $\varphi(U)\subseteq\R^n$ or $\mathbb{H}^n$, its integral is define as discussed above.
</MathBox>