...
3. $\mathcal{T}$ is closed under finite intersection: $\mathcal{A}\subseteq\mathcal{T} \land \mathcal{A}\textrm{ is finite } \implies \bigcap \mathcal{A} \in \mathcal{T}$
...
1. $M$ is *Hausdorff space*: for every pair of distinct points $p,q\in M$ there are disjoint subsets $U, V\subseteq M$ such that $p\in U$ and $q \in V$.
...
A (coordinate) chart on a topolical $n$-manifold $M$ is a pair $(U, \varphi)$ where $U$ is an open subset of $M$, called a coordinate domain, and $\varphi:U\to U'$, called a (local) coordinate map, is a homemorphism to an open subset $U' = \varphi(U)\subseteq\R^n$. The collection of charts $\Set{ \left( U_\alpha, \varphi_\alpha \right) }$ covering $M$, i.e. $M = \bigcup_\alpha U_\alpha$, is called an atlas. 
</MathBox>
...
there is an open neighbourhood $U \in\mathcal{T}$ containing $p$ and a homeomorphism $\varphi: U \rightarrow U'$ with $U' \subseteq \R^n$ open. The map $\varphi$ is called a chart of $M$. 
...
Let $f:B^n \to \R$ be the continuous function $f(\mathbf{u}) = \sqrt{1 - \lVert\mathbf{u}\rVert^2$. For each $i=1,\dots,n+1$, it is easily seen that $U_i^+ \cap S^n$ is the grap of the function
...
<LatexFig width={50} src='/fig/n_sphere.svg' alt=''
