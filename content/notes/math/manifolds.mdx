...
$$
  \operatorname{grad}(f) = \mathrm{grad}(f) = (\mathrm{d}f)^\sharp = \hat{g}^{-1}(\mathrm{d}f)
$$
...
Thus $\operatorname{grad}(f)$ is the unique vector field that satisfies $\langle\operatorname{grad}(f), X\rangle_g = Xf$ for every vector field $X$, or equivalently 
