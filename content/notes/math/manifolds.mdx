...
If $x\neq y$, there exists disjoint neighbourhoods $A$ and $B$ of $x$ and $y$, respectively. There exist $M, N \in\N$ such that $x_n \in A$ for all $n \geq M$ and $x_n \in B$ for all $n\geq N$. However, if $n \geq\max\{M, N\}$, then $x_n \in A$ and $x_n \in B$, giving a contradiction.
</details>
</MathBox>
...
A projective space $P^n (\R)$ is the set  of 1-dimensional subspaces of $\R^{n+1}$. It is defined as the quotient space $P^n := \mathbb{S}^n \setminus \sim$ with the equivalence relation $x \sim y :\iff (x = y \textrm{ or } x = -y)$ and where 

$$
  \mathbb{S}^n := \{ x \in \R^{n+1} \,|\, \lVert x \rVert = 1 \}
$$
...
An $n$-dimensional topological manifold $M$ is a Hausdorff topological space $(X, \mathcal{T})$ with a second-countable basis for the topology which is locally homeomorphic to $\R^n$ (Euclidean space).

The local homeomorphism to the Euclidean space implies that for every point $p \in M$ there is an open neighbourhood $U \in \mathcal{T}$ and a homeomorphism $h: U \rightarrow U'$ with $U' \subseteq \R^n$ open. The map $h$ is called a chart of $M$. The collection of charts $\left\{ \left( U_\alpha, \phi_\alpha \right) \right\}$ covering $M$, i.e. $M = \bigcup_\alpha U_\alpha$, is called an atlas.
...
$$
  S^n := \left\{ x \in \R^{n+1} |\; \lVert x \rVert = 1 \right\}
$$

The $n$-sphere can be covered by open hemispheres given by 

$$
  U_{\pm i} := \left\{ x \in \R^{n+1} \middle| \pm x_i > 0 \right\}
$$

Homemorphic charts $h: S^2 \rightarrow \R^n$ can be formed by removing the $i$-th component
...
Recall that the projective space is defined as $P^n (\R) := S^n / \sim$ with the equivalence relation $x\sim y :\iff x=\pm y$. The canonical projection $q: S^{n} \rightarrow S^n / \sim$ is given by $x \mapsto [x]_\sim$,  The projective space can be covered by open sets

$$
  V_i := \left\{ [x]_\sim \in P^n (\R) \mid x_i \neq 0 \right\}
$$
...
where $U_{\pm i} := \left\{ x \in \R^{n+1} \middle|\; \pm x_i > 0 \right\}$ are open hemispheres. Homeomorphic charts $h_i : V_i \rightarrow V'_i \subseteq \R^n$ can be formed by taking the slopes wrt the $i$-component
...
Let $M$ be a topological $n$-manifold, and let $(U, \phi)$ and $(V, \psi)$ be two charts on $M$. If $U \cap V \neq \emptyset$, we may define the chart transformation $\omega := \psi \circ \phi^{-1} : \phi(U \cap V) \subseteq \R^n \mapsto \psi(U \cap V) \subseteq \R^n$. The two charts $\phi$ and $\psi$ are called $C^k$-smoothly compatible if $\omega$ is a $C^k$-diffeomorphism.

An atlas $\mathcal{A} = \left\{ (U_i, \alpha_i)_{i\in I \subseteq \N} \right\}$ on $M$ is called a $C^k$-atlas (smooth atlas) if any two charts are $C^k$-smoothly compatible. A smooth atlas $\mathcal{A}$ is maximal (complete) if it is not properly contained in any larger smooth atlas. A smooth structure on $M$ is a maximal smooth atlas.
...
$$
  \mathrm{T}_p M := \mathrm{d}\phi_{\tilde{p}}[\R^k] = \{ J_\phi \left( \phi^{-1}(p)x \right) | x\in\R^k \} \subseteq\R^n 
$$
