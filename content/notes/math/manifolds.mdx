...
Note that $\mathcal{T} = \{ U^c \mid U\in\mathcal{C} \}$ defines a collection of open sets that satsify the axioms of a topology
...
$$
  \left( \bigcup\mathcal{A} \right)^c &= \bigcap \left\{ A^c \mid A\in\mathcal{A} \right\} \in\mathcal{C}
$$
2. for an arbitrary collection $\mathcal{A}\subseteq\mathcal{T}$, then $\bigcup\mathcal{A}\in\mathcal{T}$. Noting that $\mathcal{A}^c = \{A^c \mid A\in\mathcal{A}\subseteq\mathcal{T}\}\subseteq\mathcal{C}$. Applying DeMorgan's law shows that $\mathcal{C}$ is closed under arbitrary intersection
$$
  \left( \bigcup\mathcal{A} \right)^c = \bigcap \left\{ A^c \mid A\in\mathcal{A} \right\} = \bigcap \mathcal{A}^c \in\mathcal{C}
$$
3. for a finite collection $\mathcal{A}\subseteq\mathcal{T}$, then $\bigcap\mathcal{A}\in\mathcal{T}$. Noting that $\mathcal{A}^c = \{A^c \mid A\in\mathcal{A}\subseteq\mathcal{T}\}\subseteq\mathcal{C}$. Applying DeMorgan's law shows that $\mathcal{C}$ is closed under finite union
$$
  \left( \bigcap\mathcal{A} \right)^c = \bigcup \left\{ A^c \mid A\in\mathcal{A} \right\} = \bigcup \mathcal{A}^c \in\mathcal{C}
$$
</details>
</MathBox>
...
$$
  \overline{S} := \left\{\bigcap C \subseteq X \mid C\textrm{ is closed and }S\subseteq C \right\}
$$
...
$$
  \mathrm{int}(S) := \left\{\bigcup U\subseteq X \mid U\textrm{ is open and }U\subseteq S \right\}
$$
...
$$
  \mathrm{ext}(S) := \left\{\bigcup U\subseteq X \mid U\textrm{ is open and }U\subseteq X\setminus S \right\}
$$
...
**Property 1:** We will show that $C^c = X\setminus C$ is open, so fix $x\in C^c$. Since $C$ is Hausdorff, each $y\in C$ can be separated from $x$, meaning there are disjoint open sets $U_y, V_y\in\mathcal{T}$ with $x\in U_y$ and $y\in V_y$. Trivially, the collection $\{ V_y \mid y\in C\}$ form an open cover of $C$

$$
  C \subseteq \bigcup_{y \in C} V_y
$$

Since $C$ is compact, it has a finite subscover and hence there is a finite subset $B\subseteq C$ such that $\{ V_y \mid y\in B \}$ covers $C$. But then $U = \bigcap_{y\in B} U_y$ is open (since $B$ is finite) and disjoint from $\bigcup_{y\in B} V_y$. Thus, $U$ is also disjoint from $C$. To summarize, $U$ is open and $x\in U\subseteq C^c$

**Property 2:** For each $y\in C$ there exist disjoint open sets $U_y$ and $V_y$ with $x\in U_y$ and $y\in V_y$. Trivially, the collection $\{V_y \mid y\in C\}$ is an open cover of $C$, and by compactness there exists a finite subset $B\subseteq C$ such that $\{V_y \mid y\in B\}$ covers $C$. Thus, let $U = \bigcap_{y\in B} U_y$ and $V = \bigcup_{y\in B} V_y$. Then $U$ is open, since $B$ is finite, and $V$ is open. Moreover $U$ and $V$ are disjoint, and $x\in U$ and $C\subseteq V$.
...
$$
  \mathcal{B} := \left\{ \prod_{i\in I} U_i \mid U_i \in\mathcal{T}_i \textrm{ and }U_i = X_i \textrm{ for all but a finite number of }i\in I \right\}
$$
<details>
<summary>Proof</summary>
...
$$
  V_i := \left\{ [x]_\sim \in P^n (\mathbb{R}) \mid x_i \neq 0 \right\}
$$
...
$$
  \mathrm{T}_p M = \{ \gamma'(0) \mid \gamma:(-\varepsilon, \varepsilon) \to M \textrm{ differentiable with } \gamma(0) = p \}
$$
...
This shows that $T_p M \subseteq \{ \gamma'(0) \mid \gamma:(-\varepsilon, \varepsilon) \to M \textrm{ differentiable with } \gamma(0) = p \}$
...
This shows that $T_p M \supseteq \{ \gamma'(0) \mid \gamma:(-\varepsilon, \varepsilon) \to M \textrm{ differentiable with } \gamma(0) = p \}$
</details>
</MathBox>
...
$$
  C_p (M) := \{ \gamma:(-\varepsilon, \epsilon) \to M \mid \gamma \textrm{ differentiable } \gamma(0) = p \}
$$

For any chart $h: U\subseteq M \to\mathbb{R}^n$ there is an equivalence relation

$$
  \gamma\sim\alpha :\iff (\h\circ\gamma)'(0) = (h\circ\alpha)'(0)
$$

The equivalence class $[\gamma]_\sim := \{\alpha \mid \gamma\sim\alpha \}$ represents a tangent vector. The tangent space of $M$ at $p$, denoted $T_p M$ is defined as the set of all tangent vectors at $p$
...
Let $M$ be an $n$-dimensional smooth manifold having a chart $h:U\subseteq M \to \mathbb{R}^n$ with inverse $\phi = h^{-1}$. Since the tangent space $T_p M$ is isomorphic to $\mathbb{R}^n$ there is a linear and bijective map $h_* : T_p M \to \mathbb{R}^n$ given by $[\gamma]\mapsto (h\circ\gamma)'(0)$ where the equivalence class $[\gamma]$ is a tangent vector in $T_p M$. The coordinate basis with respect to $h$ is defined as

$$
  \partial_j := \phi_* (e_j)
$$

where $\phi_* := h_*^{-1}$ and $(e_i)_{i=1}^n$ is the standard basis of $\mathbb{R}^n$.
</MathBox>
...
$$
  TM := \bigscup_{p\in M} T_p M := \bigcup_{p\in M} \{p\} \times T_p M
$$
