...
### Bounds
<MathBox title='Upper and lower bound' boxType='definition'>
...
<MathBox title='Bolzano-Weierstrass theorem' boxType='theorem'>
...
<MathBox title='Cauchy sequence' boxType='proposition'>
...
<MathBox title='Series' boxType='definition'>
...
<MathBox title='Properties of series' boxType='proposition'>
...
<MathBox title='Geometric series' boxType='example'>
...
<MathBox title='Harmonic series' boxType='example'>
...
<MathBox title='Cauchy criterion' boxType='criterion'>
...
$$
  \left| \sum_{k=m}^n a_k \right| \tleq \sum_{k=m}^n \left| a_k \right| < \varepsilon
$$
...
<MathBox title='Leibniz criterion' boxType='criterion'>
...
<MathBox title='Majorant criterion (convergence)' boxType='criterion'>
...
<MathBox title='Majorant criterion (divergence)' boxType='criterion'>
...
<MathBox title='Reordering' boxType='definition'>
...
\begin{align*}
  ...
  &\tleq \left| \sum_{k=1}^\infty a_k - \sum_{k=1}^{N_1 - 1} a_k \right| + \left| \sum_{k=1}^{N_1 - 1} a_k - \sum_{k=1}^n a_{\tau(k)} \right|
\end{align*}
$$
...
<MathBox title='Cauchy product' boxType='definition'>
...
<MathBox title='Real function' boxType='definition'>
A real function is a map $f: I \subseteq\R\to\R$. The function is bounded if its range $f[I] = \operatorname{ran}(f) = \Set{f(x) | x \in I}$ is bounded in $\R$. Equivalently, this holds if the supremum of the functions absolute value is finite
$$
...
Uniform convergence means $\lim_{n\to \infty} \norm{ f_n - f}_\infty = \sup_{x\in I} \left| f_n (x) - f(x) \right| = 0$. The condition for uniform convergence is more rigorous than for pointwise convergence. Thus uniform convergence guarantees pointwise convergence, and not vice versa.
</MathBox>
...
<MathBox title='Epsilon-delta criterion for continuity' boxType='criterion'>
...
$$
  f'(a) \equiv \frac{\d f}{\d x}(a) := \lim_{x\to a} \frac{f(x)-f(a)}{x - a}
$$
...
$$
  \Delta_{f, a} \equiv f'(a) \equiv \frac{\d f(a)}{\d x} := \lim_{x \to a} \frac{f(x) - f(a)}{x - a}
$$

This can alternatively be formulated as 
$$
  \frac{\d f(x)}{\d x} \equiv f'(x) := \lim_{h \to 0}{\frac{f(x + h) - f(x)}{h}}
$$
...
Let $f: I \subseteq \R \to \R$ and define inductively $f^{(n)} := \left( f^{(n-1)} \right)' = \frac{\d}{\d x}\left(\frac{\d^{n-1}f}{\d x^{n-1}}\right)$ for $n \in \N$,  where $f^{(0)} := f$.
...
<MathBox title='Product rule' boxType='proposition'>
...
<MathBox title='Quotient rule' boxType='proposition'>
...
<MathBox title='Chain rule' boxType='proposition'>
...
\begin{gather*}
  ...
  \left. \frac{\d f\left[g(x)\right]}{\d x} \right|_{x_0} = \left. \frac{\d f(g)}{\d g} \right|_{g(x_0)} \cdot \left. \frac{\d g(x)}{\d x} \right|_{x_0}
\end{gather*}
$$
...
- There is $g: I \to \R$ with $\norm{ f_n' - g}_\infty \xrightarrow{n\to\infty} 0$

Then $\norm{ f_n' - f}_\infty \xrightarrow{n\to\infty}{\longrightarrow} 0$ and $f$ is differentiable with $f' = g$.
<details>
<summary>Proof</summary>
...
<MathBox title="Rolle's theorem" boxType='theorem'>
...
$$
  R_n (h) \equiv O\left(h^{n+1}\right)
$$
...
$$
\begin{align*}
  F_{n, h}' (t) &= \frac{\d}{\d t} \sum_{k=0}^n \frac{f^{(k)}(t)}{k!}(h + x_0 - t)^k \\
...
<MathBox title='Riemann-integrable function' boxType='definition'>
A bounded function $f: [a, b] \to \R$ is Riemann-integrable if
$$
  \sup\Set{ \int_a^b \phi(x) \d x | \phi \in S([a, b]),\; \phi \leq f } = \inf\Set{ \int_a^b \phi(x) \d x | \phi \in S([a, b]),\; \phi \geq f}
$$

In this case $\int_a^b f(x) \d x$ is called the (Riemann) integral of $f$. The set of all Riemann-integrable functions on an interval $[a, b]$ is denoted
$$
...
$$
  \int_c^d f(x) \d x := \int_c^d f|_{[c, d]} (x) \d x
$$
...
<MathBox title='' boxType='proposition'>
The Riemann integral defines a linear and monotonic map $\int_a^b f(x) \d x : \mathcal{R}([a, b]) \to \R$.
</MathBox>
...
$$
  \int_a^b \phi(x) \d x := \sum_{j=1}^n c_j \left( x_j - x_{j-1} \right)
$$

and describes a map $S([a, b]) \to \R$ given by $\phi \mapsto \int_a^b \phi(x) \d x$, which is linear and monotonic.
...
<MathBox title='' boxType='proposition'>
The Riemann integral of step functions $\int_a^b \phi(x) \d x : S([a, b]) \to \R$ defines a linear and monotonic map satisfying:
- Homogeneous: for $\lambda \in \R$: $\int_a^b \lambda \phi(x) \d x = \lambda\int_a^b \phi(x) \d x$
- Additive: for $\phi, \psi \in S([a, b])$: $\int_a^b (\phi + \psi)(x) \d x = \int_a^b \phi(x) \d x + \int_a^b \psi(x) \d x$
- Monotonic: for $\phi, \psi \in S([a, b])$ with $\phi \leq \psi$, then $\int_a^b \phi(x) \d x \leq \int_a^b \psi(x) \d x$
<details>
<summary>Proof</summary>
...
$$
\begin{align*}
  \int_a^b \phi(x) \d x + \int_a^b \psi(x) \d x &= \sum_{j=1}^N c_j \left(\hat{x}_{j} - \hat{x}_{j-1}\right) + \sum_{j=1}^N d_j \left( \hat{x}_{j} - \hat{x}_{j-1} \right) \\
  &= \sum_{j=1}^N \left( c_j + d_j \right) \left( \hat{x}_{j} - \hat{x}_{j-1} \right) \\
  &= \int_a^b (\phi + \psi)(x) \d x
\end{align*}
$$
</details>
</MathBox>
...
1. $\lim_{b\to\infty}\int_a^b$ exists, then $\int_a^\infty f(x)\d x$ converges 
2. $\lim_{a\to -\infty} f(x) \d x$ exists, then $\int_{-\infty}^b f(x)\d x$ converges

If there is a $c \in \R$ such that $\int_{-\infty}^c f(x)\d x$ and $\int_c^\infty f(x)\d x$ converge, we can define
...
Let $f: (a, b] \to \R$ be a function with the property that $f|_{[a+\varepsilon, b]} \in \mathcal{R}([a, b])$ for all $\varepsilon > 0$. The integral $\int_a^b f(x) \d x$ converges if the following limit exists
$$
  \lim_{\varepsilon \to 0} \int_{a+\varepsilon}^b f(x) \d x
$$

For a function $f:[a, b]\backslash \Set{p} \to \R$ with a pole at $p$, the improper Riemann integral is defined as
$$
  \int_a^b f(x) \d x := \lim_{\varepsilon_1 \to 0} \int_a^{p-\varepsilon_1} f(x)\d x + \lim_{\varepsilon_2 \to 0} \int_{p-\varepsilon_2}^b f(x)\d x
$$
...
1. $f$ has a singularity at $c\in [a, b]$ such that $\int_a^c f(x) \d x = \pm \infty$ and $\int_c^b f(x)\d x = \mp \infty$, then the Cauchy principal value is defined as
$$
  \mathrm{p.v.}\int_a^b f(x)\d x := \lim_{\varepsilon\to 0} \left( \int_a^{c-\varepsilon} f(x)\d x + \int_{c+\varepsilon}^b f(x)\d x \right)
$$

2. $f$ has singularities at infinity such that $\int_{-\infty}^0 f(x)\d x = \pm\infty$ and $\int_0^\infty f(x)\d x = \mp\infty$, then the Cauchy principal value is defined as
$$
  \mathrm{p.v.}\int_{-\infty}^\infty f(x)\d x := \lim_{a\to \infty} \int_{-a}^a f(x)\d x
$$
</MathBox>
...
$$
  \int_a^b f(x)g(x) \d x = f\left(\hat{x}\right) \int_a^b g(x) \d x
$$

In the special case where $g = 1$, this reduces to
$$
  \int_a^b f(x) \d x = f\left(\hat{x}\right) (b - a)
$$
...
$$
  m\int_a^b g(x) \d x \leq \int_a^b f(x) g(x) \d x \leq M \int_a^b g(x) \d x
$$

This implies there is $\mu \in [m, M]$ such that
$$
  \mu \int_a^b g(x) \d x = \int_a^b f(x) g(x) \d x
$$
...
$$
\begin{gather*}
  F(x + h) - F(x) = \int_x^{x+h} f(t) \d t = f\left(\hat{x}\right),\; \hat{x} \in [x, x + h] \\
...
$$
  \int_a^b f(x) \d x = F(b) - F(a) := \left. F(x) \right|_a^b \, , \quad F'(x) = f(x)
$$

<details>
<summary>Proof</summary>

By definition $F(x) := \int_a^x f(t) \d t$ is an anti-derivative of $f$ with $F(a) = 0$. Choosing an arbitrary antiderivative of $f$ by $\tilde{F} = F + c$ for some constant $c \in \R$ we get

$$
  \tilde{F}(b) - \tilde{F}(a) = F(b) - F(a) = \int_a^b f(t)\d t
$$
</details>
</MathBox>
...
<MathBox title='Integration by substitution' boxType='proposition'>
Let $f: I \subseteq{I} \to \R$ be continuous, and $\phi: [a, b] \to I$ continuously differentiable. Substituting $x = \phi(t)$ with $\frac{\d x}{\d t} = \phi'(t)$, then 
$$
  \int_a^b f\left( \phi(t) \right) \underbrace{\phi'(t)}_{\d x\d t} = \int_{\phi(a)}^{\phi(b)} f(x) \d x
$$
...
$$
  \int_a^b f(x) \d x = \int_{\phi^{-1}(a)}^{\phi^{-1}(b)} f\left( \phi(t) \right)\phi'(t) \d t
$$
...
$$
\begin{align*}
  \int_a^b f\left(\phi(t)\right)\phi'(t) \d t &= \int_a^b \left(F\circ\phi\right)'(t)\d t = \left.\left(F\circ\phi\right)(t)\right|_{t=a}^{t=b} \\
  &= \left. F(x) \right|_{x=\phi(a)}^{x=\phi(b)} = \int_{\phi(a)}^{\phi(b)}f(x) \d x
\end{align*}
$$
</details>
</MathBox>

<MathBox title='Integration by parts (partial integration)' boxType='proposition'>
Let $f, g: [a, b] \to \R$ be continuously differentiable functions. Then

$$
  \int_a^b f'(x) g(x) \d x = \left.f(x)g(x) \right|_{x=a}^{x=b} - \int_a^b f(x) g'(x) \d x
$$
...
$$
  \int_a^b (f\cdot g)'(x)\d x = \int_a^b f'(x)g(x)\d x + \int_a^b f(x)g(x) \d x
$$

By the second fundamental theorem of calculus
$$
  \int_a^b (f\cdot g)'(x)\d x = \left. f(x)g(x) \right|_a^b
$$
...
<MathBox title='Integration by partial fraction decomposition' boxType='proposition'>
...
$$
  \int_a^b \frac{p(x)}{q(x)} \d x = \int_a^b \left[ \sum_{i=1}^k \sum_{j=1}^{\alpha_i} \left(\frac{a_i}{x - x_i}\right)^j \right] \d x  = \sum_{i=1}^k \sum_{j=1}^{\alpha_i} \left[ \int_a^b \left(\frac{a_i}{x - x_i}\right)^j\d x \right]
$$
</MathBox>
...
Letting $u = (\sin x)^{n-1}$ and $\d v = \sin x \;\d x$ so that $v = -\cos x$ and $\d u = (n - 1) (\sin x)^{n-2} \cos x \;\d x$, we get

$$
\begin{align*}
  \int \sin^n x \;\d x =& \int (\sin x)^{n-1} \sin x \;\d x \\
  =& -(\sin x)^{n-1} \cos x + (n - 1) \int (\sin x)^{n-2} \underbrace{\cos^2 x}_{= 1 - \sin^2 x} \;\d x \\
  =& -(\sin x)^{n-1} \cos x + (n - 1) \int (\sin x)^{n-2} \;\d x - (n - 1) \int \sin^n x \;\d x
\end{align*}
$$

The result follows by moving the last term on the right over to the left and dividing by $n$.
</MathBox>
...
1. $|f(x)| \leq g(x) \; \forall x\in[a, \infty)$, then if $\int_a^\infty g(x) \d x$ converges so does $\int_a^\infty f(x) \d x$.
2. $g(x) \leq f(x) \; \forall x\in[a, \infty)$, then if $\int_a^\infty g(x) \d x$ diverges so does $\int_a^\infty f(x) \d x$.

## Integral test for series

Let $f: [0, \infty) \to [0, \infty)$ be monotonically decreasing. Then $\sum_{k=0}^\infty$ is convergent if and only if $(\iff)$ $\int_0^\infty f(x)\d x$ is convergent. In this case
$$
  0 \leq \sum_{k=0}^\infty f(k) - \int_0^\infty f(x)\d x \leq f(0)
$$
...
$$
  f(k) = \int_{k-1}^k f(k) \d x \leq \int_{k-1}^k f(x)\d x \leq \int_{k-1}^k f(k - 1) \d x = f(k - 1)
$$

Applying a sum preserves the inequalities
$$
  \sum_{k=1}^n f(k) \leq \sum_{k=1}^n \int_{k-1}^k f(x)\d x \leq \sum_{k=1}^n f(k-1) = \sum_{k=0}^{n-1} f(k) 
$$

In the limit $n\to \infty$ we get
$$
  \sum_{k=1}^\infty \leq \int_0^\infty f(x)\d x \leq \sum_{k=0}^\infty f(k)
$$
</details>
...
$$
  I_a^n f(x) := \int_a^x \int_a^{x_{n-1}}\cdots\int_a^{x_1} f(x_0)\;\d x_0\dots\d x_{x-1},\quad n\in\N
$$

is given by

$$
  I_a^n f(x) = \frac{1}{(n - 1)!}\int_a^x (x - t)^{n-1}f(t)\;\d t
$$
...
$$
  I_a^1 f(x) = \frac{1}{0!}\int_a^x (x - t)^0 f(t)\;\d t = \int_a^x f(t)\;\d t
$$

Suppose the formula is true for $n\in\N$. Note that by the Leibniz integral rule

$$
  \frac{\d}{\d x}\left[ \frac{1}{n!}\right] \int_a^x (x-t)^n f(t)\,mathrm{d}t = \frac{1}{(n - 1)!}\int_a^x (x-t)^{n-1} f(t)\;\d t
$$
...
$$
\begin{align*}
  I_a^{n+1} f(x) &= \int_a^x \int_a^{x_1}\cdots\int_a^{x_n} f(x_n)\;\d x_{n+1}\d x_n\dots\d x_1 \\
  &= \int_a^x \frac{1}{(n - 1)!}\int_a^{x_1} (x_1 - t)^{n-1} f(t)\;\d t\,mathrm{d}x_1 \\
  &= \int_a^x \frac{\d}{\d x_1} \left[\frac{1}{n!} \int_a^{x_1} (x_1 - t)^n f(t)\;\d t \right]\,mathrm{d}x_1 \\
  &= \frac{1}{n!} \int_a^x (x - t)^n f(t)\;\d t
\end{align*}
$$
</details>
</MathBox>
...
$$
  I^n f(x) = \frac{1}{(n - 1)!}\int_a^x (x - t)^{n-1} f(t) \d t
$$

## Convolution

$$
  (g * f)(x) = \int_0^x g(x - u)f(u) \d u 
$$
...
$$
\begin{align*}
  \left( \int_{-\infty}^\infty e^{-x^2}\d x \right)^2 &= \int_{-\infty}^\infty e^{-x^2}\d x \, \int_{-\infty}^\infty e^{-y^2}\d y = \int_{-\infty}^\infty \int_{-\infty}^\infty e^{-(x + y)^2}\d x\d y \\
  &= \int_0^{2\pi} \int_0^\infty e^{-r^2}r\d r \d\theta \\
  &= 2\pi \int_0^\infty r e^{-r^2} \d r \\
  &= 2\pi \int_{-\infty}^0 \frac{1}{2} e^s \d s\, , \quad s = -r^2\, , \quad \d r = -\frac{\d s}{2r} \\
  &= \pi \int_{-\infty}^0 e^s \d s \\
  &= \pi
\end{align*}
$$
