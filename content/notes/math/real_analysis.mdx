...
A set $I \subseteq \mathbb{R}$ is called a neighbourhood of $x \in \mathbb{R}$ if there is an $\varepsilon > 0$ such that $I \supset B_\varepsilon (x)$ where $B_\varepsilon (x)$ is the $\varepsilon$-neighbourhood of $x$
...
## Convergence
A sequence $(a_n)_{n\in \mathbb{N}}$ is called convergent to $a \in \mathbb{R}$ if for any $\varepsilon > 0$ there exists $N \in \mathbb{N}$ such that $\left| a_n - a \right| < \varepsilon$ for all $n \geq N$. If there is no such $a \in \mathbb{R}$, the sequence is divergent.
...
$$
\begin{gather*}
  \exists N \in \mathbb{N} \; \forall n \geq N: \; \left| a_n - a \right| < \varepsilon \\
  \exists \tilde{N} \in \mathbb{N} \; \forall n \geq \tilde{N}: \; \left| a_n - \tilde{a} \right| < \varepsilon
\end{gather*}
$$
...
The sequence $(a_n)_{n\in \mathbb{N}} = (\frac{1}{n})_{n\in \mathbb{N}}$ converges to $0$. For $\varepsilon > 0$ we choose $N \in \mathbb{N}$ such that $N\varepsilon > 1$. Then for $n \geq N$ we have
...
The sequence $(a_n)_{n\in \mathbb{N}} = ((-1)^n)_{n\in \mathbb{N}}$ is divergent. Assume the sequence is convergent to $a \in \mathbb{R}$. Then for any $\varepsilon > 0$ there is an $N \in \mathbb{N}$ such that $\left| a_n - a \right| < \varepsilon$. Choosing $\varepsilon = 1$ we have
...
$$
  a = \limsup_{n\to\infty} a_n = \lim_{n \to \infty} \sup\left\{ a_k \mid k \geq n \right\}
$$
- limit inferior of $\left( a_n \right)_{n\in\mathbb{N}}$ if $a$ is the smallest (improper) accumulation value of $a_n$
$$
  a = \liminf_{n\to\infty} a_n = \lim_{n \to \infty} \inf\left\{ a_k \mid k \geq n \right\}
$$
...
If $\forall \varepsilon > 0$ there is an $N \in\mathbb{N}$ such that $\forall n, m \geq N: \left| a_n - a_m \right| < \varepsilon$, then $\left( a_n \right)_{n\in\mathbb{N}}$ is called a Cauchy sequence. A Cauchy sequence is equivalent to a convergent sequence.

<details>
<summary>Proof</summary>

Let $(a_n)_{n\in\mathbb{N}}$ be a convergent sequence with $\lim_{n\to \infty} a_n = a$. Consider $\varepsilon > 0$ and set $\varepsilon' := \frac{\varepsilon}{2}$. Since $(a_n)_{n\in\mathbb{N}}$ is convergent, there is $N \in \mathbb{N}$ such that
$$
...
Let $\varepsilon > 0$. Then there is an $N \in \mathbb{N}$ with $\forall n\geq N: \left| b_n - a_n \right| \geq \left| d_n - 0 \right| < 0$. This implies that $\left( d_n \right)_{n\in\mathbb{N}}$ is convergent with $\lim_{n\to\infty} d_n = 0$. Thus $\left( c_n \right)_{n\in\mathbb{N}} = \left( d_n + a_n \right)_{n\in\mathbb{N}}$ is convergent with $\lim_{n\to\infty} c_n = \lim_{n\to\infty} a_n$.
</details>
</MathBox>
...
A series $\sum_{k=1}^\infty a_k$ is convergent if and only if for all $\varepsilon > 0$ there is $N \in \mathbb{N}$ such that for all $n \geq m \geq N$
$$
...
Consider the sequence of partial sums $S_n := \sum_{k=1}^n a_k$. We know that $\left( S_n \right)_{n \in \mathbb{N}}$ is convergent if and only if it is a Cauchy sequence, from which it follows that $\forall \varepsilon > 0 \; \exists N \in \mathbb{N} \; \forall \tilde{n} \geq \tilde{m} \geq N$ we have $\left| s_{\tilde{n}} - s_{\tilde{m}} \right| < \varepsilon$. This is equivalent to
$$
...
A series $\sum_{k=1}^\infty a_k$ is called absolutely convergent if $\sum_{k=1}^\infty \left| a_k \right|$ is convergent. Applying the triangle inequality and the Cauchy criterion it can be shown that absolute convergence implies ordinary convergence. Let $\sum_{k=1}^\infty \left| a_k \right|$ be convergent, i.e. $\sum_{k=1}^\infty a_k$ is absolutely convergent, then $\forall \varepsilon > 0 \; \exists N \in \mathbb{N} \; \forall n \geq m \geq N$ we get
$$
...
Applying the Cauchy criterion we have $\forall \varepsilon > 0 \; \exists N_1 \in \mathbb{N} \; \forall n \geq m \geq N_1$ that
$$
...
In summary we have $\forall \varepsilon' = 2\varepsilon > 0 \; \exists N \in \mathbb{N} \; \forall n \geq N$ that
$$
...
$$
  \forall \tilde{x} \in I \; \forall \varepsilon > 0 \; \exists N \in \mathbb{N} \; \forall n \geq N : \left| f_n \left(\tilde{x} \right) - f\left(\tilde{x}\right) \right| < \varepsilon
$$
...
$$
  \forall \varepsilon > 0 \; \exists N \in \mathbb{N} \; \forall n \geq N \; \forall \tilde{x} \in I : \left| f_n \left(\tilde{x} \right) - f\left(\tilde{x}\right) \right| < \varepsilon
$$
...
Let $\varepsilon > 0$ and $x_0 \in I$, and set $\varepsilon' := \frac{\varepsilon}{3}$. The uniform convergence of $\left(f_n \right)_{n\in\mathbb{N}}$ gives that
$$
  \forall \varepsilon' > 0 \; \exists N \in\mathbb{N} \; \forall n \geq N \; \forall \tilde{x} \in I : \left| f_n \left( \tilde{x} \right) - f \left( \tilde{x} \right) \right| < \varepsilon'
$$
...
Let $f: (a, b] \to \mathbb{R}$ be a function with the property that $f|_{[a+\varepsilon, b]} \in \mathcal{R}([a, b])$ for all $\varepsilon > 0$. The integral $\int_a^b f(x) \mathrm{d}x$ converges if the following limit exists
$$
...
Let $f:\mathbb{R}\to\mathbb{R}$ be a continous function. The $n$-th repeated integral of $f$ with basepoint $a\in\mathbb{R}$, i.e.

$$
  I_a^n f(x) := \int_a^x \int_a^{\x_{n-1}}\cdots\int_a^{x_1} f(x_0)\,mathrm{d}x_0\dots\mathrm{d}x_{x-1},\quad n\in\mathbb{N}
$$
...
Suppose that the formula is true for $n\in\mathbb{N}$. Note that by the Leibniz integral rule
