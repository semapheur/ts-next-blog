...
Let $\left( a_n \right)_{n\in\N}$ be a sequence of real numbers. An element $a \in \R \cup \{ -\infty, \infty \} $ is called
...
For the second absolute term we note for some $n \geq N \in \N$ that $\left\{ t(i) \right\}_{i=1}^n \supset \left\{j\right\}_{j=1}^{N_1 - 1}$. Thus, the series $\sum_{k=1}^{N_1 - 1} a_k$ vanishes altogether leaving
$$
...
A real function is a map $f: I \subseteq \R \to \R$. The function is bounded if its range $f[I] = \mathrm{Ran}(f) = \left\{ f(x) | x \in I \right\}$ is bounded in $\R$. Equivalently, this holds if the supremum of the functions absolute value is finite
$$
...
Let $f: I \to \R$ and consider a fixed point $x_0 \in I$. If there is $c \in \R$ and all sequences $\left( x_n \right)_{n\in\N} \subset I \backslash \left\{ x_0 \right\}$ with $\lim_{n\to\infty} x_n = x_0$, then we have $\left( f\left(x_n\right) \right)_{n\in\N}$ is also convergent with $\lim_{n\to \infty} f\left( x_n\right) = c$. If this holds then we write
$$
\begin{align*}
  \lim_{x \to x_0} f(x) &= c & \\
...
or if $x_0$ is isolated in $I$, i.e. there is no sequence $\left(x_n \right)_{n \in \N} \subseteq I \backslash \left\{ x_0 \right\}$ with $\lim_{n\to\infty} x_n = x_0$. If $f$ is continuous at all $x_0 \in I$, then $f$ is called a continuous function (on $I$).
</MathBox>
...
Choose a sequence $\left(x_n \right)_{n\in\N} \subseteq J\backslash \left\{ x_0 \right\}$ with limit $x_0$, then
$$
...
A function $f: \R \to \R$ can be linearly approximated by a secant $s: \R \to \R$ of the form
$$
...
A function $f: I \subseteq \R \mapsto \R$ is differentiable at $x_0 \in I$ if there is an affine linear function $\Delta_{f, x_0}: I \mapsto \R$ with
$$
...
A function $f: I \subseteq \R \mapsto \R$ is called differentiable (on $I$) if $f$ is differentiable at all $x_0\in I$. In this case, $f': I \to \R$ defined by $x \mapsto f'(x)$ is called the derivative of $f$.
</MathBox>

<MathBox title='Proposition' boxType='proposition'>
If $f: I \subseteq \R\to\R$ is differentiable at $x_0$ then $f$ is also continuous at $x_0$
</MathBox>
...
$$
  C^n (I) := \left\{ f: I \to \R | f\textrm{ n-times continuously differentiable} \right\}
$$
...
Choose a sequence $\left( y_n \right)_{n\in\N} \subseteq J \backslash \{ y_0 \}$ with $f(x_n) = y_n$ and $\lim_{n\to\infty} y_n = y_0$, then
$$
\begin{align*}
...
- $f$ has a local maximum at $x_0$ if there is a neighbourhood $U \subseteq \R$ of $x_0$ with $f(x_0) = \max\left\{ f(x) | x \in U \cap I \right\}$
- $f$ has a local minimum at $x_0$ if there is a neighbourhood $U \subseteq \R$ of $x_0$ with $f(x_0) = \min\left\{ f(x) | x \in U \cap I \right\}$
</MathBox>
...
Consider the case where $f$ has a local maximum at $x_0$. Then there is a neighbourhood $U \subseteq \R$ of $x_0$ with $f(x_0) = \max\left\{ f(x) | x \in U \cap I \right\}$. Because $f$ is differentiable at $x_0$, we have $f(x) = f(x_0) + (x - x_0)\Delta_{f, x_0}(x)$. 
...
$$
  \mathcal{R}([a, b]) := \left\{ f: [a, b] \to \R \textrm{ bounded } \middle| f \textrm{ Riemann-integrable } \right\}
$$
</MathBox>
...
A map $\phi: [a, b] \to \R$ is called a step function if it is piecewisely constant. This means that there is a partition of $[a, b]$, and there are numbers $c1, \dots, c_n \in \R$ such that $\phi|_{(x_{j-1}, x_j)} = c_j$ for all $j \in \{1, \dots, n\}$. The set of all step functions on an interval $[a, b]$ is commonly denoted $S([a, b])$.
</MathBox>
...
For a function $f:[a, b]\backslash \{p\} \to \R$ with a pole at $p$, the improper Riemann integral is defined as
$$
