...
Let $X$ and $Y$ be normed spaces. A function $f:X\to Y$ is *Fréchet differentiable* at $x\in X$ if there exists a bounded linear operator $T\in\mathcal{B}(X, Y)$ such that

$$
  \lim_{\lVert h \rVert_X \to 0} \frac{\lVert f(x + h) - f(x) - Ah \rVert_Y}{\lVert h \rVert_X} = 0
$$

or equivalently, such that the first-order expansion holds

$$
  f(x + h) = f(x) + Ah + \mathcal{O}(h)
$$

The operator $A$ is called the *Fréchet derivative/differential* of $F$, denoted $T = \D f(x)$. A functional $f$ is *Fréchet differentiable* for any point of $X$ if the function $\D f : X\to \mathcal{B}(X, Y)$ is continuous. In this case, we write $f \in C^1 (X, Y)$,
</MathBox>

<MathBox title='Fréchet chain rule' boxType='proposition'>
Let $U, V, W$ be normed spaces. If $f:U\to V$ is Fréchet differentiable at $u\in U$ and $g:V\to W$ is Fréchet differentiable at $v\in V$, the composition $g\circ f$ is Fréchet differentiable in $u$ with differential
...
<MathBox title='Functional derivative' boxType='definition'>

</MathBox>


<MathBox title='Gateaux derivative' boxType='definition'>
Let $X$ and $Y$ are locally convex topological vector spaces. The Gateaux derivative of a function $f: X \to Y$ at $x\in X$ in the direction $h\in H$ is 
</MathBox>


<MathBox title='Functional ' boxType='definition'>
Let $F$ be the space of $n$-dimensional smooth curves, i.e.

$$
  \mathcal{F} = \Set{\mathbf{r}: I\subset\R\to\R^n | \mathbf{r}\in C^\infty (I,\R^n)} 
$$

A *functional* is a map $S[]:\mathcal{F}\to\R$. In this context, a smooth curve $\mathbf{r}(t)$ represents the *trajectory* of a system of $n$ degrees of freedom. 
</MathBox>
