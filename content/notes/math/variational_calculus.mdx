...
$$
  \lim_{y\to y_0} \norm{ z - x_0 } = \norm{ y - y_0 } = 0 \implies z \xrightarrow{y\to y_0} x_0 
$$
...
<MathBox title='Boundedness of continuous quadratic functionals' boxType="proposition" tag="11">
...
Since $f$ is continuous, the pre-image $f^{-1}[B(0,1)]$ is open. Thus, there is $\epsilon > 0$ such that $|f(x)| < 1$ for every $x \in B(0,\epsilon)\subset X$. Set $M = \epsilon^{-2}$, then for $x = 0$ we have $|f(x)| = 0 < M$. For $x\in B(0,1) \setminus\Set{0}$, let $y = \frac{\epsilon}{\norm{ x }}x$. Then $\norm{ y } = \epsilon$, giving

$$
  |f(x)| = \frac{\norm{ x }^2}{\epsilon^2} |f(y)| < \frac{\norm{ x }^2}{\epsilon^2} = M
$$
</details>
</MathBox>
...
$$
\begin{align*}
  & \frac{f(x + h) - f(x) - \delta f(x)h}{\norm{ h }} \\
  &= \frac{K(x + h, x + h) - K(x,x) - (K(x, h) + K(h, x))}{\norm{ h }} \\
  &= \frac{K(h, h)}{\norm{ h }} \\
  &= \norm{ h } K\left(\frac{h}{\norm{ h }}, \frac{h}{\norm{ h }} \right) \\
  &= \norm{ h } f\left(\frac{h}{\norm{ h }}\right) \\
  &< \norm{ h } M \xrightarrow{h\to 0} 0
\end{align*}
$$
...
$$
  \lim_{h\to 0} \frac{J(x + h) - J(x) - J'(x)h}{\norm{ h }_X}
$$
...
$$
  \frac{f(x + h) - f(x) - \delta f(x)h - \frac{1}{2}\delta^2 f(x)h}{\norm{ h }^2} = \frac{K(h,h) - f(h)}{\norm{ h }^2} = 0
$$
...
For convenience, choose the Euclidean norm $\norm{\mathbf{x}} = \sqrt{\mathbf{x}^\top \mathbf{x}}$. Setting $\mathbf{Q} = \frac{1}{2}(\mathbf{P} + \mathbf{P}^\top)$, we can write $J(\mathbf{x}) = \mathbf{x}^\top \mathbf{Qx}$. Note that $J(\mathbf{x}) > \varepsilon\mathbf{x}^\top\mathbf{x}$ if and only if 
...
<MathBox title='Local extrema are critical points' boxType="proposition" tag="5">
...
<MathBox title='Gâteaux conditions for local minima' boxType="proposition" tag="8">
...
$$
  J(x) \leq J(y + \tau h),\; \tau \in\left(-\frac{\epsilon}{\norm{ h }}, \frac{\epsilon}{\norm{ h }} \right)
$$
...
$$
  \lim_{h\to 0} \frac{J(x + h) - J(x) - \frac{1}{2}J''(x)h}{\norm{ h }^2} = 0 
$$

so there is $\delta > 0$

$$
  \frac{J(x + h) - J(x) - \frac{1}{2}J''(x)h}{\norm{ h }^2} > -\frac{\varepsilon}{2}
$$

for $h\in B(0,\delta)$. Hence

$$
  J(x + h) - J(x) > \frac{1}{2}(J''(x) h - \varepsilon\norm{ h }^2) > 0
$$
...
$$
  |\delta^2 J(x; h) - \delta^2 J(y; h)| \leq\norm{ \d^2 J(x) - \d^2 J(y) }\cdot|h|^2
$$
...
Suppose $C \subseteq X$ is a convex subspace and $J:C\to\mathbb{F}$ is a convex functional. If the Gâteaux derivative exists at an interior point $x\in C$ and satisfies $\delta J(x;h) = 0$ for all $h\in X$, then $x$ is a global minimum.
...
Suppose $C \subseteq X$ is a convex subspace and $J:C\to\mathbb{F}$ is twice Gâteaux differentible on $C$. Then $J$ is convex if and only if $\delta^2 J(x; h)\geq 0$ for all $x\in C$ and $h\in X$. Moreover, $J$ is strictly convex if and only if $\delta^2 F(x; h) > 0$ for all $x\in C$ and $h \in X\setminus\Set{0}$.
...
<MathBox title='Euler-Lagrange equations' boxType="theorem" tag="1">
...
<MathBox title='' boxType="proposition" tag="15">
...
<MathBox title='Beltrami identity' boxType="corollary" tag="1">
