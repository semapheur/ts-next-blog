...
Let $\mathcal{F}$ be the space of $n$-dimensional smooth curves, i.e.

$$
  \mathcal{F} = \Set{\mathbf{r}: I\subset\R\to\R^n} 
$$
...
A functional $S[]:\mathcal{F}\to\R$ is *differentiable*, if for any $\mathcal{F}\ni\mathbf{r}:I\to\R^n$ and $\delta\mathbf{r}(t) \in\mathcal{F}_\varepsilon$, where
...
The map $\delta S[\cdot,\cdot]$ is called the *differential* of $S[]$.
</MathBox>
