...
$$
  J(\mathbf{h}(t)) = \int_{t_0}^{t_1} (R\dot{\mathbf{h}}(t)^2 + S\mathbf{h}(t)^2)\;\d t
$$
...
$$
  J(\mathbf{h}) = \int_{t_0}^{t_1} (R\dot{\mathbf{h}}^2 + S\mathbf{v}^2)\;\d t
$$
...
$$
\begin{align*}
  J(\mathbf{h}) =& \int_{t_0}^{t_1} \left(\sum_{i=1}^n R_i \dot{h}_i^2 + T_i v_i^2 \right)\;\d t \\
