...
where
- $b_X:\mathcal{R}(\Omega)\to[0,1]$ is a belief mass distribution over the possible state values of $X$, defined on the reduced power set $\mathcal{R}(\Omega) := \mathcal{P}(\Omega)\setminus\{\Omega, \emptyset\}$
