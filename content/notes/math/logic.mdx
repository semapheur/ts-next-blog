...
| | **Binomial**\$\lvert\Omega\rvert = 2$\$\Omega = \mathcal{R}(\Omega)$ | **Multinomial**\$\lvert\Omega\rvert > 2$\Focal elements $x = \Omega$ | **Hyper**\$\lvert\Omega\rvert = 2$\Focal elements $x\in\mathcal{R}(\Omega)$ |
| --- | :-: | :-: | :-: |
| **Uncertain**\$u_X > 0$ | **Uncertain binomial opinion**\Beta PDF | **Uncertain multinomial opinion**\Dirichlet PDF over $\Omega$ | **Uncertain hyper opinion**\Dirichlet PDF over $\mathcal{R}(\Omega)$ |
| **Dogmatic**\$u_X = 0$ | **Dogmatic binomial opinion**\Scalar probability | **Dogmatic multinomial opinion**\Probabilities on $\Omega$ | **Dogmatic hyper opinion**\ on $\mathcal{R}(\Omega)$ | 
