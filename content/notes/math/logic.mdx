...
| $A\to B, A \vdash B$ (modus ponens) | $P(A\Vert B) = P(A)P(B|A) + P\left(\bar{A}\right)P\left(B|\bar{A}\right)$ (deduction) |
| $A\to B\therefore \neg B\to\neg A$ (modus tollens) | $P\left(A\bar{\Vert} B \right) = P(A)P(A|B) + P\left(\bar{A}\right) P\left( A|\bar{B} \right)$ (abduction) |
...
- $p_X(\Omega) = 1$
- $p_X left(\bigcup_{i\in\mathbb{N}} E_i \right) = \sum_{i\in\mathbb{N}} p_X(E_i)$ for any countable collection of disjoint sets $\{A_i\}_{i\in\mathbb{N}} \subseteq \mathcal{F}$
...
| | **Binomial**</br>$|\Omega| = 2$</br>$\Omega = \mathcal{R}(\Omega)$ | **Multinomial**</br>$|\Omega| > 2$</br>Focal elements $x = \Omega$ | **Hyper**</br>$|\Omega| = 2$</br>Focal elements $x\in\mathcal{R}(\Omega)$ |
| --- | :-: | :-: | :-: |
...
| **Dogmatic**</br>$u_X = 0$ | **Dogmatic binomial opinion**</br>Scalar probability | **Dogmatic multinomial opinion**</br>Probabilities on $\Omega$ | **Dogmatic hyper opinion**</br>Probabilitiese on $\mathcal{R}(\Omega)$ | 
...
$$
  E[p_X] = \frac{\alpha}{\alpha + \beta} = \frac{r_X + a_X W}{r_X + s_X + W}
$$
...
| Deduction | $\omega_{X\Vert Y}^A = \omega_{X|Y}^A \circledcirc \omega_Y^A$ | Modus ponens |
| Abduction | $\omega_{X\bar{\Vert} Y}^A = \omega_{X|Y}^A \tilde{\circledcirc} \left(a_X, \omega_Y^A \right$ | Modus tollens |
| Subjective Bayes' theorem | $\omega_{X\tilde{|} Y}^A = \omega_{X|Y}^A \tilde{\phi} a_X$ | Contraposition |
