...
For the variance estimators, we assume additionally that the fourth central moment of $X$ is finite, i.e. $\mu_4 = \mathbb{E}[(X - \mu)^4] < \infty$.
...
Since $\mathbb{E}[(X - \mu)^2] = \sigma^2$, the expectation of $W_n^2$ is

$$
  \mathbb{E}(W_n^2) = \frac{1}{n} \sum_{i=1}^n \mathbb{E}[(X_i - \mu)^2] = \sigma^2
$$

showing that $W_n^2$ is an unbiased estimator of $\sigma^2$ when $\mu$ is known. The variance of $W_n^2$ is
...
Write $X_i - M_n = (X_i - \mu) - (M_n - \mu)$ and expand
...
showing that $S_n^2$ is an unbiased estimator of $\sigma^2$. The variance of $S_n^2$ is
...
Comparing $\mathbf{W}^2$ and $\mathbf{S}^2$ we have
- $\operatorname{var}(W_n^2) < \operatorname{var}(S_n^2)$
- The assymptotic relative efficiency of $\mathbf{W}^2$ to $\mathbf{S}^2$ is $1$
