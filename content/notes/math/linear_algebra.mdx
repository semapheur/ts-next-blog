...
<MathBox title='' boxType='proposition'>
Let $\mathrm{T}:V\to W$ be a linear transformation, where $\dim(V) = n$ and $\dim(W) = m$, and let $B = \{\mathbf{b}_i\}_{i=1}^n$ be an ordered basis for $V$ and $C$ and ordered basis for $W$. Then $\mathrm{T}$ can be represented with respect to $B$ and $C$ as the matrix product $[\mathrm{T}(\mathbf{v})]_C = [\mathrm{T}]_{B,C}[\mathbf{v}]_B$ where
...
The set of all bilinear functions from $U\times V$ to $W$ is denoted $\mathrm{hom}_{\mathbb{F}}(U\times V, W)$. A bilinear function $f:U\times V\to\mathbb{F}$ taking values in the base field $\mathbb{F}$ is called a *bilinear form* on $U\times V$.
</MathBox>

Note the difference between bilinear functions $\mathrm{hom}_{\mathbb{F}}(V\times V;W)$ and linear functions $\mathcal{L}(V\times V,W)$. For the latter, $V\times V$ is the direct product of vector spaces. For the cartesion product of sets $V\times V$ we dot not consider any algebraic structure.
...
$$
  ...
  W \mapsto \mathrm{Hom}_{\mathbb{F}}(U\times V, W)
$$
...
In other words, there is a universal pair $(T, b)$ of a vector space $T$ and a bilinear map $b: U\times V\to T$ such that for any pair $(W, f)$ of a vector space $W$ and a bilinear map $g:U\times W$ there is a unique linear map $\mathrm{L}:T\to W$ with $f = L \circ b$ according to the diagram below.
...
The tensor product of two vector spaces $U$ and $V$ is a vector space denoted $U\otimes V$ together with a bilinear map $\otimes:U\times V\to U\otimes V$ given by $(u,v)\mapsto u\otimes v$ satisfying the universal property. That is for every for every bilinear map $f:U\times V\to W$ there is a unique linear map $\mathcal{L}:U\otimes V\to W$ such that $f = \mathcal{L} \circ\otimes$, i.e. $f(u,v) = \mathcal{L}(u\otimes v)$ for every $u\in U$ and $v\in V$.
...
Let $U$ and $V$ be vector spaces with respective bases $B = \{\mathbf{b}_i\}_{i\in I}$ and $C = \{\mathbf{c}_j \}_{j\in J}$. The tensor product from $U\otimes V$ can be constructed as vector space with basis $D = \{ \mathbf{b}_i \otimes \mathbf{c}_j \mid \mathbf{b}_i \in B, \mathbf{c}_j \in C \}$ where $\otimes:U\times V \to U\otimes V$ maps $(\mathbf{b}_i, \mathbf{c}_j)\mapsto \mathbf{b}_i \otimes\mathbf{c}_i$.

For any bilinear function $f:U\times V \to W$, then universal condition $f = \otimes \circ \mathrm{L}$ is equivalent to

$$
  L(\mathbf{b}_i, \mathbf{c}_j) = g(\mathbf{b}_i, \mathbf{c}_i)
$$
...
Since $\otimes(\mathbf{u},\mathbf{v}) = \mathbf{u}\times\mathbf{v} = \mathrm{R}\circ h(\mathbf{u}, \mathbf{v})$ it follows that $\otimes = \mathrm{R}\circ h$. The universal property of vector spaces implies that there is a unique linear transformation $\mathrm{S}: L_{U\times V}\to W$ for which $\mathrm{S}\circ j = f$.
...
\begin{align*}
  ...
  =& \mathrm{S}[\alpha h(\mathbf{u}, \mathbf{v}) + \beta h(\mathbf{u}', \mathbf{v}) - h(\alpha\mathbf{u} + \beta\mathbf{u}',\mathbf{v})] \\
  =& \alpha\mathrm{S}h(\mathbf{u}, \mathrm{v}) + \beta\mathrm{S}h(\mathbf{u}',\mathbf{v}) - \mathrm{S}h(\alpha\mathbf{u} + \beta\mathbf{u}',\mathbf{v}) \\
...
$$
  \mathrm{S}' \circ h = \mathrm{T}' \circ \mathrm{R} = \mathrm{T}' \circ \otimes = f
$$
...
Let $U\otimes V$ be the tensor product of $n$-dimensional vector spaces $U$ and $V$. If $\{\mathbf{u}_i\}_{i=1}^n$ are are linearly independent vectors in $U$ and $\{\mathbf{v}_i\}_{i=1}^n$ arbitrary vectors in $V$, then
...
Let $B = \{\mathbf{b}_i\}_{i\in I}$ be a basis for a vector space $U$ and $C = \{\mathbf{c}_i\}_{j\in J}$ be a basis for a vector space $V$. Then the set

$$
  D = \{\mathbf{b}_i \otimes \mathbf{v}_j \mid i\in I, j\in J\}
$$
...
If $U$ and $V$ are vector spaces over $\mathbb{F}$ with respective bases $B = \{\mathbf{b}_i\}_{i\in I}$ and $C = \{\mathbf{c}_j\}_{j\in J}$, then any vector $\mathbf{w}\in U\otimes V$ can be expressed as a summand
...
Suppose that $X = \{\mathbf{x}_i\}_{i\in I}$ and $Y = \{\mathbf{y}_j\}{j\in J}$ also are bases for $U$ and $V$, respectively and that
...
Each $\mathbf{x}_1,\dots,\mathbf{x}_c$ is a finite linear combination of basis vectors in $B$. We can reindex $B$ so that each $\mathbf{w}_k$ is a linear combination of the vectors $\{\mathbf{b}_i\}_{i=1}^n$ where $a\leq n$ generating the subspace

$$
  U_n = \mathrm{span}\{\mathbf{b}_i\}_{i=1}^n
$$

Next, extend $\{\mathbf{x}_i\}_{i=1}^c$ to a basis $X' = \{\mathbf{x}_1,\dots,\mathbf{x}_c,\mathbf{x}_{c+1},\dots,\mathbf{x}_n\}$ for $U_n$. Thus
...
Repeating the same process for $V$, where we reindex $C$ so that the subspace $V_m = \mathrm{span}\{\mathbf{v}_j\}_{j=1}^m$ contains $\mathbf{y}_1,\dots,\mathbf{y}_d$ and extend to a basis $Y' = \{\mathbf{y}_1,\dots,\mathbf{y}_d,\mathbf{y}_{d+1},\dots,\mathbf{y}_m\}$ for $V_m$. Then
...
A tensor of the form $\mathbf{u}\otimes\mathbf{v}\in U\otimes V$ for called decomposable. If $B = \{\mathbf{b}_i\}_{i\in I}$ is a basis for $U$ and $C = \{\mathbf{c}_j\}_{j\in J}$ is a basis for $V$, then any decomposable tensor has the form
...
Let $U$ and $V$ be vector spaces over $\mathbb{F}$ with respective bases $B = \{\mathbf{b}_i\}_{i\in I}$ and $C = \{\mathbf{c}_j\}_{j\in J}$. Then every $\mathbf{w} \in U\otimes V$ has a unique expression as a finite sum (up to order of zero terms) of the forms
...
where $\{\mathbf{u}_i\}_{i\in I}\subseteq U$ and $\{\mathbf{v}_j\}_{j\in J}\subseteq V$ are linearly independent sets. As to uniqueness, $n$ is the rank of $\mathbf{w}$ and so it is unique. Also, we have

$$
  \sum_{i=1}^r \mathbf{u}_i \otimes\mathbf{v}_i = \sum_{i=1}^r \mathbf{x}_i \otimes\mathbf{y}_i
$$

where $\{\mathbf{x}_i\}_{i\in I}\subseteq U$ and $\{\mathbf{y}_j\}_{j\in J}\subseteq V$, if and only if there exists $r \times r$ matrices $\mathbf{A} = (\alpha_{i,j})$ and $\mathbf{B} = (\beta_{i,j})$ for which $\mathbf{A}^T \mathbf{B} = \mathbf{I}_r$ such that for $j=1,\dots,r$
...
**(1):** The first sum merely expresses the fact that $D = \{ \mathbf{b}_i \otimes\mathbf{c}_i \mid i\in I, j\in J \}$ is a basis for $U\otimes V$
...
**(4)**: Assume from the second sum that none of $\mathbf{v}_i$ are zero. If the set $\{\mathbf{v}_i\}$ is linearly independent, we are done. If not, then we may suppose (after reindexing if necessary) that
...
The vectors $\{\mathbf{b}_i + \rho_i \mathbf{b}_n \mid 1\leq i \leq n-1\}$ are linearly independent. This reduction can be repeated until the second coordinates are linearly independent. Also, the identity matrix $\mathbf{I}_n$ is a coordinate matrix for $\mathbf{w}$ giving $n = \mathrm{rank}(\mathbf{I}_n) = \mathrm{rank}(\mathbf{w})$.
</details>
</MathBox>
