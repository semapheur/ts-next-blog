...
The determinant is a function $\det:\mathcal{M}_{n}(\mathbb{F}) \to\mathbb{F}$ defined by

$$
  \det(\mathbf{A}) = \sum_{\sigma\in S_n} \left( \mathrm{sgn}(\sigma) \prod_{i=1}^n \mathbf{A}_{i,\sigma(i)} \right)
$$
...
$$
\begin{align*}
  \operatorname{vol}_n ( \mathbf{u}_1,\dots,\mathbf{u}_n ) =& \operatorname{vol}_n \left( \begin{bmatrix} u_{11} \\ \vdots \\ u_{n1} \end{bmatrix}, \dots,\begin{bmatrix} u_{1n} \\ \vdots \\ u_{nn} \end{bmatrix} \right) \\
  =& \operatorname{vol}_n \left( \sum_{i_1 = 1}^n u_{i_1, 1} \mathbf{e}_{i_1},\dots, \sum_{i_n = 1}^n u_{i_n, n} \mathbf{e}_{i_n} \right) \\
  =& \sum_{i_1 = 1}^n \cdots \sum_{i_n = 1}^n u_{i_1, 1}\cdots u_{i_n, n} \operatorname{vol}_n (\mathbf{e}_{i_1},\dots,\mathbf{e}_{i_n}) \\
  =& \sum_{(i_i,\dots,i_n)\in S_n} u_{i_1, 1}\cdots u_{i_n, n} \operatorname{vol}_n (\mathbf{e}_{i_1},\dots,\mathbf{e}_{i_n})
\end{align*}
$$

where $S_n$ is the set of all permutations of $\Set{1,\dots,n}$. Due to the antisymmetry and normalization properties of the volume measure we  
...
This is precisely the definition of the signature of permutations $\operatorname{sgn}: S_n \to \Set{-1,1}$
</details>
</MathBox>
...
The determinant of a $2\times 2$-matrix $\mathbf{A} = \begin{smallmatrix} a_{11} & a_{12} \\ a_{21} & a_{22} \end{smallmatrix}$ is $\det(\mathbf{A}) = a_{11}a_{22} - a_{21}a_{12}$.
