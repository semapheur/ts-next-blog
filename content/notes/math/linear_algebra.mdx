...
Accordingly we can define the *coordinate map* $\phi_B: V\to\mathbb{F}^n$ by

$$
  \phi_B (v) = [v]_B := \begin{bmatrix} \lambda_i \\ \vdots \\ \lambda_n \end{bmatrix}
$$

where the column matrix $[v]_B$ is known as the *coordinate matrix* of $v$ with respect to the ordered basis $B$. The coordinate is an isomorphism (bijection) with inverse $\phi_B^{-1}:\mathbb{F}^n \to V$ defined by
...
For any ordered basis $B$ of an $\mathbb{F}$-vector space $V$ with $\dim(V) = n$, the coordinate map $\phi_B: V\to\mathbb{F}^n$ is an isomorphism. Hence, any $n$-dimensional vector space over $\mathbb{F}$ is isomorphic to $\mathbb{F}^n$.
...
$$
  v = \sum_{i=1}^n v^i e_i = \begin{bmatrix} e_1 & \cdots & e_n \end{bmatrix} \begin{bmatrix} v^1 \\ \vdots \\ v^n \end{bmatrix} = E[v]_E 
$$
...
$$
\begin{align*}
  f(v) =& f\left(\sum_{i=1}^n v^i e_i \right) = \sum_{i=1}^n f(e_i)v^i \\
...
Let $B $ and $C$ be ordered bases for an $n$-dimensional vector space $V$. For any $v\in V$, the map $\phi_{B,C} = \phi_C \phi_B^{-1}$ given by $[v]_B \mapsto [v]_C$ is called the change of basis operator. 
</MathBox>

<MathBox title='' boxType='proposition'>
Let $B = \{b_i\}_{i=1}^n$ and $C$ be ordered bases for an $n$-dimensional vector space $V$. The change of basis operator $\phi_{B,C} = \phi_C \phi_B^{-1}$ from $B$ to $C$ is an automorphism of $\mathbb{F}^n$ whose standard matrix is
...
Since $\phi_{B,C}$ is an operator on $\mathbb{F}^n$ it has the form $f_M$ where $M\in\mathcal{M}_n$

$$
\begin{align*}
  M =& \begin{bmatrix} \phi_{B,C}(e_1) & \cdots & \phi_{B,C}(e_n) \end{bmatrix} \\
  =& \begin{bmatrix} \phi_C \phi_B^{-1}([b_1]_B) & \cdots & \phi_C \phi_B^{-1}([b_n]_B) \end{bmatrix} \\
...
$$
  A = A[v]_B = \begin{bmatrix} [f(b_i)]_C & \cdots & [f(b_n)]_C  \end{bmatrix}
$$
</details>
</MathBox>
