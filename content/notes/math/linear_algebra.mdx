...
$$
  ...
  =& \Set{ f\in W^* | f(\mathrm{T}(V)) = \{0} \} \\
  =& \Set{ f\in W^* | f(\mathrm{ran}(\mathrm{T})) = \{0} \} \\
...
For a vector space $V$, the outer product of its basis $B = \Set{ e_i }_{i \in I \subset \N}$ and its dual basis $B^* = \left\{ e^i }_{i \in I \subset \N}$ spans all endomorphic linear transforms. Every linear transform $L : V \to V$ can be writen as a linear combination of vector-covector pairs 
...
Given a basis $B = \Set{ e_i }_{i \in I \subseteq \N}$, a new basis $\tilde{B} = \left\{ \tilde{e}_i }_{i \in I \subseteq \N}$ can be formed by transforming the old basis vectors, and vice versa
...
Given a basis $B = \Set{ e_i }_{i \in I \subset \N}$ for a finite-dimensional vector space $V$, a basis $B^* = \left\{ e^i }_{i \in I \subset \N}$, called the dual basis, can be formed through the bi-orthogonality property
