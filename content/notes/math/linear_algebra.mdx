...
Note the difference between bilinear functions $\mathrm{Hom}_{\mathbb{F}}(V\times V;W)$, mapping from cartesian products of vector spaces, and linear functions $\mathcal{L}(V\times V,W)$, mapping from direct products of vector spaces. Unlike direct products, cartesion products of sets are not associated with any algebraic structure.
...
For any bilinear function $F:U\times V \to W$, then the universal condition $F = \otimes \circ \mathrm{L}$ is equivalent to

$$
  \mathrm{L}(\mathbf{b}_i \otimes \mathbf{c}_j) = G(\mathbf{b}_i, \mathbf{c}_i)
$$
...
Since $(U \times V)^* \cong \mathrm{Hom}(U\times V; \mathbb{F})$ it follows that

$$
  U^* \otimes V^* \cong (U\otimes V)^* \cong \mathrm{Hom}(U\times V, \mathrm{F})
$$
...
It remains to show that $\mathrm{T}$ is bijective. Let $\{\mathbf{b}_i\}_{i\in I}$ be a basis for $U$, with dual basis $\{b_i\}_{i\in I}$ and let $\{\mathbf{c}_j\}_{j\in J\}$ be a basis for $V$ with dual basis $\{ c_j\}_{j\in J}$. Then
...
showing that $\{\mathrm{T}(b_i \otimes c_i) \}\subseteq (U\otimes V)^*$ is the dual basis to basis $\{\mathbf{b}_u \otimes \mathbf{c}_v\}$ for $U\otimes V$. Thus, $\mathrm{T}$ takes the basis $\{ b_i \otimes c_j \}$ for $U^* \otimes V^*$ to the basis $\{\mathrm{T}(b_i \otimes c_j\}$ and is therefore bijective.
</details>
</MathBox>
...
As to the dimension of $W_{\mathbb{K}}$ it is not hard to so that if $\{\mathbf{b}_i\}_{i\in I}$ is a basis for $V_{\mathbb{F}}$ then $\{\mathbf{1}\otimes\mathbf{b}_i\}_{i\in I}$ is a basis for $W_{\mathbb{K}}$, hence
...
\begin{CD}
  ...
  \mathbb{K}\otimes V_{\mathbb{K}} @>>\bar{\mathrm{K}}> \mathbb{K}\otimes W_{\mathbb{K}}
\end{CD}
$$
