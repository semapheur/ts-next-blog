...
$$
  \det(A) = \sum_{\pi\in S_n} \left( \mathrm{sgn}(\sigma) \prod_{i=1}^n A_{i,\sigma(i)} \right)
$$
...
2. $A$ is invertible (nonsingular) if and only if $\det(A) \neq 0$
3. if $A$ is an upper/lower triangular matrix, then $\det(A) = \prod_{i=1}^n A_{i,j}$
