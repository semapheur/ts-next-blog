...
\begin{tikzcd}[row sep=large, column sep=large, every label/.append style = {font = \scriptsize}]
  V \arrow[d, "\pi_S" swap] \arrow[r, "T"] & W \\
...
If $S$ is a subspace of $V$, then $\dim(V/S)$ is called the *codimension* of $S$ in $V$ and denoted by $\codim_V(S)$.
</MathBox>
...
$$
  \dim(V) = \dim(S) + \dim(V/S) = \codim_V(S)
$$

If $V$ is finite-dimensional then

$$
  \codim_V(S) = \dim(V) - \dim(S)
$$
</MathBox>
...
\begin{align*}
  ...
  =& \mathrm{T}(\mathrm{S}^\times(f)) = (\mathrm{T}^\times \mathrm{S}^times)(f)
\end{align*}
$$
