...
An important case occurs when $V_i = V$ for all $i\in I$. If $V^I$ denotes the set of all functions from $I$ to $V$ and $(V^K)_0$ denote the set of all functions in $I$ that have finite support then
...
<MathBox title='' boxType='proposition'>
Let $f:V\to W$ be a linear transformation, where $\dim(V) = n$ and $\dim(W) = m$, and let $B = \{b_i\}_{i=1}^n$ be an ordered basis for $V$ and $C$ and ordered basis for $W$. Then $f$ can be represented with respect to $B$ and $C$ as the matrix product $[f(v)]_C = [f]_{B,C}[v]_B$ where
...
$$
  \mathcal{M}^{-1}(O) = \{(\alpha, v)\in\R\times V \mid \alphaLet $V$ be any real vector space with $\dim(V) = n$ and fix and ordered basis $B = \{ \mathbf{v}_i \}_{i=1}^n$ for $V$. Consider the coordinate map
...
Next we show that if every linear functional on $V$ is continuous under a topology $\mathcal{T}$ on $V$ then the coordinate map $\varphi$ is continuous. For $v\in V$ let $[\mathbf{v}]_{B,i}$ denote the $i$th coordinate of $[v]_B$. The map $\mu:V \to\R$ defined by $\mu(\mathbf{v}) = [\mathbf{v}]_{B,i}$ is a linear functional and so is continuous by assumption. Thus, for any open interval $I_i \in\R$ the set

$$
  A_i = \{ \mathbf{v}\in V \mid [\mathbf{v}]_{B,i} \in I_i \}
$$

is open. If $I_i$ are open intervals in $\R$ then

$$
  \varphi^{-1}(\prod_{i=1}^n I_i) = \{ \mathbf{v}\in V \mid [\mathbf{v}]_B \in \prod_{i=1}^n I_i \} = \bigcap_{i=1}^n A_i
$$
...
Let $f$ be a linear functional. Since $\varphi$ is continuous if and only if $f\circ\varphi^{-1}$ is continuous, we can confine attention to $V=\R^n$. In this case, if $\{\mathbf{e}_i\}_{i=1}^n$ is the standard basis for $\R^n$ and $|f(\mathbf{e}_i)|\leq M$, then for any $x=(\lambda_i)\in\R^n$ we have
...
$$
  \mathbf{u} \sim \mathbf{v} \iff \mathbf{u} - \mathbf{v} \in S,\; \mathbf{u},\mathbf{v}\in V
$$

is an equivalence relation. When $u\sim v$, we say that $u$ and $v$ are *congruent modulo* $S$ written as

$$
  \mathfb{u}\sim \mathfb{v} \mathrm{mod} S
$$

The equivalence class

$$
  [\mathbf{v}]_\sim = \mathbf{v} + S = \{\mathbf{v}+\mathbf{s} \mid s\in S\}
$$

is called a *coset* of $S$ in *V* and $v$ is called a *coset representative* for $v + S$. The set of all cosets of $S$ in $V$ is denoted
...
The zero vector in $V/S$ is the coset $0 + S = S$

<details>
<summary>Details</summar>
...
For $\mathbf{u},\mathbf{v}\in V$ and $\alpha,\beta\in\mathbb{F}$ we have
$$
\begin{align*}
...
Let $S$ be a subspace of $V$ and let $T\in\mathcal{L}(V,W)$ satisfy $S\subseteq\mathrm{ker}(T)$. Then there is a unique linear transformation $T':V/S\to W$ with the property that 

$$
  T' \circ \pi_S = T
$$

and $\mathrm{ker}(T') = \mathrm{ker}(T)/S$ and $\mathrm{ran}(T') = \mathrm{ran}(T)$.

<details>
<summary>Details</summar>

There is no other choice but to define $T'$ by the condition $T' = \pi_S = T$, i.e.

$$
  T'(\mathbf{v} + S) = T(\mathbf{v})
$$

This function is well-defined if and only if

$$
  \mathbf{v} + S = \mathbf{u} + S \implies T'(\mathbf{v} + S) = T'(\mathbf{u} + S)
$$
...
$$
\begin{align*}
  \mathbf{v} + S = \mathbf{u} + S \implies& T(\mathbf{v}) = T(\mathbf{u}) \\
  \mathbf{v} - \mathbf{u} \in S \implies& T(\mathbf{v} - \mathbf{u}) = \mathbf{0} \\
  \mathbf{x}\in S \implies& T(\mathbf{x}) = \mathbf{0} \\
  S \subseteq& \mathrm{ker}(T)
\end{align*}
$$

Thus, $T':V/S\to W$ is well defined. Also

$$
\begin{align*}
  \mathrm{ran}(T') =& \{ T'(\mathbf{v} + S) \mid \mathbf{v}\in V \} \\
  =& \{ T(\mathbf{v}) \mid \mathbf{V} \} \\
  =& \mathrm{ran}(T)
\end{align*}
$$
...
$$
\begin{align*}
  \mathrm{ker}(T') =& \{\mathbf{v} + S \mid T'(\mathbf{v} + S) = \mathbf{0} \} \\
  =& \{\mathbf{v} + S \mid T(\mathbf{v}) = \mathbf{0} \} \\
  =& \{\mathbf{v} + S \mid \mathbf{v}\in\mathrm{ker}(T) \} 
  =& \mathrm{ran}(T)/S
\end{align*}
$$

The uniqueness of $T'$ is evident.
</details>
</MathBox>

<MathBox title='First isomorphism theorem' boxType='theorem'>
For $T\in\mathcal{L}(V,W)$, the linear transformation $T':V/\mathrm{ker}(T) \to W$ defined by

$$
  T'(\mathbf{v} + \mathrm{ker}(T)) = T(\mathbf{v})
$$

is injective and 

$$
  V/\mathrm{ker}(T) \simeq \mathrm{ran}(T)
$$
...
Let $S$ be a subspace of $V$ and let $T$ be a complement of $S$, i.e. $V = S\oplus T$. The linear operator $P_T:V\to V$ defined by

$$
  P(\mathbf{s} + \mathbf{t}) = \mathbf{t},\; \mathbf{s}\in S,\mathbf{t}\in T
$$
...
\begin{align*}
  \mathrm{ran}(P_T) =& T \\
  \mathrm{ker}(P_T) =& \{ \mathbf{s}+\mathbf{t}\in V \mid \mathbf{t} = \mathbf{0} \} = S
\end{align*}
$$
<MathBox/>
...
The function $T: (S+T) \to S/(S\cap T)$ defined by

$$
  T(\mathbf{s} + \mathbf{t}) = \mathbf{s} + (S \cap T)
$$

is a well-defined surjective linear transformation with $\mathrm{ker} = T$. Hence, by the first isomorphism theorem $(S + T)/T \simeq S/(S\cap T)$.
</details>
</MathBox>
...
The function $T: V/S \to V/S$ defined by

$$
  T(\mathbf{s} + S) = \mathbf{s} + T
$$

is a well-defined surjective linear transformation with $\mathrm{ker} = T/S$. Hence, by the first isomorphism theorem $T(\mathbf{s} + S) = \mathbf{s} + T$.
</details>
</MathBox>
...
The function $T: V\to V_1/S_1 \boxplus V_2/S_2$ defined by

$$
  T(\mathbf{v_1} + \mathbf{v_2}) \simeq (\mathbf{v_1} + S, \mathbf{v_2} + S_2)
$$

is a well-defined since $V = V_1 \oplus V_2$ is a direct sum. It is also a surjective linear transformation with $\mathrm{ker} = S_1 \oplus S_2$. Hence, by the first isomorphism theorem $T(\mathbf{v_1} + \mathbf{v_2}) \simeq (\mathbf{v_1} + S, \mathbf{v_2} + S_2)$.
</details>
</MathBox>
...
<MathBox title='Linear functional and dual space' boxType='theorem'>
Let $V$ be a vector space over $\mathbb{F}$. A linear tranformation $f\in\mathcal{L}(V,\mathbb{F})$ taking values in $\mathbb{F}$ is a *linear functional* on $V$. The vector space of all linear functionals on $V$ is denoted by $V^*$ and is called the *algebraic dual space* of $V$. The elements of $V^*$ are called *covectors*.
</MathBox>
