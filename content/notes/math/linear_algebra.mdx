...
<LatexFig width={50} src='/fig/universal_property_set_function.svg' alt=''
  caption='Universal property for set functions'
>
```tex
\documentclass[tikz]{standalone}
\usepackage{amssymb}
...
A pair $(S, f:A\to S)$, where $S\in\mathcal{F}$ and $f\in\mathcal{F}$ has the universal property for $\mathcal{F}$ as measured by $\mathcal{H}$, or is a universal pair for $(\mathcal{F},\mathcal{H})$, if for any $X\in S$ and any $g:A\to X$ in $\mathbb{F}$ there is a unique $\tau:S\to X$ in $\mathcal{H}$ such that
