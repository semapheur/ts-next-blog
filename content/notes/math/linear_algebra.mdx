...
$$
  ...
  \end{bmatrix},\; a_{i,j}\in\mathbf{F}
$$

This notation can be abbreviated by writing the entries as $\mathbf{A} = (a_{i,j})_{1\leq i \leq m,\; 1\leq j\leq n}$. The set of $m\times n$ matrices with entries in a field $\mathbb{F}$ is denoted $\mathcal{M}_{m,n}(\mathbb{F})$. For $n\times n$ matrices this set is denoted $\mathcal{M}_{n,n}(\mathbb{F})$. 
...
\begin{align*}
  ...
  \mathrm{ran}(\mathbf{M}\mathbf{A}) =& \mathbf{M}\;\ker(\mathbf{A})
\end{align*}
$$
</MathBox>
...
For $\mathbf{A}\in M_{m,n}(\mathbf{F})$ and $\mathbf{b}\in\mathbb{F}^m$, the following claims are equivalent
1. The system $\mathbf{A}\mathbf{x} = \mathbf{b}$ has at least one solution.
2. $\mathbf{b}\in\mathrm{ran}(\mathbf{A})$
...
**(1)** $\iff$ **(2)** is given by the definition of $\mathrm{ran}(\mathbf{A})$.
**(2)** $\iff$ **(3)** is given by the column representation of $\mathrm{ran}(\mathbf{A})$

$$
  \mathrm{ran}(A) = \Set{[\mathbf{a}_1 \cdots \mathbf{a_n}]\mathbf{x} | \mathbf{x}\in\mathbb{F}^n}
$$
...
For $\mathbf{A}\in M_{m,n}(\mathbf{F})$, the following claims are equivalent
...
For square matrices, i.e. $\mathbf{A}\in M_{n,n}(\mathbf{F})$, it follows that $\ker(\mathbf{A}) = \Set{0} \iff \mathrm{ran}(\mathbf{A}) = \mathbb{F}^n$, or equivalently that $f_\mathbf{A}$ is bijective.
</MathBox>
...
The dimension of $\ker(\mathrm{T})$ is called the *nullity* of $\mathrm{T}$ and is denoted $\mathrm{null}(\mathrm{T})$. The dimension of $\mathrm{ran}(\mathrm{T})$ is called the *rank* of $\mathrm{T}$ is denoted $\mathrm{rank}(\mathrm{T})$.
</MathBox>
...
Thus, if $\ker(\mathrm{T}) = \Set{0}$ then $\mathrm{T}(u) = \mathrm{T}(v) \iff u = v$, showing that $f$ is injective. Conversely, if $\mathrm{T}$ is injective and $u\in\ker(\mathrm{T})$, then $\mathrm{T}(u) = \mathrm{T}(0) \iff u = 0$. Hnece $\ker(\mathrm{T}) = \Set{\mathbf{0}}$.
</details>
</MathBox>
...
<MathBox title='' boxType='proposition'>
Let $\mathrm{T}\in\mathcal{L}(V,W)$. Any complement of $\ker(\mathrm{T})$ is isomorphic to $\mathrm{ran}(\mathrm{T})$.
...
Also, $\mathrm{ran}(\mathrm{T}^c)\subseteq \mathrm{ran}(\mathrm{T})$. For the reverse inclusion, if $\mathrm{T}\in\mathrm{ran}(f)$ then since $\mathbf{v} = \mathbf{u} + \mathbf{w}$ for $\mathbf{u}\in\ker(\mathrm{T})$ and $\mathbf{w}\in\ker(\mathrm{T})^c$, we have

$$
  \mathrm{T}(\mathbf{v}) = \mathrm{T}(\mathbf{u}) + \mathrm{T}(\mathbf{w}) = \mathrm{T}(\mathbf{w}) = \mathrm{T}^c(\mathbf{w}) \in\mathrm{ran}(\mathrm{T}^c)
$$

Thus $\mathrm{ran}(\mathrm{T}^c) = \mathrm{ran}(\mathrm{T})$ and it follows that $\ker(\mathrm{T})^c \cong \mathrm{ran}(\mathrm{T})$.
</details>
</MathBox>
...
$$
\begin{gather*}
  \dim(\ker(\mathrm{T})) + \dim(\mathrm{ran}(\mathrm{T})) = \dim(V) \\
...
and $\ker(\mathrm{T}') = \ker(\mathrm{T})/S$ and $\mathrm{ran}(\mathrm{T}') = \mathrm{ran}(\mathrm{T})$.
...
$$
  V/\ker(\mathrm{T}) \cong \mathrm{ran}(\mathrm{T})
$$
...
<MathBox title='' boxType='proposition'>
Let $\mathrm{T}\in\mathcal{L}(V,W)$, then
1. $\ker(\mathrm{T}^\times) = \mathrm{ran}(\mathrm{T})^0$
2. $\mathrm{ran}(\mathrm{T}^\times) = \ker(\mathrm{T})^0$
...
**(2):** If $f = g\mathrm{T} = \mathrm{T}^\times g\in\mathrm{ran}(\mathrm{T}^\times)$ then $\ker(\mathrm{T})\subseteq \ker(f)$ and so $f\in\ker(\mathrm{T})^0$.
