...
Matrices $A, B\in\mathcal{M}_{m,n}$ are row equivalent, denoted $A\sim B$, if eiter one can be obtained from the other by a series of elementary row operations. 
