...
mapping a vector space $W$ to the set of bilinear maps $U\times V\to W$. It can be shown that this functor is representable, i.e. there is a vector space $S$ such that $\mathbf{Bilin}(U,V;W)\cong\mathbf{Vect}_{\mathbb{F}}(S,W) = \mathcal{L}(S,W)$ naturally in $W$. 
...
\begin{align*}
  ...
  =& \alpha f(\mathbf{u},\mathbf{v}) + \beta f(\mathbf{u}', \mathbf{v}) - \alpha f(\mathbf{u}, \mathbf{v}) - \beta f(\mathb{u}', \mathbf{v})
  =& \mathbf{0}
\end{align*}
$$
