...
The set of exponential functions $\left\{ e_n := e^{inx} = \cos nx + i\sin nx : n \in\mathbb{Z} \right\}$ is an orthogonal basis for the Hilbert space $L^2\left([-\pi, \pi ]\right)$ of square integrable functions with an inner product given by
...
The Fourier series for an $L$-periodic function $f\in L^2\left([0, L]\right)$ can be similarly found using the orthogonal basis $\left\{ e_n := e^{\frac{i2\pi n x}{L}} \right\}$ giving
...
Both integrals converge as long as $f$ and $\hat{f}$ are integrable, i.e. $f, \hat{f}\in L^1 (-\infty, \infty)$. When the independent variable $x$ represents time, denoted $t\in\mathbb{R}$, the transform variable $\xi$ represents frequency, denoted $f\in\mathbb{R}$. In terms of angular frequency $\omega = 2\pi f$, the Fourier transform takes the form

$$
	\hat{f}(\omega) = \frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} f(t) e^{-i \omega t}\,\mathrm{d}x \quad \forall \omega\in\mathbb{R}
$$

## Properties

<MathBox title='Linearity of Fourier transforms' boxType='proposition'>
The Fourier transform is a linear operator, so that for $f, g\in L^1 (\mathbb{R}^n, \mathbb{C})$ and $\alpha, \beta\in\mathbb{C}$, we have
...
Let $f, \hat{f} \in L^1\left(\mathbb{R}\right)$ be integrable functions. The Fourier transform of a translation in $f$ is given by
...
Let $f:\mathbb{R}\to\mathbb{C}$ be an absolutely continuous differentiable function, and suppose $f, f' \in L^1(\mathbb{R})$ are integrable. The Fourier transform of the derivative $f'$ is given by
...
Let $f, g \in L^1(\mathbb{R})$ be integrable functions. The Fourier transform of the convolution
...
Let $f, g \in L^1(\mathbb{R})\cap L^2(\mathbb{R})$, then the following identity holds (Parseval's theorem)
...
The Schwartz space on $\mathbb{R}^n$ is the space of all smooth functions whose derivatives are rapidly decreasing

$$
\begin{align*}
	\mathcal{S}\left(\mathbb{R}^n, \mathbb{C}\right) &:= \left\{ f\in C^\infty (\mathbb{R}^n, \mathbb{C}) \mid \forall \alpha, \beta \in \mathbb{N}^n \;: \lVert f \rVert_{\alpha, \beta} < \infty \right\} \subset L^1\left(\mathbb{R}^n, \mathbb{C}\right) \\
	\lVert f \rVert_{\alpha, \beta} &:= \sup_{x\in\mathbb{R}^n} \left| x^\alpha \left(D^\beta f \right)(x) \right|
\end{align*}
$$

The Fourier transform defines an automorphism on Schwartz spaces, i.e. $f\in\mathcal{S}\left(\mathbb{R}^n, \mathbb{C}\right) \implies \hat{f}\in \mathcal{S}\left(\mathbb{R}^n, \mathbb{C}\right)$
</MathBox>

<MathBox title='Uncertainty principle' boxType='theorem'>
For any Schwartz function $f\in\mathcal{S}(\mathbb{R})$ and any $x_0, \xi_0 \in\mathbb{R}$, the following inequality holds
...
The discrete Fourier transform of a complex sequence $\left\{ f_n \right\}_{n=0}^N \subseteq\mathbb{C}$ produces a complex sequence $\left\{ F_k \right\}_{k=0}^{N-1}$ where

$$
	\mathcal{F}\left\{ f_k \right\} = F_k = \sum_{n=0}^{N-1} x_n \cdot e^{-\frac{i2\pi}{N}kn}
$$
...
The short-time Fourier transform of an integrable function $f:\mathbb{R}\to\mathbb{C}$ is defined by
...
$$
	\tau = j\Delta t \quad \nu = k \Delta\omega \quad j, k \in\mathbb{N}
$$
...
A function $\psi\in L^2(\mathbb{R})$ is called an orthonormal wavelet if it generates a basis for $L^2(\mathbb{R})$. The basis is constructed as a family of functions by means of translations and dilations of $\psi$

$$
	\left\{\psi_{a,b}(t) = \frac{1}{\sqrt{a}}\left( \frac{t - b}{a} \right) \middle| b \in\mathbb{R},\; a\mathbb{R}^+ \right\}
$$

In terms of dyadic translations and dilations with $a = 2^{-j}$ and $b = k2^{-j}$ for $j, k \in \mathbb{Z}$, we get
...
Suppose $\psi\in L^2 (\mathbb{R})$ is an orthonormal wavelet satisfying the boundedness property

$$
	C_\psi = \int_{-\infty}^\infty \frac{\left| \hat{\psi}(\omega) \right|^2}{|\omega |}\mathrm{d}\omega < \omega
$$

The wavelet transform of a function $f\in L^1(\mathbb{R}$ is defined as
...
Every function $f\in L^2(\mathbb{R})$ may be expanded in the wavelet basis as (using dyadic translations and dialtions)
