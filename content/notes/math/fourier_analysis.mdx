...
$$
2. The basis $\mathcal{B}_2 = \set{x\mapso 1} \cup \set{x\mapsto\frac{1}{\sqrt{\pi}}\cos{nx}}_{n\in\N_+} \cup \set{x\mapsto\frac{1}{\sqrt{\pi}}\sin(nx)}_{n\in\N_+}$ is orthonormal with respect to the inner product
$$
...
Let $f\in L_{2\pi}^2 (\R,\mathbb{F})$ be a square integrable $2\pi$-periodic function. For any $\n\in\N$ define
...
$$
\begin{align*}
	\tilde{a_0} =& \langle x\mapsto \frac{1}{\sqrt}, f \rangle = \frac{1}{\pi} \int_{-\pi}^\pi \frac{1}{\sqrt(x)} f(x) \;\d x \\
