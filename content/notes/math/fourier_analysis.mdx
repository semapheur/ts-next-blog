...
The set of exponential functions $\left\{ e_n := e^{inx} = \cos nx + i\sin nx : n \in\Z \right\}$ is an orthogonal basis for the Hilbert space $L^2\left([-\pi, \pi ]\right)$ of square integrable functions with an inner product given by
...
$$
\begin{align*}
	\mathcal{S}\left(\R^n, \mathbb{C}\right) &:= \left\{ f\in C^\infty (\R^n, \mathbb{C}) \mid \forall \alpha, \beta \in \N^n \;: \lVert f \rVert_{\alpha, \beta} < \infty \right\} \subset L^1\left(\R^n, \mathbb{C}\right) \\
...
$$
	\left\{\psi_{a,b}(t) = \frac{1}{\sqrt{a}}\left( \frac{t - b}{a} \right) \middle| b \in\R,\; a\R^+ \right\}
$$
