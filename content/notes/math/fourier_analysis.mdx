...
<MathBox title='Geometric properties of Fourier series' boxType='proposition'>
...
Since $\Set{x\mapsto e^{ikx}}_{k\in\Z}$, it follows that 

$$
\begin{align*}
	\lVert \mathcal{F}_n (f) \rVert^2 =& \langle \mathcal{F}_n (f), \mathcal{F}_n (f) \rangle \\
	=& \left\langle \sum_{k=-n}^n c_k e^{ikx}, \sum_{k=-n}^n c_k e^{ikx} \right\rangle \\
	=& \sum_{k=-n}^n c_k \overline{c_k} \sunderbrace{\langle e^{ikx}, e^{ikx} \rangle}_{=\lVert e^{ikx} \rVert^2 = 1} \\
	=& \sum_{k=-n}^n |c_k|^2
\end{align*}
$$
</details>
