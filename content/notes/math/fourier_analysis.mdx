...
Let $k\in\R$. A function $f:\R\to\R$ is $k$-periodic if $f(x + k) = x$ for all $x\in\R$. The set of all $k$-period real functions is denoted

$$
	\mathcal{F}_k (\R,\R) = \Set{f:\R\to\R | f(x + k) = f(x)\; \forall x\in\R}
$$
</MathBox>
...
<MathBox title='Orthonormal trigonometric bases' boxType='proposition'>
...
$$
2. The basis $\mathcal{B}_2 = \set{x\mapsto\frac{1}{\sqrt{\pi}}\cos{nx}}_{n\in\N} \cup \set{x\mapsto\frac{1}{\sqrt{\pi}}\sin(nx)}_{n\in\N_+}$ is orthonormal with respect to the inner product
$$
	\langle f, g \rangle_2 := \int_{-\pi}^\pi f(x)g(x)\;\d x
$$
3. The basis $\mathcal{B}_3 = \set{x\mapsto\cos{nx}}_{n\in\N} \cup \set{x\mapsto\sin(nx)}_{n\in\N_+}$ is orthonormal with respect to the inner product
$$
