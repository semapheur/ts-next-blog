...
$$
\begin{align*}
	c_n &:= \frac{1}{2\pi}\left\langle f, e^{inx} \right\rangle = \frac{1}{2\pi}\int_{-\pi}^\pi f(x) e^{-inx}\;\d x = \frac{1}{2\pi}\int_{-\pi}^\pi f(x) \left[\cos(nx) - i \sin(nx) \right]\;\d x \\
...
$$
	a_n = 2\alpha_n = \frac{1}{\pi} \int_{-\pi}^\pi f(x)\cos(nx)\;\d x \quad b_n = 2\beta_n = \frac{1}{\pi} \int_{-\pi}^\pi f(x)\sin(nx)\;\d x
$$
...
$$
	f(x) = \sum_{n=-\infty}^\infty c_n e^{\frac{i2\pi n x}{L}} \quad c_n = \frac{1}{L}\langle f(x), e_n \rangle = \frac{1}{L}\int_{-L/2}^{L/2} f(x)e^{-\frac{i2\pi n x}{L}}\;\d x
$$

Defining $\xi = \frac{n}{L}$ and $\Delta\xi = \frac{1}{L}$. Taking the limit $L\to\infty  \iff \Delta\xi\to 0$ we get

$$
	f(x) = \lim_{\Delta\xi \to 0} \sum_{n=-\infty}^\infty \Delta\xi \int_{-\frac{1}{2\xi}}^{\frac{1}{2\xi}} f(\xi) e^{-i2\pi \xi x}\;\d\xi e^{i2\pi n \Delta\xi x} = \lim_{\Delta\xi\to 0}\sum_{n=-\infty}^\infty \Delta\xi \langle f(x), e^{i2\pi \xi x} \rangle e^{i 2\pi n \Delta\xi x}
$$

Defining $\omega = \frac{n\pi}{L}$ and $\Delta\omega = \frac{\pi}{L}$. Taking the limit $L\to\infty$ or $\Delta\omega\to 0$ we get

$$
	f(x) = \lim_{\Delta\omega\to 0} \sum_{n=-\infty}^\infty \frac{\Delta\omega}{2\pi} \int_{-\frac{\pi}{\Delta\omega}}^{\frac{\pi}{\Delta\omega}} f(\xi) e^{-i\omega\xi}\;\d\xi e^{in\Delta\omega x} = \lim_{\Delta\omega\to 0}\sum_{n=-\infty}^\infty \frac{\Delta\omega}{2\pi}\langle f(x), e_i\omega x \rangle e^{in\Delta\omega x}
$$
...
$$
	\| f \|_{L^2}^2 = \int_{-\infty}^\infty |f(x)|^2\;\d x = \int_{-\infty}^\infty \left|\hat{f}(\xi)\right|^2\;\d\xi
$$
</MathBox>
...
$$
\begin{align*}
	\mathcal{S}\left(\R^n, \mathbb{C}\right) &:= \Set{ f\in C^\infty (\R^n, \mathbb{C}) | \forall \alpha, \beta \in \N^n \;: \| f \|_{\alpha, \beta} < \infty } \subset L^1\left(\R^n, \mathbb{C}\right) \\
	\| f \|_{\alpha, \beta} &:= \sup_{x\in\R^n} \left| x^\alpha \left(D^\beta f \right)(x) \right|
\end{align*}
$$
...
$$
	\| f(x) \|_{L^2} \leq 4\pi \left\| (x - x_0) f(x) \right\|_{L^2}\cdot\left\| (\xi - \xi_0) \hat{f}(x) \right\|_{L^2}
$$

If $f$ is normalized, i.e. $\| f(x) \|_{L^2} = 1$, then the inequality can be written
...
Applying integration by parts on $\| f \|_{L^2}^2$ gives

$$
\begin{align*}
	\| f \|_{L^2}^2 &= \int_{-\infty}^\infty |f(x)|^2\;\d x = \int_{-\infty}^\infty f(x)\overline{f(x)}\;\d x \\
	...
	&\leq 2 \| x f(x) \|_{L_2} \cdot \left\| f'(x) \right\|_{L_2}
\end{align*}
$$
...
$$
	\| f \|_{L^2}^2 \leq 4\pi \| x f(x) \|_{L^2} \cdot \left\| \xi \hat{f}(\xi) \right\|_{L^2}
$$
...
$$
\begin{align*}
	\| g \|_{L^2}^2 &= \int_{-\infty}^\infty \left| e^{-i2\pi\xi_0 x} f(x + x_0) \right|^2 \;\d x = \int_{-\infty}^\infty \left| f(t) \right|^2 \;\d t = \| f \|_{L^2}^2 \\
	\| u g(u) \|_{L^2} &= \left( \int_{-\infty}^\infty \left| u e^{-i2\pi\xi_0 u} f(u + x_0) \right|^2 \;\d u \right)^{1/2} \\
	&= \left( \int_{-\infty}^\infty \left| (x - x_0) f(x) \right|^2 \;\d x \right)^{1/2} = \| (x - x_0)f(x) \|_{L^2} \\
	\| t\hat{g}(t) \|_{L^2} &= \left( \int_{-\infty}^\infty \left| t e^{i2\pi x_0 t} \hat{f}(t + \xi_0) \right|^2 \;\d t \right)^{1/2} \\
...
$$
	f(x) = \alpha e^{-\frac{\lambda}{2} x^2} \quad \lambda > 0,\; |\alpha|^2 = \sqrt\frac{\lambda}{\pi}\| f \|_{L^2}^2
$$
