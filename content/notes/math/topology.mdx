...
<MathBox title='Topological space' boxType="definition" tag="1">
...
2. $\mathcal{T}$ is closed under arbitrary unions: if $\mathcal{A}\subseteq\mathcal{T}$, then $\bigcup_{A\in\mathcal{A}} A \in \mathcal{T}$
3. $\mathcal{T}$ is closed under finite intersections: if $\mathcal{A} \subseteq\mathcal{T}$ is finite, then $\bigcap_{A \in \mathcal{A}} A \in \mathcal{T}$
...
2. $\mathcal{C}$ is closed under arbitrary intersection: if $\mathcal{A}\subseteq\mathcal{C}$, then $\bigcap_{A\in\mathcal{A}} A \in \mathcal{C}$
3. $\mathcal{C}$ is closed under finite union: $\mathcal{A}\subseteq\mathcal{C}$ is finite, then $\bigcup_{A\in\mathcal{A}} \mathcal{A} \in \mathcal{C}$
...
Suppose $(X,\mathcal{T})$ is a topological space, and consider a point $x\in X$. A neighbourhood of $x$ is a subset $V\subseteq X$ that includes an open set $U\in\mathcal{T}$ containing $x$, i.e.
...
Let $(X, \mathcal{T})$ be a topological space, and let $\mathbb{F}$ denote a topological field. The support of a function $f: X\to\mathbb{F}$ is the set 

$$
  \operatorname{supp}(f) := \overline{\set{x\in X : f(x) \neq 0}}
$$

where the closure is taken with respect to $\mathcal{T}$. Equivalently, since $\set{0} \subset\mathbb{F}$ is closed, we can write

$$
  \operatorname{supp}(f) = \overline{f^{-1} (f \setminus\set{0)}
$$
</MathBox>
...
A topological space $(X, \mathcal{T})$ is a Hausdorff space if for every pair of distinct points $u, v \in X$ with $u \neq v$, there exist disjoint open subsets $U, V \in\mathcal{T}$, such that 
- $u \in U$
- $v \in V$
- $U \cap V = \emptyset$ 

A Hausdorff space guarantees the uniqueness of limits of sequences.
</MathBox>
...
<MathBox title="Topological metric spaces are Hausdorff" boxType="proposition">
Let $(X, d)$ be a metric space, and let $\mathcal{T}_d$ denote the topology induced by $d$. Then $(X, \mathcal{T}_d)$ is a Hausdorff space

<details>
<summary>Proof</summary>

For any distinct points $x, y\in X$ with $x \neq y$, we have $d(x,y) = r > 0$. Set $\epsilon := r/3$, and consider the open balls

$$
\begin{align*}
  B_\epsilon (x) :=& \set{z\in X : d(x,z) < \epsilon} \in\mathcal{T}_d \\
  B_\epsilon (y) :=& \set{z\in X : d(y,z) < \epsilon} \in\mathcal{T}_d
\end{align*}
$$

We show that $B_\epsilon (x) \cap B_\epsilon (y) = \emptyset$. If $z \in B_\epsilon (x) \cap B_\epsilon (y)$, then by the triangle inequality

$$
\begin{align*}
  d(x,y) \tleq& d(x,z) + d(z,y) \\
  <& \epsilon + \epsilon = \frac{2r}{\epsilon} < r
\end{align*}
$$

which contradicts $d(x,y) = r$.
</details>
</MathBox>
...
<MathBox title="Metric topology" boxType="example">
...
**(1):** We show that $C^c = X\setminus C$ is open. Fix $x\in C^c$. Since $X$ is Hausdorff, then $x\neq y$ for each $y\in C$. Thus, there exist disjoint open sets $U_y, V_y\in\mathcal{T}$ such that 
- $x\in U_y$
- $y\in V_y$
- $U_y \cap V_y = \emptyset$

The collection $\set{ V_y | y\in C}$ forms an open cover of $C$ since

$$
  C \subseteq \bigcup_{y \in C} V_y
$$

By compactness of $C$ is compact, there is a finite subset $B\subseteq C$ such that 

$$
  C \subseteq \bigcup_{y\in B V_y
$$

Since $B$ is finite, we can define an open set $U = \bigcap_{y\in B} U_y$ with $x\in U$. We show that $U\cap C = \emptyset$ by contradiction. If $z \in U\cap C$, then since $C \subseteq\bigcap_{y\in B} V_y$, there exists $y\in B$ such that $z\in V_y$. We also have $z \in U \subseteq U_y$, which contradicts $U_y \cap V_y = \emptyset$. Thus, $U \subseteq X\setminus C = C^c$, and hence $C^c$ is open.
...
The space of continuous functions with compact support is defined as

$$
  C_\text{c} (X,\mathbb{F}) := \set{f: X\to\mathbb{F} \in C(X, \mathbb{F}) : \operatorname{supp}(f) \text{ compact in } X}
$$
</MathBox>
...
<MathBox title="Continuity preserves compactness" boxType="proposition">
Let $(X, \mathcal{T}_X)$ and $(Y, \mathcal{T}_Y)$ be topological spaces and let $f: X\to Y$ a continuous function. If $A\subseteq X$ is compact, then $f[A] \subseteq Y$ is compact.

<details>
<summary>Proof</summary>

Let $\set{U_i}_{i\in I} \subseteq \mathcal{T}_Y$ be an open cover of $f[A]$, i.e.

$$
  f[A]\subseteq\bigcup_{i\in I} U_i
$$

Applying $f^{-1}$, we obtain

$$
  f^{-1} \left[\bigcup_{i\in I} U_i \right] \supseteq f^{-1} \left[f[A]\right] \supseteq A
$$

Using preservation of unions under preimages, we have

$$
  f^{-1} \left[\bigcup_{i\in I} U_i \right] = \bigcup_{i\in I} f^{-1} [U_i]
$$

Since $f$ is continuous, each $f^{-1} [U_i]$ is open in $X$. Thus, $\Set{f^{-1}[U_i]}_{i\in I}$ is an open cover of $A$. By compactness of $A$, there exists a finite subset $I_0 \subseteq I$ such that

$$
  A \subseteq \bigcup_{i\in I_0} f^{-1} [U_i]
$$
...
$$
\begin{align*}
  f[A] \subseteq& f\left[\bigcup_{i\in I_0} f^{-1} [U_i] \right] \\
  =& \bigcup_{i\in I_0} f\left[f^{-1} [U_i] \right]
\end{align*}
$$

Using the general inclusion $f\left[f^{-1} [U_i] \right] \subseteq U_i$, we conclude

$$
  f[A] \subseteq \bigcup_{i\in I_0} U_i
$$

Thus $\set{U_i}_{i\in I_0}$ is a finite subcover of $f[A]$, proving that $f[A]$ is compact.
</details>
</MathBox>
...
<MathBox title="Urysohn's lemma" boxType="lemma">
Let $(X, \mathcal{T})$ be a locally compact Hausdorff space, let $U\in\mathcal{T}$ be open, and $K\subseteq U$ be compact. Then there is a function $f \in C_\text{c} (X, \R)$ such that
1. $0 \leq f(x) \leq 1$ for all $x\in X$
2. $\operatorname{supp}(f) \subseteq U$
3. $f|_K = 1$
</MathBox>

### Semicontinuity

<MathBox title="Semicontinuity" boxType="definition">
Let $(X, \mathcal{T}_X)$ be a topological space. For every $\lambda\in\R$, a function $f:X\to\R$ is 
- upper semicontinuous if $f^{-1} [(-\infty, \lambda)] \in\mathcal{T}_X$
- lower semicontinuous if $f^{-1} [(\lambda, \infty)] \in\mathcal{T}_X$
</MathBox>

<MathBox title="Indicator function" boxType="example">
Let $(X, \mathcal{T})$ be a topological space. For an open set $A\in\mathcal{T}$, define the indicator function $\chi_A : X\to\R$ by
...
Since $\chi_A$ only takes values in $\set{0,1}$, a direct computation yields

$$
  \chi_A^{-1} [(\lambda, \infty)] = \begin{cases}
    \emptyset,\quad& \lambda \geq 1 \\
    A,\quad& \lambda\in[0,1) \\
    X,\quad& \lambda < 0
  \end{cases} \in \mathcal{T}
$$

By Definition $\ref{definition-1}$, $\emptyset, X \in\mathcal{T}$ and by assumption $A\in\mathcal{T}$. Thus, $\chi_A^{-1} [(\lambda, \infty)] \in\mathcal{T}$ for all $\lambda\R$. This shows that $\chi_A$ is lower semicontinuous.
</MathBox>
...
Geometrically, two nonzero points in $\R^{n+1}$ are equivalent if and only if they lie on the same line through the origin. Thus, $P_n (\R)$ can be interpreted as the set of all lines through the origin in $\R^{n+1}$. Each line through the origin in $\R^{n+1}$ meets the unit sphere $\mathbb{S}^n = \Set{\mathbf{x}\in\R^{n+1} | \norm{\mathbf{x}} = 1}$ in a pair of antipodal points. Conversely, a pair of antipodal points on $\mathbb{S}^n$ determines a unique line through the origin. This gives an equivalence relation $\sim$ on $\mathbb{S}^n$ by identifying antipodal points
...
For $\mathbf{x} = (x^i)_{i=1}^n \in\R^n$, let $\norm{ \mathbf{x} } = \sqrt{\sum_{i=1}^n (x^i)^2}$ be the norm of $\mathbf{x}$. The map $f: \R^{n+1}\setminus\Set{0} \to\mathbb{S}^n$ given by

$$
  f(\mathbf{x}) = \frac{\mathbf{x}}{\norm{\mathbf{x}}}
$$
...
Define $\bar{f}: P_n (\R^n) \to\mathbb{S}^n /{\sim}$ by $\bar{f}([\mathbf{x}]) = \left[\frac{\mathbf{x}}{\norm{ \mathbf{x} }}\right] \in \mathbb{S}^n /{\sim}$. This map is well defined because

$$
  \bar{f}([t\mathbf{x}]) = \left[\frac{t\mathbf{x}}{\norm{ t\mathbf{x}}}\right] = \left[\pm\frac{\mathbf{x}}{\norm{\mathbf{x}}} \right] = \left[\frac{\mathbf{x}}{\norm{\mathbf{x}}} \right]
$$
...
$$
  \bar{g}\circ\bar{f}([\mathbf{x}]) = \left[\frac{\mathbf{x}}{\norm{\mathbf{x}}}] = [\mathbf{x}]
$$
...
<MathBox title="" boxType="proposition" tag="21">
...
<MathBox title="Möbius strip" boxType="example">
...
A topological space $(X, \mathcal{T})$ is *locally compact* if every point $x\in X$ has a neighbourhood contained in a compact subset of $X$. A subset of $X$ is *precompact* in $X$ if its closure in $X$ is compact.
</MathBox>
