...
The set of all equivalence classes, denoted $X\setminus\sim$, is a called a *quotient* of $X$. The quotient $X\setminus\sim$ is a partition of $X$ into a collection of nonempty, mutually disjoint subsets, defined as 

$$
  X\setminus\sim := \Set{ [x]_\sim | x \in X }
$$

There is a surjective quotient map $p: X\to X\setminus\sim$ with $x \mapsto [x]_\sim$ called the *canonical/natural projection*. 

If $X$ is a topological space, we define a topology $\mathcal{T}_\sim$ on $X\setminus\sim$ be declaring by declaring a subset $U \subseteq X \setminus\sim$ to be open if and only if $p^{-1}[U] \subset X$ is open, i.e.

$$
  p^{-1} [U] \in \mathcal{T} \iff U \in T_\sim
$$

showing that $p$ is also continuous. Furthermore, since
...
$$
  p^{-1} \left(\bigcap_i U_i \right) = \bigcup_i p^{-1} (U_i)
$$

the collection of open sets in $X\setminus\sim$ is closed under arbitrary unions and finite intersections, and is therefore a topology. It is called the *quotient topology* of $X$ by the equivalence relation $\sim$.

### Continuity of a map on a quotient

Let $\sim$ be an equivalence realtion on the topological space $X$ and give $X\setminus\sim$ the quotient topology. Suppose a function $f:X\to Y$ from $X$ to another topological space $Y$ is constant on each equivalence class. Then it induces a map $\bar{f}: X\setminus\sim\to Y$ by
...
\begin{CD}
  X @>{f}>> Y \\
  @V{p}VV @| \\
  X\setminus\sim @>>{\bar{f}}> Y
\end{CD}
$$

<MathBox title='' boxType='proposition' tag='proposition-17'>
The induced map $\bar{f}: X\setminus\sim \to Y$ is continuous if and only if $f:X\to Y$ is continuous.
...
Conversely, suppose $f$ is continuous. Let $V$ be open in $Y$. Then $f^{-1} (V) = p^{-1} (\bar{f}^{-1}(V))$ is open in $X$. By the definition of quotient topology, $\bar{f}^{-1}(V)$ is open in $X\setminus\sim$. Since $V$ was arbitrary, $\bar{f}:X\setminus Y$ is continuous.
</details>
</MathBox>
...
This is an equivalence relation on $X$. We say that the quotient space $X\setminus\sim$ is obtained from $X$ by identifying $U$ to a point.

<MathBox title='' boxType='proposition'>
Let $I = [0,1]$ be the unit interval and $I\setminus\sim$ the quotient space obtained from $U$ by identifying the two points $\Set{0,1}$ to a point. Suppose $\mathbb{S}^1$ is the unit circle in the complex pane. The function $\bar{f}:I\to\mathbb{S}^1 \subset\mathbb{C}$ by $f(x) = e^{i2\pi x}$ induces a function $\bar{f}:I\setminus\sim\to\mathbb{S}^1$ since $f(0) = f(1)$.

The function $\bar{f}:I\setminus\sim\to\mathbb{S}^1$ is a homeomorphism.

<details>
<summary>Proof</summary>

Since $f$ is continuous, $\bar{f}$ is also continuous by proposition $\ref{proposition-17}$. Clearly, $\bar{f}$ is a continuous bijection. As the continuous image of the compact set $I$, the quotient $I\setminus\sim$ is compact. Thus, $\bar{f}$ is a continuous bijection from the compact space $I\setminus\sim$ to the Hausdorff space $\mathbb{S}^1$. By corollary, $\bar{f}$ is a homeomorphism.
</details>
</MathBox>
...
<MathBox title='' boxType='proposition'>
Let $X$ be a topological space with an equivalence relation $\sim$. If the quotient space $X\setminus\sim$ is Hausdorff, then the equivalence class $[x]$ of any point $x$ in $X$ is closed in $X$.

<details>
<summary>Proof</summary>

Suppose $p:X\to X\setminus\sim$ is the canonical projection and $X\setminus\sim$ is Hausdorff. Since every singleton set in a Hausdorff space is closed, then for any $x\in X$, its image $\Set{p(x)}$ is closed in $X\setminus\sim$. By the continuity of $p$, the inverse image $p^{-1}(\Set{p(x)}) = [x]$ is closed in $X$.
</details>
<MathBox>
...
An equivalence relation $\sim$ on a topological space $X$ is open if the canonical projection $p:X\to X\setminus\sim$ is open.
</MathBox>
...
If the equivalence relation $\sim$ is equality, then the quotient space $X\setminus\sim$ is $X$ itself and the graph $R$ of $\sim$ is simpy the diagonal
...
Suppose $\sim$ is an open equivalence relation on a topological space $X$. Then the quotient space $X\setminus\sim$ is Hausdorff if and only if the graph $R$ of $\sim$ is closed in $X\times X$.
...
- $\iff (X\times X) - R$ is open in $X\times X$
...
- $\iff$ for any two points $[x] \neq [y]$ is $X\setminus\sim$, there exist neighbourhoods $U$ of $x$ and $V$ of $y$ in $X$ such that $p(U)\cap p(V) = \emptyset$ in $X\setminus\sim$ **(\*)**

It remains to show that the last statement is equivalent to $X\setminus\sim$ being Hausdorff. First assume **(\*)**. Since $\sim$ is an open equivalence relation, $p(U)$ and $p(V)$ are disjoint open sets in $X\setminus\sim$ containing $[x]$ and $[y]$ respectively. Thus, $X\setminus\sim$ is Hausdorff.

Conversely, suppose $X\setminus\sim$ is Hausdorff. Let $[x]\neq [y]$ in $X\setminus\sim$. Then there exist disjoint open sets $A$ and $B$ in $X\setminus\sim$ such that $[x]\in A$ and $[y]\in B$. By the surjectivity of $p$, we have $A = p(p^{-1} (A))$ and $B = p(p^{-1}(B)$. Let $U = p^{-1}(A)$ and $V = p^{-1}(B)$. Then $x\in U$, $y\in V$ and $A = p(U)$ and $B = p(V)$ are disjoint open sets in $X\setminus\sim$.
</details>
</MathBox>
...
<MathBox title='' boxType='theorem'>
Let $\sim$ be an open equivalence relation on a topological space $X$ with canonical projection $p:X\to X\setminus\sim$. If $\mathcal{B} = \Set{B_\alpha}$ is a basis for $X$, then its image $\Set{p(B_\alpha)}$ under $p$ is a basis for $X\setminus\sim$.

<details>
<summary>Proof</summary>

Since $p$ is an open map, $\Set{p(B_\alpha)}$ is a collection of open sets in $X\setminus\sim$. Let $W$ be an open set in $X\setminus\sim$ and $[x]\in W$ with $x\in X$. Then $x\in p^{-1}(W)$. Since $p^{-1}(W)$ is open, there is a basic open set $B\in\mathcal{B}$ such that
...
$$
  [x] = p(x) \in p(B) \subset W
$$

which proves that $\Set{p(B_\alpha)}$ is a basis for $X\setminus\sim$.
</details>
</MathBox>

<MathBox title='' boxType='corollary'>
If $\sim$ is an opne equivalence relation on a second-countable space $X$, then the quotient space $X\setminus\sim$ is second countable.
</MathBox>
