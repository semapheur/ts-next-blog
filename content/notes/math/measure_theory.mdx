...
Consider a finite set $X = \Set{ (x_i)_{i=1}^n }$. The $\sigma$-algebra generated by an arbitrary singleton subset $\mathcal{M} = \Set{\{ x_j }\} = \Set{ X_j }$ for $1 \leq j \leq n$ is 

$$
  \sigma(\mathcal{M}) = \Set{ \emptyset, \Set{ x_j }, \Set{ (x_i)_{i=1, i\neq j}^n }, \Set{ (x_i)_{i=1}^n } } = \left\{ \emptyset, X_j, X_j^c, X }
$$

The $\sigma$-algebra generated by a arbitrary subset of singletons $\mathcal{M} = \Set{ \left(\Set{ x_j } \right)_{j\geq 1}^{m \leq n} } = \left\{ \left(X_j \right)_{j\geq 1}^{m\leq n} }$ is
...
$$
  \sigma(\mathcal{A}) = \Set{ \bigcup_{j\in J} A_j |dle| J\subseteq I }
$$
<details>
<summary>Proof</summary>

Let $\mathcal{S} = \Set{ \bigcup_{j\in J} A_j |dle| J\subseteq I }$. Note that $X = \bigcup_{i\in I} A_i$ so $X\in\mathcal{S}$. Suppose, that $B\in\mathcal{S}$. Then $B = \bigcup_{j\in J} A_j$ for some $J\subseteq I$, and $B^c = \bigcup_{j\in J^c} A_j$ so $B^c\in\mathcal{S}$. Suppose that $B_k\in\mathcal{S}$ for $k\in K$ where $K$ is a countable index set. Then for each $k\in K$ there exists $J_k\subseteq I$ such that $B_k = \bigcup_{j\in J_k} A_j$. However, then $\bigcup_{k\in K} B_k = \bigcup_{k\in K}\bigcup_{j\in J_k} A_j = \bigcup_{j\in J} A_j \in\mathcal{S}$ where $J = \bigcup_{k\in K} J_k$. Hence $\mathcal{S}$ is a $\sigma$-algebra of $X$.
...
2. $\mathcal{C} = \sigma\Set{\{x} | x\in X \}$
...
2. Let $\mathcal{D} = \sigma\Set{\{x | x\in X}\}$. Clearly $\Set{x}\in\mathcal{D}$. If $A^c$ is countable, then by identical argument, $A^c\in\mathcal{D}$ so $A\in\mathcal{D}$. Hence $\mathcal{C} = \sigma\Set{\{x} | x\in X \}$ 
</details>
</MathBox>
...
Since $(X, \mathcal{A}, \mu)$ is $\sigma$-finite there exists a countable collection $\left{A_n\}_{n\in\N}\subseteq\mathcal{A}$ such that $A_n<\infty$ for each $n\in\N$ and $X = \bigcup_{n\in\N}A_n$.
...
$$
  H_\delta^k (A) := \inf\Set{ \sum_{B\in\mathcal{B}} \left[ \mathrm{diam}(B) \right]^k |dle| \bigcup_{B\in\mathcal{B}} B \supseteq A,\; \mathrm{diam}(B) < \delta }
$$
...
$$
  \int_X f\;\mathrm{d}\mu = \sup\Set{ \int_X h\;\mathrm{d}\mu | h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq f } \leq \sup\left\{ \int_X h\;\mathrm{d}\mu \mid h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq g } = \int_{X} g\;\mathrm{d}\mu
$$
</details>
</MathBox>
