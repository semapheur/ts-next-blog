...
Suppose $f,g:X\to\R$ are measurable functions whose integrals relative to $\mu$ exist. If $g \overset{\mu\textrm{-a.e.}}{=} f$ on $X$ then $\int_A f\;\mathrm{d}\mu = \int_A g\;\mathrm{d}\mu$ for every $A\in\mathcal{A}$. Therefore, if $f$ is a density function for $\nu$ relative to $\mu$, then so is $g$. Conversely, if $\int_A f\;\mathrm{d}\mu = \int_A g\;\mathrm{d}\mu$ for every $A\in\mathcal{A}$, it follows that $f \overset{\mu\textrm{-a.e.}}{=} g$ since $\mu$ is $\sigma$-finite.
...
$$
\begin{align*}
  \int_A \max\Set{g_1,g_2}\;\mathrm{d}\mu = \int_{A_1} \max\Set{g_1,g_2}\;\mathrm{d}\mu + \int_{A_2} \max\Set{g_1,g_2}\;\mathrm{d}\mu \\
...
Suppose $g = \sum_{i\in I}a_i\chi_{A_i}$ is a non-negative simple function where $I$ is a finite index set, $a_i\in[0,\infty)$ and for $i\in I$ and $\Set{A_i}_{i\in I}$ is a disjoint collection of sets in $\mathcal{A}$. Then $\int_X g\;\mathrm{d}\mu = \sum_{i\in I} a_i\nu(A_i)$. However $\nu(A_i) = \int_{A_i} f\;\mathrm{d}\mu = \int_X \chi_{A_i}f\;\mathrm{d}\mu$ for each $i\in I$ so
...
Suppose $g:X\to[0,\infty)$ is measurable. There exists an increasing sequence of non-negative simple functions $(g_n)_{n\in\N}$ on $X$ with $g_n\xrightarrow{n\to\infty}g$. Since $f$ is non-negative, $g_n f$ is increasing in $n$ on $X$ and $g_n f\xrightarrow{n\to\infty} gf$. By the last result $\int_X g_n\;\mathrm{d}\nu = \int_X g_n f\;\mathrm{d}\mu$ for each $n\in\N$. By the monotone convergence theorem $\int_X g_n\;\mathrm{d}\nu\xrightarrow{n\to\infty}\int_X g\;\mathrm{d}\nu$ and $\int_X g_n f\;\mathrm{d}\mu\xrightarrow{n\to\infty}\int_X gf\;\mathrm{d}\mu$. Hence $\int_X g\;\mathrm{d}\nu = \int_X gf\;\mathrm{d}\mu$.
...
Suppose $G$ is a distribution function on $[0,\infty)$ and $f:[0,\infty)\to\R$ is locally bounded. Then $g: [0,\infty)\to\R$ defined by $g(t) = \int_0^t f(s)\;\mathrm{d}G(s)$ is also locally bounded.
...
$$
\begin{align*}
  \int_Y g \;\mathrm{d}(h*\mu) &= \sup\Set{ \int_Y \tilde{s} \mathrm{d}\mathrm{d}(h*\mu) | \tilde{s}: Y \to [0, \infty), \tilde{s} \leq g } \\
  &= \sup\Set{ \int_X \tilde{s} \circ h \mathrm{d}\mu | \tilde{s}: Y \to [0, \infty), \tilde{s}\circ h \leq g \circ h } \\
  &= \sup\Set{ \int_X s \mathrm{d}\mu | s: X \to [0, \infty), s \leq g \circ h } \\
  &= \int_X g \circ h \;\mathrm{d}\mu
\end{align*}
$$
</details>
...
$$
  \phi_\alpha (x) = \int\frac{\mathrm{d}\mu(y)}{\rho(x,y)^\alpha}
$$

The $\alpha$-energy of $\mu$ is defined As

$$
  I_\alpha = \int\phi_\alpha(x)\;\mathrm{d}\mu(x) = \iint\frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha}
$$
</MathBox>
...
$$
  H_\varepsilon^\alpha \geq \mu(E)\left( \iint_{\rho(x,y)< \varepsilon} \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha} \right)^{-1}
$$
...
$$
  \iint_{\rho(x,y) < \varepsilon} \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha} \geq \sum_{n\in\N_+} \iint_{A_n \times A_n}  \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha} \geq \sum_{n\in\N_+} \frac{\mu(A_n)^2}{|A_n|^2}
$$
...
\begin{align*}
  ...
  &\leq H_\varepsilon^\alpha (E) \iint_{\rho(x,y) < \varepsilon} \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha}
\end{align*}
$$
...
$$
\begin{gather*}
  \int_X f (x) \;\mathrm{d}\mu (x) := \sup \Set{ \int_X h\;\mathrm{d}\mu \; | \; h \in \mathcal{S}^+ , \; h \leq f } \\
...
$$
  \int_X f\;\mathrm{d}\mu = \sup\Set{ \int_X h\;\mathrm{d}\mu | h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq f } \leq \sup\Set{\int_X h\;\mathrm{d}\mu | h\in\mathcal{S}^+ \text{ and } 0\leq h\leq g} = \int_{X} g\;\mathrm{d}\mu
$$
</details>
</MathBox>
...
Furthermore, $\int_X (f_n + g_n)\;\mathrm{d}\mu = \int_X f_n\;\mathrm{d}\mu + \int_X g_n\;\mathrm{d}\mu$ for each $n\in\N$. Then in the limit $n\to\infty$

$$
  \int_X (f+g)\mathrm{d}\mu = \int_X f\;\mathrm{d}\mu + \int_X g\;\mathrm{d}\mu
$$

2. Note that $(cf_n)_{n\in\N}$ is an increasing sequence of non-negative simple functions with $cf_n\xrightarrow{n\to\infty} cf$. By the monotone convergence theorem, $\int_X cf_n\;\mathrm{d}\mu\xrightarrow{n\to\infty}\int_X cf\;\mathrm{d}\mu$. However, $\int_X cf_n\;\mathrm{d}\mu = c\int_X f_n \;\mathrm{d}\mu$ for each $n\in\N$ and in the limit $n\to\infty$ we get
...
Let $(X,\mathcal{A},\mu)$ be a measure space. A measurable function $f:X\to\R$ is integrable if and only if $\int_X |f|\;\mathrm{d}\mu$. As a direct result, if $f$ is integrable and $A\in\mathcal{A}$, then the integral of $f$ over $A$ exists.

<details>
<summary>Proof</summary>

Suppose $f$ is integrable. Recall that $|f| = f^+ + f^-$. By the additive property for non-negative functions, $\int_X |f|\;\mathrm{d}\mu = \int_X f^+\;\mathrm{d}\mu + \int_X f^-\;\mathrm{d}\mu <\infty$. Conversely, suppose $\int_X |f|\;\mathrm{d}\mu<\infty$. Then $f^+\leq|f|$ and $f^-\leq|f|$ and by the monotonicity property for non-negative functions $\int_X f^+\;\mathrm{d}\mu\leq\int_X |f|\;\mathrm{d}\mu<\infty$ and $\int_X f^-\;\mathrm{d}\mu\leq\int_X |f|,\mathrm{d}\mu<\infty$.
...
First, suppose $\int_X f\;\mathrm{d}\mu<\infty$ and $\int_X g\;\mathrm{d}\mu<\infty$. Note that $(f-g)^+ \leq f$ and $(f-g)^-\leq g$. By the monotonicity property for non-negative functions
...
Next, suppose $\int_X f\;\mathrm{d}\mu = \infty$ and $\int_X g\;\mathrm{d}\mu<\infty$. Then $f-g\leq (f-g)^+$ and hence $f \leq (f-g)^+ + g$. The order and additive properties for non-negative functions give
...
Finally, suppose $\int_X f\;\mathrm{d}\mu<\infty$ and $\int_X g\;\mathrm{d}\mu=\infty$. By the same argument in the previous case, we have 
...
*4a*
Note that if $\mu(A) = 0$ then clearly $\int_X f\;\mathrm{d}\mu = \int_A f\;\mathrm{d}\mu + \int_{A^c} f\;\mathrm{d}\mu = 0$. In the opposite case, let $B_n = \Set{x\in X| f(x)\geq\frac{1}{n}}$ for $n\in\N$ and $B = \Set{x\in X| f(x)>0}$. Then $B_n$ is increasing in $n$ and $\bigcup_{n\in\N} B_n = B$. If $\mu(B) > 0$ then $\mu(B_n) > 0$ for some $n\in\N$. However, $f\geq \frac{1}{n}\chi_{B_n}$ on $A$, so by increasing monotonicity
...
**5. Null property**
First, suppose $g$ is a non-negative simple function with $g = 0$ on $A^c$. Then $g$ has the representation $g = \sum_{i\in I}a_i \chi_{A_i}$ where $a_i\in(0,\infty)$ and $A_i\subseteq A$ for $i\in I$. However, $\mu(A_i) = 0$ for each $i\in I$ and so $\int_X g\;\mathrm{d}\mu = \sum_{i\in I}a_i \mu(A_i) = 0$.

Next, suppose $f:X\to[0,\infty)$ is measurable. If $g$ is a non-negative simple function with $g\leq\chi_{A} f$, then $g = 0$ on $A^c$ and by $(1)$ it follows that $\int_X g\;\mathrm{d}\mu = 0$. Hence

$$
  \int_A f\;\mathrm{d}\mu = \int_X \chi_A f\;\mathrm{d}\mu
$$

Suppose $f:X\to\R$ is measurable. Then $\int_A f\;\mathrm{d}\mu = \int_A f^+\;\mathrm{d}\mu - \int_A f^-\;\mathrm{d}\mu$. Both integrals on the right are $0$ by $(2)$.
...
Suppose $f_n:X\to\R$ is measurable and integrable on $(X,\mathcal{A},\mu)$ for each $n\in\N$. If $(f_n)_{n\in\N}$ is increasing $X$ and $\int_X f_1\;\mathrm{d}\mu > -\infty$, or $(f_n)_{n\in\N}$ is decreasing on $X$ and $\int_X f_1\;\mathrm{d}\mu<\infty$ then
...
Suppose $f_n$ is increasing in $n\in\N$. If $\int_X f_1\;\mathrm{d}\mu = \infty$ then by increasing monotonicity, $\int_X f_n\;\mathrm{d}\mu = \infty$ for all $n\in\N$ and $\int_X f\;\mathrm{d}\mu = \infty$. In this case, the monotone converge theorem trivially holds.
...
Suppose $|f_n|$ is bounded in $n\in\N$ on $A$ by $c\in(0,\infty)$. The constant $c$ is integrable on $A$ since $\int_A c\;\mathrm{d}\mu = c\mu(A)<\infty$ and $|f_n|\leq c$ on $A$. Therefore, the result follows from the dominated convergence theorem.
</details>
</MathBox>
...
**Non-negative functions**
4. Suppose $f:X\times Y\to[0,\infty)$ is measurable. Then there exists an increasing sequence of non-negative simple functions $(g_n)_{n\in\N}$ on $X\times Y$ such that $g_n\xrightarrow{n\to\infty} f$. By the monotone convergence theorem, $\int_{X\times Y} g_n\;\mathrm{d}(\mu\otimes\nu)\xrightarrow{n\to\infty}\int_{X\times Y} f\;\mathrm{d}(\mu\otimes\nu)$. For a fixed $x\in X$, the function $y\mapsto g_n(x, y)$ is increasing in $n$ on $Y$ and converges to $f(x, y)$ in the limit $n\to\infty$. Applying the monotone convergence theorem gives
