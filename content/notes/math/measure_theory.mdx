...
$$
  \mathcal{F} := \{ A\subseteq X \;|\; A\textrm{ is finite or }A^c\textrm{ is finite}\}
$$
<details>
<summary>Proof</summary>
...
$$
  \sigma(\mathcal{B}) := \bigcup \{ \mathcal{A} \;|\; \mathcal{A}\textrm{ is a }\sigma\textrm{-algebra of X and }\mathcal{B}\subseteq\mathcal{A} \}
$$
...
1. $\mathcal{B} := \left\{ B_x \;|\; x \in\{0, 1\}^n \right\}$ partitions $S$.
2. $A_i = \bigcup\left\{ B_x \;|\; x\in\{0, 1 \}^n,\; x_i = 1 \right\}$ for $i\in\{1, 2, \dots n\}$.
3. $\sigma(\mathcal{A}) = \sigma(\mathcal{B}) = \left\{ \bigcup_{x\in J} B_x \;|\; J\subseteq \{0, 1\}^n \right\}$
...
2. Fix $i\in\{1,2,\dots, n\}$. If $x\in\{0, 1\}^n$ and $x_i = 1$ then $B_x\subseteq A_i$. Thus $\bigcup\left\{ B_x \;|\; x\in\{0, 1\}^n,\; x_i = 1 \right\} \subseteq A_i$. Conversely, suppose $s\in A_i$ and define $y\in\{0, 1\}^n$ by $y_j = 1$ if $s\in A_j$ and $y_j = 0$ if $s\notin A_j$ for each $j\in\{1,2,\dots,n \}$. Then $y_i = 1$ and $s\in B_y$ and hence $s\in \bigcup\left\{ B_x \;|\; x\in\{0, 1\}^n,\; x_i = 1 \right\}$ 
...
$$
  \sigma(\mathcal{A}) = \left\{ \bigcup_{j\in J} A_j \mid| J\subseteq I \right\}
$$
<details>
<summary>Proof</summary>

Let $\mathcal{S} = \left\{ \bigcup_{j\in J} A_j \mid| J\subseteq I \right\}$. Note that $X = \bigcup_{i\in I} A_i$ so $X\in\mathcal{S}$. Suppose, that $B\in\mathcal{S}$. Then $B = \bigcup_{j\in J} A_j$ for some $J\subseteq I$, and $B^c = \bigcup_{j\in J^c} A_j$ so $B^c\in\mathcal{S}$. Suppose that $B_k\in\mathcal{S}$ for $k\in K$ where $K$ is a countable index set. Then for each $k\in K$ there exists $J_k\subseteq I$ such that $B_k = \bigcup_{j\in J_k} A_j$. However, then $\bigcup_{k\in K} B_k = \bigcup_{k\in K}\bigcup_{j\in J_k} A_j = \bigcup_{j\in J} A_j \in\mathcal{S}$ where $J = \bigcup_{k\in K} J_k$. Hence $\mathcal{S}$ is a $\sigma$-algebra of $X$.
...
$$
  \sigma(\mathcal{B}) = \{ A\subseteq X \;|\; A\in\mathcal{C}\textrm{for some countable }\mathcal{C}\subseteq\mathcal{B} \}
$$
<details>
<summary>Proof</summary>
...
Suppose that $(X, \mathcal{A})$ is a measurable space, and consider a subset $R\subseteq S$. Let $\mathcal{R} := \{ A\cap R \;|\; A\in\mathcal{A} \}$. Then
1. $\mathcal{R}$ is a $\sigma$-algebra of subsets of $R$
2. if $R\in\mathcal{A}$ then $\mathcal{R} = \{ B\in\mathcal{A} \;|\; B\subseteq R\}$
<details>
<summary>Proof</summary>
...
$$
  \mathcal{C} := \{ A\subseteq X \;|\; A\textrm{ is countable or }A^c\textrm{ is countable} \}
$$

Then:
1. $\mathcal{C}$ is a $\sigma$-algebra
2. $\mathcal{C} = \sigma\{\{x\} \;|\; x\in X \}$
...
2. Let $\mathcal{D} = \sigma\{\{x \;|\; x\in X\}\}$. Clearly $\{x\}\in\mathcal{C}$ for $x\in X$. Hence $\mathcal{D}\subseteq\mathcal{C}$. Conversely, suppose that $A\in\mathcal{C}$. If $A$ is countable, then $A = \bigcup_{x\in A} \{x\}\in\mathcal{D}$. If $A^c$ is countable, then by identical argument, $A^c\in\mathcal{D}$ so $A\in\mathcal{D}$. Hence $\mathcal{C} = \sigma\{\{x\} \;|\; x\in X \}$ 
</details>
</MathBox>
...
1. $\mathcal{I}_1 = \{I\subseteq\mathbb{R} \;|\; I\textrm{ is an interval}\}$
2. $\mathcal{I}_2 = \{(a, b]\;|\; a,b\in\mathbb{R},\; a<b\}$
3. $\mathcal{I}_3 = \{(-\infty, b]\;|\; b\in\mathbb{R}\}$
<details>
<summary>Proof</summary>
...
$$
  \mathcal{D}_n = \left\{\left(\frac{j}{2^n},\frac{j+1}{2^n}\right) \;|\; j\in\mathbb{Z} \right\}\quad n\in\mathbb{N}
$$
...
<MathBox title='Proposition' boxType='proposition'>
Let $(X, \mathcal{A}_X)$ and $(Y, \mathcal{A}_Y)$ be two measurable spaces with $\mathcal{A}_Y = \sigma(\mathcal{B})$ for a collection $\mathcal{B}$ of subsets of $\mathcal{Y}$. Then $f:X\to Y$ is measurableif and only if$f^{-1}[B]\in\mathcal{A}_X$ for every $B\in\mathcal{B}$.
<details>
<summary>Proof</summary>

If $f:X\to Y$ is measurable, then the condition in the proposition holds trivially since $\mathcal{B}\subseteq\mathcal{A}_Y$. Conversely, suppose that $f^{-1}[B]\in\mathcal{A}_X$ for every $B\in\mathcal{B}$, and let $\mathcal{U} = \{ A\in\mathcal{A}_Y \;|\; f^{-1}[A]\in\mathcal{A}_X \}$. Then $Y \in U$ since $f^{-1}[Y] = X \in\mathcal{A}_X$. If $A\in \mathcal{U}$ then $f^{-1}[A^c] = \left( f^{-1}[A] \right)^c \in\mathcal{A}_X$, so $A^c\in \mathcal{U}$. If $A_i \in \mathcal{U}$ for $i$ in a countable index set $I$, then $f^{-1}\left[\bigcup_{i\in I} A_i \right] = \bigcup_{i\in I} f^{-1} [A_i] \in\mathcal{A}_X$, and hence $\bigcup_{i\in I} A_i \in \mathcal{U}$. Thus $\mathcal{U}$ is a $\sigma$-algebra of $\mathcal{A}_Y$. However, $\mathcal{B}\subseteq\mathcal{U}$ by assumption, so $\mathcal{A}_Y = \sigma(\mathcal{B})\subseteq\mathcal{U}$. Since $\mathcal{U}\subseteq\mathcal{A}_Y$ by definition, then $\mathcal{U} = \mathcal{A}_Y$ and hence $f$ is measurable.
</details>
</MathBox>
...
Suppose that $X$ is a set and $(Y, \mathcal{A}_Y)$ is a measurable space. Consider a function $f:X\to Y$ and define $\sigma(f) = \{ f^{-1}[A] \;|\; A\in\mathcal{A}_Y \}$. Then
...
$$
  \sigma\{f_i \}_{i\in I} = \sigma\{\sigma(f_i) \}_{i\in I} = \sigma\left\{ f_i^{-1}(A) \;|\; A\in\mathcal{A}_{Y_i} \right\}_{i\in I}
$$
...
$$
  \sigma\left\{ \prod_{i\in I} A_i \;|\; A_i\in\mathcal{A}_i\textrm{for each }i\in I\textrm{ and }A_i = X_i\textrm{ for all but finitely many }i\in I \right\}
$$
</MathBox>
...
3. $\{x_2 \in\mathcal{X_2} \;|\; (x_1, x_2)\in A,\; x_1\in X_1 \}\in\mathcal{A}_2$ 
4. $\{x_1 \in\mathcal{X_1} \;|\; (x_1, x_2)\in A,\; x_2\in X_2 \}\in\mathcal{A}_1$
...
3. $1_{x_1}^{-1}[A] = \{ x_2\in X_2 \;|\; (x_1, x_2)\in A,\; x_1\in X_1 \}\in\mathcal{A}_2$
4. $2_{x_2}^{-1}[A] = \{ x_1\in X_1 \;|\; (x_1, x_2)\in A,\; x_2\in X_2 \}\in\mathcal{A}_1$
...
Let $(X, \mathcal{A}, \mu)$ be a measure space, and consider a sequence of sets $\(A_i \in\mathcal{A}\)_{i\in \mathbb{N}}$ in $\mathcal{A}$.
...
1. The subsets of $A$ that are in $\mathcal{A}$ are $\bigcup_{k\in K} A_k$ where $K\subseteq J$. Hence $A$ is an atomif and only if$\mu(A_j) > 0$ for one and only one $j\in J$.
...
1. The positive measure $\mu_+$ is called the *positive variation measure* of $\mu$.
  - $\mu_+(A) = \sup\{\mu(B)\;|\; B\in\mathcal{A},\,B\subseteq A\}$
2. The positive measure $\mu_+$ is called the *negative variation measure* of $\mu$.
  - $\mu_-(A) = -\inf\{\mu(B)\;|\; B\in\mathcal{A},\,B\subseteq A\}$
3. The positive measure $|\mu| = \mu_+ + \mu_-$ is called the *total variation measure* of $\mu$.
  - $|\mu| = \sup\left\{\sum_{i=1}^{n\in\mathbb{N}}\mu(A_i)\;|\; A=\bigcup_{i=1}^{n\in\mathbb{N}} A_i\}$
4. $\lVert\mu\rVert = |\mu(X)|$ is the total variation of $\mu$.
  - $\lVert\mu\rVert = \sup\left\{\sum_{i=1}^{n\in\mathbb{N}}\mu(A_i)\;|\; X=\bigcup_{i=1}^{n\in\mathbb{N}} A_i\}$
</MathBox>
...
$$
  \mathrm{supp}(\mu):=\{x\in X \;|\; \mu(U)>0\textrm{ for every open neighbourhood }U\textrm{ of }x \}
$$
...
1. $\mu$ is inner regular if $\mu(A) = \sup\{\mu(C) \;|\; C\textrm{ is compact and }C\subseteq A\}$
2. $\mu$ is outer regular if $\mu(A) = \inf\{\mu(U) \;|\; U\textrm{ is open and }A\subseteq U\}$
...
<MathBox title='Proposition' boxType='proposition'>
Let $(X,\mathcal{A}, \mu)$ be a measure space and let $\mathcal{D} = \{A\in\mathcal{A} \;|\; \mu(A) = 0 \lor \mu(A^c) = 0\}$ be the collection of null and co-null sets. Then $\mathcal{D}$ is a sub $\sigma$-algebra of $\mathcal{A}$.
<details>
<summary>Proof</summary>
...
2. If $\mu(A) = 0$ then $A\equiv B$if and only if$\mu(B) = 0$
  a. If $\mu(A^c) = 0$ then $A\equiv B$if and only if$\mu(B^c) = 0$
...
Note that $\{x\in X \;|\; \mathbf{1}_A (x)\neq \mathbf{1}_B (x) \} = A\Delta B$.
</details>
</MathBox>
...
Let $(X,\mathcal{A},\mu)$ and $(Y,\mathcal{B},\nu)$ be measure spaces. Two measurable functions $f, g: X\to Y$ are equivalent if $\mu\{x\in X \;|\; f(x)\neq g(x) \} = 0$, denoted $f\equiv g$. Note that if $f$ and $g$ are measurable functions, then $(f,g):X\to Y\times Y$ is measurable with respect to the usual product $\sigma$-algebra $\mathcal{B}\otimes\mathcal{B}$. Then $\{ x\in X \;|\; f(x)\neq g(x) \} = \{ x\in X \;|\; (f(x), g(x))\in D \}^c\in\mathcal{A}$ where $D = \{(y, y) \;|; y\in Y \}\in\mathcal{B}\otimes\mathcal{B}$ is the diagonal set.
...
1. Note that $f^{-1}[B]\Delta g^{-1}[B]\subseteq \{x\in X \;|\; f(x)\neq g(x)\}$
2. Note that $\{ x\in X \;|\; h[f(x)] \neq h[g(x)]\}\subseteq \{x\in X\;|\; f(x)\neq g(x)\}$
</details>
</MathBox>
...
$$
  \{x\in X \;|\; (f_1(x), g_1(x)) \neq (f_2(x), g_2(x))\} = \{x\in X\;|\; f_1(x) \neq f_2(x)\}\cup\{x\in X\;|\; g_1(x) \neq g_2(x)\}
$$
...
Let $(X,\mathcal{A},\mu)$ be a measure space, and let $\mathcal{N} = \{A\in\mathcal{A}\;|\; \mu(A) = 0\}$ be the collection of null sets. The measure space is complete if $A\in\mathcal{N}$ and $B\subseteq A$ imply $B\in\mathcal{A}$ (and hence $B\in\mathcal{N}$).
</MathBox>

<MathBox title='Equivalence relation on power sets' boxType='definition'>
Let $(X,\mathcal{A},\mu)$ be a measure space, and let $\mathcal{N} = \{A\in\mathcal{A}\;|\; \mu(A) = 0\}$ be the collection of null sets. For $A, B\subseteq X$, define $A\equiv B$if and only ifthere exists $N\in\mathcal{N}$ such that $A\Delta B \subseteq N$. Then $\equiv$ is an equivalence relation on $\mathcal{P}(S)$ satisfying for $A,B,C\subseteq X$
1. Reflexivity: $A\equiv A$
2. Symmetry: $A\equiv B\implies B\equiv A$
...
<MathBox title='Proposition' boxType='proposition'>
Let $(X,\mathcal{A})$ be a measurable space and consider the collection $\mathcal{A}_0 = \{ A\subseteq X \;|\; A\equiv B\textrm{ for some }B\in\mathcal{A}\}$. Then $\mathcal{A}_0$ is a $\sigma$-algebra of subsets of $X$, and in fact is the $\sigma$-algebra generated by $\mathcal{A}\cup\{A\subseteq X\;|\; A\equiv\emptyset\}$.
<details>
<summary>Proof</summary>
...
Finally, suppose that $\mathcal{B}$ is a $\sigma$-algebra on $X$ and that $\mathcal{A}\cup\{A\subseteq X \;|\; A\equiv\emptyset \}\subseteq\mathcal{B}$. We need to show that $\mathcal{A}_0 \subseteq\mathcal{B}$. Suppose that $A\in\mathcal{A}_0$, then there is a $B\in\mathcal{A}$ such that $A\equiv B$. However, $B\in\mathcal{B}$ and $A\Delta B\in\mathcal{B}$ so $A\cap B = B\setminus(A\Delta B)\in\mathcal{B}$. Moreover, $A\setminus B\in\mathcal{B}$ so $A=(A\cap B)\cup(A\setminus B)\in\mathcal{B}$.
</details>
</MathBox>
...
$$
  \mu*(A) = \inf\left\{ \sum_{i\in I} \mu(A_i) \;|\; A\subseteq\bigcup_{i\in I} A_i \right\}
$$
...
Suppose that $B\in\mathcal{B}$ and that $\mu_1(B) = \mu_2(B)<\infty$. Let $\mathcal{L}_B = \{A\in\mathcal{A}\;|\;\mu_1(A\cap B) = \mu_2(A\cap B)\}$. Then $X\in\mathcal{L}_B$ since $B = (X\cap B)$ and $\mu_1(B) =\mu_2(B)$. If $A\in\mathcal{L}_B$ then $\mu_1(A\cap B)=\mu_2(A\cap B)$ so 
...
Note that a distribution function $f:\mathbb{R}\to\mathbb{R}$ is continuous at $x\in\mathbb{R}$if and only if$\mu\{x\} = 0$. In particular, $\mu$ is continuous measureif and only if$f$ is continuous on $\mathbb{R}$. On the other hand, $f$ is discontinuous at $x\mathbb{R}$if and only if$\mu\{x\}>0$, so that $\mu$ has an atom at $x$. That is, $\mu$ is a discrete measureif and only if$f$ is a step function.

<MathBox title='n-fold Lebesgue measure' boxType='definition'>
For $n\in\mathbb{N}$, let $(\mathbb{R}^n, \mathcal{B}(\mathbb{R}^n))$ be the $n$-dimensional Euclidean measurable space where $\mathcal{B}(\mathbb{R}^n) = \sigma\left\{ \prod_{i=1}^n I_i \;|\; I_i \in\mathcal{I} \right\}$ and $\mathcal{I}$ is a semi-ring of bounded intervals in $\mathbb{R}$. The $n$-fold Lebesgue measure $\lambda_n:\mathcal{B}(\mathbb{R}^n)\to[0,\infty]$ is given by
...
- $\lambda_n(A) = \sup\{\lambda_n(C) \;|\; C\textrm{ is compact and }C\subseteq A \}$ (inner regularity)
- $\lambda_n(A) = \inf\{\lambda_n(U) \;|\; U\textrm{ is open and }A\subseteq U \}$ (outer regularity)
</MathBox>
...
Our first step is to show that $\mathcal{F}$ is closed under the max operator. Let $g_1,g_2\in\mathcal{F}$. For $A\in\mathcal{A}$, let $A_1 = \{x\in A\;|\; g_1(x)\geq g_2(x)\}$ and $A_2 = \{x\in A\;|\; g_1(x)<g_2(x)\}$. Then $A_1,A_2\in\mathcal{A}$ partition $A$ such that
...
Suppose that $\nu$ is a positive measure on $(X,\mathcal{A},\mu)$ with $\nu\ll\mu$, and that $\nu$ has a density function $f = f^+ - f^-$ relative to $\mu$. If $P = \{x\in X\;|\; f(x)\geq 0\}$ then
...
$$
\begin{align*}
  \int_Y g \;\mathrm{d}(h*\mu) &= \sup\left\{ \int_Y \tilde{s} \mathrm{d}\mathrm{d}(h*\mu) \;|\; \tilde{s}: Y \to [0, \infty), \tilde{s} \leq g \right\} \\
  &= \sup\left\{ \int_X \tilde{s} \circ h \mathrm{d}\mu \;|\; \tilde{s}: Y \to [0, \infty), \tilde{s}\circ h \leq g \circ h \right\} \\
  &= \sup\left\{ \int_X s \mathrm{d}\mu \;|\; s: X \to [0, \infty), s \leq g \circ h \right\} \\
  &= \int_X g \circ h \;\mathrm{d}\mu
\end{align*}
$$
</details>
...
$$
  H_\delta^k (A) := \inf\left\{ \sum_{B\in\mathcal{B}} \left[ \mathrm{diam}(B) \right]^k \;|\; \bigcup_{B\in\mathcal{B}} B \supseteq A,\; \mathrm{diam}(B) < \delta \right\}
$$
...
$$
  \mathrm{dim}_H (A) := \inf\left\{ k\in[0, \infty] \;|\; H^k(A) = 0 \right\}
$$
</MathBox>
...
If $\mathrm{Ran}(f) = \{c_i\}_{i\in I}\subset\mathbb{R}$ is a set of distinct values with $A_i = \{x\in X \;|\; f(x) = a_i \}$, then the linear combination is called a canonical representation of $f$.
...
1. $\{I_{n,k} \;|\; k = 1,\dots,n2^n \}\cup\{I_n\}$ is a partition of $[0,\infty)$ 
...
1. $\{A_{n,k} \;|\; k = 1,\dots,n2^n \}\cup\{A_n\}$ is a partition of $X$ 
...
Note that $\{h \;|\; h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq f \}\subseteq\{ h \;|\; h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq g \}$. Therefore

$$
  \int_X f\,\mathrm{d}\mu = \sup\left\{ \int_X h\,\mathrm{d}\mu \;|\; h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq f \right\} \leq \sup\left\{ \int_X h\,\mathrm{d}\mu \;|\; h\in\mathcal{S}^+ \textrm{ and } 0\leq h\leq g \right\} = \int_{X} g\,\mathrm{d}\mu
$$
</details>
</MathBox>
...
Let $(X,\mathcal{A},\mu)$ be a measure space. A measurable function $f:X\to\mathbb{R}$ is integrableif and only if$\int_X |f|\,\mathrm{d}\mu$. As a direct result, if $f$ is integrable and $A\in\mathcal{A}$, then the integral of $f$ over $A$ exists.
...
4. positive definiteness: if $f\overset{a.e.}{\geq} 0$ on $X$ then $\int_X f\,\mathrm{d}\mu$
  a. $\int_X f\,\mathrm{d}\mu = 0$if and only if$f\overset{a.e.}{=}0$ on $X$
...
7. absolute value inequality: $\left| \int_X f\,\mathrm{d}\mu \right|\leq\int_X |f|\,\mathrm{d}\mu$. If $f$ is integrable, the equality holdsif and only if$f\overset{a.e.}{\geq}0$ or $f\overset{a.e.}{\leq}0$ on $X$.
...
Let $A = \{x\in X \;|\; f(x)\geq 0\}$. Then $A\in\mathcal{A}$ and $\mu(A^c) = 0$. By addivitiy of disjoint sets we have
...
*4a*
Note that if $\mu(A) = 0$ then clearly $\int_X f\,\mathrm{d}\mu = \int_A f\,\mathrm{d}\mu + \int_{A^c} f\,\mathrm{d}\mu = 0$. In the opposite case, let $B_n = \{x\in X\;|\; f(x)\geq\frac{1}{n}\}$ for $n\in\mathbb{N}$ and $B = \{x\in X\;|\; f(x)>0\}$. Then $B_n$ is increasing in $n$ and $\bigcup_{n\in\mathbb{N}} B_n = B$. If $\mu(B) > 0$ then $\mu(B_n) > 0$ for some $n\in\mathbb{N}$. However, $f\geq \frac{1}{n}\chi_{B_n}$ on $A$, so by increasing monotonicity
...
**6. Equality property**
Note that $f = g$if and only if$f^+ = g^+$ and $f^- = g^-$. Let $A = \{x\in X \;|\; f^+(x) = g^+(x) \}$. Then $A\in\mathcal{A}$ and $\mu(A^c) = 0$. By the additive property
...
The equality holdsif and only if$f\overset{\textrm{a.e.}}{=}-|f|\leq 0$ or $f\overset{\textrm{a.e.}}{=}|f|\geq 0$ on $X$.
</details>
</MathBox>
...
where $C_x = \{y\in Y\;|\; (x,y)\in C\}$ for $x\in X$ and $C^y = \{x\in X\;|\; (x,y)\in C\}$ for $y\in Y$. 
...
where
- $C_x = \{ y \in Y \;|\; (x, y) \in C \}$ and $D_x = \{ y \in Y \;|\; (x, y) \in D \}$ for $x\in X$
- $C^y = \{ x \in X \;|\; (x, y) \in C \}$ and $D^y = \{ x \in X \;|\; (x, y) \in D \}$ for $y\in Y$
</MathBox>
