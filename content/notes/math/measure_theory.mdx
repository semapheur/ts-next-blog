...
Suppose that $A, B\in\mathcal{A}*$. Then for finite index sets $I, J$ there exist finite, disjoint collections $\Set{A_i}_{i\in I}\subseteq\mathcal{A}$ and $\Set{B_j}_{j\in J}\subseteq\mathcal{A}$ such that $A = \bigcup_{i\in I} A_i$ and $B = \bigcup_{j\in J} B_j$. Thus
...
1. Suppose that $x, y\in\Set{0, 1}^n$ and that $x\neq y$. Without loss of generality, suppose that $x_j = 0$ while $x_j = 1$ for some $j\in\Set{1, 2, \dots, n}$. Then $B_x\subseteq A_j^c$ and $B_y\subseteq A_j$, so $B_x$ and $B_y$ are disjoint. Suppose that $s\in S$ and construct $x\in\Set{0, 1}^n$ by $x_i = 1$ if $s\in A_i$ and $x_i = 0$ if $s\notin A_i$ for each $i\in\Set{1,2,\dots,n}$. Then by definition $s\in B_x$, showing that $\mathcal{B}$ partitions $S$.
...
Let $\mathcal{S} = \Set{ \bigcup_{j\in J} A_j | J\subseteq I }$. Note that $X = \bigcup_{i\in I} A_i$ so $X\in\mathcal{S}$. Suppose, that $B\in\mathcal{S}$. Then $B = \bigcup_{j\in J} A_j$ for some $J\subseteq I$, and $B^c = \bigcup_{j\in J^c} A_j$ so $B^c\in\mathcal{S}$. Suppose that $B_k\in\mathcal{S}$ for $k\in K$ where $K$ is a countable index set. Then for each $k\in K$ there exists $J_k\subseteq I$ such that $B_k = \bigcup_{j\in J_k} A_j$. However, then $\bigcup_{k\in K} B_k = \bigcup_{k\in K}\bigcup_{j\in J_k} A_j = \bigcup_{j\in J} A_j \in\mathcal{S}$ where $J = \bigcup_{k\in K} J_k$. Hence $\mathcal{S}$ is a $\sigma$-algebra of $X$.
...
Suppose that $(X, \mathcal{A})$ is a measurable space, and consider a subset $R\subseteq S$. Let $\mathcal{R} := \Set{ A\cap R | A\in\mathcal{A} }$. Then
...
If $f:X\to Y$ is measurable, then the condition in the proposition holds trivially since $\mathcal{B}\subseteq\mathcal{A}_Y$. Conversely, suppose that $f^{-1}[B]\in\mathcal{A}_X$ for every $B\in\mathcal{B}$, and let $\mathcal{U} = \Set{ A\in\mathcal{A}_Y | f^{-1}[A]\in\mathcal{A}_X }$. Then $Y \in U$ since $f^{-1}[Y] = X \in\mathcal{A}_X$. If $A\in \mathcal{U}$ then $f^{-1}[A^c] = \left( f^{-1}[A] \right)^c \in\mathcal{A}_X$, so $A^c\in \mathcal{U}$. If $A_i \in \mathcal{U}$ for $i$ in a countable index set $I$, then $f^{-1}\left[\bigcup_{i\in I} A_i \right] = \bigcup_{i\in I} f^{-1} [A_i] \in\mathcal{A}_X$, and hence $\bigcup_{i\in I} A_i \in \mathcal{U}$. Thus $\mathcal{U}$ is a $\sigma$-algebra of $\mathcal{A}_Y$. However, $\mathcal{B}\subseteq\mathcal{U}$ by assumption, so $\mathcal{A}_Y = \sigma(\mathcal{B})\subseteq\mathcal{U}$. Since $\mathcal{U}\subseteq\mathcal{A}_Y$ by definition, then $\mathcal{U} = \mathcal{A}_Y$ and hence $f$ is measurable.
</details>
</MathBox>
...
Suppose that $X$ is a set and $(Y, \mathcal{A}_Y)$ is a measurable space. Consider a function $f:X\to Y$ and define $\sigma(f) = \Set{ f^{-1}[A] | A\in\mathcal{A}_Y }$. Then
...
<MathBox title='Proposition' boxType='proposition'>
Suppose that $(X_1, \mathcal{A}_1)$ and $(X_2, \mathcal{A}_2)$ are measurable spaces. Then $\mathcal{A}_1\otimes\mathcal{A}_2 = \sigma\Set{p_1, p_2 }$, where $p_1: X_1\times X_2\to X_1$ and $p_2: X_1\times X_2\to X_2$ are projections given by
$$
  p_1(x_1, x_2) = x_1 \quad p_2(x_1, x_2) = x_2, \quad (x_1, x_2)\in X_1 \times X_2 
$$

In the general case, suppose that $(X_i, \mathcal{A}_i)$ is a measurable space for each $I$ in an index set $I$. Then the product $\sigma$-algebra on $\prod_{i\in I} X_i$ is $\mathcal{A} = \sigma\Set{ p_i }_{i\in I}$.
<details>
<summary>Proof</summary>
...
In the general case, suppose that $(X_i, \mathcal{A}_i)$ is a measurable space for each $i$ in an index set $I$ with at least two elements. For $j\in I$ and $u\in X_j$, define the function $j_u:\prod_{i\in I\setminus\Set{j}}\to\prod_{i\in I} X_i$ by $j_u(x) = y$ where $y_i = x_i$ for $i\neq j$ and $y_j = u$. Then $j_u$ is measurable with respect to the product $\sigma$-algebras. If $A\subseteq\prod_{i\in I} X_i$, then $j_u^{-1}[A]$ is the cross section of $A$ in coordinate $j$ at $u$.
<details>
<summary>Proof</summary>
...
Let $\mathcal{L}$ denote the $\lambda$-system generated by $\mathcal{A}$. Then $\mathcal{A}\subseteq\mathcal{A}\subseteq\mathcal{B}$. For $A\in\mathcal{L}$, let $\mathcal{L}_A = \Set{ B\subseteq X | B\cap A\in\mathcal{L} }$. We will show that $\mathcal{L}_A$ is a $\lambda$-system. Note that $X\cap A = A\in\mathcal{L}$ and thus $X\in\mathcal{L}_A$. Next, suppose that $B_1, B_2 \in\mathcal{L}_A$ and that $B_1\subseteq B_2$. Then $B_1 \cap A\in\mathcal{L}$ and $B_2\cap A\in\mathcal{L}$ and $B_1\cap A\subseteq B_2\cap A$. Thus $(B_2\setminus B_1)\cap A = (B_2\cap A)\setminus (B_1\cap A)\in\mathcal{L}$, so that $B_2\setminus B_1 \in\mathcal{L}_A$. Finally, suppose that $\Set{B_i}_{i\in I}$ is a countable, disjoint collection of sets in $\mathcal{L}_A$. Then $B_i \cup A\in\mathcal{L}$ for each $i\in I$, and $\Set{B_i\cap A}_{i\in I}$ is also a disjoint collection. Thus, $\bigcup_{i\in I}(B_i\cap A) = \left(\bigcup_{i\in I} B_i \right) \cap A\in\mathcal{L}$, resulting in $\bigcup_{i\in I} B_i \in\mathcal{L}_A$.
...
Assume that $I = \N$ and let $B_1 = A_1$ and $B_i = A_i \setminus \left(\bigcup_{j=1}^{i-1} A_j \right)$ for $i\in\N\setminus\Set{1}$. Then $\Set{B_i}_{i\in I}$ is a disjoint collection of sets in $\mathcal{A}$ with the same union as $\Set{A_i}_{i\in I}$. Since $B_i\subseteq A_i$ for each $i$ it follows that $\mu(B_i)\leq\mu(A_i)$. Hence
...
Suppose that the formula holds for a given $n\in\N$ and consider the case $n+1$. Then
...
Suppose that $\nu$ is a finite measure; the proof when $\mu$ is finite is analogous. First, $\delta(\emptyset) = \mu(\emptyset)-\nu(\emptyset)$. Next, suppose that $\Set{A_i}_{i\in I}$ is a countable, disjoint collection of sets in $\mathcal{A}$ and let $A = \bigcup_{i\in I} A_i$. Then
...
First, we prove that $\mu(A) = \sum_{j\in J}\mu(A_j)$ defines a positive measure on $(X,\mathcal{A}$. Recall that every $A\in\mathcal{A}$ has a unique representation of the form $\bigcup_{j\in J} A_j$ where $J\subseteq I$. In particular $J=\emptyset$ gives $A = \emptyset$. The sum over an empty index set is $0$ so $\mu(\emptyset)=\emptyset$. Suppose that $\Set{B_k }_{k\in K}\mathcal{A}$ is a countable, disjoint collection. Then there exists a disjoint collection $\Set{J_k \substeq I }_{k\in K}$ such that $B_k = \bigcup_{j\in J_k} A_j$ giving
...
Suppose that $B\subseteq P$ and $\mu(B)<0$, then $B\subseteq A_n$ and by definition $a_n\leq\mu(B)$ for every $n\in\N$. If follows that $b_n\leq\frac{1}{2}\mu(B)$ or $b_n = -1$ for every $n\in\N$. Hence $\sum_{n\in\N} b_n = -\infty$ and therefore $\mu(P) = \infty$, which is a contradiction since $\mu(A)<\infty$. Hence we must have $\mu(B)\geq 0$ and thus $P$ is positive for $\mu$. 
</details>
</MathBox>
...
Let $P_{n+1} = P_n\cup A$. Then $P_{n+1}\in\mathcal{A}$ is positive for $\mu$. If the recursion terminates after finite number of steps, then $P$ is well defined and $(P,P^c)$ is a Hahn decomposition. Otherwise, we have a disjoint sequence of sets $(A_n)_{n\in\N}$ in $\mathcal{A}$, each positive for $\mu$. Let $P = \bigcup_{n\in\N} A_n$, then $P\in\mathcal{A}$ is positive for $\mu$ by closure of positive sets. Suppose that $A\subseteq P^c$. If $\mu(A) > 0$ then $\mu(A)\leq a_n$ for every $n\in\N$. Hence $b_n \geq \frac{1}{2}\mu(A)$ or $b_n = 1$ for every $n\in\N$. However, it follows that
...
Suppose that $\mu:\mathcal{A}\to\R*$ is a measure on $(X,\mathcal{A},\mu)$. Then $\mu$ can be decomposed uniquely in the form $\mu = \mu_+ - \mu_-$ where $\mu_+$ and $\mu_-$ are positive measures, of which at least one is finite, and with the property that if $(P,P^c)$ is any Hahn decomposition of $X$, then $P^c$ is a null set of $\mu_+$ and $P$ is a null set of $\mu_-$. The pair $(\mu_+,\mu_-)$ is the Jordan composition of $\mu$.
...
Let $\mathcal{M}$ be the collection of finite measures on $(X,\mathcal{A})$. Suppose that $\mu_n\in\mathcal{M}$ for $n\in\N$ and $\mu\in\mathcal{M}$. We say that $\mu_n\xrightarrow{n\to\infty}\mu$ in total variation if $\|\mu_n - \mu\|\xrightarrow{n\to\infty} 0$.
</MathBox>
...
Let $(X,\mathcal{A}, \mu)$ be a measure space, and suppose that $\Set{A_i}_{i\in I}$ is a countable collection of sets in $\mathcal{A}$. Then for $i\in I$
...
Finally, suppose that $\mathcal{B}$ is a $\sigma$-algebra on $X$ and that $\mathcal{A}\cup\Set{A\subseteq X | A\equiv\emptyset }\subseteq\mathcal{B}$. We need to show that $\mathcal{A}_0 \subseteq\mathcal{B}$. Suppose that $A\in\mathcal{A}_0$, then there is a $B\in\mathcal{A}$ such that $A\equiv B$. However, $B\in\mathcal{B}$ and $A\Delta B\in\mathcal{B}$ so $A\cap B = B\setminus(A\Delta B)\in\mathcal{B}$. Moreover, $A\setminus B\in\mathcal{B}$ so $A=(A\cap B)\cup(A\setminus B)\in\mathcal{B}$.
</details>
</MathBox>
...
Suppose that $(A_n)_{n\in\N}$ is a sequence of pairwise disjoint sets in $\mathcal{A}_0$ so that there exists a sequence $(B_n)_{n\in\N}$ of sets in $\mathcal{A}$ such that $A_n\equiv B_n$ for each $n\in\N$. For a fixed $i\in I$
...
Suppose that $B\in\mathcal{B}$ and that $\mu_1(B) = \mu_2(B)<\infty$. Let $\mathcal{L}_B = \Set{A\in\mathcal{A}|\mu_1(A\cap B) = \mu_2(A\cap B)}$. Then $X\in\mathcal{L}_B$ since $B = (X\cap B)$ and $\mu_1(B) =\mu_2(B)$. If $A\in\mathcal{L}_B$ then $\mu_1(A\cap B)=\mu_2(A\cap B)$ so 

$$
  \mu_1(A^c\cap B) = \mu_1(B) - \mu_1(A\cap B) = \mu_2(B) - \mu_2(A\cap B) = \mu_2(A^c\cap B) 
$$

and thus $A^c\in\mathcal{L}_B$. Finally, suppose that $\Set{A_j}_{j\in J}\subseteq\mathcal{L}_B$ is a countable, disjoint collection. Then $\mu_1(A_j\cap B) = \mu_2(A_j\cap B)$ for each $j\in J$ such that
...
Let $\mathcal{I}$ be a semi-algebra (collection of bounded intervals) in $\R$ that generates a Borel $\sigma$-algebra on $\R$, i.e. $\sigma(\mathcal{I}) = \mathcal{B}(\R)$. Suppose that $\mu:\mathcal{I}\to[0,\infty]$ is a pre-measure on $\mathcal{I}$ given by $\mu_f(I) := f(b) - f(a)$ for a distribution function $f:\R\to\R$ and a left-open interval $I = (a, b]\in\mathcal{I}$. By Carathéodory's theorem there is a unique extension $\mu'_f:\mathcal{B}(\R)\to[0,\infty]$ called the Lebesgue-Stieltjes measure.

<details>
<summary>Proof</summary>

Suppose that $f$ is a distribution function on $\R$, and suppose that $\mathcal{I}$ is a collection of subsets of $\R$ consisting of intervals of the form, for $a, b\in\R$ and $a\leq b$
- $(a,b]$
- $(-\infty, a]$
- $(a, \infty)$
...
Suppose that $\mu$ is a positive measure on $(\R,\mathcal{B}(\R))$ with the property that $\mu(A)<\infty$ if $A$ is bounded. Then there exists a distribution function that generates $\mu$.
...
Suppose that $f:\R\to\R$ is a distribution function and $\mu$ is the positive measure on $(\R,\mathcal{B}(\R))$ associated with $f$. The following properties hold for $a, b\in\R$ with $a< b$
1. $\mu[a, b] = f(b) - f(a^-)$
2. $\mu\Set{a} = f(a) - f(a^-)$
3. $\mu(a,b) = f(b^-) - f(a)$
4. $\mu[a,b) = f(b^-) - f(a^-)$
...
Suppose that $(x_n)_{n\in\N}$ is a sequence of distinct points in $\R$
...
Suppose that $A\in\mathcal{B}(\R^n)$. The Lebesgue measure $\lambda_n$ has the following properties
...
Suppose that $\mu$ is $\sigma$-finite measure on $(X,\mathcal{A})$, and that $f:X\to\R$ is a measurable function whose integral with respect to $\mu$ exist. If $A\in\mathcal{A}$, the function $\nu$ defined by
...
Suppose that $f,g:X\to\R$ are measurable functions whose integrals relative to $\mu$ exist. If $g \overset{\mu\textrm{-a.e.}}{=} f$ on $X$ then $\int_A f\;\mathrm{d}\mu = \int_A g\;\mathrm{d}\mu$ for every $A\in\mathcal{A}$. Therefore, if $f$ is a density function for $\nu$ relative to $\mu$, then so is $g$. Conversely, if $\int_A f\;\mathrm{d}\mu = \int_A g\;\mathrm{d}\mu$ for every $A\in\mathcal{A}$, it follows that $f \overset{\mu\textrm{-a.e.}}{=} g$ since $\mu$ is $\sigma$-finite.
...
Suppose that $g = \sum_{i\in I}a_i\chi_{A_i}$ is a non-negative simple function where $I$ is a finite index set, $a_i\in[0,\infty)$ and for $i\in I$ and $\Set{A_i}_{i\in I}$ is a disjoint collection of sets in $\mathcal{A}$. Then $\int_X g\;\mathrm{d}\mu = \sum_{i\in I} a_i\nu(A_i)$. However $\nu(A_i) = \int_{A_i} f\;\mathrm{d}\mu = \int_X \chi_{A_i}f\;\mathrm{d}\mu$ for each $i\in I$ so
...
Suppose that $g:X\to[0,\infty)$ is measurable. There exists an increasing sequence of non-negative simple functions $(g_n)_{n\in\N}$ on $X$ with $g_n\xrightarrow{n\to\infty}g$. Since $f$ is non-negative, $g_n f$ is increasing in $n$ on $X$ and $g_n f\xrightarrow{n\to\infty} gf$. By the last result $\int_X g_n\;\mathrm{d}\nu = \int_X g_n f\;\mathrm{d}\mu$ for each $n\in\N$. By the monotone convergence theorem $\int_X g_n\;\mathrm{d}\nu\xrightarrow{n\to\infty}\int_X g\;\mathrm{d}\nu$ and $\int_X g_n f\;\mathrm{d}\mu\xrightarrow{n\to\infty}\int_X gf\;\mathrm{d}\mu$. Hence $\int_X g\;\mathrm{d}\nu = \int_X gf\;\mathrm{d}\mu$.

**General functions**
Suppose that $g:X\to\R$ is a measurable functions whose integral exists relative to $\nu$. Note that $g = g^+ - g^-$. By the previous result, 
...
Suppose that $f,g:[0,\infty)\to\R$ are locally bounded functions. Then the following are locally bounded functions
...
Suppose that $G$ is a distribution function on $[0,\infty)$ and $f:[0,\infty)\to\R$ is locally bounded. Then $g: [0,\infty)\to\R$ defined by $g(t) = \int_0^t f(s)\;\mathrm{d}G(s)$ is also locally bounded.
...
Suppose that $f,g:[0,\infty)\to\R$ are locally bounded and that $H$ is a distribution function on $[0,\infty)$. The convolution of $f$ and $g$ with respect to $H$ is the function $f*g: [0,\infty)\to\R$ defined by
...
Suppose that $f:[0,\infty)\to\R$ is locally bounded and that $G$ is a distribution function on $[0,\infty)$. The convolution of $f$ with $G$ is the function $f*G: [0,\infty)\to\R$ defined by
...
1. Suppose that $f:[0,\infty)\to\R$ is locally bounded and that $G$ and $H$ are distribution functions on $[0,\infty)$. Then
...
**Linearity**
Suppose that $c\in\R$. Then
2. $f * (G + H) = (f * G) + (f * H)$
3. $f*(cG) = c(f*G)$

Suppose that $g:[0,\infty)\to\R$ is locally bounded. Then
4. $(f+g)*(H) = (f*H) + (g*H)$
5. $(cf)*H = c(f*H)$
...
Suppose that $G$ is a distribution on $[0,\infty)$. The Laplace transform of $f:[0,\infty)\to\R$ with respect to $G$ is the function given below, defined for all $s\in(0,\infty)$ for which the integral exists in $\R$
...
In the general case, suppose that $(X_i, \mathcal{A}_i, \mu_i)$ is a $\sigma$-finite measure space for $\Set{i}_{i=1}^{n\in\N}$. Let $X = \prod_{i=1}^n S_i$ and $\mathcal{A} = \otimes_{i=1}^n \mathcal{A}_i$. Then there exists a unique $\sigma$-finite measure on $(X,\mathcal{A})$ satisfying
...
Suppose that $\dim(A) < \beta < \infty$. Then there exists a cover $(A_i)_{i\in\N_+}$ such that $A \subset \bigcup_{i} A_i$ and $\sum_i |A_i|^\beta < \varepsilon$. Then $(f(A_i))_{i\in\N_+}$ is a covering of $f(A)$ and $|f(A_i)| \leq C|A_i|^\alpha$ where $C$ is the Hölder constant. Thus
...
Suppose that $\mu$ is a measure on a metric space $E$. If $0 \mu(E) < \infty$, then $\mu$ is a mass distribution on $E$.
</MathBox>

<MathBox title='Mass distribution principle' boxType='theorem'>
Suppose that $E$ is a metric space and $\alpha\geq 0$. If there is a mass distribution $\mu$ on $E$ and constants $C > 0$ and $\delta > 0$ such that
...
Suppose that $\mathcal{U} = \Set{U_n}_{n\in\N_+}$ is a cover of $E$ with $|U_n| < \delta$. Let $V_n$ be the closure of $U_n$ and note that $|U_n| = |V_n|$. We
...
Suppose that $\mu$ is a mass distribution on a metric space $E$ and $\alpha\geq 0$. The $\alpha$-potential of a point $x\in E$ with respect to $\mu$ is defined as
...
Let $\mu$ be a mass distribution on a metric space $E$ and suppose that $\alpha\geq 0$. For every $\varepsilon > 0$
...
Suppose that $(A_n)_{n\in\N_+}$ is a pairwise disjoint cover of $E$ by sets of diameter $\varepsilon > 0$. Then
...
1. Clearly $\mu K(A)\geq 0$ for $A\in\mathcal{T}$. Suppose that $\Set{A_j}_{j\in J}$ is a countable collection of disjoint sets in $\mathcal{T}$ and $A = \bigcup_{j\in J} A_j$, then
...
<MathBox title='Proposition' boxType='proposition'>
Suppose that $K$ is a kernel from $(S,\mathcal{S})$ to $(T, \mathcal{T})$ and that $f:T\to\R$ is bounded, i.e. $f \in\mathcal{B}(T,\R)$
...
The measurability of $x\mapsto (KL)(x, A)$ for $A\in\mathcal{T}$ follows from basic properties of the integral. For the second property, fix $x\in R$. Clearly $KL(x, A)\geq 0$ for $A\in\mathcal{T}$. Suppose that $\Set{A_j}_{j\in J}$ is a countable collection of disjoint sets in $\mathcal{T}$ and $A = \bigcup_{j\in J} A_j$, then
...
Suppose that $K$ is a kernel from $(S,\mathcal{S})$ to $(T,\mathcal{T})$, $\mu$ is a positive measure on $(S,\mathcal{S})$, $c\in[0,\infty)$ and $f:T\to\R$ is measurable. Then the following associative properties hold
...
Suppose that $K$ is a kernel from $(R,\mathcal{R})$ to $(S,\mathcal{S})$, $L$ is a kernel from $(S,\mathcal{S})$ to $(T,\mathcal{T})$ and $M$ is a kernel from $(T,\mathcal{T})$ to $(U,\mathcal{U})$. Suppose also that $\mu$ is a positive measure on $(R,\mathcal{R})$, $c\in[0,\infty)$ and $f:T\to\R$ is measurable. Then the following associative properties hold
...
Suppose that $K$ and $L$ are kernels from $(R,\mathcal{R})$ to $(S,\mathcal{S})$ and that $M$ and $N$ are kernels from $(S,\mathcal{S})$ to $(T,\mathcal{T})$. Suppose also that $\mu$ is a positive measure on $(R, \mathcal{R})$, and $f:S\to\R$ is measurable. Then the following distributive properties hold
...
Let $(X, \mathcal{A})$ be a measurable space and suppose that $\Set{A_i}_{i\in I}\subseteq\mathcal{A}$ is a finite collection of measurable sets partitioning $X$. A simple function $f:X\to\R$ is a finite linear combination of indicator functions on $\Set{A_i}_{i\in I}$
...
It can be shown that the canonical representation of a simple function is unique. First, suppose that $f:X\to\R$ is simple. By definition the range of $f$ is a finite set of distinct real numbers of the form $\operatorname{ran}(f) = \Set{a_i}_{i\in I}\subset\R$. It follows that $I$ is a finite index set and that $\Set{A_i = f^{-1}[a_i]}_{i\in I}\subseteq\mathcal{A}$ is a collection of measurable sets partitioning $X$. Thus, $f$ can be represented as $f = \sum_{i\in I}a_i \chi_{A_i}$. Conversely, suppose that $f$ has a representation of this form. Then $\operatorname{ran}(f) = \Set{a_i}_{i\in I}$ and $A_i = f^{-1}[a_i]$, so the representation is unique.
</details>
</MathBox>
...
Let $(X, \mathcal{A})$ and $(Y, \mathcal{B})$ be measurable spaces and suppose that $f:X\to Y$ is measurable. If $g:Y\to\R$ is a simple function with representation $g = \sum_{i\in I}b_i\chi_{B_i}$ then $g\circ f$ is a simple function on $X$ with representation $g\circ f=\sum_{i\in I}b_i\chi_{f^{-1}[B_i]}$.
...
Suppose that $f$ is a non-negative simple function on a measure space $(X,\mathcal{A},\mu)$ and that $(A_n)_{n\in \N}$ is an increasing sequence of sets in $\mathcal{A}$ with $A = \bigcup_{n=1}^\infty A_n$. Then
...
Let $(X, \mathcal{A})$ be a measurable space and suppose that $f:X\to[0,\infty)$ is measurable. Then there exists an increasing sequence $(f_n)_{n\in\N}$ of nonnegative simple functions on $X$ with $f_n \xrightarrow{n\to\infty} f$.
...
Let $(X, \mathcal{A},\mu)$ be a measure space and suppose that $f_n:X\to[0,\infty)$ is measurable for each $n\in\N$ and that the sequence $(f_n)_{n\in\N}$ is increasing. Then
...
Suppose that $f:X\to\R$ is measurable. Then $\int_A f\;\mathrm{d}\mu = \int_A f^+\;\mathrm{d}\mu - \int_A f^-\;\mathrm{d}\mu$. Both integrals on the right are $0$ by $(2)$.
...
Suppose that $f:X\to\R$ is measurable and integrable on $(X,\mathcal{A},\mu)$, and that $A,B\in\mathcal{A}$ are disjoint. Then
...
Suppose that $u:X\to\R$ is measurable and integrable on $(X,\mathcal{A},\mu)$. If $f:Y\to\R$ is measurable on $(Y,\mathcal{B},\nu)$ then
...
Finally, suppose that $f:Y\to\R$ is measurable, so that $f\circ u:X\to\R$ is also measurable. Note that $(f\circ u)^+ = f^+\circ u$ and $(f\circ u)^- = f^-\circ u$. By the last result
...
First, suppose that $X$ is finite. In this case, every function $f:X\to\R$ is simple and has the representation $f = \sum_{x\in X} f(x)\chi_x$. The result follows from the definition of Lebesgue integration of simple functions.

Next, suppose that $X$ is countable infinite and $f:X\to[0,\infty)$. Let $(A_n)_{n\in\N}$ be an increasing sequence of finite subsets of $X$ with $\bigcup_{n\in\N} A_n = X$. Define $f_n = \sum_{x\in A_n} f(x)\chi_x$. Then $(f_n)_{n\in\N}$ is an increasing sequence of simple functions with $f_n\xrightarrow{n\to\infty} f$. Thus
...
Suppose that $f_n:X\to\R$ is measurable and integrable on $(X,\mathcal{A},\mu)$ for each $n\in\N$. If $(f_n)_{n\in\N}$ is increasing $X$ and $\int_X f_1\;\mathrm{d}\mu > -\infty$, or $(f_n)_{n\in\N}$ is decreasing on $X$ and $\int_X f_1\;\mathrm{d}\mu<\infty$ then
...
Suppose that $f_n$ is increasing in $n\in\N$. If $\int_X f_1\;\mathrm{d}\mu = \infty$ then by increasing monotonicity, $\int_X f_n\;\mathrm{d}\mu = \infty$ for all $n\in\N$ and $\int_X f\;\mathrm{d}\mu = \infty$. In this case, the monotone converge theorem trivially holds.
...
Suppose that $f_n : X \to \R$ be measurable on $(X,\mathcal{A},\mu)$ for all $n\in\N$, and that $\lim_{n\to\infty} f_n$ exists on $X$. If there is a $\mu$-integrable $g:X\to[0,\infty)$ with $|f_n| \leq g$ for $n\in\N$, then
...
<MathBox title='Corollary' boxType='corollary'>
Let $f_n : X \to \R$ be measurable on $(X,\mathcal{A},\mu)$ for all $n\in\N$, and suppose that $\sum_{n\in\N} |f_n|$ is integrable. Then
...
Let $f_n : X \to \R$ be measurable on $(X,\mathcal{A},\mu)$ for all $n\in\N$, and suppose that there exists $A\in\mathcal{A}$ such that
- $\mu(A)<\infty$
...
Suppose that $|f_n|$ is bounded in $n\in\N$ on $A$ by $c\in(0,\infty)$. The constant $c$ is integrable on $A$ since $\int_A c\;\mathrm{d}\mu = c\mu(A)<\infty$ and $|f_n|\leq c$ on $A$. Therefore, the result follows from the dominated convergence theorem.
</details>
</MathBox>
...
**Non-negative functions**
4. Suppose that $f:X\times Y\to[0,\infty)$ is measurable. Then there exists an increasing sequence of non-negative simple functions $(g_n)_{n\in\N}$ on $X\times Y$ such that $g_n\xrightarrow{n\to\infty} f$. By the monotone convergence theorem, $\int_{X\times Y} g_n\;\mathrm{d}(\mu\otimes\nu)\xrightarrow{n\to\infty}\int_{X\times Y} f\;\mathrm{d}(\mu\otimes\nu)$. For a fixed $x\in X$, the function $y\mapsto g_n(x, y)$ is increasing in $n$ on $Y$ and converges to $f(x, y)$ in the limit $n\to\infty$. Applying the monotone convergence theorem gives
...
**General functions**
5. Suppose that $f:X\times Y\to\R$ is measurable. By $(4)$, the double integral and the iterated integral agree with integrands $f^+$ and $f^-$. Assuming that at least one of these is finite, then by the additive property, the integrals agree with integrand $f = f^+ - f^-$.
</details>
</MathBox>

<MathBox title='Corollary' boxType='corollary'>
Suppose that $g:X\to\R$ and $h:Y\to\R$ are measurable on $(X\times Y,\mathcal{A}\otimes\mathcal{B},\mu\otimes\nu)$, and are either non-negative or integrable with respect to $\mu$ and $\nu$, respectively. Then
