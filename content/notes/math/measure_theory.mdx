...
If $\mathrm{ran}(f) = \Set{x\in X | f(x) = a_i }$, then the linear combination is called a canonical representation of $f$.
...
It can be shown that the canonical representation of a simple function is unique. First, suppose that $f:X\to\R$ is simple. By definition the range of $f$ is a finite set of distinct real numbers of the form $\mathrm{ran}(f) = \Set{a_i}_{i\in I}\subset\R$. It follows that $I$ is a finite index set and that $\Set{A_i = f^{-1}[a_i]}_{i\in I}\subseteq\mathcal{A}$ is a collection of measurable sets partitioning $X$. Thus, $f$ can be represented as $f = \sum_{i\in I}a_i \chi_{A_i}$. Conversely, suppose that $f$ has a representation of this form. Then $\mathrm{ran}(f) = \Set{a_i}_{i\in I}$ and $A_i = f^{-1}[a_i]$, so the representation is unique.
</details>
</MathBox>
...
Note that $g\circ f:X\to\R$ and $\mathrm{ran}(g\circ f)\subseteq\mathrm{ran}(g)$ so $g\circ f$ has finite range. Since $f$ is measurable, it follows that $\Set{f^{-1}[B_i]}_{i\in I}$ is a measurable partition of $X$. Finally, if $x\in f^{-1}[B_i]$ for $i\in I$, then $f(x)\in B_i$ such that $g[f(x)] = b_i$.
</details>
</MathBox>
