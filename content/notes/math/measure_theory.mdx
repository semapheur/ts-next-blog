...
- closed under finite unions: $\{A_i\}_{i=1}^{n\in\N}\subseteq\mathcal{F}\implies\bigcup_{i=1}^{n\in\N} A_i \in\mathcal{F}$
  - equivalently closed under finite intersections: $\{A_i\}_{i=1}^{n\in\N}\subseteq\mathcal{F}\bigcap_{i=1}^{n\in\N} A_i \in\mathcal{F}$ 
...
- differences are partitioned by finite, disjoint unions: $A,B\in\mathcal{S}\implies A\setminus B = \bigcup_{i=1}^{n\in\N} C_i$ for some finite, disjoint collection $\{C_i\}_{i=1}^{n\in\N}\subseteq\mathcal{S}$

The semi-ring $\mathcal{S}$ is a semi-algebra of sets if complements are partitioned by finite, disjoint unions, i.e. $A\in\mathcal{S}\implies A^c = X\setminus A = \bigcup_{i=1}^{n\in\N} C_i$ for some finite, disjoint collection $\{C_i\}_{i=1}^{n\in\N}$.
</MathBox>
...
- closed under countable unions: $\{A_i\}_{i\in I\subseteq\N}\subseteq\mathcal{A} \implies\bigcup_{i\in I} A_i \in\mathcal{A}$
  - equivalently closed under countable intersections: $\{A_i\}_{i\in I\subseteq\N}\subseteq\mathcal{A} \implies\bigcap_{i\in I} A_i \in\mathcal{A}$
...
Since $X$ is infinite, it contains a countably infinite subset $\{x_n\}_{n\in\N}$. Let $A_n = \{x_{2n}\}$ for some $n\in\N$. Then $A_n$ is finite and $A_n\in\mathcal{F}$ for each $n\in\N$. Let $E = \bigcup_{n\in\N} A_n = \{x_{2n}\}_{n\in\N}$. Then $E$ is infinite by construction, but so is $\{x_{2n+1}\}_{n\in\N} \subseteq E^c$. Hence $E\notin\mathcal{F}$, showing that $\mathcal{F}$ is not a $\sigma$-algebra.
</details>
</MathBox>
...
1. $\mathcal{I}_1 = \{I\subseteq\R \mid I\textrm{ is an interval}\}$
2. $\mathcal{I}_2 = \{(a, b]\mid a,b\in\R,\; a<b\}$
3. $\mathcal{I}_3 = \{(-\infty, b]\mid b\in\R\}$
<details>
<summary>Proof</summary>
...
$$
  \mathcal{D}_n = \left\{\left(\frac{j}{2^n},\frac{j+1}{2^n}\right) \mid j\in\Z \right\}\quad n\in\N
$$
...
Let $(X, \mathcal{A})$ be a measurable space. A measure on $\mathcal{A}$ is a function $\mu:\mathcal{A}\to\R* = \R\cup\{-\infty,\infty\}$ satisfying
- $\mu(\emptyset) = 0$
- countable additivity ($\sigma$-additivity): If $\{A_i\}_{i\in I\subseteq\N}$ is a countable, pairwise disjoint collection of sets in $\mathcal{A}$ then 
...
Assume that $I = \N$ and let $B_1 = A_1$ and $B_i = A_i \setminus \left(\bigcup_{j=1}^{i-1} A_j \right)$ for $i\in\N\setminus\{1\}$. Then $\{B_i\}_{i\in I\}$ is a disjoint collection of sets in $\mathcal{A}$ with the same union as $\{A_i\}_{i\in I\}$. Since $B_i\subseteq A_i$ for each $i$ it follows that $\mu(B_i)\leq\mu(A_i)$. Hence
...
2. Let $C_1 = A_1$ and $C_n = A_n\setminus \bigcup_{i=1}^{n-1} A_i$ for $n\in\N\setminus\{1\}$. Then $C_n\in\mathcal{A}$ for each $n\in\N$ and $(C_n)_{n\in\N}$ is disjoint. Moreover, $C_n\subseteq A_n$ so $\mu(C_n)\leq\mu(A_n)<\infty$ and $\bigcup_{n\in\N} C_n = \bigcup_{n\in\N} A_n = X$. 
</details>
</MathBox>
...
Clearly $c\mu:\mathcal{A}\to\R*$ and $(c\mu)(\emptyset) = c\mu(\emptyset) = 0$. If $\{A_i\}_{i\in I}\subseteq\mathcal{A}$ is countable disjoint collection then
...
Clearly $\mu:\mathcal{A}\to\R*$ and $\mu(\emptyset) = \sum_{i\in I}\mu_i(\emptyset) = 0$. If $\{A_j\}_{j\in J}\subseteq\mathcal{A}$ is a countable disjoint collection then
...
2. Suppose that $\mu_i$ is $\sigma$-finite for each $i\in I$. Then for each $i\in I$, there exists a collection $\mathcal{A}_i = \{A_{ij}\}_{j\in\N}\subseteq\mathcal{A}$ such that $\bigcup_{j\in\N} A_i,j = X$ and $\mu_i(A_{i,j})\lessgtr\pm\infty$ for each $j\in\N$. Let $B_j = \bigcap_{i\in I} A_{i, j}$ so that $B_j\in\mathcal{A}$ for each $j\in\N$ and
...
3. The positive measure $|\mu| = \mu_+ + \mu_-$ is called the *total variation measure* of $\mu$.
  - $|\mu| = \sup\left\{\sum_{i=1}^{n\in\N}\mu(A_i)\mid A=\bigcup_{i=1}^{n\in\N} A_i\right\}$
4. $\lVert\mu\rVert = |\mu(X)|$ is the total variation of $\mu$.
  - $\lVert\mu\rVert = \sup\left\{\sum_{i=1}^{n\in\N}\mu(A_i)\mid X=\bigcup_{i=1}^{n\in\N} A_i\right\}$
</MathBox>
...
Suppose that $\mu$ and $\nu$ are measures on $(X,\mathcal{A})$ and that $a,b\in\R\setminus\{0\}$. Then
1. $\nu\ll\mu \iff a\nu\ll b\mu$
2: $\nu\perp\mu \iff a\nu\perp b\mu$
...
- $\mu(\emptyset) = 0$
- countable additivity: If $\{A_i\}_{i\in I\subseteq\N}$ is a countable, pairwise disjoint collection of sets in $\mathcal{A}$ and if $\bigcup_{i\in I}A_i \in\mathcal{A}$ then 
...
$$
  \mu_k\left[\left(\bigcup_{i\in\N} B_i \right)\cap A \right] = \mu_k\left[\bigcup_{i\in\N} (A\cap B_i) \right] \quad k\in\{1,2\}
$$
...
To show that $\mu$ can be extended to a measure on $\mathcal{B}(\R)$, we first note that empty sets of $\mathcal{I}$ are intervals of the form $(a, a]$ for $a\in\R$, which by definition gives $\mu(\emptyset) = 0$. Next, $\mu$ is finitely additive on $\mathcal{I}$. If $\{A_i\}_{i\in I}$ is a finite, disjoint collection of sets in $\mathcal{I}$ with $\bigcup_{i\in I} A_i\in\mathcal{I}$ then
...
Note that a distribution function $f:\R\to\R$ is continuous at $x\in\R$ if and only if $\mu\{x\} = 0$. In particular, $\mu$ is continuous measure if and only if $f$ is continuous on $\R$. On the other hand, $f$ is discontinuous at $x\R$ if and only if $\mu\{x\}>0$, so that $\mu$ has an atom at $x$. That is, $\mu$ is a discrete measure if and only if $f$ is a step function.

<MathBox title='n-fold Lebesgue measure' boxType='definition'>
For $n\in\N$, let $(\R^n, \mathcal{B}(\R^n))$ be the $n$-dimensional Euclidean measurable space where $\mathcal{B}(\R^n) = \sigma\left\{ \prod_{i=1}^n I_i \mid I_i \in\mathcal{I} \right\}$ and $\mathcal{I}$ is a semi-ring of bounded intervals in $\R$. The $n$-fold Lebesgue measure $\lambda_n:\mathcal{B}(\R^n)\to[0,\infty]$ is given by
...
In the general case, suppose that $(X_i, \mathcal{A}_i, \mu_i)$ is a $\sigma$-finite measure space for $\{i\}_{i=1}^{n\in\N}$. Let $X = \prod_{i=1}^n S_i$ and $\mathcal{A} = \otimes_{i=1}^n \mathcal{A}_i$. Then there exists a unique $\sigma$-finite measure on $(X,\mathcal{A})$ satisfying
...
$$
  \dim_H (A) := \inf\left\{ k\in[0, \infty] \mid H^k(A) = 0 \right\}
$$
</MathBox>
...
For any $\varepsilon > 0$ and any cover $\mathcal{B} = \{ B_n\}_{n\in\N}$ of $X$ with $\mathrm{diam}(B_n) \leq \varepsilon$ we have
...
$$
  \dim(G_f) \leq 1 + (1 - \alpha)\min\left\{n, \frac{1}{\alpha}\right\}
$$
...
Suppose that $\mathcal{U} = \{U_n\}_{n\in\N_+}$ is a cover of $E$ with $|U_n| < \delta$. Let $V_n$ be the closure of $U_n$ and note that $|U_n| = |V_n|$. We
...
Let $(X, \mathcal{A})$ be a measurable space and suppose that $\{A_i\}_{i\in I}\subseteq\mathcal{A}$ is a finite collection of measurable sets partitioning $X$. A simple function $f:X\to\R$ is a finite linear combination of indicator functions on $\{A_i\}_{i\in I}$

$$
  f(x) = \sum_{i\in I} c_i \chi_{A_i}(x)\; c_i\in\R\forall i\in I
$$

If $\mathrm{Ran}(f) = \{c_i\}_{i\in I}\subset\R$ is a set of distinct values with $A_i = \{x\in X \mid f(x) = a_i \}$, then the linear combination is called a canonical representation of $f$.
...
It can be shown that the canonical representation of a simple function is unique. First, suppose that $f:X\to\R$ is simple. By definition the range of $f$ is a finite set of distinct real numbers of the form $\mathrm{Ran}(f) = \{a_i\}_{i\in I\}\subset\R$. It follows that $I$ is a finite index set and that $\{A_i = f^{-1}[a_i]\}_{i\in I}\subseteq\mathcal{A}$ is a collection of measurable sets partitioning $X$. Thus, $f$ can be represented as $f = \sum_{i\in I}a_i \chi_{A_i}$. Conversely, suppose that $f$ has a representation of this form. Then $\mathrm{Ran}(f) = \{a_i\}_{i\in I}$ and $A_i = f^{-1}[a_i]$, so the representation is unique.
</details>
</MathBox>
...
Note that $g\circ f:X\to\R$ and $\mathrm{Ran}(g\circ f)\subseteq\mathrm{Ran}(g)$ so $g\circ f$ has finite range. Since $f$ is measurable, it follows that $\{f^{-1}[B_i]\}_{i\in I}$ is a measurable partition of $X$. Finally, if $x\in f^{-1}[B_i]$ for $i\in I$, then $f(x)\in B_i$ such that $g[f(x)] = b_i$.
</details>
</MathBox>
...
For $n\in\N$ and $k\in\{1, 2,\dots, n2^n\}$ let $I_{n,k} = \left[\frac{k-1}{2^n}, \frac{k}{2^n} \right)$ and $I_n = [n,\infty)$. Note that
...
Let $A_{n,k} = f^{-1}[I_{n,k}]$ and $A_n = f^{-1}[I_n]$ for $n\in\N$ and $k\in\{1,2,\dots,n2^2\}$. Since inverse images preserve set operations it follows that
...
such that $0\leq f_n\leq f$ for each $n\in\N$. To show convergence, fix $x\in X$. If $n > f(x)$ then $|f(x)-f_n(x)|\leq 2^{-n}$ and hence $f_n(x)\xrightarrow{n\to\infty} f(x)$. It remains to show that $f_n$ is increasing in $n$. Let $x\in X$ and $n\in\N$. If $x\in A_{n,k}$ for some $k\in\{1,2,\dots n2^n\}$, then $f_n(x) = \frac{k-1}{2^n}$. However, either
...
\begin{gather*}
  ...
  \mathcal{S}^+ := \left\{ h: X\to\R \; | \; h = \sum_{i=1}^n c_i \chi_{A_i}(x), \; c_i \geq 0 \right\}
\end{gather*}
$$
</MathBox>
...
*4a*
Note that if $\mu(A) = 0$ then clearly $\int_X f\,\mathrm{d}\mu = \int_A f\,\mathrm{d}\mu + \int_{A^c} f\,\mathrm{d}\mu = 0$. In the opposite case, let $B_n = \{x\in X\mid f(x)\geq\frac{1}{n}\}$ for $n\in\N$ and $B = \{x\in X\mid f(x)>0\}$. Then $B_n$ is increasing in $n$ and $\bigcup_{n\in\N} B_n = B$. If $\mu(B) > 0$ then $\mu(B_n) > 0$ for some $n\in\N$. However, $f\geq \frac{1}{n}\chi_{B_n}$ on $A$, so by increasing monotonicity
...
For $n\in\N$, let $u_n = \inf\{f_k\}_{k\geq n}$ and let $v_n = \sup\{f_k \}_{k\geq n}$. Then
