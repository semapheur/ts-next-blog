...
such that $\mathrm{dim}(E) \geq\alpha$.

<details>
<summary>Proof</summary>

Suppose that $\mathcal{U} = \{U_n\}_{n\in\mathbb{N}_+}$ is a cover of $E$ with $|U_n| < \delta$. Let $V_n$ be the closure of $U_n$ and note that $|U_n| = |V_n|$. We

$$
\begin{align*}
  0 &< \mu(E) \leq \mu\left(\bigcup_{n\in\mathbb{N}_+} U_n \right) \\
  &\leq \mu\left(\bigcup_{n\in\mathbb{N}_+} V_n \right) \leq \sum_{n\in\mathbb{N}_+}\mu(V_n) \\
  &\leq C\sum_{n\in\mathbb{N}_+} |U_n|^\alpha
\end{align*}
$$
...
Hence, if $I_\alpha (\mu) < \infty$, then $H^\alpha (E) = \infty$ and in particular $\mathrm{dim}(E) \geq \alpha$.

<details>
<summary>Proof</summary>

Suppose that $(A_n)_{n\in\mathbb{N}_+}$ is a pairwise disjoint cover of $E$ by sets of diameter $\varepsilon > 0$. Then

$$
  \iint_{\rho(x,y) < \varepsilon} \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha} \geq \sum_{n\in\mathbb{N}_+} \iint_{A_n \times A_n}  \frac{\mathrm{d}\mu(x)\mathrm{d}\mu(y)}{\rho(x,y)^\alpha} \geq \sum_{n\in\mathbb{N}_+} \frac{\mu(A_n)^2}{|A_n|^2}
$$

The metric space $E$ can be bounded as follows

$$
  \mu(E) \leq \sum_{n\in\mathbb{N}_+} \mu(A_n) = \sum_{n\in\mathbb{N}_+} |A_n|^{\alpha/2} \frac{\mu(A_n)}{|A_n|^{\alpha/2}}
$$
...
$$
\begin{align*}
  \mu(E) &\leq \sum_{n\in\mathbb{N}_+} |A_n|^\alpha \sum_{n\in\mathbb{N}_+} \frac{\mu(A_n)^2}{|A_n|^\alpha} \\
