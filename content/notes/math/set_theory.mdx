...
2. the range of $R$ is the set of second relation coordinates: $\mathrm{ran}(R) := \Set{ y\in Y | (x, y)\in R,\; x\in X }$
...
2. the quotient set of $X$ under $\sim$ is $X\setminus\sim := \Set{ f^{-1}[y] | y \in \mathrm{ran}(f) }$
...
**(2)**: Recall that $y\in\mathrm{ran}(f)$ if and only if $f(x) = y$ for some $x\in X$. Then by definition, $[x] = f^{-1}[y] = \Set{\tilde{x}\in X| f\left(\tilde{x}\right) = y} = \Set\ \tilde{x}\in X \mid f\left(\tilde{x}\right) = f(x) }$ 
</details>
</MathBox>
...
- Range: $\mathrm{ran}(f) := \Set{ y \in Y | \exists x \in X : f(x) = y } = \Set{ y \in Y | x \in X }$
