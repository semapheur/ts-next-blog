...
1. the domain of $R$ is the set of first relation coordinates: $\mathrm{dom}(R) := \set{ x\in X | (x, y)\in R,\; y\in Y }$
2. the range of $R$ is the set of second relation coordinates: $\operatorname{ran}(R) := \et{ y\in Y | (x, y)\in R,\; x\in X }$
