...
## Number sets
- Empty set: $\emptyset := \{ \}$
- Natural numbers: $\N := \{ i \}_{i=1}^\infty$
- Natural numbers including zero: $\N_0 := \{ i \}_{i=0}^\infty$
- Integers: $\Z := \{ i \}_{i=-\infty}^\infty$
- Dyadic rationals $\mathbb{D} = \left\{\frac{j}{2^n} \mid j\in\Z \; n\in\N \right\}$
- Rational numbers $\mathbb{Q} = \left( \Z\times\Z\setminus\{0\} \right) / \sim \;= \left\{ \frac{a}{b} := [(a, b)]_\sim \mid (a, b) \in \Z\times\Z\setminus\{0\} \right\}$ with $\sim \;:= (a, b) \sim (c, d) \iff ad = cb$
- Algebraic numbers: $\mathbb{A} = \left\{x\in\R \mid p(x) = 0 \right\}$ for some polynomial $p$ with integer coefficients
- Real number: $\R$
...
1. Addition rule: if $\{A_i\}_{i=1}^{n\in\N}$ is a collection of disjoint subsets of $S$ then $\left|\bigcup_{i=1}^n A_i \right| = \sum_{i=1}^n \left|A_i\right|$
...
6. Boole's inequality: if $\{A_i\}_{i=1}^{n\in\N}$ is a collection of subsets of $S$ then $\left|\bigcup_{i=1}^n A_i \right| \leq \sum_{i=1}^n \left|A_i\right|$
7. Bonferroni's inequality: if $\{A_i\}_{i=1}^{n\in\N}$ is a collection of subsets of $S$ then $\left|\bigcap_{i=1}^n A_i \right| \geq |S| - \sum_{i=1}^n \left(|S| - |A|\right)$
...
9. Multiplication rule: If $S_i$ is a set with $n_i$ elements for $\{i\}_{i=1}^{k\in\N}$, then $\left| \prod_{i=1}^k S_i \right| = \prod_{i=1}^k n_i$
...
Let $\{ A_i \}_{i\in I}$ be a collection of sets with $|I| = \iota$. If $|A_i| \leq \lambda$ for all $i\in I$ then
...
<MathBox title='Proposition' boxType='proposition'>
If $S$ is a set then $\mathcal{P}(S)\sim\{0, 1\}^S$ implying that $P(S) = 2^S$. Suppose $S$ has cardinality $|S| = \sigma\in\N$. Then the cardinality of $\mathcal{P}(S)$ is
...
Suppose that $D$ is a set with $n\in\N$ elements. A permutation of length $k\in\{0, 1,\dots, n\}$ from $D$ is an ordered sequence of $k$ distinct elements of $D$, i.e. a sequence of the form 
...
Suppose that $D$ is a set with $n\in\N$ elements. A combination of length $k\in\{0, 1,\dots, n\}$ from $D$ is an unordered subset of $k$ distinct elements of $D$, i.e. a subset of the form 
...
The Cartesian product of a collection of sets $\{S_i\}_{i=1}^{n\in\N}$ is defined as
...
1. $\mathbf{1}_{\bigcap_{i\in I} A_i} = \prod_{i\in I} \mathbf{1}_{A_i} = \min\{\mathbf{1}_{A_i} \mid i\in I\}$
2. $\mathbf{1}_{\bigcup_{i\in I} A_i} = 1 - \prod_{i\in I} \left(1 - \mathbf{1}_{A_i}\right) = \max\{\mathbf{1}_{A_i} \mid i\in I\}$
</MathBox>
