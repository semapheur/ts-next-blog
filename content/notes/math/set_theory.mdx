...
A permutation can be defined as a bijection $\sigma: D\xrightarrow{\sim} D$. The set of all permutations of $D$ of length $n$ forms the symmetric group $S_n$. The signature $\operatorname{sgn}(\sigma)$ of a permutation $\sigma$ is $+1$ if the permutation can be obtained with an even number of transpositions, otherwise it is $-1$.
</MathBox>
