...
A point $p\in D$ is called an accumulation point of the set $M\subseteq D$ if for all open sets $U$ with $p\in U$, then $U\setminus\Set{p}\cap M \neq\emptyset$.
</MathBox>
...
- Power set (set of all subsets): $P(A) := \Set{ X | X\subseteq A }$
- Union (set of combined elements): $A \cup B := \Set{ x | x \in A \lor x \in B }$
- Big union: $\bigcup_{i\in I} A_i := \Set{ x | \exists i\in I : x \in A_i }$
- Intersection (set of common elements): $A \cap B := \Set{ x | x \in A \land x \in B }$
- Big intersection: $\bigcap_{i\in I} A_i := \Set{ x | \forall i\in I : x \in A_i }$
...
4. $A\cup B = S\setminus\Set{(S\setminus A)\setminus [(S\setminus A)\setminus(S\setminus B)] }$
</MathBox>

## Number sets
- Empty set: $\emptyset := \Set{ }$
- Natural numbers: $\N := \Set{ i }_{i=1}^\infty$
- Natural numbers including zero: $\N_0 := \Set{ i }_{i=0}^\infty$
- Integers: $\Z := \Set{ i }_{i=-\infty}^\infty$
- Dyadic rationals $\mathbb{D} = \Set{\frac{j}{2^n} | j\in\Z \; n\in\N }$
- Rational numbers $\mathbb{Q} = \left( \Z\times\Z\setminus\Set{ \frac{a}{b} := [(a, b)]_\sim | (a, b) \in \Z\times\Z\setminus\Set{0} }$ with $\sim \;:= (a, b) \sim (c, d) \iff ad = cb$
- Algebraic numbers: $\mathbb{A} = \Set{x\in\R | p(x) = 0 }$ for some polynomial $p$ with integer coefficients
- Real number: $\R$
    - Irrational numbers: $\R\setminus\mathbb{Q}$
    - Transcedental numbers: $\R\setminus\mathbb{A}$
- Complex numbers: $\mathbb{C}$

Note that $\N\subset\N_0\subset\Z\subset\mathbb{D}\subset\mathbb{Q}\subset\mathbb{A}\subset\R$.
...
For general (possibly infinite) sets $A$ and $B$, then
1. $|A| = |B|$ if there is a bijection from $A$ to $B$
2. $|A| \leq |B|$ if there is a bijection from $A$ to a subset of $B$
3. $|A| < |B|$ if there is a bijection from $A$ to a proper subset of $B$
</MathBox>

<MathBox title='Cardinality rules' boxType="proposition">
Suppose $S$ is finite set.
1. Addition rule: if $\Set{A_i}_{i=1}^{n\in\N}$ is a collection of disjoint subsets of $S$ then $\left|\bigcup_{i=1}^n A_i \right| = \sum_{i=1}^n \left|A_i\right|$
...
6. Boole's inequality: if $\Set{A_i}_{i=1}^{n\in\N}$ is a collection of subsets of $S$ then $\left|\bigcup_{i=1}^n A_i \right| \leq \sum_{i=1}^n \left|A_i\right|$
7. Bonferroni's inequality: if $\Set{A_i}_{i=1}^{n\in\N}$ is a collection of subsets of $S$ then $\left|\bigcap_{i=1}^n A_i \right| \geq |S| - \sum_{i=1}^n \left(|S| - |A|\right)$
8. Inclusion-exclusion formula: if $\Set{A_i}_{i\in I}$ is a collection subsets of $S$ with $|I| = n$, then $\left|\bigcup_{i\in I} A_i \right| = \sum_{k=1}^n (-1)^{k-1}\left( \sum_{J\subseteq I,\, |J| = k} \left|\bigcap_{j\in J} A_j \right|\right)$
    - If $A$ and $B$ are subsets of $S$, then $|A\cup B| = |A| + |B| - |A\cap B|$
9. Multiplication rule: If $S_i$ is a set with $n_i$ elements for $\Set{i}_{i=1}^{k\in\N}$, then $\left| \prod_{i=1}^k S_i \right| = \prod_{i=1}^k n_i$
    - If $S$ is a set with $n$ elements, then $\left|S^k\right| = n^k$
10. Power rule: $\left|T^S \right| = |T|^{|S|}$ where $T^S$ is the set of all functions from $S$ to $T$
</MathBox>

<MathBox title='Cardinality partial order' boxType="definition">
Suppose $\mathcal{S}$ is a collection of sets. The cardinality partial order on $\mathcal{S}$ is a reflexive and transitive relation $\leq$ defined by $A\leq B$ if and only if there is an injection $f:A\to B$ if and only if there is a surjection $g:B\to A$. If $A\leq B$ and $A\sim B$, then we write $A < B$.

<details>
<summary>Proof</summary>

To show reflexivity of $\leq$, recall that the identity function $\operatorname{id}_A: A\to A$ for $A\in\mathcal{S}$ is injective (and also surjective), so $A\leq A$. To show transitivity of $\sim$, suppose $A,B,C\in\mathcal{S}$ and that $A\leq B$ and $B\leq C$. Then there exists injections $f:A\to B$ and $g:B\to C$, and hence $g\circ f: A\to C$ is also injective, so $A\leq C$.
</details>
</MathBox>

<MathBox title='Schröder-Bernsteim theorem' boxType="theorem">
...
$$
\begin{align*}
  A_A =& \Set{a\in A | a \text{ originates in } A} \\
  A_B =& \Set{a\in A | a \text{ originates in } B} \\
  A_\infty =& \Set{a\in A | a \text{ has no originator } } \\
\end{align*}
$$
...
<MathBox title='Upper bound of cardinality' boxType="proposition">
Let $\Set{ A_i }_{i\in I}$ be a collection of sets with $|I| = \iota$. If $|A_i| \leq \lambda$ for all $i\in I$ then

$$
  \left| \bigcup_{i\in I} A_i \right| \leq \lambda \iota
$$
</MathBox>

### Infinity
<MathBox title='Cardinality terms' boxType="definition">
A set $A$ is called
1. finite if $A\sim \N_k = {i}_{i=0}^{k\in\N}$, in which case $k$ is the cardinality of $A$, i.e. $|A| = k$
2. infinite if $A$ is not finite
3. countably infinite if $A\sim\N$
...
The cardinality of the natural numbers is denoted $\aleph_0$. Since the natural, integer and rational numbers are mutually bijective

$$
  |\N| = |\Z| = |\mathbb{Q}| = \aleph_0
$$
</MathBox>

<MathBox title='Absorption property of infinite cardinals' boxType="proposition">
If $\kappa$ and $\lambda$ are cardinal numbers, of which at least one is infinite, then

$$
  \kappa + \lambda = \kappa\lambda = \max\Set{\kappa,\lambda}
$$
</MathBox>
...
Suppose $f:A\to B$ is injective, then
1. If $B$ is finite, then $A$ is finite
2. If $A$ is infinite, then $B$ is infinite
3. If $B$ is countable, then $A$ is countable
4. If $A$ is uncountable, then $B$ is uncountable

Suppose $f:A\to B$ is surjective, then
...
<MathBox title="Proposition" boxType="proposition">
If $S$ is a set then $\mathcal{P}(S)\sim\Set{0, 1}^S$ implying that $P(S) = 2^S$. Suppose $S$ has cardinality $|S| = \sigma\in\N$. Then the cardinality of $\mathcal{P}(S)$ is

$$
  |S| < |\mathcal{P(S)}| = 2^{|S|} = 2^\sigma
$$

<details>
<summary>Proof</summary>

To prove the equivalence $\mathcal{P}(S)\sim \Set{0, 1}^S$. Recall that the indicator function $\mathcal{1}_A$ maps from some set $A$ into $\Set{0, 1}$, so that $\Set{0, 1}^S$ is the set of all indicator functions for $S$. Suppose $A\in\mathcal{P}(S)$ is a subset of $S$ and consider the function $f:\mathcal{P}(S)\to\Set{0, 1}^S$ given by $f(A) = \mathbf{1}_A$. If $A,B\in\mathcal{P}(S)$ and $\mathbf{1}_A = \mathbf{1}_B$, then $A = B$, so $f$ is injective. If $g\in\Set{0, 1}^S$, then $g = \mathbf{1}_A$ with $A = \Set{x\in S | f(x) = 1}$, so $f$ is surjective. Hence $f:\mathcal{P}(S)\to\Set{0, 1}^S$ is a bijection. 
</details>
</MathBox>

It can be shown that $\R$ is bijective to $\mathcal{P}(\N)$ such that $|\R| = 2^{|\N|} = 2^{\aleph_0}$. Since the real numbers represents a continuum, the cardinal number $2^{\aleph_0}$ is called the *power of continuum*.
...
The function $\iota:S\to\mathcal{P}(S)$ defined by $\iota(s) = \Set{s}$ is an injection from $S$ to $\mathcal{P}(S)$ and so $|S|\leq|\mathcal{P}(S)|$. It remains to show that no injection $f:S\to\mathcal{P}(S)$ can be surjective.

Let $X = \Set{ s\in S | s\notin f(s) }\in\mathcal{P}(S)$. We claim that $X\notin f[\mathcal{P}(S)]$. Suppose $X = f(x)$ for some $x\in S$. Then if $x\in X$, we have by the definition of $X$ that $x\notin X$. On the other hand, if $x\notin X$ we have again by the definition of $X$ that $x\in X$. This contradiction implies that $X\notin f[\mathcal{P}(S)]$ and so $f$ is not surjective.
</details>
</MathBox>

<MathBox title='Absorption property of aleph-zero' boxType="proposition">
For any $n\in\mathbb{N}_+$, then 
1. $\aleph_0 \cdot\aleph_0 = \aleph_0$ and $\aleph_0^n = \aleph_0$
2. $\aleph_0 \cdot 2^{\aleph_0} = 2^{\aleph_0}$ and $\left(2^{\aleph_0}\right)^{\aleph_0} = 2^{\aleph_0}$
</MathBox>

Using this proposition, it follows that
...
Suppose $D$ is a set with $n\in\N$ elements. A permutation of length $k\in\Set{0, 1,\dots, n}$ from $D$ is an ordered sequence of $k$ distinct elements of $D$, i.e. a sequence of the form 
...
A permutation can be defined as a bijection $\sigma: D\xrightarrow{\sim} D$. The set of all $n!$ permutations of $D$ of length $n$ forms the symmetric group $S_n$. A *transposition* is a permutation $\tau \in S_n$ swapping two entries and fixing the rest, i.e.

$$
  \tau(i) = j, \quad \tau(j) = i, \quad \tau(k) = k,\; k\neq i,j
$$

The signature $\operatorname{sgn}(\sigma)$ of a permutation $\sigma$ is $+1$ if the permutation can be obtained with an even number of transpositions, otherwise it is $-1$.
</MathBox>

<MathBox title="Discriminant" boxType="definition">
The discriminant of variables $x_1,\dots,x_n$ is

$$
  P(x_1,\dots,x_n) = \prod_{i<j}(x_i - x_j)
$$

Permuting variables only changes the signature of the discriminant

$$
  P(\sigma(x_1,\dots,x_n)) = \prod_{i<j} (x_{sigma(i)} - x_{\sigma(j)}) = \operatorname{sgn}(\sigma) \prod_{i<j}(x_i - x_j)
$$
</MathBox>

<MathBox title='Properties of permuation signature' boxType="proposition">
1. $\operatorname{sgn}(\sigma_1 \circ \sigma_2) = \operatorname{sgn}(\sigma_1)\operatorname{sgn}(\sigma_2)$ for any permutations $\sigma_1, \sigma_2 \in S_n$
2. $\sigma(\tau) = -1$ for any transposition $\tau\in S_n$
3. every $\sigma\in S_n$ can be written as a composition of transpositions, i.e. $\sigma = \tau_k \circ\cdot\circ\tau_1$, whose parity is unique
4. if $\sigma = \tau_k \circ\cdot\circ\tau_1$, then $\operatorname{sgn}(\sigma) = (-1)^k$
...
If $\sigma = \tau_k \circ\cdot\circ\tau_1$, then

$$
\begin{align*}
  \operatorname{sgn}(\sigma) =& \operatorname{sgn}(\tau_k \circ\cdot\circ\tau_1) \\
  =& \prod_{i=1}^k \underbrace{\operatorname{sgn}(\tau_i)}_{=-1} \\
  =& (-1)^k
\end{align*}
$$
...
Suppose $D$ is a set with $n\in\N$ elements. A combination of length $k\in\Set{0, 1,\dots, n}$ from $D$ is an unordered subset of $k$ distinct elements of $D$, i.e. a subset of the form 

$$
  \Set{x_i \mid x_i\in D,\; x_i\neq x_j\; \forall $i\neq j$ }_{i=1}^{k}$
$$
...
$$
  \bigcap_{i=1}^n p_{Y_i}^{-1} (A_i) = \Set{ x = \left( x_{Y_i} \right)_{i=1}^n \in X | x_{Y_i}\in A_i \subseteq Y_i }
$$
...
## Set identities
Suppose $A$, $B$ and $C$ are subsets of a universal set $S$, and that $\mathcal{A} = \Set{ A_i }_{i\in I}$ is a collection of subsets over $S$ for some nonempty index set $I$. 
...
If $(a, b) = (c, d)$ then $\Set{\Set{a}, \Set{a, b}} = \Set{\Set{c}, \Set{c, d}}$. In the case that $a = b$ we note that 

$$
  (a, b) = \Set{\Set{a}, \Set{a, b}} = \Set{\Set{a}, \Set{a, a}} = \Set{\Set{a}, \Set{a}} = \Set{\Set{a}}
$$

Thus, we must have $\Set{c} = \Set{c, d} = \Set{a}$ and hence $c = d = a = b$. 

In the case that $a\neq b$ we must have $\Set{a} = \Set{c}$ and hence $c = a$. Suppose
- $\Set{c, d} = \Set{a}$ gives $c = d = a$ and $(c, d) = \Set{\Set{a}}$. Hence $\Set{a, b} = \Set{a}$ which forces $a = b$, contradicting $a\neq b$.
- $\Set{c} = \Set{a, b}$ gives $a = b = c$, contradicting $a\neq b$

Thus by contradiction, we must have $\Set{c, d} = \Set{a, b}$. Since $c = a$ and $a \neq b$ we must have $d = b$.

The converse is trivial: if $a = c$ and $b = d$ then $\Set{a} = \Set{c}$ and $\Set{a, b} = \Set{c, d}$ so $(a, b) = (c, d)$. 
</details>
</MathBox>

<MathBox title='Cartesian product' boxType="definition">
The Cartesian product of a collection of sets $\Set{S_i}_{i=1}^{n\in\N}$ is defined as

$$
  S_1 \times S_2 \times \dots S_n = \prod_{i=1}^n S_i = \Set{ \left( x_i \right)_{i=1}^n | x_i \in S_i }
$$

Using functions, the Cartesian product of a collection of sets $\Set{S_i}_{i\in I}$ indexed by the (possibly infinite) set $I$ can be defined as

$$
  \prod_{i\in I} S_i := \Set{ x: I\to\bigcup_{i\in I} S_i | \forall i \in I: x(i)\in X_i }
$$
...
Consider the Cartesian product $P = S\times T$ and suppose $A, B\subseteq S$ and $C, D\subseteq T$
...
Suppose $C\subseteq S\times T$, then we may define the following subsets of $C$

**Cross sections**
1. The cross section of $C$ in the first coordinate at $x\in S$ is $C_x = \Set{y\in T | (x, y)\in C}\subseteq T$
2. The cross section of $C$ in the second coordinate at $y\in S$ is $C^y = \Set{x\in S | (x, y)\in C}\subseteq S$

**Projections**
1. The projection of $C$ onto $T$ is $C_T = \Set{y\in T | (x, y)\in C\textrm{ for some }x\in S }$
2. The projection of $C$ onto $S$ is $C^S = \Set{x\in S | (x, y)\in C\textrm{ for some }y\in T }$
...
<MathBox title="Proposition" boxType="proposition">
Suppose $C, D\subseteq S\times T$. The following properties hold for cross sections and projections
...
<MathBox title="Binary relation" boxType="definition">
Let $X$ and $Y$ are sets. A binary relation from $X$ to $Y$ is a subset $R\subseteq X\times Y$ where
1. the domain of $R$ is the set of first relation coordinates: $\operatorname{dom}(R) := \set{ x\in X | (x, y)\in R,\; y\in Y }$
2. the range of $R$ is the set of second relation coordinates: $\operatorname{ran}(R) := \set{ y\in Y | (x, y)\in R,\; x\in X }$

A relation $R \subseteq X \times Y$ from $X$ to $Y$ is usually denoted $xYy$ for $(x, y)\in R$.
</MathBox>

<MathBox title="Inverse relation" boxType="definition">
If $R$ is a relation from $X$ to $Y$, the inverse of $R$ is the relation from $Y$ to $X$ defined by $R^{-1} := \set{(y, x) | (x, y) \in R}$
</MathBox>
...
Suppose $Q$ is a relation from $X$ to $Y$ and that $R$ is a relation from $Y$ to $Z$. The composition $Q\circ R$ is the relation from $X$ to $Y$ defined by (for $x\in X$ and $y\in Y$)
...
Suppose $(S, \leq)$ is a partially ordered set with the property that $\sup(A)$ exists for every $A\subset S$. If $f:S\to S$ is increasing, then $f$ has a fixed point.

<details>
<summary>Proof</summary>

Let $A = \Set{x\in S| x\leq f(x)}$ and let $z = \sup(A)$. If $x\in A$, then $x\leq z$ so $x\leq f(x) \leq f(z)$. Hence $f(z)$ is an upper bound of $A$ so $z\leq f(z)$. However $f(z)\leq f(f(z))$, since $f$ is increasing, so $f(z)\in A$ and hence $f(z)\leq z$. Therefore $f(z) = z$ showing that $z$ is a fixed point of $f$.
</details>
</MathBox>
...
Let $S$ be a set. A sequence of subsets $(A_n \subseteq S)_{n\in\N}$ is called monotone if it is increasing or decreasing, defined as follows
1. $(A_n)_{n\in\N}$ is increasing if $A_n\subseteq A_{n+1}$ for every $n\in\N$, defining $\lim_{n\to\infty} A_n = \bigcup_{n\in\N} A_n$
2. $(A_n)_{n\in\N}$ is increasing if $A_{n+1}\subseteq A_{n}$ for every $n\in\N$, defining $\lim_{n\to\infty} A_n = \bigcap_{n\in\N} A_n$
</MathBox>
...
Let $S$ be a set. The following properties hold for a sequence of subsets $(A_n \subseteq S)_{n\in\N}$:
1. The initial union $\bigcup_{i=1}^n A_i$ is increasing in $n\in\N$ and $\bigcup_{i=1}^\infty A_i = \lim_{n\to\infty}\bigcup_{i=1}^n A_i$
    a. The tail union $\bigcup_{i=n}^\infty A_i$ is increasing in $n\N$ 
2. The initial intersection $\bigcap_{i=1}^n A_i$ is decreasing in $n\in\N$ and $\bigcap_{i=1}^\infty A_i = \lim_{n\to\infty}\bigcap_{i=1}^n A_i$
    a. The tail intersection $\bigcap_{i=n}^\infty A_i$ is decreasing in $n\N$ 
</MathBox>

<MathBox title="Bounded limits of sequences of sets" boxType="definition">
Let $S$ be a set. The following bounds can be defined for a sequence of subsets $(A_n \subseteq S)_{n\in\N}$:
...
Suppose $\Set{S_i, \leq_i}_{i\in I}$ is a collection of partially ordered sets for an index set $I$. The product order $\leq$ on the product set $S^I = \prod_{i\in I} S_i$ is defined as (for $x,y\in S^I$)
...
Suppose $\Set{S_i, \leq_i}_{i\in I}$ is a collection of partially ordered sets for an index set $I$, and suppose $\leq$ is a total order on $I$. The lexicographic order $\leq$ on the product set $S^I = \prod_{i\in I} S_i$ is defined as (for $x,y\in S^I$)
...
2. $a \sim b \iff b \sim a$ **(symmetry)**
3. $a \sim b \land b \sim c \implies a \sim c$ **(transitivity**)
...
**(1) Reflexivity**<br/>
The identity function $\operatorname{id}_A : A \to A$ given by $x\mapsto x$ for $x\in A$ is bijection on $A$, and hence $A\sim A$

**(2) Symmetry**<br/>
If $A\sim B$ then there is a bijection $f:A\to B$. Since $f$ is bijective, then $f^{-1}:B\to A$ is a bijection, and hence $B\sim A$

**(3) transitivity**<br/>
Suppose $A\sim B$ and $B\sim C$. Then there exists bijections $f:A\to B$ and $g:B\to C$. Since $f$ and $g$ are bijective, the composition $g\circ f: A\to C$ is also a bijection, and hence $A\sim C$
</details>
</MathBox>
...
2. the quotient set of $X$ under $\sim$ is $X/{\sim} := \Set{ f^{-1}[y] | y \in \operatorname{ran}(f) }$
3. the function $F:X/{\sim}\to Y$ defined by $F([x]) = f(x)$ is well defined and is injective.
4. if $f$ is injective, then $\sim$ is the equality relation, and hence $[x] = \Set{x}$ for each $x\in X$
...
**(2)**: Recall that $y\in\operatorname{ran}(f)$ if and only if $f(x) = y$ for some $x\in X$. Then by definition, $[x] = f^{-1}[y] = \Set{\tilde{x}\in X| f\left(\tilde{x}\right) = y} = \Set\ \tilde{x}\in X \mid f\left(\tilde{x}\right) = f(x) }$ 
</details>
</MathBox>

<MathBox title="Proposition" boxType="proposition">
Suppose $\sim$ is an equivalence relation on a set $S$. Define $f:S\to\mathcal{P}(S)$ by $f(x) = [x]$. Then $\sim$ is the equivalence relation associated with $f$
...
Suppose $\sim$ and $\approx$ are equivalence relations on a set $S$. Let $\equiv$ denote the intersection of $\sim$ and $\approx$, i.e. $x\equiv y$ if and only if $x\sim y$ and $x\approx y$ for $x, y \in S$. Then
...
Suppose $\leq$ is a relation on a set $S$ that is reflexive and transitive. Define the relation $\sim$ on $S$ by $x\sim y$ if and only if $x\leq y$ and $y\leq x$ for all $x, y\in S$. Then
...
3. define the relation $\leq$ on the quotient set $S/{\sim}$ by $A\leq B$ if and only if $a\leq b$ for all $a\in A$ and $b\in B$. Then $\leq$ is a partial order on $S/{\sim}$
</MathBox>

<MathBox title="Invariant" boxType="definition">
Let $\sim$ be an equivalence relation on $S$. A function $f:S\to T$ is called an invariant of $\sim$ if it is constant on the equivalence classes of $\sim$, i.e.

$$
  a\sim b \implies f(a) = f(b)
$$

and a complete invariant if it is constant and distinct on the equivalence classes of $\sim$, i.e.

$$
  a\sim b \iff f(a) = f(b)
$$

A collection $\Set{f_1,\dots,f_n}$ of invariants is called a complete system of invariants if

$$
  a\sim b \iff f_i(a) = f_i(b),\; \forall i=1,\dots,n
$$
</MathBox>

<MathBox title='Canonical form' boxType="definition">
Let $\sim$ be an equivalence relation on $S$. A subset $C\subseteq S$ is a set of canonical forms for $\sim$ if for every $s\in S$, there is exactly one $c\in C$ such that $c\sim s$. Equivalently, each equivalence class under $\sim$ contains exactly one member of $C$.
</MathBox>
...
The identity function on a set $X$ is the function $\operatorname{id}_X: X \to X$ defined by $x \mapsto x$.
</MathBox>

<MathBox title="Injectivity, surjectivity and bijectivity" boxType="definition">
A function $f: X \to Y$ is
- injective (one-to-one) if each $y \in Y$ is mapped to by at most one $x \in X$.
    - $\forall x, x' \in X,\; f(x) = f(x') \implies x = x'$
- surjective (onto) if each $y \in Y$ is mapped to by at least one $x \in X$, i.e. $\operatorname{ran}(f) = Y$
    - $\forall y \in Y,\; \exists x \in X : y = f(x)$
- bijective (invertible) if each $y \in Y$ is mapped to by exactly one $x \in X$ (both injective and surjective), i.e. there is a unique inverse function $f^{-1}: Y \to X$ defined by $f^{-1}(y) := x \implies f(x) = y$
</MathBox>

<MathBox title="Properties of functions" boxType="definition">
Let $X, Y$ be sets. For subsets $A, \tilde{A} \subseteq X$ and $B, \tilde{B} \subseteq Y$, the function $f:X\to Y$ satisfies the following properties:
1. **Monotonicity of image:** $A \supseteq \tilde{A} \implies f[A] \supseteq f[\tilde{A]$
2. **Monotonicity of preimage:** $B \subseteq \tilde{B} \implies f^{-1}[B] \subseteq f^{-1}[\tilde{B]$
3. **Image of unions:** $f\left[\bigcup_{i\in I} A_i \right] = \bigcup_{i\in I} f[A_i]$
4. **Preimage of unions:** $f^{-1} \left[\bigcup_{i\in I} B_i \right] = \bigcup_{i\in I} f^{-1} [B_i]$
5. **Image of preimage:** $f\left[f^{-1} [B] \right] \subseteq B$
6. **Preimage of image:** $f^{-1} \left[f[A] \right] \supseteq A$
...
$$
\begin{align*}
  y \in f\left[\bigcup_{i\in I} A_i \right] \iff& \exists x\in \bigcup_{i\in I} A_i : f(x) = y \\
  \iff& \exists i\in I,\; \exists x\in A_i : f(x) = y \\
  \iff& \exists i\in I : y\in f[A_i] \iff y\in \bigcup_{i\in I} f[A_i]
\end{align*}
$$
...
$$
\begin{align*}
  x \in f^{-1}\left[\bigcup_{i\in I} B_i \right] \iff& \exists y\in \bigcup_{i\in I} B_i : f(x) = y \\
  \iff& \exists i\in I,\; \exists y\in B_i : f(x) = y \\
  \iff& \exists i\in I : x\in f^{-1} [B_i] \iff x\in \bigcup_{i\in I} f^{-1} [B_i]
\end{align*}
$$
...
$$
\begin{align*}
  y\in f\left[f^{-1} [B] \right] \implies& \exists x\in f^{-1} [B] : f(x) = y \\
  \implies& \exists x\in X,\; \exists \tilde{y}\in B : f(x) = \tilde{y} \lor f(x) = y \\
  \implies& y = \tilde{y} \in B
\end{align*}
$$
...
$$
\begin{align*}
  x \in A \implies& \exists y\in f[A] : f(x) = y \\
  \implies& x \in f^{-1} \left[f[A] \right]
\end{align*}
$$
</details>
</MathBox>
...
An $n$-ary operation $f$ from $(X_i)_{i=1}^{n\in\N}$ to $Y$ is a function 

$$
  f: \prod_{i=1}^n X_i \to Y
$$

Suppose $f$ is an $n$-ary operator on a set $S$ and $A\subset S$. Then $A$ is closed under $f$ if $a \in A^n$ implies $f(a) \in A$.
</MathBox>

<MathBox title='Properties of binary operators' boxType="definition">
Suppose $f$ and $g$ are binary operators on a set $S$ and $x, y, z \in S$. Then the following properties can be defined
...
- Range: $\operatorname{ran}(f) := \Set{ y \in Y | \exists x \in X : f(x) = y } = \Set{ y \in Y | x \in X }$
- Image: $f[\tilde{X}] := \Set{ y \in Y | \exists x \in \tilde{X} : f(x) = y } = \Set{ y \in Y | x \in \tilde{X} }$ for $\tilde{X} \subseteq X$
- Preimage: $f^{-1}[\tilde{Y}] := \Set{ x \in X | f(x) = \tilde{Y} }$ for $\tilde{Y} \subseteq Y$
...
Property 1 and 2 hold for arbitrary unions and intersections, respectively. Consider a collection of subsets $\Set{A_i \subseteq X | i\in I}$ for an index set $I$. Then
...
Property 1 and 2 hold for arbitrary unions and intersections, respectively. Consider a collection of subsets $\Set{A_i \subseteq Y | i\in I}$ for an index set $I$. Then
...
The indicator function of a subset $A\subseteq S$ is the function $\mathbf{1}_A: S\to\Set{0, 1 }$ defined by
$$
    \mathbf{1}_A (x) := \begin{cases} 1,&\quad x\in A \\ 0,&\quad x\notin A \end{cases}
$$

If $f:S\to\Set{ 1 }] = \Set{ x\in S | f(x) = 1 }$
</MathBox>

<MathBox title='Properties of the indicator function' boxType="proposition">
Consider the subsets $A, B\subseteq S$. Then
1. $\mathbf{1}_{A\cap B} = \mathbf{1}_{A}\mathbf{1}_{B} = \min\Set{\mathbf{1}_{A}, \mathbf{1}_{B}}$
2. $\mathbf{1}_{A\cup B} = 1 - \left(1 - \mathbf{1}_{A}\right)\left(1 - \mathbf{1}_{B}\right) = \max\Set{\mathbf{1}_{A}, \mathbf{1}_{B}}$
3. $\mathbf{1}_{A^c} = 1 - \mathbf{1}_A$
4. $\mathbf{1}_{A\setminus B} = \mathbf{1}_A\left(1 - \mathbf{1}_B\right)$
5. $A\subseteq B$ if and only if $\mathbf{1}_A \leq \mathbf{1}_B$

Property 1 and 2 extends to arbitrary intersections and unions, respectively. Consider the collection of subsets $\Set{A_i \subseteq S | i\in I}$ for some index set $I$. Then
1. $\mathbf{1}_{\bigcap_{i\in I} A_i} = \prod_{i\in I} \mathbf{1}_{A_i} = \min\Set{\mathbf{1}_{A_i} | i\in I}$
2. $\mathbf{1}_{\bigcup_{i\in I} A_i} = 1 - \prod_{i\in I} \left(1 - \mathbf{1}_{A_i}\right) = \max\Set{\mathbf{1}_{A_i} | i\in I}$
</MathBox>
...
Suppose $\left(x, \tilde{x}\right)\in X$ and $(f\circ g)(x) = (f\circ g)\left(\tilde{x}\right)$. Then $f(g(x)) = f\left(g\left(\tilde{x}\right)\right)$. Since $f$ is injective, $g(x) = g\left(\tilde{x}\right)$. Since $g$ is injective, $x = \tilde{x}$.

**3. Preservation of surjectivity**
Suppose $z \in Z$. Since $f$ is surjective, there exists $y\in Y$ with $f(y) = z$. Since $g$ is surjective, there exists $x\in X$ with $g(x) = y$. Then
