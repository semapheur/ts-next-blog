...
By definition, a set is completely determined by its elements; thus two sets $A$ and $B$ are equalif and only ifthey have the same elements.
...
- Power set (set of all subsets): $P(A) := \{ X \;|\; X\subseteq A \}$
- Union (set of combined elements): $A \cup B := \left\{ x \;|\; x \in A \lor x \in B \right\}$
- Big union: $\bigcup_{i\in I} A_i := \left\{ x \;|\; \exists i\in I : x \in A_i \right\}$
- Intersection (set of common elements): $A \cap B := \left\{ x \;|\; x \in A \land x \in B \right\}$
- Big intersection: $\bigcap_{i\in I} A_i := \left\{ x \;|\; \forall i\in I : x \in A_i \right\}$
- Difference: $A \setminus B := \left\{ x \;|\; x \in A \land x \notin B \right\}$
...
- Complement: $\bar{A} = A^c := \left\{ x \;|\; x \notin A \right\}$
- Cartesian product: $A \times B := \left\{ (a, b) \;|\; a \in A \land b \in B \right\}$
...
- Dyadic rationals $\mathbb{D} = \left\{\frac{j}{2^n} \;|\; j\in\mathbb{Z} \; n\in\mathbb{N} \right\}$
- Rational numbers $\mathbb{Q} = \left( \mathbb{Z}\times\mathbb{Z}\setminus\{0\} \right) / \sim \;= \left\{ \frac{a}{b} := [(a, b)]_\sim \;|\; (a, b) \in \mathbb{Z}\times\mathbb{Z}\setminus\{0\} \right\}$ with $\sim \;:= (a, b) \sim (c, d) \iff ad = cb$
- Algebraic numbers: $\mathbb{A} = \left\{x\in\mathbb{R} \;|\; p(x) = 0 \right\}$ for some polynomial $p$ with integer coefficients
...
Suppose that $\mathcal{S}$ is a collection of sets. The cardinality partial order on $\mathcal{S}$ is a reflexive and transitive relation $\leq$ defined by $A\leq B$if and only ifthere is an injection $f:A\to B$if and only ifthere is a surjection $g:B\to A$. If $A\leq B$ and $A\sim B$, then we write $A < B$.
...
2. A set $S$ is infiniteif and only if$S$ is equivalent to a proper subset of $S$
...
$$
  \mathcal{P} := \left{X \;|\; X\subseteq S \right\}
$$
</MathBox>
...
$$
  \bigcap_{i=1}^n p_{Y_i}^{-1} (A_i) = \left\{ x = \left( x_{Y_i} \right)_{i=1}^n \in X \;|\; x_{Y_i}\in A_i \subseteq Y_i \right\}
$$
...
$$
  S_1 \times S_2 \times \dots S_n = \prod_{i=1}^n S_i = \left\{ \left( x_i \right)_{i=1}^n \;|\; x_i \in S_i \right\}
$$
...
$$
  \prod_{i\in I} S_i := \left\{ x: I\to\bigcup_{i\in I} S_i \mid| \forall i \in I: x(i)\in X_i \right\}
$$
...
**Cross sections**
1. The cross section of $C$ in the first coordinate at $x\in S$ is $C_x = \{y\in T \;|\; (x, y)\in C\}\subseteq T$
2. The cross section of $C$ in the second coordinate at $y\in S$ is $C^y = \{x\in S \;|\; (x, y)\in C\}\subseteq S$

**Projections**
1. The projection of $C$ onto $T$ is $C_T = \{y\in T \;|\; (x, y)\in C\textrm{ for some }x\in S \}$
2. The projection of $C$ onto $S$ is $C^S = \{x\in S \;|\; (x, y)\in C\textrm{ for some }y\in T \}$
...
1. the domain of $R$ is the set of first relation coordinates: $\mathrm{dom}(R) := \{ x\in X \;|\; (x, y)\in R,\; y\in Y \}$
2. the range of $R$ is the set of second relation coordinates: $\mathrm{ran}(R) := \{ y\in Y \;|\; (x, y)\in R,\; x\in X \}$

A relation $R$ from $X$ to $Y$ is usually denoted $xRy$ for $(x, y)\in R$.
</MathBox>

<MathBox title='Inverse relation' boxType='definition'>
If $R$ is a relation from $X$ to $Y$, the inverse of $R$ is the relation from $Y$ to $X$ defined by $R^{-1} := \{(y, x) \;|\; (x, y) \in R\}$
</MathBox>
...
A partial order $\leq$ on $S$ is a total or linear order if for every $x, y\in S$ either $x\leq y$ or $y\leq x$.
</MathBox>
...
2. $a\in A$ is a minimal element of $A$if and only ifno $x\in A$ satisfies $x < a$.
3. $b\in A$ is the maximum element of $A$if and only if$b\geq x$ for every $x\in A$, and is denoted $\max(A)$.
4. $b\in A$ is a maximal element of $A$if and only ifno $x\in A$ satisfies $x > b$.
</MathBox>
...
Let $A = \{x\in S\;|\; x\leq f(x)\}$ and let $z = \sup(A)$. If $x\in A$, then $x\leq z$ so $x\leq f(x) \leq f(z)$. Hence $f(z)$ is an upper bound of $A$ so $z\leq f(z)$. However $f(z)\leq f(f(z))$, since $f$ is increasing, so $f(z)\in A$ and hence $f(z)\leq z$. Therefore $f(z) = z$ showing that $z$ is a fixed point of $f$.
</details>
</MathBox>
...
1. $f$ is increasingif and only if$x\leq_X \tilde{x} \implies f(x)\leq_Y f(\tilde{x})$
2. $f$ is increasingif and only if$x\leq_X \tilde{x} \implies f(x)\geq_Y f(\tilde{x})$
</MathBox>
...
Suppose $\mathcal{S}$ is collection of sets. Define a relation $\sim$ on $\mathcal{S}$ by $A\sim B$ for $A, B\in\mathcal{S}$if and only ifthere is a bijection $f: A\to B$. The relation $\sim$ is an equivalence relation on $S$ satisfying for $A, B, C\in\mathcal{S}$
1. reflexivity: $A\sim A$
...
2. the quotient set of $X$ under $\sim$ is $X\setminus\sim := \left\{ f^{-1}[y] \;|\; y \in \mathrm{ran}(f) \right\}$
...
**(2)**: Recall that $y\in\mathrm{ran}(f)$if and only if$f(x) = y$ for some $x\in X$. Then by definition, $[x] = f^{-1}[y] = \left\{\tilde{x}\in X\;|\; f\left(\tilde{x}\right) = y\right\} = \left\{ \tilde{x}\in X \;|\; f\left(\tilde{x}\right) = f(x) \right\}$ 
</details>
</MathBox>
...
Consider a function $f: X\to Y$ and a subset $A\subseteq X$. The restriction of $f$ to $A$ is the function $f_A: A\to Y$ defined by

$$
  f_A := \{(x, y) \in f \;|\; x\in A \}
$$

The image $f[A]$ is the range of $f_A$.
</MathBox>

## Images
<MathBox title='Function subsets' boxType='proposition'>
A function $f:X\to Y$ generates the following subsets
- Range: $\mathrm{Ran}(f) := \left\{ y \in Y \;|\; \exists x \in X : f(x) = y \right\} = \{ f(x) \;|\; x \in X \}$
- Image: $f[\tilde{X}] := \left\{ y \in Y \;|\; \exists x \in \tilde{X} : f(x) = y \right\} = \{ f(x) \;|\; x \in \tilde{X} \}$ for $\tilde{X} \subseteq X$
- Preimage: $f^{-1}[\tilde{Y}] := \left\{ x \in X \;|\; f(x) = \tilde{Y} \right\}$ for $\tilde{Y} \subseteq Y$
...
Property 1 and 2 extends to arbitrary intersections and unions, respectively. Consider the collection of subsets $\{A_i \subseteq S \;|\; i\in I\}$ for some index set $I$. Then
1. $\mathbf{A}_{\bigcap_{i\in I} A_i} = \prod_{i\in I} \mathbf{1}_{A_i} = \min\{\mathbf{1}_{A_i} \;|\; i\in I\}$
2. $\mathbf{A}_{\bigcup_{i\in I} A_i} = 1 - \prod_{i\in I} \left(1 - \mathbf{1}_{A_i}\right) = \max\{\mathbf{1}_{A_i} \;|\; i\in I\}$
</MathBox>
