...
If $(a, b) = (c, d)$ then $\Set{\Set{a}, \Set{a, b}} = \Set{\{c}, \Set{c, d}}$. In the case that $a = b$ we note that 

$$
  (a, b) = \Set{\Set{a}, \Set{a, b}} = \Set{\{a}, \Set{a, a}} = \Set{\{a}, \Set{a}} = \Set{\{a}\}
$$

Thus, we must have $\Set{c} = \Set{c, d} = \Set{a}$ and hence $c = d = a = b$. 

In the case that $a\neq b$ we must have $\Set{a} = \Set{c}$ and hence $c = a$. Suppose
- $\Set{c, d} = \Set{a}$ gives $c = d = a$ and $(c, d) = \Set{\{a}\}$. Hence $\Set{a, b} = \Set{a}$ which forces $a = b$, contradicting $a\neq b$.
...
$$
  \prod_{i\in I} S_i := \Set{ x: I\to\bigcup_{i\in I} S_i \;|dle|\; \forall i \in I: x(i)\in X_i }
$$
...
**(2)**: Recall that $y\in\mathrm{ran}(f)$ if and only if $f(x) = y$ for some $x\in X$. Then by definition, $[x] = f^{-1}[y] = \Set{\tilde{x}\in X| f\left(\tilde{x}\right) = y} = \left\{ \tilde{x}\in X \mid f\left(\tilde{x}\right) = f(x) }$ 
</details>
</MathBox>
