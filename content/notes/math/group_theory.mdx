...
The set of congruence classes module $n\in\N_+$
...
showing that modulus addition is associative. The identity element is $[0]_n$, while inverse elements are of the form $[a]_n^{-1} = [-a]_n$ for all $a\in\Z$.
...
<MathBox title='Quotient group' boxType='proposition'>
...
Write the cosets as left cosets, and define $(aN)(bN) = (ab)N$. We have to check **(1)** that this is well-defined, and **(1)** that it gives a group structure on the set of cosets. It will then be obvious that the map $g\mapsto gN$ is a homomorphism with kernel $N$.
...
<MathBox title='Universal property of quotient groups' boxType='proposition'>
...
## Homorphism/isomorphism theorems
...
By Proposition $\ref{}$, the kernel of $\alpha$ is a normal subgroup of $G$. If $b = \alpha(a)$ and $b' = \alpha(a')$, then $bb' = \alpha(aa')$ and $b^{-1} = \alpha(a^{-1})$, and so $I := \alpha(G)$ is a subgroup of $G'$. The universal property of quotients (Proposition $\ref{}$) shows that the map $x\mapsto \alpha(x):G\to I$ defines a homomorphism $\bar{\alpha}:G/N\to I$ with $\bar{\alpha}(gN) = \alpha(g)$. The homomorphism $\bar{\alpha}$ is certainly surjective, and if $\bar{\alpha} (gN) = e$, then $g\in\ker(\alpha) = N$. Thus, $\bar{\alpha}$ has a trivial kernel, making it injective. 
</details>
</MathBox>

<MathBox title='First osomorphism theorem' boxType='theorem'>
Let $G$ be a group, $H$ a subgroup of $G$ and $N$ a normal subgroup of $G$. Then $H\cap N$ is normal in $H$ and $H/(H\cap N) \cong NH/N$.

<details>
<summary>Proof</summary>

By Proposition $\ref{}$, $HN$ is a subgroup. Consider the map $h\mapsto hN: H\to G/N$. This is a homomorphism, and its kernel is $H\cap N$, which is therefore normal in $H$. By Theorem $\ref{theorem-2}$, the map induces an isomorphism $H/(H\cap N) \to I$, where $I$ is its image. However, $I$ is the set of cosets of the form $hN$ with $h\in H$, i.e., $I = HN/N$.
</details>
</MathBox>
...
under which a subgroup $H$ of $G$ containing $N$ corresponds to $\bar{H} = \alpha(H)$ and a subgroup $\bar{H}$ of $\bar{G}$ corresponds to $H = \alpha^{-1} (\bar{H})$. Moreover, if $H \leftright \bar{H}$ and $H' \leftright \bar{H}'$, then
...
This is a special case of the correspondence theorem ($\ref{}$) with $\alpha:G\to G/N$ given by $g\mapsto gN$.
</details>
</MathBox>
