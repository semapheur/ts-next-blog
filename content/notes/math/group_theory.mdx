...
\begin{split}
  ...
  gkH \underbrace{=}_{\eqref{\equation-}}& \set{gkh}_{h\in H} = \set{gh'}_{h' \in H} = gH
\end{split}
...
From Theorem $\ref{\theorem-}$ we know that there are exactly $|G|/|H|$ distinct cosets in $\set{[g]_H}_{g\in G}$, which is the set of elements of the group $G/H$.
</details>
</MathBox>
...
where the isomorphism is given by the map $\hat{\phi}: G_1 /\ker(\phi) \to \phi\set{G_1}$ as
...
$$
\begin{align*}
  \hat{\phi}([g_a]_{\ker(\phi)}) \underbrace{=}_{\eqref{equation-48}}& \phi(g_a) \underbrace{=}_{\eqref{equation-49}} \\
  \underbrace{=}_{\eqref{equation-41}}& \phi(g_b) \phi(h) \underbrace{=}_{h\in\ker(\phi)} \phi(g_b) e_2 \\
  \underbrace{=}_{\eqref{equation-8}} \phi(g_b) \underbrace{=}_{\eqref{equation-48}} \hat{\phi}([g_b]_{\ker(\phi)})
\end{align*}
$$

proving that $\hat{\phi}$ is well defined. Injectivity of $\hat{\phi}$ is proven by the following chain of implications for $g_c, g_d \in G_1$

$$
\begin{align*}
  \hat{\phi}([g_c]_{\ker(\phi)}) = \hat{\phi}([g_d]_{\ker(\phi)}) \underbrace{\implies}_{\eqref{equation-48}}& \phi(g_c) = \phi(g_d) \\
...
$$
  h = \phi(g) \underbrace{=}_{\eqref{equation-48}} = \hat{\phi}([g]_{\ker(\phi)})
$$

proving that $\hat{\phi}$ is surjective. It remains to show that $\hat{\phi}$ is a homomorphism. For this consider

$$
\begin{align*}
  \hat{\phi}([g_1]_{\ker(\phi)} [g_2]_{\ker(\phi)}) \underbrace{=}_{\eqref{equation-31}}& \hat{\phi}([g_1 g_2]_{\ker(\phi)}) \\
  \underbrace{=}_{\eqref{equation-48}}& \phi(g_1 g_2) \underbrace{=}_{\eqref{equation-41}} \phi(g_1) \phi(g_2) \\
  \underbrace{=}_{\eqref{equation-48}}& \hat{\phi}([g_1]_{\ker(\phi)}) \hat{\phi}([g_2]_{\ker(\phi)})
\end{align*}
$$

which shows that $\hat{\phi} \in\operatorname{Hom}_{G_1 / \ker(\phi), \phi\set{G_1}}$.
</details>
</MathBox>
...
The characters $\hat{G} = \operatorname{Hom}(G,\operatorname{U}(1))$ of an Abelian group $G$ form an Abelian group with the trivial character $1_G$ as the identity element and with the group operation $*: \hat{G}\times\hat{G}\to\hat{G}$ given by
...
We show that $\hat{G}$ satisfies the group axioms. 

**Closure:** For $\chi_1, \chi_2 \in\hat{G}$ and $g_1, g_2 \in G$, we find
...
**Identity:** For any $\chi\in\hat{G}$ and $g\in G$ we have

$$
  \chi 1_G (g) \underbrace{=}_{\eqref{equation-57}} \chi(g) 1_G (g) \underbrace{=}_{\eqref{equation-58}} \chi(g)
$$

showing that $\hat{1}_G$ is the identity element.

**Inverses:** For any $\chi\in\hat{G}$ its inverse is given by its conjugate character $\overline{\chi} \in\hat{G}$ since for any $g\in G$
...
Let $G$ be an Abelian group. The group $\hat{G} := \operatorname{Hom}(G, \operatorname{U}(1))$ formed by its characters with the group operation in $\eqref{equation-56}$ and $\eqref{equation-57}$ is the *dual* or *character group* of $G$.
</MathBox>
...
which exists since at least $g_1^m = e_G \in H$ for some finite $m\in\N$. Consequently, for any charcter $\chi_H \in \hat{H}$, we have that $\chi(g_1^k) = e^{i\alpha} \in\operatorname{U}(1)$ for some $\alpha\in\R$. There are $k$ $k$-th roots of $e^{i\alpha}$, which are of the form
...
For each such $l$ we want to define a character $\chi_l \in\hat{H}_l$, which acts on a generic element $hg_1^j \in H_1$ by
...
We choose any of these extensions and denote it by $\chi_{H_1}$. We thus have a subgroup $H_1$ and a character $\chi_{H_1} \in\hat{H}_1$. Now we pick any $g_2 \in G\setminus H_1$ and repeat the previous construction with $H_2 = \braket{H_1, g_2}$ leading to a character $\chi_{H_2} \in \hat{H}_2$ that satisfies $\chi_{H_2} |_{H_1} = \chi_{H_1}$. We continue with this until there is no more $g_{n+1} \in G\setminus H_n$ to be found, which must happend after a finite number of steps since $G$ is assumed finite.

Altogether, we thus have a sequence of subgroups $H_r = \braket{H_{r-1}, g_{r-1}}$, elements $g_r \in G\setminus H_{r-1}$ and characters $\chi_{H_r} \in \hat{H}_r$ for $r\in\set{0,\dots,n}$ such that
...
<MathBox title='' boxType='corollary'>
The dual group $\hat{G}$ of a finite Abelian group $G$ satisfies
...
ways. Any character $\chi\in\hat{G}$, when restricted to $\braket{e}$, is equal to $1_{\braket{e}}$. Hence, it must be one of the $|G|$ extensions constructed from $1_{\braket{e}}$ since, otherwise, there would be more than $|G|$ extensions. Hence, the number of characters is $|G|$.
</details>
</MathBox>

<MathBox title='' boxType='corollary'>
Let $G$ be a finite Abelian group with identity element $e$ and $g\in G\setminus\braket{e}$. Then there exists a character $\chi\in\hat{G}$ such that $g\notin\ker(\chi)$.
...
$$
\begin{equation*}
  H^\perp := \set{\chi\in\hat{G} | H \subseteq\ker(\chi)}
...
Let $e_{\hat{G}}$ denote the neutral element of the dual group $\hat{G}$. Since $e_{\hat{G}} = 1_G$ and $\ker(1_G) = G$, we have $H\subseteq\ker(e_{\hat{G}})$ and thus $e_{\hat{G}} \in H^\perp$.
...
Consequently, $\chi^{-1} = \overline{\chi} \in H^\perp$, verifying $\eqref{equation-}$
</details>
</MathBox>

<MathBox title='' boxType='lemma'>
Let $H\leq G$ be a subgroup of the finite Abelian group $G$ and let $H^\perp$ be as defined in $\eqref{equation-}$. Then
...
<MathBox title='' boxType='corollary'>
Let $G$ be a finite Abelian group and $\hat{G}$ its dual group. For any $\chi_1, \chi_2 \in\hat{G}$ we then have
...
$$
\begin{align*}
  \sum_{g\in G} \chi_1 (g) \chi_2 (g) \underbrace{=}_{\eqref{equation-}} \sum_{g\in G} \chi_1 \chi_2 (g)\\
  \underbrace{=}_{\eqref{equation-69}}& \begin{cases}\
    |G|,\quad& \chi_1 \chi_2 \in G^\perp \underbrace{=}_{\eqref{equation-}} \braket{e_{\hat{G}}} \\
    0,\quad& \text{else}
  \end{cases} \\
  =& \begin{cases}
    |G|,\quad& \chi_2 = \chi_1^{-1} \\
