...
If $U\in\R^n$ is an open set, a real-valued function $f:U\to\R$ is differentiable at a point $\mathbf{a}\in U$ if there exists a real-valued affine function $\ell:\R^n \to \R$ given by $\ell(\mathbf{x}) = T(\mathbf{x}) + b$ such that

$$
  \lim_{\mathbf{x}\to\mathbf{a}} \frac{f(\mathbf{x}) - \ell(\mathbf{x})}{\lVert \mathbf{x} - \mathbf{a}\rVert} = 0
$$

The affine function $\ell$ gives the linear approximation of $f$ at $\mathbf{a}$ such that $f(\mathbf{a}) = \ell(\mathbf{a}) = T(\mathbf{a}) + b$. Thus, $b = f(\mathbf{a}) - T(\mathbf{a})$ such that
...
In a neighbourhood of $\mathbf{a}$, $\mathbf{x}$ takes the form $\mathbf{x} = \mathbf{a} + h\sum_{i=1}^n \mathbf{e}_i$. Suppose that $\mathbf{x}$ approaches $\mathbf{a}$ in the $x_i$-direction, i.e. $\mathbf{x} = \mathbf{a} + h\mathbf{e}_i$. Then $\mathbf{x} - \mathbf{a} = h\mathbf{e}_1$ and $\lVert \mathbf{x} - \mathbf{a} \rVert = |h|$. For the limit to hold true then we require in both limits $h\downarrow 0$ and $h\uparrow 0$ that
...
The linear transformation $T$ takes the form of $1\times n$ matrix
...
called the *Jacobian matrix* of $f$ at $\mathbf{a}$.
