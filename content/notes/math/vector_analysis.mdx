...
$$
  B_r(\mathbf{a}) = \Set{ \mathbf{x}\in\R | \norm{ \mathbf{x} - \mathbf{a}} < r }
$$
</MathBox>
...
Suppose $(\mathbf{x}_n)_{n\in\N}$ and $(\mathbf{y}_n)_{n\in\N}$ are sequences in $\R^m$ with $\lim_{n\to\infty} \mathbf{x}_n = \mathbf{x}$ and $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$. Then the following sequences converge
...
**(2):** Since $\lim_{n\to\infty} \mathbf{x}_n = \mathbf{x}$, there is $N_1 \in\N$ such that $\norm{ \mathbf{x}_n - \mathbf{x}} < \frac{\epsilon}{2}$ for all $n\geq N_1$. Similary, since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$, there is $N_2 \in\N$ such that $\norm{ \mathbf{y}_n - \mathbf{y}} < \frac{\epsilon}{2}$ for all $n\geq N_1$. For $N = \max\Set{N_1, N_2}$, we then get using the triangle inequality

$$
\begin{align*}
  \norm{(\mathbf{x}_n + \mathbf{y}_n) - (\mathbf{x} + \mathbf{y})} =& \norm{(\mathbf{x}_n - \mathbf{x}) + (\mathbf{y}_n - \mathbf{y})} \\
  \tleq& \norm{\mathbf{x}_n - \mathbf{x}\| +}\mathbf{y}_n - \mathbf{y}_n| \\
...
\begin{align*}
  ...
  \tleq& |\mathbf{x}_n \cdot \mathbf{y}_n - \mathbf{x}\cdot\mathbf{y}_n| + |\mathbf{x}\cdot\mathbf{y}_n - \mathbf{x}\cdot\mathbf{y} | \\
  \leq& \norm{ \mathbf{x}_n - \mathbf{x}} \cdot \norm{ \mathbf{y}_n} + \norm{ \mathbf{x}} \cdot \norm{ \mathbf{y}_n - \mathbf{y}}
\end{align*}
$$

Assume $\norm{ \mathbf{x}} \neq 0$. Since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$ there is $N_1\in\N$ such that $\norm{ \mathbf{y}_n - \mathbf{y}}\frac{\varepsilon}{2 \|\mathbf{x}\|}$ for all $n\geq N_1$.

Since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$, there is $N_2 \in\N$ such that $\|\mathbf{y}_n - \mathbf{y}\| \leq 1$ when $n \geq N_2$. Thus

$$
\begin{align*}
  \norm{ \mathbf{y}_n} =& \norm{ \mathbf{y} + (\mathbf{y}_n - \mathbf{y})} \\
  \tleq& \norm{ \mathbf{y}} + \norm{ \mathbf{y}_n - \mathbf{y}} \\
  \leq& \norm{ \mathbf{y} + 1}
...
Since $\lim_{n\to\infty} \mathbf{x}_n = \mathbf{x}$, there exists $N_3 \in\N$ such that $\norm{ \mathbf{x}_n - \mathbf{x}} \leq \frac{\varepsilon}{2(\|\mathbf{y}\| + 1)}$. If $n\geq\max\Set{N_1,N_2}$ then

$$
  \norm{ \mathbf{x}_n - \mathbf{x}}\cdot \norm{ \mathbf{y}_n} < \frac{\varepsilon}{2(|\mathbf{y}| + 1|)} |\mathbf{y}| + 1| = \frac{\varepsilon}{2}
$$
...
$$
\begin{align*}
  | \mathbf{x}_n \cdot \mathbf{y}_n - \mathbf{x}\cdot\mathbf{y} | \leq& \norm{ \mathbf{x}_n - \mathbf{x}}\cdot\norm{ \mathbf{y}_n} + \norm{ \mathbf{x}}\cdot \norm{ \mathbf{y}_n - \mathbf{y}} \\
  <& \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon
\end{align*}
$$
</details>
...
provided the limit exists. The derivative is also called the velocity $\mathbf{v}(t) := \alpha'(t) = \frac{\d\alpha}{\d t}$ of $\alpha$. The norm of $\mathbf{v}$ is called the speed $v$ of $\alpha$ and is defined as
...
$$
  s(t) := \int_a^b v(t)\;\d t = \int_a^b \|\mathbf{v}(t)\|\;\d t
$$
</MathBox>
...
$$
  \lim_{\Delta t \to \infty} \sum_i \frac{\left\| \mathbf{R} \left(t_{i + 1} \right) - \mathbf{R} \left(t_i \right) \right\|}{\Delta t} \Delta t = \int \left\| \frac{\d\mathbf{\mathbf{R}}}{\d t} \right\| \d t
$$
...
$$
\begin{align*}
  \left\| \frac{\d\mathbf{R}}{\d t} \right\|^2 = \frac{\d\mathbf{R}}{\d t} \cdot \frac{\d\mathbf{R}}{\d t} &= \left( \frac{\d x^i}{\d t} \frac{\partial \mathbf{R}}{\partial x^i} \right) \cdot \left( \frac{\d x^j}{\d t} \frac{\partial \mathbf{R}}{\partial x^j} \right) \\
  &= \frac{\d x^i}{\d t} \frac{\d x^j}{\d t} \left( \frac{\partial \mathbf{R}}{\partial x^i} \cdot \frac{\partial \mathbf{R}}{\partial x^j} \right) \\
  &= \frac{\d x^i}{\d t} \frac{\d x^j}{\d t} g_{ij}
\end{align*}
$$
...
$$
  \left\| \frac{\d\mathbf{r}}{\d t} \right\| = \sqrt{\sum_i \sum_j \frac{\d x_i}{\d t} \frac{\d x_j}{\d t} \left( \frac{\partial \mathbf{r}}{\partial x_i} \frac{\partial \mathbf{r}}{\partial x_j} \right)} - \sqrt{\frac{\d x_i}{\d t} \frac{\d x_j}{\d t} \left( \frac{\partial \mathbf{r}}{\partial x_i} \frac{\partial \mathbf{r}}{\partial x_j} \right)}
$$
...
(a) Since $\mathbf{B}$ is a unit vector, $\norm{ \mathbf{B}}$ is constant. Hence $\mathbf{B}$ and its derivative $\mathbf{B}'$ are always orthogonal.
...
Let $\alpha:I\to\R^3$ be a differentiable path with speed $v$, Frenet vectors $(\mathbf{T}, \mathbf{N}, \mathbf{B})$, curvature $\kappa$ and torsion $\tau$. Assume $v\neq 0$ and $\mathbf{T}' \neq\mathbf{0}$ so that the Frenet vectors are defined for all $t$. Then
...
$$
  \frac{\d(QR)}{\d t}(QM)^T = \frac{\d Q}{\d t}RR^T Q^T = \frac{\d Q}{\d t} Q^T
$$

Hence the entries $\kappa$ and $\tau$ of $\frac{\d Q}{\d t}Q^T$ (Frenet-Serret formulas) are preserved under rotations. 
...
$$
\begin{gather*}
  \tilde{\mathbf{T}}(t)\cdot\mathbf{T}(t) = \norm{\tilde{\mathbf{T}}(t)\| \cdot}\mathbf{T}(t)\|\cos\theta \\
...
\begin{align*}
  ...
  &\tleq \left|f(\mathbf{x}) - f(\mathbf{a})\right| + \left|g(\mathbf{x}) - g(\mathbf{a})\right|
\end{align*}
$$
...
Suppose $\lim_{\mathbf{x}\to\mathbf{a}}f(\mathbf{x})$ and $\lim_{\mathbf{x}\to\mathbf{a}}f(\mathbf{x})$ both exist for real-valued functions $f,g:\R^n \to\R$.
...
Suppose $f:U\subseteq\R^n \to\R$ is defined on an open set $U$ except possibly at $\mathbf{a}\in U$, and that $\lim_{\mathbf{x}\to\mathbf{a}} f(\mathbf{x}) = L$. Let $\alpha: I\subseteq\R \to U$ be a continuous path in $U$ passing through $\mathbf{a}$, i.e. $\alpha(t_0) = \mathbf{a}$ for some $t_0 \in I$. Consider that function $g:I\to\R$ given by
...
Suppose $\mathbf{x}$ approaches $\mathbf{a}$ in the $x_i$-direction, i.e. $\mathbf{x} = \mathbf{a} + h\mathbf{e}_i$. Then $\mathbf{x} - \mathbf{a} = h\mathbf{e}_1$ and $\norm{ \mathbf{x} - \mathbf{a}} = |h|$. For the difference quotient to hold true then we require in both limits $h\downarrow 0$ and $h\uparrow 0$ that
...
$$
  \D f(\mathbf{a}) = \begin{bmatrix} \frac{\partial f}{\partial x^1}(\mathbf{a}) & \cdots & \frac{\partial f}{\partial x_n}(\mathbf{a}) \end{bmatrix}
$$
...
$$
  \lim_{\mathbf{x}\to\mathbf{a}}\frac{f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a})}{\| \mathbf{x} - \mathbf{a}\|} = 0
$$

If this limit exists, the matrix $\D f(\mathbf{a})$ is called the *derivative* of $f$ at $\mathbf{a}$. It is also known as the *Jacobian matrix*. The affine function $\ell(\mathbf{x}) = f(\mathbf{a}) + \D f(\mathbf{a})(\mathbf{x}-\mathbf{a})$ is called the *first-order affine transformation* of $f$ at $\mathbf{a}$.
</MathBox>
...
$$
  Q(\mathbf{x}) = \begin{cases}
    \frac{f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a})}{\norm{ \mathbf{x} - \mathbf{a}}}, \quad& \mathbf{x}\neq\mathbf{a} \\
...
$$
\begin{align*}
  |f(\mathbf{x}) - f(\mathbf{a})| =& |Q(\mathbf{x})\norm{ \mathbf{x} - \mathbf{a}} + \nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})| \\
  \tleq& |Q(\mathbf{x})\norm{ \mathbf{x} - \mathbf{a}}| + |\nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})| \\
  \leq& |Q(\mathbf{x})|\cdot\norm{ \mathbf{x} - \mathbf{a}} + \norm{ \nabla f(\mathbf{a})\| \cdot}\mathbf{x} - \mathbf{a}\|
\end{align*}
$$
...
\begin{align*}
  ...
  \lim_{\mathbf{x}\to\mathbf{a}}|Q(\mathbf{x})|\cdot\norm{ \mathbf{x} - \mathbf{a}} \\
...
$$
  \D f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a}) = \nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})
$$
...
$$
\begin{align*}
  f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a}) =& \nabla f(\mathbf{c})\cdot (\mathbf{x} - \mathbf{a}) - \nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a}) \\
  =& (\nabla f(\mathbf{c}) - \nabla f(\mathbf{a}))\cdot (\mathbf{x} - \mathbf{a})
\end{align*}
$$
...
$$
\begin{align*}
  \frac{|f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a})|}{\norm{ \mathbf{x} - \mathbf{a}}} =& \frac{|(\nabla f(\mathbf{c}) - \nabla f(\mathbf{a}))\cdot (\mathbf{x} - \mathbf{a})|}{\norm{ \mathbf{x} - \mathbf{a}}} \\
  \leq& \norm{ \nabla f(\mathbf{c}) - \nabla f(\mathbf{a})} \frac{\norm{\mathbf{x} - \mathbf{a}\|}{\| \mathbf{x} - \mathbf{a}}} \\
  =& \norm{ \nabla f(\mathbf{c}) - \nabla f(\mathbf{a})}
\end{align*}
$$

Since $\norm{ \mathbf{c} - \mathbf{a}} \leq \norm{ \mathbf{x} - \mathbf{a}}$, it follows that $\mathbf{c}\xrightarrow{\mathbf{x}\to \mathbf{a}} \mathbf{a}$. By continuity of partial derivatives, this mean that $\nabla f(\mathbf{c}) \xrightarrow{\mathbf{x}\to\mathbf{a}} \nabla f(\mathbf{a})$. Hence

$$
\begin{align*}
  &\lim_{\mathbf{x}\to\mathbf{a}} \frac{|f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a})|}{\norm{ \mathbf{x} - \mathbf{a}}} \\
...
If a scalar function $f:U\subseteq\R^n \to\R$ is differentiable at $\mathbf{a}\in U$, its total derivative at this point is the linear transformation $\D f(\mathbf{a}):\R^n \to\R$ in the form of the $1\times n$ Jacobian matrix

$$
  \D f(\mathbf{a}) = \begin{bmatrix} \frac{\partial f}{\partial x^1}(\mathbf{a}) & \cdots & \frac{\partial f}{\partial x^n}(\mathbf{a}) \end{bmatrix}
$$

Transposing $\D f(\mathbf{a})$ gives a vector ($n\times 1$ matrix) called the *gradient* of $f$ at $\mathbf{a}$, denoted

$$
\begin{align*}
  \nabla f(\mathbf{a}) =& \D f(\mathbf{a})^T \\
...
On a sidenote, since $\D f(\mathbf{a})$ maps vectors in $\R^n$ into $\R$ it is technically a differential 1-form, or a linear functional, identified with dual vector space $(\R^n)^*$, which is the set of all linear functionals. The total derivative $\D f(\mathbf{a})$ and the gradient $\nabla f(\mathbf{a})$ are thus dual to each other. 

<MathBox title="Gradient" boxType="definition">
If a function $f:U\subseteq\R^n \to\R$ is totally differentiable at $\mathbf{a} \in\R^n$, then the gradient of $f$ at $\mathbf{a}$ is defined as
...
$$
\begin{align*}
  (f\circ t)'(t) =& \frac{\d}{\mathbf{d}t} f(\alpha(t)) = \frac{\d}{\mathbf{d}t} f(x_i(t))_{i=1}^n \\
...
$$
  \alpha'(t_0) = \lim_{t\to t_0} \frac{\norm{ \alpha(t) - \alpha(t_0)}}{t - t_0} =  \lim_{t\to t_0} \frac{\norm{ \mathbf{x} - \mathbf{a}}}{t - t_0}
$$
...
\begin{align*}
  ...
  =& \lim_{t \to t_0} \frac{|f(\alpha(t)) - f(\alpha(t_0)) - \nabla f(\alpha(t_0))\cdot(\alpha(t) - \alpha(t_0))|}{\norm{ \alpha(t) - \alpha(t_0)}}
\end{align*}
$$
...
$$
\begin{align*}
  \D_\mathbf{v}f =& \left.\frac{\d}{\d t}f(\mathbf{a} + t\mathbf{v})\right|_{t=0} = (f\circ\alpha)'(0) \\
...
Suppose $\mathbf{v}$ is a vector tangent to $S$ at $\mathbf{a}$ and that $\alpha:I\subseteq \R \to S$ is path that lies in $S$ passing through $\mathbf{a}$ with veloctiy $\mathbf{v}$. That is, $\alpha(t_0) = \mathbf{a}$ for some $t_0 \in I$ and $\alpha'(t_0) = \mathbf{v}$. Then $f(\alpha(t)) = c$ for all $t\in I$, so $(f\circ a)'(t) = c' = 0$. By the little chain rule
...
Without loss of generality, we can assume $f: U\subseteq\R^2 \to\R$, since the second-order mixed partials treat all but two variables as fixed. Suppose $f_{xy} = \frac{\partial f^2}{\partial x \partial y}$ and $f_{yx} \frac{\partial f^2}{\partial x \partial y}$ exists in a neighbourhood of $\mathbf{a} = (a,b) \in U$ and are continuous at $\mathbf{a}$.
...
<MathBox title='' boxType="proposition">
Suppose $f:U\subseteq\R^n \to\R$ has a local extrema at $\mathbf{a}\in U$. If $f$ is differentiable at $\mathbf{a}$, then $\nabla f(\mathbf{a}) = \mathbf{0}$, i.e. $\frac{\partial f}{\partial x_i}(\mathbf{a}) = 0$ for all $i$.

<details>
<summary>Proof</summary>

Suppose $f$ has a local extrema at $\mathbf{a} = (a_i)_{i=1}^n$. Define a function $g:\R \to\R$ by
...
Suppose $f:U\subseteq\R^n \to\R$ is second-order differentiable at $\mathbf{a}\in U$. The *Hessian matrix* of $f$ at $\mathbf{a}$ is an $n\times n$ matrix of all the second-order partial derivatives of $f$ at $\mathbf{a}$
...
Suppose $f:U\subseteq\R^n \to\R$ has continuous second-order partial derivatives in an open ball $B_r(\mathbf{a})$ centered at $\mathbf{a}\in U$. 
...
Suppose $f:U\subseteq\R^n \to\R$ has a critical point at $\mathbf{a}\in U$ and that the second-order partial derivatives of $f$ are continuous in a neighbourhood of $\mathbf{a}$.
...
Suppose $f:U\subseteq\R^2 \to\R$ has a critical point at $\mathbf{a}\in U$ and that the second-order partial derivatives of $f$ are continuous in a neighbourhood of $\mathbf{a}$. 
...
Suppose $f, g_1, \dots, g_k:U\subseteq\R^n \to\R$ for $k\in\N_+$ are continunously differentiable functions on an open subset $\R^n$. If $\mathbf{a}$ is a local extrema for $f$ on the subset
...
Assume $1 \geq k < n$. It suffices to show that if the gradients $(\nabla g_i(\mathbf{a}))_{i=1}^k$ are linearly independent for $\mathbf{a} = (a_i)_{i=1}^n$, there are constants $(\lambda_i)_{i=1}^k$ such that 

$$
  \nabla f(\mathbf{a}) = \sum_{i=1}^k \lambda_i \nabla g_i(\mathbf{a}) 
$$

For any constraint $g_i(\mathbf{x}) = b_i$ for $i=1,\dots,k$ we may define a function $\tilde{g}_i (\mathbf{x}) = g_i(\mathbf{x} - b$, giving the constraint $\tilde{g}_i(\mathbf{x}) = 0$. We can therefore without loss of generality assume $b_i = 0$.

Defining $\mathbf{G}:\R^n \to\R^k$ by $\mathbf{G}(\mathbf{x}) = (g_i(\mathbf{x}))_{i=1}^k$, we can concisely write the constraint equations as $\mathbf{G}(\mathbf{x}) = \mathbf{0}$. Differentiating $\mathbf{G}$ we get the Jacobian $k\times n$ matrix $\mathbf{G}'(\mathbf{x}) = (\nabla g_i(\mathbf{x}))_{i=1}^k$. Since the gradients are linearly independent at $\mathbf{a}$, it follows by the rank theorem that $\mathbf{G}'(\mathbf{a})$ has $k$ independent columns. By reindexing if necessary, we may assume the $k$ last columns are linearly independent. 
...
Assume we want to find a local minimal point of a function $f:U\subseteq\R^n \to\R$ starting from $\mathbf{x}_0\in U$. Since the gradient $\nabla f(\mathbf{x}_0)$ gives the direction of the steepest ascent, we want to travel in the opposite direction along the line
...
\begin{aligned}
  ...
  \int \nabla \cdot \left(f \mathbf{A} \right) \d\tau &= \int f\left( \nabla \cdot \mathbf{A} \right) \d\tau + \int \mathbf{A} \cdot \left( \nabla f \right) \d\tau = \oint f\mathbf{A} \cdot \d\mathbf{a}
\end{aligned} \\
  \int_\mathcal{V} \nabla \cdot \left(f \mathbf{A} \right) \d\tau = -\int_\mathcal{V} \mathbf{A} \cdot \left( \nabla f \right) \d\tau + \oint_\mathcal{S} f\mathbf{A} \cdot \d\mathbf{a}
\end{gather*}
$$
...
<MathBox title="Local Lipschitz continuity" boxType="definition">
A vector field $\mathbf{V}: U\subseteq \R^n \to\R^n$, where $U$ is an open subset of $\R^$, is locally Lipschitz continuous if for every point $\mathbf{x}\in U$ there exists constants $\epsilon > 0$ and $L > 0$ such that $B_\epsilon (\mathbf{x}) \subseteq U$ and

$$
\begin{equation*}
  \norm{\mathbf{V}(\mathbf{y}) - \mathbf{V}(\mathbf{z})} \leq L \norm{\mathbf{y} - \mathbf{z}}
\tag{\label{equation-1}}
\end{equation*}
$$

for all $\mathbf{y}, \mathbf{z} \in B_\epsilon (\mathbf{x})$, where 

$$
  B_\epsilon (\mathbf{x}) = \set{\mathbf{u}\in\R^n : \norm{\mathbf{u} - \mathbf{x}} < \epsilon}
$$

denotes the open Euclidean ball centered at $\mathbf{x}$, and $\norm{\cdot}$ is the standard Euclidean norm on $\R^n$.
</MathBox>

<MathBox title="Properties of Lipschitz continuity" boxType="proposition">
Let $\mathbf{V}:  U \subseteq\R^n \to\R^n$ be a vector field defined on an open subset $U$ of $\R^n$. Then the following properties hold.
1. **Local Lipschitz continuit implies continuity:** If $\mathbf{V}$ is locally Lipschitz continuous, then $\mathbf{V}$ is continuous at every point of $U$, i.e. for any converging sequence $(\mathbf{y}_k)_{k\in\N}$
$$
  \lim_{k\to\infty} \mathbf{y}_k = \mathbf{y} \implies \lim_{n\to\infty} \norm{\mathbf{V}(\mathbf{y}) - \mathbf{V}(\mathbf{y})} = 0
$$

2. **Local Lipschitz estimates:** If $\mathbf{V}$ is locally Lipschitz continuous, then for every $\mathbf{x}\in U$, there exists constants $\epsilon > 0$ and $L\geq 0$ such that
3. 
$$
  \frac{\norm{\mathbf{V}(\mathbf{y}) - \mathbf{V}(\mathbf{z})}}{\norm{\mathbf{y} - \mathbf{z}}} \leq L
$$

for all distinct $\mathbf{y}, \mathbf{z} \in B_\epsilon (\mathbf{x})$.


3. **Continuous differentiable vector fields are locally Lipschitz:** Let $\mathbf{V} \in C^1 (U, \R^n)$ be continuously differentiable. Then $\mathbf{V}$ is locally Lipschitz continuous.

<details>
<summary>Proof</summary>

**(1):** If $\mathbf{V}$ is locally Lipschitz continuous near $\mathbf{y} \in U$, then there exists $\epsilon > 0$ and $L \geq 0$ such that

$$
  \norm{\mathbf{V}(\mathbf{u}) - \mathbf{V}(\mathbf{v})} \leq L \norm{\mathbf{u} - \mathbf{v}}
$$

for all $\mathbf{u}, \mathbf{v} \in B_\epsilon (\mathbf{y})$. Thus, for sufficiently large $k$

$$
  \norm{\mathbf{V}(\mathbf{y}_k) - \mathbf{V}(\mathbf{y})} \leq L \norm{\mathbf{y}_k - \mathbf{y}} \xrightarrow{k\to\infty} 0
$$

**(2):** This is simply a reformulation of the local Lipschitz condition for $\mathbf{y} \neq \mathbf{z}$

**(3):** Fix $x\in U$. Since $U$ is open there exists $\epsilon > 0$ such that the closed ball $\overline{B_\epsilon (\mathbf{x})} \subseteq U$. Becaue the derivative $\D\mathbf{V}$ is continuous, the function $\mathbf{u}\mapsto \norm{\D\mathbf{V}(\mathbf{u})}$ is bounded on $\overline{B_\epsilon (\mathbf{x})}$. Define

$$
  L := \sup_{\mathbf{u}\in\overline{B_\epsilon (\mathbf{x})}} \norm{\D\mathbf{V}(\mathbf{u})} < \infty
$$

Then, by the mean value inequality for vector-valued functions,

$$
  \norm{V(\mathbf{y}) - \mathbf{V}(\mathbf{z})} \leq L \norm{\mathbf{y} - \mathbf{z}}
$$

for all $\mathbf{y}, \mathbf{z} \in B_\epsilon (\mathbf{x})$. Thus, $\mathbf{V}$ is locally Lipschitz continuous on $U$.
</details>
</MathBox>
...
$$
\begin{align*}
  \iint_\Sigma \left(\nabla \times \mathbf{F}\right) \cdot \d\mathbf{f} &= \oint_{\partial\Sigma} \mathbf{F} \cdot \d\mathbf{l} \\
  \iint_\Sigma \left[ \left( \frac{\partial R}{\partial y} - \frac{\partial Q}{\partial z} \right) \d y\d z + \left( \frac{\partial P}{\partial z} - \frac{\partial R}{\partial x} \right) \d z\d x + \left( \frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y} \right) \d x\d y \right] &= \oint_{\partial\Sigma} \left( P \d x + Q \d y + R P \d z \right)
\end{align*}
$$
...
$$
  \iiint_V \left(\nabla \cdot \mathbf{F} \right) \d V = \oiint_S \left( \mathbf{F} \cdot \mathbf{\hat{n}} \right) \d S
$$
...
$$
  \oint \left( \frac{\hat{\mathbf{r}}}{R^2} \right) \cdot \left( R^2 \sin\theta \;\d\theta \;\d\phi \,\hat{\mathbf{r}} \right) = \left( \int_0^\pi \sin\theta \;\d\theta \right) \left( \int_0^{2\pi} \d\phi \right) = 4\pi
$$

By the divergence theorem

$$
    \int_V \nabla \cdot \left( \frac{\hat{\mathbf{r}}}{r^2} \right) \d V = \oint \left( \frac{\hat{\mathbf{r}}}{R^2} \right) \cdot \d\mathbf{a} = 4\pi \\
...
$$
\begin{align*}
  \int_\mathcal{V} \nabla \cdot \left( \psi \nabla \phi \right)\d V &= \int_\mathcal{V} \left( \nabla\psi \cdot \nabla \phi + \psi \nabla^2 \phi \right) \d V \\
  &= \oint_\mathcal{S} \psi \left(\nabla\phi \cdot \mathbf{n} \right) \d S = \oint_\mathcal{S} \psi \nabla \phi \cdot \d\mathbf{S}
\end{align*}
$$
...
$$
  \int_U \left[ \psi \nabla \cdot \left( \epsilon\nabla\phi \right) - \phi \nabla \cdot \left( \epsilon\nabla\psi \right) \right] \d V = \oint_{\partial U} \epsilon \left( \psi \frac{\partial\phi}{\partial\mathbf{n}} - \phi \frac{\partial\psi}{\partial\mathbf{n}} \right) \d S
$$

In the special case of $\epsilon = 1$, the identity reduces to

$$
  \int_U \left( \psi \nabla^2 \phi - \phi \nabla^2 \psi \right)\d V = \oint_{\partial U} \left( \psi \frac{\partial\phi}{\partial\mathbf{n}} - \phi \frac{\partial\psi}{\partial\mathbf{n}} \right) \d S = \oint_{\partial U} \left( \psi \nabla \phi - \phi\nabla\psi \right) \d\mathbf{S}
$$
...
A differential form $\d f$ maps a scalar field $f$ to a covector field (level sets). The covector field is a dual space (linear functionals) equipped with addition and scalar multiplication

$$
\begin{gather*}
  \d f(\mathbf{u} + \mathbf{v}) = \d f(\mathbf{u}) + \d f(\mathbf{v}) \\
  \d f(c\mathbf{u}) = c \left[ \d f(\mathbf{u}) \right]
\end{gather*}
$$

The linear functional $\d f(\mathbf{v})$ can geometrically be interpreted as a directional derivative.

$$
    \d f(\mathbf{v}) \equiv \nabla_\mathbf{v} = \mathbf{v} \cdot \nabla f
$$

The dual basis covectors $\epsilon^i \equiv \d x^i$ are formed by the bi-orthogonality property

$$
  \epsilon^i \left( \mathbf{e}_j \right) \equiv \d x^i \left( \frac{\partial}{\partial x^j} \right) = \frac{\partial x^i}{\partial x^j} = \delta_j^i
$$

The dual basis covectors transform covariantly

$$
  \d\tilde{x}^i = \frac{\partial \tilde{x}^i}{\partial x} \d x^j = \left( J^{-1} \right)_i^j \d x^j
$$

A differential form can be expressed as a linear combination of the basis covectors $\d x^i$. The derivative of a curve tangent vector is given by

$$
\begin{align*}
  \d f \left( \frac{\d}{\d\lambda} \right) &= \frac{\d f}{\d\lambda} \\
  &= \frac{\partial f}{\partial x^i} \frac{\d x^i}{\d \lambda} \\
  &= \frac{\partial f}{\partial x^i} \d x^i \left( \frac{\d}{\d \lambda} \right) \\
\end{align*}
$$
