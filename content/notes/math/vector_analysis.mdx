...
**(2):** Since $\lim_{n\to\infty} \mathbf{x}_n = \mathbf{x}$, there is $N_1 \in\N$ such that $\lVert \mathbf{x}_n - \mathbf{x}\rVert < \frac{\epsilon}{2}$ for all $n\geq N_1$. Similary, since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$, there is $N_2 \in\N$ such that $\lVert \mathbf{y}_n - \mathbf{y}\rVert < \frac{\epsilon}{2}$ for all $n\geq N_1$. For $N = \max\Set{N_1, N_2}$, we then get using the triangle inequality

$$
\begin{align*}
  \lVert(\mathbf{x}_n + \mathbf{y}_n) - (\mathbf{x} + \mathbf{y})\rVert =& \lVert(\mathbf{x}_n - \mathbf{x}) + (\mathbf{y}_n - \mathbf{y})\rVert \\
  \tleq& \lVert\mathbf{x}_n - \mathbf{x}\| +\rVert\mathbf{y}_n - \mathbf{y}_n| \\
...
\begin{align*}
  ...
  \leq& \lVert \mathbf{x}_n - \mathbf{x}\rVert \cdot \lVert \mathbf{y}_n\rVert + \lVert \mathbf{x}\rVert \cdot \lVert \mathbf{y}_n - \mathbf{y}\rVert
\end{align*}
$$

Assume $\lVert \mathbf{x}\rVert \neq 0$. Since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$ there is $N_1\in\N$ such that $\lVert \mathbf{y}_n - \mathbf{y}\rVert\frac{\varepsilon}{2 \|\mathbf{x}\|}$ for all $n\geq N_1$.

Since $\lim_{n\to\infty} \mathbf{y}_n = \mathbf{y}$, there is $N_2 \in\N$ such that $\|\mathbf{y}_n - \mathbf{y}\| \leq 1$ when $n \geq N_2$. Thus

$$
\begin{align*}
  \lVert \mathbf{y}_n\rVert =& \lVert \mathbf{y} + (\mathbf{y}_n - \mathbf{y})\rVert \\
  \tleq& \lVert \mathbf{y}\rVert + \lVert \mathbf{y}_n - \mathbf{y}\rVert \\
  \leq& \lVert \mathbf{y} + 1\rVert

\end{align*}
$$

Since $\lim_{n\to\infty} \mathbf{x}_n = \mathbf{x}$, there exists $N_3 \in\N$ such that $\lVert \mathbf{x}_n - \mathbf{x}\rVert \leq \frac{\varepsilon}{2(\|\mathbf{y}\| + 1)}$. If $n\geq\max\Set{N_1,N_2}$ then

$$
  \lVert \mathbf{x}_n - \mathbf{x}\rVert\cdot \lVert \mathbf{y}_n\rVert < \frac{\varepsilon}{2(|\mathbf{y}| + 1|)} |\mathbf{y}| + 1| = \frac{\varepsilon}{2}
$$
...
$$
\begin{align*}
  | \mathbf{x}_n \cdot \mathbf{y}_n - \mathbf{x}\cdot\mathbf{y} | \leq& \lVert \mathbf{x}_n - \mathbf{x}\rVert\cdot\lVert \mathbf{y}_n\rVert + \lVert \mathbf{x}\rVert\cdot \lVert \mathbf{y}_n - \mathbf{y}\rVert \\
  <& \frac{\varepsilon}{2} + \frac{\varepsilon}{2} = \varepsilon
\end{align*}
$$
</details>
...
$$
\begin{gather*}
  \tilde{\mathbf{T}}(t)\cdot\mathbf{T}(t) = \lVert\tilde{\mathbf{T}}(t)\| \cdot\rVert\mathbf{T}(t)\|\cos\theta \\
...
Suppose $\mathbf{x}$ approaches $\mathbf{a}$ in the $x_i$-direction, i.e. $\mathbf{x} = \mathbf{a} + h\mathbf{e}_i$. Then $\mathbf{x} - \mathbf{a} = h\mathbf{e}_1$ and $\lVert \mathbf{x} - \mathbf{a}\rVert = |h|$. For the difference quotient to hold true then we require in both limits $h\downarrow 0$ and $h\uparrow 0$ that
...
$$
  Q(\mathbf{x}) = \begin{cases}
    \frac{f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a})}{\lVert \mathbf{x} - \mathbf{a}\rVert}, \quad& \mathbf{x}\neq\mathbf{a} \\
...
$$
\begin{align*}
  |f(\mathbf{x}) - f(\mathbf{a})| =& |Q(\mathbf{x})\lVert \mathbf{x} - \mathbf{a}\rVert + \nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})| \\
  \tleq& |Q(\mathbf{x})\lVert \mathbf{x} - \mathbf{a}\rVert| + |\nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})| \\
  \leq& |Q(\mathbf{x})|\cdot\lVert \mathbf{x} - \mathbf{a}\rVert + \lVert \nabla f(\mathbf{a})\| \cdot\rVert\mathbf{x} - \mathbf{a}\|
\end{align*}
$$
...
\begin{align*}
  ...
  \lim_{\mathbf{x}\to\mathbf{a}}|Q(\mathbf{x})|\cdot\lVert \mathbf{x} - \mathbf{a}\rVert \\
  &+ \lim_{\mathbf{x}\to\mathbf{a}} \lVert \nabla f(\mathbf{a})\| \cdot\rVert(\mathbf{x} - \mathbf{a})\| \\
  =& 0
\end{align*}
$$
...
$$
\begin{align*}
  \frac{|f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a})|}{\lVert \mathbf{x} - \mathbf{a}\rVert} =& \frac{|(\nabla f(\mathbf{c}) - \nabla f(\mathbf{a}))\cdot (\mathbf{x} - \mathbf{a})|}{\lVert \mathbf{x} - \mathbf{a}\rVert} \\
  \leq& \lVert \nabla f(\mathbf{c}) - \nabla f(\mathbf{a})\rVert \frac{\lVert\mathbf{x} - \mathbf{a}\|}{\| \mathbf{x} - \mathbf{a}\rVert} \\
  =& \lVert \nabla f(\mathbf{c}) - \nabla f(\mathbf{a})\rVert
\end{align*}
$$

Since $\lVert \mathbf{c} - \mathbf{a}\rVert \leq \lVert \mathbf{x} - \mathbf{a}\rVert$, it follows that $\mathbf{c}\xrightarrow{\mathbf{x}\to \mathbf{a}} \mathbf{a}$. By continuity of partial derivatives, this mean that $\nabla f(\mathbf{c}) \xrightarrow{\mathbf{x}\to\mathbf{a}} \nabla f(\mathbf{a})$. Hence

$$
\begin{align*}
  &\lim_{\mathbf{x}\to\mathbf{a}} \frac{|f(\mathbf{x}) - f(\mathbf{a}) - \D f(\mathbf{a})(\mathbf{x} - \mathbf{a})|}{\lVert \mathbf{x} - \mathbf{a}\rVert} \\
  \leq& \lim_{\mathbf{x}\to\mathbf{a}} \lVert \nabla f(\mathbf{c}) - \nabla f(\mathbf{a})\rVert = 0
\end{align*}
$$
...
$$
  \alpha'(t_0) = \lim_{t\to t_0} \frac{\lVert \alpha(t) - \alpha(t_0)\rVert}{t - t_0} =  \lim_{t\to t_0} \frac{\lVert \mathbf{x} - \mathbf{a}\rVert}{t - t_0}
$$
...
\begin{align*}
  ...
  =& \lim_{t \to t_0} \frac{|f(\alpha(t)) - f(\alpha(t_0)) - \nabla f(\alpha(t_0))\cdot(\alpha(t) - \alpha(t_0))|}{\lVert \alpha(t) - \alpha(t_0)\rVert}
\end{align*}
$$
...
$$
  \nabla f(\mathbf{a}) \cdot \mathbf{v} = \lVert \nabla f(\mathbf{a})\| \cdot\rVert \mathbf{u} \| \cos\theta 
$$
...
$$
  0 = (f\circ a)'(t) = \nabla f(\alpha(t))\circ \alpha'(t) = \nabla f(\mathbf{a})\cdot\mathbf{v} = \lVert \nabla f(\mathbf{a})\rVert \cdot \lVert \mathbf{v}\rVert \cos\theta
$$
