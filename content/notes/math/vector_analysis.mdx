...
$$
  C = \{ \mathbf{x}\in\mathbb{R}^n \mid \mathbf{x} = \alpha(t)\; t\in I \}
$$
...
For a curve $\mathbf{R}:[a, b] \mapsto \mathbb{R}^n$, the length is defined as the limit of the sum of line segments, as the partition approaches infinity
...
$$
  \left\{ \left(x_i \right)_{i=0}^{n+1} \in\mathbb{R}^{n+1} \mid \left(x_i \right)_{i=1}^n \in A \textrm{ and } x_{n+1} = f(x_1,x_2,\dots,x_n) \right\}
$$
</MathBox>
...
$$
  \mathcal{B}(\mathbf{a}, r) = \left\{ \mathbf{x}\in\mathbb{R} \mid \lVert \mathbf{x} - \mathbf{a} \rVert < r \right\}
$$
</MathBox>
...
- closed if its complement $U^c = \mathbb{R}^n \backslash U = \{\mathbf{x}\in\mathbb{R}^n \mid \mathbf{x}\not\in U\}$ is open.
...
Let $\mathbf{F} = \left[P(x, y, z), Q(x, y, z), R(x, y, z) \right]$ be a continuously differentiable vector field defined in a region $\Sigma \in \mathbb{R}^3$ with boundary $\partial\Sigma$, then
...
Let $\mathbf{F}$ be a continuously differentiable vector field defined on a compact subset $V \in \mathbb{R}^n$ with the smooth boundary $\S$, then
...
If $\phi$ and $\psi$ are both twice continuously differentiable on $U \in \mathbb{R}^3$, and $\epsilon$ is once continuously differentiable, one may choose the vector field $\mathbf{F} = \psi \epsilon\nabla \phi - \phi\epsilon\nabla\psi$ to obtain
...
The transformation from polar coordinates $(r, \theta)$ to Cartesian coordinates $(x, y)$ is given by the function $\mathbf{f} : \mathbb{R} \times [0, 2\pi) \mapsto \mathbb{R}^2$ with components
...
The transformation from Cartesian coordinates $(x, y)$ to polar coordinates $(r, \theta)$ is given by the function $\mathbf{f} : \mathbb{R}^2 \mapsto \mathbb{R}^+ \times [0, 2\pi)$ with components
