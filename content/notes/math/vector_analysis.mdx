...
Assume that $1 \geq k < n$. It suffices to show that if the gradients $(\nabla g_i(\mathbf{a}))_{i=1}^k$ are linearly independent, there are constants $(\lambda_i)_{i=1}^k$ such that 
...
$$
\mathbf{G}'(\mathbf{a}) = \begin{array}{c|c}
...
and $\boldsymbol{\Phi}(\tilde{\mathbf{z}}) = \tilde{\mathbf{y}} = (\tilde{x}_j)_{j=m+1}^n$. We also know that
