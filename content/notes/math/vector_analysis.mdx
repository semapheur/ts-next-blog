...
Let $\alpha,\beta :I\to\mathbb{R}^3$ be differentiable paths with nonzero $v$ and $\mathbf{T}'$ so that the Frenet vectors are defined for all $t$. Then either path can be translated and rotated so that it fits exactly on top of the otherif and only ifthey have the same speed $v$, curvature $\kappa$ and torsion $\tau$. In this case, the two paths are said to be congruent to each other. 
