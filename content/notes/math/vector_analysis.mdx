...
On a sidenote, since $\mathrm{D}f(\mathbf{a})$ maps from $\R^n$ into $\R$ it is technically a differential 1-form, or a linear functional, identified with dual vector space $(\R^n)^*$, which is the set of all linear functionals. The total derivative $\mathrm{D}f(\mathbf{a})$ and the gradient $\nabla f(\mathbf{a})$ are thus dual to each other. 
