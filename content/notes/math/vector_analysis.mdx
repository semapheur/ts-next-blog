...
Generalizing to the multivariable case, we expect that a function $f:U\subseteq\R^n \to\R$ to be differentiable at a point $\mathbf{a}\in U$ if there is a linear affine function $\ell:\R^n \to\R$ given by $\ell(\mathbf{x}) = T(\mathbf{x}) + b$ such that
...
A function $\ell:\R^n \to R^m$ is called an *affine function* if it has the form $\ell(\mathbf{x}) = T(\mathbf{x}) + \mathbf{b}$, $T:\R^n \to\R^m$ is a linear transformation and $\mathbf{b}\in\R^m$.
</MathBox>

The affine function $\ell$ gives the linear approximation of $f$ at $\mathbf{a}$ such that $f(\mathbf{a}) = \ell(\mathbf{a}) = T(\mathbf{a}) + b$. Thus, $b = f(\mathbf{a}) - T(\mathbf{a})$ yielding
...
In a neighbourhood of $\mathbf{a}$, $\mathbf{x}$ takes the form $\mathbf{x} = \mathbf{a} + h\sum_{i=1}^n \mathbf{e}_i$ for $h\in\R$. Suppose that $\mathbf{x}$ approaches $\mathbf{a}$ in the $x_i$-direction, i.e. $\mathbf{x} = \mathbf{a} + h\mathbf{e}_i$. Then $\mathbf{x} - \mathbf{a} = h\mathbf{e}_1$ and $\lVert \mathbf{x} - \mathbf{a} \rVert = |h|$. For the difference quotient to hold true then we require in both limits $h\downarrow 0$ and $h\uparrow 0$ that

$$
\begin{align*}
  &\lim_{h\to 0} \frac{f(\mathbf{a} + h\mathbf{e}_i) - f(\mathbf{a}) - T(h\mathbf{e}_i)}{h} \\
  =& \lim_{h\to 0} \frac{f(\mathbf{a} + h\mathbf{e}_i) - f(\mathbf{a}) - hT(\mathbf{e}_i)}{h} = 0 
\end{align*}
$$

Solving for for $T(\mathbf{e}_i)$ gives

$$
\begin{align*}
  T(\mathbf{e}_i) =& \lim_{h\to 0} \frac{f(\mathbf{a} + h\mathbf{e}_i) - f(\mathbf{a})}{h} \\
  =& \lim_{h\to 0} \frac{f(a_i,\dots,a_i + h,dots,a_n) - f(a_1,\dots,a_n)}{h}
\end{align*}
$$
...
$$
  J_f(\mathbf{a}) = \mathrm{D}f(\mathbf{a}) = \begin{bmatrix} \frac{\partial f}{\partial x^1}(\mathbf{a}) & \cdots & \frac{\partial f}{\partial x_n}(\mathbf{a}) \end{bmatrix}
$$

called the *Jacobian matrix* of $f$ at $\mathbf{a}$. Taking the transpose of the Jacobian matrix gives a vector ($n\times 1$ matrix) called the *gradient* of $f$ at $\mathbf{a}$, denoted

$$
  \nabla f(\mathbf{a}) = \left( \frac{\partial f}{\partial x_i}(\mathbf{a}) \right)_{i=1}^n \in\R^n
$$
...
$$
  \lim_{\mathbf{x}\to\mathbf{a}}\frac{f(\mathbf{x}) - f(\mathbf{a}) - \mathrm{D}f(\mathbf{a})(\mathbf{x} - \mathbf{a})}{\lVert \mathbf{x} - \mathbf{a}\rVert}
$$
