...
\begin{align*}
  ...
  \stackrel{\triangle}{\leq}& \|\mathbf{x}_n - \mathbf{x}\| + \|\mathbf{y}_n - \mathbf{y}_n| \\
...
\begin{align*}
  ...
  \stackrel{\triangle}{\leq}& \|\mathbf{x}_n \cdot \mathbf{y}_n - \mathbf{x}\cdot\mathbf{y}_n\| + \|\mathbf{x}\cdot\mathbf{y}_n - \mathbf{x}\cdot\mathbf{y} \| \\
...
\begin{align*}
  ...
  \stackrel{\triangle}{\leq}& \| \mathbf{y} \| + \| \mathbf{y}_n - \mathbf{y} \| \\
  \leq& \| \mathbf{y} + 1 \|
...
Alternatively, using the definition of continuity to $f$ then $\lim_{\mathbf{x}\to\mathbf{a}} f(\mathbf{x}) = L$ if for any $\epsilon > 0$, there exists a $\delta > 0$ such that $|f(\mathbf{x}) - L| < \epsilon$ whenever $\lVert\mathbf{x} - \mathbf{a}\rVert < \delta$, except possibly when $\mathbf{x} = \mathbf{a}$.
</MathBox>
...
$$
  \tilde{f}(\mathbf{x}) + \tilde{g}(\mathbf{x}) = \begin{align*}
...
$$
  \lim_{\mathbf{x}\to\mathbf{a}}\frac{f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a})}{\lVert \mathbf{x} - \mathbf{a}\rVert} = 0
$$
...
\begin{align*}
  ...
  \stackrel{\triangle}{\leq}& |Q(\mathbf{x})\lVert \mathbf{x} - \mathbf{a} \rVert| + |\nabla f(\mathbf{a})\cdot (\mathbf{x} - \mathbf{a})| \\
  \leq& |Q(\mathbf{x})|\cdot\lVert \mathbf{x} - \mathbf{a} \rVert + \lVert \nabla f(\mathbf{a})\rVert \cdot \lVert\mathbf{x} - \mathbf{a}\rVert
\end{align*}
$$
...
Since $f: U\to\R$ is differentiable at $\mathbf{a} = \alpha(t_0)$, then

$$
  \lim_{\mathbf{x}\to\mathbf{a}}\frac{f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a})}{\lVert \mathbf{x} - \mathbf{a}\rVert} = 0
$$

Note that $f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a}) = \lVert \mathbf{x} - \mathbf{a}\rVert Q(\mathbf{x})$. By the definition of limits, it follows that

$$
\begin{align*}
  0 =& \lim_{\mathbf{x}\to\mathbf{a}} \lVert \mathbf{x} - \mathbf{a}\rVert Q(\mathbf{x}) \\
  =& \lim_{\mathbf{x}\to\mathbf{a}} f(\mathbf{x}) - f(\mathbf{a}) - \nabla f(\mathbf{a})\cdot(\mathbf{x} - \mathbf{a})
\end{align*}
$$

If $\alpha:I\subseteq\R\to U$ is differentiable at $t_0$, then its derivative at this point is

$$
  \alpha'(t_0) = \lim_{t\to t_0} \frac{\alpha(t) - \alpha(t_0)}{t - t_0}
$$

It follows that $\alpha$ is continuous at $t_0$, and since $\mathbf{a} = \alpha(t_0)$, the composition $t \mapsto \lVert \alpha(t) - \alpha(t_0) \rVert \left|\frac{Q(\alpha(t))}{t - t_0} \right|$ is continuous at $t_0$ such that

$$
\begin{align*}
  0 =& \lim_{\alpha(t) \to \alpha(t_0)} \left|\frac{f(\alpha(t)) - f(\alpha(t_0)) - \nabla f(\alpha(t_0))\cdot (\alpha(t) - \alpha(t_0))}{t - t_0}\right| \\
  =& \lim_{t \to t_0} \left|\frac{f(\alpha(t)) - f(\alpha(t_0)) - \nabla f(\alpha(t_0))\cdot (\alpha(t) - \alpha(t_0))}{t - t_0}\right|
\end{align*}
$$

The limit holds if the numerator in the absolute value is zero, hence

$$
\begin{align*}
  \lim_{t \to t_0} \frac{f(\alpha(t)) - f(\alpha(t_0))}{t - t_0} =& \lim_{t \to t_0}\nabla f(\alpha(t_0)) \frac{\alpha(t) - \alpha(t_0)}{t - t_0} \\
...
$$
  \nabla f(\mathbf{a}) \cdot \mathbf{v} = \lVert \nabla f(\mathbf{a})\rVert \cdot \lVert \mathbf{u} \rVert \cos\theta 
$$
...
<MathBox title='Equality of mixed partials' boxType='definition'>
