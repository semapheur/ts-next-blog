...
On a sidenote, since $\mathrm{D}f(\mathbf{a})$ maps from $\R^n$ into $\R$ it is technically a linear functional, also called a covector, belonging to the dual space of $\R^n$. The total derivative $\mathrm{D}f(\mathbf{a})$ and the gradient $\nabla f(\mathbf{a})$ are thus dual pairs. 
