...
Since $\mathbf{X}f = (\d f)(\mathbf{X})$, the Leibniz rule can be written as

$$
  \nabla_\mathbf{X} (f\mathbf{Y}) = (\d f)(\mathbf{X})\mathbf{Y} + f\nabla_\mathbf{X} \mathbf{Y}
$$

or, suppressing $\mathbf{X}$,

$$
  \nabla (fY) = \d f\cdot\mathbf{Y} + f\nabla\mathbf{Y}
$$
</details>
</MathBox>

A connection on the tangent bundle $TM$ of a smooth manifold $M$ can be viewed as a linear map

$$
  \nabla: \mathfrak{X}(M) \to \hom_{C^\infty (M)} (\mathfrak{X}(M),\mathfrak{X}(M))
$$

such that for any fixed $\mathbf{Y}\in\mathfrak{X}(M)$, the map $\nabla \mathbf{Y} \mapsto \nabla_\mathbf{X} \mathbf{Y}$ is linear over $C^\infty (M)$, implying that $\nabla \mathbf{Y}$ is a $\binom{1}{1}$-tensor field on $M$.
...
The directional derivative $\D_\mathbf{X}$ of a vector field $\mathbf{Y}$ on $\R^n$ is an affine connection on $\R^n$ called the *Euclidean connection* on $\R^n$.
...
$$
\begin{equation}
  \nabla_{\mathbf{E}_i} \mathbf{E_j} = \Gamma_{ij}^k \mathbf{E}_k,\, \forall i, j=1,\dots,n \tag{\label{equation-1}}
\end{equation}
$$
...
$$
\begin{equation}
  \nabla_\mathbf{X} \mathbf{Y} = (\mathbf{X}Y^k + X^i Y^k \Gamma_{ij}^k) \mathbf{E}_k \tag{\label{equation-2}}
\end{equation}
$$
...
$$
\begin{align*}
  \nabla_\mathbf{X} \mathbf{Y} =& \nabla_\mathbf{X} \left(Y^j \mathbf{E}_j \right) \\
  =& (\mathbf{X}Y^j)\mathbf{E}_j + Y^j \nabla_{X^i \mathbf{E}_i} \mathbf{E}_j \\
  =& (\mathbf{X}Y^j)\mathbf{E}_j + X^i Y^j \nabla_{\mathbf{E}_i} \mathbf{E}_j \\
  =& \mathbf{X}Y^k \mathbf{E}_k + X^i Y^j \Gamma_{ij}^k \mathbf{E}_k \\
  =& (\mathbf{X}Y^k + X^i Y^j \Gamma_{ij}^k) \mathbf{E}_k
\end{align*}
$$
</details>
</MathBox>
...
<MathBox title='Torsion is a tensor field' boxType="proposition">
Let $\nabla$ be an affine connection on a smooth manifold $M$. The *torsion tensor* of $\nabla$ is a bilinear map
...
which is a $\binom{1}{2}$-tensor field on $M$ given in local coordinates by

$$
  T = ({\Gamma_{ij}}^k - {\Gamma_{ji}}^k) \;\boldsymbol{\partial}_k \otimes \d x^i \otimes \d x^j
$$
...
Noting that $\mathbf{X}(1) = 0$, we get

$$
\begin{align*}
  [f\mathbf{X}, \mathbf{Y}] =& f[\mathbf{X},\mathbf{Y}] + f(\mathbf{X}(1))\mathbf{Y} - (\mathbf{Y}f)\mathbf{X} \\
  =& f[\mathbf{X},\mathbf{Y}] - (\mathbf{Y}f)\mathbf{X}
\end{align*}
$$

By the tensor characterization lemma, we need to show that $T$ is linear over $C^\infty (M)$ in its arguments. That is, for any $f\in C^\infty (M)$

$$
  T(f\mathbf{X}, \mathbf{Y}) = T(\mathbf{X}, f\mathbf{Y}) = fT(\mathbf{X}, \mathbf{Y})
$$

Applying $f$ on $\mathbf{X}$ we have
...
\begin{align*}
  ...
  &- f[\mathbf{X}, \mathbf{Y}] + \cancel{(\mathbf{Y}f)\mathbf{X}} \\
  =& f (\nabla_\mathbf{X} \mathbf{Y} - \nabla_\mathbf{Y} \mathbf{X} - [\mathbf{X}, \mathbf{Y}]) \\
  =& fT(\mathbf{X},\mathbf{Y})
\end{align*}
$$

Linearity over $C^\infty (M)$ in $\mathbf{Y}$ follows from the skew-symmetry of $T$

$$
  T(\mathbf{X}, f\mathbf{Y}) = -T(f\mathbf{Y}, \mathbf{X}) = -fT(\mathbf{Y}, \mathbf{X}) = fT(\mathbf{X}, \mathbf{Y})
$$

This shows that $T$ is a tensor field. In local coordinates, the components of $T$ are given by

$$
\begin{align*}
  T(\boldsymbol{\partial}_i, \boldsymbol{\partial}_j) =& \nabla_{\boldsymbol{\partial}_i} \boldsymbol{\partial_j} - \nabla_{\boldsymbol{\partial}_j} \boldsymbol{\partial}_i - \underbrace{[\boldsymbol{\partial}_i, \boldsymbol{\partial}_j]}_{=0} \\
  =& {\Gamma_{ij}}^k \boldsymbol{\partial}_k - {\Gamma_{ji}}^k \boldsymbol{\partial}_k \\
  =& ({\Gamma_{ij}}^k - {\Gamma_{ji}}^k) \boldsymbol{\partial}_k \\
  =& {T_{ij}}^k \boldsymbol{\partial}_k
\end{align*}
$$

This shows that $T$ is a $\binom{1}{2}$-tensor field on $M$ with coordinate representation
...
Let $(U,(x^i)_{i=1}^n)$ be a chart on $M$. Since partial differentiation is independent of the order of differentiation
...
Since $(\partial/\partial x^i)_{i=1}^n$ are linearly independent at each point, $\Gamma_{ij}^k = \Gamma_{ji}^k$.

Conversely, suppose $\Gamma_{ij}^k = \Gamma_{ji}^k$ in the coordinate chart $(U,(x^i)_{i=1}^n)$. Then,

$$
  T\left(\frac{\partial}{\partial x^i},\frac{\partial}{\partial x^i}\right) = \nabla_{\boldsymbol{\partial}_i} \frac{\partial}{\partial x_j} - \nabla_{\boldsymbol{\partial}_j} \frac{\partial}{\partial x_i} = 0
$$

Since $T$ is a bilinear function on $T_\mathbf{p}M$ for $\mathbf{p}\in M$, this proves that $T(\mathbf{X},\mathbf{Y})_\mathbf{p} := T(\mathbf{X}_\mathbf{p},\mathbf{Y}_\mathbf{p}) = 0$ for all $\mathbf{X}_\mathbf{p},\mathbf{Y}_\mathbf{p}\in T_\mathbf{p}M$. Thus, for all $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$, we have $T(\mathbf{X},\mathbf{Y}) = 0$.
</details>
</MathBox>

### Curvature tensor

<MathBox title='Curvature of affine connections' boxType="definition">
Let $\nabla$ be an affine connection on a smooth manifold $M$. The *curvature* of $\nabla$ is a bilinear map $R:\mathfrak{X}(M)\times\mathfrak{X}(M)\to\operatorname{end}(\mathfrak{X}(M))$ defined by
...
<MathBox title='Curvature is a tensor field' boxType="proposition">
Let $\nabla$ be an affine connection on a smooth manifold $M$. The *curvature tensor* of $\nabla$ is a trilinear map
...
which is a $\binom{1}{3}$-tensor field on $M$ given in local coordinates by

$$
  R = {R_{ijk}}^l \;\boldsymbol{\partial}_l \otimes \d x^i \otimes \d x^j \otimes \d x^k 
$$

with components

$$
  R_{ijk}^l = \boldsymbol{\partial}_i \Gamma_{jk}^l - \boldsymbol{\partial}_j \Gamma_{ik}^l + \Gamma_{jk}^s \Gamma_{is}^l - \Gamma_{ik}^s \Gamma_{js}^l
$$
...
By the tensor characterization lemma, we need to show that $R$ is linear over $C^\infty (M)$ in its arguments. That is, for any $f\in C^\infty (M)$

$$
  R(f\mathbf{X},\mathbf{Y})\mathbf{Z} = R(\mathbf{X},f\mathbf{Y})\mathbf{Z} = R(\mathbf{X},\mathbf{Y})(f\mathbf{Z}) = fR(\mathbf{X},\mathbf{Y})
$$

Applying $f$ to $\mathbf{X}$ we get

$$
\begin{align*}
  R(f\mathbf{X},\mathbf{Y})\mathbf{Z} =& \nabla_{f\mathbf{X}} \nabla_\mathbf{Y} \mathbf{Z} - \nabla_\mathbf{Y} \nabla_{f\mathbf{X}} \mathbf{Z} - \nabla_{[f\mathbf{X},\mathbf{Y}]}\mathbf{Z} \\
  =& \nabla_{f\mathbf{X}} \nabla_\mathbf{Y} \mathbf{Z} - \nabla_\mathbf{Y} \nabla_{f\mathbf{X}} \mathbf{Z} - \nabla_{f[\mathbf{X},\mathbf{Y}] - (\mathbf{Y}f)\mathbf{X}}\mathbf{Z} \\
  =& f\nabla_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} - (\cancel{(\mathbf{Y}f)\nabla_\mathbf{X} \mathbf{Z}} + f\nabla_\mathbf{Y} \nabla_\mathbf{X} \mathbf{Z}) \\
  &- (f \nabla_{[\mathbf{X},\mathbf{Y}]} - \cancel{(\mathbf{Y}f) \nabla_\mathbf{X}\mathbf{Z}}) \\
  =& f(\nabla_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} - \nabla_\mathbf{Y} \nabla_\mathbf{X} \mathbf{Z} - \nabla_{[\mathbf{X},\mathbf{Y}]}\mathbf{Z}) \\
  =& fR(\mathbf{X},\mathbf{Y})\mathbf{Z}
\end{align*}
$$

Linearity over $C^\infty (M)$ in $\mathbf{Y}$ follows from the skew-symmetry of $R$ in $\mathbf{X}$ and $\mathbf{Y}$

$$
  R(\mathbf{X},f\mathbf{Y}) = -R(f\mathbf{Y},\mathbf{X}) = -fR(\mathbf{Y},\mathbf{X}) = fR(\mathbf{X},\mathbf{Y})
$$

Applying $f$ to $\mathbf{Z}$, we get

$$
\begin{align*}
  R(\mathbf{X},\mathbf{Y})f\mathbf{Z} =& \nabla_\mathbf{X} \nabla_\mathbf{Y} (f\mathbf{Z}) - \nabla_\mathbf{Y} \nabla_\mathbf{X} (f\mathbf{Z}) - \nabla_{[\mathbf{X},\mathbf{Y}]} (f\mathbf{Z}) \\
  =& \nabla_\mathbf{X} f (\nabla_\mathbf{Y} \mathbf{Z}) + \nabla_\mathbf{X} (\mathbf{Y}f)\mathbf{Z} \\ 
  &- \nabla_\mathbf{Y} f \nabla_\mathbf{X} \mathbf{Z} - \nabla_\mathbf{Y} (\mathbf{X}f)\mathbf{Z} \\
  &- f\nabla_{[\mathbf{X},\mathbf{Y}]} \mathbf{Z} - ([\mathbf{X},\mathbf{Y}]f) \mathbf{Z} \\
  =& f \nabla_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} + \textcolor{red}{(\mathbf{X}f)\nabla_\mathbf{Y} \mathbf{Z}} \\
  &+ \textcolor{lime}{(\mathbf{Y}f)\nabla_\mathbf{X} \mathbf{Z}} + \textcolor{cyan}{(\mathbf{X}(\mathbf{Y}f))\mathbf{Z}} \\
  &- f\nabla_\mathbf{Y} \nabla_\mathbf{X} \mathbf{Z} - \textcolor{lime}{(\mathbf{Y}f)\nabla_\mathbf{X} \mathbf{Z}} \\
  &- \textcolor{red}{(\mathbf{X}f)\nabla_\mathbf{Y} \mathbf{Z}} - \textcolor{orange}{(\mathbf{Y}(\mathbf{X}f))\mathbf{Z}} \\
  &- f\nabla_{[\mathbf{X},\mathbf{Y}]} \mathbf{Z} - \textcolor{cyan}{(\mathbf{X}(\mathbf{Y}f))\mathbf{Z}} + \textcolor{orange}{(\mathbf{Y}(\mathbf{X}f))\mathbf{Z}} \\
  =& f(\nabla_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} - \nabla_\mathbf{Y} \nabla_\mathbf{X} \mathbf{Z} - \nabla_{[\mathbf{X},\mathbf{Y}]} \mathbf{Z}) \\
  =& fR(\mathbf{X}, \mathbf{Y})\mathbf{Z}
\end{align*}
$$

This shows that $R$ is a tensor field. In local coordinates, the components of $R$ are given by

$$
\begin{align*}
  R(\boldsymbol{\partial}_i, \boldsymbol{\partial}_j)\boldsymbol{\partial}_k =& \nabla_{\boldsymbol{\partial}_i}(\nabla_{\boldsymbol{\partial}_j}\boldsymbol{\partial}_k) - \nabla_{\boldsymbol{\partial}_j} (\nabla_{\boldsymbol{\partial}_i} \boldsymbol{\partial}_k) - \underbrace{\nabla_{[\boldsymbol{\partial}_i,\boldsymbol{\partial}_j]} \boldsymbol{\partial}_k}_{=0} \\
  =& \nabla_{\boldsymbol{\partial}_i} (\Gamma_{jk}^s \boldsymbol{\partial}_s) - \nabla_{\boldsymbol{\partial}_j}(\Gamma_{ik}^s \boldsymbol{\partial}_s) \\
  =& \Gamma_{jk}^s \nabla_{\boldsymbol{\partial}_i} \boldsymbol{\partial}_s + \boldsymbol{\partial}_i \Gamma_{jk}^l \boldsymbol{\partial}_l - \Gamma_{ik}^s \nabla_{\boldsymbol{\partial}_j} \boldsymbol{\partial}_s - \boldsymbol{\partial}_j \Gamma_{ik}^l \boldsymbol{\partial}_l \\
  =& (\boldsymbol{\partial}_i \Gamma_{jk}^l - \boldsymbol{\partial}_j \Gamma_{ik}^l + \Gamma_{jk}^s \Gamma_{is}^l - \Gamma_{ik}^s \Gamma_{js}^l)\boldsymbol{\partial}_l \\
  =& {R_{ijk}}^l \boldsymbol{\partial}_l
\end{align*}
$$

This shows that $R$ is a $\binom{1}{3}$-tensor field on $M$ given in local coordinates by
...
Suppose $M$ is a smooth $n$-manifold covered by a single coordinate chart $(U, (x^i)_{i=1}^n)$. There is an injective correspondence between affine connections on $M$ and choices of $n^3$ Christoffel symbols $\Gamma_{ij}^k$ on by the rule

$$
\begin{equation}
\begin{split}
  \nabla_\mathbf{X} \mathbf{Y} =& \left(X^i \boldsymbol{\partial}_i Y^k + X^i Y^j \Gamma_{ij}^k \right)\boldsymbol{\partial}_k \\
  =& \left(\mathbf{X} Y^k + X^i Y^j \Gamma_{ij}^k \right)\boldsymbol{\partial}_k
\end{split}
\tag{\label{equation-3}}
\end{equation}
$$
...
$$
  \nabla_\mathbf{X}\omega = \left(X^i \boldsymbol{\partial}_i \omega_k - X^i \omega_j \Gamma_{ik}^j \right)\;\d x^k
$$
</MathBox>
...
$$
  \nabla_\mathbf{X} f = \mathbf{X}f,\; \mathbf{X}\in\mathfrak{X}(M),\, f\in C^\infty (M)
$$

<MathBox title='Covariant derivative of tensor fields' boxType="proposition" tag="6">
...
Let $f\in C^\infty (M)$ be a smooth function on a smooth manifold $M$. Then the total covariant derivative $\nabla f \in \mathcal{T}^1 (M) = \Lambda^1$ is just the $1$-form $\d f$, because both tensors have the same action on vectors

$$
  \langle\nabla f, \mathbf{X}\rangle = \nabla_\mathbf{X} f = \mathbf{X}f = \langle \d f, \mathbf{X}\rangle,\; \mathbf{X}\in\mathfrak{X}(M)
$$
...
$$
  \nabla \mathbf{Y} = {Y^i}_{;j} \frac{\partial}{\partial x^i} \otimes \d x^j
$$
...
For a smooth vector field $\mathbf{V}(t) = \sum_{i=1}^n v^i(t) \frac{\partial}{\partial x^i}$ along a smooth curve $\gamma: I\to\R^n$ in $\R^n$, the derivative $\frac{\d\mathbf{V}}{\d t} = \sum_{i=1}^n \dot{v}^i (t) \frac{\partial}{\partial x^i}$ satisfies the following properties
1. $\frac{\d\mathbf{V}}{\d t}$ is linear over $\R$ in $\mathbf{V}$
2. **Leibniz rule:** for any smooth function $f$ on $[a,b]$
$$
  \frac{\d(f\mathbf{V})}{\d t} = \frac{\d f}{\d t}\mathbf{V} + f\frac{\d\mathbf{V}}{\d t}
$$
3. if $\mathbf{V}$ is induced from a smooth vector field $\tilde{\mathbf{V}}$ on $\R^n$, in the sense that $\mathbf{V}(t) = \tilde{\mathbf{V}}_{c(t)}$ and $\D$ is the directional derivative in $\R^n$, then
$$
  \frac{\d\mathbf{V}}{\d t} = \D_{c'(t)}\tilde{\mathbf{V}}
$$

It turns out that to every connection $\nabla$ on a manifold $M$ one can associate a way of differentiating vector fields along a curve having the same properties as $\frac{\d\mathbf{V}}{\d t}$.

<MathBox title='Covariant derivative of vector fields along curves' boxType="proposition" tag="11">
...
$$
  \D_t := \frac{\D}{\d t}: \Gamma(TM|_{\gamma(t)})\to\Gamma(TM|_{\gamma(t)})
$$

with the following properties
1. **Linearity over $\R$:** for any $\mathbf{V},\mathbf{W}\in\Gamma(TM|_{\gamma(t)})$ and $a,b\in\R$ 
$$
  \frac{\D}{\d t} (a\mathbf{V} + b\mathbf{W}) = a\frac{\D\mathbf{V}}{\d t} + b\frac{\D\mathbf{W}}{\d t}
$$
...
$$
  \frac{\D\mathbf{V}}{\d t}(t) = \nabla_{\gamma'(t)} \tilde{\mathbf{V}} 
$$

For any $\mathbf{V}\in\Gamma(TM|_{\gamma(t)})$, we call $\D_t \mathbf{V}$ the *covariant derivative* (associated to $\nabla$) of the vector field $\mathbf{V}$ along the curve $\gamma$ in $M$.

<details>
<summary>Proof</summary>

To prove uniqueness, suppose such a covariant derivative $\D_t$ exists and let $t_0 \in I$ be arbitrary. For any $\mathbf{V}\in\Gamma(TM|_{\gamma(t)})$, an argument similar to lemma $\ref{lemma-1}$ shows that the value of $\D_t \mathbf{V}$ at $t_0$ depends only on the values of $\mathbf{V}$ in any interval $(t_0 - \varepsilon, t_0 + \varepsilon)$ containing $t_0$. In a chart $(U, (x^i)_{i=1}^n)$ containing $\gamma(t_0)$, the vector field $\mathbf{V}$ along $\gamma$ can written as a linear combination

$$
  \mathbf{V}(t) = \sum_{i=1}^n V^i (t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)}
$$

Then, by the properties of $\D_t$ and since $\boldsymbol{\partial}_i$ is extendible

$$
\begin{equation}
\begin{split}
  \frac{\D\mathbf{V}}{\d t}(t_0) =& \sum_{i=1}^n \frac{\D}{\d t} \left(V^i(t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t_0)}\right) \\
  =& \sum_{i=1}^n \frac{\d V^i}{\d t}(t_0) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t_0)} + V^i(t_0) \frac{\D}{\d t}\left(\left.\frac{\partial}{\partial x^i}\right|_{\gamma(t_0)} \right) \\
...
This shows that $\D_t$ is unique if it exists.

For existence, if $\gamma(I)$ is contained in a single chart, we define $\D_t \mathbf{V}$ by $\eqref{equation-4}$. It is easily verified that $\D_t \mathbf{V}$ satisfiess the requisite properties. In the general case, we can cover $\gamma(I)$ with coordinate charts and define $\D_t \mathbf{V}$ by $\eqref{equation-4}$ in each chart, and uniqueness implies the various definitions agree whenever two or more charts overlap.
</details>
</MathBox>
...
$$
  \frac{\d}{\d t}\langle V,\mathbf{W}\rangle = \left\langle \frac{\D\mathbf{V}}{\d t},\mathbf{W} \right\rangle + \left\langle V,\frac{\D\mathbf{W}}{\d t} \right\rangle
$$
...
$$
\begin{align*}
  \frac{\d}{\d t}\langle V,\mathbf{W}\rangle =& \frac{\d}{\d t}\sum_{i,j=1}^n v^i w^j \\
  =& \sum_{i=1}^n \frac{\d v^i}{\d t}w^i + \sum_{i=1}^n v^i \frac{\d w^i}{\d t}
\end{align*}
$$
...
$$
  \frac{\D\mathbf{V}}{\d t} =& \sum_{i=1}^n \frac{\d v^i}{\d t}\mathbf{E}_{i,\gamma(t)} + v^i \frac{\D\mathbf{E}_{i,\gamma(t)}}{\d t} \\
  =& \sum_{i=1}^n \frac{\d v^i}{\d t}\mathbf{E}_{i,\gamma(t)} + v^i \nabla_{c'(t)}\mathbf{E}_{i,\gamma(t)}
$$

Similarly,

$$
  \frac{\D\mathbf{W}}{\d t} =& \sum_{j=1}^n \frac{\d w^j}{\d t}\mathbf{E}_{j,\gamma(t)} + \sum_{j=1}^n \nabla_{c'(t)}\mathbf{E}_{j,\gamma(t)}
$$
...
$$
\begin{align*}
  \left\langle \frac{\D\mathbf{V}}{\d t},\mathbf{W} \right\rangle + \left\langle V,\frac{\D\mathbf{W}}{\d t} \right\rangle =& \sum_{i=1}^n \frac{\d v^i}{\d t}w^i + \sum_{i,j=1}^n v^i w^j \langle \nabla_{c'(t)}\mathbf{E}_{i,\gamma(t)}, \mathbf{E}_{j,\gamma(t)}\rangle \\
  &+ \sum_{i=1}^n v^i \frac{\d w^i}{\d t} + \sum_{i,j=1}^n v^i w^j \langle \mathbf{E}_{i,\gamma(t)},\nabla_{c'(t)}\mathbf{E}_{j,\gamma(t)}\rangle 
\end{align*}
$$
...
$$
\begin{align*}
  \left\langle \frac{\D\mathbf{V}}{\d t},\mathbf{W} \right\rangle + \left\langle V,\frac{\D\mathbf{W}}{\d t} \right\rangle =& \sum_{i=1}^n \frac{\d v^i}{\d t}w^i + \sum_{i=1}^n v^i \frac{\d w^i}{\d t}\mathbf{E}_{j,\gamma(t)} \\
  =& \frac{\d}{\d t}\langle V,\mathbf{W}\rangle
\end{align*}
$$
</details>
</MathBox>
...
$$
\begin{align*}
  \frac{\D\mathbf{V}}{\d t} =& \sum_{i=1}^n \frac{\d V^i}{\d t}\frac{\partial}{\partial x^i} + \sum_{i=1}^n V^i \D_{\gamma'(t)} \frac{\partial}{\partial x^i} \\
  =& \sum_{i=1} \frac{\d V^i}{\d t}\frac{\partial}{\partial x^i} = \frac{\d\mathbf{V}}{\d t}
\end{align*}
$$

Since $\D_{\gamma'(t)}\frac{\partial}{\partial x^i} = 0$.
</MathBox>

### Connection-preserving diffeomorphisms

Although it is in general not possible to push forward a vector field except under a diffeomorphism, it is always possible to push forward a vector field along a curve under any smooth map. Let $f:M\to\tilde{M}$ be a smooth map (not necessarily a diffeomorphism) of manifolds, and $c:[a,b]\to M$ be a smooth curve in $M$. The *pushforward* of the vector fields $\mathbf{V}(t)$ along $c$ in $M$ is the vector field $(\d F \mathbf{V})(t)$ along the image curve $f\circ c$ in $\tilde{M}$ defined by

$$
  (\d F \mathbf{V})(t) = \d F_{\gamma(t)}(\mathbf{V}(t))
$$
...
$$
  \d F (\nabla_\mathbf{X} \mathbf{Y}) = \tilde{\nabla}_{\d F \mathbf{X}} \d F \mathbf{Y}
$$

In this terminology, an isometry of Riemannian manifolds preserves the Riemannian connection.

<MathBox title='' boxType="proposition">
Suppose $F:(M,\nabla)\to(\tilde{M},\tilde{\nabla})$ is a connection-preserving diffeomorphism, $\gamma(t)$ a smooth curve in $M$, and $\frac{\D}{\d t}$, $\frac{\tilde{\D}}{\d t}$ the covariant derivatives along $\gamma$ in $M$ and $f\circ\gamma$ in $\tilde{M}$, respectively. If $\mathbf{V}(t)$ is a vector field along $\gamma$ in $M$, then

$$
  \d F \left(\frac{\D\mathbf{V}}{\d t}\right) = \frac{\tilde{\D}(\d F (\mathbf{V}))}{\d t}
$$

<details>
<summary>Proof</summary>

Choose a neighbourhood $U$ of $\gamma(t)$ on which there is a frame $\Set{\mathbf{e}_i}_{i=1}^n$ and write $\mathbf{V}(t) = \sum_{i=1}^n V^i (t)\mathbf{E}_{i,\gamma(t)}$. Then $\tilde{\mathbf{e}}_i := \d F \mathbf{E}_i$ for $i=1,\dots,n$ is a frame on the neighbourhood $F(U)$ of $F(\gamma(t))$ in $\tilde{M}$ and

$$
  (\d F \mathbf{V})(t) = \sum_{i=1}^n V^i (t)\tilde{\mathbf{E}}_{i,(F\circ\gamma)(t)}
$$

By definition of the covariant derivative

$$
  \frac{\D\mathbf{V}}{\d t}(t) = \sum_{i=1}^n \frac{\d V^i}{\d t}(t)\mathbf{e}_{i,\gamma(t)} + V^i (t)\nabla_{\gamma'(t)}\mathbf{e}_i
$$
...
$$
\begin{align*}
  \left( \d F \frac{\D\mathbf{V}}{\d t} \right)(t) =& \sum_{i=1}^n \frac{\d V^i}{\d t} \d F_{\gamma(t)}(\mathbf{e}_{i,\gamma(t)}) + V^i(t) \tilde{\nabla}_{\d F \gamma'(t)} \d F \mathbf{e}_i \\
  =& \sum_{i=1}^n \frac{\d V^i}{\d t}\tilde{\mathbf{e}}_{i,(F\circ\gamma)(t)} + V^i (t) \tilde{\nabla}_{(F\circ\gamma)'(t)} \tilde{\mathbf{e}}_i \\
  =& \left(\frac{\tilde{D}}{\d t} \d F \mathbf{V} \right)(t)
\end{align*}
$$
</details>
</MathBox>
...
Suppose $\mathbf{X_p} = \sum_{i=1}^n a^i \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}$ is a tangent vector at point $\mathbf{p}\in\R^n$ and $f:\R^n \to\R$ is a smooth function defined in a neighbourhood of $\mathbf{p}$. The *directional derivative* of $f$ at $\mathbf{p}$ along $\mathbf{X_p}$, denoted $\D_{\mathbf{X_p}} f$ is

$$
\begin{align*}
  \D_{\mathbf{X_p}} f =& \lim_{t\to 0} \frac{f(\mathbf{p} + t\mathbf{a}) - f(\mathbf{p})}{t} = \left.\frac{\d}{\d t}\right|_{t=0} f(\mathbf{p} + t\mathbf{a}) \\
  =& \sum_{i=1}^n \left.\frac{\partial f}{\partial x^i}\right|_{\mathbf{p}} \cdot \left.\frac{\d x^i}{\d t}\right|_{t=0} = \sum_{i=1}^n \left.\frac{\partial f}{\partial x^i}\right|_{\mathbf{p}} \cdot a^i \\
...
$$
\begin{align*}
  \D_{\mathbf{X_p}} f =& \mathbf{X_p} f = c'(0) f \\
  =& c_* \left(\left.\frac{\d}{\d t}\right|_{t=0}\right) f \\
  =& \left.\frac{\d}{\d t}\right|_{t=0} f(c(t))
\end{align*}
$$
...
$$
  \D_{\mathbf{X_p}} \mathbf{Y} = \sum_{i=1}^n (\mathbf{X_p} b^i) \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}
$$

This shows that $\D_{\mathbf{X_p}} \mathbf{Y}$ is $\R$-linear in $\mathbf{X_p}$.

When $\mathbf{X}$ is a smooth vector field on $\R^n$, we define the vector field $\D_\mathbf{X} \mathbf{Y}$ on $\R^n$ by

$$
  (\D_\mathbf{X} \mathbf{Y})_\mathbf{p} = \D_{\mathbf{X_p}} \mathbf{Y},\; \forall \mathbf{p}\in\R^n
$$

If $\mathbf{X}$ and $\mathbf{Y}$ are smooth vector fields on $\R^n$, then so is $\D_\mathbf{X} \mathbf{Y}$. Let $\mathfrak{\R^n}$ be the vector space of all smooth vector fields on $\R^n$. The directional derivative in $\R^n$ gives a bilinear map

$$
  \D: \mathfrak{X}(\R^n)\times\mathfrak{X}(\R^n)\to\mathfrak{X}(\R^n)
$$

which we write as $\D_\mathbf{X} \mathbf{Y}$ instead of $\D(\mathbf{X},\mathbf{Y})$. Let $\mathcal{F} = C^\infty(\R^n)$ be the ring of smooth functions on $\R^n$. Then $\mathfrak{X}(\R^n)$ is both a vector space over $\R$ and a module over $\mathcal{F}$.

<MathBox title='Properties of the directional derivative of smooth vector fields' boxType="proposition">
Let $\mathcal{F} = C^\infty(\R^n)$ be the ring of smooth functions on $\R^n$. For smooth vector fields $\mathbf{X}, \mathbf{Y}\in\mathfrak{X}(\R^n)$, the directional derivative $\D_\mathbf{X} \mathbf{Y}$ satisfies the following properties
1. $\D_\mathbf{X} \mathbf{Y}$ is $\mathcal{F}$-linear in $\mathbf{X}$ and $\R$-linear in $\mathbf{Y}$
2. **Leibniz rule:** if $f$ is a smooth function on $\R^n$, then
$$
  \D_\mathbf{X} (f\mathbf{Y}) = (\mathbf{X}f)\mathbf{Y} + f\D_\mathbf{X} \mathbf{Y}
$$
...
$$
\begin{align*}
  (\D_{f\mathbf{X}} \mathbf{Y})_\mathbf{p} =& \D_{f(\mathbf{p})\mathbf{X_p}} \mathbf{Y} \\
  =& f(\mathbf{p})\D_{\mathbf{X_p}} \mathbf{Y} \\
  =& (f\D_\mathbf{X} \mathbf{Y})_\mathbf{p}
\end{align*}
$$
...
$$
  (\D_{\mathbf{X} + \mathbf{Z}} \mathbf{Y})_\mathbf{p} = \D_{\mathbf{X_p}} + \mathbf{Z}_\mathbf{p} \mathbf{Y} = \D_{\mathbf{X_p}} \mathbf{Y} + \D_{\mathbf{Z}_\mathbf{p}} \mathbf{Y} = (\D_\mathbf{X} + \D_\mathbf{Z})_\mathbf{p} 
$$

This proves that $\D_\mathbf{X} \mathbf{Y}$ is $\mathcal{F}$-linear in $\mathbf{X}$. The $\R$-linearity is clear from the directional derivative formula of $\mathbf{Y} = \sum_{i=1}^n b^i \frac{\partial}{\partial x^i}$ along $\mathbf{X_p}$
...
$$
\begin{align*}
  (\D_\mathbf{X} (f\mathbf{Y}))_\mathbf{p} =& \sum_{i=1}^n \mathbf{X_p} (fb^i) \left.\frac{\partial}{\partial x^i}\right|_{\mathbf{p}} \\
  ...
  =& (\mathbf{X_p} f) \mathbf{Y}_\mathbf{p} + f(\mathbf{p})\D_{\mathbf{X_p}}\mathbf{Y} \\
  =& ((\mathbf{X}f)\mathbf{Y} + f\D_\mathbf{X} \mathbf{Y})_\mathbf{p}
\end{align*}
$$
</details>
</MathBox>
...
$$
  \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} = [\mathbf{X}, \mathbf{Y}] = \mathcal{L}_\mathbf{X} \mathbf{Y}
$$
...
Let $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(\R^n)$ be smooth vector fields over $\R^n$. The *torsion* of the directional derivative $\D$ is defined as 

$$
  T(\mathbf{X},\mathbf{Y}) := \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}]
$$
</MathBox>

For each smooth vector field $\mathbf{X}\in\mathfrak{X}(\R^n)$, the directional derivative $\D_\mathbf{X}: \mathfrak{X}(\R^n)\to\mathfrak{X}(\R^n)$ is an $\R$-linear endomorphism. This gives rise to a map $\mathfrak{X}(\R^n) \to\operatorname{end}_\R (\mathfrak{X}(\R^n))$ by $\mathbf{X}\mapsto\D_\mathbf{X}$. The vector space $\mathfrak{X}(\R^n)$ of smooth vector fields on $\R^n$ is a Lie algebra under the Lie bracket of vector fields. For any vector space $V$, the endomorphism ring $\operatorname{end}_\R (\mathbf{V})$ of endormorphisms of $V$ is also a Lie algebra, with Lie bracket

$$
  [A, B] = A \circ B - B \circ A,\; A,B \in\operatorname{end}(\mathbf{V})
$$

This shows that the map $\mathbf{X}\mapsto \D_\mathbf{X}$ is an $\R$-linear map of Lie algebras. A measure fo the deviation of the linear map $\mathbf{X}\mapsto\D_\mathbf{X}$ from being a Lie homomorphism is given by the function

$$
  R(\mathbf{X},\mathbf{Y}) = [\D_\mathbf{X}, \D_\mathbf{Y}] - \D_{[\mathbf{X},\mathbf{Y}]} = \D_\mathbf{X} \D_\mathbf{Y} - \D_\mathbf{Y} \D_\mathbf{X} - \D_{[\mathbf{X},\mathbf{Y}]} \in\operatorname{end}_\R (\mathfrak{X}(M))
$$

called the *curvature* of $\D$.

<MathBox title='' boxType="proposition">
Let $\D$ be the directional derivative in $\R^n$ and $\mathbf{X},\mathbf{Y},\mathbf{Z}\in\mathfrak{X}(\R^n)$ smooth vector fields on $\R^n$. Then
1. **Zero torsion:** $T(\mathbf{X},\mathbf{Y}) = \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}] = 0$
2. **Zero curvature:** $R(\mathbf{X},\mathbf{Y}) = \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} - \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} - \D_{[\mathbf{X},\mathbf{Y}]}\mathbf{Z} = 0$
3. **Compatibility with the metric:** $\mathbf{X}\langle \mathbf{Y},\mathbf{Z} \rangle = \langle \D_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle + \langle\mathbf{Y}, \D_\mathbf{X} \mathbf{Z}\rangle$
...
$$
  \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} = \D_\mathbf{X} \left(\sum_{i=1}^n\mathbf{Y} (c^i) \frac{\partial}{\partial x^i} \right) = \sum_{i=1} (\mathbf{XY}c^i)\frac{\partial}{\partial x^i}
$$

By symmetry,

$$
  \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} = \sum_{i=1}^n (\mathbf{YX}c^i)\frac{\partial}{\partial x^i}
$$
...
$$
\begin{align*}
  \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} - \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} =& \sum_{i=1}^n (\mathbf{XY} - \mathbf{YX})c^i \frac{\partial}{\partial x^i} \\
  =& \D_{[\mathbf{X},\mathbf{Y}]} Z
\end{align*}
$$
...
\begin{align*}
  ...
  =& \langle \D_\mathbf{X} \mathbf{Y},\mathbf{Z} \rangle + \langle\mathbf{Y}, \D_\mathbf{X} \mathbf{Z}\rangle
\end{align*}
$$
</details>
</MathBox>
...
$$
  c'(t) = c_{*,t}\left(\left.\frac{\d}{\d t}\right|_t \right) \in T_{c(t)}M
$$
...
$$
  \frac{\d\mathbf{V}}{\d t}(t) = \sum_{i=1}^n \frac{\d v^i}{\d t}(t) \left.\frac{\partial}{\partial x^i}\right|_{c(t)}
$$
...
$$
  \frac{\d}{\d t}\langle \mathbf{V}(t),\mathbf{W}(t) \rangle = \left\langle\frac{\d\mathbf{V}}{\d t},\mathbf{W}\right\rangle + \left\langle\mathbf{V}, \frac{\d\mathbf{W}}{\d t}\right\rangle
$$
</MathBox>
...
Suppose $M$ is a regular submanifold of $\R^n$. At any point $\mathbf{p}\in M$, if $\mathbf{X_p}\in T_\mathbf{p} M$ is a tangent vector at $\mathbf{p}$ and $\mathbf{Y} = \sum_{i=1}^n b^i \frac{\partial}{\partial x^i}$ is a vector field along $M$ in $\R^n$, then the directional derivative $\D_{\mathbf{X_p}} \mathbf{Y}$ is defined as

$$
  \D_{\mathbf{X_p}} \mathbf{Y} = \sum_{i=1}^n (\mathbf{X_p} b^i) \left.\frac{\partial}{\partial x^i}\right|_\mathbf{p}
$$

As $\mathbf{p}$ varies over $M$, this allows us to associate to a smooth vector field $\mathbf{X}$ on $M$ and a smooth vector field $\mathbf{Y}$ along $M$, a smooth vector field $\D_\mathbf{X} \mathbf{Y}$ along $M$ by

$$
  \D_\mathbf{X} \mathbf{Y} = \sum_{i=1}^n (X b^i) \frac{\partial}{\partial x^i} \in \Gamma(T\R^n |_M)
$$

For any $\mathbf{p}\in M$, we have $(\D_\mathbf{X} \mathbf{Y})_\mathbf{p} = \D_{\mathbf{X_p}} \mathbf{Y}$ by definition. It follows that there is an $\R$-bilinear map $\D:\mathfrak{X}(M)\times\Gamma(T\R^n |_M) \to\Gamma(T\R^n |_M)$ by $\D(\mathbf{X},\mathbf{Y}) = \D_\mathbf{X} \mathbf{Y}$ called the directional derivative on $M$. Because of the asymmetry between $\mathbf{X}$ and $\mathbf{Y}$, where $\mathbf{X}$ is a vector field on $M$ while $\mathbf{Y}$ is a vector field along $M$, the torsion $T(\mathbf{X},\mathbf{Y}) := \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}]$ no longer makes sense. Otherwise, $\D$ satisfies the same properties as the directional derivative $\D$ on $\R^n$. 

<MathBox title='' boxType="proposition">
Let $\mathcal{F} = C^\infty (M)$ be the ring of smooth functions on $M$. Suppose $M$ is a regular submanifold of $\R^n$ and $\D$ is the directional derivative
1. $\D(\mathbf{X},\mathbf{Y}) = \D_\mathbf{X} \mathbf{Y}$ is $\mathcal{F}$-linear in $\mathbf{X}$ and $\R$-linear in $\mathbf{Y}$
2. **Leibniz rule:** if $f\in C^\infty (M)$, then
$$
  \D(\mathbf{X},f\mathbf{Y}) = \D_\mathbf{X} (f\mathbf{Y}) = (\mathbf{X}f)\mathbf{Y} + f\D_\mathbf{X} \mathbf{Y}
$$
</MathBox>

<MathBox title='' boxType="proposition">
Suppose $M$ is a regular submanifold of $\R^n$ and $\D$ is the directional derivative on $M$
1. **Zero curvature:** If $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$ and $Z\in\Gamma(T\R^n |_M)$, then
$$
  \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} - \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} - \D_{[\mathbf{X},\mathbf{Y}]} \mathbf{Z} = 0
$$
2. **Compatibility with the metric:** If $\mathbf{X}\in\mathfrak{X}(M)$ and $\mathbf{Y},\mathbf{Z}\in\Gamma(T\R^n |_M)$, then
$$
  \mathbf{X}\langle\mathbf{Y},\mathbf{Z}\rangle = \langle\D_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle + \langle\mathbf{Y}, \D_\mathbf{X} \mathbf{Z}\rangle
$$
</MathBox>
...
$$
  c'(t)\tilde{v}^i = c_* \left(\frac{\d}{\d t}\right)\tilde{v}^i = \frac{\d}{\d t}(\tilde{v}^i \circ c) = \frac{\d}{\d t}v^i
$$
...
$$
\begin{align*}
  \D_{c'(t)} \tilde{\mathbf{V}} =& \sum_{i=1}^n (c'(t)\tilde{v}^i) \left.\frac{\partial}{\partial x^i}|_{c(t)} \\
  =& \sum_{i=1}^n \frac{\d v^i}{\d t} \left.\frac{\partial}{\partial x^i}\right|_{c(t)} = \frac{\d\mathbf{V}}{\d t}
\end{align*}
$$
</details>
</MathBox>
...
Let $M$ be a smooth manifold. The *acceleration* of a smooth curve $\gamma:I\subset\R\to M$ in $M$ is the covariant derivative of its velocity, i.e. the vector field $\D_t \gamma$ along $\gamma$.
</MathBox>
...
$$
  \frac{\D\gamma'}{\d t} (t) = 0,\; \forall t\in I
$$

The geodesic is *maximal* if its domain $I$ cannot be extended to a larger interval.
</MathBox>

<MathBox title='Geodesic equations' boxType="proposition" tag="22">
...
$$
  \frac{\d^2 y^k}{\d t^2} (t) + \sum_{i,j=1}^n \frac{\d y^i}{\d t}(t) \frac{\d y^j}{\d t}(t) \Gamma_{ij}^k (y(t))  = 0,\; k = 1,\dots,n
$$
...
$$
\begin{align*}
  \gamma'(t) =& \sum_{i=1}^n \frac{\d y^i}{\d t}(t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)} \\
  =& \sum_{i=1}^n \dot{y}^i (t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)}
\end{align*}
$$

The acceleration $\D_t \gamma'$ becomes

$$
\begin{align*}
  \frac{\D\gamma'}{\d t}(t) =& \sum_{i=1}^n \frac{\D}{\d t}\left( \sum_{i=1}^n \dot{y}^i(t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)} \right) \\
  =& \sum_{i=1}^n \frac{\d \dot{y}^i}{\d t}(t) \left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)} + \dot{y}^i (t) \frac{\D}{\d t}\left(\left.\frac{\partial}{\partial x^i}\right|_{\gamma(t)}\right) \\
...
Since $\gamma$ is geodesic if $\D_t \gamma'(t) \equiv 0$, it follows that $\gamma$ is a geodesic if and only if
...
<MathBox title='Picard-Lindelöf theorem (existence and uniqueness of first-order ODEs)' boxType="theorem" tag="3">
...
<MathBox title='Existence and uniqueness of geodesics' boxType="theorem" tag="4">
...
<MathBox title='Rescaling ' boxType="proposition">
...
$$
\begin{equation}
  \gamma_\mathbf{v} (kt) = \gamma_{k\mathbf{v}}(t),\; k\in\R \tag{\label{equation-12}}
\end{equation}
$$
...
It suffices to show that $\gamma_{k\mathbf{v}} (t)$ exists and $\eqref{equation-12}$ holds whenever the right-hand side is defined, for then the converse statement follows by replacing $\mathbf{v}$ by $k\mathbf{v}$, $t$ by $kt$ and $k$ by $1/k$.

Suppose the domain of $\gamma_\mathbf{v}$ is the open interval $I\subset\R$. Define a new curve $\tilde{\gamma}$ by $\tilde{\gamma}(t) = \gamma(kt)$ defined on $k^{-1} I = \Set{t | kt \in I}$. We will show that $\tilde{\gamma}$ is a geodesic with initial point $\mathbf{p}$ and initial velocity $k\mathbf{v}$. It then follows by the uniqueness that it must be equal to $\gamma_{k\mathbf{v}}$.

It is immediate from the definition that $\tilde{\gamma}(0) = \gamma_\mathbf{v}(0) = \mathbf{p}$. Writing $\gamma_\mathbf{v} (t) = (\gamma^i (t))^n$ in local coordinates, the chain rule gives

$$
\begin{align*}
  \dot{\tilde{\gamma}}^i (t) =& \frac{\d t}{\d t} \gamma^i (kt) \\
  =& k\dot{\gamma}^i (kt)
\end{align*}
$$

In particular, it follows that $\dot{\tilde{\gamma}} (0) = k\dot{\gamma}_\mathbf{v}(0) = k\mathbf{v}$.

Let $\D_t$ and $\tilde{D}_t$ denote the covariant differentiation operators along $\gamma$ and $\tilde{\gamma}$, respectively. Using the chain rule again in local coordinates

$$
\begin{align*}
  \tilde{\D}_t \dot{\tilde{\gamma}}(t) =& \left(\frac{\d}{\d t} \dot{\tilde{\gamma}}^k (t) + \Gamma_{ij}^k (\tilde{\gamma}(t) \dot{\tilde{\gamma}})^i (t) \dot{\tilde{\gamma}}^j (t) \right)\boldsymbol{\partial}_k \\
  =& (k^2 \ddot{\gamma}_\mathbf{v}(kt) + k^2 \Gamma_{ij}^k (\gamma_\mathbf{v}(kt))) \dot{\gamma}^i (kt) \dot{\gamma}^j (kt) \boldsymbol{\partial}_k \\
  =& k^2 \D_t \dot{\gamma}_\mathbf{v} (kt) = 0
\end{align*}
$$

Thus $\tilde{\gamma}$ is a geodesic, implying that $\tilde{\gamma} = \gamma_{k\mathbf{v}}$.
</details>
</MathBox>
...
$$
  \tilde{\mathbf{T}}(t) = \d F \left(\d\gamma \left(\frac{\d}{\d t} \right)\right) = \d F (\gamma'(t)) = \d F (\mathbf{T})
$$

Let $\D_t$ and $\tilde{\D}_t$ be the covariant derivatives along $\gamma(t)$ and $(F\circ \gamma)(t)$, respectively. Then $\D_t T \equiv 0$, because $\gamma(t)$ is a geodesic. If follows that

$$
  \frac{\tilde{\D}\tilde{\mathbf{T}}}{\d t} = \d fF\left(\frac{\D\mathbf{T}}{\d t}\right) = \d F(0) = 0
$$
...
Let $M$ be a smooth manifold with an affine connection $\nabla$. A smooth vector field $\mathbf{V}\in\mathfrak{X}(M)$ along a curve $\gamma:I\subset\R\to M$ is *parallel* along $\gamma$ with respect to $\nabla$ if $\D_t \mathbf{V} \equiv 0$. A vector field $\mathbf{V}\in\mathfrak{X}(M)$ on $M$ is *parallel* if it is parallel along every curve.
</MathBox>

<MathBox title='Parallel translation' boxType="theorem" tag="5">
...
$$
  \mathbf{V}(t) = V^i (t) \boldsymbol{\partial}_i |_{\gamma(t)}
$$
...
$$
\begin{align*}
  \frac{\D\mathbf{V}}{\d t} = \left(\dot{V}^k(t) + V^j(t) \dot{\gamma}^i(t) \Gamma_{ij}^k (\gamma(t)) \right)\boldsymbol{\partial}_k|_{\gamma(t)}
\end{align*}
$$

Since $\mathbf{V}$ is parallel along $\gamma$ if $\D_t \mathbf{V} \equiv 0$, it follows that $\mathbf{V}$ is parallel along $\gamma$ if and only if
...
$$
  P_{t, t_0}^\gamma: T_{\gamma(t_0)}M \to T_{\gamma(t)} M
$$

by $P_{t, t_0}^\gamma (\mathbf{V}_0) = \mathbf{V}(t)$ where $\mathbf{V}$ is a parallel vector field along $\gamma$ with $\mathbf{V}(t_0) = \mathbf{V}_0$. The collection of maps $P_\gamma (t, t_0)$ for $t, t_0 \in I$ is called *parallel transport* along $\gamma$.
</MathBox>

<MathBox title='Parallel transport is a linear isomorphism' boxType="proposition">
Let $M$ be a smooth manifold with an affine connection $\nabla$ and suppose $\gamma:I\subset\R\to M$ is a curve in $M$. The parallel transport $P_{t, t_0}^\gamma: T_{\gamma(t_0)}M \to T_{\gamma(t)} M$ for $t, t_0 \in I$ is a linear isomorphism.

<details>
<summary>Proof</summary>

Let $\mathbf{V}$ be a vector field along $\gamma$ with coordinate representation
...
$$
  P_{t, t_0}^\gamma (\mathbf{V}(t_0)) = P_{t, t_0}^\gamma (V^i (t_0) \boldsymbol{\partial}_i|_{\gamma(t_0)}) = V^i (t) \boldsymbol{\partial}_i |_{\gamma(t)} = \mathbf{V}(t)
$$

where $\mathbf{V}(t)$ is parallel along $\gamma$. Since this means that $\D_t \mathbf{V} \equiv 0$, it follows that its components satisfy the linear system of first-order ODEs

$$
  \dot{V}^k (t) = -V^j (t) \dot{\gamma}^i (t) \Gamma_{ij}^k (\gamma(t)),\; k=1,\dots,n
$$

with initial condition $\mathbf{V}(t_0)$. By the superposition principle, any vector field $\mathbf{V} = a\mathbf{U} + b\mathbf{W}$ along $\gamma$ for $a,b\in\R$ is also a solution to the linear system of ODEs with initial condition $\mathbf{V}(t_0) = a\mathbf{U}(t_0) + b \mathbf{W}(t_0)$ such that

$$
  P_{t, t_0}^\gamma (a\mathbf{U}(t_0) + b\mathbf{W}(t_0)) = a P_{t, t_0}^\gamma (\mathbf{U}(t_0)) + b P_{t, t_0}^\gamma (\mathbf{W}(t_0))
$$

This shows that $P_{t, t_0}^\gamma$ is linear.

To see that $P_{t, t_0}^\gamma$ is invertible, fix $t\in I$ and let $\tilde{\gamma}(s) := \gamma(t - s)$, i.e. the reverse of $\gamma$. Define $P_{t-t_0, 0}^{\tilde{\gamma}} : T_{\gamma(t)} M \to T_{\gamma(t_0)} M$ by

$$
  P_{t-t_0, 0}^{\tilde{\gamma}} (\tilde{\mathbf{V}}(0)) = P_{t-t_0, 0}^{\tilde{\gamma}} (\mathbf{V}(t)) = \tilde{\mathbf{V}}(t - t_0) = \mathbf{V}(t_0)
$$

for a vector field $\tilde{\mathbf{V}}(s) = \mathbf{V}(t - s)$ along $\tilde{\gamma}$. Since $\tilde{\mathbf{V}}(t - t_0)$ is parallel along $\tilde{\gamma}$ it follows that $0 \equiv \tilde{D}_t \tilde{\mathbf{V}}(t) = \tilde{D}_t \mathbf{V}(t - s)$. This shows that $P_{t, t_0}^\gamma \circ P_{t - t_0, 0}^{\tilde{\gamma}} = \operatorname{id}_{T_{\gamma(t_0)} M}$. Thus, $P_{t-t_0, 0}^{\tilde{\gamma}}$ is the inverse of $P_{t,t_0}^\gamma$.
</details>
</MathBox>

<MathBox title='Covariant derivative along curves in terms of parallel transport' boxType="proposition">
...
$$
  \frac{\D\mathbf{V}}{\d t}(t_0) = \lim_{t\to t_0} \frac{(P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) - \mathbf{V}(t_0)}{t - t_0}
$$

where $P_{t_0, t}^\gamma :T_{\gamma(t_0)}M \xrightarrow{\sim} T_{\gamma(t)}M$ is the parallel transport along $\gamma$.

<details>
<summary>Proof</summary>

Let $\Set{\mathbf{e}_i}_{i=1}^n$ be a basis of $T_{\gamma(t_0)}M$ and define $\mathbf{E}_i (t) = P_{t_0, t}^\gamma (\mathbf{e}_i) \in T_{\gamma(t)}M$. Then we can write

$$
  \mathbf{V}(t) = V^i (t) \mathbf{E}_i (t) = V^i (t) P_{t,t_0}^\gamma (\mathbf{e}_i) = P_{t,t_0}^\gamma (V^i (t)\mathbf{e}_i)
$$

such that

$$
  (P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) = V^i (t) \mathbf{E}_i (t)
$$
...
$$
\begin{align*}
  \lim_{t\to t_0} \frac{(P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) - \mathbf{V}(t_0)}{t - t_0} &= \lim_{t\to t_0} \frac{V^i (t)\mathbf{E}_i (t) - V^i(t_0)\mathbf{E}_i (t_0)}{t - t_0} \\
  &= \dot{V}^i (t_0)\mathbf{E}_i (t_0)
\end{align*}
$$
...
$$
\begin{align*}
  \D_t \mathbf{V}(t_0) =& \D_t \left(V^i (t_0) \mathbf{E}_i (t_0) \right) \\
  =& (\D_t (V^i(t_0))\mathbf{E}_i (t_0) + V^i (t_0) \underbrace{\D_t \mathbf{E}_i (t_0)}_{=0} \\
  =& \dot{V}^i (t_0)\mathbf{E}_i (t_0)
\end{align*}
$$
</details>
...
$$
\begin{align*}
  \frac{\D\mathbf{V}}{\d t} =& \sum_{i=1}^n \dot{v}^i \mathbf{e}_{i,c(t)} + \sum_{i=1}^n v^i \frac{\D\mathbf{e}_{i,c(t)}}{\d t} \\
...
Thus $\frac{\D\mathbf{V}}{\d t} = 0$ if and only if
...
Because the parallel transport equation $\frac{\D\mathbf{V}}{\d t} = 0$ is $\R$-linear in $\mathbf{V}$, a linear combination with constant coefficients or parallel vector fields alon $c$ is again parallel along $c$.
...
On a Riemannian manifold $M$ parallel translation preserves length and inner product: if $\mathbf{V}(t)$ and $\mathbf{W}(t)$ are parallel vector fields along a smooth curve $c:[a,b]\to M$, then the length $\norm{\mathbf{V}(t)}$ and the inner product $\langle\mathbf{V}(t),W(t)\rangle$ are constant for all $t\in[a,b]$.

<details>
<summary>Proof</summary>

Since $\norm{\mathbf{V}(t)} = \sqrt{\langle\mathbf{V}(t),\mathbf{V}(t)\rangle}$, it suffices to prove that $\langle\mathbf{V}(t),\mathbf{W}(t)\rangle$ is constant. By the product rule for the covariant derivative of a connection compatible with the metric

$$
  \frac{\d}{\d t}\langle\mathbf{V},\mathbf{W}\rangle = \left\langle\frac{\D\mathbf{V}}{\d t},\mathbf{W}\right\rangle + \left\langle\mathbf{V},\frac{\D\mathbf{W}}{\d t}\right\rangle = 0
$$

since $\frac{\D\mathbf{V}}{\d t} = 0$ and $\frac{\D\mathbf{W}}{\d t} = 0$. Thus, $\langle\mathbf{V}(t),\mathbf{W}(t)\rangle$ is constant as a function of $t$.
</details>
</MathBox>
...
$$
  \cos(\theta) = \frac{\langle\mathbf{u},\mathbf{v}\rangle}{\norm{\mathbf{v}}\cdot\norm{\mathbf{v}}},\; 0\leq\theta\leq\pi
$$
...
<MathBox title='' boxType="theorem">
Let $M$ be a Riemannian manifold. For any point $\mathbf{p}\in M$, there are a neighbourhood $U$ of $\mathbf{p}$ and numbers $\delta, a > 0$ so that for any $\mathbf{q}\in U$ and $\mathbf{v}\in T_\mathbf{q}M$ with $\norm{\mathbf{v}} < \delta$, there is a unique geodesic $\gamma:(-a,a)\to M$ with $\gamma(0) = \mathbf{q}$ and $\gamma'(0) = \mathbf{v}$. 
</MathBox>

<MathBox title='' boxType="corollary">
Let $M$ be a Riemannian manifold. For any point $\mathbf{p}\in M$, there are a neighbourhood $U$ of $\mathbf{p}$ and a real number $\varepsilon > 0$ so that for any $\mathbf{q}\in U$ and $\bar{\mathbf{v}}\in T_\mathbf{q}M$ with $\norm{\bar{\mathbf{v}}} < \varepsilon$, there is a unique geodesic $\bar{\gamma}:(-2,2)\to M$ with $\bar{\gamma}(0) = \mathbf{q}$ and $\bar{\gamma}'(0) = \mathbf{v}$.
...
$$
  \norm{\mathbf{v}} = \norm{\gamma'(0)} < \delta \iff \norm{\bar{\mathbf{v}}} = \norm{\bar{\gamma}'(0)} = k\norm{\mathbf{v}} < k\delta = \frac{a\delta}{2}
$$
...
<MathBox title='Metric compatility for affine connections' boxType="definition">
On a Riemannian manifold $(M, g)$, an affine connection $\nabla$ is *metric compatible* if it satisfies the following product rule for all $\mathbf{X},\mathbf{Y},\mathbf{Z}\in\mathfrak{X}(M)$
...
On a Riemannian manifold $(M,g)$, an affine connection $\nabla$ is called a *Levi-Civita connection*, or a *Riemannian connection*, if $\nabla$ torsion-free and compatible with $g$. 
</MathBox>
...
We need to show that if $\tilde{\mathbf{X}}\in\mathfrak{X}(M)$ and $\langle\mathbf{X},\mathbf{Z}\rangle = \langle\tilde{\mathbf{X}},\mathbf{Z}\rangle$ for all $\mathbf{Z}\in\mathfrak{X}(M)$, then $\mathbf{X} = \tilde{\mathbf{X}}$. With $\mathbf{Y} = \mathbf{X} - \tilde{\mathbf{X}}$, this is equivalent to showing that if $\langle\mathbf{Y},\mathbf{Z}\rangle = 0$ for all $\mathbf{Z}\in\mathfrak{X}(M)$, then $\mathbf{Y} = 0$. Take $\mathbf{Z} = \mathbf{Y}$. By the positive-definiteness of Riemannian metric at point $\mathbf{p}\in M$

$$
\begin{align*}
  \langle\mathbf{Y},\mathbf{Y}\rangle = 0 \implies& \langle\mathbf{Y_p}, \mathbf{Y_p}) = 0 \; \forall \mathbf{p}\in M \\
...
<MathBox title='Fundamental theorem of Riemannian geometry' boxType="theorem">
On a Riemannian manifold $(M,g)$ there is a unique Levi-Civita connection $\nabla$ with Christoffel symbols given in local coordinates by

$$
\begin{equation}
  \Gamma_{ij}^k = \frac{1}{2} g^{kl} (\boldsymbol{\partial}_i g_{jl} + \boldsymbol{\partial}_j g_{il} - \boldsymbol{\partial}_l g_{ij}) \tag{\label{equation-11}}
\end{equation}
$$
...
Suppose $\nabla$ is a Levi-Civita connection on $M$ and let $\mathbf{X},\mathbf{Y},\mathbf{Z}\in\mathfrak{X}(M)$ be arbitrary smooth vector fields. We prove uniqueness first by deriving a formula for $\nabla$. By lemma $\ref{lemma-5}$, to specify $\nabla_\mathbf{X} \mathbf{Y}$, it suffices to know $\langle\nabla_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle$. 
...
Cyclically permuting $\mathbf{X},\mathbf{Y},\mathbf{Z}$ in the compatibility equation gives two additional equations
...
Applying the torsion-free equation on the last term in each of the compatibility equations, we obtain

$$
\begin{align*}
  \mathbf{X}\langle\mathbf{Y},\mathbf{Z}\rangle =& \langle\nabla_\mathbf{X} \mathbf{Y}, \mathbf{Z}\rangle + \langle\mathbf{Y},\nabla_\mathbf{Z} \mathbf{X}\rangle + \langle\mathbf{Y}, [\mathbf{X},\mathbf{Z}]\rangle \\
  \mathbf{Y}\langle\mathbf{Z},\mathbf{X}\rangle =& \langle\nabla_\mathbf{Y} \mathbf{Z}, \mathbf{X}\rangle + \langle\mathbf{Z},\nabla_\mathbf{X} \mathbf{Y}\rangle + \langle\mathbf{Z}, [\mathbf{X},\mathbf{Y}]\rangle \\
  \mathbf{Z}\langle \mathbf{X},\mathbf{Y}\rangle =& \langle\nabla_\mathbf{Z} \mathbf{X},\mathbf{Y}\rangle + \langle\mathbf{X}, \nabla_\mathbf{Y} \mathbf{Z}\rangle + \langle\mathbf{X}, [\mathbf{Z}, \mathbf{Y}]\rangle
\end{align*}
$$
...
$$
\begin{align*}
  \mathbf{X}\langle\mathbf{Y},\mathbf{Z}\rangle +& \mathbf{Y}\langle\mathbf{Z},\mathbf{X}\rangle - \mathbf{Z}\langle \mathbf{X},\mathbf{Y}\rangle =& 2\langle\nabla_\mathbf{X} \mathbf{Y}, \mathbf{Z}\rangle + \langle\mathbf{Y}, [\mathbf{X},\mathbf{Y}]\rangle + \langle\mathbf{Z},[\mathbf{Y},\mathbf{X}]\rangle - \langle\mathbf{X}, [\mathbf{Z},\mathbf{Y}]\rangle
\end{align*}
$$
...
\begin{equation}
\begin{split}
  \langle\nabla_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle =& \frac{1}{2}\left(\mathbf{X}\langle \mathbf{Y},\mathbf{Z}\rangle + \mathbf{Y}\langle \mathbf{Z},\mathbf{X}\rangle - \mathbf{Z}\langle\mathbf{X},\mathbf{Y}\rangle \\
  &\left.- \langle\mathbf{X},[\mathbf{Y},\mathbf{Z}]\rangle + \langle\mathbf{Y},[\mathbf{Z},\mathbf{X}]\rangle + \langle\mathbf{Z},[\mathbf{X},\mathbf{Y}]\rangle\right)
\end{split}\tag{\label{equation-10}}
\end{equation}
$$

Now suppose $\nabla^1$ and $\nabla^2$ are two symmetric and metric-compatible connections. Since the right-hand side of $\eqref{equation-10}$ does not depend on the connection, it follows that $\langle \nabla_\mathbf{X}^1 \mathbf{Y} - \nabla_\mathbf{X}^2 \mathbf{Y}, \mathbf{Z}\rangle = 0$ for all $\mathbf{X},\mathbf{Y},\mathbf{Z}\in\mathfrak{X}(M)$. By the positive definiteness of $g$, this can only happen if $\nabla_\mathbf{X}^1 \mathbf{Y} = \nabla_\mathbf{X}^2 \mathbf{Y}$, hence $\nabla^1 = \nabla^2$.

To prove existence, it suffices to show that such a connection exists in each coordinate chart; uniqueness ensures that such the connections constructed in different charts agree where they overlap.

Let $(U, (x^i)_{i=1}^n)$ be any local chart. Applying $\eqref{equation-10}$ to the coordinate vector fields, whose Lie brackets are zero, we get

$$
  \langle\nabla_{\boldsymbol{\partial}_i} \boldsymbol{\partial}_j, \boldsymbol{\partial}_l \rangle = \frac{1}{2}\left(\boldsymbol{\partial}_i \langle\boldsymbol{\partial}_j, \boldsymbol{\partial}_l \rangle + \boldsymbol{\partial}_j \langle\boldsymbol{\partial}_l, \boldsymbol{\partial}_i \rangle - \boldsymbol{\partial}_l \langle\boldsymbol{\partial}_i, \boldsymbol{\partial}_j \rangle \right)
$$

Applying the definitions of the metric coefficients and the Christoffel symbols

$$
  g_{ij} = \langle\boldsymbol{\partial}_i, \boldsymbol{\partial}_j\rangle,\quad \nabla_{\boldsymbol{\partial}_i} \boldsymbol{\nabla}_j = \Gamma_{ij}^m \boldsymbol{\partial}_m 
$$

we get

$$
  \Gamma_{ij}^m g_{ml} = \frac{1}{2} \left(\boldsymbol{\partial}_i g_{jl} + \boldsymbol{\partial}_j g_{il} - \boldsymbol{\partial}_l g_{ij} \right)
$$

Multiplying both sides by the inverse matrix $g^{lk}$ and noting that $g_{ml} g^{lk} = \delta_m^k$, we get

$$
  \Gamma_{ij}^k = \frac{1}{2} g^{kl} (\boldsymbol{\partial}_i g_{jl} + \boldsymbol{\partial}_j g_{il} - \boldsymbol{\partial}_l g_{ij})
$$

This formula certainly defines a connection in each chart. Since $g$ is symmetric, it is evident from the formula that $\Gamma_{ij}^k = \Gamma_{ji}^k$, so the connection is symmetric. Thus only compatibility with $g$ needs to be checked. By lemma, it suffices to show that $\nabla g = 0$. In local coordinates, the components of $\nabla g$ are

$$
  g_{ij;k} = \boldsymbol{\partial}_k g_{ij} - \Gamma_{ki}^l g_{lj} - \Gamma_{kj}^l g_{il}
$$

Applying $\eqref{equation-11}$ twice, we conclude that

$$
\begin{align*}
  \Gamma_{ki}^l g_{lj} + \Gamma_{kj}^l g_{il} =& \frac{1}{2}(\boldsymbol{\partial}_k g_{ij} + \boldsymbol{\partial}_i g_{kj} - \boldsymbol{\partial}_j g_{ki}) + \frac{1}{2}(\boldsymbol{\partial}_k g_{ji} + \boldsymbol{\partial}_j g_{ki} - \boldsymbol{\partial}_i g_{kj}) \\
  =& \boldsymbol{\partial}_k g_{ij}
\end{align*}
$$

which shows that $g_{ij;k} = 0$.
</details>
</MathBox>

The Levi-Civita connection on $\R^n$ with its usual Euclidean metric is the directional derivative $\D$ on $\R^n$.
...
$$
  \d F_\mathbf{p} (\mathbf{X_p}) = \mathbf{X_p}(f\circ F)
$$
...
The speed of a curve $c:[a,b]\to M$ in a Riemannian manifold $M$ is defined as the magnitude $\norm{ c'(t) }$ of its velocity $c'(t)$. The arc length of the curve is
...
The function $s:[a,b]\to[0,\ell]$ is the arc length function of the curve $c$. By the fundamental theorem of calculus, the derivative of $s$ with respect to $t$ is $s'(t) = \norm{ c'(t) }$, the speed of $c$.
...
Because $c(t)$ is regular, $s'(t) = \norm{ c'(t) }$ is never zero, i.e. $s'(t) > 0$ for all $t\in[a,b]$. This implies that $s(t)$ is a monotonically increasing function, and so has an inverse $t(s)$. By the inverse function theorem, $t$ is a smooth function of $s$.
</details>
</MathBox>
...
$$
  \norm{\gamma'(s) } = \norm{ c'(t(s)) } \cdot |t'(s)| = \frac{\d s}{\d t}\left|\frac{\d t}{\d s}\right| = \left|\frac{\d s}{\d t} \frac{\d t}{\d s}\right| = 1
$$

Conversely, if a curve $c(t)$ has unit speed, then its arc length is

$$
  s(t) = \int_a^t \norm{ c'(u) }\;\d u = \int_a^t 1\;\d u = t - a
$$
...
$$
\begin{align*}
  s =& \int_0^t \norm{ c'(u) }\;\d u = \int_0^t \Norm{\begin{bmatrix} -a\sin t \\ a\cos t \end{bmatrix}}\;\d u \\
  =& \int_0^t a\;\d u = at
\end{align*}
$$
...
$$
  \mathbf{T}'(s) = \frac{\d\mathbf{T}}{\d s}(s) = \gamma''(s)
$$
...
$$
\begin{align*}
  0 =& \left.\frac{\d}{\d t}\right|_{t=0} f(c(t)) \\
...
$$
  L_\mathbf{p}(\mathbf{X_p}) := -\D_{\mathbf{X_p}} \mathbf{N}
$$
...
$$
\begin{align*}
  0 = \mathbf{X} \langle\mathbf{Y},\mathbf{N} \rangle =& \langle\D_\mathbf{X} \mathbf{Y},\mathbf{N} \rangle + \langle\mathbf{Y}, \D_\mathbf{X} \mathbf{N} \rangle \\
  =& \langle \D_\mathbf{X} \mathbf{Y},\mathbf{N} \rangle - \langle\mathbf{Y}, L(\mathbf{X}) \rangle  
\end{align*}
$$
...
$$
  \langle L(\mathbf{X_p}), \mathbf{Y}\rangle = \langle \D_\mathbf{X} \mathbf{Y},\mathbf{N} \rangle
$$
...
$$
  \langle L(\mathbf{Y}),\mathbf{X}\rangle = \langle\D_\mathbf{Y} \mathbf{X},\mathbf{N} \rangle
$$

By the properties of the directional derivative, we have $\D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} = [\mathbf{X},\mathbf{Y}]$. Combining these equations, we get

$$
\begin{align*}
  \langle L(\mathbf{X}),\mathbf{Y}\rangle - \langle L(\mathbf{Y}),\mathbf{X}\rangle =& \langle \D_\mathbf{X} \mathbf{Y},\mathbf{N} \rangle - \langle\D_\mathbf{Y} \mathbf{X},\mathbf{N} \rangle \\
  =& \langle\D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X},\mathbf{N}\rangle \\
...
$$
  \langle L(c'(t)), \mathbf{V}(t)\rangle = \left\langle \frac{\d\mathbf{V}}{\d t}(t), \mathbf{N}_{c(t)} \right\rangle
$$
...
$$
  0 =& \frac{\d}{\d t} \langle \mathbf{V}(t), \mathbf{N}_{c(t)}\rangle \\
  =& \left\langle \frac{\d\mathbf{V}}{\d t}(t), \mathbf{N}_{c(t)} \right\rangle + \left\langle \mathbf{V}(t), \frac{\d}{\d t}\mathbf{N}_{c(t)} \right\rangle \\
  =& \left\langle \frac{\d\mathbf{V}}{\d t}(t), \mathbf{N}_{c(t)} \right\rangle + \langle \mathbf{V}(t), \D_{c'(t)}\mathbf{N} \rangle
$$

Thus

$$
  \langle \mathbf{V}(t), L(c'(t)) \rangle = \langle \mathbf{V}(t), -\D_{c'(t)} N\rangle = \left\langle \frac{\d\mathbf{V}}{\d t}, \mathbf{N}_{c(t)} \right\rangle
$$
</details>
</MathBox>
...
\begin{align*}
  ...
  =& \left\langle\frac{\d T}{\d s},\mathbf{N}_{\gamma(s)}\right\rangle \\
...
Consider a smooth, not necessarily orientable surface $M$ in $\R^3$. Given two smooth vector fields $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$ and a point $\mathbf{p}\in M$, the directional derivative $\D_{\mathbf{X_p}} \mathbf{Y}$ is not in general tangent to $M$. Define

$$
  (\nabla_\mathbf{X} \mathbf{Y})_\mathbf{p} := \nabla_{\mathbf{X_p}} \mathbf{Y} = \operatorname{pr}_\mathbf{p}(\D_{\mathbf{X_p}} \mathbf{Y}) \in T_\mathbf{p} M
$$
...
$$
  \nabla_\mathbf{X} \mathbf{Y} = \operatorname{pr}(\D_\mathbf{X} \mathbf{Y})
$$
...
Suppose $M$ is a regular submanifold of $\R^n$. For $\mathbf{X}$ a tangent vector field on $M$ and $\mathbf{Z}$ a vector field along $M$ in $\R^n$, the directional derivative $\D_\mathbf{X} \mathbf{Z}$ is a vector field along $M$ in $\R^n$. Since the Levi-Civita connection $\nabla$ of a surface $M$ in $\R^3$ is defined in terms of the directional derivative $\D$ on $M$, it is easy to compare the curvature tensors of $\nabla$ and $\D$. This will lead to a formula for the curvature $R$ of $\nabla$, called the *Gauss curvature equation*.

A vector field $\mathbf{Y}\in\mathfrak{X}(M)$ is also a vector field along $M$ in $\R^3$. Hence, if $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$, the directional derivative $\D_\mathbf{X} \mathbf{Y}$ makes sense. Assume $M$ is oriented with a unit normal vector field $\mathbf{N}$. At any point $\mathbf{p}\in M$, the normal component of the vector $\D_{\mathbf{X_p}} \mathbf{Y}$ is $\langle \D_{\mathbf{X_p}} \mathbf{Y}, \mathbf{N}_\mathbf{p}\rangle \mathbf{N}_\mathbf{p}$, and therefore the tangent component is

$$
  \operatorname{pr}(\D_{\mathbf{X_p}} \mathbf{Y}) = \D_{\mathbf{X_p}} \mathbf{Y} - \langle \D_{\mathbf{X_p}} \mathbf{Y}, \mathbf{N}_\mathbf{p}\rangle \mathbf{N}_\mathbf{p}
$$
...
$$
\begin{align*}
  \D_{\mathbf{X_p}} \mathbf{Y} =& \nabla_{\mathbf{X_p}}\mathbf{Y} + \langle \D_{\mathbf{X_p}} \mathbf{Y}, \mathbf{N}_\mathbf{p}\rangle \mathbf{N}_\mathbf{p} \\
...
$$
  \D_\mathbf{X} \mathbf{Y} = \nabla_\mathbf{X}\mathbf{Y} + \langle L(\mathbf{X}),\mathbf{Y}\rangle\mathbf{N}
$$

which decomposes the vector field $\D_\mathbf{X} \mathbf{Y}$ into its tangential and normal components.
...
$$
  R_\D(\mathbf{X},\mathbf{Y})\mathbf{Z} := \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} - \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} - \D_{[\mathbf{X},\mathbf{Y}]}\mathbf{Z} = 0
$$

Decomposing $\D_\mathbf{Y} Z$ into its tangential and normal components we get $\D_\mathbf{Y} \mathbf{Z} = \lambda_Y \mathbf{Z} + \langle L(\mathbf{Y}),\mathbf{Z}\rangle\mathbf{N}$. Thus,

$$
\begin{align*}
  \D_\mathbf{X} \D_\mathbf{Y} \mathbf{Z} =& \D_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} + \D_\mathbf{X} (\langle L(\mathbf{Y}),\mathbf{Z}\rangle\mathbf{N}) \\
  =& \nabla_\mathbf{X} \nabla_\mathbf{Y} \mathbf{Z} + \langle L(\mathbf{X}), \nabla_\mathbf{Y} \mathbf{Z}\rangle \mathbf{N} + \mathbf{X}\langle L(\mathbf{Y}),\mathbf{Z}\rangle \mathbf{N} + \langle L(\mathbf{Y}),\mathbf{Z}\rangle\D_\mathbf{X} \mathbf{N} \\
...
$$
  \D_\mathbf{Y} \D_\mathbf{X} \mathbf{Z} = \nabla_\mathbf{Y} \nabla_\mathbf{X} \mathbf{Z} - \langle L(\mathbf{X}),\mathbf{Z}\rangle L(\mathbf{Y}) + \langle L(\mathbf{Y}), \nabla_\mathbf{X} \mathbf{Z}\rangle \mathbf{N} + \mathbf{Y}\langle L(\mathbf{X}),\mathbf{Z}\rangle\mathbf{N}
$$

Also

$$
  \D_{[\mathbf{X},\mathbf{Y}]} \mathbf{Z} = \nabla_{[\mathbf{X},\mathbf{Y}]}\mathbf{Z} + \langle L([\mathbf{X},\mathbf{Y}]),\mathbf{Z}\rangle\mathbf{N}
$$

Subtracting $\D_\mathbf{X} \D_\mathbf{X} \mathbf{Z}$ and $\D_{[\mathbf{X},\mathbf{Y}]} Z$ from $\D_\mathbf{X} \D_\mathbf{Y} Z$ shows that $R_\D$ can be decomposed into a tangential and normal component

$$
\begin{align*}
  0 =& R_\D(\mathbf{X},\mathbf{Y})\mathbf{Z} \\
...
From the proof of the Gauss curvature equation, the normal component of $R_\D$ satisfies
...
A *hypersurface* $M$ in $\R^{n+1}$ is a submanifold of codimension $1$. Assume there is a smooth unit normal vector field $\mathbf{N}$ on $M$; note that this is always possible locally on any hypersurface. Recall that the directional derivative $\D$ on $\R^{n+1}$ is the Levi-Civita connection of $\R^{n+1}$.

For any point $\mathbf{p}\in M$ and tangent vector $\mathbf{X_p}\in T_\mathbf{p}M$, since $\langle \mathbf{N},\mathbf{N}\rangle\equiv 1$

$$
  0 = \mathbf{X_p}\langle\mathbf{N},\mathbf{N}\rangle = 2\langle\D_{\mathbf{X_p}} \mathbf{N},\mathbf{N}\rangle
$$

Therefore, $\D_{\mathbf{X_p}}\mathbf{N}$ is tangent to $M$. The *shape operator* $L_\mathbf{p}: T_\mathbf{p}M \to T_\mathbf{p}M$ is defined to be

$$
  L_\mathbf{p}(\mathbf{X_p}) = -\D_{\mathbf{X_p}}\mathbf{N},\; \mathbf{X_p}\in T_\mathbf{p}M
$$
...
1. $\langle L(\mathbf{X}),\mathbf{Y}\rangle = \langle\D_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle$
...
Since $\langle\mathbf{Y},\mathbf{N}\rangle = 0$, by the compatibility of $\D$ with the metric

$$
  0 = \mathbf{X}\langle\mathbf{Y},\mathbf{N}\rangle = \langle \D_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle + \langle\mathbf{Y}, \D_\mathbf{X} N\rangle
$$

Hence

$$
  \langle \D_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle = \langle\mathbf{Y}, -\D_\mathbf{X} N\rangle = \langle\mathbf{Y}, L(\mathbf{X})\rangle
$$

Reversing the roles of $\mathbf{X}$ and $\mathbf{Y}$ gives

$$
  \langle \D_\mathbf{Y} \mathbf{X},\mathbf{N}\rangle = \langle\mathbf{X}, L(\mathbf{Y})\rangle
$$
...
By torsion-freeness, subtracting $\langle \D_Y \mathbf{X},\mathbf{N}\rangle$ and $\langle [\mathbf{X},\mathbf{Y}],\mathbf{N}\rangle$ from $\langle \D_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle$ gives

$$
\begin{align*}
  0 =& \langle \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}],\mathbf{N}\rangle \\
...
<MathBox title='' boxType="theorem">
Let $M$ be a hypersurface in $\R^{n+1}$ and $\D$ the directional derivative on $\R^{n+1}$. For $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$ the tangential component of $\D_\mathbf{X} \mathbf{Y}$ defines the Levi-Civita connection $\nabla$ of $M$
...
**Torsion-freeness:** Let $T_\D$ and $T_\nabla$ be the torsions of $\D$ and $\nabla$, respectively. By definition, for $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$

$$
\begin{align*}
  \D_\mathbf{X} \mathbf{Y} =& \nabla_\mathbf{X}\mathbf{Y} + (\D_\mathbf{X} \mathbf{Y})_{\mathrm{nor}} \\
  \D_\mathbf{Y} \mathbf{X} =& \nabla_\mathbf{Y} \mathbf{X} + (\D_\mathbf{Y} \mathbf{X})_{\mathrm{nor}}
\end{align*}
$$

Since $\D$ is torsion-free, $\D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} = [\mathbf{X},\mathbf{Y}]$. Equating the normal components of both sides, we get $(\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} - (\D_\mathbf{Y} \mathbf{X})_\mathrm{nor} = 0$. Thus,

$$
\begin{align*}
  0 =& T_\D(\mathbf{X},\mathbf{Y}) \\
  =& \D_\mathbf{X} \mathbf{Y} - \D_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}] \\
  =& (\nabla_\mathbf{X}\mathbf{Y} - \nabla_\mathbf{Y} \mathbf{X} - [\mathbf{X},\mathbf{Y}]) + (\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} - (\D_\mathbf{Y} \mathbf{X})_\mathrm{nor} \\
...
\begin{align*}
  \mathbf{X}\langle \mathbf{Y},\mathbf{Z}\rangle =& \langle\D_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle + \langle\mathbf{Y}, \D_\mathbf{X} \mathbf{Z}\rangle \\
  =& \langle\nabla_\mathbf{X}\mathbf{Y} + (\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} \mathbf{Z}\rangle + \langle\mathbf{Y}, \nabla_\mathbf{X} \mathbf{Z} + (\nabla_\mathbf{X} \mathbf{Y})_\mathrm{nor}\rangle \\
  =& \langle \nabla_\mathbf{X} \mathbf{Y},\mathbf{Z} \rangle + \langle\mathbf{Y}, \nabla_\mathbf{X} \mathbf{Z} \rangle
\end{align*}
$$
</details>
...
For $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$, the directional derivative $\D_\mathbf{X} \mathbf{Y}$ decomposes into the sum of a tangential and normal component. The tangential component is the Levi-Civita connection on $M$; the normal component is essentially the second fundamental form.
...
$$
  (\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} = \mathrm{II}(\mathbf{X},\mathbf{Y})\mathbf{N}
$$

<details>
<summary>Proof</summary>

The normal component $(\D_\mathbf{X} \mathbf{Y})_\mathrm{nor}$ is a multiple of $\mathbf{N}$, so

$$
\begin{align*}
  \D_\mathbf{X} \mathbf{Y} =& (\D_\mathbf{X} \mathbf{Y})_\mathrm{tan} + (\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} \\
...
$$
  \langle \D_\mathbf{X},\mathbf{N}\rangle = \lambda\langle\mathbf{N},\mathbf{N}\rangle = \lambda
$$
...
$$
\begin{align*}
  (\D_\mathbf{X} \mathbf{Y})_\mathrm{nor} =& \langle\D_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle\mathbf{N} \\
...
$$
  \Omega_j^i = \d\omega_j^i + \sum_k \omega_k^i \wedge\omega_j^k
$$
...
\begin{align*}
  ...
  =& sum_i \d\omega_j^i (\mathbf{X},\mathbf{Y})\mathbf{e}_i + \sum_{i,k} \omega_k^i \wedge \omega_j^k (\mathbf{X},\mathbf{Y})\mathbf{e}_i \\
  =& \sum_{i,k} (\d\omega_j^i + \omega_k^i \wedge \omega_j^k)(\mathbf{X},\mathbf{Y})\mathbf{e}_i
\end{align*}
$$
...
\begin{align*}
  ...
  =& -\d\omega_j^i + \sum_{k=1}^n (-\omega_j^k)\wedge(-\omega_k^i) \\
  =& -\d\omega_j^i - \sum_{k=1}^n \omega_k^i \wedge \omega_j^k \\
  =& -\Omega_j^i
\end{align*}
$$
</details>
...
1. **First structural equation:** $\tau^i = \d\theta^i + \sum_{j=1}^n \omega_j^i \wedge\theta^j$
2. **Second structural equation:** $\Omega_j^i = \d\omega_j^i + \sum_{k=1}^n \omega_k^i \wedge \omega_j^k$
...
\begin{align*}
  ...
  =& (\d\theta^i + \omega_j^i \wedge \theta^j)(\mathbf{X},\mathbf{Y})\mathbf{e}_i
\end{align*}
$$
...
$$
  \tau^i = \d\theta^i + \sum_{j=1}^n \omega_j^i \wedge \theta^j
$$
</details>
</MathBox>
...
$$
  \d\theta^i + \sum_{j=1}^n \omega_j^i \wedge \theta^j = 0,\; \forall i=1,\dots,n
$$
...
$$
  \d\theta^i + \sum_{j=1}^n \omega_j^i \wedge\theta^j = 0
$$
...
and $\d\mathbf{A}$ is defined to be $[\d\alpha_j^i]$. In matrix notation we write
...
$$
  \tau = \d\theta + \omega + \omega\wedge\theta
$$
...
\begin{align*}
  ...
  =& (\nabla_\mathbf{X} \mathbf{s})f + \mathbf{s}\d f(\mathbf{X})
\end{align*}
$$
...
$$
  \nabla(\mathbf{s}f) = (\nabla\mathbf{s})f + s\d f,\; \mathbf{s}\in\Gamma(E), f\in C^\infty(M)
$$
...
1. $\bar{\omega} = \mathbf{A}^{-1} \omega\mathbf{A} + \mathbf{A}^{-1}\d\mathbf{A}$
2. $\bar{\Omega} = \mathbf{A}^{-1} \Omega\mathbf{A}$

<details>
<summary>Proof</summary>

**(1):** We use the recasted Leibniz rule to derive the transformation rule for the connection matrix under a change of frame. Since $\bar{\mathbf{e}} = \mathbf{eA}$, we have $\mathbf{e} = \bar{\mathbf{e}}\mathbf{A}^{-1}$. Recall that if $\mathbf{A} = [a_j^i]$ is a matrix of smooth functions, then $\d\mathbf{A} = [\d a_j^i]$ is a matrix of smooth $1$-forms.

$$
\begin{align*}
  \nabla\bar{\mathbf{e}} =& \nabla(\mathbf{eA}) & \\
  =& (\nabla\mathbf{e})\mathbf{A} + \mathbf{e}\d\mathbf{A} \quad& (\text{Leibniz rule}) \\
  =& (\mathbf{e}\omega)\mathbf{a} + \mathbf{e}\d\mathbf{A} \quad& (\nabla\mathbf{e}= \mathbf{e}\omega) \\
  =& \bar{\mathbf{e}}\mathbf{A}^{-1}\omega\mathbf{A}+ \bar{\mathbf{e}}\mathbf{A}^{-1}\d\mathbf{A} \quad& (\mathbf{e} = \bar{\mathbf{e}}\mathbf{A}^{-1}) \\
  =& \bar{\mathbf{e}}(\mathbf{A}^{-1} \omega\mathbf{A} + \mathbf{A}^{-1}\d\mathbf{A})
\end{align*}
$$
...
$$
  \bar{\omega} = \mathbf{A}^{-1}\omega\mathbf{A} + \mathbf{A}^{-1}\d\mathbf{A}
$$
...
$$
  \d\tau = \Omega\wedge\varepsilon - \omega\wedge\tau
$$
...
$$
\begin{align*}
  \d\tau =& \d(\d\varepsilon) + \d\omega\wedge\varepsilon - \omega\wedge\d\varepsilon \\
...
$$
  \d\left(\bigwedge_{i=1}^k \Omega \right) = \left(\bigwedge_{i=1}^k \Omega \right)\wedge\omega - \omega\wedge\left(\bigwedge_{i=1}^k \Omega \right)
$$
...
For the connection $\D$ on the bundle $T\R^3|_M$, let $[\omega_j^i]$ be the connection matrix of $1$-forms relative to the orthonormal frame $B$ over $U$. Since $\D$ is metric compatible and the frame $B$ is orthonormal, the matrix $[\omega_j^i]$ is skew-symmetric. Thus, for $\mathbf{X}\in\mathfrak{X}(M)$

$$
\begin{align*}
  \D_\mathbf{X} \mathbf{e}_1 =& -\omega_2^1 (\mathbf{X})\mathbf{e}_2 -\omega_3^1 (\mathbf{X})\mathbf{e}_3 \\
  \D_\mathbf{X} \mathbf{e}_2 =& -\omega_2^1 (\mathbf{X})\mathbf{e}_1 -\omega_3^2 (\mathbf{X})\mathbf{e}_3 \\
  \D_\mathbf{X} \mathbf{e}_3 =& -\omega_3^1 (\mathbf{X})\mathbf{e}_1 -\omega_3^2 (\mathbf{X})\mathbf{e}_2 
\end{align*}
$$

Let $\nabla$ be the Levi-Civita connection on the surface $M$. Recall that for $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$, the directional derivative $\D_\mathbf{X} \mathbf{Y}$ need not be tangent to the surface $M$, and $\nabla_\mathbf{X} \mathbf{Y}$ is simply the tangential component of $\D_\mathbf{X} \mathbf{Y}$
...
\begin{align*}
  ...
  =& \begin{bmatrix} 0 & \d\omega_2^1 \\ -\d\omega_2^1 & 0 \end{bmatrix} = \begin{bmatrix} 0 & 1 \\ -1 & 0 \end{bmatrix} \d\omega_2^1
\end{align*}
$$

So the curvature matrix of $\nabla$ is completely described by $\Omega_2^1 = \d\omega_2^1$. Set the unit normal vector field $\mathbf{N}$ on $U$ to be $\mathbf{N} = -\mathbf{e}_3$. The shape operator becomes

$$
\begin{align*}
  L(\mathbf{X}) =& -\D_\mathbf{X} \mathbf{N} = \D_\mathbf{X} \mathbf{e}_3,\; \mathbf{X}\in\mathfrak{X}(M) \\
...
Let $\tilde{\Omega_j^i}$ be the curvature forms of the connection $\D$ on $T\R^3|_M$. Because the curvature tensor of $\D$ is zero, the second equation for $\tilde{\Omega}$ gives

$$
  \tilde{\Omega}_j^i = \d\omega_j^i + \sum_{k=1}^3 \omega_k^i \wedge\omega_j^k = 0
$$

In particular

$$
  \d\omega_2^1 + \omega_1^1 \wedge\omega_2^1 + \omega_2^1 \wedge \omega_2^2 + \omega_3^1 \wedge \omega_2^3 = 0
$$
...
Since the matrix $[\omega_j^i]$ is skew-symmetric and $\Omega_2^1 = \d\omega_2^1$

$$
  \d\omega_2^1 = \omega_3^1 \wedge \omega_3^2 = \Omega_2^1
$$
</details>
</MathBox>
...
$$
  \d\omega_2^1 = K\varepsilon^1 \wedge\varepsilon^2
$$
...
and a $2$-form on $U$ is completely determined by its value on $\mathbf{e}_1,\mathbf{e_2}$ we have $\Omega_2^1 = K\varepsilon^1 \wedge\varepsilon^2$ and since $\Omega_2^1 = \d\omega_2^1$ we get
...
<MathBox title='Gaussian curvature of the Poincaré half-plane' boxType="example" tag="6">
...
$$
  g_{xy} = \frac{\d x \otimes \d x + \d y \otimes \d y}{y^2}
$$
...
$$
  \varepsilon_1 = \frac{1}{y}\d x \quad \varepsilon_2 = \frac{1}{y}\d y
$$

Hence

$$
  \d\varepsilon^1 = \frac{1}{y^2}\d x \wedge \d y \quad \d\varepsilon^2 = 0
$$

On the Poincaré half-plane the connection form $\omega_2^1$ is a linear combination of $\d x$ and $\d y$, taking the form

$$
  \omega_2^1 = a\;\d x + b\;\d y
$$
...
$$
\begin{align*}
  \d\varepsilon^1 =& -\omega_2^1 \wedge\varepsilon^2 \\
  \d\varepsilon^2 =& -\omega_1^2 \wedge\varepsilon^1 = \omega_2^1 \wedge\varepsilon^1
\end{align*}
$$
...
$$
\begin{align*}
  \frac{1}{y^2}\d x \wedge\d y =& \d\varepsilon^1 \\
  =& -(a\d x + b\d y)\wedge \frac{1}{y}\d y \\
  =& -\frac{a}{y}\d x\wedge\d y
\end{align*}
$$
...
$$
\begin{align*}
  0 =& \d\varepsilon^2 = \left(-\frac{1}{y}\d x + b\d y \right)\wedge\frac{1}{y}\d x \\
  =& -\frac{b}{y}\d x\wedge\d y
\end{align*}
$$
...
$$
\begin{align*}
  \omega_2^1 =& -\frac{1}{y}\d x \\
  \d\omega_2^1 =& \frac{1}{y^2}\d y\wedge\d x = -\frac{1}{y^2}\d x\wedge\d y 
\end{align*}
$$
...
$$
\begin{align*}
  K =& \Omega_2^1 (\mathbf{e}_1,\mathbf{e}_2) = -\frac{1}{y^2} (\d x\wedge\d y)\left(y\frac{\partial}{\partial x}, y\frac{\partial}{\partial y} \right) \\
  =& -(\d x\wedge\d y)\left(\frac{\partial}{\partial x},\frac{\partial}{\partial y}\right) = -1
\end{align*}
$$
...
<MathBox title='Christoffel symbols for the Poincaré half-plane' boxType="example" tag="8">
...
\begin{align*}
  ...
  =& -\frac{1}{x^2} (\mathbf{X}x^2)\frac{\partial}{\partial x^1} + \frac{1}{(x^2)^2} \d x (\mathbf{X})x^2 \frac{\partial}{\partial x^2} \\
...
<MathBox title="Geodesic" boxType="definition">
Let $M$ be a smooth manifold with an affine connection $\nabla$ and $I\subseteq\R$ a real open, closed or half-closed interval. A parametrized curve $c:I\to M$ is a *geodesic* if the covariant derivative $\frac{\D\mathbf{\D}}{\d t}$ of its velocity vector field $\mathbf{T}(t) = c'(t)$ is zero. The geodesic is *maximal* if its domain $I$ cannot be extended to larger interval.
</MathBox>
...
$$
\begin{align*}
  f'(t) =& \frac{\d}{\d t}\langle\mathbf{T},\mathbf{T}\rangle \\
  =& 2\left\langle\frac{\D\mathbf{T}}{\d t},\mathbf{T} \right\rangle = 0
\end{align*}
$$
...
$$
  \nabla_\mathbf{X} \mathbf{Y} = (\D_\mathbf{X}\mathbf{Y})_\text{tan},\; \mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)
$$

the tangential component of the directional derivative $\D_\mathbf{X}\mathbf{Y}$. If $\mathbf{N}$ is a unit normal vector field on an open subset $U$ of $M$, then the tangential component of a vector field $Z$ along $M$ is
...
$$
  \frac{\D\mathbf{V}}{\d t} = \left(\frac{\d\mathbf{V}}{\d t} \right)_\text{tan}
$$
...
$$
  \frac{\D\mathbf{T}}{\d t} = \left(\frac{\d\mathbf{T}}{\d t}\right)_\text{tan} = \gamma''(t)_\text{tan} = 0
$$
...
$$
\begin{align*}
  \bar{\mathbf{T}}(t) =& \frac{\d}{\d t} \gamma(u(t)) = \gamma'(u(t))u'(t) \\
  =& u'(t)\mathbf{T}(u(t))
\end{align*}
$$
...
$$
\begin{align*}
  \frac{\D\bar{\mathbf{T}}}{\d t} =& u''(t)\mathbf{T}(u(t)) + u'(t)\frac{\D\mathbf{T}(u(t))}{\d t} \\
  =& u''(t)\mathbf{T}(u(t)) + u'(t)\frac{\D\mathbf{T}}{\d\mathbf{u}}\frac{\d u}{\d t} \\
  =& u''(t)\mathbf{T}(u(t))
\end{align*}
$$

where $\frac{\D\mathbf{T}}{\d u} = 0$ because $\gamma(u)$ is a geodesic. Since $\mathbf{T}(u)$ has constant length, it is never zero. Thus,

$$
  \frac{\D\mathbf{T}}{\d t} = 0 \iff u'' = 0 \iff u = \alpha t + \beta,\; \alpha,\beta\in\R
$$
</details>
</MathBox>
...
$$
  \frac{\D\bar{\mathbf{T}}}{\d t}(t) = k^2 \frac{\D\mathbf{T}}{\d t}(kt)
$$
...
$$
  \frac{\d x}{\d y} = \pm\frac{ky^2}{y\sqrt{1 - k^2 y^2}} = \pm\frac{ky}{\sqrt{1 - k^2 y^2}}
$$
...
$$
\begin{align*}
  (\exp_\mathbf{p})_{*,\mathbf{0}}(\mathbf{v}) =& \left.\frac{\d}{\d t}\right|_{t=0} \exp_\mathbf{p}(c(t)) \\
  =& \left.\frac{\d}{\d t}\right|_{t=0} \exp_\mathbf{p}(t\mathbf{v}) \\
  =& \left.\frac{\d}{\d t}\right|_{t=0} \gamma_\mathbf{v}(t) \\
...
$$
\begin{align*}
  \frac{\d}{\d t} (\mathbf{g}\varphi_t (\mathbf{p})) = (\ell_\mathbf{g})_* \frac{\d}{\d t}\varphi_t (\mathbf{p}) \\
...
$$
\begin{align*}
  \frac{\d}{\d t}c_\mathbf{X}(st) =& sc'_\mathbf{X}(st) \quad \text{chain rule} \\
...
$$
  \frac{\d}{\d t} c_\mathbf{X}(s + t) = c'_\mathbf{X}(s + t) = \mathbf{X}_{c_\mathbf{X}(s+t)}
$$
...
$$
\begin{align*}
  \exp_{*,\mathbf{0}}(\mathbf{X_e}) =& \left.\frac{\d}{\d t}\right|_{t=0} \\
  =& \left.\frac{\d}{\d t}\right|_{t=0} \exp(t\mathbf{X_e}) = \mathbf{X_e}
\end{align*}
$$
...
\begin{align*}
  ...
  =& \left.\frac{\d}{\d t}\left|_{t=0} \operatorname{Ad}(\exp(t\mathbf{A}))(\mathbf{B}) \\
  =& \left.\frac{\d}{\d t}\left|_{t=0} (c_{\exp(t\mathbf{A})})_* (\mathbf{B}) \\
  =& \left.\frac{\d}{\d t}\left|_{t=0} (r_{\exp(-t\mathbf{A})})_* (\ell_{\exp(t\mathbf{A})})_* \mathbf{B} \\
  =& \left.\frac{\d}{\d t}\left|_{t=0} (r_{\exp(-t\mathbf{A})})_* \tilde{\mathbf{B}}_{\exp(t\mathbf{A})} \\
  =& \left.\frac{\d}{\d t}\left|_{t=0} (\varphi_{-t})_* (\tilde{B}_{\exp(t\mathbf{A})}) \\
...
A *right-invariant metric* is defined similarly with $r_\mathbf{g}$ in place of $\ell_\mathbf{g}$. A metric that is both left- and right-invariant is *bi-invariant*.
...
$$
\begin{align*}
  0 =& \left.\frac{\d}{\d}\right|_{t=0} \langle\operatorname{Ad}(c(t))\mathbf{X_e}, \operatorname{Ad}(c(t))\mathbf{Z_e} \rangle \\
...
Suppose $c(t(u))$ with $a_1 \leq u \leq b_1$ is an orientation-preserving reparametrization. Then $\frac{\d}{\d t} > 0$. By the chain rule,

$$
  \frac{\d}{\d u} c(t(u)) = c'(t(u))\frac{\d t}{\d u}
$$
...
$$
\begin{align*}
  \int_{a_1}^{b_1} \norm{ \frac{\d}{\d u} c(t(u))}\;\d u =& \int_{a_1}^{b_1} \norm{ c'(t(u)) } \left|\frac{\d t}{\d u}\right| \d u \\
  =& -\int_{a_1}^{b_1} \norm{ c'(t(u))} \frac{\d t}{\d u}\d u \\
  =& \int_{b_1}^{a_1} \norm{ c'(t(u)) } \frac{\d t}{\d u}\d u \\
  =& \int_a^b \norm{ c'(t) } \d t
\end{align*}
$$
...
is $\d x\wedge\d y$. The unit outward normal $\mathbf{X}$ along its boundary $\partial D = \mathbb{S}^1$ is
...
$$
\begin{align*}
  \omega_{\mathbb{S}^1} =& \iota_\mathbf{X}(\d x\wedge\d y) \\
  =& (\iota \d x) \d y - \d x (\iota_\mathbf{X} \d y) \\
  =& (\mathbf{X}x) \d y - \d x (\mathbf{X}y) = x\d y - y\d x
\end{align*}
$$
</MathBox>
...
$$
  \mathrm{vol}_M = \sqrt{\det(g)} \bigwedge_{i=1}^n \d x^i
$$
...
$$
\begin{align*}
  \mathrm{vol}_M =& \bigwedge_{i=1}^n = \det(\mathbf{B})^{-1} \bigwedge_{i=1}^n \d x^i \\
  =& \det(\mathbf{A}) \bigwedge_{i=1}^n \d x^i
\end{align*}
$$
...
$$
  \mathrm{vol}_M = \det(A) \bigwedge_{i=1}^n \d x^i = \sqrt{\det(g)}\bigwedge_{i=1}^n \d x^i
$$
</details>
</MathBox>
...
$$
  \tilde{\kappa}_g = \Norm{\frac{\D T}{\d s}}
$$

It follows directly from the definition that a unit-speed curve is a geodesic if and only if its geodesic curvature is zero. The integral $\int_a^b \kappa_g \;\d s$ is called the *total geodesic curvature* of $\gamma$.
</MathBox>
...
$$
  \cos^{-1}\left(\frac{\langle c'(t),\mathbf{e}_{1,c(t)} \rangle}{\norm{ c'(t)}}\right)
$$

Let $\theta_0$ be a real number such that

$$
  \cos\theta_0 = \frac{\langle c'(a),\mathbf{e}_{1,c(a)} \rangle}{\norm{ c'(a)}}
$$
...
$$
  \frac{\D T}{\d s} = \kappa_g \mathbf{n}
$$

The signed geodesic curvature $\kappa_g$ can also be computed as

$$
  \kappa_g = \left\langle\frac{\D T}{\d s},\mathbf{n}\right\rangle
$$
...
$$
  \frac{\d}{s}\langle T,T\rangle = 0
$$

and

$$
  2\left\langle\frac{\D T}{\d s},T\right\rangle = 0
$$

Since the tangent space of $M$ at $\gamma$ is $2$-dimensional and $\frac{\D T}{\d s}$ is perpendicular ot $T$, it follows that $\frac{\D T}{\d s}$ must be a scalar multiple of $\mathbf{n}$.
</details>
</MathBox>
...
$$
  \kappa_g = \frac{\d\theta}{\d s} - \omega_2^1 (T)
$$
...
$$
\begin{align*}
  \frac{\D T}{\d s} =& \left(\frac{\d}{\d s}\cos\theta\right)\mathbf{e}_{1,c(t)} + \cos(\theta)\frac{\D\mathbf{e}_{1,c(t)}}{\d s} \\
  &+ \left(\frac{\d}{\d s} \sin\theta \right)\mathbf{e}_{2,c(t)} + \sin(\theta)\frac{\D\mathbf{e}_{2,c(t)}}{\d s}
\end{align*}
$$
...
$$
\begin{align*}
  \frac{\D T}{\d s} =& -\sin(\theta)\frac{\d\theta}{\d s}\mathbf{e}_{1,c(t)} - \cos(\theta)\omega_2^1 (T)\mathbf{e}_{2,c(t)} \\
  &+ \cos(\theta)\frac{\d\theta}{\d s}\mathbf{e}_{2,c(t)} + \sin(\theta)\omega_2^1 (T)\mathbf{e}_{1,c(t)} \\
  =& \left(\frac{\d\theta}{\d s} - \omega_2^1 (T) \right)\mathbf{n}
\end{align*}
$$
...
$$
  \int_a^b \kappa_g \;\d s = \theta(a) - \theta(b) - \int_C \omega_2^1
$$
...
$$
  \gamma^* \omega_2^1 = f(s)\;\d s
$$

for some smooth function $f(s)$. Applying $\frac{\d}{\d s}$ to the above gives

$$
\begin{align*}
  f(s) =& (\gamma* \omega_2^1)\left(\frac{\d}{\d s}\right) = \omega_1^2 (\gamma*\frac{\d}{\d}s) \\
  =& \omega_2^1 (\gamma'(s)) = \omega_2^1 (T)
\end{align*}
$$

Hence $\gamma* \omega_2^1 = \omega_2^1 (T)\;\d s$. By previous proposition

$$
\begin{align*}
  \int_a^b \kappa_g \;\d s = \int_a^b \frac{\d\theta}{\d s}\d s - \int_a^b \omega_2^1 (T)\;\d s \\
  =& \theta(b) - \theta(a) - \int_a^b \omega_2^1 (T)\;\d s \\
...
$$
  \int_a^b \kappa_g \;\d s = 2\pi - \sum_{i=1}^m \phi_i - \int_R K \;\mathrm{vol}
$$

<details>
<summary>Proof</summary>

The integral $\int_a^b \kappa_g \;\d s$ is the sum of the total geodesic curvature on each edge of a simple closed curve. Denote the change in the angle $\theta$ along the $i$th edge of $\gamma$ by $\Delta\theta_i$. By previous corollary

$$
  \int_a^b \kappa_g \;\d s =& \sum_{i=1}^m \int_{s_i - 1}^{s_i} \kappa_g \;\d s \\
...
$$
  \Omega_2^1 = \d\omega_2^1 = K \varepsilon^1 \wedge\varepsilon^2
$$
...
Denote the polygonial regions by $R_j$. If we sum up $\int \kappa_g \;\d s$ over the boundaries of all the regions $R_j$, the result is zero because each edge occurs twice, with opposite orientations. Let $\beta_i^(j)$ for $1\leq i \leq n_j$ be the interior angles of the $j$th polygon. Then by the Gauss-Bonnet formula for a polygon

$$
\begin{align*}
  0 =& \sum_j \int_{\partial R_j} \kappa_g \;\d s \\
...
$$
  \frac{\d}{\d \lambda} = \frac{\d x^i}{\d \lambda} \frac{\partial }{\partial \mathbf{x}^i} 
$$
...
$$
\begin{align*}
  \frac{\d}{\d \lambda} = \frac{\d x^i}{\d \lambda} \frac{\partial }{\partial \mathbf{x}^i} &= \frac{\d x^i}{\d \lambda} \left( \frac{\partial \tilde{x}^j}{\partial x^i} \frac{\partial}{\partial \tilde{\mathbf{x}}^j} \right) \\
  &= \left( \frac{\d x^i}{\d \lambda} \frac{\partial \tilde{x}^j}{\partial x^i} \right) \frac{\partial}{\partial \tilde{\mathbf{x}}^j} \\
  &= \frac{\d \tilde{x}^i}{\d \lambda} \frac{\partial}{\partial \tilde{\mathbf{x}}^i}
\end{align*}
$$
...
$$
\begin{align*}
  \d f = \operatorname{grad}(f) \cdot \_ &= g\left( \operatorname{grad}(f), \_ \right) \\ 
  &= \left[ g_{ij} \left( \d x^i \otimes \d x^j \right) \right] \left[ \left( \operatorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} \right] \\
  &= g_{ij} \left( \operatorname{grad}(f) \right)^k \d x^i \left( \frac{\partial}{\partial x^k} \right) \d x^j \\
  &= g_{ij} \left( \operatorname{grad}(f) \right)^k \delta_k^i \d x^j \\
  &= g_{ij} \left( \operatorname{grad}(f) \right)^i  \d x^j
\end{align*}
$$
...
$$
  \d f (\mathbf{v}) \equiv \operatorname{grad}(\)mathbf{v} f = \lim_{h \to 0} \frac{f(\mathbf{x} + \mathbf{h} \cdot \mathbf{v}) - f(\mathbf{x})}{\mathbf{h}} = \mathbf{v} \cdot \operatorname{grad}(f)
$$

This implies that the gradient (vector) $\operatorname{grad}(f)$ and the differential form (covector) $\d f$ are dual pairs

$$
\begin{align*}
  \d f(\mathbf{v}) = \operatorname{grad}(f) \cdot \mathbf{v} &= \left[ \left( \operatorname{grad}(f)\right)^i \frac{\partial}{\partial x^i} \right] \cdot \left(v^j \frac{\partial}{\partial x^j} \right) \\
...
\begin{align*}
  ...
  &= \left[ g_{ij} \left( \d x^i \otimes \d x^j \right) \right] \left[ \left( \operatorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} , v^l \frac{\partial}{\partial x^l} \right] \\
  &= g_{ij} \d x^i \left[ \left( \operatorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} \right] \d x^j \left( v^l \frac{\partial}{\partial x^l} \right) \\
  &= g_{ij} \left( \operatorname{grad}(f) \right)^k v^l \d x^i \left( \frac{\partial}{\partial x^k} \right) \d x^j \left( \frac{\partial}{\partial x^l} \right) \\
