...
Suppose $\mathbf{X} = \set{X_t: \Omega\to S}_{t\in T}$ is a time-homogeneous Markov process on a measurable space $(\mathsf{S}, \mathcal{S})$. For each $t\in T$ define
...
Suppose $\mathbf{X}=\Set{X_t: \Omega\to S}_{t\in T}$ is a time-homogeneous Markov process with transition kernels $\mathbf{P} = \Set{ P_t }_{t\in T}$. For $s,t\in T$ then $P_s P_t = P_{s+t}$ given by
