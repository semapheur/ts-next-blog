...
$$
\begin{gather*}
  \lVert Tx \rVert_{\ell^p (\N) \to \mathbb{F}} = \sup\Set{ \left| (Tx)(y) \right| | \lVert y \rVert_p = 1 } \leq \sup\left\{ \lVert y \rVert_p \cdot \lVert x \rVert_q \mid \lVert y \rVert_p = 1 } \leq \lVert x \rVert_q \\
...
$$
  \sigma (T) = \underbrace{\Set{ \mu \in \mathbb{C} | \mu \notin \left\{ \lambda_j \right}_{j\in\N} \wedge \mu \in \overline{\Set{\lambda_j \right}_{j\in\N}}} = \sigma_p(T) \cup \sigma_c(T)
$$