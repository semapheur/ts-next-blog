...
which shows that $T$ is continuous. Conversly, suppose $T$ is continuous. Then there exists $\delta > 0$ such that $\| Tx \|_Y < 1$ for all $x \in X$ with $\| x \|_X < \delta$, thus
...
1. The predicate $\| f\|_p \geq 0$ follows from the definitions. For the second part, we first consider the case $p\in(0,\infty)$. Trivially, $\| 0\|_p = 0$. Conversely, if $\| f\|_p = 0$ then $\int_X \| f\|^p\;\mathrm{d}\mu = 0$ and hence $|f|^p \overset{\textrm{a.e.}}{=} 0$ and $f \overset{\textrm{a.e.}}{=} 0$. For the case $p = \infty$, it follows trivially that $\| 0\|_\infty = 0$. Conversely, suppose $\| f\|_\infty = 0$. Then for each $n\in\N$ there exists $b_n\in[0,\infty)$ with $b_n \xrightarrow{n\to\infty}0$ and $|f\overset{\textrm{a.e.}\leq b_n$, resulting in $f\overset{\textrm{a.e.}}{=}0$.
