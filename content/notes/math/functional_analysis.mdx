...
The element $x^*$ is called the *adjoint* of the element $x$, and the mapping $x\mapsto x^*$ is referred to as the adjoint map or as adjunction. An element $x$ is *self-adjoint* if $x = x^*$.
</MathBox>
...
<MathBox title='Gelfand-Naimark theorem for commutative C*-algebras' boxType='proposition'>
