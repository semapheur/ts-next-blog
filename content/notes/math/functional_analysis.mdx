...
2. For each $x\in H$ and $y\in K$, consider the seminorm $p_{x,y}$ defined on $\mathcal{L}(H,K)$ by $p_{x,y} (T) = |\langle Tx, y\rangle|$. The topology on $\mathcal{L}(H,K)$ defined by the collection $\Set{p_{x,y} | x\in H,\; y\in K}$ of seminorms is called the *weak operator topology*.
</MathBox>
...
Likewise, the net $\Set{T_i}_{i\in I}$ converges to $T$ with respect to the weak operator topology if and only if $\langle(T_i - T)x, y\rangle \xrightarrow{i\to\infty} 0$ for all $x\in H$ and $y\in K$, i.e. if and only if the net $\Set{T_i x}$ converges to $Tx$ with respect to the weak norm topology on $K$, for every $x\in H$.
...
2. $\Set{T_i}_{i\in I}$ converges weakly to $0$ if and only if $\lim_{i\to\infty} \langle T_i, y\rangle = 0$ for all $x\in S_1$ and $y\in S_2$.
...
Since $I$ is a directed set, the assumption that $\lim_{i\to\infty} \langle T_i x_k, y_j \rangle = 0$ for all $j,k$ implies that we can find an index $i_0 \in I$ with the property that $|\langle T_i x_k, y_j| < \epsilon/(3N^2 mn)$ whenever $i \geq i_0$, $1 \leq k\leq m$ and $1\leq j \leq n$. It then follows that for all $i \geq i_0$, we have

$$
\begin{align*}
  |\langle T_i x, y\rangle| \leq& |\langle T_i x, y - y'\rangle| + |\langle T_i (x - x'), y'\rangle| + |\langle T_i x', y'\rangle| \\
  \leq& 2KM \frac{\epsilon}{3KM} + \sum_{k=1}^m \sum_{j=1}^n |\alpha_k \beta_j|\cdot|\langle T_i x_k, y_j\rangle| \\
...
$$
  \left\langle x_i^{(1)}, x_j^{(1)}\right\rangle_{H_1} = \left\langle x_i^{(2)}, x_j^{(2)}\right\rangle_{H_2},\;\forall i,j\in I
$$ 
...
$$
  \phi(x) = \langle\pi_\phi (x)\mathbf{x}_\phi, \mathbf{x}_\phi\rangle,\;\forall x\in A
$$

The triple $(H_\phi, \pi_\phi, \mathbf{x}_\phi)$ is unique in the sense that if $\pi:A\to H$ is a cyclic representation with cyclic vector $\mathbf{x}$ such that $\phi(x) = \langle\pi(x)\mathbf{x}, \mathbf{x}\rangle$ for all $x\in A$, then there exists a unique unitary operator $U:H\to H_\phi$ such that $U\mathbf{x} = \mathbf{x}_\phi$ and $U\pi(x)U^* = \pi_\phi$ for all $x\in A$.
...
$$
  \langle x + N_\phi, y + N_\phi\rangle = \phi(y^* x)
$$
...
$$
\begin{align*}
  \langle\pi_\phi (x)\eta(y),\eta(z)\rangle =& \phi(z^* (xy)) \\
  =& \phi((x^* z)^* y) \\
  =& \langle\eta(y),\pi_\phi (x^*)\eta(z)\rangle
\end{align*}
$$
...
$$
  \langle\pi(x)\mathbf{x},\pi(y)\mathbf{x}\rangle_H = \phi(y^* x) = \langle\pi_\phi (x)\mathbf{x}_\phi, \pi_\phi(y)\mathbf{x}_\phi\rangle_{H_\phi}
$$
...
$$
  \lVert x\rVert = |\phi_0 (x)| = |\phi(x)| = |\langle\pi_\phi (x)\mathbf{x}_\phi, \mathbf{x}_\phi\rangle|\leq\lVert\pi_\phi (x)\rVert
$$
</details>
</MathBox>
...
$$
  p_i = \sum_{\begin{subarray}{c} n\leq m\leq\aleph_0 \\ \mu(A\cap E_m) > 0 \end{subarray}} q_i^{(m)}
$$
...
$$
  \mu_{\mathbf{x},\mathbf{y}} (E) = \langle P(E)\mathbf{x}, \mathbf{y}\rangle
$$
...
$$
  \langle\pi(f)\mathbf{x}, \mathbf{y}\rangle = \int_X f \;\d\mu_{\mathbf{x},\mathbf{y}}
$$
...
$$
  \mu(E) = \sum_{n\in I} \langle P(E),\mathbf{x}_n, \mathbf{x}_n \rangle = \sum_{n\in I} \lVert P(E) \mathbf{x}_n \rVert^2
$$
...
$$
  \langle P_\mu (E)f, g\rangle = \int_E f(x)\overline{g(x)}\;\d\mu(x)
$$
...
$$
  \langle Tx, y\rangle = \int_{\Sigma} \lambda\;\d\mu_{x,y} (\lambda)
$$
where $\mu_{x,y} (E) = \langle P(E)x, y\rangle$.
...
$$
\begin{equation}
  \langle f(T)x, y\rangle = \int_{\Sigma_0} f\;\d\mu_{x,y},\; \forall f,x,y tag{\label{equation-4}}
\end{equation}
$$
...
$$
  \lVert Tx \rVert^2 = \langle T^* Tx, x\rangle = \langle (T^* T)^{1/2} x, (T^* T)^{1/2} x = \lVert (T^* T)^{1/2} x \rVert^2
$$
...
$$
\begin{align*}
  x \in\operatorname{ran}(T^*)^\perp \iff& \langle x, T^* x\rangle = 0\; \forall y\in K \\
  \iff& \langle Tx, y\rangle = 0\; \forall y\in K \\
  \iff& Tx = 0
\end{align*}
$$
...
$$
  \lVert Px \rVert^2 = \langle Px, x\rangle = \langle U^* Ux, x\rangle = \lVert U x\rVert^2
$$
...
$$
\begin{align*}
  \langle U^* Uz_1, z_2 \rangle =& \langle Uz_1, Uz_2 \rangle \\
  =& \langle Ux_1, Ux_2 \rangle \\
  =& \langle x_1, x_2 \rangle \\
  =& \langle x_1, z_2 \rangle
\end{align*}
$$
...
$$
  \langle Tx, Ty\rangle = \langle T^* Tx, y\rangle = \langle|T|^2 x, y\rangle = \langle|T|x, |T|y\rangle
$$
...
$$
  Tx = \sum_{n\in\N} \lambda_n \langle x,x_n \rangle x_n,\; \forall x\in H
$$
...
$$
  Tx = \sum_{n\in N} \lambda_n \left(\sum_{k\in I_n} \langle x, x_k^{(n)}\rangle y_k^{(n)} \right),\; x\in H_1
$$
...
$$
\begin{align*}
  \sum_{n\in N} \lVert Te_n \rVert^2 =& \sum_{n\in N} \sum_{m\in M} |\langle Te_n, f_m\rangle|^2 \\
  =& \sum_{m\in M} \sum_{n\in N} |\langle T^* f_m, e_n\rangle|^2 \\
