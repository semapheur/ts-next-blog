...
<MathBox title="von Neumann's density theorem" boxType='theorem'>
...
<MathBox title='Hahn-Hellinger theorem' boxType='theorem'>
...
**(2)$\implies$(3):** Since $t\geq \epsilon\operatorname{id}_\epsilon (t)$ for all $t\geq 0$, we find easily that $|T|$ is bounded below by $\epsilon$ on $\operatorname{ran}(P_\epsilon)$ and **(3)** follows from Lemma $\ref{lemma-9}$.
...
**(1)$\implies$(5):** This follows from Lemma $\ref{lemma-9}$.
