...
Fix $\tilde{M} > M\in\R$ and consider $d(\tau) := \norm{ f(\tau) - f(s)}_X - \tilde{M}(\tau - s)$ for $\tau\in[s,t]$. Suppose $\tau_0$ is the largest $\tau$ for which $d(\tau) \leq 0$ holds. Then tehre must be a sequence $\epsilon_n \downarrow 0$ such that
