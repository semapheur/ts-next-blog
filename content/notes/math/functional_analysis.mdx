...
Let $(X, d)$ be a metric space. A subset $A \subseteq X$ is open if for each $x \in A$ there exists a radius $\epsilon > 0$ such that the open ball $B_\epsilon (x)$ is contained in $A$, i.e. $B_\epsilon (x) \subseteq A$. Conversely, a subset $B\subseteq X$ is closed if its complement $B^c := X \setminus A$ is open.

<details>
<summary>Proof</summary>

To show that open balls in fact are open, consider $x\in B_r (x)\subseteq X$. Let $y\in B_\epsilon (x)$, and let $a = d(x, y) < r$. We need to find $s > 0$ such that $B_s (y)\subseteq B_r (x)$. If $z\in B_{r-a}(y)$, then $d(y, z) < r - a$, so by the triangle inequality
...
<MathBox title='Properties of open sets' boxType="proposition">
Let $(X, d)$ be a metric space, and let $I \subseteq\N_0$ be an index set. The collection of open susbsets of $X$ satisfies:
1. $\emptyset$ and $X$ are open
2. if $A, B \subseteq X$ are open, then their intersection $A \cap B$ is open
3. if $(A_i)_{i\in I}$ is a collection of open sets in $X$, then the union $\bigcup_{i\in I} A_i$ is open
</MathBox>
...
<MathBox title='Cauchy sequences are bounded' boxType="proposition" tag="6">
...
<MathBox title='Cauchy sequences with a convergent subsequence are convergent' boxType="proposition" tag="8">
...
Assume the subsequence $\left( x_{n_k} \in A \right)_{k\in\N}$ converges to $x \in X$. Let $\epsilon > 0$, then there exists $j\in\N$ such that $d\left(x_{n_k}, x\right) < \frac{\epsilon}{2}$. Since $\left( x_n \right)_{n\in\N}$ is Cauchy, there exists $N\in\N$ such that for $d(x_m, x_n) < \frac{\epsilon}{2}$ for $m, n \geq N$. Choose $k\in\N$ such that $k \geq j$ and $n_k \geq N$. By the triangle inequality
...
Given two metric spaces $(X, d_X)$ and $(Y, d_Y)$, a function $f: X\to Y$ is a contraction if there is a constant $C\in(0, 1)$ such that, for all $x, \tilde{x}\in X$

$$
  d_Y \left(f(x), f\left(\tilde{x}\right)\right) \leq C d_X \left(x, \tilde{x} \right)
$$
...
Let $(X, d)$ be a complete metric space. Then any contraction $f:X\to X$ has a unique fixed point $x^* \in X$ such that $f(x*) = x*$. Alternatively, for any initial point $x_0 \in X$, define the sequence $(x_n)_{n\in\N_+}$ recursively by $x_n = f(x_{n-1})$ for $n\in\N$. Then, the sequence $(x_n)$ converges to the fixed point $x^*$, i.e. $x_n \xrightarrow{n\to\infty} x*$.
</MathBox>
...
<MathBox title='Characteristics of compactness' boxType="proposition" tag="10">
...
**(2)$\implies$(3):** Let $(x_k)_{k\in\N_+}$ be a sequence in $X$. For each $n\in\N$, let $B_n = \Set{x_n, x_{n+1},\dots}$. Then $(\overline{B}_n)_{n\in\N_+}$ is a collection of closed sets with the finite intersection property. By hypothesis, the intersection $\bigcap_{n\in\N_+} \overline{B}_n$ is nonempty.
...
<MathBox title='' boxType="proposition" tag="11">
...
<MathBox title='' boxType="proposition" tag="12">
...
We subdivide $I_2$ and repeat the argument to obtain a sequence of nested $n$-cells $(I_k)_{k\in\N_+}$, i.e. $I_{k+1}\subset I_k$ for all $k$, where each $I_k$ is not covered by a finite subcollection of $\mathcal{G}$. Furthermore, for $x,y\in I_k$ we have $\norm{x - y }_2 \leq 2^{-n} \norm{b - a}_2$.

We form a sequence $(x_k)_{k\in\N_+}$ by chooseing $x_k \in I_k$ for all $k$. This sequence is Cauchy because for $\epsilon > 0$ the choice of $N\in\N$ satisfying $2^{-N} \norm{b - a } < \epsilon$ gives $\norm{x_n - x_m }_2 \leq 2^{-N} \norm{b - a } < \epsilon$ whenever $n, m \geq N$.Since $\R^n$ is complete, the Cauchy sequence $(x_k)_{k\in\N_+}$ converges to $x\in\R^n$.

Since $\mathcal{G}$ is an open cover of $[a,b]$, there is some $\alpha\in J$ such that $x\in G_\alpha$. Since $G_\alpha$ is open and $x\in G_\alpha$, there is $r > 0$ such that $B(x,r)\subset G_\alpha$. For this $r > 0$, there is $N\in\N$ such that $2^{-N} \norm{b - a}_2 < r$. Since $\norm{x - y }_2 \leq 2^{-N} \norm{b - a }_2 < r$ for all $x, y \in I_N$ we obtain $I_N \subset B(x,r) \subset G_\alpha$.
...
Let $\mathbb{F} \in \Set{\R, \mathbb{C}}$ be a field of numbers and let $X$ be a $\mathbb{F}$-vector space. A map $\norm{\cdot} : X \mapsto [0, \infty)$ is called a norm if it satisfies

1. $\norm{x} = 0 \iff x = 0$ (positive definite)
2. $\norm{\lambda x} = |\lambda| \norm{x}, \quad \lambda \in \mathbb{F}$ (absolutely homogenous)
3. $\norm{x + y} \leq \norm{x} + \norm{y}$ (triangle inequality)

The pair $(X, \norm{\cdot})$ is called a normed space. The norm $\norm{\cdot}$ induces a metric by

$$
  d_{\norm{\cdot}}(x, y) := \norm{x - y}
$$

which makes the normed space $(X, \norm{\cdot})$ into a metric space $(X, d_{\norm{\cdot}})$.
</MathBox>

The norm $\norm{\cdot}$ is a continuous map. Considering the sequence $\left( x_n \right)_{n\in\N} \subseteq X$ with limit $\tilde{x} \in X$, then by the triangle inequality

$$
\begin{gather*}
  \norm{x_n} = \norm{\left(x_n - \tilde{x}\right) + \tilde{x}} \leq \norm{x_n - \tilde{x}} + \norm{\tilde{x}} = d \left(x_n, \tilde{x} \right) + \norm{\tilde{x}} \\
...
$$
\begin{gather*}
  \norm{\tilde{x}} = \norm{\left(\tilde{x} - x_n \right) + x_n} \leq \norm{\tilde{x} - x_n} + \norm{x_n} = d(\tilde{x}, x_n) +  \norm{x_n} \\
  \implies \norm{\tilde{x}} \leq \lim_{n\to\infty}  \norm{x_n}
\end{gather*}
$$

<MathBox title='Banach space' boxType="definition">
A normed space $(X,\norm{\cdot})$ is called a Banach space if $\left(X, d_{\norm{\cdot}} \right)$ is a complete metric space. A Banach space is a complete normed space.
</MathBox>
...
For $x, y\in\overline{M}$ there exists sequences $\Set{x_n}_{n\in\N}$ and $\Set{y_n}_{n\in\N}$ in $M$ such that $x_n \xrightarrow{n\to\infty} x$ and $y_n \xrightarrow{n\to\infty} y$. Then
...
Let $X, Y$ be normed spaces, and let $f:X\to Y$ and $\alpha: X\to (0,\infty)$ be functions. 

**Big O notation**

For a limit point $a\in X$, we write $f(x) = O(\alpha(x))$ as $x\to a$ if there exists constants $C > 0$ and $r > 0$ such that

$$
  \norm{f(x)}_Y \leq C\alpha (x),\; \forall x\in B_r (a) \setminus\set{a}
$$

where $B_r (a) = \set{x\in X : \norm{x - a}_X < r}$ is the open ball of radius $r$ centered at $a\in X$. 

For the case $\norm{x}_X \to\infty$, we write $f(x) = O(\alpha(x))$ if there exists constants $C > 0$ and $K > 0$ such that 

$$
  \norm{f(x)}_Y \leq C \alpha(x),\;  \forall x\in X,\, \norm{x}_X > K
$$

**Little o-notation**

For a limit point $a\in X$, we write $f(x) = o(\alpha(x))$ as $x\to a$ if

$$
  \lim_{x\to a} \frac{\norm{f(x) }_X}{\alpha (x)} = 0
$$

or, equivalently, if for every $\epsilon > 0$ there exists $\delta > 0$ such that

$$
  \frac{\norm{f(x)}_Y}{\alpha (x)} \leq \epsilon,\; \forall x\in B_\delta (a) \setminus\set{a}
$$

For the case $\norm{x}_X \to\infty$, we write $f(x) = o(\alpha(x))$ if

$$
  \lim_{\norm{x}_X \to\infty} \frac{\norm{f(x)}_Y}{\alpha(x)} = 0
$$

or, equivalently, if for every $\epsilon > 0$ there exists $K > 0$ such that

$$
  \frac{\norm{f(x)}_Y}{\alpha (x)} \leq \epsilon,\; \forall x\in X,\,\norm{x}_X > K
$$
</MathBox>

<MathBox title='Properties of the big O symbol' boxType="proposition">
Let $f_i : X\to Y$ and $\alpha_i : X\to (0,\infty)$ for $i\in\set{1,2}$, where $X, Y$ are normed spaces. Suppose $f_i (x) = O(\alpha_i(x))$ as $\norm{x}_X \to\infty$. Then
1. $f_1 (x) + f_2 (x) = O(\alpha_1(x) + \alpha_2 (x))$
2. $f_1 (x) f_2 (x) = O(\alpha_1 (x) \alpha_2 (x))$
3. If there exists $a \in X$ such that for all $x \in X$ with $\norm{x}_X > \norm{a}_X$, we have $\alpha_1 (x) < \alpha_2 (x)$, implying
$$
  f_1 (x) + g_2 (x) = O(\alpha_2 (x))
$$

<details>
<summary>Proof</summary>

The assumption $f_i (x) = O(\alpha_i(x))$ as $\norm{x}_X \to\infty$ for $i\in\set{1,2}$ means that there exist $C_i > 0$ and $K_i > 0$ such that

$$
  \norm{f_i (x)}_X \leq C_i \alpha_i,\; \forall x\in X,\,\norm{x} > K_i
$$
...
$$
\begin{align*}
  \norm{f_1 (x) + f_2 (x)}_X \leq& \norm{f_1 (x)}_X + \norm{f_2 (x)}_X \\
  \leq& C_1 \alpha_1 (x) + C_2 \alpha_2 \\
  \leq& \max\set{C_1, C_2} [\alpha_1 (x) + \alpha_2 (x)]
\end{align*}
$$

Hence, $f_1 (x) + f_2 (x) = O(\alpha_1(x) + \alpha_2 (x))$.
...
\begin{align*}
  \norm{f_1 (x) f_2 (x)} \leq& \norm{f_1 (x)}\cdot\norm{f_2 (x)} \\
  \leq& C_1 \alpha_1 (x) C_2 \alpha_2 (x) \\
  =& C_1 C_2 \alpha_1 (x) \alpha_1 (x)
\end{align*}
$$

Hence, $f_1 (x) f_2 (x) = O(\alpha_1 (x) \alpha_2 (x))$.

**(3):** Since $\alpha_1 (x) < \alpha_2 (x)$ by assumption, we have

$$
\begin{align*}
  \norm{f_1 (x) + f_2 (x)} \leq& \norm{f_1 (x)} + \norm{f_2 (x)} \\
  \leq& C_1 \alpha_1 (x) + C_2 \alpha_2 (x) \\
  \leq& (C_1 + C_2) \alpha_2 (x)
\end{align*}
$$

such that $f_1 (x) + f_2 (x) = O(\alpha_2 (x))$.
</details>
</MathBox>
...
Let $X'$ be the dual space of a normed space $X$. The dual space itself is a normed space $\left(X', \norm{\cdot}_{X \to \mathbb{F}}\right)$, which is also a Banach space.
...
$$
  \forall \epsilon > 0 \quad \exists N \in \N \quad \forall n, m \geq N : \norm{\ell_n - \ell_m}_{X\to\mathbb{F}} < \epsilon
$$

Since the operator norm $\norm{\cdot}_{X\to\mathbb{F}}$ is defined by a supremum, we know that for all $x \in X$ with $x\neq 0$

$$
\begin{gather*}
  \frac{1}{\norm{x}_X} \left| \ell_n - \ell_m \right| \leq \norm{\ell_n - \ell_m}_{X\to\mathbb{F}} < \epsilon \\
  \left| \ell_n - \ell_m \right| < \epsilon \norm{x}_X
\end{gather*}
$$
...
We then have to show that $\ell: X \to \mathbb{F}$ is linear and bounded and that $\norm{\ell_k - \ell}_{X \to \mathbb{F}} \xrightarrow{k\to\infty} 0$. Boundedness follows from the Cauchy property of $\left( \ell_k \right)_{k\in\N}$:

$$
\begin{gather*}
  \norm{\ell_n}_{X\to\mathbb{F}} \tleq \underbrace{\norm{\ell_n - \ell_N}_{X\to\mathbb{F}}}_{< \epsilon} + \underbrace{\norm{\ell_N}_{X\to\mathbb{F}}}_{:= C} \leq \epsilon + C \quad \forall n\geq N \\
  \implies \left| \ell(x) \right| = \left| \lim_{k\to\infty} \ell_k (x) \right| = \lim_{k\to\infty} \left| \ell_k (x) \right| \leq \lim_{k\to\infty} \underbrace{\norm{\ell_{k}}_{X\to\mathbb{F}}}_{\leq \tilde{C}} \norm{x}_X \\
  \implies \norm{\ell}_{X\to\mathbb{F}} = \frac{1}{\norm{x}_X} \left| \ell(x) \right| \leq \tilde{C} < \infty
\end{gather*}
$$

To show the convergence of the operator norm $\norm{\cdot}_{X\to\mathbb{F}}$, we have the for $\epsilon > 0$ there is a $N \in \N$ such that for all $n, m \geq N$

$$
\begin{gather*}
  \frac{1}{\norm{x}_X} \left| \ell_n (x) - \ell_m (x) \right| < \epsilon \\
  \implies \sup_{\overset{x\in X}{x\neq 0}} \frac{1}{\norm{x}_X}  \left| \ell_n (x) - \lim_{m\to\infty} \ell_m (x) \right| = \sup_{\overset{x\in X}{x\neq 0}} \frac{1}{\norm{x}_X}  \left| \ell_n (x) - \ell (x) \right| \leq \epsilon \\
  \implies \norm{\ell_n - \ell}_{X\to\mathbb{F}} \leq \epsilon
\end{gather*}
$$
</details>
</MathBox>

<MathBox title='Hahn-Banach theorem' boxType="theorem" tag="5">
Let $\left(X, \norm{\cdot}_X \right)$ be a normed space, $U \subset X$ a subspace and $\ell \in U^*$ a continuous linear functional on $U$. Then there exists a linear functional $\tilde{\ell} \in X^*$ that extends $\ell$ with $\tilde{\ell}(u) = \ell(u)$ for all $u \in U$ and $\norm{\tilde{\ell}}_{X^*} = \norm{\ell}_{U^*}$.
</MathBox>

## Arzelà-Ascoli theorem

<MathBox title='Arzelà-Ascoli theorem' boxType="theorem">
Consider the Banach space $\left(C([a, b]), \norm{\cdot}_\infty \right)$ of continuous functions on an interval $I = [a, b] \in \R$ with the supremum norm $\norm{f}_\infty := \sup \Set{ |f(t)| | t\in[a, b]}$. A subset $F \in C([a, b])$ is compact if and only if $F$ is closed, bounded and uniformly equicontinuous.
</MathBox>
...
$$
  \norm{f}_\infty := \inf\Set{b\in[0,\infty] : |f|\overset{\textrm{a.e.}}{\leq} b}
$$

The function $f$ is called $p$-integrable if $\norm{f}_p < \infty$. The set of all $p$-integrable functions is denoted

$$
  L^p (X,\mathcal{A},\mu) := \Set{ f:X\to\R: \norm{f}_p < \infty} \subseteq \mathcal{M}(X,\R)
$$

and is called a Lebesgue space. Here $\mathcal{M}(X,\R)$ denotes the collection of all measurable functions from $X$ into $\R$.
</MathBox>

<MathBox title='$L^p$ spaces are normed vector spaces' boxType="proposition">
For $p\in[1,\infty]$, let $\mathcal{L}^p = \Set{[f] : f\in L^p}$ and define $\norm{[f]}_p = \norm{f}_p$ for $f\in\mathcal{M}(X,\R)$ where $\mathcal{M}(X,\R)$ is the collection of measurable functions $f:X\to\R$. Then $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ and $\norm{\cdot}$ is a norm on $\mathcal{L}^p$ that satisfies for $f,g\in L^p$ and $c\in\mathcal{R}$
1. **Positive definiteness:** $\norm{f}_p \geq 0$ and $\norm{f}_p = 0$ if and only if $f\equiv 0$
...
That $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ follows immediately from that $L^p\subseteq\mathcal{M}$. It remains to show that $\norm{\cdot}_p$ satisfies the norm properties.

1. The predicate $\norm{f}_p \geq 0$ follows from the definitions. For the second part, we first consider the case $p\in(0,\infty)$. Trivially, $\norm{0}_p = 0$. Conversely, if $\norm{f}_p = 0$ then $\int_X \norm{f}^p \;\d\mu = 0$ and hence $|f|^p \overset{\textrm{a.e.}}{=} 0$ and $f \overset{\textrm{a.e.}}{=} 0$. For the case $p = \infty$, it follows trivially that $\norm{0}_\infty = 0$. Conversely, suppose $\norm{f}_\infty = 0$. Then for each $n\in\N$ there exists $b_n\in[0,\infty)$ with $b_n \xrightarrow{n\to\infty}0$ and $|f\overset{\textrm{a.e.}}{\leq} b_n$, resulting in $f\overset{\textrm{a.e.}}{=}0$.
2. For $p\in(0,\infty)$ we have

$$
\begin{gather*}
  \norm{cf}^p \int_X |cf|^p\;\d\mu = |c|^p \int_X |f|^p\;\d\mu = |c|^p\cdot\norm{f}^p \\
  \iff \norm{cf} = |c|\cdot\norm{f}_p
\end{gather*}
$$
...
\begin{align*}
  ...
  =& y^q \left(\frac{1}{p} + \frac{1}{q}\right) - y^q \\
  =& y^q - y^q = 0
\end{align*}
$$
...
$$
  \norm{fg}_1 = \int_X |fg|\;\d\mu\leq \norm{f}_p \cdot\norm{g}_q
$$

If $(X,\mathcal{P}(X),\#)$ is a discrete measure, Hölder's inequality reduces to

$$
  \norm{xy}_1 = \left(\sum_{i\in X} |x_i + y_i| \right) \leq \norm{x}_p \cdot \norm{y}_q
$$

<details>
<summary>Proof</summary>

The result follows trivially if $\norm{f}_p = \infty$ or $\norm{g}_\infty = \infty$, so assume $f\in L^p$ and $g\in L^q$. For the case $p = 1$ and $q = \infty$, note that $|g|\overset{\mu\textrm{-a.e.}}{\leq}\norm{g}_\infty$, hence

$$
  \int_X |fg|\;\d\mu = \int_X |f|\cdot|g|\;\d\mu \leq\norm{g}_\infty \int_X |f|\;\d\mu = \norm{f}_1 \cdot \norm{g}_\infty
$$

Consider the case where $p,q\in(1,\infty)$. By positive definiteness, the result holds if $\norm{f}_p = 0$ or $\norm{g}_q = 0$, so assume $\norm{f}_p\geq 0$ and $\norm{g}_q\geq 0$. By Young's inequality

$$
  |fg|\leq\frac{1}{p}|f|^p + \frac{1}{q}|g|^q
$$

Suppose first that $\norm{f}_p = \norm{g}_q = 1$. From the increasing monotonicity and linearity properties of the integral

$$
  \int_X |fg|\;\d\mu\leq \frac{1}{p}\int_X |f|^p \;\d\mu + \frac{1}{q}\int_X |g|^q \;\d\mu = \frac{1}{p} + \frac{1}{q} = 1
$$

For the general case where $\norm{f}_p > 0$ and $\norm{g}_q > 0$, let $f_1 = \frac{f}{\norm{f}_p}$ and $g_1 = \frac{g}{\norm{g}_q}$. Then $\norm{f_1}_p = \norm{g_1}_q = 1$ and $\norm{f_1g_1}_q\leq 1$. By the scaling property of the norm
...
$$
  \norm{f + g}_p = \left(\int_X |f + g|^p \;\d\mu\right)^{1/p} \leq \norm{f}_p + \norm{g}_p
$$

If $(X,\mathcal{P}(X),\#)$ is a discrete measure, Minkowski's inequality reduces to

$$
  \norm{x + y}_p = \left(\sum_{i\in X} |x_i + y_i|^p\right)^{1/p} \leq \norm{x}_p + \norm{y}_p
$$

<details>
<summary>Proof</summary>

The result is trivial if $\norm{f}_p = \infty$ or $\norm{g}_\infty = \infty$, so assume $f,g\in L^p$. When $p = 1$, the result is the triangle inequality

$$
\begin{align*}
  \norm{f + g}_p =& \int_X |f + g|\;\d\mu \leq\int_X (|f| + |g|)\;\d\mu \\
  =& \int_X |f|\;\d\mu + \int_X |g|\;\d\mu = \norm{f}_1 + \norm{g}_1
\end{align*}
$$

For the case $p = \infty$, note that if $a,b\in[0,\infty]$ are an essential bounds for $f$ and $g$, respectively, then $a+b$ is an essential bound for $f + g$. Hence $\norm{f + g}_\infty \leq\norm{f}_\infty + \norm{g}_\infty$.
...
$$
  \norm{f + g}_p^p \leq \int_X |f+g|^{p-1}|f|\;\d\mu + \int_X |f+g|^{p-1}|g|\;\d\mu
$$
...
$$
\begin{align*}
  \int_X |f+g|^{p-1}|f|\;\d\mu &\leq \Norm{|f+g|^{p-1}}_q \cdot \norm{f}_p \\
  \int_X |f+g|^{p-1}|g|\;\d\mu &\leq \Norm{|f+g|^{p-1}}_q \cdot \norm{g}_p
\end{align*}
$$
...
$$
\begin{align*}
  \Norm{|f+g|^{p-1}}_q =& \left(\int_X |f+g|^{p-1}q \;\d\mu \right)^{1/q} \\
  =& \left(\int_X |f+g|^p \;\d\mu \right)^{(p-1)/p} \\
  =& \norm{f+g}_p^{p-1}
\end{align*}
$$
...
$$
  \norm{x + y}_1 = \sum_{j=1}^\infty \left| x_j + y_j \right| \leq \sum_{j=1}^\infty \left| x_j \right| + \left| y_j \right| = \norm{x}_1 + \norm{y}_1
$$

For $p \in (1, \infty)$, we have $q = \frac{p}{p-1}$, yielding

$$
\begin{align*}
  \norm{x + y}_p^p =& \sum_{j=1}^\infty \left| x_j + y_j \right|^p = \lim_{n\to\infty} \sum_{j=1}^n \left| x_j + y_j \right|^p \\
...
$$
  \norm{b}_q = \left( \sum_{j=1}^n \left| \left( \left| x_j \right| + \left| y_j \right| \right)^{p-1} \right|^q \right)^{\frac{1}{q}} = \left( \sum_{j=1}^n\left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{q}}
$$
...
$$
\begin{align*}
  \sum_{j=1}^n \left(\left| x_j \right| + \left| y_j \right| \right)^p \leq& \norm{a}_p \cdot \norm{b}_q + \norm{c}_p \cdot \norm{b}_q \\
  =& \left( \norm{a}_p + \norm{c}_p \right) \left( \sum_{j=1}^n \left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{q}}
\end{align*}
$$
...
$$
\begin{align*}
  \implies \left( \sum_{j=1}^n \left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{1 - \frac{1}{q}} =& \left( \sum_{j=1}^n \left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{p}} \leq \norm{a}_p + \norm{c}_p \\
  =& \left( \sum_{j=1}^n \left| x_j \right|^p \right)^{\frac{1}{p}} + \left( \sum_{j=1}^n \left| y_j \right|^p \right)^{\frac{1}{p}}
\end{align*}
$$
...
$$
  \norm{x}_p := \left( \sum_{i\in X} |x_i|^p \right)^(1/p)
$$

and for $p=\infty$ we define $\norm{x}_\infty = \sup\Set{x_i | i\in X}$. The Lebesgue space takes the form

$$
  L^p(X,\mathcal{P}(X),\#) = \Set{ (x_i)_{i\in X} \in\R^n : \norm{p}_p < \infty}
$$
...
The normed space $\left( \ell^p, \norm{\cdot}_p \right)$ is a Banach space.

<details>
<summary>Proof</summary> 

To show the completeness of $\left( \ell^p, \norm{\cdot}_p \right)$ we consider a Cauchy sequence $\left( x^{(k)} \right)_{k \in \N}$ in $\ell^p$. 
...
$$
  \norm{x_m^{(k)} - x_m^{(l)} }^p \leq \sum_{n=1}^\infty \norm{x_n^{(k)} - x_n^{(l)} }^p = \norm{x^{k} - x^{l}}_p^p
$$

Because $\left( x^{(k)} \right)_{k \in \N}$ is a Cauchy sequence, it follows that $\forall \epsilon > 0 \; \exists N \in \N:\; \Norm{x^{(k)} - x^{(l)}}^p$. Hence, $\left( x_m^{(k)} \right)_{k \in \N}$ is a Cauchy sequence in $\mathbb{F}$ converging to a limit $\tilde{x}_m \in \mathbb{F}$. This suggests that $\left( x^{(k)} \right)_{k \in \N}$ converges to a limit $\tilde{x} := \left( \tilde{x}_i \right)_{i \in \N}$, which we will now show.

Let $\epsilon > 0$ and choose $K \in \N$ such that $\forall k, l \geq K:\; \Norm{x^{(k)} - x^{(l)}}_p < \epsilon'$

$$
\begin{align*}
  \norm{x^{(k)} - \tilde{x}}_p^p =& \sum_{n=1}^\infty \norm{x^{(k)} - \tilde{x} }^p = \lim_{N \to \infty} \sum_{n=1}^N \norm{x^{(k)} - \tilde{x} }^p \\
  =& \lim_{N \to \infty} \lim_{l\to\infty} \sum_{n=1}^N \norm{x^{(k)} - x^{(l)} }^p \leq (\epsilon')^p
\end{align*}
$$

Thus $\forall k \geq K: \norm{x^{(k)} - \tilde{x}}_p \leq \epsilon' < \epsilon$, since $\epsilon'$ can be choosen arbitrarily small. Furthermore, it follows that $\tilde{x} = \left(\tilde{x} - x^{(k)} \right) + x^{(k)} \in \ell^p$ since both $\tilde{x} - x^{(k)}$ and $x^{(k)}$ are in $\ell^p$ and $\ell^p$ is a vector space and thus closed under addition.
</details>
</MathBox>
...
$$
  \norm{(Tx)(y) } \leq \lim_{n\to\infty} \sum_{j=1}^n \left| y_j x_j \right| \leq \norm{y}_p \cdot \norm{x}_q < \infty
$$
...
$$
\begin{gather*}
  \norm{Tx}_{\ell^p (\N) \to \mathbb{F}} = \sup\Set{ \left\lvert (Tx)(y) \right\rvert : \norm{y}_p = 1} \leq \sup\Set{\norm{y}_p \cdot \norm{x}_q : \norm{y}_p = 1} \leq \norm{x}_q \\
  \implies \norm{T} = \frac{\norm{Tx}_{\ell^p (\N)}}{\norm{x}_q} \leq 1
\end{gather*}
$$
...
\begin{align*}
  ...
  \leq& \norm{y'}_{\ell^p (\N) \to \mathbb{F}} \norm{\sum_{j=1}^n t_j e_j}_p = \norm{y'}_{\ell^p (\N) \to \mathbb{F}} \left( \sum_{j=1}^n \left| t_j \right|^p \right)^{1/p}
\end{align*}
$$
...
$$
\begin{gather*}
  \sum_{j=1}^n \left| x_j \right|^q \leq \norm{y'}_{\ell^p (\N) \to \mathbb{F}} \left( \sum_{j=1}^n \left| x_j \right|^q \right)^{1/p} \\
  \implies \left( \sum_{j=1}^n \left| x_j \right|^q \right)^{1 - 1/p} = \left( \sum_{j=1}^n \left| x_j \right|^q \right)^{1/q} \leq \norm{y'}_{\ell^p (\N) \to \mathbb{F}} \\
  \xRightarrow{n\to\infty} \norm{x}_q \leq \norm{y'}_{\ell^p (\N) \to \mathbb{F}} \implies x \in \ell^q (\N)
\end{gather*}
$$
...
$$
  \norm{Tx}_{\ell^p (\N) \to \mathbb{F})} \leq \norm{x}_q \leq \norm{y}_{\ell^p (\N) \to \mathbb{F})}
$$

At the same time $\norm{Tx}_{\ell^p (\N) \to \mathbb{F})} = \norm{y}_{\ell^p (\N) \to \mathbb{F})}$, so that $\norm{Tx}_{\ell^p (\N) \to \mathbb{F})} = \norm{x}_q$ which shows that $T$ is ismotric.
</details>
</MathBox>
...
If $\braket{\cdot, \cdot}$ is an inner product, then $\norm{x}_{\braket{\cdot, \cdot}} = \sqrt{\braket{x, x}}$ defines a norm. 
</MathBox>
...
$$
\begin{align*}
  \norm{\braket{x_0, x_n} - \braket{x_0, \tilde{x}} } =& \norm{\braket{x_0, x_n - \tilde{x}}} \\
  \leq& \norm{x_0} \cdot \norm{x_n - \tilde{x}} \xrightarrow{n\to\infty} 0 \\
  \implies f(x_n) \xrightarrow{n\to\infty} f(\tilde{x})
\end{align*}
$$

<MathBox title='Polarization identity' boxType="proposition">
Let $\left(X, \braket{\cdot, \cdot} \right)$ be an inner product space over $\mathbb{F} = \mathbb{C}$ with induced norm $\norm{x } := \sqrt{\braket{x,x}}$. Then, for all $x,y\in X$
...
$$
\begin{align*}
  \norm{x + y}^2 =& \braket{x + y, x + y} = \braket{x,x} + \braket{x,y} + \braket{x,y} + \braket{y,y} \\
  -\norm{x - y}^2 =& \braket{x + y, x + y} = -\braket{x,x} + \braket{x,y} + \braket{x,y} - \braket{y,y} \\
  -i\norm{x + y}^2 =& -i\braket{x + y, x + y} = -i\braket{x,x} + \underbrace{-i\overline{i}}_{=-1}\braket{x,y} + \underbrace{-i^2}_{=1}\braket{x,y} - i\braket{y,y} \\
  i\norm{x + y}^2 =& i\braket{x + y, x + y} = i\braket{x,x} + \underbrace{-i\overline{i}}_{=-1}\braket{x,y} + \underbrace{-i^2}_{=1}\braket{x,y} + i\braket{y,y}
\end{align*}
$$
...
If $\left(X, \norm{\cdot}_{\braket{\cdot, \cdot}} \right)$ defines a Banach space, then $\left(X, \braket{\cdot, \cdot}\right)$ is called a Hilbert space.
</MathBox>
...
$$
\begin{gather*}
  \braket{x, x} = \sum_{i=1}^\infty \bar{x}_i x_i = \sum_{i=1}^\infty \norm{x_i }^2 \geq 0 \\
  \braket{x, x} = 0 \implies \norm{x_i }^2 = 0\; \forall i \in \N \implies x = 0
\end{gather*}
$$
2. Conjugate symmetry:
$$
...
Let $\left(X, \braket{\cdot, \cdot} \right)$ be an inner product space with norm $\norm{x} = \sqrt{\braket{x, x}}$. Then for all $x, y \in X$
...
$$
\begin{align*}
  \norm{x + y}^2 =& \braket{x + y, x + y} = \norm{x}^2 + 2\norm{\braket{x, y} } + \norm{y}^2 \\
  \leq& \norm{x}^2 + 2\norm{x} \cdot \norm{y} + \norm{y}^2 = \left( \norm{x} + \norm{y} \right)^2
\end{align*}
$$

Taking the square root we obtain the triangle inequality $\norm{x + y} \leq \norm{x} + \norm{y}$.

<details>
<summary>Proof</summary> 

Considering the non-trivial case $x \neq 0$, we define the unit vector $\hat{x} := \frac{x}{\norm{x}}$ and take the orthogonal projection of $y$ onto $\hat{x}$ (parallel component) given by $y_\parallel = \braket{\hat{x}, y} \hat{x}$. The Cauchy-Schwarz inequality can be obtained from taking the norm of the orthogonal component $y_\perp = y - y_\parallel$

$$
\begin{align*}
  0 \leq \norm{y_\perp}^2 =& \norm{y - y_\parallel}^2 = \braket{y - \braket{\hat{x}, y} \hat{x}, y - \braket{\hat{x}, y} \hat{x}} \\
  ...
  =& \norm{y}^2 - \overline{\braket{\hat{x}, y}}\braket{\hat{x}, y} - \braket{\hat{x}, y} \overline{\braket{\hat{x}, y}}  + \norm{\braket{\hat{x}, y} }^2 \underbrace{\norm{\hat{x}}^2}_{=1} \\
  =& \norm{y}^2 - \norm{\braket{\hat{x}, y} }^2
\end{align*}
$$
...
For each continuous linear functional $\ell: X \to \mathbb{F}$ there is exactly one $x_\ell \in X$ such that $\ell(x) = \braket{x_\ell, x}$ for all $x \in X$ and $\norm{\ell} = \norm{x_\ell}_X$.

<details>
<summary>Proof</summary>

To prove the existence of $x_\ell$ we consider the kernel of $\ell$ defined as $\ker(\ell) := \Set{ x \in X | \ell(x) = 0}$. In the trivial case $\ker (\ell) = X$ it follows that $x_\ell = 0$, so we assume $\ker(\ell) \neq X$. Evidently $\ell$ is continuous because $\ker(\ell) = \ell^{-1}(\Set{ 0})$. Thus $\ker(\ell)$ is a closed subset of $X$. Consider $\hat{x} \in \ker(\ell)^\perp$ with $\norm{\hat{x}}_X = 1$ and set $x_\ell := \overline{\ell(\hat{x})}\hat{x}$, then
...
To prove that the operator norm is $\norm{\ell} = \norm{x_\ell}$, we have

$$
  \norm{\ell } = \sup\Set{ \norm{\ell(x) } : \norm{x }_X = 1} = \sup\Set{ \underbrace{\norm{\braket{x_\ell, x} }}_{\leq \norm{x_\ell }_X \cdot \norm{x }_X} : \norm{x }_X \leq 1} \leq \norm{x_\ell }
$$

Similarly

$$
  \norm{\ell} \geq \norm{\ell \left( \frac{x_\ell}{\norm{x_\ell}} \right) } = \norm{\Braket{x_\ell, \frac{x_\ell}{\norm{x_\ell}} } } = \norm{x_\ell}
$$
...
1. For each $x\in H$, consider the seminorm $p_x$ defined on $\mathcal{L}(H, K)$ by $p_x (T) = \norm{Tx}$. The topology on $\mathcal{L}(H,K)$ defined by the collection $\Set{p_x | x\in H}$ of seminorms is called the *strong operator topology*.
...
Suppose $H$ and $K$ are Hilbert spaces. If $\Set{T_i}_{i\in I}$ for an index set $I\subseteq\N$ is a net in $\mathcal{L}(H,K)$, then the net converges to $T\in\mathcal{L}(H,K)$ with respect to the strong operator topology precisely when $\norm{T_i x - Tx } \xrightarrow{i\to\infty} 0$ for all $x\in H$, i.e. precisely when the net $\Set{T_i x}_{i\in I}$ converges to $Tx$ with respect to the strong norm topology on $K$, for every $x\in H$.
...
<MathBox title='' boxType="lemma" tag="0">
Suppose $\Set{T_i}_{i\in I}$ for an index set $I\subseteq\N$ is a net in $\mathcal{L}(H,K)$ which is uniformly bounded, i.e. $\sup\Set{\norm{T_i }}_{i\in I} = K < \infty$. Let $S_1$ and $S_2$ be total sets in $H$ and $K$, respectively, i.e. the set of finite linear combinations of elements in $S_1$ and $S_2$ that are in dense in $H$ and $K$, respectively
1. $\Set{T_i}_{i\in I}$ converges strongly to $0$ if and only if $\lim_{i\to\infty} \norm{T_i x} = 0$ for all $x\in S_1$
...
**(b):** Let $x\in H$ and $y\in K$ be arbitrary, and suppose $\epsilon > 0$ is given. We assume, without loss of generality, that $\epsilon < 1$. Let $M = 1 + \max\Set{\norm{x}, \norm{x}}$, and $K = 1 + \sup_{i\in I} \norm{T_i }$.

The assumed totality of the sets $S_i$ for $i=1,2$ implies the existence of vectors $x' = \sum_{k=1}^m \alpha_k x_k$ and $y' = \sum_{j=1}^n \beta_j y_j$, with the property that $x_k \in S_1$, $y_j \in S_2$ for all $k,j$ and such that $\norm{x - x' } < \epsilon/(3KM)$ and $\norm{y - y'} < \epsilon/(3KM)$. Since $\epsilon 1 \leq K,M$, it follows that $\norm{x' } \leq\norm{x} + 1 \leq M$. Similarly, also $\norm{y' } \leq M$. 
...
3. **Sub-multiplicativity of norm**: $\norm{xy}\leq\norm{x}\cdot\norm{y }$
...
Suppse $A_0$ is a normed algebra, and $A$ denotes its completion as a Banach space. Then $A$ acquires a natural Banach algebra structure as follows: if $x,y\in A$, pick sequences $(x_n)$ and $(y_n)$ in $A_0$ such that $x_n \xrightarrow{n\to\infty} x$ and $y_n \xrightarrow{n\to\infty} y$. Then, pick a constant $K$ such that $\norm{x_n }, \norm{y_n } \leq n$, note that

$$
\begin{align*}
  \norm{x_n y_n - x_m y_m} =& \norm{x_n (y_n - y_m) + (x_n - x_m)y_m} \\
  \leq& \norm{x_n (y_n - y_m)} + \norm{(x_n - x_m)y_m} \\
  \leq& \norm{x_n} \cdot \norm{y_n - y_m} + \norm{x_n - x_m} \cdot \norm{y_m} \\
  \leq& (\norm{y_n - y_m} + \norm{x_n - x_m})
\end{align*}
...
<MathBox title='Group of units properties' boxType="proposition" tag="21">
...
4. If $x\in A$ and $\norm{1 - x} < 1$, then $x\in G(A)$ and
$$
\begin{equation}
  x^{-1} = \sum_{n=0}^\infty (1 - x)^n \tag{\label{equation-1}}
\end{equation}
$$
In particular, if $\lambda\in\mathbb{C}$ and $|\lambda| > \norm{x }$, then $(x - \lambda)$ is invertible and
$$
\begin{equation}
...
**(5):** The assertions in **(4)** imply that $1\in\operatorname{int}(G(A))$ and is a point of continuity of the inversion map. By **(1)** and **(2)**, we may map this local picture at $1$ via the linear homeomorphism $L_x$ (which maps $G(A)$ onto itself) to a corresponing local picture at $x$, and deduce that any element of $G(A)$ is an interior point and is a point of continuity of the inversion map.
</details>
</MathBox>
...
$$
\begin{equation}
  r(x) = \sup\Set{\norm{\lambda} : \lambda\in\sigma(x)} \leq\norm{x } \tag{\label{equation-3}}
\end{equation}
$$
...
<MathBox title='Resolvent properties' boxType="proposition" tag="22">
Let $x\in A$, then
...
$$
  \norm{R_x (\lambda) } \leq \frac{C}{|\lambda|},\; |\lambda| > \norm{x }
$$
...
<MathBox title='' boxType="proposition" tag="24">
...
<MathBox title='Spectral radius formula' boxType="theorem" tag="12">
...
which is valid in the exterior of the disc $\Set{\lambda\in\mathbb{F}: |\lambda|\leq\norm{x}}$. Since $F$ vanishes at infinity, the function $F$ is analytic at the point at infinity, and consequently the Laurent expansion above is valid in the larger region $|\lambda| > r(x)$.
...
$$
\begin{gather*}
  \norm{x^n} \leq K|\lambda|^n,\; \forall n\in\N \\
  \norm{x^n}^{1/n} \leq K^{1/n} |\lambda|
\end{gather*}
$$
...
On the other hand, from Proposition $\ref{proposition-24}.2$ and equation $\eqref{equation-3}$ that $r(x) = r(x^n)^{1/n} \leq \norm{x^n }^{1/n}$. Hence, we have 

$$
  r(x) \leq \liminf_{n\in\N} \norm{x^n }^{1/n}
$$
</details>
</MathBox>

<MathBox title='' boxType="proposition" tag="25">
...
<MathBox title='Criteria for proper ideals' boxType="proposition" tag="26">
...
<MathBox title='' boxType="proposition" tag="27">
...
<MathBox title='Gelfand-Mazur theorem' boxType="theorem" tag="13">
...
3. $\phi\in\hat{A}$
4. $\phi\in A^*$, $\norm{\phi} = 1$, and $\phi(xy) = \phi(x)\phi(y)$ for all $x,y\in A$
...
and hence $\norm{\phi}\leq 1$. On the other hand, since $\phi(1) = 1$, we must have $\norm{\phi} = 1$, and hence **(3)** $\implies$ **(4)**. The other implication is obvious.
</details>
</MathBox>

<MathBox title='' boxType="corollary" tag="1">
Let $A$ be a non-unital commutative Banach algebra. Then $\phi\in\hat{A} \implies\phi\in A^*$ and $\norm{\phi}\leq 1$.

<details>
<summary>Proof</summary>

Let $A^+$ denote the unitisation of $A$. Define $\varphi^+: A^+ \to \mathbb{F}$ by $\phi^+ (x,\alpha) = \phi(x) + \alpha$. It is easily verified that $\phi^+ \in\hat{A}^+$. Hence, by Lemma $\ref{lemma-1}.3$, we see that $\phi^+ \in (A^+)^*$ and $\norm{\phi^+ } \leq 1$, and the desired conclusion follows easily.
</details>
</MathBox>

<MathBox title='Gelfand transform' boxType="theorem" tag="14">
...
**(1):** Consider the following sets: for fixed  $x,y\in A$, let $K_{x,y} = \Set{\phi\in\operatorname{ball}(A^*) | \phi(xy) = \phi(x)\phi(y)}$, and let $V = \Set{\phi\in\operatorname{ball}(A^*) | \phi\neq 0}$, where $\operatorname{ball}(A^*) = \Set{\phi\in A^* : \norm{\phi}\leq 1}$. The definition of weak* topology implies that $K_{x,y}$ (respectively, $V$) is a closed (respectively, open) subset of $\operatorname{ball}(A^*)$. Thus, $K = \bigcap_{x,y\in A} K_{x,y}$ is also a closed, hence compact, set in the weak* topology. Notice now that $\hat{A}= K\cap V$, and since an open subset of a (locally) compact Hausdorff space is locally compact and Hausdorff, we see that $\hat{A}$ is a local compact Hausdorff space. In case $A$ has an identity, then $F = \Set{\phi\in\operatorname{ball}(A^*) | \phi(1) = 1}$ is weak* closed, and $\hat{A} = K\cap F$ is a closed subset of $\operatorname{ball}(A*)$.

**(2):** The definition of the weak* topology guarantees that $\hat{x}$ is a continuous function on $\hat{A}$, for every $x\in A$. Obviously, the mapping $\Gamma: x\mapsto \hat{x}$ is linear and preserves products. To complete the proof, we need to verify that $\Gamma$ maps $A$ into $C_0 (\hat{A})$, and that $\norm{\Gamma(x)}\leq\norm{x}$.

First, consider that case when $A$ has an identity. In this case, $C_0 (\hat{A}) = C(\hat{A})$, while, by Lemma $\ref{lemma-1}$, we find that

$$
\begin{align*}
  \norm{\Gamma(x)} =& \sup\Set{\norm{\hat{x}(\phi)} : \phi\in\hat{A}} \\
  =& \sup\Set{\norm{\lambda} : \lambda\in\sigma(x)} \\
  =& r(x) \leq\norm{x}
\end{align*}
$$

Now suppose $A$ does not have an identity. By Corollary $\ref{corollary-1}$, it follows that $\Gamma$ is contractive, so we only need to verify that functions on $(A^+\hat{)}$ of the form $\hat{x}$ vanish at infinity.

Let $A^+$ be the unitisation of $A$, and let $f:\hat{A}\to (A^+\hat{)}$ by $f(\phi) = \phi^+$ be as in the proof of Corollary $\ref{corollary-1}$. Note that $(A^+\hat{)} = f(\hat{A})\cup\Set{\phi_0}$, where $\phi_0 (x,\alpha) = \alpha$, since $\phi_0$ is the unique complex homomorphism of $A^+$ whose restriction to $A$ is identically equal to $0$. On the other hand, the topologies on $\hat{A}$ and $(A^+\hat{)}$ show that a net $\Set{\phi_i}_{i\in I}$ converges to $\phi$ in $\hat{A}$ if and only if $\Set{\phi_i^+}_{i\in I}$ converges to $\phi^+$ in $\hat{A}$. In other words, the function $f$ maps $\hat{A}$ homemorphically onto $f(\hat{A})$.

Thus, we find - from the compactness of $(A^+\hat{)}$, and from the nature of the topologies on the spaces concerned - that we may identify $(A^+\hat{)}$ with the one-point compactification $(\hat{A})^+$ of the locally compact Hausdorff space $\hat{A}$, with the element $\phi_0$ playing the role of the point at infinity. Note that if we identify $\hat{A}$ as a subspace of $(A^+\hat{)}$ via $f$, then the mapping $\widehat{(x,0)}$ is a continuous function on $(A^+\hat{)}$ which extends $\hat{x}$ and which vanishes at infinity (since $\phi_0 (x,0) = 0$). Thus, we find that $\hat{x}\in C_0 ((A^+\hat{)})$ for all $x\in A$.
</details>
</MathBox>
...
Suppose first that $A$ has an identity. Since $\norm{\Gamma(x)} = r(x)$, the equivalence of the four conditions follows easily from Proposition $\ref{proposition-27}$, Lemma $\ref{lemma-1}$ and the spectral radius formula (Theorem $\ref{theorem-12}$).
...
<MathBox title='' boxType="proposition" tag="28">
...
Define the obvious involution on $A^+$ by demanding that $(L_x 0 \alpha)^* = L_{x^*} + \overline{\alpha}$. It is easily verified that this involution satisfies the C* properties. It remains to verify that $\norm{z^* z} \geq\norm{z}^2$ for all $z\in A^+$. We assume $z \neq 0$, since the other case is trivial. For $z = L_x + \alpha$, we have $z^* z = L_{x^* x + \overline{\alpha}x + \alpha x^*} + |\alpha|^2$. Note that for arbitrary $y\in A$, we have

$$
\begin{align*}
  \norm{z(y) } =& \norm{xy + \alpha y }^2 \\
  =& \norm{(y^* x^* + \overline{\alpha}y^* (xy + \alpha y))} \\
  =& \norm{y^* \cdot (z^* z)(y) } \\
  \leq& \norm{z^* z } \cdot \norm{y }^2
\end{align*}
$$

where we have used the C* identity in the second line, and the submultiplicativity of the norm and the definition of the norm on $A^+$ in the last line. Since $y$ is arbitrary, this shows that we have $\norm{z }^2 \leq \norm{z^* z}$.
</details>
</MathBox>

<MathBox title='Gelfand-Naimark theorem for commutative C*-algebras' boxType="theorem" tag="15">
...
1. if $x\in A$ is arbitrary, then $\Gamma(x^*) = \Gamma(x)^*$ and $\norm{\Gamma(x)} = \norm{x}$
2. $\hat{A}$ is compact
...
If $\phi\in\hat{H}$, then $|\phi(u_t)| \leq 1$ for all $t\in\R$ since $\norm{\phi} = 1$. On the other hand, since $\phi$ is continuous and multiplicative, it is seen that $\phi(u_t) = e^{it\phi(x)}$. In other words, $|e^{it\phi(x)}|\leq 1$ for all $t\in\R$. This clearly implies that $\phi(x)\in\R$. Thus, we have shown that $\hat{x}$ is a real-valued function, for every self-adjoint element $x\in A$. By Cartesian decomposition of an element as a sum of a self-adjoint and skew-adjoint element, we may conclude that $\Gamma$ is a homomorphism of *-algebra. If $x = x^* \in A$, note that

$$
  \norm{x}^2 = \norm{x^* x} = \norm{x^2 }
$$

and conclude by induction that $\norm{x}^{2n} = \norm{x^{2n} }$ for all $n\in\N_+$. It follows from the spectral radius formula (Theorem $\ref{theorem-12}$) that

$$
  r(x) = \lim_{n\to\infty} \norm{x^{2n} }^{1/2^n} = \norm{x}
$$

On the other hand, we know that $\norm{\Gamma(x)} = r(x)$ (Lemma $\ref{lemma-1}$). Thus, for self-adjoint $x$, we find that $\norm{\Gamma(x)} = \norm{x }$. The case of general $x$ follows from this, the fact that $\Gamma$ is a *-homomorphism, and the C* identity. Hence
...
Let $A^+$ and $I$ be as in the proof of Proposition $\ref{proposition-28}$. By Case 1, we know that $\Gamma_{A^+}$ is an isometric *-isomorphism of $A^+$ onto $C(\hat{A})$, and that there is a point $\phi_0 \in (A^+\hat{)}$ such that $\Gamma_{A^+} (I) = \Set{f\in C(\hat{A}) | f(\phi_0) = 0}$. As in the proof of Theorem $\ref{theorem-14}$, note that if $\pi:A\to I$ is the natural isometric *-isomorphism of $A$ onto $I$, the mp $\phi\mapsto\phi\circ\pi$ defines a bijective correspondence between $(A^+\hat{)} \setminus\Set{\phi_0}$ and $\hat{A}$ which is a homeomorphism from the domain, with the subspace topology inherited from $(A^+\hat{)}$, and the range. 

Finally, it is clear that if $x\in A$ and $\phi_0 \neq\pho \in (A^+\hat{)}$, then $(\Gamma_{A^+}(\pi(x)))(\phi) = (\Gamma_A (x))(\phi\circ\pi)$, and it is this seen that even in the non-unital case, the Gelfand transform is an isometric *-isomorphism of $A$ onto $C_0 (\hat{A})$.
...
<MathBox title='' boxType="proposition" tag="30">
...
$$
  D' = \Set{f | f(z) = \sum_{i,j=0}^n \alpha_{i,j} z^i \overline{z}^j,\,n\in\N,\,\alpha_{i,j}\in\mathbb{C}}
$$
...
<MathBox title='' boxType="proposition" tag="31">
...
<MathBox title='' boxType="proposition" tag="32">
If $A$ is a C* algebra, if $x,y\in A$, and if $x,y \geq 0$ are positive, then also $(x + y) \geq 0$ is positive.

<details>
<summary>Proof</summary>

To start with (by embedding $A$ in a larger unital C* algebra, if necessary), we may assume that $A$ is itself a unital C* algebra. Next, we may (by scaling both $x$ and $y$ down by the same small positive scalar, if necessary) assume without loss of generality that $\norm{x}, \norm{y } \leq 1$. Thus, the spectral radius is $r(x) < 1$ and we may conclude that $\sigma(x) \subset [0,1]$, and consequently deduce that $\sigma(1 - x) \subset[0,1]$, and hence that $(1 - x) \geq 0$ and $\norm{1 - x} = r(1 -x) \leq 1$. Similarly, also $\norm{1 - y }\leq 1$. Then, $\norm{1 - \frac{x + y}{2}} = \frac{1}{2}\norm{(1 - x) + (1 - y)}\leq 1$ and by the spectral mapping theorem (Proposition $\ref{proposition-24}$), we have $\sigma(x + y) \subset[0,4]$.
</details>
</MathBox>
...
- Anti-symmetry implies $x = x^*$ and $x, -x \geq 0 \iff x = 0$. Such an $x$ should have the property that $\sigma(x) \subset [0,\infty)$ and $\sigma(x)\subset (-\infty,0]$. This would mean that $\sigma(x) = \Set{0}$, and since $x = x^*$, it would follow that $\norm{x } = r(x) = 0$.
</details>
</MathBox>
...
A representation of a unital C* algebra $A$ is a *-homomorphism $\pi:A\to\mathcal{L}(H)$, where $H$ is a Hilbert space. This will always be assumed to be a unital homomorphism, meaning that $\pi(1) = I$, where $1$ is the identity of $A$ and $I\in\mathcal{L}(H)$ is the identity operator on $H$.
...
A representation $\pi: A\to\mathcal{L}(H)$ is *cyclic* if thre is a vector $\mathbf{x}\in H$ such that $\Set{\pi(x)(\mathbf{x}): x\in A}$ is dense in $H$. In this case, the vector $\mathbf{x}\in H$ is said to be a *cyclic vector* for the representation $\pi$.
</MathBox>
...
<MathBox title='' boxType="proposition" tag="34">
...
**(3):** Note that **(a)** is a an immediate consequence of the definition, as is the fact that $\subset S''$. Applying **(a)** to this inclusion, we find that $S' \supset S'''$. On the other hand, if we replace $S$ by $S'$ in the inclusion, we see that $S' \subset S''$, proving **(b)**.
...
<MathBox title="von Neumann's density theorem" boxType="theorem" tag="16">
...
If $z\in A''$, consider the element $z^{(n)} \in (A^{(n)})''$ and the vector $\mathbf{x} = (\mathbf{x}_i)_{i=1}^n \in H^n$. By **Case 1**, there is an element of $a^{(n)} \in A^{(n)}$ with $a\in A$ such that $\norm{(z^{(n)} - a^{(n)}\mathbf{x}} < \epsilon$. This implies that $\norm{(z - a)\mathbf{x}_i } < \epsilon$ for $1 \leq i \leq n$.
</details>
</MathBox>
...
<MathBox title='Characterization of states' boxType="proposition" tag="35">
...
In particular, setting $y = 1$ and using the obvious inequality $x^* x \leq \norm{x }^2 1$ and positivity of $\phi$, we find that

$$
  |\phi(x)|^2 \leq \phi(x^* x) \phi(1) \leq\norm{x }^2 \phi(1)^2 = \norm{x }^2
$$

so that $\phi\in A^*$ and $\norm{\phi}\leq 1$. Since $\norm{1} = 1 = \phi(1)$, we find that $\norm{\phi} = \phi(1) = 1$, as desired.

**(2)$\implies$(1):** We want to show that if $x = x^*$, and if $\sigma(x)\subset[a,b]$, then $\phi(x) \in [a,b]$. This will imply that $\phi$ is positive and that $\phi(1) = 1$. Set $c = (a+b)/2$ and $r = (b-a)/2$, and note that $\sigma(x-c)\subset[-r,r]$ and consequently $\norm{x - c}\leq r$. Hence, it follows from the assumptions **(2)** that
...
<MathBox title='Gelfand-Naimark-Segal (GNS) construction' boxType="theorem" tag="17">
...
We claim now that each $L_x$ is a bounded operator on the inner product space $V$ and that $\norm{L_x }_{\mathcal{L}(V)} \leq \norm{x }_A$. This amounts to the assertion that

$$
  \phi(y^* x^* xy) = \norm{L_x \eta(y)}^2 \leq \norm{x}^2 \norm{\eta(y)}^2 = \norm{x }^2 \phi(y^* y),\; \forall x,y\in A
$$

Note now that, for each fixed $y\in A$, if we consider the functional $\psi(z) = \phi(y^* zy)$, then $\psi$ is a positive linear functional. By Proposition $\ref{proposition-35}$, we find that $\norm{\psi} = \psi(1) = \phi(y^* y)$. In particular, we find that for arbitrary $x,y\in A$, we must have $\phi(y^* x^* xy) = \psi(x^* x) \leq\norm{\psi}\cdot\norm{x^* x}$. In other words, $\phi(y^* x^* xy) \leq\norm{x}^2 \phi(y^* y)$ as asserted.
...
Let $x$ be a self-adjoint element of a C* algebra $A$. Then there exists a cyclic representation $\pi$ of $A$ such that $\norm{\pi(x)} = \norm{x}$.

<details>
<summary>Proof</summary>

Let $A_0 = C^* (\Set{1,x})$ be the commutative unital C* subalgebra generated by $x$. Since $A_0 \cong C(\sigma(x))$, there exists a complex homomorphism $\phi_0 \in \hat{A}_0$ such that $|\phi_0 (x)| = \norm{x}$. Notice that $\norm{\phi_0 } = 1 = \phi_0 (1)$.

By the Hahn-Banach theorem ($\ref{theorem-5}$), we can find a $\phi\in A^*$ such that $\phi|_{A_0} = \phi_0$ and $\norm{\phi} = \norm{\phi_0 }$. It follows then that $\norm{\phi} = 1 = \phi(1)$. Hence $\phi$ is a state on $A$, by Proposition $\ref{proposition-35}$.

If $\pi_\phi$ is the (cyclic) GNS-representation afforded by the state $\phi$ as in Theorem $\ref{theorem-17}$, it follows from Lemma $\ref{lemma-4}$ that

$$
  \norm{x} = |\phi_0 (x)| = |\phi(x)| = |\braket{\pi_\phi (x)\mathbf{x}_\phi, \mathbf{x}_\phi}|\leq\norm{\pi_\phi (x)}
$$
</details>
</MathBox>
...
Let $\Set{x_i}_{i\in I}$ be a dense set in $A$. For each $i\in I$, pick a cyclic representation $\pi_i A\to\mathcal{L}(H_i)$ such that $\norm{\pi_i (x_i^* x_i)} = \norm{x_i^* x_i}$, which is possible by Lemma $\ref{lemma-7}$. Note that the C*-identity shows that we have $\norm{\pi_i (x_i)} = \norm{x_i }$ for all $i\in I$.

Let $\pi = \bigoplus_{i\in I} \pi_i$. From Lemma $\ref{lemma-4}$ and the previous argument, it follows that for arbitrary $i,j\in I$, we have $\norm{\pi_j (x_i)}\leq\norm{x_i } = \norm{\pi_i (x_i)}$. Thus, we see that $\norm{\pi(x_i)} = \norm{x_i }$ for all $i\in I$. Since the set $\Set{x_i}_{i\in I}$ is dense in $A$, we conclude that the representation $\pi$ is necessarily isometric.
...
<MathBox title='' boxType="proposition" tag="36">
...
$$
\begin{align*}
  \norm{U\mathbf{x}}^2 =& \int |\mathbf{x}|^2 \phi^2\;\d\mu = \int |\mathbf{x}|^2 \frac{\d\nu}{\d\mu}\d\mu \\
  =& \int |\mathbf{x}|^2 \d\nu = \norm{\mathbf{x}}^2
\end{align*}
$$
...
Since $C(X)$ is dense in $L^2 (X,\mathcal{B}_X, \mu)$, we may find a sequence $\Set{h_n}_{n\in\N}$ in $C(X)$ such that $\norm{h_n - \phi}_{L^2 (X,\mathbb{B}_X,\mu)} \xrightarrow{n\to\infty} 0$. Since every null convergent sequence in $L^2 (X,\mathcal{B}_X,\mu)$ is known, we can find a subsequence, say $\Set{g_n}_{n\in\N}$ of $\Set{h_n}_{n\in\N}$, such that $g_n (x) \xrightarrow{n\to\infty} \phi(x)$ almost everywhere.

Let $\norm{\phi}_{L^\infty} = K$, and define a continuous "retraction" of $\mathbb{C}$ onto the disc of radius $K+1$ as follows
...
Finally, consider the continuous functions defined by $f_n = r\circ g_n$. It should be clear that $\norm{f_n } \leq (K + 1)$ for all $n\in\N$, and that $f_n (x) \xrightarrow{n\to\infty} f(x)\mu$ almost everywhere.
</details>
</MathBox>

<MathBox title='Hahn-Hellinger theorem' boxType="theorem" tag="19">
...
$$
2. Suppose $\mu_1$ and $\mu_2$ are two probability measures, and suppose that for each $i=1,2$, we have a collection $\Set{E_n^{(i)}}_{n=1}^{\aleph_0}$ of pairwise disjoint Borel subsets of $X$ such that $\mu_i$ is supported on $\bigcup_{1\leq n\leq\aleph_0} E_n^{(i)}$. Then the following conditions are equivalent:
...
<MathBox title='' boxType="lemma" tag="11">
...
1. If $\pi:C(X)\to\mathcal{L}(H)$ is a representation, there exists a probability measure $\mu$ defined on $\mathcal{B}_X$, and a representation $\tilde{\pi}:L^\infty (X, \mathcal{B}_X, \mu)\to\mathcal{L}(H)$ such that
    ...
    - c. $\tilde{\pi}$ respects bounded convergence, meaning that whenever $\phi$ is the $\mu$-almost everywhere limit of a sequence $\Set{\phi_n}_{n\in\N}\subset L^\infty (X,\mu)$, which is uniformly bounded, i.e. $\sup_{n\in\N} \norm{\phi_n }_{L^\infty (\mu)} < \infty$, then it is the case that the sequence $\Set{\tilde{\pi}(\phi_n)}_{n\in\N}$ converges in the strong operator topology to $\tilde{\pi}(\phi)$.
...
**(1):** First consider the case when $\pi$ is a cyclic representation. Then there exists a probability measure $\mu$ such that $\pi\cong\pi_\mu$. Take this $\mu$ as the measure in the statement of **(a)** and define $\tilde{\pi}$ to be the natural multiplication representation defined by $\tilde{\pi}(\phi)\mathbf{x} = \phi\mathbf{x}$ for all $\phi\in L^\infty (\mu)$ and $\mathbf{x}\in L^2 (\mu)$. Then we see that **(a)** is is obvious, while **(c)** is an immediate consequence of the dominated convergence theorem. 

As for **(a)**, suppose $\phi\in L^\infty (\mu)$ and $\epsilon > 0$. If $E=\Set{x : |\phi(x)|\geq \norm{\phi} - \epsilon}$. Thus, if $\mathbf{x} = \mu(E)^{-1/2} 1_E$, we find that $\mathbf{x}$ is a unit vector in $L^2 (\mu)$ such that $\norm{\tilde{\pi}(\phi)\mathbf{x}}\geq\norm{\phi} - \epsilon$. The validity of **(1)**, at least in the cyclic case under consideration, follows from the arbitrariness of $\epsilon$ and from Lemma $\ref{lemma-4}$ applied to the C* algebra $L^\infty (\mu)$. 

For general (separable) $\mu$, we may assume without loss of generality that $\pi = \bigoplus_{n\in N} \pi_{\mu|_{E_n}}^n$ by the Hahn-Hellinger theorem (Theorem $\ref{theorem-19}.1$). Then define $\tilde{\pi} = \bigoplus_{n\in\N} = (\tilde{\pi}_{\mu|_{E_n}})^n$ where the summands are defined as above. Since $\mu$ is supported on $\bigcup_{n\in\N} E_n$ by Theorem $\ref{theorem-19}.1.b$, and since $\tilde{\pi}_{\mu|_{E_n}}$ is an isometric map of $L^\infty (E_n, \mu|_{E_n})$, it is fairly easy to deduce that $\tilde{\pi}$ is an isometric *-homomorphism of $L^\infty (X,\mu)$, thus establishing **(1.a)**. The assertion **(b)** follows immediately from the definition and the already established cyclic case.
...
where $x_{n,m} = \tilde{\pi}_{\mu|_{E_n}} (\phi)$ for $1\leq m\leq n\leq\aleph_0$. The assumption that $\sup_k \norm{\phi_k } < \infty$ shows that the sequence $\Set{x^{(k)}}_k$ is uniformly bounded in norm. Furthermore, if we let $S = \bigcup_{1\leq m\leq n\leq\aleph_0} H_{n,m}$, where we naturally regard the $H_{n,m}$ as subspaces of $H$, then we find that $S$ is a total set in $H$ and that $x^{(k)} \mathbf{x} \xrightarrow{k\to\infty} x\mathbf{x}$ whenever $\mathbf{x}\in S$ (by the already established cyclic case). Thus, we may deduce from Lemma $\ref{lemma-0}$ that the sequence $\Set{x^{(k)}}$ converges strongly to $x$.

**(2):** Suppose $U:H_1 \to H_2$ is a unitary operator such that $U\pi_1 (f)U^* = \pi_2 (f)$ for all $f\in C(X)$. Define $\mu = \frac{1}{2}(\mu_1 + \mu_2)$, and note that both $\mu_1$ and $\mu_2$ are absolutely continuous with respect to $\mu$. Let $\phi$ be any bounded measurable function on $X$. By Lemma $\ref{lemma-9}$, we may find a sequence $\Set{f_k}_l \subset C(X)$ such that $\sup_k \norm{f_k } < \infty$ and such that $f_k (x) \to \phi(x)$ for all $x\in X\setminus N$ where $\mu(N) = 0$. Then also $\mu_i (N) = 0$ for $i=1,2$. Hence, by the assumed property of the $\tilde{\pi}_i$, we find that $\Set{\pi_i (f_k)}_k$ converges in the strong operator topology to $\tilde{\pi}_i (\phi)$. From the second intertwining nature of the unitary operator $U$, we may deduce that $U\tilde{\pi}_1 (\phi) U^* = \tilde{\pi}_2 (\phi)$ for every bounded measurable function $\phi: X\to\mathbb{C}$. In particular, we see that
...
<MathBox title='' boxType="lemma" tag="12">
...
\begin{align*}
  ...
  \leq& \frac{1}{r}\norm{\phi 1_{E_r} }_H \\
  =& \frac{1}{r}\norm{x 1_{E_r} }_H \\
  \leq& \frac{1}{r} \norm{x }_{\mathcal{L}(H)} \cdot \norm{1_{E_r} }_H
\end{align*}
$$

which clearly implies that $\mu(E_r) = 0$ for all $r > \norm{x}_{\mathcal{L}(H)}$. In other words, $\phi\in L^\infty (X,\mu)$. However, then $x$ and $\tilde{\pi}(\phi)$ are two bounded operators on $H$ that agree on the dense set $L^\infty (X,\mu)$, and hence $x = \tilde{\pi}(\phi)$ as desired.
...
In order for $\oplus_n z_n$ to be bounded, it must be the ase that the collection $\Set{\norm{z_n } = \norm{\phi_n }_{L^\infty (E_n, \mu|_{E_n})}}$ be uniformly bounded, or equivalently, that the equation $\phi = \sum_n 1_{E_n} \phi_n$ define an element of $L^\infty (X,\mu)$. In other words, we have shown that an operator belongs to $A''$ if and only if it has the form $z = \tilde{\pi}(\phi)$ for some $\phi\in L^\infty (X,\mu)$, i.e. $A'' \subset M$.
</details>
</MathBox>

<MathBox title='' boxType="lemma" tag="13">
...
\begin{equation}
\begin{gathered}
  \phi_m (i,i) \eqae \sum_{j=1}^n |\phi_m (i,j)|^2 \\
...
\begin{equation}
\begin{gathered}
  \sum_{j=1}^n \phi_m (i,j)\overline{\phi_k (l,j)} \eqae 0 \\
...
Let $(X, \norm{\cdot}_X)$ and $(Y, \norm{\cdot}_Y)$ be two normed spaces. An operator $T: X \to Y$ is linear if it is closed under addition and scalar multiplication
...
$$
  \norm{T} = \norm{T}_{X\to Y} := \sup\Set{ \frac{\norm{T_X}_Y}{\norm{X}_X} | x \in X\setminus\Set{0}}
$$
</MathBox>
...
Let $T\in\mathcal{L}(X,Y)$ be a linear operator between normed spaces $X$ and $Y$. If there exists some $M > 0$ such that $\norm{T x}_Y \leq M\norm{x}_X  < \infty$ for all $x \in X$, then $T$ is a bounded operator with operator norm $M$. The set of all linear and bounded operators $T: X \to Y$ is denoted $\mathcal{B}(X, Y)$.

If $W$ is a subspace of $X$, then $T$ is *bounded below* on $W$ if there exists $\epsilon > 0$ such that $\norm{Tx}\geq\epsilon\norm{x }$ for all $x\in W$. 
</MathBox>

<MathBox title='The space of bounded linear operators is Banach' boxType="proposition">
Let $X$ and $Y$ be normed spaces, where $X$ is also a Banach space. The space $\mathcal{B}(X,Y)$ of bounded linear operators from $X$ to $Y$ is a Banach space when equipped with the induced norm $\norm{\cdot}_{X\to Y}$.
...
We show that $A$ is bounded. For $\epsilon > 0$ there is $N\in\N$ such that $\norm{A_m - A_n }_{X\to Y} < \epsilon$ whenever $m,n \geq N$. This implies that for all $x\in X$

$$
  \norm{A_m (x) - A_n (x) }_Y \leq \norm{A_n - A_m }_{X\to Y} \cdot\norm{x }_X < \epsilon\norm{x }_X
$$

Taking the limit $m\to\infty$ and using the continuity of the norm we get

$$
  \norm{A(x) - A_n (x) }_Y \leq \epsilon\norm{x }_X
$$
...
\begin{align*}
  \norm{A(x) }_Y \leq& \norm{A(x) - A_n (x)}_Y + \norm{A_n (x) } \\
  \leq& \norm{x }_X + \norm{A_n }_{X\to Y} \cdot \norm{x }_X \\
  =& (\epsilon + \norm{A_n }_{X\to Y})\norm{X }_X
\end{align*}
$$

Since $(A_n)_{n\in\N_+}$ is Cauchy, it is bounded by Proposition $\ref{proposition-6}$. Thus, there is $M > 0$ such that $\norm{A_n }_{X\to Y} \leq M$ for all $n\in\N$. This implies that

$$
  \norm{A(x) }_Y \leq (\epsilon +  M)\norm{X }_X
$$

and also that

$$
  \norm{A }_{X\to Y} = \sup\Set{\frac{\norm{A(x) }_Y}{\norm{x }_X} | x\in X\setminus\Set{0}} \leq (\epsilon + M)
$$

Hence $A$ is bounded.

**(3):** We show that $\lim_{k\to\infty} \norm{A - A_k }_{X\to Y} = 0$. We established in **(2)** that for $\epsilon > 0$ there is $N\in\N$ such that $\norm{A(x) - A_n (x) }_Y < \epsilon\norm{x }_X$ whenever $n \leq N$.
...
$$
  \norm{A - A_n }_{X\to Y} \leq\epsilon
$$

Hence, $(A_n)_{n\in\N_+}$ converges to $A$ in the norm $\norm{\cdot}_{X\to Y}$.
</details>
</MathBox>

<MathBox title='' boxType="proposition" tag="18">
...
$$
  \norm{Tx_n - T\tilde{x}}_Y = \norm{T(x_n - \tilde{x})}_Y \leq M \norm{x_n - \tilde{x}}_X \xrightarrow{n\to\infty} 0
$$

which shows that $T$ is continuous. Conversely, suppose $T$ is continuous. Then there exists $\delta > 0$ such that $\norm{Tx}_Y < 1$ for all $x \in X$ with $\norm{x}_X < \delta$, thus

$$
  \norm{Tx}_Y = \frac{\norm{x}}{\delta} \Norm{T\left( \delta \frac{x}{\norm{x}} \right)} = \frac{\norm{x}}{\delta} \Norm{T\left(\delta \frac{x}{\norm{x}} \right)} \leq \frac{\norm{x}}{\delta} < \infty
$$
</details>
</MathBox>

<MathBox title='Norm-preserving linear extension of bounded operators' boxType="theorem">
Let $X$ and $Z$ be normed spaces, $S\subseteq Z$ a subspace of $Z$ with norm $\norm{\cdot}_Z$. If $T\in\mathcal{B}(S,Z)$ is a bounded linear operator, a norm-preserving extension of $T$ is a bounded linear operator $\overline{T}\in\mathcal{Z,X}$ such that $\overline{T}(s) = T(s)$ for all $s\in S$ and $\norm{\overline{T}}_{Z\to Y} = \norm{T }_{S\to Z}$.
</MathBox>

<MathBox title='Continuous linear extension theorem' boxType="theorem" tag="9">
...
Since $S$ is dense in $Z$, there is for each $z\in Z$ a Cauchy sequence $(s_k)_{k\in\N_+}$ in $S$ such that $\lim_{k\to\infty} \norm{z - s_k }_Z = 0$. Since $T\in\mathcal{B}(S,X)$, the operator $T$ is uniformly continuous on $S$ (Proposition $\ref{proposition-18}$). Thus, the sequence $(T(s_k))_{k\in\N_+}$ is Cauchy in $X$. This sequence converges because $X$ is a Banach space. For each $z\in Z$ we define $\overline{T}:Z\to X$ by $\overline{T}(z) = \lim_{k\to\infty} T(s_k)$. Note that for $s\in S$ we get $\overline{T}(s) = T(s)$ by the continuity of $T$.

Because we have used a sequence to define $\overline{T}$ we cannot assume that $\overline{T}$ is well-defined. For $\epsilon > 0$, there is $K_1 \in\N$ such that $\norm{\overline{T}(z) - T(s_k)}_X < \epsilon/2$ whenever $k \geq K_1$. Suppose $(s'_k)_{k\in\N_+}$ is another sequence in $S$ converging to $z$. By the uniform continuity of $T$ on $S$, for each $\epsilon < 0$ there is $\delta > 0$ such that $\norm{T(a) - T(b) }_X < \epsilon/2$ when $\norm{a - b }_S < \delta$. For this $\delta > 0$, there exists $K_2, K_3 \in\N$ such that $\norm{s_k - Z }_Z < \delta$ when $k \geq K_2$ and $\norm{s'_k - z }_Z < \delta$ when $k \geq K_3$.

Choose $K = \max\Set{K_1,K_2,K_3}$, then $\norm{s_k - z }_Z < \delta$ and $\norm{s'_k - z } < \delta$ whenever $k \geq K$. Thus, we obtain

$$
\begin{align*}
  \norm{T(s'_k) - \overline{T}(z)}_X \leq& \norm{T(s'_k) - T(s_k) } + \norm{T(s_k) - \overline{T}(z) }_X \\
...
Next, we show that $\norm{\overline{T}}_{Z\to X} = \norm{T }_{S\to X}$. For $(s_k)_{k\in\N_+}$ in $S$ converging to $z\in Z$, we have

$$
\begin{align*}
  \norm{\overline{T}(z)}_Z =& \norm{\lim_{k\to\infty} T(s_k)} \\
  =& \lim_{k\to\infty} \norm{T(s_k)} \\
  \leq& \lim_{k\to\infty} \norm{T}_{S\to X} \norm{s_k}_S \\
  =& \norm{T}_{S\to X} \norm{z}_Z 
\end{align*}
...
$$
  \norm{\overline{T}}_{Z\to X} = \sup\Set{\frac{\norm{\overline{T}(z)}_X}{\norm{z }_Z} | z\in Z\setminus\Set{0}} \leq \norm{T }_{S\to X}
$$
...
$$
\begin{align*}
  \norm{\overline{T}}_{Z\to X} =& \sup\Set{\frac{\norm{\overline{T}(z)}_X}{\norm{z }_Z} | z\in Z\setminus\Set{0}} \\
  \geq& \frac{\norm{\overline{T}(s)}_X}{\norm{z }_S} = \norm{T }_{S\to X}
\end{align*}
$$

Thus, $\norm{\overline{T}}_{Z\to X} = \norm{T }_{S\to X} < \infty$, which implies that $\overline{T}$ is bounded.
...
A map linear and bijective map $f: X \to Y$ is an isometric isomorphism if $\norm{f(x)}_Y = \norm{x}_X$ for any $x \in X$.
</MathBox>

## Uniform boundedness principle

<MathBox title='Banach-Steinhaus theorem' boxType="theorem">
Let $X$ and $Y$ be normed spaces, where $X$ is also a Banach space. For every subset $\mathcal{M} \subseteq \mathcal{B}(X, Y)$, then $\mathcal{M}$ is bounded pointwise on $X$ if and only if $\mathcal{M}$ is uniformly bounded. Pointwise boundedness implies that for all $x\in X$ there is a $C_x \geq 0$ such that $\norm{Tx}_Y \leq C_x$, while uniformly boundedness implies that for all $T \in \mathcal{M}$ there is a $C \geq 0$ such that $\norm{T}_{X \to Y} \leq C$.
</MathBox>
...
Convergence is guaranteed when $X$ is a Banach space with $\norm{T} < 1$.
</MathBox>
...
$$
  \sigma(T) := \Set{ \lambda \in \mathbb{C} | \text{$(T - \lambda I)$ not bijective}}
$$

The resolvent set of $T$ is defined by

$$
  \rho(T) := \Set{ \lambda \in \mathbb{C} | \text{$(T - \lambda I)$ bijective and $(T - \lambda I)^{-1}$ bounded}}
$$
...
$$
  \sigma_p(T) := \Set{ \lambda \in \mathbb{C} | \text{$(T - \lambda I)$ not injective}}
$$
- Continuous spectrum: 
$$
  \sigma_c(T) := \Set{ \lambda \in \mathbb{C} | \text{$(T - \lambda I)$ injective and not surjective with $\overline{\operatorname{ran}(T - \lambda I)} = X$}}
$$
- Residual spectrum: 
$$
  \sigma_r(T) := \Set{ \lambda \in \mathbb{C} | \text{$(T - \lambda I)$ injective and not surjective with  $\overline{\operatorname{ran}(T - \lambda I)} \neq X$}}
$$
</MathBox>
...
$$
  \norm{(T - \lambda)^{-1}} \geq \frac{1}{d(\lambda, \sigma(T))}
$$

<details>
<summary>Proof</summary>

The first property can be shown by choosing $\lambda_0 \in \rho(T)$ and setting $c := \norm{(T - \lambda_0)^{-1}}$ and $\epsilon := \frac{1}{c}$. Consider any $\lambda \in \mathbb{C}$ with $\left| \lambda - \lambda_0 \right| < \epsilon$, then

$$
  T - \lambda = (T - \lambda_0) - (\lambda - \lambda_0) = (T - \lambda_0) \left( I - (\lambda - \lambda_0)(T - \lambda_0)^{-1} \right) = (T - \lambda_0) (I - S)
$$

Furthermore, $\norm{S} = \norm{\lambda - \lambda_0} \cdot \norm{(T - \lambda_0)^{-1}} < \epsilon c = 1$. This implies that $I - S$ is a convergent Neumann series, and thus invertible. Since, $T - \lambda$ is product a two invertible operators, it is also invertible. This shows that $\lambda \in \rho(T)$, meaning that $\rho(T)$ is open. Thus, $\sigma(T)$ is closed.
...
$$
  |\lambda - \lambda_0 | \geq \epsilon \implies \frac{1}{| \lambda - \lambda_0 |} \leq C = \norm{(T - \lambda)^{-1}}
$$

Hence, we get

$$
  \frac{1}{d(\lambda, \sigma(T))} = \frac{1}{\inf_{\lambda \in \sigma(T)} | \lambda - \lambda_0 |} = \sup_{\lambda \in \sigma(T)} \frac{1}{| \lambda - \lambda_0 |} \leq \norm{(T - \lambda)^{-1}}
$$
</details>
</MathBox>
...
$$
\begin{align*}
  r(T) :=& \sup_{\lambda\in\sigma(T)}|\lambda| = \lim_{k\to\infty} \norm{T^k}^{1/k} = \inf_{k\in\N} \norm{T^k}^{1/k} \\
  \leq& \norm{T} < \infty
\end{align*}
$$
...
The boundedness implied by the first and last property can be proved by choosing $\lambda \in \mathbb{C}$ with $|\lambda| > \norm{T}$ and consider $\frac{T}{\lambda}$ as a Neumann series
...
$$
\begin{align*}
  f_\ell (\lambda) = \ell\left[ (T - \lambda)^{-1} \right] =& \ell\left( \sum_{k=0}^\infty (T - \lambda_0)^{-(k + 1) (\lambda - \lambda_0)^k} \right) \\
  =& \sum_{k=0}^\infty \ell \left( T^{-(k+1)} \right)\lambda^k
\end{align*}
$$
...
defines a complex measure $\mu_{\mathbf{x},\mathbf{y}}$ on $(X,\mathcal{B}_X)$. The Cauchy-Shwarz inequality implies that $\mu_{\mathbf{x},\mathbf{y}}$ has total variation norm bounded by $\norm{\mu_{\mathbf{x},\mathbf{y}}} \leq \norm{\mathbf{x}}\cdot\norm{\mathbf{y}}$.
...
$$
  \mu(E) = \sum_{n\in I} \braket{P(E),\mathbf{x}_n, \mathbf{x}_n} = \sum_{n\in I} \norm{P(E) \mathbf{x}_n }^2
$$
...
**(2):** Let $T = \int_\sigma \lambda\;\d P(\lambda)$ as in **(1)** above. Define the measure $\mu$ by $\mu(E) = \sum_{n\in N} 2^{-n} \norm{P(E)e_n }^2$, where $\Set{e_n}_{n\in N}$ is an orthonormal basis for $H$. There is an isometric *-isomorphism $L^\infty (\Sigma_0, \mu) \ni \phi\mapsto \int_{\Sigma_0} \phi\;\d P \in\mathcal{L}(H)$, where $\Sigma_0 = \sigma(T)$. Under this isomorphism, the function $f_1 (\lambda) = \lambda$ for $\lambda\in\Sigma_0$ corresponds to $T$. However, it is easy to see that $\lambda_0 \in\sigma_{L^\infty (\Sigma_0, \mu)}(f_1)$ if and only if $\mu(\Set{\lambda: |\lambda-\lambda_0| < \epsilon}) > 0$ for every $\epsilon > 0$. The negation of this latter condition is clearly equivalent to the statement that the function $\lambda\mapsto 1/(\lambda-\lambda_0)$ belongs to $L^\infty (\Sigma_0,\mu)$. Finally, $\mu(E) = 0 \iff P(E) = 0$.
</details>
</MathBox>

<MathBox title='' boxType="corollary">
A complex number $\lambda_0$ belongs to the spectrum of a normal operator $T\in\mathcal{L}(H)$ on a separable Hilbert space $H$ if and only $\lambda_0$ is an approximate eigenvalue for $T$, meaning that there exists a sequence $\Set{x_n}_{n\in\N}$ of unit vectors in $H$ such that $\norm{T\mathbf{x}_n - \lambda_0 x_n } \xrightarrow{n\to\infty} 0$.
...
On the other hand, if $T$ is normal, if $P$ is the associated spectral measure, and if $\lambda_0 \in\sigma(T)$, then $P(U) \neq 0$ for every neighbourhood $U$ of $\lambda_0$. In particular, we can find a unit vector $x_n$ belonging to the range of the projection $P(U_n)$, where $U_n = \Set{\lambda\in\mathbb{C} : |\lambda - \lambda_0| < 1/n}$. Since $|(\lambda - \lambda_0)1_{U_n} (\lambda)| < 1/n \;\forall\lambda$, we find that $\norm{(T-\lambda_0)P(U_n) }\leq 1/n$, and in particular $\norm{(T - \lambda_0)x_n } \leq 1/n \;\forall n$.
</details>
</MathBox>

<MathBox title='' boxType="proposition">
Let $T\in\mathcal{L}(H)$ be a normal operator on a separable Hilbert space $H$. Let $\Sigma = \sigma(T)$ and let $P$ be the unique spectral measure associated to $T$ and let $\mu(E) = \sum_{n\in I} 2^{-n} \norm{P(E) e_n }^2$, where $\Set{e_n}_{n\in\N}$ is any orthonormal basis for $H$. Then the assignment
...
<MathBox title='' boxType="lemma" tag="14">
...
$$
  \norm{Tx}^2 = \braket{T^* Tx, x} = \braket{(T^* T)^{1/2} x, (T^* T)^{1/2} x} = \norm{(T^* T)^{1/2} x }^2
$$
...
Finally, if $\Set{p_n}_{n\in\N}$ is any sequence of polynomial with the property that $p_n (0) = 0$ for all $n\in\N$ and such that $\Set{p_n (t)}$ converges uniformly to $\sqrt{t}$ on $\sigma(T^* T)$, it follows that $\norm{p_n (T^* T) - (T^* T)^{1/2} } \xrightarrow{n\to 0}$, and hence
...
<MathBox title='Partial isometry' boxType="proposition" tag="42">
...
$$
  \norm{Px }^2 = \braket{Px, x} = \braket{U^* Ux, x} = \norm{U x}^2
$$

This implies that $\ker(U) = \ker(U) = M^\perp$, and that $U$ is isometric on $M$ because $P$ is identity on $M$.

**(3)$\implies$(2):** Let $M = \ker(U)^\perp$. For $i=1,2$, suppose $z_i \in H$ and $x_i \in M$, $y_i \in M^\perp$ are such that $z_i = x_i + y_i$. Then
...
<MathBox title='' boxType="lemma" tag="15">
...
If $T$ is normal then the spectral radius of $T$ is equal to the operator norm, i.e. $r(T) = \norm{T}$.
</MathBox>

<MathBox title="Stone's theorem" boxType="theorem">
Let $(U_t)_{t\in\R}$ be a strongly continuous one-parameter unitary group. Then there exists a unique Hermitian operator $T$ such that

$$
  U_t = e^{-itT}
$$

The operator $T$ is an infinitesimal generator of $U_t$ in the sense that

$$
  \lim_{t\to 0} \frac{e^{itT} - I}{t} = iT
$$
</MathBox>
...
Let $\left(X ,\norm{\cdot}_X\right)$ and $\left(X ,\norm{\cdot}_X\right)$ be normed spaces. A bounded linear operator $T: X \to Y$ is *compact* if for every bounded sequence $\Set{x_n}_{n\in\N} \subset X$, the sequence $\Set{Tx_n}_{n\in\N}$ has a subsequence which converges with respect to the norm in $Y$. Equivalently, $T$ is compact if it maps the unit ball $B_1 (0) \subset X$ into a set whose closure is compact, i.e. $\overline{T\left[\mathcal{B}_1(0)\right]}$ is compact. The set of compact operators from $X$ to $Y$ is denoted $\mathcal{K}(X,Y)$.
</MathBox>

<MathBox title='' boxType="proposition" tag="39">
...
If $T$ is bounded below on $\mathcal{M}$, then it is also bounded below (by the same constant) on the closure $\overline{M}$. We can therefore assume without loss of generality that $\mathcal{M}$ is closed. If $\mathcal{M}$ contains an infinite orthonormal set, say $\Set{e_n}_{n\in\N}$, and if $T$ is bounded below by $\epsilon$ on $\mathcal{M}$, then note that $\norm{Te_n - Te_m }\geq \epsilon\sqrt{2}$ for all $n\neq m$. Then $\Set{e_n}_{n\in\N}$ would be a bounded sequence in $H$ such that $\Set{Te_n}_{n\in\N}$ would have no Cauchy subsequence, this contradicting the assumed compactness of $T$. Hence, $\mathcal{M}$ must be finite-dimensional.
...
<MathBox title='' boxType="proposition" tag="40">
...
4. There is a sequence $\Set{T_n}_{n\in\N_+} \subset\mathcal{L}(H_1, H_2)$ such that for each $n\in\N$
    - a. $\norm{T_n - T } \xrightarrow{n\to\infty} 0$
...
**(3)$\implies$(4):** Define $T_n = TP_{1/n}$ and note that $0\leq t(1 - \operatorname{id}_{1/n} (t) \leq \frac{1}{n}$ for all $t\geq 0$ and conclude that $\norm{|T|(1 - \operatorname{id}_{1/n}(|T|))} \leq\frac{1}{n}$. If $T = U|T|$ is the polar decomposition of $T$, it follows that $\norm{T - T_n }\leq\frac{1}{n}$. Finally, **(3)** clearly implies that each $P_{1/n}$ and consequently $T_n$ has finite-dimensional range.
...
**(2):** Let $\mu$ be the measure defined by $\mu(E) = \sum_{n\in\N} \norm{P(E) e_n }^2$, where $\Set{\epsilon_n}_{n\in\N}$ is some orthonormal basis for $H$. Then the measurable functional calculus yields an embedding of $L^\infty (F_\epsilon, \mathcal{B}_{F_\epsilon}, \mu|_{F_\epsilon})$ into $\mathcal{L}(\operatorname{ran}(P_\epsilon))$. It follows from **(1)** that $L^\infty (F_\epsilon,\mathcal{B}_{F_\epsilon},\mu|_{F_\epsilon})$ is finite-dimensional, where $F_\epsilon$ is as in the last paragraph. This implies that there is a finite set $\Set{\lambda_i}_{i=1}^{n\in\N} \subset F_\epsilon$ such that $\mu|_{F_\epsilon} = \sum_{i=1}^n \mu_{\Set{\lambda_i}}$. 
...
1. $\sum_{n\in N\subseteq\N} \norm{Te_n }^2 \rVert < \infty$, for some orthonormal basis $\Set{e_n}_{n\in N\subseteq\N}$ of $H_1$
2. $\sum_{m\in M\subseteq\N} \norm{T^* f_m }^2 < \infty$, for every orthonormal basis $\Set{f_m}_{m\in M\subseteq\N}$ of $H_2$
3. $\sum_{n\in N\subseteq\N} \norm{T e_n}^2 < \infty$, for some orthonormal basis $\Set{e_n}_{n\in N\subseteq\N}$ of $H_1$
...
$$
\begin{align*}
  \sum_{n\in N} \norm{Te_n }^2 =& \sum_{n\in N} \sum_{m\in M} |\braket{Te_n, f_m}|^2 \\
...
$$
  \norm{T }_2 = \left(\sum_{n\in N} \norm{Te_n }^2 \right)^{1/2}
$$
...
$$
\begin{align*}
  \norm{(T - \mu I)^{-1}} \geq& \norm{(T - \mu I)^{-1} e_j}_{\ell^p (\N)} \\
  =& \norm{(\lambda_j - \mu I)^{-1} e_j}_{\ell^p (\N)} \\
...
$$
  \norm{L }_{X\to\mathcal{B}^{n-1}(X,Y)} = \sup\Set{\frac{\norm{L(h_1,\dots,h_n) }_Y}{\prod_{i=1}^n \norm{h_i }_X} | h_i \in X_i \setminus\Set{0}}
$$
...
$$
  \norm{L }_{X\to\mathcal{B}(X,Y)} = \sup\Set{\frac{\norm{L(h_1) }_{X\to Y}}{\norm{h_1 }_X} | h_1 \in X\setminus\Set{0}}
$$

is finite, where for each $h_1 \in X$, the linear transformation $L(h_1): X \to Y$ is bounded, i.e.

$$
  \norm{L(h_1)}_{X\to Y} = \sup\Set{\frac{\norm{L(h_1)h_2 }_Y}{\norm{h_2 }_X} | h_2 \in X\setminus\Set{0}} < \infty
$$

We combine $\norm{L(h_1) }_{X\to Y} \leq \norm{L }_{X\to\mathcal{B}(X,Y)}\norm{h_1 }_X$ and $\norm{L(h_1)h_2 }_Y \leq \norm{L(h_1) }_{X\to Y} \norm{h_2 }_X$ to get

$$
  \norm{L(h_1)h_2 }_Y \leq \norm{L }_{X\to\mathcal{B}(X,Y)} \norm{h_1 }_X \norm{h_2 }_X
$$

or when both $\norm{h_1 }_X$ and $\norm{h_2 }_X$ are nonzero, that

$$
  \norm{L }_{X\to\mathcal{B}(X,Y)} \geq \frac{\norm{L(h_1)h_2 }_Y}{\norm{h_1 }_X \norm{h_2 }_X}
$$

The upper bound $\norm{L }_{X\to\mathcal{B}(X,Y)}$ on the ratios is the supremum because for $\epsilon > 0$ there exists a nonzero $h_1 \in X$ such that

$$
  \frac{\norm{L(h_1) }_{X\to Y}}{\norm{h_1 }_X} > \norm{L }_{X\to\mathcal{B}(X,Y)} - \frac{\epsilon}{2}
$$

and there exists a nonzero $h_2 \in X$ such that

$$
  \frac{\norm{L(h_1)h_2 }_Y}{\norm{h_2 }_X} > \norm{L(h_1) }_{X\to Y} - \frac{\epsilon\norm{h_1 }_X}{2}
$$
...
$$
\begin{align*}
  \frac{\norm{L(h_1) h_2 }_Y}{\norm{h_1 }_X \norm{h_2 }_X} >& \frac{\norm{L(h_1) }_{X\to Y}}{\norm{h_1 }_X} - \frac{\epsilon}{2} \\
  >& \norm{L }_{X\to\mathcal{B}(X,Y)} -\epsilon
\end{align*}
$$
...
$$
  \norm{L }_{X\to\mathcal{B}(X,Y)} = \sup\Set{\frac{\norm{L(h_1)h_2 }_Y}{\norm{h_1 }_X \norm{h_2 }_X} | h_1, h_2 \in X\setminus\Set{0}}
$$
</details>
</MathBox>
...
Let $(X, \norm{\cdot}_X)$ and $(Y, \norm{\cdot}_Y)$ be normed spaces. A function $f:X\to Y$ is *Fréchet differentiable* at $x\in X$ if there exists a bounded linear operator $T\in\mathcal{B}(X, Y)$ such that
...
$$
\begin{align*}
  \frac{\norm{T_1 v - T_2 v }_Y}{\norm{v }_X} =& \frac{|\tau|}{|\tau|}\frac{\norm{T_1 v - T_2 v}_Y}{\norm{v }_X} \\
  =& \frac{\norm{T_1 (\tau v) - T_2 (\tau v) }_Y}{\norm{\tau v }_X} \\
  =& \frac{\norm{(f(x + \tau v) - f(x) - T_2 (\tau v)) - (f(x + \tau v) - f(x) - T_1 (\tau v)) }_Y}{\norm{\tau v}_X} \\
  \tleq \frac{\norm{f(x + \tau v) - f(x) - T_2 (\tau v) }_Y}{\norm{\tau v }_X} + \frac{\norm{f(x + \tau v) - f(x) - T_1 (\tau v) }_Y}{\norm{\tau v }_X} \xrightarrow{\tau\to 0} \to 0
\end{align*}
$$
...
$$
  \norm{f(x + h) - f(x) - Th }_Y \leq \epsilon \norm{h }_X
$$
</MathBox>

<MathBox title='Fréchet differentiable functions are Lipschitz' boxType="proposition" tag="23">
...
$$
  \norm{f(x) - f(x_0) }_Y \leq L\norm{x - x_0 }_X
$$

<details>
<summary>Proof</summary>

Since $f:X\to Y$ is Fréchet differentiable at $x_0 \in X$, then for $\epsilon = 1$ there is $\delta > 0$ such that for all $x\in X$ with $\norm{x - x_0 }_X < \delta$ we have

$$
  \frac{\norm{f(x) - f(x_0) - \d f(x_0) (x - x_0) }_Y}{\norm{x - x_0 }_X} < 0
$$
...
$$
\begin{align*}
  \norm{f(x) - f(x_0) }_Y =& \norm{f(x) - f(x_0) - \d f(x_0) (x - x_0) + \d f(x_0)(x - x_0) }_Y \\
  \leq& \norm{f(x) - f(x_0) - \d f(x_0) (x - x_0) }_Y + \norm{\d f(x_0)(x - x_0) }_Y \\
  \leq& \norm{x - x_0 }_X + \norm{\d f(x_0)(x - x_0) }_Y 
\end{align*}
$$
...
$$
  \norm{\d f(x_0)(x - x_0) }_Y \leq \norm{\d f(x_0) } \cdot \norm{}_X
$$

Thus, we obtain

$$
  \norm{f(x) - f(x_0) }_Y \leq (1 + \norm{\d f(x_0)})\norm{x - x_0 }_X
$$

Taking $L = 1 + \norm{\d f(x_0) }$ shows that $f$ is locally Lipschitz at $x_0$.
</details>
</MathBox>
...
$$
\begin{align*}
  \norm{f(x + h) - f(x) - \d f(x)h }_Y \leq& \frac{\epsilon \norm{h }_X}{2(|\alpha| + 1)} \\
  \norm{g(x + h) - g(x) - \d g(x)h }_Y \leq& \frac{\epsilon \norm{h }_X}{2(|\beta| + 1)}
\end{align*}
$$
...
$$
\begin{align*}
  & \norm{\alpha f(x + h) + \beta g(x + h) - \alpha f(x) - \beta g(x) - \alpha\d f(x)h - \beta\d g(x) h }_Y \\
  &\leq |\alpha|\cdot\norm{f(x + h) - f(x) - \d f(x)h }_Y + |\beta|\cdot\norm{g(x + h) - g(x) - \d g(x)h }_Y \\
  &\leq \frac{\epsilon|\alpha|\cdot\norm{h }_X}{2(|\alpha| + 1)} + \frac{\epsilon|\beta|\cdot\norm{h }_X}{2(|\beta| + 1)} = \epsilon \norm{h }_X \left(\frac{|\alpha|}{2(|\alpha| + 1)} + \frac{|\beta|}{2(|\beta| + 1)} \right) \\
  < & \epsilon \norm{h }_X
\end{align*}
$$
...
By differentiability of $f$ and $g$ at $x$, for each $\epsilon > 0$ there is $\delta_x > 0$ with $B(x,\delta_x)\subset X$, and a constant $L > 0$ (by Proposition $\ref{proposition-23}$) such that for all $0 < \norm{h} < \delta_x$
...
$$
\begin{align*}
  |f(x + h) - f(x) - \d f(x)h| \leq& \frac{\epsilon \norm{h }_X}{3(|g(x)| + 1)} \\
  |g(x + h) - f(x) - \d f(x)h| \leq& \frac{\epsilon \norm{h }_X}{3(|f(x)| + L)}
\end{align*}
$$
...
$$
  \delta = \min\Set{1, \delta_x, \frac{\epsilon}{3L(\norm{\d g(x) } + 1)}}
$$
...
\begin{align*}
  ...
  &+ |f(x + h) - f(x)|\cdot\norm{\d g(x) }\cdot\norm{h }_X \\
  \leq& (|f(x)| + L)\frac{\epsilon\norm{h }_X}{3(|f(x) + L|)} \\
  &+ |g(x)|\frac{\epsilon\norm{h }_X}{3(|g(x)| + 1)} + \delta L \norm{\d g(x) }\cdot\norm{h }_X \\
  <& \epsilon \norm{h }_X
\end{align*}
$$
...
$$
  |f(x + h) - f(x)| \leq L\norm{h }_X \implies |f(x + h)| \leq |f(x)| + L\norm{h }_X
$$
...
<MathBox title='Bilinear Fréchet product rule' boxType="proposition" tag="43">
...
Since $|L(h_1,h_2)| \leq \norm{L }\cdot|h_1|\cdot|h_2| = o(|h|^2)$ we get
...
Choose $\epsilon > 0$. By differentiability of $f:X\to Y$ at $x\in X$ and of $g:Y\to Z$ at $y = f(x) \in Y$, there is $\delta_1 > 0$ such that $B(x,\delta_1)\subset X$ and for all $\xi \in X$ satisfying $0 \norm{\xi}_X < \delta_1$ we have

$$
  \norm{f(x + \xi) - f(x) - \d f(x)\xi }_Y \leq \frac{\epsilon\norm{\xi}_X}{2(\norm{\d g(y) } + 1)}
$$

By Proposition $\ref{proposition-23}$, $f$ is locally Lipschitz at $x$, i.e. there is $\delta_2 > 0$ and $L > 0$ such that $B(x,\delta_2)\subset X$ and for all $\xi\in X$ satisfying $0 < \norm{\xi}_X < \delta_2$ we have

$$
  \norm{f(x + \xi) - f(x) }_X \leq L\norm{\xi}_X
$$

Set $\delta_x = \min\Set{\delta_1, \delta_2}$. Since $g$ is differentiable at $y$, there is $\delta_y > 0$ such that $B(y, \delta_y) \subset Y$ and for all $0 \leq \norm{\eta}_Y < \delta_y$ we have

$$
  \norm{g(y + \eta) - g(y) -\d g(y)\eta }_Z \leq \frac{\epsilon\norm{\eta}_Y}{2L}
$$
...
Thus for all $\xi\in X$ satisfying $\norm{\xi}_X < \delta$ we have

$$
  \norm{}_Y = \norm{f(x + \xi) - f(x) }_Y \leq L\norm{\xi}_X < L\delta \leq \delta_y
$$
...
$$
\begin{align*}
  &\norm{h(x + \xi) - h(x) - \d g(y) \d f(x)\xi }_Z \\
  =& \norm{g(y + \eta(\xi)) - g(y) - \d g(y)\eta(\xi) + \d g(y) \eta(\xi) - \d g(y) \d f(x)\xi }_Z \\
  \tleq& \norm{g(y + \eta(\xi)) - g(y) - \d g(y)\eta(\xi) }_Z \\
  &+ \norm{\d g(y)\eta(\xi) - \d g(y) \d f(x)\xi }_Z \\
  \leq& \norm{g(y + \eta(\xi)) - g(y) - \d g(y)\eta(\xi) }_Z \\
  &+ \norm{\d g(y) } \cdot \norm{\eta(\xi) - \d f(x)\xi }_Y \\
  =& \norm{g(y + \eta(\xi)) - g(y) - \d g(y)\eta(\xi) }_Z \\
  &+ \norm{\d g(y) } \cdot \norm{f(x + \xi) - f(x) - \d f(x)\xi }_Y \\
  \leq& \frac{\epsilon\norm{\eta(\xi)}_Y}{2L} + \norm{\d g(y) } \frac{\epsilon\norm{\xi}_X}{2(\norm{\d g(y)} + 1)} \\
  <& \frac{\epsilon L\norm{\xi}_X}{2 L} + \frac{\epsilon\norm{\xi}_X}{2} = \epsilon\norm{\xi}_X
\end{align*}
$$
...
Fix $\tilde{M} > M\in\R$ and consider $d(\tau) := \norm{f(\tau) - f(s)}_X - \tilde{M}(\tau - s)$ for $\tau\in[s,t]$. Suppose $\tau_0$ is the largest $\tau$ for which $d(\tau) \leq 0$ holds. Then there must be a sequence $\epsilon_n \downarrow 0$ such that

$$
\begin{align*}
  0 \leq& d(\tau_0 + \epsilon_n) \\
  \leq& \norm{f(\tau_0 + \epsilon_n) - f(\tau_0) } - \tilde{M}\epsilon_n + d(\tau_0) \\
  =& \norm{\d f(\tau_0)\epsilon_n + o(\epsilon_n) } - \tilde{M}\epsilon_n \\
...
<MathBox title='Matrix representation of Fréchet derivatives' boxType="proposition" tag="33">
...
$$
\begin{align*}
  0 =& \lim_{\mathbf{h}\to\mathbf{0}} \frac{\norm{f(\mathbf{x} + \mathbf{h}) - f(\mathbf{x}) - \d f(\mathbf{x}\mathbf{h}) }}{\norm{\mathbf{h} }} \\
  =& \lim_{r\to 0} \frac{\norm{f(\mathbf{x} + r\mathbf{e}_j) - f(\mathbf{x}) - r\d f(\mathbf{x}) \mathbf{e}_j }}{|r|\cdot\norm{\mathbf{e}_j}} \\
  =& \lim_{r\to 0} \frac{\norm{f(x_1,\dots,x_j + r,\dots,x_n) - f(x_1,\dots,x_n) - rJ_j }}{|r|}
\end{align*}
$$
...
$$
\begin{align*}
  \lim_{t\to 0} \frac{\d f(x)(tv) + o(\norm{tv })}{t} =& \lim_{t\to 0} \frac{t \d f(x)(v) + o(\norm{tv })}{t} \\
  =& \d f(x)(v) + \lim_{t\to 0} \frac{o(|t|\cdot\norm{v })}{t} \\
  =& \d f(x)(v)
\end{align*}
$$
...
\begin{align*}
  ...
  \leq& \left(\int_0^1 \norm{\D f(x + th) - \D f(x) }\;\d t \right)|h| \\
  \leq& \max_{t\in[0,1]} \norm{\D f(x + th) - \D f(x) }\cdot|h|
\end{align*}
$$
...
$$
\begin{gather*}
  \norm{f(x) - f(y) }_Y \leq M \norm{x - y }_X \\
  M := \sup{t\in[0,1]} \Set{\D f((1 - t)x + ty), \frac{x - y}{\norm{x - y }_X}}
\end{gather*}
$$
...
Define $g(t) = f((1 - t)x + ty)$ with $\D f(t) = \delta g(t) = \delta f((1 - t)x + ty;y - x)$ for $t\in[0,1]$. This implies $\norm{\d g(t) } \leq \tilde{M} := M\norm{x - y }$ by homogeneity of the Gâteaux derivative.  
</details>
</MathBox>
...
\begin{align*}
  ...
  &\leq \frac{|h|^k}{(k - 1)!} \int_0^1 (1 - t)^{k-1} \norm{\d^k f(x + th) - d^k f(x) }\;\d t \\
  &\leq \frac{|h|^k}{k!} \sup_{t\in[0,1]} \norm{\d^k f(x + th) - d^k f(x) }
\end{align*}
$$
</details>
</MathBox>
...
If $\norm{\d^k (fx + th) } < M$ for all $t\in[0,1]$, then

$$
  \norm{R_k }_Y \leq \frac{M}{k!} \norm{h }_X^k
$$
</MathBox>
...
<MathBox title='' boxType="proposition" tag="52">
Let $X$ be a Banach space. The set $S([a,b], X)$ is a subspace of $(L^\infty ([a,b], X), \norm{\cdot}_\infty)$.
...
$$
  \norm{f }_\infty = \sup\Set{\norm{x_i }}_{i=1}^N < \infty
$$
...
<MathBox title='Integral of step functions is a bounded linear operator' boxType="proposition" tag="53">
Let $X$ be a Banach space. The integral of step functions $I:S([a,b],X)\to X$ is a bounded linear transformation with $\norm{I } = b - a$.
...
$$
\begin{align*}
  \norm{I(f) } =& \norm{\sum_{i=1}^N x_i (t_i - t_{i-1}) } \\
  \leq& \sum_{i=1}^N (t_i - t_{i-1}) \norm{x_i } \\
  \leq& \sum_{i=1}^N (t_i - t_{i-1}) \sup\Set{\norm{x_i }}_{i=1}^N \\
  =& (b - a)\norm{f }_\infty
\end{align*}
$$
...
$$
  \frac{\norm{I(f) }}{\norm{f }_\infty} \leq b - a
$$

Thus, $\norm{I } \leq b - a$. The upper bound on $\norm{I }$ is realized for the function $f = \chi_[a,b] \in S([a,b], X)$ and so $\norm{I } = b - a$.
</details>
</MathBox>
...
Any $f\in C([a,b], X)$ is uniformly continuous because $[a,b]$ is compact. For $\epsilon > 0$ there is $\delta > 0$ such that $\norm{f(x) - f(y) } < \epsilon$ whenever $x,y \in [a,b]$ satisfy $|x - y| < \delta$. Choose $N\in\N$ such that $(b-a)/N < \delta$. Set $t_i = a + i(b - a)/N$ for $i=0,\dots,N$. Define the step funcion $f_N \in S([a,b],X)$ by

$$
  f_N = \left(\sum_{i=1}^{N} f(t_{i-1})\chi_{[t_{i-1}, t_i)}\right) + f(t_{N-1})\chi_{[t_{N-1},t_N]}
$$

Since any $t\in [t_{i-1}, t_i)$ for $i=1,\dots,N-1$ and $t\in[t_{N-1}, t_N]$ is within $\delta$ of the endpoints, we have $\norm{f(t) - f_N (t)} < \epsilon$. As this holds for all $t\in[a,b]$, we obtain $\norm{f - f_N} < \epsilon$. Since $\epsilon > 0$ is arbitrary, we have that $f$ is a limit point of $S([a,b], X)$. This shows that $C([a,b],X)\subset\overline{S([a,b],X)}$.
</details>
</MathBox>

<MathBox title='Properties of single-variable Banach-valued integrals' boxType="proposition" tag="37">
Let $X$ be a Banach space. If $f\in\overline{S([a,b],X)}$ and $\alpha,\beta,\gamma\in[a,b]$ with $\alpha < \gamma < \beta$, then
1. $\norm{\int_a^b f(t)\;\d t} \leq (b-a)\norm{f}_\infty$
2. $\norm{\int_a^b f(t)\;\d t} \leq \int_a^b \norm{f }\;\d t$
...
**(1):** This is a restatement of $\norm{\overline{I}} = b-a$ in Proposition $\ref{proposition-53}$.

**(2):** The function $t\mapsto \norm{f(t) }$ belongs to $\overline{S([a,b],X)}$, and so $\int_a^b \norm{f(t) }\;\d t$ is defined.
...
<MathBox title='Constant functions have zero derivative' boxType="proposition" tag="29">
...
\begin{align*}
  \norm{f(t + h) - f(t) }_X =& \norm{f(t + h) - f(t) - 0 }_X \\
  =& \norm{f(t + h) - f(t) - \d f(t)h }_X \leq \epsilon |h|
\end{align*}
$$

The collection of open intervals $(t - \delta_t, t + \delta_t)$ is an open covering of the compact $[a,b]$ so there is a finite subcovering $(t_i - \delta_{t_i}, t_i + \delta_{t_i})$ of $[a,b]$ for $i = 1,\dots,n$. Without loss of generality we can assume that $a < t_1 < \cdots < t_n < b$. Choose points $x_0,\dots,x_n \in [a,b]$ so that
...
$$
\begin{align*}
  \norm{f(b) - f(a) }_X =& \norm{\sum_{i=1}^n f(x_i) - f(x_{i-1}) }_X \\
  =& \norm{\sum_{i=1}^n (f(x_i) - f(t_i)) + (f(t_i) - f(x_{i-1})) }_X \\
  \tleq& \sum_{i=1}^n \norm{f(x_i) - f(t_i) }_X + \sum_{i=1}^n \norm{f(t_i) - f(x_{i-1}) }_X \\
...
$$
\begin{align*}
  &\norm{\int_a^{t+h} f(x)\;\d s - \int_a^t f(s)\;\d s - f(t)h }_X \\
  &= \norm{\int_t^{t+h} f(s)\;\d s - f(t)h }_X \\
  &= \norm{\int_t^{t+h} (f(s) - f(t))\;\d s }_X \\
  &\leq \left| \int_t^{t+h} \norm{f(s) - f(t) }_X \;\d s \right| \\
...
$$
  \norm{f(x_1) - f(x_0) }_Y \leq \sup_{c\in\ell(x_0,x_1)} \norm{\d f(c) }\cdot\norm{x_1 - x_0 }
$$
</MathBox>
...
1. is Cauchy in $C(X, Y)$ if for every compact $K\subset U$, the sequence $(f_n |_K )_{n\in\N_+}$ is Cauchy in $(C(K,Y),\norm{\cdot}_\infty)$ and
2. converges uniformly on compact subsets of $X$ to $f\in C(X,Y)$ if for every compact $K\subset U$, the sequence $(f_n |_K)_{n\in\N_+}$ converges to $f|_K$ in $(C(K,Y),\norm{\cdot}_\infty)$.

Note that a Cauchy sequence $(g_n)_{n\in\N_+}$ in $(C(K,Y),\norm{\cdot}_\infty)$ converges uniformly to a unique $g\in C(K,Y)$ because $(C(K,Y), \norm{\cdot}_\infty)$ is a Banach space. 
</MathBox>
...
$$
  f(x_*) = z + \int_0^1 g(x_* + t0)0\;\d t = z = \lim_{n\to\infty} f_n (x_*)
$$
...
\begin{align*}
  &\norm{f_n (x) - f(x)}_Y \\
  =& \norm{f_n (x_*) + \int_0^1 \d f_n (x_* + th)h\;\d - z - \int_0^1 g(x_* + th)h\;\d t}_Y \\
  \leq& \norm{f_n (x_*) - z}_Y + \norm{\int_0^1 (\d f_n (x_* + th) - g(x_* + th))h\;\d t}_Y \\
  \leq& \norm{f_n (x_*) - z}_Y + \sup_{c\in K} \norm{\d f_n (c) - g(c)}_{X\to Y} \norm{h}_X
\end{align*}
$$
...
$$
  \norm{f_n (x_*) - z }_Y < \frac{\epsilon}{2}
$$

and since $\d f_n \xrightarrow{n\to\infty} g$ uniformly on the compact $K$, there is $M \geq N$ such that for $n\geq M$ we have

$$
  \sup_{c\in K} \norm{\d f_n (c) - g(c) }_Y < \frac{\epsilon}{2r}
$$

Thus, for all a$n\geq M$, and with $\norm{h }_X$, we have

$$
  \norm{f_n (x) - f(x) }_Y < \frac{\epsilon}{2} + \frac{\epsilon}{2r}r = \epsilon
$$

The choice of $N$ and $M$ is independent of the point $x\in K$ chosen, so for all $n\geq N$ we have

$$
  \norm{f_n - f}_\infty = \sup_{x\in K} \norm{f_n (x) - f(x) }_Y \leq \epsilon
$$

Thus, $f_n \xrightarrow{n\to\infty} f$ uniformly on the compact $K$. 

Finally, we show that $\d f(x) = g(x)$ for all $x\in U$. For a fixed $x\in B(x_*, r) = U$, there is $a > 0$ such that $K = \overline{B(x,a)}\subset U$. For $\norm{h }_X \leq a$ we have by the integral mean value theorem that
...
$$
\begin{align*}
  \norm{f(x + h) - f(x) - g(x)h } =& \norm{\int_0^1 (g(x + th)h - g(x)h)\;\d t }_Y \\
  =& \norm{\int_0^1 (g(x + h) - g(x))h\;\d t }_Y
\end{align*}
$$
...
$$
  \norm{g(x + h) - g(x)}_{X\to Y} < \epsilon
$$

We thus arrive at

$$
  \norm{f(x + h) - f(x) - g(x)h }_Y \leq \epsilon\norm{h }_X
$$
