...
1. $\| x \| = 0 \iff x = 0$ (positive definitive)
2. $\| \lambda x \| = |\lambda| \| x \|, \quad \lambda \in \mathbb{F}$ (absolutely homogenous)
3. $\| x + y \| \leq \| x \| + \| y \|$ (triangle inequality)

The pair $(X, \|\cdot\|)$ is called a normed space. The norm $\| \cdot \|$ induces a metric by
...
The norm $\| \cdot \|$ is a continuous map. Considering the sequence $\left( x_n \right)_{n\in\N} \subseteq X$ with limit $\tilde{x} \in X$, then by the triangle inequality

$$
\begin{gather*}
  \| x_n \| = \| \left(x_n - \tilde{x}\right) + \tilde{x} \| \leq \| x_n - \tilde{x} \| + \| \tilde{x} \| = d \left(x_n, \tilde{x} \right) + \| \tilde{x} \| \\
  \implies \lim_{n\to\infty} \| x_n \| \leq \| \tilde{x} \|
\end{gather*}
$$
...
$$
\begin{gather*}
  \| \tilde{x} \| = \| \left(\tilde{x} - x_n \right) + x_n \| \leq \| \tilde{x} - x_n \| + \| x_n \| = d(\tilde{x}, x_n) +  \| x_n \| \\
  \implies \| \tilde{x} \| \leq \lim_{n\to\infty}  \| x_n \|
\end{gather*}
$$

<MathBox title='Banach space' boxType='definition'>
A normed space $(X,\|\cdot\|)$ is called a Banach space if $\left(X, d_{\| \cdot \|} \right)$ is a complete metric space. A Banach space is a complete normed space.
</MathBox>

## Linear operators

<MathBox title='Linear operator' boxType='definition'>
Let $(X, \| \cdot \|_X)$ and $(Y, \| \cdot \|_Y)$ be two normed spaces. An operator $T: X \to Y$ is linear if it is closed under addition and scalar multiplication
...
$$
  \| T \| = \| T \|_{X\to Y} := \sup\Set{ \left. \frac{\| T_X \|_Y}{\| X \|_X} \right| x \neq 0 \in X }
$$
</MathBox>
...
Let $T$ be a linear operator. If there exists some $M > 0$ such that $\| T x \|_Y \leq M\| x \|_X  < \infty$ for all $x \in X$, then $T$ is a bounded operator with operator norm $M$. The set of all linear and bounded operators $T: X \to Y$ is denoted $\mathcal{B}(X, Y)$.
</MathBox>
...
$$
  \| Tx_n - T\tilde{x} \|_Y = \| T(x_n - \tilde{x}) \|_Y \leq M \| x_n - \tilde{x} \|_X \xrightarrow{n\to\infty} 0
$$

which shows that $T$ is continuous. Conversly, suppose that $T$ is continuous. Then there exists $\delta > 0$ such that $\| Tx \|_Y < 1$ for all $x \in X$ with $\| x \|_X < \delta$, thus

$$
  \| Tx \|_Y = \left\| \frac{\| x \|}{\delta} T\left( \delta \frac{x}{\| x \|} \right) \right\| = \frac{\| x \|}{\delta} \left\| T\left( \delta \frac{x}{\| x \|} \right) \right\| \leq \frac{\| x \|}{\delta} < \infty
$$
</details>
</MathBox>
...
A map linear and bijective map $f: X \to Y$ is an isometric isomorphism if $\| f(x) \|_Y = \| x \|_X$ for any $x \in X$.
</MathBox>

### Uniform boundedness principle

<MathBox title='Banach-Steinhaus theorem' boxType='theorem'>
Let $X$ and $Y$ be normed spaces, where $X$ is also a Banach space. For every subset $\mathcal{M} \subseteq \mathcal{B}(X, Y)$, then $\mathcal{M}$ is bounded pointwise on $X$ if and only if $\mathcal{M}$ is uniformly bounded. Pointwise boundedness implies that for all $x\in X$ there is a $C_x \geq 0$ such that $\| Tx \|_Y \leq C_x$, while uniformly boundedness implies that for all $T \in \mathcal{M}$ there is a $C \geq 0$ such that $\| T \|_{X \to Y} \leq C$.
</MathBox>
...
Convergence is guaranteed when $X$ is a Banach space with $\| T \| < 1$.
</MathBox>
...
Let $X'$ be the dual space of a normed space $X$. The dual space itself is a normed space $\left(X', \| \cdot \|_{X \to \mathbb{F}}\right)$, which is also a Banach space.
...
$$
  \forall \epsilon > 0 \quad \exists N \in \N \quad \forall n, m \geq N : \| \ell_n - \ell_m \|_{X\to\mathbb{F}} < \epsilon
$$

Since the operator norm $\| \cdot \|_{X\to\mathbb{F}}$ is defined by a supremum, we know that for all $x \in X$ with $x\neq 0$

$$
\begin{gather*}
  \frac{1}{\| x \|_X} \left| \ell_n - \ell_m \right| \leq \| \ell_n - \ell_m \|_{X\to\mathbb{F}} < \epsilon \\
  \left| \ell_n - \ell_m \right| < \epsilon \| x \|_X
\end{gather*}
$$
...
We then have to show that $\ell: X \to \mathbb{F}$ is linear and bounded and that $\| \ell_k - \ell \|_{X \to \mathbb{F}} \xrightarrow{k\to\infty} 0$. Boundedness follows from the Cauchy property of $\left( \ell_k \right)_{k\in\N}$:

$$
\begin{gather*}
  \| \ell_n \|_{X\to\mathbb{F}} \overset{\triangle}{\leq} \underbrace{\| \ell_n - \ell_N \|_{X\to\mathbb{F}}}_{< \epsilon} + \underbrace{\| \ell_N \|_{X\to\mathbb{F}}}_{:= C} \leq \epsilon + C \quad \forall n\geq N \\
  \implies \left| \ell(x) \right| = \left| \lim_{k\to\infty} \ell_k (x) \right| = \lim_{k\to\infty} \left| \ell_k (x) \right| \leq \lim_{k\to\infty} \underbrace{\| \ell_{k} \|_{X\to\mathbb{F}}}_{\leq \tilde{C}} \| x \|_X \\
  \implies \| \ell \|_{X\to\mathbb{F}} = \frac{1}{\| x \|_X} \left| \ell(x) \right| \leq \tilde{C} < \infty
\end{gather*}
$$

To show the convergence of the operator norm $\| \cdot \|_{X\to\mathbb{F}}$, we have the for $\epsilon > 0$ there is a $N \in \N$ such that for all $n, m \geq N$

$$
\begin{gather*}
  \frac{1}{\| x \|_X} \left| \ell_n (x) - \ell_m (x) \right| < \epsilon \\
  \implies \sup_{\overset{x\in X}{x\neq 0}} \frac{1}{\| x \|_X}  \left| \ell_n (x) - \lim_{m\to\infty} \ell_m (x) \right| = \sup_{\overset{x\in X}{x\neq 0}} \frac{1}{\| x \|_X}  \left| \ell_n (x) - \ell (x) \right| \leq \epsilon \\
  \implies \| \ell_n - \ell \|_{X\to\mathbb{F}} \leq \epsilon
\end{gather*}
$$
</details>
</MathBox>
...
Let $\left(X, \| \cdot \|_X \right)$ be a normed space, $U \subset X$ a subspace and $\ell \in U^*$ a continuous linear functional on $U$. Then there exists a linear functional $\tilde{\ell} \in X^*$ that extends $\ell$ with $\tilde{\ell}(u) = \ell(u)$ for all $u \in U$ and $\| \tilde{\ell} \|_{X^*} = \| \ell \|_{U^*}$.
</MathBox>

## Arzelà-Ascoli theorem

<MathBox title='Arzelà-Ascoli theorem' boxType='theorem'>
Consider the Banach space $\left(C([a, b]), \| \cdot \|_\infty \right)$ of continuous functions on an interval $I = [a, b] \in \R$ with the supremum norm $\| f \|_\infty := \sup \Set{ |f(t)| | t\in[a, b] }$. A subset $F \in C([a, b])$ is compact if and only if $F$ is closed, bounded and uniformly equicontinuous.
</MathBox>
...
$$
  \| f \|_\infty := \inf\Set{b\in[0,\infty] : |f|\overset{\textrm{a.e.}}{\leq} b }
$$
...
For $p\in[1,\infty]$, let $\mathcal{L}^p = \Set{[f] : f\in L^p }$ and define $\|[f]\|_p = \| f \|_p$ for $f\in\mathcal{M}(X,\R)$ where $\mathcal{M}(X,\R)$ is the collection of measurable functions $f:X\to\R$. Then $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ and $\|\cdot\|$ is a norm on $\mathcal{L}^p$ that satisfies for $f,g\in L^p$ and $c\in\mathcal{R}$
1. $\| f\|_p \geq 0$ and $\| f\|_p = 0$ if and only if $f\equiv 0$ (positive definite)
2. $\| cf\|_p = |c|\cdot\| f\|_p$ (absolutely homogenous)
3. $\| f+g\|_p \leq \| f\|_p + \| p\|_p$ (triangle inequality)
...
1. The predicate $\| f\|_p \geq 0$ follows from the definitions. For the second part, we first consider the case $p\in(0,\infty)$. Trivially, $\| 0\|_p = 0$. Conversely, if $\| f\|_p = 0$ then $\int_X \| f\|^p\;\mathrm{d}\mu = 0$ and hence $|f|^p \overset{\textrm{a.e.}}{=} 0$ and $f \overset{\textrm{a.e.}}{=} 0$. For the case $p = \infty$, it follows trivially that $\| 0\|_\infty = 0$. Conversely, suppose that $\| f\|_\infty = 0$. Then for each $n\in\N$ there exists $b_n\in[0,\infty)$ with $b_n \xrightarrow{n\to\infty}0$ and $|f\overset{\textrm{a.e.}\leq b_n$, resulting in $f\overset{\textrm{a.e.}}{=}0$.
2. For $p\in(0,\infty)$ we have

$$
\begin{gather*}
  \| cf\|^p \int_X |cf|^p\;\mathrm{d}\mu = |c|^p \int_X |f|^p\;\mathrm{d}\mu = |c|^p\cdot\| f\|^p \\
  \iff \| cf\| = |c|\cdot\| f\|_p
\end{gather*}
$$
...
$$
  \| fg\|_1 = \int_X |fg|\;\mathrm{d}\mu\leq\| f \|_p \cdot \| g \|_q
$$

If $(X,\mathcal{P}(X),\#)$ is a discrete measure, Hölder's inequality reduces to

$$
  \| xy \|_1 = \left(\sum_{i\in X} |x_i + y_i| \right) \leq \| x \|_p \cdot \| y \|_q
$$

<details>
<summary>Proof</summary>

The result follows trivially if $\| f\|_p = \infty$ or $\| g\|_\infty = \infty$, so assume that $f\in L^p$ and $g\in L^q$. For the case $p = 1$ and $q = \infty$, note that $|g|\overset{\mu\textrm{-a.e.}{\leq}}\| g\|_\infty$, hence

$$
  \int_X |fg|\;\mathrm{d}\mu = \int_X |f|\cdot|g|\;\mathrm{d}\mu \leq\| g\|_\infty \int_X |f|\;\mathrm{d}\mu = \| f\|_1 \cdot \| g\|_\infty
$$

Consider the case where $p,q\in(1,\infty)$. By positive definiteness, the result holds if $\| f\|_p = 0$ or $\| g\|_q = 0$, so assume that $\| f\|_p\geq 0$ and $\| g\|_q\geq 0$. By Young's inequality

$$
  |fg|\leq\frac{1}{p}|f|^p + \frac{1}{q}|g|^q
$$

Suppose first that $\| f\|_p = \| g\|_q = 1$. From the increasing monotonicity and linearity properties of the integral

$$
  \int_X |fg|\;\mathrm{d}\mu\leq \frac{1}{p}\int_X |f|^p \;\mathrm{d}\mu + \frac{1}{q}\int_X |g|^q \;\mathrm{d}\mu = \frac{1}{p} + \frac{1}{q} = 1
$$

For the general case where $\| f\|_p > 0$ and $\| g\|_q > 0$, let $f_1 = \frac{f}{\| f\|_p}$ and $g_1 = \frac{g}{\| g\|_q}$. Then $\| f_1\|_p = \| g_1\|_q = 1$ and $\| f_1g_1\|_q\leq 1$. By the scaling property of the norm
...
$$
  \| f + g\|_p = \left(\int_X |f + g|^p \;\mathrm{d}\mu\right)^{1/p} \leq \| f \|_p + \| g \|_p
$$

If $(X,\mathcal{P}(X),\#)$ is a discrete measure, Minkowski's inequality reduces to

$$
  \| x + y \|_p = \left(\sum_{i\in X} |x_i + y_i|^p\right)^{1/p} \leq \| x \|_p + \| y \|_p \quad
$$

<details>
<summary>Proof</summary>

The result is trivial if $\| f\|_p = \infty$ or $\| g\|_\infty = \infty$, so assume that $f,g\in L^p$. When $p = 1$, the result is the triangle inequality

$$
\begin{align*}
  \| f + g\|_p &= \int_X |f + g|\;\mathrm{d}\mu \leq\int_X (|f| + |g|)\;\mathrm{d}\mu \\
  & = \int_X |f|\;\mathrm{d}\mu + \int_X |g|\;\mathrm{d}\mu = \| f\|_1 + \| g\|_1
\end{align*}
$$

For the case $p = \infty$, note that if $a,b\in[0,\infty]$ are an essential bounds for $f$ and $g$, respectively, then $a+b$ is an essential bound for $f + g$. Hence $\| f + g\Vert_\infty \leq\| f\|_\infty + \| g\|_\infty$.
...
$$
  \| f + g\|_p^p \leq \int_X |f+g|^{p-1}|f|\;\mathrm{d}\mu + \int_X |f+g|^{p-1}|g|\;\mathrm{d}\mu
$$
...
$$
\begin{align*}
  \int_X |f+g|^{p-1}|f|\;\mathrm{d}\mu &\leq \left\| |f+g|^{p-1} \right\|_q \cdot \| f\|_p \\
  \int_X |f+g|^{p-1}|g|\;\mathrm{d}\mu &\leq \left\| |f+g|^{p-1} \right\|_q \cdot \| g\|_p
\end{align*}
$$
...
$$
  \| f + g\|_p^p \leq \left\| |f+g|^{p-1} \right\|_q (\| f\|_p + \| g\|_p)
$$
...
$$
\begin{align*}
  \left\| |f+g|^{p-1} \right\|_q &= \left(\int_X |f+g|^{p-1}q \;\mathrm{d}\mu \right)^{1/q} \\
  &= \left(\int_X |f+g|^p \;\mathrm{d}\mu \right)^{(p-1)/p} = \| f+g \|_p^{p-1}
\end{align*}
$$
...
$$
\begin{gather*}
  \| f+g \|_p^p = \| f+g \|_p^{p-1}\left(\| f\|_p + \| g\|_p \right) \\
  \iff \| f + g\|_p \leq \| f \|_p + \| g \|_p
\end{gather*}
$$
...
$$
  \| x + y \|_1 = \sum_{j=1}^\infty \left| x_j + y_j \right| \leq \sum_{j=1}^\infty \left| x_j \right| + \left| y_j \right| = \| x \|_1 + \| y \|_1
$$
...
$$
\begin{align*}
  \| x + y \|_p^p &= \sum_{j=1}^\infty \left| x_j + y_j \right|^p = \lim_{n\to\infty} \sum_{j=1}^n \left| x_j + y_j \right|^p \\
...
Applying Hölder's inequality $\| a b \|_1 + \| c b \|_1 \leq \| a \|_p \cdot \| b \|_{q} + \| c \|_p \cdot \| b \|_{q}$ with

$$
  \| b \|_{q} = \left( \sum_{j=1}^n \left| \left( \left| x_j \right| + \left| y_j \right| \right)^{p-1} \right|^{q} \right)^{\frac{1}{q}} = \left( \sum_{j=1}^n\left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{q}}
$$
...
$$
\begin{align*}
  \sum_{j=1}^n \left(\left| x_j \right| + \left| y_j \right| \right)^p &\leq \| a \|_p \cdot \| b \|_{q} + \| c \|_p \cdot \| b \|_{q} \\
  &= \left( \| a \|_p + \| c \|_p \right) \left( \sum_{j=1}^n \left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{q}} \\
  ...
  &\leq \| a \|_p + \| c \|_p = \left( \sum_{j=1}^n \left| x_j \right|^p \right)^{\frac{1}{p}} + \left( \sum_{j=1}^n \left| y_j \right|^p \right)^{\frac{1}{p}}
\end{align*}
$$
...
$$
  \| x \|_p := \left( \sum_{i\in X} |x_i|^p \right)^(1/p)
$$

and for $p=\infty$ we define $\| x\|_\infty = \sup\Set{x_i | i\in X}$. The Lebesgue space takes the form

$$
  L^p(X,\mathcal{P}(X),\#) = \Set{ (x_i)_{i\in X} \in\R^n : \| p \|_p < \infty}
$$
...
The normed space $\left( \ell^p, \| \cdot \|_p \right)$ is a Banach space.

<details>
<summary>Proof</summary> 

To show the completeness of $\left( \ell^p, \| \cdot \|_p \right)$ we consider a Cauchy sequence $\left( x^{(k)} \right)_{k \in \N}$ in $\ell^p$. 
...
$$
  \left\lvert x_m^{(k)} - x_m^{(l)} \right\rvert^p \leq \sum_{n=1}^\infty \left\lvert x_n^{(k)} - x_n^{(l)} \right\rvert^p = \| x^{k} - x^{l} \|_p^p
$$
...
$$
  \lvert (Tx)(y) \rvert \leq \lim_{n\to\infty} \sum_{j=1}^n \left| y_j x_j \right| \leq \| y \|_p \cdot \| x \|_q < \infty
$$
...
$$
\begin{gather*}
  \| Tx \|_{\ell^p (\N) \to \mathbb{F}} = \sup\Set{ \left| (Tx)(y) \right| | \| y \|_p = 1 } \leq \sup\Set{\| y \|_p \cdot \| x \|_q | \| y \|_p = 1} \leq \| x \|_q \\
  \implies \| T \| = \frac{\| Tx \|_{\ell^p (\N)}}{\| x \|_q} \leq 1
\end{gather*}
$$
...
\begin{align*}
  ...
  &\leq \| y' \|_{\ell^p (\N) \to \mathbb{F}} \| \sum_{j=1}^n t_j e_j \|_p = \| y' \|_{\ell^p (\N) \to \mathbb{F}} \left( \sum_{j=1}^n \left| t_j \right|^p \right)^{1/p}
\end{align*}
$$
...
$$
\begin{gather*}
  \sum_{j=1}^n \left| x_j \right|^{q} \leq \| y' \|_{\ell^p (\N) \to \mathbb{F}} \left( \sum_{j=1}^n \left| x_j \right|^q \right)^{1/p} \\
  \implies \left( \sum_{j=1}^n \left| x_j \right|^{q} \right)^{1 - 1/p} = \left( \sum_{j=1}^n \left| x_j \right|^{q} \right)^{1/q} \leq \| y' \|_{\ell^p (\N) \to \mathbb{F}} \\
  \xRightarrow{n\to\infty} \| x \|_q \leq \| y' \|_{\ell^p (\N) \to \mathbb{F}} \implies x \in \ell^q (\N)
\end{gather*}
$$
...
$$
  \| Tx \|_{\ell^p (\N) \to \mathbb{F})} \leq \| x \|_q \leq \| y \|_{\ell^p (\N) \to \mathbb{F})}
$$

At the same time $\| Tx \|_{\ell^p (\N) \to \mathbb{F})} = \| y \|_{\ell^p (\N) \to \mathbb{F})}$, so that $\| Tx \|_{\ell^p (\N) \to \mathbb{F})} = \| x \|_q$ which shows that $T$ is ismotric.
</details>
</MathBox>
...
If $\langle \cdot, \cdot \rangle$ is an inner product, then $\| x \|_{\langle \cdot, \cdot \rangle} = \sqrt{\langle x, x \rangle}$ defines a norm. 
</MathBox>
...
$$
  \lvert \langle x_0, x_n \rangle - \langle x_0, \tilde{x} \rangle \rvert = \lvert \langle x_0, x_n - \tilde{x} \rangle \rvert \leq \| x_0 \| \cdot \| x_n - \tilde{x} \| \xrightarrow{n\to\infty} 0 \implies f(x_n) \xrightarrow{n\to\infty} f(\tilde{x}).
$$

## Hilbert space

<MathBox title='Hilbert space' boxType='definition'>
If $\left(X, \| \cdot \|_{\langle \cdot, \cdot \rangle} \right)$ defines a Banach space, then $\left(X, \langle \cdot, \cdot \rangle \right)$ is called a Hilbert space 
</MathBox>
...
Let $\left(X, \langle \cdot, \cdot \rangle \right)$ be an inner product space with norm $\| x \| = \sqrt{\langle x, x \rangle}$. Then for all $x, y \in X$

$$
  \lvert \langle x, y \rangle \rvert \leq \| x \| \cdot \| y \|
$$
...
$$
\begin{align*}
  \| x + y \|^2 &= \langle x + y, x + y \rangle = \| x \|^2 + 2\lvert \langle x, y \rangle \rvert + \| y \|^2 \\
  &\leq \| x \|^2 + 2\| x \| \cdot \| y \| + \| y \|^2 = \left( \| x \| + \| y \| \right)^2
\end{align*}
$$

Taking the square root we obtain the triangle inequality $\| x + y \| \leq \| x \| + \| y \|$.

<details>
<summary>Proof</summary> 

Considering the non-trivial case $x \neq 0$, we define the unit vector $\hat{x} := \frac{x}{\| x \|}$ and take the orthogonal projection of $y$ onto $\hat{x}$ (parallel component) given by $y_\parallel = \langle \hat{x}, y \rangle \hat{x}$. The Cauchy-Schwarz inequality can be obtained from taking the norm of the orthogonal component $y_\perp = y - y_\parallel$

$$
\begin{align*}
  0 \leq \| y_\perp \|^2 &= \| y - y_\parallel \|^2 = \langle y - \langle \hat{x}, y \rangle \hat{x}, y - \langle \hat{x}, y \rangle \hat{x} \rangle \\
  ...
  &= \| y \|^2 - \overline{\langle \hat{x}, y \rangle}\langle \hat{x}, y \rangle - \langle \hat{x}, y \rangle \overline{\langle \hat{x}, y \rangle}  + \lvert \langle \hat{x}, y  \rangle \rvert^2 \underbrace{\| \hat{x} \|^2}_{=1} \\
  &= \| y \|^2 - \lvert \langle \hat{x}, y  \rangle \rvert^2
\end{align*}
$$
...
$$
\begin{gather*}
  \| y \|^2 \geq \lvert \langle \hat{x}, y  \rangle \rvert^2 = \left\lvert \left\langle \frac{x}{\| x \|}, y  \right\rangle \right\rvert^2 = \frac{1}{\| x \|^2} \lvert \langle x, y \rangle \rvert^2 \\
  \implies \lvert \langle x, y \rangle \rvert \leq \| x \| \cdot \| y \|
\end{gather*}
$$
</details>
</MathBox>
...
For each continuous linear functional $\ell: X \to \mathbb{F}$ there is exactly one $x_\ell \in X$ such that $\ell(x) = \langle x_\ell, x \rangle$ for all $x \in X$ and $\| \ell \| = \| x_\ell \|_X$.

<details>
<summary>Proof</summary>

To prove the existence of $x_\ell$ we consider the kernel of $\ell$ defined as $\ker(\ell) := \Set{ x \in X | \ell(x) = 0 }$. In the trivial case $\ker (\ell) = X$ it follows that $x_\ell = 0$, so we assume $\ker(\ell) \neq X$. Evidently $\ell$ is continuous because $\ker(\ell) = \ell^{-1}(\Set{ 0 })$. Thus $\ker(\ell)$ is a closed subset of $X$. Consider $\hat{x} \in \ker(\ell)^\perp$ with $\| \hat{x} \|_X = 1$ and set $x_\ell := \overline{\ell(\hat{x})}\hat{x}$, then
...
To prove that the operator norm is $\| \ell \| = \| x_\ell \|$, we have

$$
  \| \ell \| = \sup\Set{ \lvert \ell(x) \rvert \; | \; \| x \|_X = 1 } = \sup\Set{ \underbrace{\lvert \langle x_\ell, x\rangle \rvert}_{\leq \| x_\ell \|_X \cdot \| x \|_X }\; | \; \| x \|_X \leq 1 } \leq \| x_\ell \|
$$

Similarly

$$
  \| \ell \| \geq \left\lvert \ell \left( \frac{x_\ell}{\| x_\ell \|} \right) \right\rvert = \left\lvert \left\langle x_\ell, \frac{x_\ell}{\| x_\ell \|} \right\rangle \right\rvert = \| x_\ell \|
$$

Hence, we conclude that $\| \ell \| = \| x_\ell \|$.
</details>
</MathBox>
...
$$
  \| (T - \lambda)^{-1} \| \geq \frac{1}{d(\lambda, \sigma(T))}
$$

<details>
<summary>Proof</summary>

The first property can be shown by choosing $\lambda_0 \in \rho(T)$ and setting $c := \| (T - \lambda_0)^{-1} \|$ and $\epsilon := \frac{1}{c}$. Consider any $\lambda \in \mathbb{C}$ with $\left| \lambda - \lambda_0 \right| < \epsilon$, then

$$
  T - \lambda = (T - \lambda_0) - (\lambda - \lambda_0) = (T - \lambda_0) \left( I - (\lambda - \lambda_0)(T - \lambda_0)^{-1} \right) = (T - \lambda_0) (I - S)
$$

Furthermore, $\| S \| = \| \lambda - \lambda_0 \| \cdot \| (T - \lambda_0)^{-1} \| < \epsilon c = 1$. This implies that $I - S$ is a convergent Neumann series, and thus invertible. Since, $T - \lambda$ is product a two invertible operators, it is also invertible. This shows that $\lambda \in \rho(T)$, meaning that $\rho(T)$ is open. Thus, $\sigma(T)$ is closed.
...
$$
  |\lambda - \lambda_0 | \geq \epsilon \implies \frac{1}{| \lambda - \lambda_0 |} \leq C = \| (T - \lambda)^{-1} \|
$$

Hence, we get

$$
  \frac{1}{d(\lambda, \sigma(T))} = \frac{1}{\inf_{\lambda \in \sigma(T)} | \lambda - \lambda_0 |} = \sup_{\lambda \in \sigma(T)} \frac{1}{| \lambda - \lambda_0 |} \leq \| (T - \lambda)^{-1} \|
$$
</details>
</MathBox>
...
2. $\sigma(T) \neq \emptyset$ for $X \neq \Set{ 0 }$
3. $r(T) := \sup_{\lambda\in\sigma(T)}|\lambda| = \lim_{k\to\infty} \| T^k \|^{1/k} = \inf_{k\in\N} \| T^k \|^{1/k} \leq \| T \| < \infty$
...
The boundedness implied by the first and last property can be proved by choosing $\lambda \in \mathbb{C}$ with $|\lambda| > \| T \|$ and consider $\frac{T}{\lambda}$ as a Neumann series
...
If $T$ is normal then the spectral radius of $T$ is equal to the operator norm, i.e. $r(T) = \| T \|$.
</MathBox>

## Compact operators

Let $\left(X ,\| \cdot \|_X\right)$ and $\left(X ,\| \cdot \|_X\right)$ be normed spaces. A bounded linear operator $T: X \to Y$ is called compact if $\overline{T\left[\mathcal{B}_1(0)\right]}$ is compact.

<MathBox title='' boxType='proposition'>
Suppose $\left(X ,\| \cdot \|_X\right)$ is a Banach space, and $T: X \to X$ a compact operator, then
...
$$
  \| (T - \mu I)^{-1} \| \geq \| (T - \mu I)^{-1} e_j \|_{\ell^p (\N)} = \| (\lambda_j - \mu I)^{-1} e_j \|_{\ell^p (\N)} = \frac{1}{\left| \lambda_j - \mu \right|}
$$
