...
The pair $(X, \norm{\cdot\|)$ is called a normed space. The norm $\| \cdot}$ induces a metric by
...
A normed space $(X,\norm{\cdot\|)$ is called a Banach space if $\left(X, d_{\| \cdot}} \right)$ is a complete metric space. A Banach space is a complete normed space.
</MathBox>
...
$$
  \norm{ f(x) }_Y \leq C\alpha (x),\; \forall x\in B(a,r) \subset X
$$
...
$$
  \frac{\norm{ f(x) }_Y}{\alpha (x)} \leq \epsilon,\; \forall x\in B(a,\delta) \subset X
$$
...
if there exists constants $C > 0$ and $K > 0$ such that $\norm{ f(x) }_Y < C \alpha(x)$ for all $x\in X$ with $\norm{ x }_X > K$.
...
$$
  \frac{\norm{ f(x) }_Y}{\alpha (x)} \leq \epsilon
$$

for all $x \in X$ with $\norm{ x }$, or equivalently
...
For $p\in[1,\infty]$, let $\mathcal{L}^p = \Set{[f] : f\in L^p}$ and define $\norm{[f]\|_p =} f \|_p$ for $f\in\mathcal{M}(X,\R)$ where $\mathcal{M}(X,\R)$ is the collection of measurable functions $f:X\to\R$. Then $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ and $\|\cdot\|$ is a norm on $\mathcal{L}^p$ that satisfies for $f,g\in L^p$ and $c\in\mathcal{R}$
1. $\| f\|_p \geq 0$ and $\| f\|_p = 0$ if and only if $f\equiv 0$ (positive definite)
2. $\| cf\|_p = |c|\cdot\| f\|_p$ (absolutely homogenous)
3. $\norm{ f+g\|_p \leq} f\norm{_p +} p\|_p$ (triangle inequality)
...
1. The predicate $\norm{ f\|_p \geq 0$ follows from the definitions. For the second part, we first consider the case $p\in(0,\infty)$. Trivially, $\| 0\|_p = 0$. Conversely, if $\| f\|_p = 0$ then $\int_X} f\|^p\;\d\mu = 0$ and hence $|f|^p \overset{\textrm{a.e.}}{=} 0$ and $f \overset{\textrm{a.e.}}{=} 0$. For the case $p = \infty$, it follows trivially that $\| 0\|_\infty = 0$. Conversely, suppose $\| f\|_\infty = 0$. Then for each $n\in\N$ there exists $b_n\in[0,\infty)$ with $b_n \xrightarrow{n\to\infty}0$ and $|f\overset{\textrm{a.e.}}{\leq} b_n$, resulting in $f\overset{\textrm{a.e.}}{=}0$.
...
$$
  \norm{ fg\|_1 = \int_X |fg|\;\d\mu\leq\| f}_p \cdot \norm{ g}_q
$$
...
$$
  \int_X |fg|\;\d\mu = \int_X |f|\cdot|g|\;\d\mu \leq\norm{ g\|_\infty \int_X |f|\;\d\mu =} f\norm{_1 \cdot} g\|_\infty
$$
...
Suppose first that $\norm{ f\|_p =} g\|_q = 1$. From the increasing monotonicity and linearity properties of the integral

$$
  \int_X |fg|\;\d\mu\leq \frac{1}{p}\int_X |f|^p \;\d\mu + \frac{1}{q}\int_X |g|^q \;\d\mu = \frac{1}{p} + \frac{1}{q} = 1
$$

For the general case where $\norm{ f\|_p > 0$ and $\| g\|_q > 0$, let $f_1 = \frac{f}{\| f\|_p}$ and $g_1 = \frac{g}{\| g\|_q}$. Then $\| f_1\|_p =} g_1\|_q = 1$ and $\| f_1g_1\|_q\leq 1$. By the scaling property of the norm
...
$$
  \norm{ f + g\|_p = \left(\int_X |f + g|^p \;\d\mu\right)^{1/p} \leq} f \norm{_p +} g \|_p
$$
...
$$
\begin{align*}
  \int_X |f+g|^{p-1}|f|\;\d\mu &\leq \left\norm{ |f+g|^{p-1} \right\|_q \cdot} f\|_p \\
  \int_X |f+g|^{p-1}|g|\;\d\mu &\leq \left\norm{ |f+g|^{p-1} \right\|_q \cdot} g\|_p
\end{align*}
$$
...
$$
  \norm{ f + g\|_p^p \leq \left\| |f+g|^{p-1} \right\|_q (\| f\|_p +} g\|_p)
$$
...
$$
  \norm{ b}_{q} = \left( \sum_{j=1}^n \left| \left( \left| x_j \right| + \left| y_j \right| \right)^{p-1} \right|^{q} \right)^{\frac{1}{q}} = \left( \sum_{j=1}^n\left( \left| x_j \right| + \left| y_j \right| \right)^p \right)^{\frac{1}{q}}
$$
...
\begin{align*}
  ...
  \leq& \norm{ a}_p + \norm{ c}_p = \left( \sum_{j=1}^n \left| x_j \right|^p \right)^{\frac{1}{p}} + \left( \sum_{j=1}^n \left| y_j \right|^p \right)^{\frac{1}{p}}
\end{align*}
$$
...
$$
  \Norm{ x_m^{(k)} - x_m^{(l)} }^p \leq \sum_{n=1}^\infty \Norm{ x_n^{(k)} - x_n^{(l)} }^p = \norm{ x^{k} - x^{l}}_p^p
$$
...
$$
\begin{align*}
  \left\| x^{(k)} - \tilde{x} \right\|_p^p =& \sum_{n=1}^\infty \Norm{ x^{(k)} - \tilde{x} }^p = \lim_{N \to \infty} \sum_{n=1}^N \Norm{ x^{(k)} - \tilde{x} }^p \\
  =& \lim_{N \to \infty} \lim_{l\to\infty} \sum_{n=1}^N \Norm{ x^{(k)} - x^{(l)} }^p \leq (\epsilon')^p
\end{align*}
$$
...
$$
\begin{gather*}
  \norm{ Tx}_{\ell^p (\N) \to \mathbb{F}} = \sup\Set{ \left| (Tx)(y) \right| | \norm{ y}_p = 1} \leq \sup\Set{\norm{ y}_p \cdot \norm{ x}_q | \norm{ y}_p = 1} \leq \norm{ x}_q \\
  \implies \norm{ T} = \frac{\norm{ Tx}_{\ell^p (\N)}}{\norm{ x}_q} \leq 1
\end{gather*}
$$
...
$$
\begin{gather*}
  \norm{ y}^2 \geq \norm{ \braket{\hat{x}, y} }^2 = \Norm{ \Braket{\frac{x}{\norm{ x}}, y} }^2 = \frac{1}{\norm{ x}^2} \norm{ \braket{x, y} }^2 \\
  \implies \norm{ \braket{x, y} } \leq \norm{ x} \cdot \norm{ y}
\end{gather*}
$$
</details>
...
$$
  \norm{ \ell} \geq \Norm{ \ell \left( \frac{x_\ell}{\norm{ x_\ell}} \right) } = \Norm{ \Braket{x_\ell, \frac{x_\ell}{\norm{ x_\ell}} } } = \norm{ x_\ell}
$$
...
Suppse $A_0$ is a normed algebra, and $A$ denotes its completion as a Banach space. Then $A$ acquires a natural Banach algebra structure as follows: if $x,y\inA$, pick sequences $(x_n)$ and $(y_n)$ in $A_0$ such that $x_n \xrightarrow{n\to\infty} x$ and $y_n \xrightarrow{n\to\infty} y$. Then, pick a constant $K$ such that $\norm{ x_n }, \norm{ y_n } \leq n$, note that
...
$$
  \norm{ x} = |\phi_0 (x)| = |\phi(x)| = |\braket{\pi_\phi (x)\mathbf{x}_\phi, \mathbf{x}_\phi\rangle|\leq\norm{\pi_\phi (x)}
$$
</details>
</MathBox>
...
which shows that $T$ is continuous. Conversly, suppose $T$ is continuous. Then there exists $\delta > 0$ such that $\norm{ Tx}_Y < 1$ for all $x \in X$ with $\norm{ x}_X < \delta$, thus

$$
  \norm{ Tx}_Y = \left\norm{ \frac{\| x}}{\delta} T\left( \delta \frac{x}{\norm{ x}} \right) \right\norm{ = \frac{\| x}}{\delta} \left\norm{ T\left( \delta \frac{x}{\| x}} \right) \right\norm{ \leq \frac{\| x}}{\delta} < \infty
$$
</details>
</MathBox>
...
\begin{gather*}
  ...
  \sup_{\lambda \in \sigma(T)} |\lambda | \leq \norm{ T}
\end{gather*}
$$
...
$$
  \left| f_\ell(\lambda) \right| \leq \norm{ \ell} \cdot \norm{ (T - \lambda)^{-1}} \leq \norm{ \ell} \frac{1}{|\lambda|}\sum_{k=0}^\infty \left\norm{ \frac{T}{\lambda} \right\|^k \leq \frac{\| \ell}}{\norm{ T}}
$$
...
$$
\begin{align*}
  \norm{ I(f) } =& \Norm{ \sum_{i=1}^N x_i (t_i - t_{i-1}) } \\
...
Since any $t\in [t_{i-1}, t_i)$ for $i=1,\dots,N-1$ and $t\in[t_{N-1}, t_N]$ is within $\delta$ of the endpoints, we have $\norm{ f(t) - f_N (t) \lVert < \epsilon$. As this holds for all $t\in[a,b]$, we obtain $\lVert f - f_N } < \epsilon$. Since $\epsilon > 0$ is arbitrary, we have that $f$ is a limit point of $S([a,b], X)$. This shows that $C([a,b],X)\subset\overline{S([a,b],X)}$.
</details>
</MathBox>
...
$$
\begin{align*}
  \norm{ f(b) - f(a) }_X =& \Norm{ \sum_{i=1}^n f(x_i) - f(x_{i-1}) }_X \\
  =& \Norm{ \sum_{i=1}^n (f(x_i) - f(t_i)) + (f(t_i) - f(x_{i-1})) }_X \\
...
$$
\begin{align*}
  &\Norm{ \int_a^{t+h} f(x)\;\d s - \int_a^t f(s)\;\d s - f(t)h }_X \\
  &= \Norm{ \int_t^{t+h} f(s)\;\d s - f(t)h }_X \\
  &= \Norm{ \int_t^{t+h} (f(s) - f(t))\;\d s }_X \\
...
\begin{align*}
  &\norm{ f_n (x) - f(x) }_Y \\
  =& \Norm{ f_n (x_*) + \int_0^1 \d f_n (x_* + th)h\;\d - z - \int_0^1 g(x_* + th)h\;\d t }_Y \\
  \leq& \norm{ f_n (x_*) - z }_Y + \Norm{ \int_0^1 (\d f_n (x_* + th) - g(x_* + th))h\;\d t }_Y \\
...
$$
\begin{align*}
  \norm{ f(x + h) - f(x) - g(x)h } =& \Norm{ \int_0^1 (g(x + th)h - g(x)h)\;\d t }_Y \\
  =& \Norm{ \int_0^1 (g(x + h) - g(x))h\;\d t }_Y
\end{align*}
$$
