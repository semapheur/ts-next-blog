...
$$
  \d^2 f = \d (\d f): X\to\mathcal{B}(X,\mathcal{X,Y}) = \mathcal{B}^2 (X,Y)
$$
...
where the remainder $R_k$ is given by the integral form

$$
  R_k (x,h) = \int_0^1 \frac{(1 - t)^{k-1}}{(k-1)!}\d_h^k f(x + th)\;\d t
$$
</MathBox>
