...
$$
  \sup\Set{\frac{\lVert A_m (x) - A_n (x) \rVert_Y}{\lVert x \rVert_X} | x\neq 0 \in X} = \lVert A_m - A_n \rVert_{X\to Y} < \epsilon
$$
...
$$
  \lVert A \rVert_{X\to Y} = \sup\Set{\frac{\lVert A(x) \rVert_Y}{\lVert x \rVert_X} | x\neq 0 \in X} \leq (\epsilon + M)
$$
...
$$
  \lVert\overline{T}\rVert_{Z\to X} = \sup\Set{\frac{\lVert\overline{T}(z)\rVert_X}{\lVert z \rVert_Z} | z\neq 0 \in Z} \leq \lVert T \rVert_{S\to X}
$$
...
This follows immediately from the continuous linear extension theorem (Theorem $\ref{proposition-9}$).
</details>
</MathBox>
...
<MathBox title='Properties of single-variable Banach-valued integrals' boxType='proposition'>
