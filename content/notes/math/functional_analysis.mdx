...
$$
  \mathrm{diam}(A) := \inf\left\{ r > 0 \mid d(x, y) < r \; \forall x,y \in A \right\}
$$
...
If $A$ is totally bounded, there exists a finite cover of $A$ with open unit balls (radius $1$). Let $C$ denote the center of the balls, and let $c = \max\{ d(u, v) \mid u, v\in C \}$ be the maximum distance between two centers. Since $C$ is finite, $c<\infty$. Now let $x, y\in A$. Since the balls cover $A$, there exists $u, v\in C$ with $x\in B_1(u)$ and $y\in B_1(v)$. By the triangle inequality
...
Consider the Banach space $\left(C([a, b]), \lVert \cdot \rVert_\infty \right)$ of continuous functions on an interval $I = [a, b] \in \mathbb{R}$ with the supremum norm $\lVert f \rVert_\infty := \sup \left\{ |f(t)| \mid t\in[a, b] \right\}$. A subset $F \in C([a, b])$ is compact if and only if $F$ is closed, bounded and uniformly equicontinuous.
</MathBox>
...
and for $p=\infty$ we define $\lVert x\rVert_\infty = \sup\{x_i \mid i\in X\}$. The Lebesgue space takes the form
...
$$
\begin{gather*}
  \lVert Tx \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} = \sup\left\{ \left| (Tx)(y) \right| \mid \lVert y \rVert_p = 1 \right\} \leq \sup\left\{ \lVert y \rVert_p \cdot \lVert x \rVert_q \mid \lVert y \rVert_p = 1 \right\} \leq \lVert x \rVert_q \\
...
$$
  \sigma(T) := \left\{ \lambda \in \mathbb{C} \mid (T - \lambda I) \textrm{ not bijective}  \right\}
$$

The resolvent set of $T$ is defined by

$$
  \rho(T) := \left\{ \lambda \in \mathbb{C} \mid (T - \lambda I) \textrm{ bijective and } (T - \lambda I)^{-1} \textrm{ bounded}  \right\}
$$
...
- Point spectrum (eigenvalues): $\sigma_p(T) := \left\{ \lambda \in \mathbb{C} \mid (T - \lambda I) \textrm{ not injective}  \right\}$
- Continuous spectrum: $\sigma_c(T) := \left\{ \lambda \in \mathbb{C} \mid (T - \lambda I) \textrm{ injective and not surjective with } \overline{\mathrm{Ran}(T - \lambda I)} = X \right\}$
- Residual spectrum: $\sigma_c(T) := \left\{ \lambda \in \mathbb{C} \mid (T - \lambda I) \textrm{ injective and not surjective with } \overline{\mathrm{Ran}(T - \lambda I)} \neq X \right\}$
</MathBox>
