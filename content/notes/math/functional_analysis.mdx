...
For $X = \mathbb{R}^n$ the line segment between to points is given by the Euclidean metric $d(x, y) = \sqrt{\sum_{i=1}^n (x_i - y_i)^2}$
...
2. Let $\{U_i \subseteq X\}_{i\in I}$ be a collection of open sets in $X$ for an arbitrary index set $I$. For any $x\in U = \bigcup_{i\in I} U_i$, clearly $x\in U_j$ for some $j\in I$. Since $U_j$ is open, there is $\epsilon > 0$ such that $B_\epsilon(x)\subseteq U_j\subseteq U$. Hence, $U$ is also open and $\mathcal{T}_d$ is closed under arbitrary unions.
3. Let $\{U_i \subseteq X\}_{i\in I}$ be a finite collection of open sets in $X$ for a finite index set $I$. Trivially, if $U=\bigcap_{i\in I} U_i = \emptyset$, then $U$ is open. For any $x\in U$, clearly $x\in U_i$ for every $i\in I$. For each $i\in I$ there is $\epsilon_i > 0$ such that $B_{\epsilon_i}(x)\subseteq U_i$. Take $\epsilon = \min\{ \epsilon_i \}_{i\in I} > 0$. Since $I$ is finite, $B_\epsilon(x) \subseteq B_{\epsilon_i}(x)\subseteq U_i$ for each $i\in I$. Hence $B_\epsilon(x)\subseteq U$, so $U$ is open and $\mathcal{T}_d$ is closed under finite intersection.  
</details>
</MathBox>
...
Consider distinct point $x, y\in X$ with $r = d(x, y) > 0$. Note that $B_{\frac{r}{2}}(x)$ and $B_{\frac{r}{2}}(y)$ are open and contain $x$ and $y$, respectively. Suppose that $z\in B_{\frac{r}{2}}(x)\cap B_{\frac{r}{2}}(y)$. Applying the triangle inequlity
...
Let $(X, d)$ be a metric space. A sequence $\left( x_n \in X \right)_{n \in \mathbb{N}}$ of points in $X$ is convergent if there is $\tilde{x}\in X$ with 

$$
  \forall \epsilon > 0 \quad \exists N \in \mathbb{N} \quad \forall n \geq N : d\left(x_n, \tilde{x} \right) < \epsilon
$$
...
Let $(X, d)$ be a metric space, and let $\left( x_n \in X \right)_{n \in \mathbb{N}}$ be a sequence of points in $X$. If $x_n \xrightarrow{n\to\infty} x$ and $x_n \xrightarrow{n\to\infty} y$ for $x,y\in X$, then $x = y$. 
...
$$
\begin{gather*}
  \exists M \in \mathbb{N} \; \forall n \geq M: \; d(x_n, x) < \frac{\epsilon}{2} \\
  \exists N \in \mathbb{N} \; \forall n \geq N: \; d(x_n, y) < \frac{\epsilon}{2}
\end{gather*}
$$
...
Let $(X, d)$ be a metric space. A subset $A \subseteq X$ is closed if and only if for every convergent sequence $\left(a_n \in A \right)_{n \in \mathbb{N}}$ of points in $A$, the limit is in $A$, i.e. $\lim_{n\to\infty} a_n \in A$.

<details>
<summary>Proof</summary>

Suppose that $A$ is closed and that $\left(a_n \in A \right)_{n \in \mathbb{N}}$ is a sequence of points in $A$ such that $a_n \xrightarrow{n\to\infty} x \in X$. Suppose that $x\in A^c$. Since $A^c$ is open, $x^n \in A^c$ for sufficiently large $n$, giving a contradiction. Hence $x\in A$. Conversely, suppose that $A$ is not closed. Then $A^c$ is not open and there exists $x\in A^c$ with the property that every neighbourhood of $x$ has points in $A$. Note that for each $n\in\mathbb{N}$ there exist $x_n \in B_{\frac{1}{n}}(x)$ with $x_n \in A$. However, clearly $x_n \xrightarrow{n\to\infty} x$, giving a contraction.
</details>
</MathBox>
...
Let $(X, d)$ be a metric space. A sequence $\left( x_n \in X \right)_{n \in \mathbb{N}}$ is called a Cauchy sequence if 
...
Let $(X, d)$ be a metric space. If a sequence $\left( x_n \in X \right)_{n \in \mathbb{N}}$ converges, then the sequence is Cauchy.
<details>
<summary>Proof</summary>

Assume that the sequence $\left( x_n \in X \right)_{n\in\mathbb{N}}$ converges to $x\in X$. Let $\epsilon > 0$, then there exists $N\in\mathbb{N}$ such that $d(x_n, x) < \frac{\epsilon}{2}$ for every $n \geq N$. If $m, n > N$, then by the triangle inequality

$$
  d(x_m, x_n) \leq d(x_m, x) + d(x_n, x) < \frac{\epsilon}{2} + \frac{\epsilon}{2} = \epsilon
$$

Hence $\left( x_n \right)_{n\in\mathbb{N}}$ is Cauchy.
</details>
</MathBox>
...
Assume that the sequence $\left( a_n \in A \right)_{n\in\mathbb{N}}$ converges to $x \in X$. Then $\left( a_n \in A \right)_{n \in \mathbb{N}}$ is a Cauchy sequence, and by completeness $x\in A$. Hence $A$ is closed.
</details>
</MathBox>

<MathBox title='' boxType='proposition'>
Let $(X, d)$ be a metric space, and let $\left( x_n \in X \right)_{n\in\mathbb{N}}$ be a Cauchy sequence. If there is a subsequence $\left( x_{n_k} \right)_{k\in\mathbb{N}}$ with $x_{n_k}\xrightarrow{k\to\infty} x \in X$, then $x_n \xrightarrow{n\to\infty} x$.

<details>
<summary>Proof</summary>

Assume that the subsequence $\left( x_{n_k} \in A \right)_{k\in\mathbb{N}}$ converges to $x \in X$. Let $\epsilon > 0$, then there exists $j\in\mathbb{N}$ such that $d\left(x_{n_k}, x) < \frac{\epsilon}{2}$. Since $\left( x_n \right)_{n\in\mathbb{N}}$ is Cauchy, there exists $N\in\mathbb{N}$ such that for $d(x_m, x_n) < \frac{\epsilon}{2}$ for $m, n \geq N$. Choose $k\in\mathbb{N}$ such that $k \geq j$ and $n_k \geq N$. By the triangle inequality
...
Let $\left( X, d \right)$ be a complete metric space. Then any contraction $f:X\to X$ has a unique fixed point. That is, there exists $x*\in X$ with $f(x*) = x*$. Alternatively, let $x_0\in X$ and recursively define $x_n = f(x_{n-1})$ for $n\in\mathbb{N}$. Then $x_n \xrightarrow{n\to\infty} x*$.  
</MathBox>
...
$$
  \mathrm{diam}(A) := \inf\left\{ r > 0 \;|\; d(x, y) < r \; \forall x,y \in A \right\}
$$
...
If $A$ is totally bounded, there exists a finite cover of $A$ with open unit balls (radius $1$). Let $C$ denote the center of the balls, and let $c = \max\{ d(u, v) \;|\; u, v\in C \}$ be the maximum distance between two centers. Since $C$ is finite, $c<\infty$. Now let $x, y\in A$. Since the balls cover $A$, there exists $u, v\in C$ with $x\in B_1(u)$ and $y\in B_1(v)$. By the triangle inequality
...
Let $(X, d)$ be a metric space. A subset $A \subseteq X$ is (sequentially) compact if for each sequence $\left( x_n \right)_{n \in \mathbb{N}} \subseteq A$ there is a convergent subsequence $\left( x_{n_k} \right)_{k \in \mathbb{N}}$ with $\tilde{x} := \lim_{k\to\infty} x_{n_k} \in A$.
</MathBox>
...
Let $A \subseteq X$ be compact and let $\left( x_n \right)_{n \in \mathbb{N}} \subseteq A$ be convergent with limit $\tilde{x} \in X$. Because $A$ is compact, there is a convergent subsequence $\left( x_{n_k} \right)_{k \in \mathbb{N}}$ with limit $\tilde{\tilde{x}} \in A$. Furthermore, since limits are unique, then $\tilde{x} = \tilde{\tilde{x}} \in A$. This shows that $A$ is closed.

Proving that compactness implies boundedness can be done by contraposition. Assuming that $A$ is not bounded, then for a given $a \in A$, there are $x_n \in A$ with $d(a, x_n) > n$. For any subsequence $\left( x_{n_k} \right)_{k \in \mathbb{N}}$ and any point $b \in A$ we have
...
Let $A\subseteq\mathbb{R}^n$ be subset of Euclidean space. Then $A$ is compact if and only if $A$ is closed and bounded.
...
Let $\mathbb{F} \in \{\mathbb{R}, \mathbb{C} \}$ be a field of numbers and let $X$ be a $\mathbb{F}$-vector space. A map $\lVert\cdot\rVert : X \mapsto [0, \infty)$ is called a norm if it satisfies
...
The norm $\lVert \cdot \rVert$ is a continuous map. Considering the sequence $\left( x_n \right)_{n\in\mathbb{N}} \subseteq X$ with limit $\tilde{x} \in X$, then by the triangle inequality
...
$$
  \lVert T \rVert = \lVert T \rVert_{X\to Y} := \sup\left\{ \left. \frac{\lVert T_X \rVert_Y}{\lVert X \rVert_X} \right| x \neq 0 \in X \right\}
$$
</MathBox>
...
Suppose that $T$ is bounded. Then for a sequence $\left(x_n \right)_{n\in\mathbb{N}} \subseteq X$ with limit $\tilde{x} \in X$ we have
...
Let $\left( \ell_k \right)_{k\in\mathbb{N}} \subseteq X'$ be a Cauchy sequence, i.e.

$$
  \forall \epsilon > 0 \quad \exists N \in \mathbb{N} \quad \forall n, m \geq N : \lVert \ell_n - \ell_m \rVert_{X\to\mathbb{F}} < \epsilon
$$
...
Hence, $\left( \ell_k (x) \right)_{k\in\mathbb{N}} \subseteq \mathbb{F}$ is a Cauchy sequence for all $x \in X$, with the limit

$$
  \ell(x) := \lim_{k\to\infty} \ell_k (x)
$$

We then have to show that $\ell: X \to \mathbb{F}$ is linear and bounded and that $\lVert \ell_k - \ell \rVert_{X \to \mathbb{F}} \xrightarrow{k\to\infty} 0$. Boundedness follows from the Cauchy property of $\left( \ell_k \right)_{k\in\mathbb{N}}$:
...
To show the convergence of the operator norm $\lVert \cdot \rVert_{X\to\mathbb{F}}$, we have the for $\epsilon > 0$ there is a $N \in \mathbb{N}$ such that for all $n, m \geq N$
...
Consider the Banach space $\left(C([a, b]), \lVert \cdot \rVert_\infty \right)$ of continuous functions on an interval $I = [a, b] \in \mathbb{R}$ with the supremum norm $\lVert f \rVert_\infty := \sup \left\{ |f(t)| \;|\; t\in[a, b] \right\}$. A subset $F \in C([a, b])$ is compact if and only if $F$ is closed, bounded and uniformly equicontinuous.
</MathBox>

## Lebesgue space ($L^p$)

<MathBox title='$L^p$ space' boxType='definition'>
Let $(X,\mathcal{A},\mu)$ be a measure space and suppose that $f: X\to\mathbb{R}$ is measurable. For $p\in(0,\infty)$ we define the $p$-norm
...
$$
  \lVert f \rVert_\infty := \inf\{b\in[0,\infty] : |f|\overset{\textrm{a.e.}}{\leq} b \}
$$

The function $f$ is called $p$-integrable if $\lVert f\rVert_p < \infty$. The set of all $p$-integrable functions is denoted

$$
  L^p (X,\mathcal{A},\mu) := \{ f:X\to\mathbb{R}: \lVert f\rVert_p < \infty \} \subseteq \mathcal{M}(X,\mathbb{R})
$$

and is called a Lebesgue space. Here $\mathcal{M}(X,\mathbb{R})$ denotes the collection of all measurable functions from $X$ into $\mathbb{R}$.
</MathBox>

<MathBox title='$L^p$ space are normed vector spaces' boxType='proposition'>
For $p\in[1,\infty]$, let $\mathcal{L}^p = \{[f] : f\in L^p \}$ and define $\lVert[f]\rVert_p = \lVert f \rVert_p$ for $f\in\mathcal{M}(X,\mathbb{R})$ where $\mathcal{M}(X,\mathbb{R})$ is the collection of measurable functions $f:X\to\mathbb{R}$. Then $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ and $\lVert\cdot\rVert$ is a norm on $\mathcal{L}^p$ that satisfies for $f,g\in L^p$ and $c\in\mathcal{R}$
...
For $p=\infty$, the result is trivially true if $c=0$. For $c\neq 0$, note that $b\in[0,\infty]$ is an essential bound of $|f|$ if and only iff $|c|b$ is an essential bound of $|cf|$.
...
Fix $y\geq 0$ and define $f:(0,\infty)\to\mathbb{R}$ by
...
Suppose that $f,g:X\to\mathbb{R}$ are measurable on $(X,\mathcal{A},\mu)$ and that $p$ and $q$ are conjugate indices. Then
...
Suppose that $f,g:X\to\mathbb{R}$ are measurable on $(X,\mathcal{A},\mu)$ and that $p\in[1,\infty]$. Then
...
Discrete measure spaces $(X,\mathcal{P}(X),\#)$ gives a special case for $L^p$ called sequence spaces. For a measurable function $x:X\to\mathbb{R}$ we introduce the notation $x_i = x(i)$.

<MathBox title='Sequence space' boxType='definition'>
Let $(X,\mathcal{P}(X),\#)$ be a discrete measure space and let $x:X\to\mathbb{R}$ be measurable. For $p \in [1, \infty)$, we define the $p$-norm

$$
  \lVert x \rVert_p := \left( \sum_{i\in X} |x_i|^p \right)^(1/p)
$$

and for $p=\infty$ we define $\lVert x\rVert_\infty = \sup\{x_i \;|\; i\in X\}$. The Lebesgue space takes the form

$$
  L^p(X,\mathcal{P}(X),\#) = \left\{ (x_i)_{i\in X} \in\mathbb{R}^n : \lVert p \rVert_p < \infty\right\}
$$

When $X = \mathbb{N}$ the $L^p$ space is usually denoted $\ell^p$.
</MathBox>
...
To show the completeness of $\left( \ell^p, \lVert \cdot \rVert_p \right)$ we consider a Cauchy sequence $\left( x^{(k)} \right)_{k \in \mathbb{N}}$ in $\ell^p$. 
...
Because $\left( x^{(k)} \right)_{k \in \mathbb{N}}$ is a Cauchy sequence, it follows that $\forall \epsilon > 0 \; \exists N \in \mathbb{N}:\; \left\lVert x^{(k)} - x^{(l)} \right\rVert^p$. Hence, $\left( x_m^{(k)} \right)_{k \in \mathbb{N}}$ is a Cauchy sequence in $\mathbb{F}$ converging to a limit $\tilde{x}_m \in \mathbb{F}$. This suggests that $\left( x^{(k)} \right)_{k \in \mathbb{N}}$ converges to a limit $\tilde{x} := \left( \tilde{x}_i \right)_{i \in \mathbb{N}}$, which we will now show.

Let $\epsilon > 0$ and choose $K \in \mathbb{N}$ such that $\forall k, l \geq K:\; \left\lVert x^{(k)} - x^{(l)} \right\rVert_p < \epsilon'$
...
The dual space of $\ell^p (\mathbb{N})$ has a natural isomorphism with $\ell^q (\mathbb{N})$ where $q$ is the Hölder conjugate satisfying $\frac{1}{p} + \frac{1}{q} = 1$. This isomorphism associates $x \in \ell^q$ with the functional $T: \ell^q(\mathbb{N}) \to \ell^p (\mathbb{N})^*$ given by

$$
\begin{gather*}
  (Tx)(y) := \sum_{j=1}^\infty x_j y_j \quad \forall y \in \ell^p (\mathbb{N}) \\
  x \mapsto \langle \bar{x}, \cdot \rangle_{\ell^2 (\mathbb{N})}
\end{gather*}
$$
...
which shows that $Tx$ is bounded for all $x \in \ell^q(\mathbb{N})$, hence it is well-defined. The operator norm for $T$ is

$$
\begin{gather*}
  \lVert Tx \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} = \sup\left\{ \left| (Tx)(y) \right| \;|\; \lVert y \rVert_p = 1 \right\} \leq \sup\left\{ \lVert y \rVert_p \cdot \lVert x \rVert_q \;|\; \lVert y \rVert_p = 1 \right\} \leq \lVert x \rVert_q \\
  \implies \lVert T \rVert = \frac{\lVert Tx \rVert_{\ell^p (\mathbb{N})}}{\lVert x \rVert_q} \leq 1
\end{gather*}
$$

Let $y' \in \ell^p (\mathbb{N})^*$ and define $x_j := y'(e_j)$ and $x := \left( x_j \right)_{j \in \mathbb{N}}$ with $e_j = \left(\delta_{jk}\right)_{k\in\mathbb{N}}$ (unit sequence). To prove that $T$ is surjective, we need to show that $x \in \ell^q (\mathbb{N})$ and $Tx = y'$.
...
\begin{align*}
  ...
  &\leq \lVert y' \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} \lVert \sum_{j=1}^n t_j e_j \rVert_p = \lVert y' \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} \left( \sum_{j=1}^n \left| t_j \right|^p \right)^{1/p}
\end{align*}
$$
...
$$
\begin{gather*}
  \sum_{j=1}^n \left| x_j \right|^{q} \leq \lVert y' \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} \left( \sum_{j=1}^n \left| x_j \right|^q \right)^{1/p} \\
  \implies \left( \sum_{j=1}^n \left| x_j \right|^{q} \right)^{1 - 1/p} = \left( \sum_{j=1}^n \left| x_j \right|^{q} \right)^{1/q} \leq \lVert y' \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} \\
  \xRightarrow{n\to\infty} \lVert x \rVert_q \leq \lVert y' \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F}} \implies x \in \ell^q (\mathbb{N})
\end{gather*}
$$

For $y \in \ell^p (\mathbb{N})$ we have
...
$$
  \lVert Tx \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F})} \leq \lVert x \rVert_q \leq \lVert y \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F})}
$$

At the same time $\lVert Tx \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F})} = \lVert y \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F})}$, so that $\lVert Tx \rVert_{\ell^p (\mathbb{N}) \to \mathbb{F})} = \lVert x \rVert_q$ which shows that $T$ is ismotric.
</details>
</MathBox>
...
Let $\mathbb{F} \in \{\mathbb{R}, \mathbb{C} \}$ be a field of numbers and let $X$ be a $\mathbb{F}$-vector space. A map $\langle \cdot, \cdot \rangle : X \times X \to \mathbb{F}$ is called an inner product if it satisfies

1. Positive definitive $\langle x, x \rangle \geq 0 \quad \forall x \in X$ and $\langle x, x \rangle = 0 \iff x = 0$
2. Conjugate symmetry
    - $\langle x, y \rangle = \langle y, x\rangle$ for $\mathbb{F} = \mathbb{R}$
...
The inner product $\langle \cdot, \cdot \rangle$ is a continuous map. Considering the sequence $\left( x_n \right)_{n\in\mathbb{N}}$ with limit $\tilde{x} \in X$ and a fixed $x_0 \in X$, then by the Cauchy-Schwartz inequality
...
The set of square summable sequences $\ell^2 (\mathbb{N}, \mathbb{F})$ endowed with the inner product  
...
\begin{gather*}
  ...
  \langle x, x \rangle = 0 \implies \lvert x_i \rvert^2 = 0\; \forall i \in \mathbb{N} \implies x = 0
\end{gather*}
$$
...
$$
  U^\perp = \left\{ x \in X | \langle x, u \rangle = 0 \; \forall u\in U \right\}
$$
</MathBox>

If $x \perp y$, then the triangle inequlity reduces to the Pytagorean theorem
...
This is proved by considering a sequence $\left( x_n \right)_{n\in\mathbb{N}} \subseteq U^\perp$ showing that the limit $\tilde{x} \in X$ is also in $U^\perp$. It follows that for all $u \in U$
...
To prove the existence of $x_\ell$ we consider the kernel of $\ell$ defined as $\ker(\ell) := \{ x \in X | \ell(x) = 0 \}$. In the trivial case $\ker (\ell) = X$ it follows that $x_\ell = 0$, so we assume $\ker(\ell) \neq X$. Evidently $\ell$ is continuous because $\ker(\ell) = \ell^{-1}(\{ 0 \})$. Thus $\ker(\ell)$ is a closed subset of $X$. Consider $\hat{x} \in \ker(\ell)^\perp$ with $\lVert \hat{x} \rVert_X = 1$ and set $x_\ell := \overline{\ell(\hat{x})}\hat{x}$, then
...
$$
  \lVert \ell \rVert = \sup\left\{ \lvert \ell(x) \rvert \; | \; \lVert x \rVert_X = 1 \right\} = \sup\{ \underbrace{\lvert \langle x_\ell, x\rangle \rvert}_{\leq \lVert x_\ell \rVert_X \cdot \lVert x \rVert_X }\; | \; \lVert x \rVert_X \leq 1 \} \leq \lVert x_\ell \rVert
$$
...
$$
  \sigma(T) := \left\{ \lambda \in \mathbb{C} \;|\; (T - \lambda I) \textrm{ not bijective}  \right\}
$$

The resolvent set of $T$ is defined by

$$
  \rho(T) := \left\{ \lambda \in \mathbb{C} \;|\; (T - \lambda I) \textrm{ bijective and } (T - \lambda I)^{-1} \textrm{ bounded}  \right\}
$$
...
- Point spectrum (eigenvalues): $\sigma_p(T) := \left\{ \lambda \in \mathbb{C} \;|\; (T - \lambda I) \textrm{ not injective}  \right\}$
- Continuous spectrum: $\sigma_c(T) := \left\{ \lambda \in \mathbb{C} \;|\; (T - \lambda I) \textrm{ injective and not surjective with } \overline{\mathrm{Ran}(T - \lambda I)} = X \right\}$
- Residual spectrum: $\sigma_c(T) := \left\{ \lambda \in \mathbb{C} \;|\; (T - \lambda I) \textrm{ injective and not surjective with } \overline{\mathrm{Ran}(T - \lambda I)} \neq X \right\}$
</MathBox>
...
2. $\sigma(T) \neq \emptyset$ for $X \neq \{ 0 \}$
3. $r(T) := \sup_{\lambda\in\sigma(T)}|\lambda| = \lim_{k\to\infty} \lVert T^k \rVert^{1/k} = \inf_{k\in\mathbb{N}} \lVert T^k \rVert^{1/k} \leq \lVert T \rVert < \infty$
...
Consequently $\ell\left(T^{-2}\right) = 0$ for all $\ell \in \mathcal{B}(X)^*$. By the Hahn-Banach theorem, we must have $T^{-2} = 0$, but this implies that $X = \{ 0 \}$ since $T$ is bijective. Hence $\sigma(T) \neq \emptyset$ for  $X \neq \{0\}$.
</details>
</MathBox>
...
3. $\sigma(T)\setminus\{0\}$ could be empty or finite. Otherwise $\sigma(T)\setminus\{0\} = \{ \lambda_n \}_{n\in\mathbb{N}}$ with no accumulation points other than $0$.
4. Each $\lambda\in\sigma(T)\setminus\{0\}$ is an eigenvalue of $T\left(\lambda \in \sigma(T)\setminus\{0\}\right)$ with $\dim\left(\mathrm{Ker}(T - \lambda) \right)$
</MathBox> 

## Example: $\ell^p$

Consider the sequence space $X = \ell^p (\mathbb{N})$. For $\left\{ \lambda_j \right\}_{j\in\mathbb{N}} \subset \mathbb{C}$ with $\sup_{j\in\mathbb{N}}\left| \lambda_j \right| < \infty$ we define a bounded linear operator $T:\ell^p (\mathbb{N}) \to \ell^p (\mathbb{N})$ given by $(Tx)_j := \lambda_j x_j$. Evidently, the $\lambda_j$ are eigenvalues with corresponding eigenvectors $e_j$ as unit sequences, and the set $\left\{ \lambda_j \right\}_{j\in\mathbb{N}} \subset \mathbb{C}$ forms the point spectrum of $T$

$$
  \left\{ \lambda_j \right\}_{j\in\mathbb{N}} = \sigma_p (T) \subseteq \sigma (T)
$$
...
$$
  \lVert (T - \mu I)^{-1} \rVert \geq \lVert (T - \mu I)^{-1} e_j \rVert_{\ell^p (\mathbb{N})} = \lVert (\lambda_j - \mu I)^{-1} e_j \rVert_{\ell^p (\mathbb{N})} = \frac{1}{\left| \lambda_j - \mu \right|}
$$
...
$$
  \sigma (T) = \underbrace{\left\{ \lambda_j \right\}_{j\in\mathbb{N}}}_{\sigma_p (T)} \cup \left\{ \mu \in \mathbb{C} \;|\; \mu \notin \left\{ \lambda_j \right\}_{j\in\mathbb{N}} \wedge \mu \in \overline{\left\{ \lambda_j \right\}_{j\in\mathbb{N}}} \right\} = \sigma_p(T) \cup \sigma_c(T)
$$