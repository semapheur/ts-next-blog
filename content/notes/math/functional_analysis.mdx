...
1. The predicate $\| f\|_p \geq 0$ follows from the definitions. For the second part, we first consider the case $p\in(0,\infty)$. Trivially, $\| 0\|_p = 0$. Conversely, if $\| f\|_p = 0$ then $\int_X \| f\|^p\;\d\mu = 0$ and hence $|f|^p \overset{\textrm{a.e.}}{=} 0$ and $f \overset{\textrm{a.e.}}{=} 0$. For the case $p = \infty$, it follows trivially that $\| 0\|_\infty = 0$. Conversely, suppose $\| f\|_\infty = 0$. Then for each $n\in\N$ there exists $b_n\in[0,\infty)$ with $b_n \xrightarrow{n\to\infty}0$ and $|f\overset{\textrm{a.e.}\leq b_n$, resulting in $f\overset{\textrm{a.e.}}{=}0$.
...
$$
  \| fg\|_1 = \int_X |fg|\;\d\mu\leq\| f \|_p \cdot \| g \|_q
$$
...
$$
  \int_X |fg|\;\d\mu = \int_X |f|\cdot|g|\;\d\mu \leq\| g\|_\infty \int_X |f|\;\d\mu = \| f\|_1 \cdot \| g\|_\infty
$$
...
$$
  \| f + g\|_p = \left(\int_X |f + g|^p \;\d\mu\right)^{1/p} \leq \| f \|_p + \| g \|_p
$$
...
$$
\begin{align*}
  \| f + g\|_p &= \int_X |f + g|\;\d\mu \leq\int_X (|f| + |g|)\;\d\mu \\
  & = \int_X |f|\;\d\mu + \int_X |g|\;\d\mu = \| f\|_1 + \| g\|_1
\end{align*}
$$
...
$$
\begin{align*}
  \int_X |f+g|^{p-1}|f|\;\d\mu &\leq \left\| |f+g|^{p-1} \right\|_q \cdot \| f\|_p \\
  \int_X |f+g|^{p-1}|g|\;\d\mu &\leq \left\| |f+g|^{p-1} \right\|_q \cdot \| g\|_p
\end{align*}
$$
...
\begin{align*}
  ...
  &= \left(\int_X |f+g|^p \;\d\mu \right)^{(p-1)/p} = \| f+g \|_p^{p-1}
\end{align*}
$$
