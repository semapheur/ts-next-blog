...
Let $\mathbb{F} \in \{\R, \mathbb{C} \}$ be a field of numbers and let $X$ be a $\mathbb{F}$-vector space. A map $\lVert\cdot\rVert : X \mapsto [0, \infty)$ is called a norm if it satisfies
...
Consider the Banach space $\left(C([a, b]), \lVert \cdot \rVert_\infty \right)$ of continuous functions on an interval $I = [a, b] \in \R$ with the supremum norm $\lVert f \rVert_\infty := \sup \left\{ |f(t)| \mid t\in[a, b] \right\}$. A subset $F \in C([a, b])$ is compact if and only if $F$ is closed, bounded and uniformly equicontinuous.
</MathBox>
...
$$
  L^p (X,\mathcal{A},\mu) := \{ f:X\to\R: \lVert f\rVert_p < \infty \} \subseteq \mathcal{M}(X,\R)
$$
...
For $p\in[1,\infty]$, let $\mathcal{L}^p = \{[f] : f\in L^p \}$ and define $\lVert[f]\rVert_p = \lVert f \rVert_p$ for $f\in\mathcal{M}(X,\R)$ where $\mathcal{M}(X,\R)$ is the collection of measurable functions $f:X\to\R$. Then $\mathcal{L}^p \subseteq\mathcal{M}\setminus\equiv$ and $\lVert\cdot\rVert$ is a norm on $\mathcal{L}^p$ that satisfies for $f,g\in L^p$ and $c\in\mathcal{R}$
...
$$
  L^p(X,\mathcal{P}(X),\#) = \left\{ (x_i)_{i\in X} \in\R^n : \lVert p \rVert_p < \infty\right\}
$$
...
$$
\begin{gather*}
  \lVert Tx \rVert_{\ell^p (\N) \to \mathbb{F}} = \sup\left\{ \left| (Tx)(y) \right| \mid \lVert y \rVert_p = 1 \right\} \leq \sup\left\{ \lVert y \rVert_p \cdot \lVert x \rVert_q \mid \lVert y \rVert_p = 1 \right\} \leq \lVert x \rVert_q \\
...
Let $\mathbb{F} \in \{\R, \mathbb{C} \}$ be a field of numbers and let $X$ be a $\mathbb{F}$-vector space. A map $\langle \cdot, \cdot \rangle : X \times X \to \mathbb{F}$ is called an inner product if it satisfies
...
3. $\sigma(T)\setminus\{0\}$ could be empty or finite. Otherwise $\sigma(T)\setminus\{0\} = \{ \lambda_n \}_{n\in\N}$ with no accumulation points other than $0$.
...
Consider the sequence space $X = \ell^p (\N)$. For $\left\{ \lambda_j \right\}_{j\in\N} \subset \mathbb{C}$ with $\sup_{j\in\N}\left| \lambda_j \right| < \infty$ we define a bounded linear operator $T:\ell^p (\N) \to \ell^p (\N)$ given by $(Tx)_j := \lambda_j x_j$. Evidently, the $\lambda_j$ are eigenvalues with corresponding eigenvectors $e_j$ as unit sequences, and the set $\left\{ \lambda_j \right\}_{j\in\N} \subset \mathbb{C}$ forms the point spectrum of $T$

$$
  \left\{ \lambda_j \right\}_{j\in\N} = \sigma_p (T) \subseteq \sigma (T)
$$
...
$$
  \sigma (T) = \underbrace{\left\{ \lambda_j \right\}_{j\in\N}}_{\sigma_p (T)} \cup \left\{ \mu \in \mathbb{C} \mid \mu \notin \left\{ \lambda_j \right\}_{j\in\N} \wedge \mu \in \overline{\left\{ \lambda_j \right\}_{j\in\N}} \right\} = \sigma_p(T) \cup \sigma_c(T)
$$