...
\begin{align*}
  ...
  =& \mathbb{E}\left( \int_0^\infty H_s^2 \;\d\langle M \rangle_s \right) = \lVert H \rVert_{L^2 (M)}^2
\end{align*}
$$
...
$$
  \int_0^t H_s\;\d X_s = \lim_{n\to\infty} \sum_{k=0}^{p_n - 1} H_{t_k^{(n)}} (X_{t_{k+1}^{(n)}} - X_{t_k^{(n)}})
$$
...
\begin{align*}
  ...
  e^{-\mu H} (X_0 - \tehta) =& \frac{-2\theta + 2X_0 + \theta - X_0}{2} \\
