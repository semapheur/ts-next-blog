...
Let $X$ denote $M$ or $(M_t^2 - t)_{t\geq 0}$. Let $S_n = \inf\Set{t\geq 0 : |X_t| \geq n}$. Then $\tilde{S} = \langle M \rangle_{S_n}$ is a $(\mathcal{G}_t)_{\t geq 0}$-stopping time. As $X^{S_n}$ is a bounded $(\mathcal{F}_t)_{t\geq 0}$ martingale, the stopping time theorem yields $X_{T_t}^{S_n} = \mathbb{E}(X_\infty^{S_n} | \mathcal{F}_{T_t}). This means that $X_{\tau_t \wedge \tilde{S}_n}$ is a $(\mathcal{G}_t)_{t\geq 0}$-martingale, and since $\langle M \rangle_{S_n}\to\infty$ we get the expected result.
</details>
</MathBox>
