...
for all $s,t\in T$ and $A\in\mathcal{S}$.

In particular, a Markov process $\mathbf{X}$ is *time homogeneous* (stationary) if
...
Let $\mathcal{B}$ denote the collection of bounded, measurable functions $f: S\to\mathbb{R}$. A stochastic process $\mathbf{X}$ is a *Markov process* if and only if

$$
  \mathbb{E}(f_{s+t}|\mathcal{F}_s) = \mathbb{E}(X_{s+t}|X_s)
$$

for all $s,t\in T$ and $f\in\mathcal{B}$.
...
Suppose that the stochastic process $\mathbf{X}$ is adapted to the filtration $\mathsc{F}$, and $\mathscr{F}\subseteq\mathscr{G}$, i.e. $\mathscr{G}$ is finer than $\mathscr{F}$. If $\mathbf{X}$ is a Markov process relative to $\mathscr{G}$, then $\mathbf{X}$ is a Markov process relative to $\mathscr{F}$.
...
The first equalit is a basic property of conditional expected value. The second uses the fact that $\mathbf{X}$ is Markov relative to $\mathscr{G}$, and the third follows since $X_s$ is measurable with respec to $\mathcal{F}_s$
</details>
</MathBox>
...
2. Continuity in time: Given $X_0 = x$ for $x\in S$, then $X_t \xrightarrow{t\downarrow 0} x$ in probability.
</MathBox>
...
Suppose that the stochastic process $\mathbf{X}$ is adapted to the filtration $\mathsc{F}$, and $\mathscr{F}\subseteq\mathscr{G}$, i.e. $\mathscr{G}$ is finer than $\mathscr{F}$. If $\mathbf{X}$ is a (sub/super-)martingale relative to $\mathscr{G}$, then $\mathbf{X}$ is a (sub/super-)martingale process relative to $\mathscr{F}$.
...
Suppose that $s,t\in T$ with $s\leq t$. Note that $\mathbf{X}$ is adapted to $\mathscr{G}$, since $\mathbf{X}$ is adapted to $\mathscr{F}$ and $\mathscr{F}\subseteq\mathscf{G}$. Appyling the tower and monotonicity of conditional expected value we have that
...
1. If $\mathbf{X}$ and $\mathbf{Y}$ are (sub/super-)martingales with respect to $\mathscf{F}$ then $\mathbf{X}+\mathbf{Y}$ is a (sub/super-)martingale with respect to $\mathscr{F}$.
...
<MathBox title='Martingale transform' title='proposition'>
...
$$
\begin{align*}
  \mathbb{E}\left[|(\mathbf{Y}\cdot\mathbf{X})_n| \right] \leq \mathbb{E}(|X_0|) \\
  &\quad + c\sum_{k=1}^n \left[\mathbb{E}(|X_k|) + \mathbb{E}(|X_{k+1}|) \right] \\
  &< \infty
\end{align*}
$$
