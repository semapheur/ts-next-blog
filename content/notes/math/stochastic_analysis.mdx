...
$$
  L^2_{\mathrm{loc}} (M) := \Set{\text{progressively measurable }H | \int_0^t H_s^2 \;\mathrm{d}\langle M \rangle_s \overset{\text{a.s.}}{<} \infty,\; t\geq 0}
$$
...
$$
  \tau_n = inf\Set{t \geq 0 | \int_0^t (1 + H_s^2)\;\mathrm{d}\langle M \rangle_s = n}
$$
...
$$
  \mathrm{d}f(B_t^1, B_t^2) = (\partial_1 f)\;\mathrm{d}B_t^2 + (\partial_2 f)\;\mathrm{d}B_t^2 + \frac{1}{2}(\nabla^2 f)\;\mathrm{d}t
$$
...
Because $K$ is simply connected and bounded, and $B_0 \notin K$, there is a path $\gamma = (x(t))_{t\in[0,1]}$ such that $x(0) = B_0$ and $x(1)$ is strictly separate from $K$ by a hyperplane $H$. Since $K$ is closed, there is some $\varepsilon > 0$ such that $\gamma_\varepsilon = \Set{x\in\R^d | \mathrm{dist}(x,\gamma) \leq\varepsilon}$ is disjoint from $K$. By the previous corollary, we get
...
$$
  \mathrm{d}f(B_t^1,\dots, B_t^d) = \sum_{k=1}^d (\partial_k f)\mathrm{d}B_t^k + \frac{1}{2}(\nabla^2 f)\;\mathrm{d}t
$$
...
$$
  f(x) = \int f(y)\;\mathrm{d}\sigma_r (y)
$$
...
$$
  f(B_{\tau_r}) = f(x) + \int_0^{\tau_r} \nabla f(B_s) \cdot \mathrm{d}B_s + \frac{1}{2}\int_0^\tau_r \nabla^2 f (B_s)\;\mathrm{d}s
$$

Assuming **(1)**, we get

$$
  \mathbb{E}(f(B_{\tau_r})) = f(x) + \mathbb{E}\left(\int_0^{\tau_r} \nabla f(B_s) \cdot \mathrm{d}B_s \right)
$$

Because the Brownian motion is invariant by rotations, the left hand side is $\int f(y)\;\mathrm{d}\sigma_r (y)$. Moreover, since $|\nabla f|$ is uniformly bounded on the ball $\mathcal{B}(x, r)$ with center $x$ and radius $r$, it follows that $X =\left(\int_0^{t\wedge\tau_r} \nabla f(B_s)\cdot \mathrm{d}B_s \right)_{t\geq 0}$ is a martingale with the expectation of its bracket uniformly bounded by
...
$$
  u(B_\tau) = u(x) + \int_0^\tau \nabla u(B_s)\cdot\mathrm{d}B_s + \frac{1}{2}\int_0^\tau \nabla^2 u(B_s)\;\mathrm{d}s
$$
...
For a Brownian motion $B$, assume that there is some $0 < \theta < \delta$ such that if $|z-x| < \theta$ for $z\in U$, then
...
\begin{align*}
  ...
  \leq& 2\lVert\varphi\rVert_\infty \mathbb{P}_z (\tau_\delta < \tau_C) + \varepsilon \\
  \leq& \varepsilon(2\lVert \varphi \rVert_\infty + 1)
\end{align*}
$$

Thus, continuity will follow if we can prove $\mathbb{P}_z (\tau_C \leq \tau_\delta) > 1 - \varepsilon$. By rotation invariance we can assume that $B_1$ corresponds to the axis of the cone, with angle $\alpha$. Note that
...
where $B$ is a Brownian motion beginning at $x$. Chose $x$ and $y$ distinct points in $\R^d$, write $\mathcal{S}^{(n)}$ the sphere with center $\frac{x+y}{2}$ and radius $n > \lVert \frac{x - y}{2} \rVert$, and $H$ the median hyperplane between $x$ and $y$. Define
...
$$
\begin{align*}
  f(Z_t) =& f(Z_0) + \int_0^t \partial z f(Z_s)\;\mathrm{d}Z_s \\
  &+ \int_0^t \partial_{\overline{z}} f(Z_s)\;\mathrm{d}\overline{Z}_s \\
  &+ \frac{1}{2} \int_0^t \nabla^2 f(Z_s) \;\mathrm{d}\langle \Re(Z) \rangle_s
\end{align*}
$$
...
$$
  f(Z_t) = f(Z_0) + \int_0^t f'(Z_s)\;\mathrm{d}Z_s
$$
...
where $B$ is a standard Brownian motion, and $\langle X \rangle_t := \int_0^t |f'(B_s)|^2 \;\mathrm{d}s$ is strictly increasing and converges to $\infty$.
...
where $X = \Re(f(B))$ and $\langle X \rangle_t = \int_0^t |f'(B_s)|^2 \;\mathrm{d}s$. Since $f'$ is entire and not identically $0$, it has a countable set of zeros so $\langle X \rangle$ is strictly increasing. It converges to $\infty$ almost surely due the recurrence property of the bi-dimensional Brownian motion.
</details>
</MathBox>
...
$$
  \tilde{B}_{int_0^t e^{2X_s} \;\mathrm{d}s} = e^{X_t + iY_t}
$$
...
$$
  \int_0^{\tau_t} e^{2X_s} \;\mathrm{d}s = t
$$
...
<MathBox title='' boxType='proposition'>
Assume that $\mathbb{Q}$ is absolutely continuous relative to $\mathbb{P}$, i.e. $\mathbb{Q} << \mathbb{P}$, on $\mathcal{F}_\infty$. For $t\in[0,\infty]$ define the process

$$
  D_t = \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F_t}}
$$

1. The process $D$ is a uniformly integrable martingale.
2. If $D$ is assumed right-continuous, then for any stopping time $T$
$$
  D_T = \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}_T}
$$
...
where $D = \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}}$. By uniqueness of the Radon-Nikodym derivative, this implies $D_t = \mathbb{E}(D|\mathcal{F}_t)$ almost surely, concluding the proof as $D\in L^1$.

**(2):** By the stopping time theorem $D_T = \mathbb{E}(D|\mathcal{F}_T)$. It follows that for any $A\in\mathcal{F}_T$

$$
\begin{align*}
  \mathbb{E}_{\mathbb{P}}\left( \mathbf{1}_A \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{Q}}\right|_{\mathcal{F}_T} \right) =& \mathbb{E}_{\mathbb{Q}}(\mathbf{1}_A) = \mathbb{E}_{\mathbb{P}}(\mathbf{1}_A D) \\
  =& \mathbb{E}_\mathbb{P} (\mathbf{1}_A D_T)
\end{align*}
$$

Hence $D_T = \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}_T}$.
...
$$
  L_t = \ln(D_0) + \int_0^t \frac{\mathrm{d}D_s}{D_s}
$$
...
$$
\begin{align*}
  \ln(D_t) =& \ln(D_0) + \int_0^t \frac{\mathrm{d}D_s}{D_s} - \frac{1}{2}\int_0^t \frac{\mathrm{d}\langle D \rangle_s}{D_s^2} \\
...
Let $\mathbb{Q}\sim\mathbb{P}$ on $\mathcal{F}$. Assume that the process defined by
...
We first prove that if a process $XD$ is a $\mathbb{P}$-local martingale, then $X$ is a $\mathbb{Q}$-local martingale. For this, we show that for any stopping time $T$ and adapted continuous process $X$ such taht $(XD)^T$ is a $\mathbb{P}$-martingale, then $X^T$ is a $\mathbb{Q}$-martingale.
...
$$
\begin{align*}
  \mathrm{d}((M - \langle M, L \rangle)D)_t =& (M - \langle M, L \rangle)_t \;\mathrm{d}D_t + (\mathrm{d}M_t + \mathrm{d}\langle M, L \rangle_t)D_t + \mathrm{d}\langle M, D \rangle_t \\
  =& (M - \langle M, L \rangle)_t \;\mathrm{d}D_t + (\mathrm{d}M_t + \mathrm{d}\langle M, L \rangle_t)D_t + D_t \;\mathrm{d}\langle M, L \rangle_t \\
  =& (M - \langle M, L \rangle)_t \;\mathrm{d}D_t + D_t \;\mathrm{d}M_t
\end{align*}
$$

where in the second equality we have used the fact that $\mathrm{d}D_t = D_t \;\mathrm{d}L_t$. Thus, $\tilde{M}D$ is a stochastic integral with respec to the $\mathbb{P}$-local martingales $D$ and $M$, so it is a $\mathbb{P}$-local martingale.
</details>
</MathBox>
...
$$
  \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}_t} = \exp\left(\nu B_t - \frac{\nu^2}{2}t \right)
$$
...
First, for a given $\varepsilon > 0$, there exists a continuous $f_0$, beginning at $0$, such that its $\varepsilon$-neighbourhood has a strictly positive probability (otherwise the measure would be null). We want to prove it for general $f$. By interpolation an up to choosing a smaller $\varepsilon$, we can suppose that $h := f - f_0$ is $C^1$. Applying the Girsanov theorem with $M = B$ and $L_t = \int_0^t \dot{h}_s \;\mathrm{d}B_s$ for a Brownian motion $B$, then $M - \langle M, L \rangle$ is $B - h$ and

$$
  \mathbb{P}(|B - f|_{L^{\infty}(0,1)} < \varepsilon) = \mathbb{E}_{\mathbb{P}}\left( \mathbf{1}_{|B - f_0|_{L^\infty (0,1)} < \varepsilon} \exp\left( \int_0^1 \dot{h}_s \;\mathrm{d}B_s - \frac{1}{2}\int_0^1 \dot{h}_s^2 \;\mathrm{d}s \right) \right)
$$

The right hand side is strictly positive because $\mathbb{P}(|B - f_0|_{L^\infty (0,1)} < \varepsilon) > 0$ and $\int_0^1 \dot{h}_s \;\mathrm{d}B_s > -\infty$ almost surely.
</details>
</MathBox>
...
$$
  \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}_t} = \exp\left( L_t - \frac{1}{2}\langle L \rangle_t \right)
$$
...
$$
  \left.\frac{\mathrm{d}\mathbb{Q}}{\mathrm{d}\mathbb{P}}\right|_{\mathcal{F}_t} = \mathcal{E}(L)_\infty
$$
...
$$
  \mathrm{d}X_t = \sigma(t, X)\;\mathrm{d}B_t + b(t, X) \mathrm{d}t 
$$

which can be decomposed into a deterministic drift term $b(t, X) \mathrm{d}t$, corresponding to an ordinary differential equation, and a random perturbation term $\sigma(t, X)\;\mathrm{d}B_t$ driven by a Wiener process $B_t$.
...
$$
  X_t = X_0 + \int_0^t \sigma(s, X)\;\mathrm{d}B_s + \int_0^t b(s, X)\;\mathrm{d}s
$$
