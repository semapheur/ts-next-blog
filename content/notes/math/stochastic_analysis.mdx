...
$$
  \mathrm{d}f(B_t^1,\dots, B_t^d) = \sum_{k=1}^d (\partial_k f)\mathrm{d}B_t^k + \frac{1}{2}(\Delta f)\;\mathrm{d}t
$$

For $f(x_1,\dots,x_d) = (x_1^2 +\cdots+ x_d^2)^{1-\frac{d}{2}}$, we get $\Delta f = 0$, so $|B_t|^{2-d}$ is a local martingale.
