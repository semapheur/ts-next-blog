...
<MathBox title='Convex affine subspace' boxType='definition'>
Let $(A,\vec{A},+)$ and $(B,\vec{B},+)$ be two affine spaces and $I\subseteq\N$ an index set. A function $f:A\to B$ is an *affine map* if and only if for every collection $((\mathbf{a}_i, \lambda_i))_{i\in I}$ of weighted points in $A$ such that $\sum_{i\in I} \lambda_i = 1$, we have
...
Let $(A,\vec{A},+)$ and $(B,\vec{B},+)$ be two affine spaces. If $h:\vec{A}\to\vec{B}$ is a linear map, the map f:A\to B$ defined by
...
Let $(A,\vec{A},+)$ and $(B,\vec{B},+)$ be two affine spaces. If $f:A\to B$ is an affine map, there is a unique linear map $\vec{f}:\vec{A}\to\vec{B}$ such that
