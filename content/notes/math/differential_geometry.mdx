...
<MathBox title='Parallel translation' boxType='theorem'>
...
This a first-order linear system of ODEs. By the existance and uniqueness theorem (Picard-Lindelöf theorem) for first-order ODEs, there is a unique solution on all of $I$ with any initial condition $\mathbf{V}(t_0) = \mathbf{V}_0$.
...
<MathBox title='Parallel translation operator' boxType='corollary'>
Let $M$ be a smooth manifold with an affine connection $\nabla$. If $y:I\subset\R\to M$ is a curve in $M$ and $a,b\in I$, parallel translation induces an isomorphism $P_{a,b}:T_{\gamma(a)}M \xrightarrow{\sim} T_{gamma(b)}M$ by setting $P_{a,b} \mathbf{V}_a = V(b)$ where $\mathbf{V}$ is the parallel translate of $\mathbf{V}_a \in T_{\gamma(a)} M$ along $\gamma$.
</MathBox>
...
where $P_{a,b}:T_{\gamma(a)}M \xrightarrow{\sim} T_{gamma(b)}M$ is the parallel translation operator.
</MathBox>
