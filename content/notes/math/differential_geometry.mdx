...
$$
\begin{equation}
  \nabla_\mathbf{X} \mathbf{Y} = (\mathbf{X}Y^k + \mathbf{X}^i \mathbf{Y}^k \Gamma_{ij}^k) \mathbf{E}_k \tag{\label{equation-2}}
\end{equation}
$$
...
<MathBox title='' boxType='proposition'>
Let $M$ be a smooth manifold. Then for any $f\in C^\infty (M)$ and $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$, the Hessian $\nabla^2 f : \mathfrak{X}(M) \times \mathfrak{X}(M) \to C^\infty (M)$ is given by
...
also satifes the three properties of the theorem. By the uniqueness of the covariant derivative, $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} = \frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t}$. This proves that the covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ is independent of the frame. By covering $M$ with framed open sets, $\eqref{equation-4}$ then defines a covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ for the curve $c(t)$ in $M$.
</details>
</MathBox>
