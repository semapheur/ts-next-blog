...
$$
  \nabla_{E_i} \mathbf{E_j} = \Gamma_{ij}^k \mathbf{E}_k,\, \forall i, j=1,\dots,n \tag{\label{equation-5}}
$$
...
$$
  \nabla_\mathbf{X} \mathbf{Y} = (\mathbf{X}Y^k + \mathbf{X}^i \mathbf{Y}^k \Gamma_{ij}^k) \mathbf{E}_k \tag{\label{equation-1}}
$$
...
$$
  \nabla_\mathbf{X} \mathbf{Y} = (X^i \frac{\partial Y^k}{\partial x^i} + X^i Y^j \Gamma_{ij}^k)\frac{\partial}{\partial x^k} \tag{\label{equation-2}}
$$

<details>
<summary>Proof</summary>

Note that $\eqref{equation-2}$ is equivalent to $\eqref{equation-1}$ when $E_i = \partial/\partial x^i$ is a coordinate frame, so for every connection the Christoffel symbols $\Set{\Gamma_{ij}^k}$ defined by $\eqref{equation-5}$ satisfy $\eqref{equation-2}$. Moreover, given $\Set{\Gamma_{ij}^k}$, it is straightforward to check that $\eqref{equation-2}$ satisfy the affine connection properties.
</details>
</MathBox>
...
also satifes the three properties of the theorem. By the uniqueness of the covariant derivative, $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} = \frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t}$. This proves that the covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ is independent of the frame. By covering $M$ with framed open sets, $\eqref{equation-3}$ then defines a covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ for the curve $c(t)$ in $M$.
</details>
</MathBox>
...
$$
  f\circ \ell_\mathbf{h} = \ell_{f(\mathbf{h})}\circ f \tag{\label{equation-4}}
$$
...
We will now show that $(f\circ c)(t)$ is an integral curve of $\tilde{f_* \mathbf{A}}$, the left-invariant vector field on $G$ generated by $f_* \mathbf{A}\in\mathcal{g}$. Taking the differential of $\eqref{equation-4}$ we get
