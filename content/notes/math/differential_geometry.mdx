...
Let $\nabla$ be an affine connection on a smooth manifold $M$. The *torsion* $T:\mathfrak{X}(M)\times\mathfrak{X}(M)\to\mathfrak{X}(M)$ is a $\binom{2}{1}$-tensor field on $M$ given in local coordinates by

$$
  T = (\Gamma_{ij}^k - \Gamma_{ji}^k) \mathrm{d}x^i \otimes \mathrm{d}x^j \otimes \frac{\partial}{\partial x^k}
$$
...
Let $\mathbf{X} = \sum_{i=1}^n X^i \boldsymbol{\partial}_i$ and $\mathbf{Y} = \sum_{i=1}^n Y^i \boldsymbol{\partial}_i$ be the coordinate expressions for the smooth vector fields $\mathbf{X}, \mathbf{Y}\in\mathfrak{X}(M)$ in terms of some local chart $(U, (x^i)_{i=1}^n)$.
...
Then the torsion $T(\mathbf{X}, \mathbf{Y})$ has the following coordinate expression

$$
\begin{align*}
  T(\mathbf{X},\mathbf{Y}) =& (\mathbf{X}Y^k + X^i Y^j \Gamma_{ij}^k) \frac{\partial}{\partial x^k} - (\mathbf{Y}X^k + Y^i X^j \Gamma_{ij}^k) \frac{\partial}{\partial x^k} \\
  &- (\mathbf{X}Y^k - \mathbf{Y}X^k)\frac{\partial}{\partial x^k} \\
  =& (X^i Y^j \Gamma_{ij}^k - Y^i X^j \Gamma_{ij}^k)\frac{\partial}{\partial x^k} \\
  =& X^i Y^j (\Gamma_{ij}^k - \Gamma_{ji}^k)\frac{\partial}{\partial x^k}
\end{align*}
$$

This shows that $T$ is a $\binom{2}{1}$-tensor field on $M$.
