...
Similarly it can be shown that $R$ is linear over $C^\infty (M)$ in $\mathbf{Z}$. This shows that $R$ is a tensor field. In local coordinates, the components of $R$ are given by
