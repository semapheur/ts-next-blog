...
As a submanifold of $\R^3$, it follows that $M$ inherits a Riemannian metric from the Euclidean metric on $\R^3$. Let $B = \nabla f = \langle f_x, f_y, f_< \rangle$ be the gradient vector field of $f$ on $M$.

<MathBox title='' boxType='proposition'>
If $0$ is a regular value of the smooth function $f$ on $\R^3$, then $N = \nabla f$ is a nowhere-vanishing normal vector field along the smooth hypersurface $M = Z(f)$.
...
$$
  \nabla\kappa = \begin{bmatrix} 2ax + 2by \\ 2bx + 2cy \end{bmatrix} = 2\mathbf{A}X_\mathbf{p}
$$

and the gradient of $f$ is $\nabla g = 2X_\mathbf{p}$. By the method of the Lagrange multiplies, at the maximum or minimum of $\kappa$, the is a scalar $\lambda$ such that
