...
$$
  \nabla_{f\mathbf{X}_1 + g\mathbf{X}_2} \mathbf{Y} = f\nabla_{\mathbf{X}_1} \mathbf{Y} + g\nabla_{\mathbf{X}_2} \mathbf{Y},\, f,g\in C^\infty (M)
$$
2. $\nabla_\mathbf{X} \mathbf{s}$ is $\R$-linear in $\mathbf{Y}$
$$
  \nabla_\mathbf{X} (a\mathbf{Y}_1 + b\mathbf{Y}_2) = a\nabla_\mathbf{X} \mathbf{Y}_1 + b\nabla_\mathbf{X} \mathbf{Y}_2,\, a,b\in\R
$$
2. **Leibniz rule:**
...
$$
  \nabla_{E_i} \mathbf{E_j} = \Gamma_{ij}^k \mathbf{E}_k,\, \forall i, j=1,\dots,n
$$
...
Let $M$ be a smooth $n$-manifold, $\nabla$ an affine connection on $M$ and $\mathbf{X}, \mathbf{Y}\in\mathfrak{X}(M)$. If $\mathbf{X} = \sum_i^n X^i \mathbf{E}_i$ and $\mathbf{Y} = \sum_{j=1}^n Y^j \mathbf{E}_j$ for a local frame $\Set{\mathbf{E}_i}_{i=1}^n$ on $TM$, then

$$
  \nabla_\mathbf{X} \mathbf{Y} = (\mathbf{X}Y^k + \mathbf{X}^i \mathbf{Y}^k \Gamma_{ij}^k) \mathbf{E}_k
$$
...
$$
\begin{align*}
  \nabla_\mathbf{X} \mathbf{Y} =& \nabla_\mathbf{X} (n Y^j \mathbf{E}_j) \\
  ...
  =& (\mathbf{X}Y^k + \mathbf{X}^i \mathbf{Y}^k \Gamma_{ij}^k) \mathbf{E}_k
\end{align*}
$$
</details>
</MathBox>
