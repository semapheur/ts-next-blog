...
<MathBox title='' boxType='proposition'>
If an affine connection $\nabla$ on a Riemannian manifold $M$ is compatible with the metric, then for vector fields $\mathbf{X},\mathbf{Y},\mathbf{Z},\mathbf{W}\in\mathfrak{X}(M)$, the tensor $\langle R(\mathbf{X},\mathbf{Y})\mathbf{Z},\mathbf{W}\rangle$ is skew-symmetric in $\mathbf{Z}$ and $\mathbf{W}$.
...
Let $\nabla$ be an affine connection on a smooth manifold $M$ and let $(U,\Set{x^i}_{i=1}^n)$ be a coordinate open set in $M$. Then $\nabla_{\partial/\partial x^i} \frac{\partial}{\partial x^j}$ is a linear combination of $\left(\frac{\partial}{\partial x^i}\right)$, so there exist numbers $\Gamma_{ij}^k$ at each point such that

$$
  \nabla_{\partial/\partial x^i} = \sum_{k=1}^n \Gamma_{ij}^k \frac{\partial}{\partial x^k}
$$
...
$$
  \nabla_{\partial/\partial x^i} \frac{\partial}{\partial x_j} - \nabla_{\partial/\partial x^j} \frac{\partial}{\partial x_i} =  \left[\frac{\partial}{\partial x^i}, \frac{\partial}{\partial x^i} \right] = 0
$$
...
The Poincaré half-plane is covered by a singe coordinate open set $(U,x^1,x^2)$. We showed in a previous example that its connection form $\omega_2^1$ relative to the orthonormal frame $\mathbf{e}_i = x^2 \frac{\partial}{\partial x^i}$ for $i=1,2$ is
