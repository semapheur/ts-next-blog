...
1. $\nabla_\mathbf{X} \mathbf{Y}$ is $\mathcal{F}$-linear in $\mathbf{X}$.
...
<MathBox title='' boxType='proposition'>
Let $\nabla$ be a connection and $\mathbf{X},\mathbf{Y}\in\mathfrak{X}(M)$ be smooth vector fields on a smooth manifold $M$.
1. If $\omega$ is a smooth $1$-form on $M$, then $\nabla_\mathbf{X}\omega$ defined by
$$
  (\nabla_\mathbf{x}\omega)(\mathbf{Y}) := \mathbf{X}(\omega(\mathbf{Y})) - \omega(\nabla_\mathbf{X}\mathbf{Y})
$$
is a smooth $1$-form on $M$.
2. If $T$ is a smooth $(a,b)$-tensor field on $M$, then $\nabla_\mathbf{X} T$ defined by
$$
\begin{align*}
  (\nabla_\mathbf{X}T)(\omega_1,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b) :=& \mathbf{X}(T(\omega_1,\dots,\omega_a,\mathbf{Y}_1),\dots,\mathbf{Y}_b) \\
  &- \sum_{i=1}^a T(\omega_1,\dots,\nabla_\mathbf{X} \omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b) \\
  &- \sum_{j=1}^b T(\omega_1,\dots,\omega_a,\mathbf{Y}_1,\dots,\nabla_\mathbf{X} \mathbf{Y}_j,\dots,\mathbf{Y}_b)
\end{align*}
$$
for $\omega_i\in\Omega^1 (M)$, $\mathbf{Y}_j\in\mathfrak{X}(M)$, is a smooth $(a,b)$-tensor field on $M$.

<details>
<summary>Proof</summary>

By the tensor criterion, it suffices to check that $\nabla_\mathbf{X} \omega$ and $\nabla_\mathbf{X} T$ are $\mathcal{F}$-linear in its arguments, where $\mathcal{F} := C^\infty (M)$ is the ring of smooth function on $M$. Let $f\in\mathcal{F}$.

**(1):** By definition and the Leibniz rule in $\mathbf{Y}$ of a connection
...
**(2):** Replace $\omega_i$ by $f\omega_i$ in the definition of $\nabla_\mathbf{X}$ T. On the right-hand side, we have

$$
\begin{align*}
  \mathbf{X}(T(\omega_1,\dots,f\omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b)) =& (\mathbf{X}f)T(\omega_1,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b) \\
  &+ f\mathbf{X}(T(\omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b))
\end{align*}
$$
...
$$
\begin{align*}
  &T(\omega_1,\dots,\nabla_\mathbf{X} f\omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b) \\
  =& T(\omega_1,\dots,(\mathbf{X}f)\omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b) \\
  &+ fT(\omega_1,\dots,\nabla_\mathbf{X} \omega_i,\dots,\omega_a,\mathbf{Y}_1,\dots,\mathbf{Y}_b)
\end{align*}
$$

The first terms of these two expressions cancel out. All the other terms on the right of the expressions are $\mathcal{F}$-linear in the $i$th argument.

Similary, the right-hand side of the definition of $\nabla_\mathbf{X} T$ is $\mathcal{F}$-linear in the argument $\mathbf{Y}_j$.
</details>
</MathBox>
...
$$
  T^{a,b}(M) = \left(\bigotimes_{i=1}^a TM \right)\otimes\left(\bigotimes_{j=1}^b T^* M \right)
$$

be the bundle of $(a,b)$-tensors on $M$. Suppose $\nabla$ is a connection on a smooth manifold and $\mathbf{X}\in\mathfrak{X}(M)$. The covariant derivative $\nabla_\mathbf{X}: \Gamma(\bigoplus_{a,b=0}^\infty T^{a,b} (M)) \to \Gamma(\bigoplus_{a,b=0}^\infty T^{a,b} (M))$, satisfies the product rule for any two tensor fields $T_1, T_2$
...
As for existence, we define $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ for a curve $c$ in a framed open set $U$ by the formula above. It is easily verified that $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ satisfiess the three properties of the theorem. Hence, $\frac{\mathrm{D}}{\mathrm{d}t}$ exists for curves in $U$. If $\Set{\bar{\mathbf{e}}_i}_{i=1}^n$ is another frame on $U$, then $\mathbf{V}(t)$ is a linear combination $\sum_{i=1}^n \bar{v}^i (t) \bar{\mathbf{e}}_{i,c(t)}$ and the covariant derivative $\frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t}$ defined by

$$
  \frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t} = \sum_{i=1}^n \frac{\mathrm{d}\bar{v}^i}{\mathrm{d}t}\mathbf{e}_{i,c(t)} + \bar{v}^i \nabla_{c'(t)}\mathbf{e}_{i,c(t)}
$$

also satifes the three properties of the theorem. By the uniqueness of the covariant derivative, $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} = \frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t}$. This proves that the covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ is independent of the frame. By covering $M$ with framed open sets, $\eqref{1}$ then defines a covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ for the curve $c(t)$ in $M$.
</details>
</MathBox>

<MathBox title='' boxType='theorem'>
Let $M$ be a Riemannian $n$-manifold, $\nabla$ an affine connection on $M$, and $c:[a,b]\to M$ a smooth curve in $M$. If $\nabla$ is metric compatible, then for any smooth vector fields $\mathbf{V},\mathbf{W}$ along $c$
...
$$
\begin{align*}
  \left\langle \frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t},\mathbf{W} \right\rangle + \left\langle V,\frac{\mathrm{D}\mathbf{W}}{\mathrm{d}t} \right\rangle =& \sum_{i=1}^n \frac{\mathrm{d}v^i}{\mathrm{d}t}w^i + \sum_{i=1}^n v^i \frac{\mathrm{d}w^i}{\mathrm{d}t}\mathbf{e}_{j,c(t)} \\
...
Let $\nabla$ be an affine connection on a smooth manifold $M$ and let $(U,\Set{x^i}_{i=1}^n)$ be a coordinate open set in $M$. Then $\nabla_{\boldsymbol{\partial}_i} \frac{\partial}{\partial x^j}$ is a linear combination of $\left(\frac{\partial}{\partial x^i}\right)$, so there exist numbers $\Gamma_{ij}^k$ at each point such that

$$
  \nabla_{\boldsymbol{\partial}_i} = \sum_{k=1}^n \Gamma_{ij}^k \frac{\partial}{\partial x^k}
$$

where the $n^3$ functions $\Gamma_{ij}^k$ are the *Christoffel symbols* of the connection $\nabla$ on the coordinate open set $(U,x^1,\dots,x^n)$. By the Leibniz rule and $\mathcal{F}$-linearity in the first argument of a connection, the Christoffel symbols completely describe a connection on $U$.

<MathBox title='' boxType='proposition'>
An affine connection $\nabla$ on a manifold is torsion-free if and only if in every coordinate chart $(U,\Set{x^i}_{i=1}^n)$ the Christoffel symbol $\Gamma_{ij}^k$ is symmetric in $i$ and $j$
...
<MathBox title='Christoffel symbols for the Poincaré half-plane' boxType='example'>
The Poincaré half-plane $\mathbb{H}^2$ is covered by a single coordinate open set $(U,x^1,x^2)$. We showed in a previous example that its connection form $\omega_2^1$ relative to the orthonormal frame $\mathbf{e}_i = x^2 \frac{\partial}{\partial x^i}$ for $i=1,2$ is
...
<MathBox title='Geodesic equations' boxType='theorem'>
On a smooth manifold $M$ with a connection $\nabla$, a parametrized curve $c(t)$ is a geodesic if and only if relative to any chart $(U,\phi) = (U,\Set{x^i}_{i=1}^n)$, the components of $(\phi\circ c)(t) = (y^1 (t),\dots,y^n(t))$ satisfy the system of differential equations

$$
  \ddot{y}^k + \sum_{i,j=1}^n \Gamma_{ij}^k \dot{y}^i \dot{y}^j = 0,\; k = 1,\dots,n
$$

where the $\Gamma_{ij}^k$ are evaluated on $c(t)$.

<details>
<summary>Proof</summary>

Suppose $M$ is a smooth manifold with a connection $\nabla$, and $(U,\phi) = (U,\Set{x^i}_{i=1}^n)$ is a chart relative to which the Christoffel symbols are $\Gamma_{ij}^k$. Let $c:[a,b]\to M$ be a smooth curve. In the chart $(U,phi)$, the curve $c$ has coordinates

$$
  (\phi\circ c)(t) = (x^1(c(t)),\dots,x^n(c(t)))
$$

We will write $y(t) = (\phi\circ c)(t)$ and $y^i (t) = (x^i \circ c)(t)$ and say that $c$ is given in $U$ by

$$
  y(t) = (y^1 (t),\dots,y^n(t))
$$

Let $\dot{y}^i = \frac{\mathrm{d}y^i}{\mathrm{d}t}$ and $\ddot{\gamma}^i = \frac{\mathrm{d}^2 y^i}{\mathrm{d}t^2}$. To simplify notation, let $\partial_{i,c(t)} = \left.\frac{\partial}{\partial x^i}\right|_{c(t)}$. Then

$$
  \mathbf{T}(t) = c'(t) = \sum_{j=1}^n \dot{y}^j \left.\frac{\partial}{\partial x^i}\right|_{c(t)}
$$
...
$$
\begin{align*}
  \frac{\mathrm{D}\mathbf{T}}{\mathrm{d}t}(t) =& \sum_{j=1}^n \ddot{y}^j \left.\frac{\partial}{\partial x^j}\right|_{c(t)} + \sum_{j=1}^n \dot{y}^j \nabla_{c'(t)} \frac{\partial}{\partial x^j} \\
  =& \sum_{j=1}^n \ddot{y}^j \left.\frac{\partial}{\partial x^j}\right|_{c(t)} + \sum_{i,j=1}^n \dot{y}^j \nabla_{\dot{y}^i \partial_{i,c(t)}} \frac{\partial}{\partial x^j} \\
  =& \sum_{k=1}^n \ddot{y}^k \left.\frac{\partial}{\partial x^k}\right|_{c(t)} + \sum_{i,j,k=1}^n \dot{y}^i \dot{y}^j \Gamma_{ij^k} \frac{\partial}{\partial x^k} \\
  =& \sum_{k=1}^n \left(\ddot{y}^k + \sum_{i,j=1}^n \dot{y}^i \dot{y}^j \Gamma_{ij}^k \right)\left.\frac{\partial}{\partial x^k}\right|_{c(t)}
\end{align*}
$$

So $c(t)$ is a geodesic if and only if

$$
  \dot{\gamma}^k + \sum_{i,j=1}^n \Gamma_{ij}^k \dot{y}^i \dot{y}^j = 0,\; k=1,\dots,n
$$
</details>
</MathBox>

<MathBox title='' boxType='theorem'>
Let $M$ be a smooth manifold with a connection $\nabla$. Given any point $\mathbf{p}\in M$ and tangent vector $\mathbf{v}\in T_\mathbf{p}M$, there is a geodesic $c(t)$ in $M$ with initial point $c(0) = \mathbf{p}$ and initial velocity $c'(0) = \mathbf{v}$. Moreover, this geodesic is unique in the sense that any other geodesic satisfying the same initial conditions must agree with $c(t)$ on the intersection of their domains.
</MathBox>
...
Suppose $c(t)$ is a geodesic in $M$, with tangent vector field $\mathbf{T}(t) = c'(t)$. If $\tilde{\mathbf{T}}(t) = (f\circ c)'(t)$ is the tangent vector field of $f\circ c$, then

$$
  \tilde{\mathbf{T}}(t) = f_* \left(c_* \frac{\mathrm{d}}{\mathrm{d}t} \right) = f_* (c'(t)) = f_* \mathbf{T}
$$

Let $\frac{\mathrm{D}}{\mathrm{d}t}$ and $\frac{\tilde{\mathrm{D}}}{\mathrm{d}t}$ be the covariant derivatives along $c(t)$ and $(f\circ c)(t)$, respectively. Then $\frac{\mathrm{D}T}{\mathrm{d}t} \equiv 0$, because $c(t)$ is a geodesic. If follows that

$$
  \frac{\tilde{\mathrm{D}}\tilde{\mathbf{T}}}{\mathrm{d}t} = f_* \left(\frac{\mathrm{D}\mathbf{T}}{\mathrm{d}t}\right) = f_* 0 = 0
$$

Hence, $f\circ c$ is a geodesic in $M$.
</details>
</MathBox>
...
Let $M$ be a smooth manifold with a connection $\nabla$ and $c:I\subseteq\R\to M$ is a smooth curve. A smooth vector field $\mathbf{V}(t)$ along $c$ is *parallel* if $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}\equiv 0$ on $I$.
</MathBox>
...
Assume that $M$ is endowed with a Riemannian metric. For any point $\mathbf{p}\in M$, there exist a neigbourhood $U$ of $\mathbf{p}$ and a real number $\varepsilon > 0$ such taht for all $\mathbf{q}\in U$ then exponential map $\exp_\mathbf{q}$ is defined on the open ball $B_\varepsilon (0)$ in $T_\mathbf{q}M$
...
$$
  f\circ \ell_\mathbf{h} = \ell_{f(\mathbf{h})}\circ f \tag{\label{2}}
$$
...
We will now show that $(f\circ c)(t)$ is an integral curve of $\tilde{f_* \mathbf{A}}$, the left-invariant vector field on $G$ generated by $f_* \mathbf{A}\in\mathcal{g}$. Taking the differential of $\eqref{2}$ we get
