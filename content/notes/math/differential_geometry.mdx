...
\begin{equation}
\begin{split}
  \frac{\mathrm{D}}{\mathrm{d}t}(t_0) =& \sum_{i=1}^n \frac{\mathrm{D}}{\mathrm{d}t} \left(V^i(t) \frac{\partial}{\partial x^i}\right) \\
  =& \sum_{i=1}^n \frac{\mathrm{d}V^i}{\mathrm{d}t}(t_0) \frac{\partial}{\partial x^i} + V^i(t_0) \frac{\mathrm{D}}{\mathrm{d}t}\left(\frac{\partial}{\partial x^i} \right) \\
  =& \sum_{i=1}^n \dot{V}^i(t_0) \frac{\partial}{\partial x^i} + V^i(t_0) \nabla_{\gamma'(t_0)} \frac{\partial}{\partial x^i} \\
...
<MathBox title='Geodesic equations' boxType='proposition'>
Let $M$ be a smooth $n$-manifold with an affine connection $\nabla$, and suppose $\gamma:I\subset\R \to M$ is a smooth curve in $M$. If $(U, \varphi = (x^i)_{i=1}^n)$ is a chart about $\gamma(t)\in M$ for any $t \in I$, then $\gamma$ is geodesic if and only if the components of $(\varphi\circ\gamma)(t) = (y^i = x^i(\gamma(t)))_{i=1}^n$ satisfy the system of differential equations
...
<MathBox title='Existence and uniqueness of geodesics' boxType='theorem'>
