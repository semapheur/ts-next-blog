...
also satifes the three properties of the theorem. By the uniqueness of the covariant derivative, $\frac{\mathrm{D}V}{\mathrm{d}t} = \frac{\bar{\mathrm{D}}V}{\mathrm{d}t}$. This proves that the covariant derivative $\frac{\mathrm{D}V}{\mathrm{d}t}$ is independent of the frame. By covering $M$ with framed open sets, $\eqref{eq-1}$ then defines a covariant derivative $\frac{\mathrm{D}V}{\mathrm{d}t}$ for the curve $c(t)$ in $M$.
