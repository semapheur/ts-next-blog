...
As a submanifold of $\R^3$, it follows that $M$ inherits a Riemannian metric from the Euclidean metric on $\R^3$. Let $B = \opertorname{grad}(f) = \langle f_x, f_y, f_< \rangle$ be the gradient vector field of $f$ on $M$.

<MathBox title='' boxType='proposition'>
If $0$ is a regular value of the smooth function $f$ on $\R^3$, then $N = \opertorname{grad}(f)$ is a nowhere-vanishing normal vector field along the smooth hypersurface $M = Z(f)$.
...
$$
  \opertorname{grad}(k)appa = \begin{bmatrix} 2ax + 2by \\ 2bx + 2cy \end{bmatrix} = 2\mathbf{A}X_\mathbf{p}
$$

and the gradient of $f$ is $\opertorname{grad}(g) = 2X_\mathbf{p}$. By the method of the Lagrange multiplies, at the maximum or minimum of $\kappa$, the is a scalar $\lambda$ such that

$$
  \opertorname{grad}(k)appa = \lambda\opertorname{grad}(f)
$$
...
Given an open subset $U$ of a smooth manifold $M$, we can think of $U\times\R^r$ as a family of vector spaces $\R^r$ parametrized by the points in $U$. Intuitively, a vector bundle is a family of vector spaces that locally resemble $U\times\R^r$

<MathBox title='Smooth vector bundle' boxType='definition'>
Let $U$ be an open subset of a smooth manifold $M$. A smooth surjection $\pi:E\to M$ is a smooth *vector bundle* of rank $r$ if
1. for every $\mathbf{p}\in M$, the set $E_\mathbf{p} := \pi^{-1}(\mathbf{p})$ is a real vector space of dimension $r$
2. every point $\mathbf{p}\in M$ has an open neighbourhood $U$ such that there is a fibre-preserving diffeomorphism
$$
  \phi_U : \pi^{-1}(U)\to U\times\R^r
$$
that restricts to a linear isomorphism $E_\mathbf{p}\to\Set{\mathbf{p}}\times\R^r$ on each fiber.

The space $E$ is called the *total space*, the space $M$ the *base space*, and the space $E_\mathbf{p}$ the *fibre* above $\mathbf{p}$ of the vector bundle. We often say that $E$ is a *vector bundle* over $M$. A vector bundle of rank $1$ is also called a *line bundle*.

Condition **(1)** says taht $\pi:E\to M$ is a family of vector spaces, while condition **(2)** formalizes the fact that this family locally looks like $\R^n$. We call the open set $U$ in **(2)** a *trivializing open subset* for the vector bundle, and $\phi_U$ a *trivialization* of $\pi^{-1}(U)$. A *trivializing open cover* for the vector bundle is an open cover $\Set{U_\alpha}$ of $M$ consisting of trivializing open sets $U_\alpha$ together with trivializations $\phi_\alpha:\pi^{-1} (U_\alpha)\to U_\alpha \times\R^r$.
</MathBox>

<MathBox title='Smooth bundle map' boxType='definition'>
Let $\pi_E: E\to M$ and $\pi_F: F\to N$ be smooth vector bundles. A *smooth bundle map* from $E$ to $F$ is a pair of smooth maps $(\varphi_E\to F, \underline{\varphi}: M\to N)$ such that the diagram commutes

$$
\begin{CD}
  E @>{\varphi}>> F \\
  @V{\pi_E}VV @VV{\pi_F}V \\
  M @>>{\underline{\varphi}}> N
\end{CD}
$$

and where $\varphi$ restricts a linear map $\varphi_\mathbf{p}:E_\mathbf{p}\to F_{\underline{\varphi}(\mathbf{p})}$ of fibres for each $\mathbf{p}\in M$.

When $E$ and $F$ are vector bundles over the same manifold $M$ and the base map $\underline{\varphi}$ is the identity map $\operatorname{id}_M$, the bundle map $(\varphi:E\to F, \operatorname{id}_M)$ is a bundle map over $M$.

If there is a bundle map $\psi:F\to E$ over $M$ such that $\psi\circ\varphi = \operatorname{id}_E$ and $\varphi\circ\psi = \operatorname{1}_F$, then $\varphi$ is called a *bundle isomorphism* over $M$, and the vector bundles $E$ and $F$ are *isomorphic* over $M$.
</MathBox>

<MathBox title='Trivial vector bundle' boxType='definition'>
A vector bundle $\pi:E\to M$ is *trivial* if it is isomorphic to a product bundle $M\times\R^r \to M$ over $M$.
</MathBox>
...
### $\mathcal{F}$-linearity and bundle maps
...
$$
  \opertorname{grad}(f) = \left( \opertorname{grad}(f) \right)^i \frac{\partial}{\partial x^i}
$$
...
$$
\begin{align*}
  \mathrm{d}f = \opertorname{grad}(f) \cdot \_ &= g\left( \opertorname{grad}(f), \_ \right) \\ 
  &= \left[ g_{ij} \left( \mathrm{d}x^i \otimes \mathrm{d}x^j \right) \right] \left[ \left( \opertorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} \right] \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^k \mathrm{d}x^i \left( \frac{\partial}{\partial x^k} \right) \mathrm{d}x^j \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^k \delta_k^i \mathrm{d}x^j \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^i  \mathrm{d}x^j
\end{align*}
$$
...
$$
  \frac{\partial f}{\partial x^j} = \left( \opertorname{grad}(f) \right)^i g_{ij}
$$
...
$$
\begin{align*}
  \frac{\partial f}{\partial x^j} \mathfrak{g}^{jk} &= \left( \opertorname{grad}(f) \right)^i g_{ij} \mathfrak{g}^{jk} \\
  &= \left( \opertorname{grad}(f) \right)^i \delta_i^k \\
  &= \left( \opertorname{grad}(f) \right)^k &= \frac{\partial f}{\partial x^j} \mathfrak{g}^{jk}
\end{align*}    
$$
...
$$
  \opertorname{grad}(f) = \left( \opertorname{grad}(f) \right)^i \frac{\partial}{\partial x^k} = \mathfrak{g}^{jk} \frac{\partial f}{\partial x^j} \frac{\partial}{\partial x^k} 
$$

For a Cartesian basis $g_{ij} = \mathfrak{g}_{ij} = \delta_{ij}$ and the gradient simplifies to

$$
  \opertorname{grad}(f) = \frac{\partial f}{\partial x^i} \frac{\partial}{\partial x^i}
$$
...
$$
  \mathrm{d}f (\mathbf{v}) \equiv \opertorname{grad}(\)mathbf{v} f = \lim_{h \to 0} \frac{f(\mathbf{x} + \mathbf{h} \cdot \mathbf{v}) - f(\mathbf{x})}{\mathbf{h}} = \mathbf{v} \cdot \opertorname{grad}(f)
$$

This implies that the gradient (vector) $\opertorname{grad}(f)$ and the differential form (covector) $\mathrm{d}f$ are dual pairs

$$
\begin{align*}
  \mathrm{d}f(\mathbf{v}) = \opertorname{grad}(f) \cdot \mathbf{v} &= \left[ \left( \opertorname{grad}(f)\right)^i \frac{\partial}{\partial x^i} \right] \cdot \left(v^j \frac{\partial}{\partial x^j} \right) \\
  &= \left( \opertorname{grad}(f) \right)^i v^j \left( \frac{\partial}{\partial x^i} \cdot \frac{\partial}{\partial x^j} \right) \\
  &= \left( \opertorname{grad}(f) \right)^i v^j g_{ij}
\end{align*}
$$
...
$$
\begin{align*}
  \opertorname{grad}(f) \cdot \mathbf{v} &= g\left(\opertorname{grad}(f), \mathbf{v} \right) \\
  &= \left[ g_{ij} \left( \mathrm{d}x^i \otimes \mathrm{d}x^j \right) \right] \left[ \left( \opertorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} , v^l \frac{\partial}{\partial x^l} \right] \\
  &= g_{ij} \mathrm{d}x^i \left[ \left( \opertorname{grad}(f) \right)^k \frac{\partial}{\partial x^k} \right] \mathrm{d}x^j \left( v^l \frac{\partial}{\partial x^l} \right) \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^k v^l \mathrm{d}x^i \left( \frac{\partial}{\partial x^k} \right) \mathrm{d}x^j \left( \frac{\partial}{\partial x^l} \right) \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^k v^l \delta_k^i \delta_l^j \\
  &= g_{ij} \left( \opertorname{grad}(f) \right)^i v^j
\end{align*}
$$