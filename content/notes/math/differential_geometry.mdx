...
$$
  T = ({\Gamma_{ij}}^k - {\Gamma_{ji}}^k) \;\boldsymbol{\partial}_k \otimes \mathrm{d}x^i \otimes \mathrm{d}x^j
$$
...
$$
  R = {R_{ijk}}^l \;\boldsymbol{\partial}_l \otimes \mathrm{d}x^i \otimes \mathrm{d}x^j \otimes \mathrm{d}x^k 
$$
...
$$
  \nabla_\mathbf{X}\omega = \left(X^i \boldsymbol{\partial}_i \omega_k - X^i \omega_j \Gamma_{ik}^j \right)\;\mathrm{d}x^k
$$
</MathBox>
...
$$
  \frac{\mathrm{D}}{\mathrm{d}t} (a\mathbf{V} + b\mathbf{W}) = a\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} + b\frac{\mathrm{D}\mathbf{W}}{\mathrm{d}t}
$$
...
$$
\begin{align*}
  (\mathrm{D}_{f\mathbf{X}} \mathbf{Y})_\mathbf{p} =& \mathrm{D}_{f(\mathbf{p})\mathbf{X_p}} \mathbf{Y} \\
...
\begin{align*}
  ...
  =& (\mathbf{X_p} f) \mathbf{Y}_\mathbf{p} + f(\mathbf{p})\mathrm{D}_{\mathbf{X_p}}\mathbf{Y} \\
...
$$
  \mathrm{D}_\mathbf{X} \mathbf{Y} - \mathrm{D}_\mathbf{Y} \mathbf{X} = [\mathbf{X}, \mathbf{Y}] = \mathcal{L}_\mathbf{X} \mathbf{Y}
$$
...
$$
\begin{align*}
  \mathrm{D}_{c'(t)} \tilde{\mathbf{V}} =& \sum_{i=1}^n (c'(t)\tilde{v}^i) \left.\frac{\partial}{\partial x^i}|_{c(t)} \\
...
$$
\begin{align*}
  \dot{\tilde{\gamma}}^i (t) =& \frac{\mathrm{d}t}{\mathrm{d}t} \gamma^i (kt) \\
  =& k\dot{\gamma}^i (kt)
\end{align*}
$$
...
Let $\mathrm{D}_t$ and $\tilde{D}_t$ denote the covariant differentiation operators along $\gamma$ and $\tilde{\gamma}$, respectively. Using the chain rule again in local coordinates

$$
\begin{align*}
  \tilde{\mathrm{D}}_t \dot{\tilde{\gamma}}(t) =& \left(\frac{\mathrm{d}}{\mathrm{d}t} \dot{\tilde{\gamma}}^k (t) + \Gamma_{ij}^k (\tilde{\gamma}(t) \dot{\tilde{\gamma}})^i (t) \dot{\tilde{\gamma}}^j (t) \right)\boldsymbol{\partial}_k \\
  =& (k^2 \ddot{\gamma}_\mathbf{v}(kt) + k^2 \Gamma_{ij}^k (\gamma_\mathbf{v}(kt))) \dot{\gamma}^i (kt) \dot{\gamma}^j (kt) \boldsymbol{\partial}_k \\
  =& k^2 \mathrm{D}_t \dot{\gamma}_\mathbf{v} (kt) = 0
\end{align*}
$$
...
Let $M$ be a smooth manifold with an affine connection $\nabla$. A smooth vector field $\mathbf{V}\in\mathfrak{X}(M)$ along a curve $\gamma:I\subset\R\to M$ is *parallel* along $\gamma$ with respect to $\nabla$ if $\mathrm{D}_t \mathbf{V} \equiv 0$. A vector field $\mathbf{V}\in\mathfrak{X}(M)$ on $M$ is *parallel* if it is parallel along every curve.
</MathBox>
...
$$
\begin{align*}
  \frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} = \left(\dot{V}^k(t) + V^j(t) \dot{\gamma}^i(t) \Gamma_{ij}^k (\gamma(t)) \right)\boldsymbol{\partial}_k|_{\gamma(t)}
\end{align*}
$$
...
by $P_{t, t_0}^\gamma (\mathbf{V}_0) = \mathbf{V}(t)$ where $\mathbf{V}$ a parallel vector field along $\gamma$ with $\mathbf{V}(t_0) = \mathbf{V}_0$. The collection of maps $P_\gamma (t, t_0)$ for $t, t_0 \in I$ is called *parallel transport* along $\gamma$.
</MathBox>

<MathBox title='Parallel transport is a linear isomorphism' boxType='proposition'>
Let $M$ be a smooth manifold with an affine connection $\nabla$ and suppose $\gamma:I\subset\R\to M$ is a curve in $M$. The parallel transport $P_{t, t_0}^\gamma: T_{\gamma(t_0)}M \to T_{\gamma(t)} M$ is a linear isomorphism.
</MathBox>
...
$$
  \frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}(t_0) = \lim_{t\to t_0} \frac{(P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) - \mathbf{V}(t_0)}{t - t_0}
$$
...
Let $\Set{\mathbf{e}_i}_{i=1}^n$ be a basis of $T_\mathbf{p}M$. Let $\mathbf{e}_i (t) = P_{t_0, t}^\gamma (\mathbf{e}_i)$. By the previous lemma, ${\mathbf{e}_i (t)}_{i=1}^n$ is a basis of $T_{\gamma(t)}$. Thus, there are functions $V^i (t)$ along $\gamma$ so that $\mathbf{V}(\gamma(t)) = V^i (t) \mathbf{e}_i (t)$. It follows that

$$
  (P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(\gamma(t))) = V^i (t) \mathbf{e}_i
$$

so

$$
  \lim_{t\to t_0} \frac{(P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(\gamma(t))) - \mathbf{V}(\mathbf{p})}{t - t_0} = \lim_{t\to t_0} \frac{V^i (t)\mathbf{e}_i - \mathbf{V}(\mathbf{p})}{t - t_0} = \dot{V}^i (t_0)\mathbf{e}_i
$$

On the other hand

$$
  \mathrm{D}_t \mathbf{V}(\mathbf{p}) = (\mathrm{D}_t V^i)(\mathbf{p})\mathbf{e}_i + V^i (t_0) \mathrm{D}_t \mathbf{e}_i (t_0) = \dot{V}^i (t_0)\mathbf{e}_i
$$
</details>
</MathBox>
...
By torsion-freeness, subtracting $\langle \mathrm{D}_Y \mathbf{X},\mathbf{N}\rangle$ and $\langle [\mathbf{X},\mathbf{Y}],\mathbf{N}\rangle$ from $\langle \mathrm{D}_\mathbf{X} \mathbf{Y},\mathbf{N}\rangle$ gives
...
\begin{align*}
  ...
  =& \int_{b_1}^{a_1} \lVert c'(t(u)) \rVert \frac{\mathrm{d}t}{\mathrm{d}u}\mathrm{d}u \\
