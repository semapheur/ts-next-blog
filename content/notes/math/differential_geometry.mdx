...
also satifes the three properties of the theorem. By the uniqueness of the covariant derivative, $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t} = \frac{\bar{\mathrm{D}}\mathbf{V}}{\mathrm{d}t}$. This proves that the covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ is independent of the frame. By covering $M$ with framed open sets, $\eqref{equation-1}$ then defines a covariant derivative $\frac{\mathrm{D}\mathbf{V}}{\mathrm{d}t}$ for the curve $c(t)$ in $M$.
</details>
</MathBox>
...
$$
  f\circ \ell_\mathbf{h} = \ell_{f(\mathbf{h})}\circ f \tag{\label{equation-2}}
$$
...
We will now show that $(f\circ c)(t)$ is an integral curve of $\tilde{f_* \mathbf{A}}$, the left-invariant vector field on $G$ generated by $f_* \mathbf{A}\in\mathcal{g}$. Taking the differential of $\eqref{equation-2}$ we get
