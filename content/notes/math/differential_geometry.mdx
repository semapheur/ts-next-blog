...
<MathBox title='' boxType='proposition'>
If an affine connection $\nabla$ on a Riemannian manifold $M$ is metric compatible, then for vector fields $\mathbf{X},\mathbf{Y},\mathbf{Z},\mathbf{W}\in\mathfrak{X}(M)$, the tensor $\langle R(\mathbf{X},\mathbf{Y})\mathbf{Z},\mathbf{W}\rangle$ is skew-symmetric in $\mathbf{Z}$ and $\mathbf{W}$.

<details>
<summary>Proof</summary>

Because $\langle R(\mathbf{X},\mathbf{Y})\mathbf{Z},\mathbf{W}\rangle$ is a tensor, it is enough to check its skew-symmetry locally, e.g. on an open set $U$ with an orthonormal frame $\Set{\mathbf{e}_i}_{i=1}^n$ (assured by the Gram-Schmidt process). Then
...
## Affine connection by covariant derivatives
