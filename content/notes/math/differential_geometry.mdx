...
$$
  P_\gamma (t,s) \circ P_\gamma (s,r) = P_\gamma (t,r)
$$
...
$$
  \frac{\d}{\d t}\tilde{\mathbf{V}}(t) = \frac{\d}{\d t}\mathbf{V}(\sigma(t)) = \frac{\d\sigma}{\d t}(t) \frac{\d\mathbf{V}}{\d t}(\sigma(t))
$$

Projecting orthogonally onto the tangent space $T_{\gamma(\sigma(t))}M$ we obtain

$$
  \frac{\D\tilde{\mathbf{V}}}{\d t}(t) = \dot{\sigma}(t)\frac{\D\mathbf{V}}{\d t}(\sigma(t)) = 0,\; \forall t\in J
$$
...
Let $\Set{\mathbf{e}_i}_{i=1}^n$ be a basis of $T_{\gamma(t_0)}M$. Let $\mathbf{e}_i (t) = P_{t_0, t}^\gamma (\mathbf{e}_i)$. By the previous lemma, ${\mathbf{e}_i (t)}_{i=1}^n$ is a basis of $T_{\gamma(t)} M$. Thus, there are functions $V^i (t)$ along $\gamma$ so that $\mathbf{V}(t) = V^i (t) \mathbf{e}_i (t)$. It follows that

$$
  (P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) = V^i (t) \mathbf{e}_i
$$

so

$$
  \lim_{t\to t_0} \frac{(P_{t_0, t}^\gamma)^{-1} (\mathbf{V}(t)) - \mathbf{V}(t_0)}{t - t_0} = \lim_{t\to t_0} \frac{V^i (t)\mathbf{e}_i - \mathbf{V}(t_0)}{t - t_0} = \dot{V}^i (t_0)\mathbf{e}_i
$$

On the other hand

$$
  \D_t \mathbf{V}(t_0) = (\D_t V^i)(t_0)\mathbf{e}_i + V^i (t_0) \D_t \mathbf{e}_i (t_0) = \dot{V}^i (t_0)\mathbf{e}_i
$$
</details>
</MathBox>
