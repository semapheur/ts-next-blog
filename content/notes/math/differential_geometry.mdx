...
$$
  \lVert\gamma'(s) \rVert = \lVert c'(t(s)) \rVert \cdot |t'(s)| = \frac{\d s}{\d t}\left|\frac{\d t}{\d s}\right| = \left|\frac{\d s}{\d t} \frac{\d t}{\d s}\right| = 1
$$

Conversely, if a curve $c(t)$ has unit speed, then its arc length is

$$
  s(t) = \int_a^t \lVert c'(u) \rVert\;\d u = \int_a^t 1\;\d u = t - a
$$
...
$$
\begin{align*}
  s =& \int_0^t \lVert c'(u) \rVert\;\d u = \int_0^t \left\lVert\begin{bmatrix} -a\sin t \\ a\cos t \end{bmatrix}\right\rVert\;\d u \\
  =& \int_0^t a\;\d u = at
\end{align*}
$$
...
$$
\begin{align*}
  \int_{a_1}^{b_1} \lVert \frac{\d}{\d u} c(t(u))\rVert\;\d u =& \int_{a_1}^{b_1} \lVert c'(t(u)) \rVert \left|\frac{\d t}{\d u}\right| \d u \\
  =& -\int_{a_1}^{b_1} \lVert c'(t(u))\rVert \frac{\d t}{\d u}\d u \\
  =& \int_{b_1}^{a_1} \lVert c'(t(u)) \rVert \frac{\d t}{\d u}\d u \\
  =& \int_a^b \lVert c'(t) \rVert \d t
\end{align*}
$$
...
$$
  \tilde{\kappa}_g = \left\lVert\frac{\D T}{\d s}\right\rVert
$$
