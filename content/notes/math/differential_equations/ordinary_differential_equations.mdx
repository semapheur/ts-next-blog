...
A first-order linear ODEs for an unknown function $f: I\to J$ with $I,J\subseteq\R$, takes the explicit form
...
where $w:I\times J\to\R$ is a prescribed function. The equation is linearly presicely when $w(x,y)$ is affine in the dependent variable $y$. That is, there exist functions $a,b:I\to\R$ such that
...
. We want to find all solutions $\alpha(x_0, x_1) \to\R$ ($\alpha' (x) = v(\alpha(x))$) with $\alpha(0) = f_0$.
Thus, any solution $\alpha$ satisfies

$$
\begin{align*}
  \frac{\alpha'(y)}{v(\alpha(y)} =& 1 \\
  \iff \int_0^x \frac{\alpha' (y)}{v(\alpha(y))} \;\d y =& x \\
  \iff \int_{f_0}^{\alpha(x)} \frac{1}{v(g)} \;\d g =& x, g = \alpha(y), \d g = \alpha' (y) \d y \\
  \iff F(\alpha(x)) - F(f_0) =& x\; \forall x\in (x_0, x_1)
\end{align*}
$$

where $F$ is an antiderivative of $1/v$. Defining the constant $c:= F(f_0)$, we can

write $F(\alpha(x)) = x - c \iff \alpha(x) = F^{-1} (x - c)$ for all $x\in (x_0, x_1)$.
