...
Let $U\subseteq \R^n$ be an open subset, let $\mathbf{V}: U\subseteq\R^n \to\R^n$ be locally Lipschitz continuous vector field and let $\mathbf{x}_0 \in U$. Then there exists a constant $\epsilon > 0$ such that the initial value problem
...
Since $\mathbf{V}$ is locally Lipschitz continuous and $U$ is open, there exists $r > 0$ and $L > 0$ such that the closed ball $\overline{B_r (\mathbf{x}_0)} \subseteq U$, and
...
$$
  M := \sup_{\mathbf{x} \in\overline{B_r (\mathbf{x}_0)}} \norm{\mathbf{V}(\mathbf{x})} < \inty
$$

which is finite because $\mathbf{V}$ is continuous and $\overline{B_r (\mathbf{x}_0)}$ is compact. Choose $\epsilon > 0$ such that $\epsilon L < 1$ and $\epsilon M \leq r$. Consider the metrix space
...
Then the metric space $(X, d)$ is complete. To show that $\Phi: X\to X$ is contraction, consider

$$
  (\Phi\boldsymbol{\alpha})(t) = \mathbf{x}_0 + \int_0^t \mathbf{V}(\alpha(s)) \;\d s
$$
...
Hence $(\Phi\boldsymbol{\alpha})(t) \in\overline{B_r (\mathbf{x}_0)}$ for all $t$, so $\Phi\boldsymbol{\alpha}\in X$.
