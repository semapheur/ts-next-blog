...
<MathBox title="Division algorithm (Euclidean division)" boxType="proposition">
Let $a, b\in\Z$ with $b > 0$. Then there exists unique integers $q, r \in\Z$ such that $a = qb + r$ with $0 \leq r < b$.

<details>
<summary>Proof</summary>

**Uniqueness:** Suppose $q_1, r_1, q_2, r_2 \in\Z$ satisfy

$$
  a = q_1 b + r_1 = q_2 b + r_2,\; 0 \leq r_1, r_2 < b
$$
...
so $b | (r_2 - r_1)$. However, we must have $-b < r_2 - r_1 < b$, since both $r_1, r_2 \in [0,b)$. The only multiple of $b$ in this interval is $0$, giving

$$
  r_2 - r_1 = 0 \implies r_1 = r_2
$$

Substituting back into $\eqref{equation-85}$ we get

$$
  (q_1 - q_2)b = 0 \implies q_1 = q_2
$$

This proves that the pair $(q, r) \in\Z^2$ is unique.

**Existence:** In the rational numbers, $a/b \in\Q$. By the Archimedean property, there exists $q\in\Z$ with $q \leq a/b < q + 1$. This implies

$$
\begin{gather*}
  qb \leq a < (q+1)b \\
  \implies 0 \leq \underbrace{a - qb}_{=r} < b
\end{gather*}
$$
...
For $a\in Z$, the remainder of $a$ after division by $N \in\N$ is denoted by $a\bmod N$ and is defined as
...
$$
\begin{equation*}
  a \bmod N = b \bmod N \iff (a - b) \bmod N = 0
\tag{\label{equation-2}}
\end{equation*}
$$
...
$$
  a\bmod N = b\bmod N \implies a - \left\lfloor \frac{a}{N} \right\rfloor N = b - \left\lfloor \frac{b}{N} \right\rfloor N
$$

and it follows that $N$ divides $a - b$, which implies $(a - b) \bmod N = 0$.

To show $\impliedby$, let $(a - b) \bmod N = 0$. Then it follows from $\eqref{equation-1}$ that there exists a $z\in\Z$ such that $a - b = z N$. Consequently,

$$
\begin{align*}
  b \bmod N =& b - \left\lfloor\frac{b}{N}\right\rfloor N \\
...
$$
  a\bmod N = a - \left\lfloor\frac{a}{N}\right\rfloor N < N
$$
...
$$
  a\bmod N = a - \left\lfloor\frac{a}{N} N < \frac{a}{2}
$$
</details>
</MathBox>

<MathBox title='' boxType="lemma" tag="1">
Let $a,N\in\N_+$. For $a, N \to\infty$ the number of computational steps required for the calculation of $a\bmod N$ scales as $O(\log_2^2 [\max\set{a,N}])$.

<details>
<summary>Proof</summary>

Because of $\eqref{equation-1}$, the computational effort to calculate $a\bmod N$ is given by
...
From properties of the big-O symbol, it follows that the number of operations to calculate $a\bmod N$ is of order $O(\log_2^2 [\max\set{a,N}])$.
</details>
</MathBox>
...
<MathBox title="Bézout's identity" boxType="theorem">
...
<MathBox title="Extended Euclidean algorithm" boxType="theorem" tag="1">
Let $a, b \in\N$. Define
...
$$
\begin{align*}
  r_j :=& r_{j-2} \bmod r_{j-1} \tag{\label{equation-4}} \\
...
Since we know from $\eqref{equation-3}$ that $u \bmod v < v$, if follows from the definition $\eqref{equation-4}$ of the $r_j$ that $0 \leq r_j \leq r_{j-1}$, that is, the $r_j$ are strictly decreasing with increasing $j$. Hence, there must exist an $n\in\N_+$ with $n\leq\min\set{a,b}$, $r_n > 0$ and $r_{n+1} = 0$. This proves $\eqref{equation-5}$.
...
$$
  0 = r_{n+1} = r_{n-1} \bmod r_n = r_{n-1} - \left\lfloor\frac{r_{n-1}}{r_n} \right\rfloor r_n
$$
...
Furthermore, from $\eqref{equation-4}$ it follows that $r_n = r_{n-2} \bmod r_{n-1}$ and thus
...
Applying the extended Euclid algorithm (Theorem $\ref{theorem-1}$) to calculate $\gcd(a,b)$, we need to compute expressions of the for $u\bmod b$, starting with $a\bmod b$ as shown in $\eqref{equation-4}$. By Lemma $\ref{lemma-1}$, the number of computational steps for each calculation of $a\bmod b$ scales as $O(\log_2^2 (\min\set{a,b}))$ for $a,b\to\infty$. From $\eqref{equation-7}$ we infer that in the Euclid algorith the number of times we have to calculate expressions of the form $u\bmod v$ grows with $a,b\to\infty$ as $O(\log_2 (\min\set{a,b}))$. By properties of the big O symbol, the total effort to calculate $\gcd(a,b)$ is given by $O(\log_2^3 (\min\set{a,b}))$.
</details>
</MathBox>

<MathBox title='' boxType="proposition" tag="1">
Let $u,v,u_j \in\Z$ and $k, a, N\in\N$. Then
1. $u(v\bmod N)\bmod N = uv\bmod N$
2. $\left(\prod_{j=1}^k (u_j \bmod N) \right) \bmod N = \left(\prod_{j=1}^k u_j \right) \bmod N$
3. $(u^a \bmod N)^k \bmod N = u^{ak} \bmod N$
4. $\left(\sum_{j=1}^k (u_j \bmod N) \right) \bmod N = \left(\sum_{j=1}^k u_j \right)$

<details>
<summary>Proof</summary>

**(1):** Repeated application of $\eqref{equation-1}$ for evaluating $u(v\bmod N) \bmod N$ yields

$$
\begin{align*}
  u(v\bmod N) \bmod N =& u(v\bmod N) - \left\lfloor\frac{u(v\bmod N)}{N}\right\rfloor N \\
  =& u\left(v - \left\lfloor\frac{v}{N}\right\rfloor N\right) - \left\lfloor\frac{u\left(v - \left\lfloor\frac{v}{N}\right\rfloor N\right) N}{N}\right\rfloor N \\
...
$$
\begin{align*}
  (u_1 \bmod N + u_2 \bmod N)\bmod N =& \left(u_1 - \left\lfloor\frac{u_1}{N}\right\rfloor N + u_2 - \left\lfloor\frac{u_2}{N}\right\rfloor N \right) \bmod N \\
  =& u_1 - \left\lfloor\frac{u_1}{N}\right\rfloor N + u_2 - \left\lfloor\frac{u_2}{N}\right\rfloor \\
  &- \left\lfloor\frac{u_1 - \left\lfloor\frac{u_1}{N}\right\rfloor N + u_2 - \left\lfloor\frac{u_2}{N}\right\rfloor}{N}\right\rfloor N \\
...
$$
\begin{equation*}
  a\bmod N = b\bmod N \iff ac\bmod N = bc\bmod N
...
$$
\begin{align*}
  ac\bmod N =& ac - \left\lfloor\frac{ac}{N}\right\rfloor N \\
  ...
  =& bc - \left\lfloor\frac{bc}{N}\right\rfloor N = bc\bmod N
\end{align*}
$$

**($\impliedby$):** Let $ac\bmod N = bc\bmod N$. Then there exists $z\in\Z$ such that

$$
  a - b = \frac{z}{n} N \in\Z
$$

Since $c$ and $N$ are coprime, we must have $z/c\in\Z$. Thus, $(a - b) \bmod N = 0$, and it follows from $\eqref{equation-2}$ that $a\bmod N = b\bmod N$.
</details>
</MathBox>

<MathBox title='Multiplicative inverse' boxType="definition">
Let $b,N\in\N$ with $\gcd(b,N) = 1$. The *multiplicative invserse* modulo $N$ of $b$ is denoted by $b^{-1} \bmod N$ and is defined as the number $x\in\set{1,\dots,N-1}$ that satisfies
...
Then $x\bmod N$ is the uniquely determined multiplicative inverse of $b$ modulo $N$, i.e. it satisfies
...
Let $u$ and $v$ be two multiplicative inverses for $b$ modulo $N$, that is, suppose $u,v\in\set{1,\dots,N-1}$ are such that $bu \bmod N = 1 = bv\bmod N$. Because of $\gcd(b,N) = 1$ and $\eqref{equation-12}$, then $bu\bmod N = bv\bmod N$ implies that $u\bmod N = v\bmod N$. From $\eqref{equation-2}$ we know that $N$ divides $u - v$. Since $0 < u$, $v < N$, it thus follows that $u = v$.
...
$$
  b(x\bmod N) \bmod N = bx\bmod N = bx\bmod N = (1 - Ny) \bmod N =1
$$

and since $0 < x\bmod N < N$, it follows that $x\bmod N$ satisfies all the defining properties of the multiplicative inverse of $b$ modulo $N$.
</details>
</MathBox>
...
$$
  ab\bmod N = 0 \implies \gcd(a,N) \gcd(b,N) > 1
$$

In particular, if $N$ is prime, then

$$
  ab\bmod N = 0 \iff a\bmod N = 0 \lor b\bmod N = 0
$$

<details>
<summary>Proof</summary>

Let $ab\bmod N = 0$. Then there exists $q\in\N$ such that $ab = qN$. From the prime decomposition of this equation
...
If $N$ is prime and $N\mid ab$ holds, then $N$ must be contained in $a$ or $b$ as a prime factor. Conversely, $a\bmod N = 0$ implies $N\mid a$ and $b\bmod N = 0$ implies $N|b$. Either case has $N\mid ab$ as a consequence.
</details>
</MathBox>
...
<MathBox title="Eulers's theorem" boxType="theorem" tag="2">
...
First, we define $a_j := r_j b \bmod N$ for all $r_j \in\N_+$ with $1 \leq r_j < N$ and $\gcd(r_j, N) = 1$ and set

$$
  P := \left(\prod_{j=1}^{\phi(N)} a_j \right) \bmod N
$$
...
$$
\begin{equation*}
  P = \left(b^{\phi(N)} \prod_{j=1}^{\phi(N)} \right) \bmod N
\tag{\label{equation-17}}
\end{equation*}
$$

For $j \neq k$ we have that $a_j \neq a_k$. To see this, suppose $a_j = a_k$, i.e. $r_j b \bmod N = r_k b \bmod N$. Since $b$ and $N$ are coprime, Lemma $\ref{lemma-2}$ implies that $r_j \bmod N = r_k \bmod N$. Because of the assumption $1 \leq r_j$, $r_k < N$, it follows that $r_j = r_k$ and thus $j = k$. Hence $a_j \neq a_k$ for $j \neq k$. Since $r_j$ and $N$ as ell as $b$ and $N$ are coprime, it follows that also $r_j b$ and $N$ are coprime, i.e.
...
Suppose $a_j = r_j b \bmod N$ has a common divisor $s > 1$ with $N$ such that $a_j = us$ and $N = vs$. Then there exists a $k\in\Z$ such that $us = r_j b + kvs$, which is equivalent to $r_j b = (u - kv)s$. This, however, would imply that $r_j b$ and $N$ have a common divisor $s > 1$, which contradicts $\eqref{equation-16}$. Consequently, all $a_j$ are coprime with $N$ and there exist $\phi(N)$ distinct $a_j$ with $1 \leq a_j < N$. This means that the set of the $a_j$ is a permutation of the set of the $r_j$, and thus

$$
  P = \left(\prod_{j=1}^{\phi(N)} a_j \right) \bmod N = \left(\prod_{j=1}^{\phi(N)} r_j \right) \bmod N
$$
...
$$
\begin{equation*}
  \left(b^{\phi(N)} \prod_{j=1}^{\phi(N)} r_j \right) \bmod N = \left(\prod_{j=1}^{\phi(N)} \right) \bmod N
...
<MathBox title="Fermat's little theorem" boxType="corollary" tag="1">
...
<MathBox title="Modular multiplicative order" boxType="definition" tag="1">
For $a,N\in\N$ with $\gcd(a,N) = 1$ we define the order of $a \bmod N$ as

$$
  \operatorname{ord}_N (a) := \min\set{m\in\N_+ | a^m \bmod N = 1}
$$
...
1. For all $k\in\N$
$$
  a^k \bmod N = 1 \iff \operatorname{ord}_N (a) | k
$$
...
4.
$$
  \set{d \in\set{1,\dots,N-1} | \gcd(d,N) = 1} = \set{a^j \bmod N | j\in\set{1,\dots,\phi(N)}}
$$
5. If $b = a^j \bmod N$ for $j\in\N$ then
$$
...
**(1):** To show $\implies$, let $a^k \bmod N = 1$ for $k \in\N_+$. Then $k \geq\operatorname{ord}_N (a)$ has to hold since $\operatorname{ord}_N (a)$ is per definition the smallest such number. Let $c = k\bmod \operatorname{ord}_N (a)$, i.e. $c \in\N_0$ with $c < \operatorname{ord}_N (a)$ and there is an $l\in\Z$ such that $k = \operatorname{ord}_N (a) l + c$ and thus $a^k = a^{\operatorname{ord}_N (a) l + c} = (a^{\operatorname{ord}_N (a) l}) a^c$. This implies

$$
\begin{align*}
  1 =& a^k \bmod N = (a^{\operatorname{ord}_N (a) l}) a^c \bmod N \\
  =& (a^{\operatorname{ord}_N (a)l} \bmod N) (a^c \bmod N) \bmod N \\
  =& (\underbrace{a^{\operatorname{ord}_N (a)} \bmod N}_{=1})^l (a^c \bmod N) \bmod N \\
  =& (a^c \bmod N) \bmod N = a^c \bmod N
\end{align*}
$$

By construction, we have $c < \operatorname{ord}_N (a)$ and since $\operatorname{ord}_N (a)$ is per definition the smallest natural number $k\in\N$ satisfying $a^k \bmod N = 1$ it follows that $c$ has to vanish and thus $\operatorname{ord}_N (a) \mid k$.
...
$$
\begin{align*}
  a^k \bmod N =& (a^{\operatorname{ord}_N (a)})^l \bmod N \\
  =& (\underbrace{a^{\operatorname{ord}_N (a)} \bmod N}_{=1})^l \bmod N \\
  = 1 \bmod N = 1
\end{align*}
$$

**(2):** By Euler's theorem (Theorem $\ref{theorem-2}$), we have $a^{\phi(N)} \bmod N = 1$, and thus **(2)** follows from **(1)**.

**(3):** Consider first that

$$
\begin{align*}
  &(ab)^{\operatorname{ord}_N (a) \operatorname{ord}_N (b)} \bmod N \\
  =& \left((a^{\operatorname{ord}_N (a) \operatorname{ord}_N (b)} \bmod N)(b^{\operatorname{ord}_N (b) \operatorname{ord}_N (a)} \bmod N)\right) \bmod N \\
  =& ((\underbrace{a^{\operatorname{ord}_N (a)} \bmod N}_{=1})^{\operatorname{ord}_N (b)} \bmod N \\
  &\times (\underbrace{b^{\operatorname{ord}_N (b)} \bmod N}_{=1})^{\operatorname{ord}_N (a)} \bmod N) \bmod N \\
  =& 1 \bmod N = 1
\end{align*}
$$
...
$$
\begin{align*}
  & a^{\operatorname{ord}_N (b) \operatorname{ord}_N (ab)} \bmod N \\
  =& (a^{\operatorname{ord}}_N (b) \operatorname{ord}_N (ab) \bmod N) (\underbrace{b^{\operatorname{ord}_N (b)} \bmod N}_{=1})^{\operatorname{ord}_N (ab)} \bmod N \\
  =& (ab)^{\operatorname{ord}_N (b) \operatorname{ord}_N (ab)} \bmod N \\
  =& (\underbrace{(ab)^{\operatorname{ord}_N (b) \operatorname{ord}_N (ab)} \bmod N}_{=1})^{\operatorname{ord}_N (b)} \bmod N \\
  =& 1
\end{align*}
$$
...
Analogously for $b^{\operatorname{ord}_N (a) \operatorname{ord}_N (ab)} \bmod N$, we find
...
$$
\begin{equation*}
  \set{a^j \bmod N}_{j=1}^{\phi(N)} \subset \set{d|\gcd(d,N) = 1}_{d=1}^{N-1}
\tag{\label{equation-23}}
\end{equation*}
$$

Then we will prove that the two sets share the same cardinality and are thus identical. To verify the inclsuion, wew show that the elements of $\set{a^j \bmod N}_{j=1}^{\phi(N)}$ have the property $\gcd(a^j \bmod N, N) = 1$. To show this, suppose $l\in\N_+$ is a common divisor of $a^j \bmod N$ and $N$. That is, suppose that there exist $u,v\in\N$ satisfying
...
$$
  lu = a^j \bmod N = a^j - \left\lfloor\frac{a^j}{N}\right\rfloor N = a^j - \left\lfloor\frac{a_j}{N}\right\rfloor lv
$$

and thus $l\mid a^j$. Consequently, every prime factor of $l$ would be a prime factor of $a$ as well. By $\eqref{equation-22}$, such prime factor would then be divisors of $a$ and $N$. However, by assumption we have $\gcd(a,N) = 1$. Hence, we must have $l = 1$, which implies $\gcd(a^j \bmod N, N) = 1$ and the inclusion $\eqref{equation-23}$ is proven.

It remains to show that $|\set{a^j \bmod N}_{j=1}^{\phi(N)}| = \phi(N)$. Let $i,j\in\N_+$ be such that $1 \leq i \leq j \leq\phi(N)$ and suppose that

$$
\begin{equation*}
  a^j \bmod N = a^i \bmod N
...
which together with $0 < j - i < \phi(N)$, contradicts the assumption that $a$ is a primitive root, which means that $\operatorname{ord}_N (a) = \phi(N)$. Thus, the set $\set{a^j \bmod N}_{j=1}^{\phi(N)}$ contains exactly $\phi(N)$ distinct elements each of which is coprime to $N$. This completes the proof of **(3)**.

**(5):** We first show that $b = a^j \bmod N$ implies
...
$$
\begin{align*}
  1 =& (a^j)^{\operatorname{ord}_N (a^j)} \bmod N \\
  =& (a^j \bmod N)^{\operatorname{ord}_N (a_j)} \bmod N \\
...
$$
  1 = (a^j)^{\operatorname{ord}_N (a_j)} \bmod N = a^{\operatorname{ord}_N (a^j) j} \bmod N
$$
...
$$
\begin{align*}
  (a^j)^{\tilde{r}} \bmod N =& (a^j)^{\phi(N)/m} \bmod N \\
  =& (a^{\phi(N)})^{j/m} \bmod N \\
  =& (\underbrace{a^{\phi(N)} \bmod N}_{=1})^{j/m} \bmod N = 1
\end{align*}
$$

This contradicts the fact that $\operatorname{ord}_N (a^j)$ is, by definition, the smallest natural number with the property $(a^j)^r \bmod N = 1$. Consequently, we must have $m_1 = \gcd(j, \phi(N))$.
</details>
</MathBox>
...
$$
  f(n_j) \bmod p = 0
$$
- $f$ is the zero polynomial modulo $p$, i.e. $f(x) \bmod p = 0$ for all $x\in\Z$

<details>
<summary>Proof</summary>

We show this by induction in the degree of the polynomial. For the base case $k = 0$, if $f(x) = f_0 \neq 0$ such that $p \nmid f_0$, then it follows that $f_0 \bmod p \neq 0$, and there is no $x\in\Z$ with $f(x) \bmod p = 0$. If $f_0 = 0$, then $f$ is the zero polynomial.
...
\begin{equation*}
\begin{split}
  g(n_l) \bmod p =& \left(f(n_l) - f_k \prod_{j=1}^k (n_l - n_j) \right) \bmod p \\
  =& f(n_l) \bmod p = 0
\end{split}
...
\begin{equation*}
\begin{split}
  \tilde{g}(x) \bmod p =& \left(\sum_{l=0}^m g_l x^l \right) \bmod p \\
  =& \left(\sum_{l=0}^{k-1} g_l x^l \right) \bmod p \\
  =& g(x) \bmod p
\end{split}
...
$$
  g(x) \bmod p = 0, \; \forall x\in Z
$$

With $\eqref{equation-43}$

$$
  f(x) \bmod p = f_k \prod_{j=1}^k (x - n_j) \bmod p
$$

and for an arbitrary zero $z$ of $f$ modulo $p$ we have

$$
  0 = f(z) \bmod p = f_k \prod_{j=1}^k (z - n_j) \bmod p
$$
...
Since we choose $n_j$ from $\set{1_0,\dots,p - 1}$, it follows that $z\bmod p = n_j$, and $z$ is either one of the $k$ zeroes selected from $\set{1,\dots,p - 1}$ or differs from one of these by a multiple of $p$ and is thus not an element of the set $\set{1,\dots,p - 1}$.
</details>
</MathBox>
...
Since $p - 1 = \phi(p)$, and by Euler's theorem (Theorem $\ref{theorem-2}$), we have $a^{\phi(p)} \bmod p = 1$ for all $a\in\set{1,\dot,p-1}$, it follows that for all $z\in\set{1,\dots,p-1}$
...
$$
  h(n_j) \bmod p = 0 \lor f(n_j) \bmod p = 0
$$
...
$$
  \left(a_q^{q^{k_q}} - 1 \right) \bmod p = 0
$$
...
$$
  a^{\operatorname{ord}_{p^{k+1}} (a)} \bmod p^{k+1} = 1
$$
...
$$
  g(a) := (a\bmod n_1,\dots,a \bmod n_J) =: (g(a)_1,\dots,g(a)_J)
$$
...
$$
\begin{equation*}
  h(\mathbf{b}) := \left(\sum_{j=1}^J m_j x_j b_j \right) \bmod N
...
$$
  \left(\sum_{j=1}^J m_j x_j b_j \right) \bmod N = \left(\sum_{j=1}^J m_j \tilde{x}_j b_j \right) \bmod N
$$
...
$$
\begin{align*}
  \frac{1}{n_k} (h(\mathbf{b}) - b_k) =& \frac{1}{n_k} \left(\left(\sum_{j=1}^J m_j x_j b_j \right) \bmod N - b_k \right) \\
...
$$
  g(h(\mathbf{b}))_k = g(b_k + z_k n_k)_k = (b_k + z_k n_k) \bmod n_k = b_k
$$
...
- the public key $k_\text{public} = (a, N)$ is used for encryption
...
$$
\begin{equation*}
  c = e(k_\text{public}, m) := m^a \bmod N
...
$$
\begin{equation*}
  d(k_\text{private}, c) := c^b \bmod N = m^{ab} \bmod N
...
$$
  \left(m^{k(p-1)} \right)^{q-1} \bmod q = 1 = \left(m^{k(q-1)} \right)^{p-1} \bmod p
$$
...
$$
\begin{align*}
  m^{ab} \bmod pq =& mm^{k(p-1)(q-1)} \bmod pq \\
  =& m(\underbrace{m^{k(p-1)(q-1)} \bmod pq}_{=1}) \bmod pq \\
  =& m\bmod pq = m
\end{align*}
$$
...
$$
  [a_0: a_1,\dots,a_n] := a_0 + \dfrac{1}{a_1 + \dfrac{1}{\ddots \dfrac{\vdots}{a_{n-1} + \dfrac{1}{a_n}}}}
$$
...
\begin{split}
  ...
  r_{j+1} = r_{j-1} \bmod r_j,\; j\in\N : r_j \neq j
\end{split}
...
\begin{align*}
  ...
  =& \frac{r_{j-1} \bmod r_j}{r_j} = \frac{r_{j+1}}{r_j}
\end{align*}
$$

Since $0 \leq r_{j+1} = r_{j-1} \bmod r_j < r_j$ hold, the sequence $(r_j)$ is strictly decreasing with non-negative integers. This implies that after a finite number of steps we must find $r_{n+1} = 0$ for some $n\in\N_0$. It follows from $\eqref{equation-60}$ that $f_{n+1} = 0$ which means that the sequence $(a_j)$ ends with $a_n$ and $\eqref{equation-55}$ implies $x=[a_0;a_i]_{i=1}^n$.
</details>
</MathBox>
...
$$
\begin{equation*}
  \left|\frac{P}{Q} - \frac{p}{q}\right| < \frac{1}{2q^2}
...
\begin{align*}
  ...
  =& \bigg|a \underbrace{\left(q_j \frac{p_n}{q_n} - p_j \right)}_{:=c_j} + b \underbrace{\left(q_{j+1} \frac{p_n}{q_n} - p_{j+1} \right)}_{:= c_{j+1}} \bigg| \tag{\label{equation-81}}
\end{align*}
$$
