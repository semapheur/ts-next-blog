...
Using set theory, the natural numbers $\N_0$ can be constructed recursively by letting $0 := \{\} = \emptyset \in \N$ and $n + 1 = n \cup \{ n \}$ for each $n \in \N_0$ such that $n = \{ i \}_{i=0}^{n-1}$, i.e. $n$ is a set of length $n$. In this way, every $n\in\N_0$ has a successor, which can be formulated using the successor map $s: \N_0 \to \N_0$ given by $n \mapsto n\cup\{ n\} \equiv n + 1$. The set of natural numbers $\N_0$ is then defined as the smallest set that
1. contains $0$
...
2. $0 \notin \mathrm{Ran}(s) = s[\N_0]$ ($0$ is not a successor to any $n\in\N_0$)
...
In the induction step, we assume that $P(n)$ is true for all $n\in\N_0$. To prove $P(n) \implies P(n+1)$ we need to show that for all $k, m \in \N_0$
...
$$
  \Z := \N_0^2 / \sim = \left\{ [(a, b)]_\sim | (a, b) \in \N_0^2 \right\}
$$

By relabeling the equivalence classes as $[(n, 0)]_\sim =: n_\Z$ and $[(0, n)]_\sim =: (-n)_\Z$, we get the familiar form for the integers $\Z := \{ n \}_{n=-\infty}^\infty$.
...
The rational numbers can be constructed as the equivalence classes of ordered pairs of integers, $(a, b) \in \Z \times \Z\backslash\{0\}$, representing the division $a/b$ (without explicitly defining division). This can be formulated by an equivalence relation $\sim$ obeying
...
$$
  \mathbb{Q} := \left(\Z \times \Z\backslash\{0\}\right)/\sim = \left\{ [(a, b)]_\sim \mid (a, b) \in \Z \times \Z\backslash\{0\} \right\}
$$
...
$$
  \mathbb{Q} := \left\{ \frac{a}{b} \;\middle|\; a \in \Z,\; b \in \Z\backslash\{0\} \right\}
$$
...
2. $(\R\backslash\{0\}, \cdot, 1)$ is an Abelian group
...
$$
  \mathcal{C} := \left\{ \left(x_n \right)_{n\in\N} \mid \forall n \in \N : x_n \in \mathbb{Q} \textrm{ and } \left(x_n \right)_{n\in\N} \textrm{ is a Cauchy sequence} \right\}
$$
...
$$
  \left[ \left(x_n \right)_{n\in\N} \right]_\sim := \left\{ \left(a_n \right)_{n\in\N} | \left(a_n \right)_{n\in\N} \sim \left(x_n \right)_{n\in\N} \right\}
$$

<MathBox title='Real numbers' boxType='definition'>
The real numbers is the set 
$$
  \R := \mathcal{C}\backslash\sim := \left\{ \left[ \left(x_n \right)_{n\in\N} \right]_\sim \middle| \left(x_n \right)_{n\in\N} \in \mathcal{C} \right\}
$$
...
The extended real number line include the infinite points and is denoted $\R* = \R\cup\{-\infty, \infty\}$. In the extended topology of $\R*$ the following is defined for $a\in\R$
...
where $r = |z|$ is the absolute value of $z$ and $\phi := \arg(z) = \{ \mathrm{Arg}(z) + 2\pi n | n \in \Z \}$ is the argument of $z$ giving the angle of $z$ with respect to the positive real axis. The principal argument $\mathrm{Arg}(z) \in [0, 2\pi)$ can be calculated using the tangent half-angle formula 
...
A quaternion is an ordered 4-tuple of real numbers of the form $q = a + b\mathbf{i} + c\mathbf{j} + d\mathbf{k} \in \mathbb{H}$ where $a,b,c,d\in\R$ and $\mathbf{i}, \mathbf{j}, \mathbf{k}$ are imaginary basis vectors satisfying the quaternion equations
