...
By relabeling the equivalence classes as $[(n, 0)]_\sim =: n_\Z$ and $[(0, n)]_\sim =: (-n)_\Z$, we get the familiar form for the integers $\Z := \Set{ n }_{n=-\infty}^\infty$.
...
A quaternion is an ordered 4-tuple of real numbers of the form $q = a + b\hat{\imath} + c\hat{\jmath} + d\hat{k} \in \mathbb{H}$ where $a,b,c,d\in\R$ and $\hat{\imath}, \hat{\jmath}, \hat{k}$ are imaginary basis vectors satisfying the quaternion equations
...
$$
  r + q = (r_0 + \mathbf{r}) + (q_0 + \mathbf{q}) = (r_0 + q_0) + (\mathbf{r} + \mathbf{q})
$$
...
$$
\begin{align*}
  q_1 q_2 =& (a_1 + b_1\hat{\imath} + c_1\hat{\jmath} + d_1\hat{k}) \cdot (a_2 + b_2\hat{\imath} + c_2\hat{\jmath} + d_2\hat{k}) \\
  =& a_1 a_2 + a_1 b_2\hat{\imath} + a_1 c_2\hat{\jmath} + a_1 d_2\hat{k} \\
    &+ b_1 a_2\hat{\imath} + b_1 b_2\hat{\imath}^2 + b_1 c_2\hat{\imath}\hat{\jmath} + b_1 d_2\hat{\imath}\hat{k} \\
    &+ c_1 a_2\hat{\jmath} + c_1 b_2\hat{\jmath}\hat{\imath}^2 + c_1 c_2\hat{\jmath}^2 + c_1 d_2\hat{\jmath}\hat{k} \\
    &+ d_1 a_2\hat{k} + d_1 b_2\hat{k}\hat{\imath}^2 + d_1 c_2\hat{k}\hat{\jmath} + d_1 d_2\hat{k}^2 \\
  =& a_1 a_2 - b_1 b_2 + c_1 c_2 - d_1 d_2\\
    &+ (a_1 b_2 + b_1 a_2 + c_1 d_2 - d_1 c_2)\hat{\imath}\\
    &+ (a_1 c_2 + b_1 d_2 + c_1 a_2 - d_1 b_2)\hat{\jmath}\\
    &+ (a_1 d_2 + b_1 c_2 + c_1 b_2 - d_1 a_2)\hat{k}
\end{align*}
$$
...
where $\cdot$ is the Euclidean inner product and $\times$ is the cross product. The Hamilton product is noncommutative as

$$
\begin{align*}
  (a_2 + \mathbf{q}_2)(a_1 + \mathbf{q}_1) =& (a_2 a_1 - \mathbf{q}_2\cdot\mathbf{q}_1) + a_2\mathbf{q}_1 + a_1\mathbf{q}_2 + \mathbf{q}_2 \times\mathbf{q}_1 \\
  =& (a_1 a_2 - \mathbf{q}_1\cdot\mathbf{q}_2) + a_1\mathbf{q}_2 + a_2\mathbf{q}_1 - \mathbf{q}_1 \times\mathbf{q}_2
\end{align*}
$$

giving the anti-commutative relation $\{q_1, q_2\} = q_1 q_2 - q_2 q_1 = 2\mathbf{q}_1 \times \mathbf{q}_2$. The Hamilton product gives the following multiplication rules for the basis elements

$$
\begin{split}
  \hat{\imath}\hat{\jmath} = \hat{\imath}&\times\hat{\jmath} = \hat{k} \\
  \hat{\jmath}\hat{k} = \hat{\jmath}&\times\hat{k} = \hat{\imath} \\
  \hat{k}\hat{\imath}= \hat{k}&\times\hat{\imath} = \hat{\jmath}
\end{split} \quad \begin{split}
  \hat{\jmath}\hat{\imath} = \hat{\jmath}&\times\hat{\imath} = -\hat{k} \\
  \hat{k}\hat{\jmath} = \hat{k}&\times\hat{\jmath} = -\hat{\imath} \\
  \hat{\imath}\hat{k} = \hat{\imath}&\times\hat{k} = -\hat{\jmath}
\end{split}
$$

The multiplication rules for the basis elements can also be derived from the defining equations for quaternions:
...
\end{split} \quad \begin{split} 
  \hat{\imath}(\hat{\imath}\hat{\jmath}\hat{k}) &= i(-1) \\
  (\hat{\imath}\hat{\imath})\hat{\jmath}\hat{k} &= \hat{\imath}(-1) \\
  (-1)\hat{\jmath}\hat{k} &= \hat{\imath}(-1) \\
  \hat{\jmath}\hat{k} &= \hat{\imath}
\end{split} \quad \begin{split} 
  -\hat{k}\hat{\imath} &= (\hat{\jmath}\hat{\imath})\hat{\imath} \\
  -\hat{k}\hat{\imath} &= \hat{\jmath}(\hat{\imath}\hat{\imath}) \\
  -\hat{k}\hat{\imath} &= \hat{\jmath}(-1) \\
...
|---|---|---|---|
| $\hat{\imath}$ | $-1$ | $\color{red}{\hat{k}}$ | $\color{cyan}{-\hat{\jmath}}$ |
| $\hat{\jmath}$ | $\color{red}{-\hat{k}}$ | $-1$ | $\color{green}{\hat{\imath}}$ |
| $\hat{\jmath}$ | $\color{cyan}{-\hat{\jmath}}$ | $\color{green}{-\hat{\imath}}$ | $-1$ |


### Conjugation

The conjugate of a quaternion $q = q_0 + q_1\hat{i} + q_2\hat{\jmath} + q_3\hat{k} = q_0 + \hat{q}$ is defined by
