...
$$
  \mu\left(\frac{\D\dot{\mathbf{x}}}{\d t}\right) = F(\dot{\mathbf{x}})
$$

where $\D\dot{\mathbf{x}}/\d t$ is the covariant derivative of $\mathbf{x}$.
</MathBox>
...
$$
  \frac{\mathbf{D}\dot{\mathbf{x}}}{\d t} = \ddot{\mathbf{x}}
$$
...
$$
\begin{align*}
  &\mu\left(\frac{\D\dot{\mathbf{x}}}{\d t} \right) = F(\mathbf{x}, \dot{\mathbf{x}}) \\
  \iff& \mu\left(\frac{\D\dot{\mathbf{x}}}{\d t} \right)(\mathbf{v}) = F(\mathbf{x}, \dot{\mathbf{x}})(\mathbf{v}) \\
...
- *conservative* if there exists a function $U: M\to\R$, called *potential energy*, such that $F(\mathbf{v}) = -(\d U)_{\pi(\mathbf{v})}$ for all $\mathbf{v}\in TM$.
</MathBox>
...
In a conservative mechanical system $(M, \langle\cdot,\cdot\rangle, -\d U)$, the *mechanical energy* $E(t) = K(\dot{\mathbf{r}}(t)) + U(\mathbf{r}(t))$ is constant along any motion $\mathbf{r}: I\subset\R\to M$.
...
$$
\begin{align*}
  \frac{\d E}{\d t}(t) =& \frac{\d}{\d t}\left(\frac{1}{2}\langle \dot{\mathbf{r}}(t), \dot{\mathbf{r}}(t) \rangle + U(\mathbf{r}(t)) \right) \\
  =& \left\langle \frac{\D\dot{\mathbf{r}}}{\d t}(t), \dot{\mathbf{r}}(t) \right\rangle + (\d U)_{\mathbf{r}(t)}\dot{\mathbf{r}}(t) \\
  =& \mu\left(\frac{\D\dot{\mathbf{r}}}{\d t}\right)(\dot{\mathbf{r}}) - F(\dot{\mathbf{r}})(\dot{\mathbf{r}}) = 0
\end{align*}
$$
</details>
</MathBox>
...
Let $(M, \langle\cdot,\cdot\rangle, -\d U)$ be a conservative mechanical system and $h\in\R$ such that
...
$$
  \tilde{\nabla}_\mathbf{X} \mathbf{Y} = \nabla_\mathbf{X} \mathbf{Y} + \d\rho(\mathbf{X}) \mathbf{Y} + \d\rho(\mathbf{Y})\mathbf{X} - \langle\mathbf{X},\mathbf{Y}\rangle \operatorname{grad}(\rho)
$$
...
\begin{align*}
  ...
  =& e^{2\rho} \mathbf{X} \cdot \langle\mathbf{Y},\mathbf{Z}\rangle + 2\d\rho(\mathbf{X}) e^{2\rho}\langle\mathbf{Y},\mathbf{X}\rangle
\end{align*}
$$
...
$$
\begin{align*}
  2\langle\langle \tilde{\nabla}_\mathbf{X} \mathbf{Y}, \mathbf{Z} \rangle\rangle =& 2e^{2\rho} \langle\nabla_\mathbf{X} \mathbf{Y}, \mathbf{Z}\rangle + 2\d\rho(\mathbf{X})e^{2\rho} \langle\mathbf{Y},\mathbf{Z}\rangle \\
  &+ 2\d\rho(\mathbf{Y})e^{2\rho} \langle\mathbf{X},\mathbf{Z}\rangle - 2\d\rho(\mathbf{Z}) e^{2\rho}\langle\mathbf{X},\mathbf{Y}\rangle \\
  =& 2\langle\langle\nabla_\mathbf{X} \mathbf{Y},\mathbf{Z}\rangle\rangle + 2\langle\langle\d\rho(\mathbf{X})\mathbf{Y},\mathbf{Z}\rangle\rangle \\
  &+ 2\langle\langle\d\rho(\mathbf{Y})\mathbf{X}, \mathbf{Z}\rangle\rangle - 2e^{2\rho}\langle\operatorname{grad}(\rho), \mathbf{Z}\rangle\rangle\langle\mathbf{X},\mathbf{Y}\rangle
\end{align*}
$$
...
$$
  \frac{\D\dot{\mathbf{r}}}{\d t} = f(t)\dot{\mathbf{r}}
$$
...
Let $s: I\to J$ be a diffeomorphism and $\boldsymbol{\gamma}: J\to M$ the reparametrization of $\mathbf{r}: I\to M$ defined by
...
$$
  \dot{\mathbf{r}}(t) = \dot{\boldsymbol{\gamma}}(s(t)) \frac{\d s}{\d t}(t) \tag{\label{equation-2}}
$$
...
$$
\begin{align*}
  \nabla_{\dot{\mathbf{r}}} \dot{\mathbf{r}} =& \nabla_{\dot{\mathbf{r}}}\left(\frac{\d s}{\d t}\dot{\boldsymbol{\gamma}} \right)\dot{\boldsymbol{\gamma}} + \left(\frac{\d s}{\d t} \right) \nabla_{(\d s/\d t)\dot{\boldsymbol{\gamma}}} \dot{\boldsymbol{\gamma}} \\
  =& \frac{\d}{\d t} \left(\ln\left|\frac{\d s}{\d t}\right| \right)\dot{\mathbf{r}} + \left(\frac{\d s}{\d t} \right)^2 \nabla_{\dot{\boldsymbol{\gamma}}}\dot{\boldsymbol{\gamma}}
\end{align*}
$$
...
The motions of a conservative mechanical system $(M,\langle\cdot,\cdot\rangle, -\d U)$ with mechanical energy $h$ are, up to reparametrization, geodesics of the Jacobi metric on $M_h$.

<details>
<summary>Proof</summary>

Let $\mathbf{r}: I\subseteq\R \to M$ be a motion of $(M,\langle\cdot,\cdot\rangle, -\d U)$ with mechanical energy $h$. Then lemma $\ref{lemma-1}$ gives

$$
  \frac{\tilde{\D\dot{\mathbf{r}}}}{\d t} = \frac{\D\dot{\mathbf{r}}}{\d t} + 2\d\rho(\dot{\mathbf{r}}) \dot{\mathbf{r}} - \langle\dot{\mathbf{r}},\dot{\mathbf{r}}\rangle \operatorname{grad}(\rho)
$$

where $\tilde{\D}/\d t$ is the covariant derivative along $\mathbf{r}$ with respect to the Jacobi metric and $e^{2\rho} = 2(h - U)$. The Newton equation gives

$$
  \mu\left(\frac{\D\dot{\mathbf{r}}}{\d t}\right) = -\d U \iff \frac{\D\dot{\mathbf{r}}}{\d t} = -\operatorname{grad}(U) = e^{2\rho} \operatorname{grad}(\rho)
$$
...
$$
  \frac{\tilde{\mathbf{D}}\dot{\mathbf{r}}}{\d t} = 2\d\rho(\dot{\mathbf{r}})\dot{\mathbf{r}}
$$
...
$$
  \mu\left(\frac{\D\dot{\mathbf{r}}}{\d t}\right) = \sum_{i=1}^n \left[\frac{\d}{\d t}\left(\frac{\partial K}{\partial v^i} (\mathbf{x}(t), \dot{\mathbf{x}}(t)) \right) - \frac{\partial K}{\partial x^i} (\mathbf{x}(t), \dot{\mathbf{x}}(t)\right]\d x^i
$$

In particular, if $F = -\d U$ is conservative then the equations of motions are

$$
  \frac{\d}{\d t}\left(\frac{\partial K}{\partial v^i}(\mathbf{x}(t), \dot{\mathbf{x}}(t)) \right) - \frac{\partial K}{\partiall x^i}(\mathbf{x}(t), \dot{\mathbf{x}}(t)) = -\frac{\partial U}{\partial x^i}(\mathbf{x}(t))
$$
...
$$
\begin{align*}
  \frac{\d}{\d t}\left(\frac{\partial K}{\partial v^i} (\mathbf{x}(t), \dot{\mathbf{x}}(t))\right) =& \sum_{j=1}^n g_{ij}(\mathbf{x}(t)) \ddot{x}^j (t) \\
...
$$
\begin{align*}
  & \frac{\d}{\d t}\left(\frac{\partial K}{\partial v^i} (\mathbf{x}(t), \dot{\mathbf{x}}(t))\right) - \frac{\partial K}{\partial x^i}(\mathbf{x}(t), \dot{\mathbf{x}}(t)) \\
...
$$
  \mu(\mathbf{v})(\mathbf{w}) = \sum_{i,j=1}^n g_{ij}v^i w^j = \sum_{i,j=1}^n g_{ij} v^i \;\d x^j (\mathbf{w})
$$

and hence

$$
  \mu(\mathbf{v}) = \sum_{i,j=1}^n g_{ij} v^i \;\d x^j = \sum_{i,j=1}^n g_{ij} v^j \;\d x^i
$$

Thus

$$
  \mu\left(\frac{\mathbf{D}\dot{\mathbf{r}}}{\d t}(t)\right) = \sum_{i,j=1}^n g_{ij}(\mathbf{x}(t))\left(\ddot{x}^j (t) + \sum_{k,l=1}^n \Gamma_{kl}^j (\mathbf{x}(t))\dot{x}^k (t) \dot{x}^l (t) \right)\d x^i
$$
...
$$
\begin{align*}
  \langle\cdot,\cdot\rangle =& \d x\otimes\d x + \d y\otimes\d y \\
  =& \d r\otimes\d r + r^2 \d\theta\otimes\d\theta 
\end{align*}
$$
...
$$
\begin{align*}
  \frac{\d}{\d t}(m\dot{r}) - mr\dot{\theta}^2 =& -u'(r) \\
...
$$
  \mu\left(\frac{\D\dot{\mathbf{r}}}{\d t}\right) = (F + R)(\dot{\mathbf{r}})
$$
...
$$
\begin{align*}
  \frac{\d K}{\d t} =& \left\langle \frac{\D\dot{\mathbf{r}}}{\d t}, \dot{\mathbf{r}} \right\rangle =& F(\dot{\mathbf{r}})(\dot{\mathbf{r}}) + R(\dot{\mathbf{r}})(\dot{\mathbf{r}}) \\
...
$$
  \mu\left( \frac{\D\mathbf{r}}{\d t} \right) = (F + R)(\dot{\mathbf{r}})
$$

are exactly the motions of the mechanical system $(N, \langle\langle\cdot,\cdot\rangle, F|_N)$ where $\langle\langle\cdot,\cdot\rangle\rangle$ is the metric induced on $N$ by $\langle\cdot,\cdot\rangle$ and $F|_N$ is the restriction of $F$ to $N$. In particular, if $F = -\d U$ is conservative then $F|_N = -\d(U|_N)$.
...
$$
\begin{align*}
  \left{\langle\langle} \left(\mu^{-1} (F(\dot{\mathbf{r}})\right)^\top, \mathbf{v}\right{\rangle\rangle} =& \left\langle \mu^{-1} F(\dot{\mathbf{r}}), \mathbf{v} \right\rangle \\
...
<MathBox title='Spherical pendulum' boxType="definition">
A spherical pendulum consists of a particle of mass $m > 0$ that is connected to a fixed pivoting point through a massless rod of length $l > 0$. The motion of the particle under a constant gravitational acceleration $g$ is modeled by the mechanical system $(\R^3, \langle\langle\cdot,\cdot\rangle\rangle, -mg\;\d z, N)$, where 

$$
  \langle\langle\mathbf{v},\mathbf{w}\rangle\rangle := m\langle\mathbf{v},\mathbf{w}\rangle
$$

and $N$ is the holonomic constraint (with the origin at the pivoting point)

$$
  N = \Set{(x,y,z)\in\R^3 | x^2 + y^2 + z^2 = l^2 \in }
$$

which is diffeomorphic to $\mathbb{S}^2$. In other words, the motion of a spherical pendulum is equivalent to motion of a particle 


The holonomic contraint can be parametrized in spherical coordinates using the map $\xi: (0,\pi) \times (0,2\pi) \to\R^3$ defined by

$$
  \xi(\theta, \phi) = l(\sin(\theta)\cos(\phi), \sin(\theta) \sin(\phi), -\cos(\theta))
$$
...
$$
\begin{align*}
  \frac{\d}{\d\theta} =& \frac{\d x}{\d\theta}\frac{\partial}{\partial x} + \frac{\d y}{\d\theta}\frac{\partial}{\partial y} + \frac{\d z}{\d\theta}\frac{\partial}{\partial z} \\
  =& l\cos(\theta)\cos(\phi) \frac{\partial}{\partial x} + l\cos(\theta)\sin(\phi) \frac{\partial}{\partial x} + l\sin(\theta) \frac{\partial}{\partial y}
\end{align*}
$$
...
$$
\begin{align*}
  \frac{\d}{\d\phi} =& \frac{\d x}{\d\phi}\frac{\partial}{\partial x} + \frac{\d y}{\d\phi}\frac{\partial}{\partial y} + \frac{\d z}{\d\phi}\frac{\partial}{\partial z} \\
  =& -l\sin(\theta)\sin(\phi) \frac{\partial}{\partial x} - l\sin(\theta)\cos(\phi) \frac{\partial}{\partial x}
\end{align*}
$$
