...
A particle of mass $m > 0$ moving in $\R^2$ subject to a constant gravitational acceleration $g$ is modeled by the mechanical system $(\R^3, \langle\langle\cdot,\cdot\rangle\rangle, -mg\;\mathrm{d}z)$, where

$$
  \langle\langle\mathbf{v},\mathbf{w}\rangle\rangle := m\langle\mathbf{v},\mathbf{w}\rangle
$$

and $\langle\cdot,\cdot\rangle$ is the Euclidean inner product on $\R^3$. A simple pendulum is obtained by connecting the particle to a fixed pivoting point by an ideal massless rod of length $l > 0$. Setting the pivoting point as the origin, the holonomic constraint becomes

$$
  N = \Set{(x,y,z)\in\R^3 | x^2 + y^2 + z^2 = l^2 \in }
$$

which is diffeomorphic to $\mathbb{S}^2$.
</MathBox>
