...
subject: 'Physics/Quantum mechanics/Many-body systems'
showToc: true
references:
  - book_becca_sorella_2017
  - book_gubernatis_etal_2016
---
...
$$
	\sum_k (P_\text{max} - P_k) = MP_\text{max} - 1
$$
...
In the bootstrap method, we generate $N_\text{boot}$ data sets containing exactly $N$ points just by selecting randomly the points from the original data set, which is denoted by $\set{x_i}_{i=1}^N$. With this procedure, the probability that a data point is selected is $1/N$ and, therefore, on average it will appear once in each data set. However, in the new sample, each point of the original data set can appear more than once (or not a all).
...
$$
  P(n_i, \alpha) = \frac{N!}{n_{i, \alpha}! (N - n_{i,\alpha})!} p^{n_{i,\alpha}} (1 - p)^{N - n_{i,\alpha}}
$$
...
$$
\begin{equation*}
	\frac{\d\mathbf{R}(t)}{\d t} = \mathbf{f}[\mathbf{R}(r)] + \boldsymbol{\eta}(t)
\tag{\label{equation-1}}
\end{equation*}
$$
...
$$
\begin{equation*}
	\frac{\partial\mathcal{P}(\mathbf{R}, t)}{\partial t} = T \sum_\alpha \frac{\partial^2 \mathcal{P}(\mathbf{R}, t)}{\partial R_\alpha^2} - \sum_\alpha \frac{\partial}{\partial R_\alpha} [\mathcal{P}(\mathbf{R}, t) f_\alpha (\mathbf{R})]
\tag{\label{equation-9}}
\end{equation*}
$$
...
There is a deep relationship between the Fokker-Planck equation and the Schrödingr equation in imaginary time. This is obtained by writing the solution of $\eqref{equation-9}$ in the following form
...
Substituting the above definition of $\mathcal{P} (\mathbf{R}, t)$ into the Fokker-Planck equation $\eqref{equation-9}$, we obtain that $\Phi(\mathbf{R}, t)$ satisfies the Schrödinger equation in imaginary time
...
The evolution generated by the Fokker-Planck equation can be shown to satisfy the detailed balance condition. The formal solution of the Schrödinger equation is given by
...
The approximate spectrum is obtained by diagonalizing this finite matrix. By the variational principle, the lowest eigenvalue of this restriction provides an upper bound to the exact ground-state energy.
...
This state corresponds to occupying the bonding molecular orbital with two opposite spins and coincides with the Hartree–Fock solution. It is exact only in the absence of electron–electron repulsion. However, as $|\mathbf{R}_1 - \mathbf{R}_2| \to\infty$, this state does not approach the atomic limit because ionic configurations remain equally weighted.
...
The Hubbard model is also defined for negative interaction in the case of an effective attractive electron-electron interaction (e.g. n non-retarded electron-phonon coupling, leading to superconductivity). Thus, the Hubbard model with $U < 0$ can be seen as the simplest mode to capture the stabilization of a superconducting phase.
...
The effective Hamiltonian in the singly occupied subspace is therefore to second order in $t/U$ given by the antiferromagnetic Heisenberg model
...
The Heisenberg model is defined in the Hilbert space where each site is singly occupied. Note that the Heisenberg model also generalizes to arbitrary spin $\unitvec{S}$.

An exact solution of the Heisenberg model can be obtained in one spatial dimension, by the Bethe Ansatz. In this solution, the ground state is not magnetically ordered and the excitation spectrum is gapless (implying power-law spin-spin correlations). The absence of a true magnetic order in the ground-state of one-dimensional systems is due to an extension of the Mermin-Wagner theorem.

In more than one spatial dimension, for bipartite lattices (i.e., where the sites can be partitioned in two sub-lattices and the super-exchange term only couples sites on different sub-lattices) there are Monte Carlo methods that provide us with numerically exact results. These stochastic approaches have been crucial to definitively show that the ground state of the Heisenberg model on the two-dimensional square lattice has long-range magnetic (N´eel) order. 

Doping the Mott insulator in the Heisenberg model leads to the $t$-$J$ model
...
The Hubbard model is also defined for bosons, in which case the Hamiltonian reads

$$
\begin{equation*}
  \hat{H} = -t \sum_{\braket{i,j}} (\hat{b}_i^\dagger \hat{b}_j + \hat{b}_j^\dagger \hat{b}_i) + \frac{U}{2} \sum_i \hat{n}_i (\hat{n}_i - 1)
\tag{\label{equation-54}}
\end{equation*}
$$
where $\hat{b}_i^\dagger$ and $\hat{b}_i$ are the bosonic creation and annihilation operators respectively, and $\hat{n}_i = \hat{b}_i^\dagger \hat{b}_i$ is the density of bosons on site $i$. The creation and annihilation operators satisfy the commutation relations
...
The Hamiltonian $\eqref{equation-54}$ commutes with the total number of bosons $\hat{N}_b$, so that any sector with different number of particles can be considered separately. The Hilbert space of the bosonic Hubbard model is much larger than the fermionic one, since any number of bosons is allowed on each site. The bosonic Hubbard model is only defined for repulsive interactions, since bosons are thermodynamically unstable in presence of attractive interactions.

In the non-interacting limit $U = 0$, the ground state is obtained by condensing all bosons in the lowest-energy single-particle state

$$
  \ket{\phi_0} = \frac{1}{\sqrt{N_b}!} (\hat{b}_{k=0}^\dagger)^{N_b} \ket{0}
$$

In the atomic limit $t = 0$, the ground state is given by replacing bosons on sites and minimizing the number multiply occupied sites. For $N_b \leq L$ the ground state has only empty or singly occupied sites. While for $N_b = L$, the ground state, with one boson per site, is unique, which is in contrast to the fermionic case.

For this lattice model, there are no exact solutions (in the sense of the Bethe Ansatz) for generic values of the bosonic density $n = N_b / L$ and interaction $U$. However, it is possible to obtain numerically exact result by using Monte Carlo techniques.

## Variational principle

Let $\hat{H}$ be self-adjoint Hamiltonian acting on a Hilbert space $\mathcal{H}$, with discrete spectrum $\set{E_i}_{i\geq 0}$ and corresponding orthonormal eigenstates $\set{\ket{\Upsilon_i}}_{i\geq 0}$ satisfying

$$
  \hat{H}\ket{\Upsilon_i} = E_i \ket{\Upsilon_i},\quad \braket{\Upsilon_i|\Upsilon_j} = \delta_{ij}
$$

Given any nonzero trial state $\ket{\Psi'} \in\mathcal{H}$, its variational energy is defined as

$$
  E' = \frac{\braket{|\Psi'|\hat{H}|\Psi'}}{\braket{\Psi'|\Psi'}}
$$

Without loss of generality, we assume $\ket{\Psi'}$ is normalized, with $\braket{\Psi'|\Psi'} = 1$. Since $\ket{\Upsilon_i}$ form a complete orthonormal basis, we can expand

$$
\begin{equation*}
  \ket{\Psi'} = \sum_i a_i \ket{\Upsilon_i},\; a_i = \braket{\Upsilon_i | \Psi'}
...
$$
\begin{equation*}
  \braket{\Psi'|\Psi'} = \sum_i |a_i|^2 = 1
\tag{\label{equation-57}}
\end{equation*}
$$

Using the spectral resolution of $\hat{H}$, we obtain

$$
  E' = \sum_{i=1}^\infty |a_i|^2 E_i
$$

Subtracting the ground-state energy $E_0$, we find

$$
\begin{equation*}
  \epsilon := E' - E_0 = \sum_{i\neq 0} |a_i|^2 (E_i - E_0) \geq 0
\tag{\label{equation-56}}
\end{equation*}
$$

which holds since $E_i \geq E_0$ for all $i$. Hence $E' \geq E_0$, which implies that any trial state $\ket{\Psi'}$ provides an upper bound to the the exact ground-state energy. Variational methods consist in minimizing $E'$ within a restricted manifold of trial states.

### Relation between energy error and wave-function overlap

Assume that the ground state is nondegenerate and define the spectral gap

$$
  \Delta := E_1 - E_0 > 0
$$

From $\eqref{equation-56}$ and the fact that $E_i \geq E_0 + \Delta$ for all $i\geq 1$, it follows that

$$
  \epsilon\geq \Delta \sum_{i\neq 0} |a_i|^2
$$

then, by using the normalization condition $\eqref{equation-57}$, we finally have that

$$
  \eta = 1 - |a_0|^2 \leq \frac{\epsilon}{\Delta}
$$

Thus, the weight of excited-state components in $\ket{\Psi'}$ is controlled by the ratio $\epsilon/\Delta$. In particular, a sufficient condition for a good approximation in norm $\eta \ll 1$ is that the variational energy satisfies $\epsilon\ll\Delta$. Thus, the quality of the wave function in Hilbert-space norm is directly controlled by the gap.

### Accuracy of correlation functions

Energy expectation values are quadratic in the amplitude $a_i$ and therefore depend on $|a_i|^2$. The accuracy of the correlation functions is usually worse than the one on the ground-state energy. Consider a generic operator $\hat{O}$ and express the variational trial state as

$$
  \ket{\Psi'} = a_0 \ket{\Upsilon_0} + \sqrt{n} \ket{\Upsilon'}
$$

where
- $\braket{\Upsilon_0 |\Upsilon'} = 0$
- $\braket{\Upsilon' |\Upsilon'} = 1$
- $\eta = 1 - |a_0|^2$

Then, the difference between the expectation value calculated with the variational state and the exact one is given by

$$
  |\braket{\Psi'|\hat{O}|\Psi'} - \hat{O}_0| = |2a_0 \sqrt{\eta} \braket{\Upsilon_0|\hat{O}|\Upsilon'} + \eta\braket{\Upsilon'|\hat{O}|\Upsilon'} - \eta\hat{O}_0|
$$

where we have denoted $\hat{O}_0 = \braket{\Upsilon_0 |\hat{O}|\Upsilon_0}$ and assumed, for simplicity, real wave functions. Whenever the variational state is close to the exact ground state, $\eta\ll\sqrt{\eta}$, and we can neglect all terms that are propertional to $\eta$

$$
  |\braket{\Psi'|\hat{O}|\Psi'} - \hat{O}_0| \approx \sqrt{\eta} |\braket{\Upsilon_0 |\hat{O}|\Upsilon'}|
$$

which shows that the accuracy in correlation functions is more problematic than the one on the ground-state energy, with a term proportional to $\sqrt{\eta}$.

## Variational wave functions

### Hartree-Fock wave function

### Gutzweiler wave function

### Density-density Jastrow wave function

### Backflow wave function

### Haldane-Shastry wave function
...
Let $\mathcal{H}$ be a discrete Hilbert space with orthonormal basis $\set{\ket{x}}_{x\in\Omega}$, where $\Omega$ denotes the set of configurations. This basis satisfies the resolution of identity

$$
\begin{equation*}
  \sum_{x\in\Omega} \ket{x}\bra{x} = \hat{I}
\tag{\label{equation-59}}
\end{equation*}
$$

where $\hat{I}$ is the identity operator. For any state $\ket{\Psi} \in\mathcal{H}$, we define its coefficients in this basis by the wave function

$$
  \Psi(x) := \braket{x|\Psi}
$$

By completeness of this basis, $\ket{\Psi}$ admits the expansion

$$
  \ket{\Psi} = \sum_x \ket{x} \braket{x|\Psi} = \sum_x \Psi(x) \ket{x}
$$

The expectation value of a linear operator $\hat{O}$ in the state $\ket{\Psi}$ is defined by

$$
  \braket{\hat{O}} = \frac{\braket{\Psi|\hat{O}|\Psi}}{\braket{\Psi|\Psi}}
$$

Applying the resolution of identity $\eqref{equation-59}$, the nominator can be refactored as

$$
\begin{align*}
  \braket{\Psi|\hat{O}|\Psi} =& \sum_{x\in\Omega} \braket{\Psi|x}\braket{x|\hat{O}|\Psi} \\
  =& \sum_{x\in\Omega} \Psi^* (x) \braket{x|\hat{O}|\Psi}
\end{align*} 
$$
...
$$
\begin{align*}
  \braket{\Psi|\Psi} =& \sum_{x\in\Omega} \braket{\Psi|x}\braket{x|\Psi} \\
  =& \sum_{x\in\Omega} \Psi^* (x) \Psi(x) = \sum_{x\in\Omega} |\Psi(x)|^2
\end{align*}
$$

Substituting back, the expectation $\braket{\hat{O}}$ becomes

$$
\begin{equation*}
  \braket{\hat{O}} = \frac{\sum_{x\in\Omega} \Psi^* (x) \braket{x|\hat{O}|\Psi}}{\sum_{x\in\Omega} |\Psi(x)|^2}
\tag{\label{equation-58}}
\end{equation*}
$$

In many-body systems, the cardinality of $\Omega$ grows exponentially with the number of particles (even after restricting to symmetry sectors such as fixed particle number or fixed total spin component). Thus, direct summation over all configurations is computationally infeasible for large systems.

However, $\braket{\hat{O}}$ can be estimated using Monte Carlo sampling by expanding it in terms of a probability distribution. For this reformulation, we define the local estimator

$$
\begin{equation*}
  O_L (x) := \braket{\braket{x|\hat{O}|\Psi}}{\braket{x|\Psi}} = \frac{\braket{x|\hat{O}|\Psi}}{\Psi(x)}
\tag{\label{equation-61}}
\end{equation*}
$$

expressing $\braket{\hat{O}}$ as

$$
  \braket{\hat{O}} = \frac{\sum_{x\in\Omega} |\Psi(x)|^2 O_L (x)}{\sum_{x\in\Omega} |\Psi(x)|^2}
$$

We can define the probability measure $\mathcal{P}$ on $\Omega$ by

$$
  \mathcal{P}(x) := \frac{|\Psi(x)|^2}{\sum_{x\in\Omega} |\Psi(x)|^2}
$$

which satisfies $\mathcal{P}(x) \geq 0$ and $\sum_{x\in\Omega} \mathcal{P}(x) = 1$. Rewriting $\braket{\hat{O}}$ in terms of $\mathcal{P}(x)$ yields

$$
  \braket{\hat{O}} = \sum_{x\in\Omega} \mathcal{P}(x) O_L (x)
$$

This is recognized as the expectation value of the random variable $O_L (x)$ with respect to the probability measure $\mathcal{P}$ over $\Omega$. In particular, for the Hamiltonian $\hat{H}$, the corresponding local estimator

$$
  E_L (x) := \frac{\braket{x|\hat{H}|\Psi}}{\Psi(x)}
$$

is called the local energy, and

$$
\begin{equation*}
  \braket{\hat{H}} = \sum_{x\in\Omega} \mathcal{P}(x) E_L (x)
\tag{\label{equation-60}}
\end{equation*}
$$

Applying Monte Carlo sampling, we can generate a sequence of configurations $\set{\ket{x_n}}_{n=1}^N$ with stationary distribution $\mathcal{P}(x)$. After equilibration, the estimator

$$
  \braket{\hat{O}}_N := \frac{1}{N}\sum_{n=1}^N \hat{O}_L (x_n)
$$

converges to $\braket{\hat{O}}$ as $N\to\infty$. Thus, the computation of a quantum expectation value is reduced to the stochastic evaluation of a classical expectation value over configuration space.

## Zero-variance property

A fundamental property of the variational Monte Carlo method is the zero-variance property, which relates statistical fluctuation of the local energy to the spectral properties of the Hamiltonian. Suppose that the variational state $\ket{\Psi}\in\mathcal{H}$ coincides with an exact eigenstate of the Hamiltonian $\hat{H}$, i.e. $\hat{H}\ket{\Psi} = E\ket{\Psi}$. Then, it follows that the local energy $E_L$ is constant

$$
  E_L (x) = \frac{\braket{x|\hat{H}|\Psi}}{\braket{x|\Psi}} = E\frac{\braket{x|\Psi}}{\braket{x|\Psi}} = E
$$

Thus, the random variable $E_L$ is constant almost surely with respect to the probability distribution

$$
  \mathcal{P}(x) = `\frac{|\Psi(x)|^2}{\braket{\Psi|\Psi}}
$$

Specifically, the expectation and variance of $E_L$ become
- $\mathbb{E}_\mathcal{P} (E_L) = E$
- $\operatorname{var}_\mathcal{P} (E_L) = 0$

Thus, when the variational state coincides with an exact eigenstate, the local energy exhibits no statistical fluctuations. In this sense, the Monte Carlo estimator becomes deterministic.

In general, the variance of $E_L (x)$ decreases as the variational state $\ket{\Psi}$ approaches an eigenstate. To derive the generic variance of $E_L$, we first calculate its second moment with respect to $\mathcal{P}$

$$
\begin{align*}
  \mathbb{E}_\mathcal{P} (|E_L|^2) =& \sum_{x\in\Omega} \mathcal{P}(x) |E_L (x)|^2 \\
  =& \frac{\sum_{x\in\Omega} |\Psi(x)|^2 |E_L (x)|^2}{\braket{\Psi|\Psi}}
\end{align*}
$$

Inserting $\eqref{equation-60}$ and applying the completeness relation $\eqref{equation-59}$, the nominator becomes

$$
\begin{align*}
  \sum_{x\in\Omega} |\Psi(x)|^2 |E_L (x)|^2 =& \sum_{x\in\Omega} |\braket{x|\hat{H}|\Psi}|^2 \\
  =& \sum_{x\in\Omega} \braket{\Psi|\hat{H}|x} \braket{x|\hat{H}|\Psi} \\
  =& \braket{\Psi|\hat{H}^2 |\Psi}
\end{align*}
$$

Consequently, the second moment of $E_L$ is

$$
  \mathbb{E}_\mathcal{P} (|E_L|^2) = \frac{\braket{\Psi|\hat{H}^2|\Psi}}{\braket{\Psi|\Psi}}
$$

The variance of $E_L$ is then given by

$$
\begin{align*}
  \operatorname{var}_\mathcal{P} (E_L) =& \mathbb{E}_\mathcal{P} (|E_L|^2) - |\mathbb{E}_\mathcal{P} (E_L)|^2 \\
  =& \frac{\braket{\Psi|\hat{H}^2|\Psi}}{\Psi|\Psi} - \left(\frac{\braket{\Psi|\hat{H}|\Psi}}{\braket{\Psi|\Psi}} \right)^2
\end{align*}
$$

When the variational state coincides with an exact eigenstate such that $\mathbb{E}(E_L) = E$, then the variance of $E_L$ becomes

$$
  \operatorname{var}_\mathcal{P} (E_L) = \frac{\braket{\Psi|(\hat{E} - E)^2|\Psi}}{\braket{\Psi|\Psi}}
$$

which is precisely the quantum variance of $\hat{H}$ in the state $\ket{\Psi}$. Thus, the vanishing of the Monte Carlo variance of the local energy is mathematically equivalent to the eigenvalue equation.

## Local occupation-number basis

In lattice many-body problems, it is natural to work in a local occupation-number basis constructe as a tensor product of on-site Hilbert spaces. Consider a lattice with $L$ sites. At each site $i=1,\dots,K$, let $\mathcal{H}_i$ denote the local Hilbert space, equipped with an orthonormal basis $\set{\ket{\xi_i}}$. The dimension of $\mathcal{H}_i$ depends on the physical model.

1. **Single-band fermionic Hubbard model:** For spin-$1/2$ fermions on site $i$, the local Hilbert space is four-dimensional, spanned by
$$
  \mathcal{H}_i = \operatorname{span}\set{\ket{0}_i, \hat{c}_{i,\uparrow}^\dagger \ket{0}_i, \hat{c}_{i,\downarrow}^\dagger \ket{0}_i, \hat{c}_{i,\uparrow}^\dagger \hat{c}_{i,\downarrow}^\dagger \ket{0}_i}
$$

2. **Bosonic Hubbard model:** For bosons, the local Hilbert space is infinite-dimension with an arbitrary number of bosons on the same site
$$
  \mathcal{H}_i = \operatorname{span}\Set{\frac{1}{\sqrt{n!}} (\hat{b}_i^\dagger)^n \ket{0_i}}_{n\in\N_0}
$$

3. **Spin-$S$ Heisenberg model:** For a spin-$S$ system, the local Hilbert space $2S + 1$, with basis given by eigenstates of $\hat{S}_i^z$
$$
  \hat{S}_i^z \ket{m}_i = m\ket{m}_i,\; m = -S,\dots,S
$$

The Hilbert space of the full lattice is the tensor product

$$
  \mathcal{H} = \bigotimes_{i=1}^L \mathcal{H}_i
$$

Once the local bases are fixed, a generic basis element of $\mathcal{H}$ is a product state

$$
  \ket{x} = \bigotimes_{i=1}^L \ket{\xi_i}
$$

In particular, the global vacuum state is

$$
  \ket{0} = \bigotimes_{i=1}^L \ket{0_i}
$$

Chososing a local basis is convenient because lattice Hamiltonians are typically sums of local operators, i.e.

$$
  \hat{H} = \sum_i \hat{h}_i + \sum_{\braket{i,j}} \hat{h}_{ij}
$$

where each term acts non-trivially only on a finite subset of sites. In particular,
- interaction terms (e.g. Hubbard $U \hat{n}_{i,\uparrow} \hat{n}_{j,\downarrow}$) are diagonal in the occupation-number basis
- on-site terms (e.g. Hund's couplings in multiband models) act locally
- hopping terms couple only neighbouring sites

As a consequence, for any fixed configuration $\ket{x}$, the number of configurations $\ket{x'}$ such that $\braket{x|\hat{H}|x'} \neq 0$ scales at most linearly with $L$. 

For fermionic systems, additional care is required because of the antisymmetric nature of the Fock space. A basic configuration must be specified together with a fixed ordering convention for creation operators. Consider $N_e$ spinless fermions on $L$ sites. A configuration is written as

$$
  \ket{x} = \left(\prod_{i=1}^{N_e} \hat{c}_{R_i}^\dagger \right) \ket{0}
$$

where $R_1 <\cdots < R_{N_e}$ is a strictly ordered sequence of occupation sites. This ordering convention ensures a unique representative of each Fock state. It is convenient to encode the configuration by an occupation vector $\boldsymbol{\kappa} \in \set{0,1}^L$, where $\kappa_i = 1$ if site $i$ is occupied and $\kappa_i = 0$ otherwise.

Consider a hopping term $\hat{c}_{R'_l}^\dagger \hat{c}_{R_l}$ acting on $\ket{x}$, where $R_l$ is the one of the occupied sites and $R'_l$ is an empty site. Formally

$$
  \hat{c}_{R'_l}^\dagger \hat{c}_{R_l} \ket{x} = \hat{c}_{R'_l}^\dagger \hat{c}_{R_l} \hat{c}_{R_1}^\dagger \cdots \hat{c}_{R_{N_e}}^\dagger \ket{0}
$$

using the anticommutation relations $[\hat{c}_i, \hat{c}_i^\dagger]_+ = \delta_{ij}$, the annihilation operator $\hat{c}_{R_l}$ must be anticommuted through the ordered string until it reaches its matching creation operators. Thus produces a sign factor $(-1)^p$, where $p$ is the number of fermionic operators it passes. After annihilation and reinsertion of $\hat{c}_{R'_l}^\dagger$, the resulting state can again be written in canonical ordered form

$$
  \ket{x'} = (-1)^{p'} \hat{c}_{R_1}^\dagger \cdots \hat{c}_{R'_l}^\dagger \cdots \hat{c}_{R_{N_e}}^\dagger \ket{0}
$$

Thus, relative to the canonical ordering convention, the new configuration differs from the old one by
1. A change of occupation at sites $R_l$ and $R'_l$
2. A computable fermionic sign determined by the parity of the permutation required to restore ordering

During the Markov process, proposed configurations are often generated by applying local Hamiltonian terms. For fermionic systems
- The configuration space consists of ordered occupation string
- Each update may produce a sign factor
- These signs must be consistently tracked when evaluating amplitude ratios $\Psi(x')/\Psi(x)$ and local estimators

Provided a consistent ordering convention is adopted and permutation signs are properly accounted for, this sign bookkeeping poses no conceptual difficulty, though it is essential for correctness.

## Monte Carlo estimation

In the Monte Carlo evaluation of the quantum expectation value $\eqref{equation-58}$, we must compute two quantities repeatedly along the Markov chain:

1. **Ratios of probabilities:** In the Metropolis scheme, only ratios of probabilities are required. For two configurations $x, x' \in\Omega$

$$
\begin{equation*}
  \frac{\mathcal{P}(x')}{\mathcal{P}(x)} = \left| \frac{\Psi(x')}{\Psi(x)} \right|^2 = \left|\frac{\braket{x'|\Psi}}{\braket{x|\Psi}} \right|^2
\tag{\label{equation-68}}
\end{equation*}
$$

Since the normalization constant of $\mathcal{P}$ cancels, only ratios of overlaps of the variational state with basis configurations must be evaluated.

2. **Local estimator:** Evaluating the local estimator $\eqref{equation-61}$ requires computing ratios of overlaps and matrix elements of the observable between states of the basis set. For the Hamiltonian, $\hat{H}$, the local energy is

$$
\begin{equation*}
\begin{split}
  E_L =& \frac{\braket{x|\hat{H}|\Psi}}{\braket{x|\Psi}} \\
  =& \sum_{x' \in\Omega} \braket{x|\hat{H}|x'} \frac{\braket{x'|\Psi}}{\braket{x|\Psi}}
\end{split}
\tag{\label{equation-99}}
\end{equation*}
$$

This expression involves a sum over the entire Hilbert space. However, due to the locality of the Hamiltonian, only few terms actually contribute to the sum, reducing the computational complexity.

The local energy or other local observables are written in terms of wave functions times the matrix elements of the operator $\braket{x|\hat{O}|x'}$ according to $\eqref{equation-99}$. Let us consider a generic $n$-body operator

$$
  \hat{O} = \sum_{I_1,\dots,I_n} \sum_{J_1,\dots,J_n} \hat{O}_{I_1,\dots,I_n;J_1\dots,J_n} \hat{d}_{I_1}^\dagger \hat{d}_{J_1} \cdots \hat{d}_{I_n}^\dagger \hat{d}_{J_n}
$$

where $\hat{d}_I^\dagger$ ($\hat{d}_I$) creates (destroy) an electron on the site $I=1,\dots,2L$ (a similar definition holds for a bosonic system). Then the local estimator of $\eqref{equation-61}$ is given by:

$$
  O_L (x) = \sum_{I_1,\dots,I_n} \sum_{J_1,\dots,J_n} \hat{O}_{I_1,\dots,I_n;J_1,\dots,J_n} \frac{\braket{x|\hat{d}_{I_1}^\dagger \hat{d}_{J_1} \cdots \hat{d}_{I_n}^\dagger \hat{d}_{J_n}|\Psi}}{\braket{x|\Psi}}
$$

### Locality and sparsity

In lattice many-body systems, $\hat{H}$ is typically a sum of local operators

$$
  \hat{H} = \sum_i \hat{h}_i
$$

where each $\hat{h}_i$ acts non-trivially only on a finite subset of degrees of freedom. In the configuration basis $\set{\ket{x}}$, this implies that for fixed $x$

$$
  \braket{x|\hat{H}|x'} \neq 0
$$

only if $x'$ differs from $x$ locally, according to the action one of the $\hat{h}_i$. Consequently, for each configuration $x$, the number of configurations $x'$ contributing to the sum for $E_L (x)$ scales at most polynomially with the system size (typically linearly in the number of sites or particles), rather than exponentially with $|\Omega|$.

<MathBox title="Fermionic Hubbard model" boxType="example">
Consider the fermionic Hubbard model $\eqref{equation-52}$ on a lattice with $L$ sites. In the occupation-number basis, a configuration $\ket{x}$ specifies the occupation of each site and spin. The hopping term connects $\ket{x}$ only to configurations $\ket{x'}$ that differ by the displacement of a single fermion between neighbouring sites. Thun number of such processes is bounded by a constant multiple of the number of bonds. The interaction term is diagonal in this basis. Thus, evaluating $E_L (x)$ requires summing only over $O(L)$ configuations, making its computation polynial in the system size.
</MathBox>

### Evaluation of amplitudes

The primary computational task in variational Monte Carlo is the evaluation of the amplitudes $\Psi(x) = \braket{x|\Psi}$, and of their ratios $\Psi(x')/\Psi(x)$ for configurations $x, x' \in\Omega$ connected by local updates. Since this computation must be performed at each Monte Carlo step, the efficiency of the algorithm strongly contrains the admissible class of variational wave functions.

For fermionic systems, antisymmetry implies that many relevant Ansätze can be expressed in terms of determinants or, more generally, Pfaffians
- The determinant of an $N\times N$ matrix can be computed in $O(N^3)$ time using standard algorithms
- Pfaffians can likewise be evaluated in polynomial time

When configurations differ by local moves (e.g. single-particle updates), matrix-update formulas allow the ratio $\Psi(x')/\Psi(x)$ to be computed in $O(N^2)$ time. This polynomial complexity makes such fermionic Ansätze computationally tractable for moderately large systems.

For bosonic systems, symmetrization leads in general to permanents rather than determinants. The permamnent of $N\times N$ matrix is defined as

$$
  \operatorname{perm}(M) = \sum_{\sigma\in S_N} \prod_{i=1}^N M_{i,\sigma(i)} 
$$

which differs from the determinant by the absence of permutation signs. Unlike determinants, permanents cannot be computed in polynomial time, but rather require time exponential in $N$. This severely limits the applicability of general bosonic permanent-based Ansätze. However, important physically relevant bosonic states have amplitudes that can be evaluated in polynomial time and are therefore amenable to variational Monte Carlo methods. This includes condensates in which all bosons occupy a single orbital.

## Jastrow variational state

A Jastrow-correlated variational state is defined by

$$
  \ket{\Psi_\text{J}} = \hat{J}\ket{\Phi_0}
$$

where $\ket{\Phi_0}$ is a generic uncorrelated (bosonic or fermionic) state and $\hat{J}$ is the Jastrow factor, encoding electron correlations. A widely used class of Jastrow factors consists of density-density correlations in the form

$$
\begin{equation*}
  \hat{J} = \exp\left(-\frac{1}{2} \sum_{i,j=1}^L v_{ij} \hat{n}_i \hat{n}_j \right)
\tag{\label{equation-62}}
\end{equation*}
$$

where $v_{ij} \in\R$ are variational parameters, whose total number is $L^2$.

<details>
<summary>Details</summary>

The pseudo-potential $v_{ij}$ couples densitites at different sites $i$ and $j$ and not density fluctuations.

The Jastrow factor is often written in terms of density fluctuations $\delta\hat{n}_i = \hat{n}_i - \bar{n}$, where $\bar{n}$ is the average density, i.e.

$$
\begin{equation*}
  \hat{J} = \exp\left(-\frac{1}{2} \sum_{i,j=1}^L v_{ij} \delta\hat{n}_i \delta\hat{n}_j \right)
\tag{\label{equation-63}}
\end{equation*}
$$

For systems with fixed total particle number

$$
  \sum_i \hat{n}_i = N
$$

The alternative forms of the Jastrow factor in $\eqref{equation-62}$ and $\eqref{equation-63}$ differ only by additive and linear terms in $\hat{n}_i$, which contribute at most a configuration-independent multiplicative factor to $\ket{\Psi_\text{J}}$. Hence, for fixed particle number, the two definitions are equivalent up to normalization.
</details>

If the system is translationally invariant, then $v_{ij}$ only depends on the distance between $i$ and $j$, i.e. $v_{ij} = v(|\mathbf{r}_i - \mathbf{r}_j|)$. In this case, the number of independent parameters is reduced from $O(L^2)$ to $O(L)$. 

Certain singular limits of the Jastrow factor can be treated exactly with Monte Carlo sampling. For example, imposing $v_{ii} \to\infty$ suppresses configurations with $n_i^2 \neq n_i$ (e.g. double occupancy for spinful fermions), effectively implementing the Gutzwiller projector relevant to the infinite-$U$ limit of the Hubbard model $\eqref{equation-52}$. In practice, this constraint is enforced by constructing a Markov chain whose state space is restricted to configurations without double-occupancy. This way we avoid evaluating the singular factor explicitly.

The Jastrow factor $\eqref{equation-62}$ is diagonal in the occupation-number basis $\set{\ket{x}}_{x\in\Omega}$. Since $\hat{n}_i \ket{x} = n_i (x) \ket{x}$, where $n_i (x) \in\N$ is the occupation number at site $i$ in configuration $x$, it follows that

$$
\begin{align*}
  \hat{J}\ket{x} =& \exp\left( -\frac{1}{2} \sum_{i,j=1}^L v_{ij} n_i (x) n_j (x) \right)\ket{x} \\
  =:& J(x) \ket{x}
\end{align*}
$$

Showing that $J(x)$ is a scalar function on configuration space. For the Jastrow-correlated state $\ket{\Psi_\text{J}} = \hat{J}\ket{\Phi_0}$, we thus obtain

$$
  \braket{x|\Psi_\text{J}} = \braket{x|\hat{J}|\Phi_0} = J(x) \braket{x|\Phi_0}
$$

In this diagonal form, $J(x)$ can be evaluated in $O(L^2)$ operations due to the double sum. In order to ensure polynomial complexity, the Jastrow factor must only contain operators that are diagonal in the basis $\ket{x}$. If $\hat{J}$ contained off-diagonal operators (e.g. hopping terms), then $\hat{J}\ket{x}$ would generate a superposition of exponentially many configurations, making the evaluation of $\braket{x|\Psi_\text{J}}$ exponentially costly in $L$. The remaining computational effor lies in evaluating $\braket{x|\Phi_0}$:
- For fermionic reference states given by Slater determinants, amplitudes can be computed in $O(L^3)$ time per local update using matrix-update formulas
- For bosonic condensed product states, the amplitude factorizes and can typically be evaluated in $O(1)$ or $O(L)$ time

### Bosonic systems

#### Non-interacting condensate state

Consider a bosonic lattice system with $L$ sites and creation and annihilation operators $\set{\hat{b}_i^\dagger, \hat{b}_i}_{i=1}^L$. Let $V_\alpha = (V_{i,\alpha})_{i=1}^L \in \mathbb{C}^L$ be a normalized single-particle wave function satisfying

$$
\begin{equation*}
  \sum_{i=1}^L |V_{i,\alpha}|^2 = 1
\tag{\label{equation-64}}
\end{equation*}
$$

where 
- $\set{V_{i,\alpha}}$ are generic amplitudes for having the boson on site $i$
- $\alpha$ is an index that specifies the orbital

We define the associated creation operator

$$
  \phi_\alpha^\dagger := \sum_{i=1}^L V_{i,\alpha} \hat{b}_i^\dagger
$$

The corresponding single-particle state is

$$
  \ket{\phi_\alpha} := \hat{\phi}_\alpha^\dagger \ket{0} = \left(\sum_i V_{i,\alpha} \hat{b}_i^\dagger \right) \ket{0}
$$

The normalization condition $\eqref{equation-64}$ ensures that

$$
  \braket{\phi_\alpha |\phi_\alpha} = \braket{0|\hat{\phi}_\alpha \hat{\phi}_\alpha^\dagger | 0} = 1
$$

In particular, the zero-momentum orbital on a translationally invariant lattice is obtained by choosing

$$
  V_{i,\alpha} = \frac{1}/{\sqrt{L}}
$$

which yields the uniform superposition over all sites.

The many-body state with $N_b \in\N_+$ bosons occupying the same orbital $\alpha$ is defined by

$$
\begin{equation*}
\begin{split}
  \ket{\Phi_0} :=& \frac{1}{\sqrt{N_b !}} (\hat{\phi}_\alpha^\dagger)^{N_b} \ket{0} \\
  =& \frac{1}{\sqrt{N_b!}} \left(\sum_i V_{i,\alpha} \hat{b}_i^\dagger \right)^{N_b} \ket{0}
\end{split}
\tag{\label{equation-65}}
\end{equation*}
$$

Define the normalized occupation-number basis states by

$$
\begin{equation*}
  \ket{n_1,\dots,n_L} := \frac{1}{\sqrt{n_1! \cdots n_L!}} \left(\prod_{i=1}^L (\hat{b}_i^\dagger)^{n_i} \right) \ket{0}
\tag{\label{equation-66}}
\end{equation*}
$$

where $n_i \in\N_0$ denotes the number of bosons at site $i$ such that
...
$$
  \left(\sum_{i=1}^L V_{i,\alpha} \hat{b}_i^\dagger \right)^{N_b} = \sum_{\substack{m_1,\dots,m_L \geq 0 \\ \sum_i m_i = N_b}} \frac{N_b!}{m_1! \cdots m_L!} \prod_{i=1}^L (V_{i,\alpha} \hat{b}_i^\dagger)^{m_i}
$$

we can refactor $\eqref{equation-65}$ as

$$
  \ket{\Phi_0} = \frac{1}{\sqrt{N_b}} \sum_{\substack{m_1,\dots,m_L \geq 0 \\ \sum_i m_i = N_b}} \frac{N_b!}{m_1! \cdots m_L!} \left(\prod_{i=1}^L V_{i,\alpha}^{m_i} (\hat{b}_i^\dagger)^{m_i} \right) \ket{0}
$$

Rewriting this in terms of the normalized basis states $\eqref{equation-66}$ yields

$$
  \ket{\Phi_0} = \sum_{\substack{m_1,\dots,m_L \geq 0 \\ \sum_i m_i = N_b}} \sqrt{\frac{N_b}{n_1! \cdots n_L!}} \left(\prod_{i=1}^L V_{i=1}^{n_i} \right) \ket{n_1,\dots,n_L}
$$

For a fixed configuration $\ket{x} := \ket{n_1,\dots,n_L}$, the overlap $\braket{x|\Phi_0}$ is therefore given by

$$
\begin{equation*}
  \braket{n_1,\dots,n_L|\Phi_0} = \sqrt{\frac{N_b!}{n_1! \cdots n_L!}} \prod_{i=1}^L V_{i,\alpha}^{n_i}
\tag{\label{equation-67}}
\end{equation*}
$$

This shows that the probability amplitude has the structure of a multinomial distribution in the coefficients $V_{i,\alpha}$, reflecting the complete condensation of all $N_b$ bosons into single one-body orbital.

#### Computation of amplitude ratio

With the overlap expression $\eqref{equaton-67}$, we can efficiently evaluate ratios of amplitudes corresponding to configurations that differ locally. 

If we consider two configurations $x, x' \in\Omega$ that only differ by a boson hopping between sites $l \to k$. In the occupation-number basis, $\ket{x}$ and $\ket{x'}$ take the form 

$$
\begin{equation*}
\begin{split}
  \ket{x} :=& \ket{n_1,\dots,n_k,\dots,n_l,\dots,n_L} \\
  \ket{x'} :=& \ket{n_1,\dots,n_k + 1,\dots,n_l - 1,\dots,n_L}
\end{split}
\tag{\label{equation-69}}
\end{equation*}
$$

then using $\eqref{equation-68}$, we obtain the amplitude ratio

$$
\begin{align*}
  \frac{\braket{x'|\Phi_0}}{\braket{x|\Phi_0}} =& \frac{\sqrt{\frac{N_b!}{(n_k + 1)! (n_l - 1)! \prod_{i\neq k,l} n_i!}} V_{k,\alpha}^{n_k + 1} V_{l,\alpha}^{n_l - 1}}{\sqrt{\frac{N_b!}{n_k! n_l! \prod_{i\neq k,l} n_i!}} V_{k,\alpha}^{n_k} V_{l,\alpha}^{n_l}} \\
  =& \sqrt{\frac{n_l}{n_k + 1}} \frac{V_{k,\alpha}}{V_{l,\alpha}}
\end{align*}
$$

If multiple bosons are moved simultaneously, analogous formulas follow from the same multinomial structure, involving only the occupations of the sites affected by the update.

For a single-boson hopping update, the ratio of amplitudes depends only on the local occupations at sites $k$ and $l$, and on the corresponding orbital coefficients. Since this ratio is indpendent of both the total number of sites $L$ and the total number of particles $N_b$, its evaluation requires $O(1)$ operations. Consequently, fully condensed bosonic systems are well suited for Monte Carlo sampling schemes, since the

The calculation of $\braket{x|\Phi_0}$ becomes dramatically complicated if the bosons are not condensed. Consider the general non-interacting state in which the $N_b$ bosons occupy (possibly distinct) single-particle orbitals

$$
  \ket{\Phi_0} = \prod_{\alpha=1}^{N_b} \left(\sum_i V_{i,\alpha} \hat{b}_i^\dagger \right) \ket{0}
$$

In this case, the overlap of a configuration $\ket{x} := \ket{n_1,\dots,n_L}$ includes the permanent of an $N_b \times N_b$ matrix in form

$$
  \braket{n_1,\dots,n_L|\Phi_0} =  \frac{1}{\sqrt{n_1! \cdots n_L!}} \operatorname{per}(V_{R_j, \alpha})_{j,\alpha=1}^{N_b}
$$

where the $R_j$ take the values of the occupied sites. The values of $R_j$ may appear more than once, according to the occupation of each site, such that site $k$ appears exactly $n_k$ times. 

The calculation of the permanent of an $N_b \times N_b$ matrix requires exponential time in $N_b$ in the worst case. Consequently, evaluating such overlaps (or their ratios) becomes computationally prohibitive for large particle numbers.

#### Computation of the Jastrow factor

For a configuration $x := (n_i)_{i=1}^L \in\Omega$ with $n_i \in\N_0$, let the value of the Jastrow factor $\eqref{equation-62}$ be given by

$$
  \hat{J}(x) = \exp\left(-\sum_{i,j=1}^L v_{ij} n_i n_j \right)
$$

where the pseudo-potential $v_{ij} \in\R$ is assumed to satisfy
- **symmetry:** $v_{ij} = v_{jk}$
- **translational invariance:** $v_{ii} = v_{jj}$ for all $i,j$

A direct evaluation of $\hat{J}(x)$ involves summing over all pairs $(i,j)$, therefore requiring $O(L^2)$ operations. Consequently, computing the ratio $\hat{J}(x')/\hat{J}(x)$ by recomputing both exponents from scratch also costs $O(L^2)$. However, if two configurations $x, x' \in\Omega$ differ only locally, e.g. by few boson hoppings, a more computationally efficient update is possible.

Consider the case in which two configurations $x, x' \in\Omega$ differ only by a single boson hopping between sites $l\to k$. In the occupation-number basis, $\ket{x}$ and $\ket{x'}$ take the same form as in $\eqref{equation-69}$. Thus, the updated occupation numbers can be written $n'_i = n_i + \delta_{ik} + \delta_{ij}$. Defining

$$
  E(x) := \sum_{i,j} v_{ij} n_i n_j
$$

the ratio of Jastrow factors can be expressed as

$$
\begin{equation*}
  \frac{\hat{J}(x')}{\hat{J}(x)} = \exp\left(-[E(x') - E(x)]\right)
\tag{\label{equation-70}}
\end{equation*}
$$

By symmetry and translation invariance of $v_{ij}$ with $v_0 = v_{ii}$, we get

$$
  E(x') - E(x) = 2\sum_i (v_{ik} - v_{il}) n_i + 2(v_0 - v_{kl})
$$

<details>
<summary>Details</summary>

Evaluating $E(x')$ yields

$$
\begin{align*}
  E(x') =& \sum_{i,j} v_{ij} n'_i n'_j \\
  =& \sum_{i,j} v_{ij} (n_i + \delta_{ik} - \delta_{jk})(n_j + \delta_{jk} - \delta_{jl})
\end{align*}
$$
...
$$
\begin{align*}
  &(n_i + \delta_{ik} - \delta_{jk})(n_j + \delta_{jk} - \delta_{jl}) \\
  =& n_i n_j + n_i (\delta_{jk} - \delta_{jl}) + n_j (\delta_{ik} - \delta_{il}) + (\delta_{ik} - \delta_{il})(\delta_{jk} - \delta_{jl})
\end{align*}
$$

Using the property $\sum_j f(j) \delta_{jk} = f(k)$, then $E(x')$ expands to

$$
\begin{align*}
  E(x') =& \sum_{i,j} v_{ij} n_i n_j + \sum_i v_{ik} n_i - v_{il} n_i + \sum_j v_{kj} n_j - v_{lj} n_j \\
  &+ v_{kk} - v_{kl} - v_{lk} + v_{ll}
\end{align*}
$$

If $v_{ij} = v_{jk}$, then the two linear terms are equal. Additionally, if $v_{ii} = v_0$ is site-independent due to translational invariance, then $E(x')$ simplifies to

$$
\begin{align*}
  E(x') =& \sum_{i,j} v_{ij} n_i n_j + 2\sum_i (v_{ik} - v_{il})n_i + \underbrace{v_{kk} + v_{ll}}_{=2v_0} - 2v_{kl} \\
  =& \sum_{i,j} v_{ij} n_i n_j + 2\sum_i (v_{ik} - v_{il})n_i + 2(v_0 - v_{kl})
\end{align*}  
$$
</details>

The ratio of Jastrow factors $\eqref{equation-70}$ therefore becomes

$$
\begin{equation*}
  \frac{\hat{J}(x')}{\hat{J}(x)} = \exp\left(-2\sum_i (v_{ik} - v_{il})n_i \right) \exp(-2(v_0 - v_{kl}))
\tag{\label{equation-72}}
\end{equation*}
$$

The second exponential factor depends only on the pseudo-potential and can therefore by precomputed in the Markov chain.

Introducing the auxiliary quantity

$$
  \mathbf{t}_\text{J} (j) := \sum_{i=1}^L v_{ij} n_i
$$

the ratio $\eqref{equation-72}$ can be expressed as

$$
  \frac{\hat{J}(x')}{\hat{J}(x)} = \exp\left(-2[\mathbf{t}_\text{J} (k) - \mathbf{t}_\text{J} (l)]\right) \exp(-2(v_0 - v_{kl}))
$$

Thus, once the vector $\mathbf{t}_\text{J}$ has been computed, the ratio evaluation requires only $O(1)$ operations. At the beginning of the Markov chain, $\mathbf{t}_\text{J} (j)$ is computed from scratch with $O(L^2)$ complexity. After each accepted move along the chain, $\mathbf{t}_\text{J} (j)$ is updated in $O(L)$ times. For a single-boson hopping update with $n'_i = n_i + \delta_{ik} - \delta_{il}$, the update $\mathbf{t}'_\text{J} (j)$ is given by

$$
\begin{align*}
  \mathbf{t}'_\text{J} (j) =& \sum_i v_{ij} n'_i = \sum_i v_{ij} (n_i + \delta_{ik} - \delta_{ij}) \\
  =& \sum_i v_{ij} n_i + v_{kj} - v_{lj} = \mathbf{t}_\text{J} + v_{kj} - v_{lj}
\end{align*}
$$

Computing $\mathbf{t}'_\text{J}$ for $j=1,\dots,L$ therefore requires $O(L)$ operations.

In practice, it is numerically advisable to recompute $\mathbf{t}_\text{J}$ periodically from scratch to prevent the accumulation of floating-point round-off errors.

### Fermionic systems with determinants

Consider a fermionic lattice system with $L$ sites and creation and annihilation operators $\set{\hat{c}_i^\dagger, \hat{c}_i}_{i=1}^L$. We combine the site index $i=1,\dots,L$ and electron spin index $\sigma\in\set{\uparrow, \downarrow}$ into a single composite index $I=1,\dots,2L$, by defining the operators

$$
  \hat{c}_{i,\uparrow} =: \hat{d}_i,\quad \hat{c}_{i,\downarrow} =: \hat{d}_{i+L}
$$
...
$$
\begin{align*}
  [\hat{d}_I, \hat{d}_J]_+ =& [\hat{d}_I^\dagger, \hat{d}_J^\dagger]_+ = 0 \\
  [\hat{d}_I, \hat{d}_J^\dagger]_+ =& \delta_{IJ}
\end{align*}
$$

Consider a non-interacting Hamiltonian taking the quadratic form

$$
  \hat{H}_0 = \sum_{I,J=1}^{2L} t_{IJ} \hat{d}_I^\dagger \hat{d}_J
$$
...
$$
\begin{equation*}
  \hat{H}_0 = \unitvec{d}^\dagger \mathbf{T} \unitvec{d}
\tag{\label{equation-77}}
\end{equation*}
$$

where 
- $\mathbf{T} = (t_{IJ}) \in \mathbb{C}^{2L\times 2L}$ is Hermitian
- $\unitvec{d}^\dagger = [\hat{d}_i^\dagger]_{i=1}^{2L}$

The Hamiltonian includes arbitrary hopping processes, possibly mixing spin sectors (i.e. matrix elements with $I \leq L$ and $J > L$ and vice versa).

Since $\hat{H}_0$ commutes with the total number of electrons 

$$
  \hat{N}_e = \sum_I \hat{d}_I^\dagger \hat{d}_I
$$

particle number is conserved, and the many-body eigenstates can be constructed from single-particle orbitals. Because $\mathbf{T}$ is Hermitian, there exists a unitary matrix $\mathbf{U}\in \operatorname{U}(2L)$ such that

$$
  \mathbf{U}^\dagger \mathbf{TU} = \mathbf{E} = \operatorname{diag}(\varepsilon_1,\dots,\varepsilon_{2L})
$$

where $\varepsilon_\alpha$ are the single-particle eigenvalues. This allows us to define the new fermionic operators

$$
\begin{align*}
  \hat{\phi}_\alpha^\dagger =& \sum_{I=1}^{2L} U_{I,\alpha} \hat{d}_I^\dagger \tag{\label{equation-78}} \\
  \hat{\phi}_\alpha =& \sum_{I=1}^{2L} U_{I,\alpha}^* \hat{d}_I \tag{\label{equation-79}}
\end{align*}
$$

Unitarity of $\mathbf{U}$ guarantees that the operators $\hat{\phi}_\alpha$ the canonical anticommutation relations.

In this basis, $\hat{H}_0$ takes the diagonal form

$$
\begin{align*}
  \hat{H}_0 =& \unitvec{d}^\dagger \mathbf{UU}^\dagger \mathbf{TUU}^\dagger \unitvec{d} \\
  =& \unitsym{\Phi}^\dagger \mathbf{E} \unitsym{\Phi} = \sum_\alpha \varepsilon_\alpha \hat{\phi}_\alpha^\dagger \hat{\phi}_\alpha
\end{align*}
$$

Assuming the eigenvalues of $\hat{H}_0$ are ordered with $\varepsilon_1 \leq\cdots\leq \varepsilon_{2L}$, the $N_e$-particle ground state $\ket{\Phi_0}$ can be constructed by occupying the $N_e$ lowest energy orbitals, giving the Slater determinant

$$
\begin{equation*}
  \ket{\Phi_0} = \left(\prod_{\alpha=1}^{N_e} \hat{\phi}_\alpha^\dagger\right) \ket{0}
\tag{\label{equation-73}}
\end{equation*}
$$

A generic occupation-number configuration $\ket{x}$ with $N_e$ electrons is

$$
\begin{equation*}
  \ket{x} = \left(\prod_{i=1}^{N_e} \hat{d}_{R_i}^\dagger\right) \ket{0}
\tag{\label{equation-90}}
\end{equation*}
$$

where
- $R_j \in\set{1,\dots,2L}$ are all distinct
- the ordering $R_1 < \cdots < R_{N_e}$ is conventionally fixed to define a canonical sign

The overlap $\braket{x|\Phi_0}$ is the given by

$$
  \braket{x|\Phi_0} = \Braket{0| \hat{d}_{R_{N_e}} \cdots \hat{d}_{R_1} \left(\sum_p (-1)^p \prod_{\alpha=1}^{N_e} U_{p\set{R_j}, \alpha} \right) \hat{d}_{R_1}^\dagger \cdots \hat{d}_{R_{N_e}}^\dagger |0}
$$

where the sum is over all the possible permutations of the $\set{R_j}$ in $\ket{x}$. Due to fermionic anticommutation relations, only terms that exactly match the ordered set $\set{R_j}$ survive. The reordering of operators produces the signature of a permutation. The form of $\braket{x|\Phi_0}$ is recognized as the determinant

$$
\begin{equation*}
  \braket{x|\Phi_0} = \det(U_{R_J, \alpha})_{j,\alpha=1}^{N_e}
\tag{\label{equation-74}}
\end{equation*}
$$

Thus, the overlap is the determinant of the $N_e \times N_e$ submatrix of $\mathbf{U}$ obtained by restricting the rows indexed by the occupied single-particle states $\set{R_j}$ and the columns corresponding to the filled orbitals $\alpha=1,\dots,N_e$.

To ensure numerically stable and reproducible Monte Carlo calculations, the overlap $\eqref{equation-74}$ must be uniquely defined up to an overall phase. Since this determinant depends only on the $N_e$-dimensional occupied subspace of the one-particle Hilbert space $\mathcal{H}^{(1)} \cong \mathbb{C}^{2L}$, the key issue is whether that subspace is uniquely determined by the spectrum of $\mathbf{T}$.

For closed shell configurations, there is a spectral gap between the highest occupied and lowest unoccupied orbitals, i.e. $\varepsilon_{N_e} < \varepsilon_{N_e + 1}$. In this case, the spectral projection

$$
  \hat{P}_\text{occ} = \chi_{(-\infty, \varepsilon_{N_e})} (\mathbf{T})
$$
 
onto the span of the $N_e$ lowest eigenvectors has rank $N_e$ and is uniquely defined. Hence occupied subspace $\mathcal{V}_\text{occ} = \operatorname{ran}(\hat{P}_\text{occ}) \subset \mathcal{H}^{(1)}$ is uniquely determined. Any two orthonormal bases of $\mathcal{V}_\text{occ}$ differ by a unitary transformation $V \in \operatorname{U}(N_e)$ with $|\det(V)| = 1$. In this case the many-body state transforms as $\ket{\Phi_0} \to \det(V) \ket{\Phi_0}$, such that the Slater determinant $\eqref{equation-73}$ is unique up to a global phase. Since Monte Carlo sampling depends only on amplitudes or squared amplitudes, this phase is physically irrelevant and does not affect numerical stability.

For open shell configurations, there is a no spectral gap between the highest occupied and lowest unoccupied orbitals, i.e. $\varepsilon_{N_e} = \varepsilon_{N_e + 1}$. Let $\varepsilon_F := \varepsilon_{N_e}$ be the Fermi energy, and denote by

$$
  \mathcal{E}_F = \operatorname{ker}(\mathbf{T} - \varepsilon_F \mathbf{I})
$$

the degenerate eigenspace at the Fermi energy, with dimension $\dim(\mathcal{E}_F) > 1$. In this case the spectral projection onto energies $\leq \varepsilon_F$ has rank strictly larger than $N_e$, and there is no $N_e$-dimensional occupied subspace. We must therefore choose an $N_e$-dimensional subspace insider a larger space. Consequently, the Slater determinant $\eqref{equation-73}$ is not uniquely determined by the spectrum of $\mathbf{T}$.

If a degenerate multiplet lies entirely below or entirely above the Fermi level (i.e. either all its eigenvectors are occupied or all are empty), then the occupied subspace $\mathcal{V}_\text{occ}$ is still uniquely determined as a whole. Any change of basis inside a fully occupied degenerate block corresponds to a unitary rotation within $\mathcal{V}_\text{occ}$, therefore modifies $\ket{\Phi_0}$ only by a global phase. Although the single-particle eigenvectors are not unique, $\ket{\Phi_0}$ is well defined up to phase so that no Monte Carlo instability aries.

If a degenerate eigenspace $\mathcal{E}_F$ is only partially filled, i.e.

$$
  0 < \dim(\mathcal{V}_\text{occ} \cap \mathcal{E}_F) < \dim(\mathcal{E}_F)
$$

then the choice of occupied subspace inside $\mathcal{E}_F$ is not uniquely determined.

Numerical diagonalization of $\mathbf{T}$ produces an arbitrary orthonormal basis of $\mathcal{E}_F$, typically depending sensitively on floating-point details. Since different orthonormal bases of $\mathcal{E}_F$ are related by nontrivial unitary transformations that mix occupied and unoccupied directions within $\mathcal{E}_F$, the resulting $N_e$-dimensional occupied subspace changes. Consequently:
- The Slater determinant $\ket{\Phi_0}$ changes nontrivially
- The matrix $\tilde{\mathbf{U}}$ changes in a way that is not a right multiplication by an $N_e \times N_e$ unitary
- The determinant in $\eqref{equation-74}$ depends on the the particular linear combinations selected by the diagonalization routine

In this case, the Monte Carlo weights are basis-dependent, and the calculation is not reproducible unless an additional prescription is imposed to fix the occupied subspace.

#### Bardeen-Cooper-Schrieffer model

The previous single-particle formalism extends to quadratic Hamiltonians that include pairing terms. In this case, particle number is not conserved, and the natural diagonalization procedure involves a Bogoliubov transformation. This can be reduced to a number-conserving problem via a particle-hole transformation. 

Consider fermionic operators $\set{\hat{c}_{i,\sigma}, \hat{c}_{i,\sigma}^\dagger}_substack{i=1,\dots,L \\ \sigma\in\set{\uparrow, \downarrow}}$ satisfying the canoncial anticommutation relations. The Hamiltonian of the Bardeen-Cooper-Schrieffer model on $L$ sites is

$$
\begin{align*}
  \hat{H}_0 =& \sum_{i,j,\sigma} t_{ij} \hat{c}_{i,\sigma}^\dagger \hat{c}_{j,\sigma} - \mu_0 \sum_{i,\sigma} \hat{c}_{i,\sigma}^\dagger \hat{c}_{i,\sigma} \\
  &+ \sum_{i,j} \Delta_{ij} (\hat{c}_{i,\uparrow}^\dagger \hat{c}_{j,\downarrow} + \delta_{ij}^* \hat{c}_{j,\downarrow} \hat{c}_{i,\uparrow})
\end{align*}
$$

where
- $\mu_0 \in\R$ is the chemical potential, which fixes, on average, the number of electrons
- $\Delta_{ij} \in\mathbb{C}$ is the pairing matrix

This Hamiltonian is quadratic, but it does not conserve total particle number since $[\hat{H}_0, \hat{H}_e] \neq 0$. Consequently, the ground state cannot be written as a Slater determinant of fixed-particle orbitals. Instead it is a quasi-free state characterized by a pairing function.

To reduce the problem to a number-conserving quadratic form, we can perform a particle-hole transformation on the spin-down sector by defining the fermionic operators

$$
\begin{align*}
  \hat{f}_{i,\uparrow} := \hat{c}_{i,\uparrow} =: \hat{d}_i \tag{\label{equation-75}} \\
  \hat{f}_{i,\downarrow}^\dagger := \hat{c}_{i,\downarrow} =: \hat{d}_{i+L}^\dagger \tag{\label{equation-76}}
\end{align*}
$$

This transformation preserves the canonical anticommutation relations and correponds to exchanging creation and annihilation operators in the spin-down sector. Apart from constant terms, the transformed Hamiltonian becomes quadratic and number-conserving in the $\hat{d}_I$ operators

$$
  \hat{H}_0 = \sum_{I,J=1}^{2L} T_{IJ} \hat{d}_I^\dagger \hat{d}_J = \unitvec{d}^\dagger \mathbf{T} \unitvec{d}
$$

The $2L \times 2L$ Hemitian matrix $\mathbf{T}$ has a block form, for $i,j \leq L$

$$
\begin{align*}
  T_{ij} =& t_{ij} -\mu_0 \delta_{ij} \\
  T_{i+L, i+L} =& \mu_0 \\
  T_{i+L,j+L} =& -t_{ij} \\
  T_{i,j+L} =& T_{j+L, i}^* = \Delta_{ij}
\end{align*}
$$

The transformed Hamiltonian conserves the number of particles, but not the $z$ component of the spin. Thus, the conserved quantity is not the orignal electron number, but a linear combination of charge and spin. In the $d$-fermion representation, the Hamiltonian can be diagonalized by a unitary transformation, yielding a Slater determinant just like in the non-pairing case.

Because $\hat{f}_{i,\downarrow} = \hat{c}_{i,\downarrow}^\dagger$, particle-hole transformation exchanges empty and occupied spin-down states. Consequently

$$
\begin{align*}
  \ket{0_i} \mapsto& \hat{f}_{i,\downarrow}^\dagger \ket{\tilde{0}_i} \\
  \hat{c}_{i,\uparrow}^\dagger \ket{0_i} \mapsto& \hat{f}_{i,\uparrow}^\dagger \hat{f}_{i,\downarrow}^\dagger \ket{\tilde{0}_i} \\
  \hat{c}_{i,\downarrow}^\dagger \ket{0_i} \mapsto& \ket{\tilde{0}_i} \\
  \hat{c}_{i,j}^\dagger \hat{c}_{i,\downarrow}^\dagger \ket{0_i} \mapsto& \hat{f}_{i,\uparrow}^\dagger \ket{\tilde{0}_i}
\end{align*}
$$

where $\ket{\tilde{0}_i}$ is the vacuum of the $f$-electrons, i.e. $\hat{f}_{i,\uparrow} \ket{\tilde{0}} = \hat{f}_{i,\downarrow} \ket{\tilde{0}} = 0$. 

**Transformation of densities and magnetization:**

In the $c$-fermion representation, the number operators are given by $\hat{n}_{i,\sigma} = \hat{c}_{i,\sigma}^\dagger \hat{c}_{i,\sigma}$. The transformed occupations are given by $\hat{u}_{i,\sigma} = \hat{f}_{i,\sigma}^\dagger \hat{f}_{i,\sigma}$. Using the canonical anticommutation relations yields

$$
\begin{align*}
  \hat{u}_{i,\uparrow} =& \hat{n}_{i,\uparrow} \\
  \hat{u}_{i,\downarrow} =& 1 - \hat{n}_{i,\downarrow}
\end{align*}
$$

In the $f$-fermion representation, the density is given by

$$
  \hat{u}_i = \hat{u}_{i,\uparrow} + \hat{u}_{i,\downarrow} = \hat{n}_{i,\uparrow} - \hat{n}_{i,\downarrow} + 1
$$

The $z$-component of the magnetization in the $c$-fermion representation is defined by $\hat{m}_i^z = \hat{n}_{i,\uparrow} - \hat{n}_{i,\downarrow}$. Hence, $\hat{u}_i = \hat{m}_i^z + 1$, showing that the original magnetization $\hat{m}_i^z$ maps to the transformed density $\hat{u}_i$ up to an additive constant.

The transformed magnetization in the $f$-fermion representation becomes

$$
  \hat{w}_i^z = \hat{u}_{i,\uparrow} - \hat{u}_{i,\downarrow} = \hat{n}_{i,\uparrow} + \hat{n}_{i,\downarrow} - 1 = \hat{n}_i - 1
$$

showing that the original density $\hat{n}_i$ maps to the transformed magnetization $\hat{w}_i^z$ up to an additive constant. In summary, the particle-hole transformation exchanges charge and spin (along $z$) up to constants.

The total density in the $f$-fermion representation is given by

$$
\begin{align*}
  \hat{U} =& \sum_{i=1}^L \hat{u}_i = \sum_i (\hat{m}_i^z + 1) \\
  =& \hat{M}_z + L
\end{align*}
$$

where the $\hat{M}_z$ is the total magnetization in the $c$-fermion representation given by

$$
  \hat{M}_z = \sum_i \hat{n}_{i,\uparrow} - \hat{n}_{i,\downarrow}
$$

Thus, if the original state has zero total magnetization, i.e. $\braket{\hat{M}_z} = 0$, the transformed systems has total density $\braket{\hat{U}} = L$. That is, the transformed problem corresponds to a half-filled system of $2L$ orbitals.

#### Computation of determinants

Consider the electronic configuration in which $\ket{x'}$ is obtained from $\ket{x}$ just by hopping the $l$th electron from $R_l$ to $K := R'_l$, i.e. $\ket{x'} = \hat{d}_K \hat{d}_{R_l} \ket{x}$. Then new matrix $\tilde{\mathbf{U}}'$ coincides with $\tilde{\mathbf{U}}$, except for the $l$th row 

$$
\begin{equation*}
  \tilde{U}'_{j,\alpha} = \tilde{U}_{j,\alpha} + \delta_{j,l} (U_{K,\alpha} - \tilde{U}_{l,\alpha})
\tag{\label{equation-86}}
\end{equation*}
$$


Then, the ratio between the overlaps of the two configurations with respect to a Slater determinant $\ket{\Phi_0}$ is

$$
  \frac{\braket{x'|\Phi_0}}{\braket{x|\Phi_0}} = \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_K|\Phi_0}}{\braket{x|\Phi_0}} = \frac{\det(\tilde{\mathbf{U}}')}{\det(\tilde{\mathbf{U}})}
$$

The ratio can be simplified by refactoring $\tilde{\mathbf{U}}'$. This can be done by defining $A_\alpha := U_{K,\alpha} - \tilde{U}_{l,\alpha}$ and $B_\beta = \tilde{U}_{\beta,l}^{-1}$ such that the elements of $\tilde{\mathbf{U}}'$ given in $\eqref{equation-86}$ can be rewritten as

$$
\begin{equation*}
\begin{split}
  \tilde{U}'_{j,\alpha} =& \tilde{U}_{j,\alpha} + \delta_{j,l} A_\alpha \\
  =& \sum_\beta \tilde{U}_{j,\beta} (\delta_{\beta,\alpha} + \tilde{U}_{\beta,l}^{-1} A_\alpha) \\
  =& \sum_\beta \tilde{U}_{j,\beta} Q_{\beta, \alpha}
\end{split}
\tag{\label{equation-86}}
\end{equation*}
$$

where $Q_{\beta,\alpha} = \delta_{\beta,\alpha} + B_\beta A_\alpha$. Thus, $\tilde{\mathbf{U}}' = \tilde{\mathbf{U}}\mathbf{Q}$, which implies that the calculation of the ratio of the determinants of $\tilde{\mathbf{U}}'$ and $\tilde{\mathbf{U}}$ is equivalent to the calculation of the determinant of $\mathbf{Q}$

$$
  \frac{\det(\tilde{\mathbf{U}}')}{\det(\tilde{\mathbf{U}})} = \det(\mathbf{Q})
$$

Since $\mathbf{Q} = \mathbf{I} + \mathbf{BA}^\top$ is a rank-one perturbation of the identity, its determinant is given by

$$
  \det(\mathbf{Q}) = 1 \mathbf{A}^\top \mathbf{B} = 1 + \sum_\alpha U_{K, \alpha} \tilde{U}_{\alpha,l}^{-1}
$$
...
$$
\begin{equation*}
  \frac{\det(\tilde{\mathbf{U}}')}{\det(\tilde{\mathbf{U}})} = \det(\mathbf{Q}) = \sum_\alpha U_{K,\alpha} \tilde{U}_{\alpha,l}^{-1}
\tag{\label{equation-81}}
\end{equation*}
$$

If $\tilde{U}^{-1}$ is stored for the configuration $\ket{x}$, this calculation requires $O(N_e)$ operations, instead of the $O(N_e^3)$ cost needed to evaluate a determinant. Then once the new configuration $\ket{x'}$ is accepted along the Markov process, the matrix $\tilde{\mathbf{U}}^{-1}$ must be updated. 

Since ${\tilde{\mathbf{U}}'}^{-1} = \mathbf{Q}^{-1} \tilde{\mathbf{U}}^{-1}$, the inverse of $\mathbf{Q}$ i given by

$$
  \mathbf{Q}^{-1} = \mathbf{I} - \frac{1}{\det(\mathbf{Q})} \mathbf{BA}^\top
$$

Then, the updated matrix elements of $(\tilde{\mathbf{U}}')^{-1}$ are given by

$$
\begin{equation*}
  {\tilde{U}'_{\alpha,j}}^{-1} = \tilde{U}_{\alpha,j}^{-1} - \frac{\tilde{U}_{\alpha,\beta}^{-1}}{\det(\mathbf{Q})} \left(\sum_\beta U_{K,\beta} \tilde{U}_{\beta,j}^{-1} - \delta_{l,j} \right)
\tag{\label{equation-82}}
\end{equation*}
$$

This update requires $O(N_e^2)$ operations.

The calculation of the ratio of determinants and the updating can be further simplified by defining a $2L \times N_e$ matrix $\mathbf{W}$, with elements

$$
\begin{equation*}
  W_{l,j} = \sum_\alpha U_{l,\alpha} \tilde{U}_{\alpha,j}^{-1}
\tag{\label{equation-84}}
\end{equation*}
$$

then the ratio of determinants $\eqref{equation-81}$ requires $O(1)$ operations, since it consists in taking the element corresponding to the new site (row) and the electron performing the hopping process (column)

$$
\begin{equation*}
  \frac{\det(\tilde{\mathbf{U}}')}{\det(\tilde{\mathbf{U}})} = W_{K,l}
\tag{\label{equation-83}}
\end{equation*}
$$

The evaluation of $\mathbf{W}$ requires the knowledge of the full matrix $\mathbf{U}$, which has been computed and stored once for all at the beginning of the simulation (it does not depend on the electronic configuration), and $\tilde{\mathbf{U}}^{-1}$, which instead depends on the configuration $\ket{x}$. Then a simple updatin scheme for $\mathbf{W}$ is possible. Multiplying both sides of $\eqref{equation-82}$ by $U_{l,\alpha}$ and summing over $\alpha$, we obtain

$$
\begin{equation*}
  W'_{I,j} = W_{I,j} - \frac{W_{I,l}}{W_{K,l}} (W_{K,j} - \delta_{l,j})
\tag{\label{equation-85}}
\end{equation*}
$$

where, we have used that $\det(\mathbf{Q}) = W_{K,l}$ according to $\eqref{equation-83}$ and $\eqref{equation-84}$. Since each matrix element must be updated with $O(1)$ operations, the total cost is $O(2LN_e)$.

By using a wave function that is constructed from filling single-particle states (also including cases with BCS pairing when a particle-hole transformation is done), the algorithm can be written in terms of the matrix elements of $\mathbf{W}$, which is the equal-time Green's function

$$
  \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_K |\Phi_0}}{\braket{x|\Phi_0}} = W_{K,l}
$$

Note that $\eqref{equation-85}$ has a direct interpretation by using the Wick theorem (with different bra and ket states) for the updated (equal-time) Green's function

$$
\begin{align*}
  W'_{l,j} =& \frac{\braket{x'|\hat{d}_{R_j}^\dagger \hat{d}_l|\Phi_0}}{\braket{x'|\Phi_0}} \\
  =& \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_K \hat{d}_{R_j}^\dagger \hat{d}_l|\Phi_0}}{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_K |\Phi_0}}
\end{align*}
$$

in this cse, we must consider the contractions without including the anomalous (superconducting) ones, which vanish since both $\ket{x}$ and $\ket{\Phi_0}$ have a fixed number of particles.

##### Multi-electron updates

The previous formalism can be generalized to multi-electron hopping. Consider a move in which $m$ electrons are simultaneously displayed

$$
  \ket{x'} = \hat{d}_{K_1}^\dagger \hat{d}_{R_{l_1}} \cdots \hat{d}_{K_m}^\dagger \hat{d}_{R_{l_m}} \ket{x}
$$

Where the indices $l_1,\dots,l_m$ are distinct and $K_r := R'_{l_r}$. The new matrix $\tilde{\mathbf{U}}'$ differs from $\tilde{\mathbf{U}}$ by the replacement of the rows $l_1,\dots,l_m$

$$
  \tilde{U}'_{j,\alpha} = \tilde{U}_{j,\alpha} + \sum_{r=1}^m \delta_{j,l_r} (U_{k_r, \alpha} - \tilde{U}_{l_r, \alpha})
$$

This can be simplified by defining $A_\alpha^{(r)} := U_{K_r,\alpha} - \tilde{U}_{l_r,\alpha}$ and $B_\beta^{(r)} := \tilde{U}^{-1}_{\beta,l_r}$ such that

$$
\begin{align*}
  \tilde{U}'_{j,\alpha} =& \tilde{U}_{j,\alpha} + \sum_{r=1}^m \delta_{j,l_r} A_\alpha^{K_r, l_r} \\
  =& \sum_\beta \tilde{j,\beta} \left(\delta_{\alpha,\beta} + \sum_{r=1}^m \tilde{U}_{\beta,l_r}^{-1} A_\alpha \right) \\
  =& \sum_\beta \tilde{U}_{j,\beta} Q_{\beta,\alpha}
\end{align*}
$$
...
$$
  Q_{\beta,\alpha} = \delta_{\beta,\alpha} + \sum_{r=1}^m B_\beta^r A_\alpha^r
$$

The block matrix $\mathbf{Q} = \mathbf{I} + \sum_{r=1}^m \mathbf{B}_r \mathbf{A}_r^\top$ is a rank-$m$ update of the identity, whose determinant is given by

$$
  \det(\mathbf{Q}) = \det(\mathbf{I} + \sum_{r=1}^m \mathbf{A}_r^\top \mathbf{B}_r)
$$

Introducing the $m\times m$ matrix $\mathbf{C} = \mathbf{I} + \sum_{r=1}^m \mathbf{A}_r^\top \mathbf{B}_r$ with elements

$$
  C_{r,s} = \delta_{r,s} + \sum_\alpha A_\alpha^r B_\alpha^s = W_{K_r, l_s}
$$

The final expression of the ratio of the two determinants is given by

$$
  \frac{\det(\tilde{\mathbf{U}}')}{\det(\tilde{\mathbf{U}})} = \det(W_{K_r, l_s})
$$

Once the move is accepted, we have to update the matrix ${\tilde{\mathbf{U}}'}^{1-} = \mathbf{Q}^{-1} \tilde{\mathbf{U}}^{-1}$. As before, the inverse of $\mathbf{Q}$ is given by

$$
  Q_{\alpha,\beta}^{-1} = \delta_{\alpha,\beta} - \sum_{r,s=1}^m B_\alpha^r C_{r,s}^{-1} A_\beta^s
$$
...
$$
\begin{equation*}
  {\tilde{U}_{\alpha,j}}^{-1} = \tilde{U}_{\alpha,j}^{-1} - \sum_{r,s=1}^m \tilde{U}_{\alpha,l_r}^{-1} C_{r,s}^{-1} \left(\sum_\beta U_{K_s, \beta} \tilde{U}_{\beta,j}^{-1} - \delta_{l_s, j} \right)
\tag{\label{equation-98}}
\end{equation*}
$$

The updated $\mathbf{W}'$ is obtained by multiplying both sides by $U_{i,\alpha}$ and summing over $\alpha$, leading the

$$
  W'_{I,j} = W_{I,j} + \sum_{r=1}^m W_{I,l_r} b_j^{(r)}
$$

where

$$
  b_j^{(r)} = -\sum_{s=1}^m C_{r,s}^{-1} (W_{K_s,j} - \delta_{l_s,j})
$$

Similarly to the update of the Jastrow factor, it is highly recommended to occasionally recompute the matrix $\mathbf{W}$ from scratch. Since this calculation requires $O(2LN_e^2)$ operations, it does not affect the complexity of the algorithm if it is done every $O(N_e)$ electron moves.

#### Delayed updates

Let us consider the simple update in which only one electron is displaced. The basic operation in the updating is the so-called rank-$1$ update of a $2L \times N_e$ matrix (see $\eqref{equation-85}$) that can be written as

$$
  W'_{I,j} = W_{I,j} + a_I b_j
$$

where $\mathbf{a}$ and $\mathbf{b}$ are two vectors of length $2L$ and $N_e$, respectively

$$
\begin{align*}
  a_I = W_{I,l} \tag{\label{equation-94}} \\
  b_j = -\frac{W_{K,j} - \delta_{I,j}}{W_{K,l}} \tag{\label{equation-95}}
\end{align*}
$$

This updating operation can be computationally inefficient, whenever, for large size, the matrix $\mathbf{W}$ cannot be continued in the cache of the processor. A way to overcome this drawback is to delay the update of the matrix $\mathbf{W}$, without loosing its information. This can be done by storing a set of left and right vectors $\mathbf{a}^{(p)}$ and $\mathbf{b}^{(p)}$ with $p=1,\dots,m$, as weel as the initial matrix, denoted $\mathbf{W}^{(0)}$, from which we begin to delay the updates. Then, the matrix $\mathbf{W}$, after $m$ updates is given by

$$
\begin{equation*}
  W_{I,j} = W_{I,j}^{(0)} + \sum_{p=1}^m a_I^{(p)} b_j^{(p)}
\tag{\label{equation-93}}
\end{equation*}
$$

Every time we accept a new configuration, a new pair of vectors $\mathbf{a}^{(m+1)}$ and $\mathbf{b}^{(m+1)}$ can be computed in a few operations in terms of $\mathbf{W}^{(0)}$ and the previous vectors with $p=1,\dots,m$ by substituting $\eqref{equation-93}$ in $\eqref{equation-94}$ and $\eqref{equation-95}$. Once the matrix is written in the form of $\eqref{equation-93}$, the number of operations required to evaluate the factors in the sum is $O[m(2L + N_e)]$, which is negligible compared to the full update for $m \ll L$.

In this way, we can find an optimal $m_\text{max}$, for which we can evaluate the full matrix $\mathbf{W}$ by a standard matrix multiplication

$$
\begin{equation*}
  \mathbf{W} = \mathbf{W}^{(0)} + \mathbf{AB}^\top
\tag{\label{equation-96}}
\end{equation*}
$$

where $\mathbf{A}$ and $\mathbf{B}$ are $2L \times m_\text{max}$ and $N_e \times m_\text{max}$ matrices. which are made of the $p=1,\dots,m_\text{max}$ vectors $\mathbf{a}^{(p)}$ and $\mathbf{b}^{(p)}$, respectively. After that, we can continue with a new delayed update with a new $\mathbf{W}^{(0)} = \mathbf{W}$, by initializing to zero the integer $m$. The advantage of this updating procedure is that after a cycle of $m_\text{max}$ steps the bulk of the computation is given by the matrix product in $\eqref{equation-96}$, which is much more efficient (and is not limited by cache memory) than the $m_\text{max}$ rank-$1$ updated of $\mathbf{W}$ given in $\eqref{equation-85}$. For large number of electrons, the delayed update procedure, with the optimal value of $m_\text{max}$, allows us to improve the speed of the variational Monte Carlo code by about an order of magnitude.

#### Backflow correlations

In the lattice case, a continuous change of coordinates implementing the standard backflow correlation $\eqref{equation-}$ is not possible. Thus, this Ansatz requires to find an expression of the orbital in a generic off-lattice backflow coordinate in terms of the allowed lattice positions, consistently to a Taylor expansion of the orbitals. In practice, we consider a quadratic Hamiltonian to construct the non-interacting orbitals $\set{U_{I,\alpha}}$ (see $\eqref{equation-})$. In the simplest approach, in which the backflow correlations act on holon-doublon (nearest-neighbour) pairs, the overlap between the generic configuration $\ket{x}$ and the backflow wave function $\ket{\Phi_0^b}$ is constructed from the correlated orbital with backflow correction

$$
\begin{equation*}
  U_{I,\alpha}^b = \eta_0 U_{I,\alpha} + \eta_1 \sum_{\braket{j}_i} \hat{D}_i \hat{H}_j U_{J,\alpha}
\tag{\label{equation-97}}
\end{equation*}
$$

where 
- $I = i$ ($I = i + L$) for electrons with spin up (down), and equivalently for $J$ and $j$
- $\braket{j}_i$ indicates the sites $j$ that are the nearest neighbour of $i$
- $\hat{D}_i$ ($\hat{H}_i$) is the operator that gives $1$ if the site $i$ doubly occupied (empty) and $0$ otherwise
- $\eta_0$ and $\eta_1$ are variational parameters

Then the wave function for the given configuration $\ket{x}$ is obtained by taking the determinant of the matrix $\tilde{\mathbf{U}}^b$ in which the rows correspon to the sites occupied by the electrons.

The fast update for a ratio between two determinant with configurations $\ket{x}$ and $\ket{x'}$, which differ by one or few electron hoppings, can be computed as before.

The only difference is that, even if there is only one electron hopping, the new matrix ${\tilde{\mathbf{U}}^b}'$ will differ not only for a single row, but for few of them. By moving one electron, an empty of doubly occupied site can be created, leading to a modification of the correlated orbitals of $\eqref{equation-97}$. Thus, in general, we must use the general update in which several rows are changed. However, in this case, it is convenient to work directly with the inverse matrix $\tilde{\mathbf{U}}'$, by updating it according to $\eqref{equation-98}$.

In practive, we can consider more general forms for the correlated orbitals, by consider further terms, but sill remaining in the same spirit of considering a linear combination of non-interacting orbitals depending on the many-body configuration $\ket{x}$ (in the previous case, the linear combination is taken for configurations having holons and doublons at nearest-neighbour sites).

### Fermionic systems with Pfaffians

#### Properties of the Pfaffian

Let $\mathbf{A} = [a_{ij}]_{i,j=1}^{2n} \in \mathbb{F}^{2n\times 2n}$ be a skew-symmetric matrix, i.e. $A^\top = -\mathbf{A}$. The Pfaffian of $\mathbf{A}$ is defined as the anti-symmetrized product

$$
  \operatorname{Pf}(\mathbf{A}) = \frac{1}{2^n n!} \sum_{\sigma\in S_{2n}} \operatorname{sgn}(\sigma) \prod_{k=1}^n a_{\sigma(2k - 1), \sigma(2k)}
$$

Equivalently, it can be written as a sum over all pair partitions of $\set{1,\dots,2n}$

$$
  \operatorname{Pf}(\mathbf{A}) = \sum_\alpha \operatorname{sgn}(\alpha) \prod_{k=1}^n a_{i_k, j_k}
$$

where the sum runs over all the $(2n - 1)!!$ pair partitions defined by $\alpha = \set{(i_k, j_k)}_{k=1}^n$ with $i_k < j_k$ and $i_1 < \cdots < i_n$. Here $\operatorname{sgn}(\alpha)$ denotes the parity of the permutation corresponding to the partition $\alpha$. 

The Pfaffian is related to the determinant via $\det(\mathbf{A}) = \operatorname{Pf}(\mathbf{A})^2$. Thus, for skew-symmetric matrices of even dimension, the determinan is a perfect square. The Pfaffian determines the determines uniquely up to a global sign.

The Pfaffian satisfies the following relations:
1. If $\mathbf{A} \in\mathbb{F}^{2n\times 2n}$ and $\mathbf{A}' \in\mathbb{F}^{2m\times 2m}$ are skew-symmetric, then
$$
\begin{equation*}
  \operatorname{Pf}\begin{bmatrix} \mathbf{A} & \mathbf{0} \\ \mathbf{0} & \mathbf{A}' \end{bmatrix} = \operatorname{Pf}(\mathbf{A}) \times \operatorname{Pf}(\mathbf{A})
\tag{\label{equation-87}}
\end{equation*}
$$

2. For any invertible matrix $\mathbf{B} \in\mathbb{F}^{2n\times 2n}$

$$
\begin{equation*}
  \operatorname{Pf}(\mathbf{BAB}^\top) = \det(\mathbf{B}) \operatorname{Pf}(\mathbf{A})
\tag{\label{equation-88}}
\end{equation*}
$$

3. Let $\mathbf{A} \in \mathbb{F}^{2n\times 2n}$ and $\mathbf{C}^{2m\times 2m}$ be invertible skew-symmetric matrices and $\mathbf{B} \in\R^{2n\times 2m}$ be arbitrary. Then

$$
\begin{align*}
  \begin{bmatrix} \mathbf{A} & \mathbf{B} \\ -\mathbf{B}^\top & \mathbf{C}^{-1} \end{bmatrix} =&  \begin{bmatrix} \mathbf{I} & \mathbf{BC} \\ \mathbf{0} & \mathbf{I} \end{bmatrix} \begin{bmatrix} \mathbf{A} + \mathbf{BCB}^\top & \mathbf{0} \\ \mathbf{0} & \mathbf{C}^{-1} \end{bmatrix} \begin{bmatrix} \mathbf{I} & \mathbf{0} \\ (\mathbf{BC})^\top & \mathbf{I} \end{bmatrix} \\
  =& \begin{bmatrix} \mathbf{I} & \mathbf{0} \\ (\mathbf{A}^{-1} \mathbf{B})^\top & \mathbf{I} \end{bmatrix} \begin{bmatrix} \mathbf{A} & \mathbf{0} \\ \mathbf{0} & \mathbf{C}^{-1} + \mathbf{B}^\top \mathbf{A}^{-1} \mathbf{B} \end{bmatrix} \begin{bmatrix} \mathbf{I} & \mathbf{A}^{-1} \mathbf{B} \\ \mathbf{0} & \mathbf{I} \end{bmatrix}
\end{align*}
$$

Using $\eqref{equation-87}$ and $\eqref{equation-88}$, we arrive at

$$
\begin{equation*}
  \frac{\operatorname{Pf}(\mathbf{A} + \mathbf{BCB}^\top)}{\operatorname{Pf}(\mathbf{A})} = \frac{\operatorname{Pf}(\mathbf{C}^{-1} + \mathbf{B}^\top \mathbf{A}^{-1} \mathbf{B})}{\operatorname{Pf}(\mathbf{C}^{-1})}
...
If the non-interacting Hamiltonian contains pairing equal spins, or simultaneously includes spin-flip hopping and pairing terms, the pairing cannot, in general, be eliminated by a particle-hole transofmration. The most general quadratic Hamiltionian for a fermionic system is given by a generalized Bardeen-Cooper-Schrieffer model

$$
\begin{align*}
  \hat{H}_0 =& \sum_{i,j,\sigma,\tau} t_{i,j}^{\sigma,\tau} c_{i,\sigma}^\dagger \hat{c}_{j,\tau} - \mu_0 \sum_{i,\sigma} \hat{c}_{i,\sigma}^\dagger \hat{c}_{i,\sigma} \\
  +& \sum_{i,j,\sigma,\tau} (\Delta_{i,j}^{\sigma,\tau} \hat{c}_{i,\sigma}^\dagger \hat{c}_{j,\tau}^\dagger + (\Delta_{i,j}^{\sigma,\tau})^* \hat{c}_{j,\tau} + \hat{c}_{i,\sigma})
\end{align*}
$$

where $t_{i,j}^{\sigma,\tau} = (t_{i,j}^{\tau,\sigma})^*$ and $\Delta_{i,j}^{\tau,\sigma} = -\Delta_{i,j}^{\sigma,\tau}$ are hopping and pairing terms, respectively.

The Hamiltonian $\hat{H}_0$ can be diagonalized by a generalized Bogoliubov transformation

$$
  \Phi_\alpha = \sum_l (u_{l,\alpha} \hat{d}_l + v_{l,\alpha} \hat{d}_l^\dagger)
$$

To preserve fermionic anticommutation relations, the $2L\times 2L$ matrices $\mathbf{u}$ and $\mathbf{v}$ are determined by imposing

$$
\begin{align*}
  \mathbf{u}^\dagger \mathbf{u} + \mathbf{v}^\dagger \mathbf{v} =& \mathbf{I} \\
  \mathbf{u}^\top \mathbf{v} + \mathbf{v}^\top \mathbf{u} =& \mathbf{0}
\end{align*}
$$

The quasiparticle operators $\Phi_\alpha$ satisfies the commutation relations

$$
  [\hat{H}_0, \Phi_\alpha] = -\varepsilon_\alpha \Phi_\alpha
$$

Due to particle-hole symmetry, the eigenvalues of $\hat{H}_0$ appear in pairs $(\varepsilon_\alpha, -\varepsilon_\alpha)$. That is, if $\Phi_\alpha$ has eigenvalue $\varepsilon_\alpha$, then $\Phi_\alpha^\dagger$ has eigenvalue $-\varepsilon_\alpha$. We can therefore restrict to the $2L$ eigenvectors with non-negative eigenvalues $\varepsilon_\alpha \geq 0$.

In most cases, the ground state of the non-interacting Hamiltonian $\hat{H}_0$ can be written as a generalized BCS state

$$
  \ket{\Phi_0} = \exp\left(\frac{1}{2} \sum_{I,J} F_{I,J} \hat{d}_I^\dagger \hat{d}_J^\dagger \right) \ket{0}
$$

where $F_{I,J}$ is the anti-symmetric pairing function, defined by imposing that $\ket{\Phi_0}$ is annihilated by all the operators $\Phi_\alpha$ with $\varepsilon_\alpha \geq 0$, which leads to

$$
  \mathbf{Fu} = \mathbf{v} \iff \mathbf{F} = \mathbf{vu}^{-1}
$$

The invertibility of $\mathbf{u}$ guarantees that $\mathbf{F}$ is well defined. If $\mathbf{u}$ is singular, then there exists single-particle orbitals (unpaired modes). In that case, the ground state is a product of a BCS paired state and additional occupied orbitals, and the wave function can no longer be written purely as an exponential of pairs. This increases the algebraic and computational complexity in Monte Carlo implementations.

Zero-energy modes, $\varepsilon_\alpha = 0$, must be excluded to ensure a unique many-body ground state (up to global phase). Otherwise, degeneracy leads to ambiguity in the definition of $\ket{\Phi_0}$ as in the case for Slater determinants.

The overlap between $\ket{\Phi_0}$ and generic configurations $\ket{x}$ in $\eqref{equation-90}$ is given by

$$
  \braket{x|\Phi_0} = \frac{1}{(N_e/2)!} \Braket{0|\hat{d}_{R_{R_e}} \cdots \hat{d}_{R_1} \left(\frac{1}{2} \sum_{I,J} F_{I,J} \hat{d}_I^\dagger \hat{d}_J^\dagger \right)^{N_e/2} |0}
$$

Expanding the product of sums yields a sum over the $(N_e - 1)!!$ pair partitions of the fermions in $\ket{x}$ of $(N_e/2)!$ identical contributions. Each pairing contributes a product of matrix elements $F_{R_i, R_j}$, and fermionic reordering produces the corresponding permutation sign. Thus, the overlap equals the Pfaffian of the $N_e \times N_e$ antisymmetric submatric $\mathbf{F}_x$ obtained by restricting $\mathbf{F}$ to the occupied orbitals

$$
  \braket{x|\Phi_0} = \operatorname{Pf}\set{F_{R_i, R_j}}
$$

For a general quadratic Hamiltonian with pairing, Monte Carlo sampling of the BCS ground state therefore naturally leads to Pfaffian wave functions, generalizing the Slater determinant structure that arises in the particle-number-conserving case.

#### Computation of Pfaffians

Consider the electronic configuration in which $\ket{x'}$ is obtained from $\ket{x}$ just by hopping the $l$th electron from $R_l$ to $R'_l$, i.e. $\ket{x'} = \hat{d}_{R'_l} \hat{d}_{R_l} \ket{x}$. The new matrix $\mathbf{F}'$ needed to compute the overlap with $\ket{x'}$, differs from the old one $\mathbf{F}$ only in the $l$th row and column. As for the case with determinants, the actual order or rows (and columns) is determined by the one of creation operators in the configuration $\ket{x}$. Then, a generalization of the vector $\boldsymbol{\kappa}$ for the spinful case can be used to select the correct order of columns and rows. Then, the overlap $\braket{x'|\Phi_0}$ is given by the Pfaffian of

$$
  \tilde{F}'_{ij} = \tilde{F}_{ij} + \delta_{il} (F_{R'_l, R_j} - \tilde{F}_{ij}) - \delta_{jl} (F_{R'_l, R_i} - \tilde{F}_{li})
$$

which can be cast in the form $\tilde{\mathbf{F}}' = \tilde{\mathbf{F}} + \mathbf{BCB}^\top$ where $\mathbf{B}$ is a $N_e \times 2$ matrix

$$
\begin{equation*}
  \mathbf{B} = \begin{bmatrix} F_{R_l, R'_l} - \tilde{F}_{il} & \delta_{il} \\
    \vdots & \vdots \\
    F_{R_{N_e}, R'_l} - \tilde{F}_{N_e, l} & \delta_{N_e, l}
  \end{bmatrix}
\tag{\label{equation-90}}
\end{equation*}
$$

and $\mathbf{C}$ is a $2\times 2$ matrix
...
Using $\eqref{equation-89}$, the ratio of the overlaps is given by

$$
\begin{align*}
  \frac{\braket{x'|\Phi_0}}{\braket{x|\Phi_0}} =& \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_{R'_l}|\Phi_0}}{\braket{x|\Phi_0}} = \frac{\operatorname{Pf}(\tilde{\mathbf{F}}')}{\operatorname{Pf}(\tilde{\mathbf{F}})} \\
  =& \frac{\operatorname{Pf}(\mathbf{C}^{-1} + \mathbf{B}^\top \tilde{\mathbf{F}}^{-1} \mathbf{B})}{\operatorname{Pf}(\mathbf{C})}
\end{align*}
$$

Since $\operatorname{Pf}(\mathbf{C}) = -1$, the explicit form is found using $\eqref{equation-90}$ as

$$
  \frac{\operatorname{Pf}(\tilde{\mathbf{F}}')}{\operatorname{Pf}(\tilde{\mathbf{F}})} = \sum_j F_{R'_l, R_j} \tilde{F}_{j,l}^{-1} =: G_{R'_l, l}
$$

Having computed $\tilde{\mathbf{F}}^{-1}$ at the beginning of the simulation, this calculation requires $O(N_e)$ operators. Once the new configuration $\ket{x'}$ is accepted along the Markov chain, the matrix $\tilde{\mathbf{F}}^{-1}$ must be updated. From $\eqref{equation-}$ we have

$$
  F'_{ij} = \sum_m \tilde{F}_{im} (\delta_{mj} + \tilde{F}_{ml}^{-1} \nu_j^{R'_l, R_l} - w_m^{R'_l, R_l} \delta_{jl})
$$
...
$$
\begin{align*}
  v_j^{R'_l, R_l} =& F_{R'_l, R_j} - \tilde{F}_{lj} \\
  w_m^{R'_l, R_l} =& \sum_j \tilde{F}_{mj}^{-1} (F_{R'_l, R_j} - \tilde{F}_{lj}) = \delta_{ml} - G_{R'_l, m}
\end{align*}
$$

Then, the inverse $\tilde{\mathbf{F}}^{-1}$ can be determined from

$$
\begin{equation*}
  {\tilde{F}'_{ij}}^{-1} = \tilde{F}_{ij}^{-1} + \frac{1}{G_{R'_l, l}} (\tilde{F}_{il}^{-1} w_j^{R'_l, R_l} + w_i^{R'_l, R_l} \tilde{F}_{lj}^{-1})
\tag{\label{equation-91}}
\end{equation*}
$$

The main difference with the case for determinants is that the updating cannot be written by using the matrix $\mathbf{G}$ along, but $\tilde{\mathbf{F}}^{-1}$ must be also kept. In fact, by multiplying both sides of $\eqref{equation-}$ by $F_{R, R_i}$ and summing over $i$, we obtain

$$
\begin{align*}
  G'_{R,j} - (F_{R,R'_l} - F_{R, R_l}) {\tilde{F}'_{lj}}^{-1} =& G_{R,j} \\
  &+ \frac{1}{G_{R'_l, l}} \left(G_{R,l} w_j^{R'_l, R_l} + \sum_i F_{R,R_i} w_i^{R'_l, R_l} \tilde{F}_{lj}^{-1} \right)
\end{align*}
$$
...
$$
\begin{align*}
  G'_{R,j} =& \sum_i F_{R,R'_i} \tilde{F}_{ij}^{-1} \\
  =& \sum_i [F_{R,R_i} + \delta_{il} (F_{R,R'_l} - F_{R,R_l})] {\tilde{F}'_{ij}}^{-1}
\end{align*}
$$

Then, by using $\eqref{equation-91}$ to express ${\tilde{F}'_{lj}}^{-1}$ in terms of $\tilde{F}_{lj}^{-1}$, we finally get

$$
\begin{equation*}
  G'_{R,j} = G_{R,j} + \frac{1}{G_{R'_l, l}} \left[G_{R,l} w_j^{R'_l, R_l} + \left(F_{R,R'_l} + \sum_k G_{R,k} F_{R'_l, R_k} \right) \tilde{F}_{lj}^{-1} \right]
\tag{\label{equation-92}}
\end{equation*}
$$

Therefore, a computationally efficient update can be performed by considering the matrices $\mathbf{G}$ and $\tilde{\mathbf{F}}^{-1}$ using $\eqref{equation-91}$ and $\eqref{equation-92}$ every time the proposed new configuration is accepted.

Within the Pfaffian wave function, in addition to the standard (equal time) Green's function

$$
  \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_R|\Phi_0}}{\braket{x|\Phi_0}} = G_{R,l}
$$

also the anomalous (superconducting) ones are present

$$
\begin{align*}
  \frac{\braket{x|\hat{d}_{R_l}^\dagger \hat{d}_{R_k}^\dagger|\Phi_0}}{\braket{x|\Phi_0}} =& \tilde{F}_{kl}^{-1} \\
  \frac{\braket{x|\hat{d}_R \hat{d}_{R'}|\Phi_0}}{\braket{x|\Phi_0}} =& F_{R,R'} + \sum_k G_{R,k} F_{R', R_k}
\end{align*}
$$

As before the interpretation of $\eqref{equation-90}$ and $\eqref{equation-91}$ is given by the Wick theorem (when bra and ket states are different) for the updated Green's functions $G'_{R,j}$ and ${\tilde{F}'_{ij}}^{-1}$. In this case, the anomalous contractions are also present, since $\ket{\Phi_0}$ has not a fixed number of particles.

The generalization to the case where more than one electron hop is straightforward by using $\eqref{equation-89}$ but it is very cumbersome. However, we can bypass the $m$-electron update by applying successively single-electron updates.

# Optimization of variational quantum states

Let $\mathcal{H}$ be a Hilbert space and let $\ket{\Psi_{\boldsymbol{\alpha}}} \in\mathcal{H}$ be a variational quantum state depending on $p$ real parameters $\boldsymbol{\alpha} \in\R^p$. If $\set{\ket{\mathbf{x}}}$ is an orthonormal basis of $\mathcal{H}$, the corresponding wavefunction representation is

$$
  \Psi_{\boldsymbol{\alpha}} (\mathbf{x}) = \braket{\mathbf{x}|\Psi_{\boldsymbol{\alpha}}}
$$

Define the probability distribution associated with the variational state

$$
  p_{\boldsymbol{\alpha}} (\mathbf{x}) = \frac{|\Psi_{\boldsymbol{\alpha}}(\mathbf{x})|^2}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}}
$$

Suppose that a set of configurations $\set{\mathbf{x}_i}_{i=1}^N$ have been sampled independently from $p_{\boldsymbol{\alpha}}$. The variational energy is defined as

$$
\begin{equation*}
  E_{\boldsymbol{\alpha}} = \frac{\braket{\Psi_{\boldsymbol{\alpha}}|\hat{H}|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}} = \mathbb{E}_{p_\alpha} [E_{L,\boldsymbol{\alpha}} (\mathbf{x})]
...
$$
  E_{L,\boldsymbol{\alpha}} (\mathbf{x}_i) = \braket{\mathbf{x}_i |\hat{H}|\Psi_{\boldsymbol{\alpha}}}{\mathbf{x}_i|\Psi_{\boldsymbol{\alpha}}} = \frac{(\hat{H}\Psi_{\boldsymbol{\alpha}})(\mathbf{x})}{\Psi_{\boldsymbol{\alpha}}(\mathbf{x})}
$$

Given the sampled configurations $\set{\mathbf{x}_i}$, the Monte Carlo estimator of the energy is

$$
  E_{\boldsymbol{\alpha}} \approx \frac{1}{N} \sum_{i=1}^N E_{L,\boldsymbol{\alpha}} (\mathbf{x}_i)
$$

Similarly,the variance of the local energy is

$$
\begin{align*}
  \sigma_{\Psi_{\boldsymbol{\alpha}}}^2 = \frac{\braket{\Psi_{\boldsymbol{\alpha}}|(\hat{H} - E_{\boldsymbol{\alpha}})^2|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}} = \mathcal{E}_{p_{\boldsymbol{\alpha}}} [(E_{L,\boldsymbol{\alpha}} (\mathbf{x}) - E_{\boldsymbol{\alpha}})^2]
...
$$
  \sigma_{\boldsymbol{\alpha}}^2 \approx \frac{1}{N} \sum_{i=1}^N [E_{L,\boldsymbol{\alpha}} (\mathbf{x}_i) - E_{\boldsymbol{\alpha}}]^2 
$$

## Reweighting technique

With a sampled configurations from $p_{\boldsymbol{\alpha}}$, the reweighting technique can be used to estimate expectation values corresponding to a nearby parameter vector $\boldsymbol{\alpha}' = \boldsymbol{\alpha} \delta\boldsymbol{\alpha}$. Using importance sampling, expectation values with respect to the distribution
...
$$
  \mathbb{E}_{p_{\boldsymbol{\alpha}'}} [f(\mathbf{x})] = \frac{\mathbb{E}_{p_{\boldsymbol{\alpha}}} [f(\mathbf{x}) \mathcal{R}_{\boldsymbol{\alpha}'}]}{\mathbb{E}_{p_{\boldsymbol{\alpha}}} [\mathcal{R}_{\boldsymbol{\alpha}'} (\mathbf{x})]}
$$

where the reweighting factor is

$$
  \mathcal{R}_{\boldsymbol{\alpha} + \delta\boldsymbol{\alpha}} (\mathbf{x}) = \left|\frac{\Psi_{\boldsymbol{\alpha} + \delta\alpha_k} (\mathbf{x})}{\Psi_{\boldsymbol{\alpha}}(\mathbf{x})} \right|^2
$$

In terms of the configurations $\set{\mathbf{x}_i}$), the energy corresponding to the parameters $\boldsymbol{\alpha}'$ can therefore be estimated as

$$
  E_{\boldsymbol{\alpha}'} = \frac{\sum_{i=1}^N E_{L,\boldsymbol{\alpha}} (\mathbf{x}_i) \mathcal{R}_{\boldsymbol{\alpha}'}(\mathbf{x}_i)}{\sum_{i=1}^N \mathcal{R}_{\boldsymbol{\alpha}'} (\mathbf{x}_i)}
$$

Similarly, the variance estimator becomes

$$
  \sigma_{\Psi_{\boldsymbol{\alpha}'}}^2 = \frac{\sum_{i=1}^N [E_{L,\boldsymbol{\alpha}} (\mathbf{x}_i) - E_{\boldsymbol{\alpha}'}]^2 \mathcal{R}_{\boldsymbol{\alpha}'} (\mathbf{x}_i)}{\sum_{i=1}^N \mathcal{R}_{\boldsymbol{\alpha}'}(\mathbf{x}_i)}
$$

Thus, a single Monte Carlo simulation performed at parameters $\boldsymbol{\alpha}$ can be used to estimate the energy and variance of nearby states $\Psi_{\boldsymbol{\alpha}'}$.

The optimal variational parameters can be obtained by minimizing either the energy $E_{\boldsymbol{\alpha}}$ and the variance $\sigma_{\boldsymbol{\alpha}}^2$ with respect to $\boldsymbol{\alpha}$. Within the reweighting approach, variance minimization is often more numerically stable since $\sigma_{\boldsymbol{\alpha}}^2 \geq 0$ so the objective function is always bounded from below. In contrast, the energy estimator can become unstable. In particular, if a $p_{\boldsymbol{\alpha}}$-sampled configuration $\mathbf{x}_i$ close to the nodal surface of $\Psi_{\boldsymbol{\alpha}'} (\mathbf{x})$, i.e. $\Psi_{\boldsymbol{\alpha}'} \to 0$, then $E_{L, \boldsymbol{\alpha}'} (\mathbf{x}_i) \to \pm\infty$. This produce large fluctuations in the energy estimator.

The reweighting technique becomes increasingly unstable when as the number of particles grows. Suppose only one parameter $\alpha_k$ is changed by $\delta\alpha_k$. For a many-body wavefunction of $N_p$ particles, the logarith of the wavefunction typically scales extensively, i.e. $\ln[|\Psi_{\boldsymbol{\alpha}}|] \sim O(N_p)$. Consequently, the reweighting factor behaves approximately as

$$
  \mathcal{R}_{\boldsymbol{\alpha} + \delta\alpha_k} (\mathbf{x}) \approx \exp(\pm c \delta\alpha_k N_p)
$$

for some system-dependent constant $c$. Thus, even a small change in the parameters can produce exponentially large or small weights. As $N_p$ increases, the variance of the reweighting factors grows exponentially, leading to severe statistical noise in the estimators of energy and variance. This severely limits the practical applicability of the reweighting technique in large many-body systems.

## Energy derivatives

The derivative of the variational energy $E_{\boldsymbol{\alpha}}$ in $\eqref{equation-101}$ with respect to a variational parameter $\alpha_k$ is given by

$$
  \frac{\partial E_{\boldsymbol{\alpha}}}{\delta\alpha_k} = \frac{\partial}{\partial\alpha_k} \frac{\braket{\Psi_{\boldsymbol{\alpha}}|\hat{H}|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}}
$$

To derive a closed analytical expression for this derivative, we consider a small variation $\alpha_k \to \alpha_k + \delta\alpha_k$. Then, the first-order Taylor expansion of $\Psi_{\boldsymbol{\alpha}} (\mathbf{x})$ about $\boldsymbol{\alpha}$ becomes

$$
\begin{equation*}
  \Psi_{\boldsymbol{\alpha} + \delta\alpha_k} (\mathbf{x}) = \Psi_{\boldsymbol{\alpha}} + \delta\alpha_k \frac{\partial\Psi_{\boldsymbol{\alpha}}(\mathbf{x})}{\partial\alpha_k} + O(\delta\alpha_k^2)
\tag{\label{equation-103}}
\end{equation*}
$$

For simplicity, we assume $\Psi_{\boldsymbol{\alpha}} \neq 0$ for all sampled configurations. In continuous fermionic systems, the nodal set $\set{\mathbf{x} : \Psi_{\boldsymbol{\alpha}} (\mathbf{x}) = 0}$ has zero measure and therefore does not contribute to Monte-Carlo integrals. On a lattie, islolated zeroes, $\Psi_{\boldsymbol{\alpha}} (\mathbf{x}) = 0$, can be removed an arbitrarily small pertubation to the variational Ansatz and taking the limit of vanishing perturbation.

Define the logarithmic derivative operator $\hat{Q}_k$ by its diagonal matrix elements

$$
\begin{equation*}
  \braket{\mathbf{x}|\hat{Q}_k|\mathbf{x}'} = \delta_{\mathbf{x},\mathbf{x}'} \hat{O}(\mathbf{x}) \tag{\label{equation-104}} 
\end{equation*}
$$

where the diaonal element is

$$
\begin{equation*}
  Q_k (\mathbf{x}) = \frac{\partial \ln {\Psi_{\boldsymbol{\alpha}}(\mathbf{x})}}{\partial\alpha_k} = \frac{1}{\Psi_{\boldsymbol{\alpha}}(\mathbf{x})} \frac{\partial\Psi_{\boldsymbol{\alpha}} (\mathbf{x})}{\partial\alpha_k}
\tag{\label{equation-105}}
\end{equation*}
$$

The operator $\hat{Q}_k$ is diagonal in the configuration basis $\set{\ket{\mathbf{a}}}$ but not necessarily Hermitian, since the logarithmic derivative of a complex wavefunction can be complex. In general, $\hat{Q}_k$ may depend on the variational parameters $\boldsymbol{\alpha}$, but this dependence will be omitted for notational simplicity.

In terms of $\hat{Q}_k$, the perturbed state $\ket{\Psi_{\boldsymbol{\alpha} + \delta\alpha_k}}$ can be compactly written as

$$
\begin{equation*}
  \ket{\Psi_{\boldsymbol{\alpha} + \delta\alpha_k}} = (1 + \delta\alpha_k \hat{Q}_k) \ket{\Psi_{\boldsymbol{\alpha}}}
...
\begin{equation*}
\begin{split}
  \norm{\Psi_{\boldsymbol{\alpha}}}^2 =& \braket{\Psi_{\boldsymbol{\alpha}}|(1 + \delta\alpha_k \hat{Q}_k)^* (1 + \delta\alpha_k \hat{Q}_k)|\Psi_{\boldsymbol{\alpha}}} \\
  =& \norm{\Psi_{\boldsymbol{\alpha}}}^2 [1 + 2\Re(\delta\alpha_k \bar{Q}_k) + \hat{O}(\delta\alpha_k^2)]
\end{split}
\tag{\label{equation-107}}
\end{equation*}
$$

The identity $\eqref{equation-106}$ follows immediately by taking the overlap of both sides with $\ket{\mathbf{x}}$, yielding

$$
  \braket{\mathbf{x}|\Psi_{\boldsymbol{\alpha} + \delta\alpha_k}} = \Psi_{\boldsymbol{\alpha}} (\mathbf{x}) + \delta\alpha_k O_k (\mathbf{x}) \Psi_{\boldsymbol{\alpha}} (\mathbf{x}) + O(\delta\alpha_k^2)
$$

Scaling the variational state $\ket{\Psi_{\boldsymbol{\alpha}}}$ by an arbitrary non-zero complex constant $c\in\mathcal{C}$, i.e. $\ket{\Psi'_{\boldsymbol{\alpha}}} = c\ket{\Psi'_{\boldsymbol{\alpha}}}$, leaves any expectation values of the form

$$
  \braket{A}_{\Psi_{\boldsymbol{\alpha}}} = \frac{\braket{\Psi'_{\boldsymbol{\alpha}}|\hat{A}|\Psi'_{\boldsymbol{\alpha}}}}{\braket{\Psi'_{\boldsymbol{\alpha}}|\Psi'_{\boldsymbol{\alpha}}}} = \frac{|c|^2}{|c|^2} \frac{\braket{\Psi'_{\boldsymbol{\alpha}}|\hat{A}|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi'_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}} 
$$

Thus, we are free to work with normalized states

$$
  \ket{\nu_{0,\boldsymbol{\alpha}}} := \frac{\ket{\Psi_{\boldsymbol{\alpha}}}}{\norm{\Psi_{\boldsymbol{\alpha}}}}
$$

Then, we introduce a set of states for each $k=1,\dots,p$

$$
  \ket{\nu_{k,\boldsymbol{\alpha}}} := (\hat{Q}_k - \bar{Q}_k) \ket{v_{0, \boldsymbol{\alpha}}}
$$

where

$$
  \bar{Q}_k = \braket{\nu_{0,\boldsymbol{\alpha}}|\hat{Q}_k|\nu_{0,\boldsymbol{\alpha}}} = \frac{\braket{\Psi_{\boldsymbol{\alpha}}|\hat{Q}_k|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}}
$$

The states $\ket{\nu_{k,\boldsymbol{\alpha}}}$ are orthogonal to $\ket{\nu_{0,\boldsymbol{\alpha}}}$ since 

$$
\begin{align*}
  \braket{\nu_{0,\boldsymbol{\alpha}}|\nu_{k,\alpha}} =& \braket{\nu_{0,\boldsymbol{\alpha}}|(\hat{Q}_k - \bar{Q}_k)|\nu_{k,\boldsymbol{\alpha}}} \\
  =& \braket{\nu_{0,\boldsymbol{\alpha}}|\hat{Q}_k|\nu_{0,\boldsymbol{\alpha}}} - \bar{Q}_k \underbrace{\braket{\nu_{0,\boldsymbol{\alpha}}\nu_{0,\boldsymbol{\alpha}}}}_{=1} \\
  =& \bar{Q}_k - \bar{Q}_k = 0
\end{align*}
$$

In general, however, the states $\braket{\nu_{0,\boldsymbol{\alpha}}|\nu_{k,\alpha}}$ are neither normalized nor mutually orthogonal. Thus, the set $\set{\ket{\nu,\boldsymbol{\alpha}}} \cup \set{\ket{\nu_{k,\boldsymbol{\alpha}}}}_{k=1}^p$ forms a semi-orthogonal basis.

Under a small variation of the parameter $\alpha_k$, the perturbed normalized state is given by

$$
  \ket{\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}} = \frac{\ket{\Psi_{\boldsymbol{\alpha} + \delta\alpha_k}}}{\norm{\Psi_{\boldsymbol{\alpha} + \delta\alpha_k}}}
$$

Using $\eqref{equation-106}$ and $\eqref{equation-107}$, the state can be expanded as

$$
  \ket{\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}} = \ket{\nu_{0,\boldsymbol{\alpha}}} + [\delta\alpha_k \hat{Q}_k - \Re(\delta\alpha_k \bar{Q}_k)]\ket{\nu_{0,\boldsymbol{\alpha}}} + O(\delta\alpha_k^2)
$$

Separating the real and imaginary parts of $\delta\alpha_k \bar{Q}_k$, we obtain

$$
  \ket{\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}} = [1 + i\Im(\delta\alpha_k \bar{Q}_k)] \ket{\nu_{0,\boldsymbol{\alpha}}} + \delta\alpha_k \ket{\nu_{k,\boldsymbol{\alpha}}} + O(\delta\alpha_k^2)
$$

which can by finally recast as

$$
\begin{equation*}
  \ket{\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}} = e^{i\delta\phi} (\ket{\nu_{0,\boldsymbol{\alpha}}} + \delta\alpha_k \ket{\nu_{k,\boldsymbol{\alpha}}}) + O(\delta\alpha_k^2)
\tag{\label{equation-113}}
\end{equation*}
$$

where $\delta\phi = \Im(\delta\alpha_k \bar{Q}_k)$.

Under this expansion, the derivative of the variational energy $E_{\boldsymbol{\alpha}}$ with respect to $\alpha_k$ becomes

$$
\begin{align*}
  \frac{\partial E_{\boldsymbol{\alpha}}}{\partial\alpha_k} =& \lim_{\delta\alpha_k \to 0} \frac{\braket{\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}|\hat{H}|\nu_{0,\boldsymbol{\alpha} + \delta\alpha_k}} - \braket{\nu_{0,\boldsymbol{\alpha}}|\hat{H}|\nu_{0,\boldsymbol{\alpha}}}}{\delta\alpha_k} \\
  =& \braket{\nu_{k,\boldsymbol{\alpha}}|\hat{H}|\nu_{0,\boldsymbol{\alpha}}} + \braket{\nu_{0,\boldsymbol{\alpha}}|\hat{H}|\nu_{k,\boldsymbol{\alpha}}} \\
  =& 2\Re\left(\frac{\braket{\Psi_{\boldsymbol{\alpha}}|\hat{H}(\hat{Q}_k - \bar{Q}_k)|\Psi_{\boldsymbol{\alpha}}}}{\Psi_{\boldsymbol{\alpha}|\Psi_{\boldsymbol{\alpha}}}} \right)
\end{align*}
$$

Notice also that, as expected, the phase factor $\delta\phi$ does not enter in the above expression.

Using the completeness relation

$$
  \sum_\mathbf{x} \ket{\mathbf{x}} \bra{\mathbf{x}} = \hat{I}
$$

the derivative can be refactored as

$$
\begin{align*}
  \frac{\partial E_{\boldsymbol{\alpha}}}{\alpha_k} =& 2\Re \left(\frac{\sum_\mathbf{x} \braket{\Psi_{\boldsymbol{\alpha}}|\hat{H}|\mathbf{x}}\braket{\mathbf{x}|(\hat{Q}_k - \bar{Q}_k)|\Psi_{\boldsymbol{\alpha}}}}{\sum_\mathbf{x} \braket{\Psi_{\boldsymbol{\alpha}}|\mathbf{x}}\braket{\mathbf{x}|\Psi_{\boldsymbol{\alpha}}}} \right) \\
  =& 2\Re \left(\frac{\sum_\mathbf{x} E_L^* (O_k (\mathbf{x}) - \bar{Q}_k) \ket|\Psi_{\boldsymbol{\alpha}} (\mathbf{x})|^2}{\sum_\mathbf{x} |\Psi_{\boldsymbol{\alpha}}(\mathbf{x})|^2} \right)
\end{align*}
$$
...
$$
\begin{equation*}
  f_k = -\frac{\partial E_{\boldsymbol{\alpha}}}{\partial\alpha_k} = -\frac{\partial}{\partial\alpha_k} \frac{\braket{\Psi_{\boldsymbol{\alpha}}|\hat{H}|\Psi_{\boldsymbol{\alpha}}}}{\braket{\Psi_{\boldsymbol{\alpha}}|\Psi_{\boldsymbol{\alpha}}}}
\tag{\label{equation-108}}
\end{equation*}
$$ 

Sampling configurations $\mathbf{x}$ from $p_{\boldsymbol{\alpha}}$ yields the estimator

$$
  f_k \approx -2\Re\left(\frac{1}{N} \sum_{i=1}^N E_L^* (\mathbf{x}_i) (Q_k (\mathbf{x}_i) - \bar{Q}_k) \right)
$$

where

$$
  \bar{O}_k \approx \frac{1}{N} \sum_{i=1}^N Q_k (\mathbf{x}_i)
$$

If the wavefunction is an exact eigenstate of $\hat{H}$, the local energy coincides with the corresponding exact eigenvalue $E$, regardless of the sample $\set{\mathbf{x}_i}$. This implies that $f_k$ identically vanishes without statistical fluctuations and thus we recover the zero-variance property for the energy derivatives.

In the general case, the variance is always bounded by the square of the number of particles $N_p^2$, because the local energy is an extensive random variable that is multiplied by a derivative estimator, i.e. $Q_k (\mathbf{x})$, that is also at most extensive. Thus, the variance of the product of two extensive quantities is at most $O(N_p^2)$. This implies that these expressions can be used to optimize wave functions with a large number of particles, since, at variance with methods based on the reweighting technique, the statistical fluctuations to estimate energy derivatives increases at most polynomially with the number of particles.

## Stochastic reconfiguration

The knowledge of energy derivatives $\eqref{equation-108}$ allows us to employ the steepest descent method to change the variational parameters $\boldsymbol{\alpha}$, even when $p$ is large

$$
\begin{align*}
  \alpha'_k =& \alpha_k + \delta\alpha_k \\
  \delta\alpha_k =& \Delta f_k \tag{\label{equation-111}}
\end{align*}
$$

where $\Delta$ is an arbitrary (small) constant. In principle, its value can be optimized to reach the lowest possible energy at each iteration. However, in most applications, it is a common practice to keep $\Delta$ constant alogn the minimization procedure. Then, the variational parameters are iteratively improved along a Markov chain procedure. In absence of noise, the steepest-descent method always converges to a minimum, where the Euler conditions $f_k = 0$ are satisfied. To see this, suppose $f_k \neq 0$ such that the energy $\boldsymbol{\alpha}'$ is given by a Taylor expansion to linear order in $\Delta$

$$
\begin{equation*}
\begin{split}
  E_{\boldsymbol{\alpha}'} =& E_{\boldsymbol{\alpha}} + \sum_k \frac{\partial E_{\boldsymbol{\alpha}}}{\partial\alpha_k} \delta\alpha_k + O(\Delta^2) \\
  =& E_{\boldsymbol{\alpha}} - \Delta \sum_k f_k^2 + O(\Delta^2)
\end{split}
\tag{\label{equation-109}}
\end{equation*}
$$

where we used that $\partial E_{\boldsymbol{\alpha}} / \partial\alpha_k = -f_k$ and $\delta\alpha_k = \Delta f_k$. Thus, for small $\Delta$, when the linear truncation is accurate enough in the Taylor expansion, we obtain that

$$
  \Delta E := E_{\boldsymbol{\alpha}'} - E_{\boldsymbol{\alpha}} = -\Delta \sum_k f_k^2 \leq 0
$$

Here, equality holds only when $f_k = 0$. Thus, the method converges to a minimum for a large number of iteraitions just because the energy monotonically decreases with the number of iterations. Within the steepest-descent method only the first derivative of the energy is computed and it is certain that a small change of the parameters $\delta\boldsymbol{\alpha} \in\R^p$ parallel to the force $\mathbf{f}\in\R^p$ will decrease the energy. The only issue concerns the size of $\Delta$, which must be taken sufficiently small to make the quadratic term in $\eqref{equation-109}$ negligible.

Let us discuss a simple case where all the parameters are equivalent and consider a wave function that is a linear combination of $p$ orthonormal Slater determinants $\ket{D_k}$ with $\braket{D_k |D_{k'}} = \delta_{k,k'}$

$$
  \ket{\Psi_{\boldsymbol{\alpha}}} = \sum_k \alpha_k \ket{D_k}
$$

then, it is reasonable to make a search with the condition that

$$
\begin{equation*}
  \delta s^2 = \sum_k \delta\alpha_k^2
\tag{\label{equation-110}}
\end{equation*}
$$

is small enough along the optimization procedure, where it is assumed that for small $\delta s$, the Taylor expansion of the energy to first order in $\delta\boldsymbol{\alpha}$ is accurate enough. In order to enforce the constraint $\eqref{equation-110}$, the usual scheme is to introduce a Lagrange multiplier $\mu$ and minimize the quadratic form

$$
  \Delta E + \mu\delta s^2 = \sum_k (-f_k \delta\alpha_k + \mu\delta\alpha_k^2)
$$

this approach yields a minimum condition

$$
  \delta\alpha_k = \frac{f}{2\mu}
$$

which is compatible with the steepest-descent approach $\eqref{equation-111}$ with $\Delta = 1/2\mu$. Although the above argument makes sense for simple wave functions, as for example the ones with linear dependence upon the parameters, serious difficulties arise in strongly-correlated states, where the dependence on the variational parameters is highly non-linear (e.g., in the Jastrow factors). Here, a small change of a given variational parameter can produce very different wave functions and physical quantities, whereas another parameter may weakly affect the wave function. In order to overcome these difficulties, it is important to introduce a more appropriate metric $\delta s^2$ that is used to estimate the "proximity" of two normalized (complex) wave functions $\ket{\nu_{0,\boldsymbol{\alpha}}}$ and $\ket{\nu_{0,\boldsymbol{\alpha} + \delta\boldsymbol{\alpha}}}$

$$
\begin{equation*}
  \delta s^2 = \min_{\delta\theta} \norm{\exp(-i\delta\theta) \nu_{0,\boldsymbol{\alpha} + \delta\boldsymbol{\alpha}} - \nu_{0,\boldsymbol{\alpha}}}^2
\end{equation*}
$$

In the previous definition of $\delta s^2$, the minimization on the phase factor $\delta\theta$ is necessary because we do not want to distinguish between two wave functions that differ only by an overall phase factor, as they produce the same correlation functions. In other words, we want to define a distance $\delta s^2$ that vanishes when we have physically equivalent wave functions. Then, we replace in $\eqref{equation-112}$ the expression for $\ket{\nu_{0,\boldsymbol{\alpha} + \delta\boldsymbol{\alpha}}}$ that is obtained by generalizing $\eqref{equation-113}$ to the case where several parameters are changed

$$
  \ket{\nu_{0,\boldsymbol{\alpha} + \delta\boldsymbol{\alpha}}} = \exp(i\delta\phi) \left(\ket{\nu_{0,\boldsymbol{\alpha}}} + \sum_k \delta\alpha_k \ket{\nu_{k,\boldsymbol{\alpha}}}\right) + O(|\delta\boldsymbol{\alpha}|^2)
$$

where $\delta\phi = \sum_k \Im(\delta\alpha_k \bar{Q}_k)$. Now, the minimization over $\delta\theta$ gives $\delta\theta = \delta\phi$, thus leading to

$$
  \delta s^2 = \sum_{k,k'} \braket{\nu_{k,\boldsymbol{\alpha}}|\nu_{k',\boldsymbol{\alpha}}} \delta\alpha_k \delta\alpha_{k'} + o(|\delta\boldsymbol{\alpha}|^2)
$$

Since all increments $\delta\alpha_k$ are assumed real, we can symmetrize the previous expression with respect to the indices $k$ and $k'$ and neglect the terms that are $o(|\delta\alpha|^2)$, obtaining

$$
  \delta s^2 = \sum_{k,k'} \left(\frac{\braket{\nu_{k,\boldsymbol{\alpha}}|\nu_{k',\boldsymbol{\alpha}}} + \braket{\nu_{k',\boldsymbol{\alpha}}|\nu_{k,\boldsymbol{\alpha}}}}{2} \right) \delta\alpha_k \delta\alpha_{k'}
$$

In this way, we can finally identify a matrix $\boldsymbol{S} \in \R^{p\times p}$ that fully determines the metric in the space of normalized wave functions

$$
  S_{k,k'} = \Re(\braket{\nu_{k,\boldsymbol{\alpha}}|\nu_{k',\boldsymbol{\alpha}}})
$$

which implies that the distance between two wave functions reads

$$
\begin{equation*}
  \delta s^2 = \sum_{k,k'} S_{k,k'} \delta\alpha_k \delta\alpha_{k'}
\tag{\label{equation-112}}
\end{equation*}
$$

At this point, it is natural to improve the steepest-descent method by using the metric given by $\mathbf{S}$. The minimization of $\Delta E + \mu\delta s^2$ with the metric $\delta s^2$ given in $\eqref{equation-114}$ improves the convergence to the minimum of the variational energy with respect to the simple steepest-descent approach, as non-equivalent parameters can be appropriately changed with a different scale. This approach is called stochastic reconfiguration. The minimization of $\Delta E \mu\delta^2$ \gives
...
which is a set of linear equations for the unknown vector $\delta\boldsymbol{\alpha}$. After having solved this linear system, we can update the variational parameters until convergence is reached. As in the steepest-descent method, we can set $\Delta = 1/2\mu$ small enough, which may be kept fixed during the optimization. We would like to stress the fact that, since the matrix $\boldsymbol{S}$ is strictly positive definite, the energy is monotonically decreasing along the optimization as

$$
  \Delta E = -\Delta \sum_{k,k'} S_{k,k'}^{-1} f_k f_{k'} < 0
$$

Within a Monte Carlo procedure, the matrix $\mathbf{S}$ is evaluated by a finite sampling of $N$ configurations $\set{\mathbf{x}_i}$ as

$$
  S_{k,k'} \approx \Re\left(\frac{1}{N} \sum_{i=1}^N (Q_k (\mathbf{x}_i) - \bar{Q}_k)(Q_{k'} (\mathbf{x}_i) - \bar{Q}_k) \right)
$$

the forces are also computed in a similar way. Thus the solution of the linear system $\eqref{equation-113}$ is affected by statistical errors, yielding statistical fluctuations of the final variational parameters $\set{\alpha_k}$, even when convergence has been reached. In this regime, it is convenient to perform several iterations, in order to obtain accurate values for the variational parameters by averaging them after equilibration.

In the limit $\Delta\to 0$, the optimization procedure reduces to a set of stochastic differential equations that are approximately described by a Langevin dynamics

$$
  \frac{\d\boldsymbol{\alpha}(t)}{\d t} = \mathbf{S}^{-1} \mathbf{f} [\boldsymbol{\alpha}(t)] + \boldsymbol{\eta}(t)
$$

where we have explicitly indicated that the forces depend upon the variational parameters. Here, we neglect the correlation between the force components in the statistical noise, since this aspect does not alter the qualitative discussion. The noise is described by

$$
\begin{align*}
  \braket{\eta_k (t)} =& 0 \\
  \braket{\eta_k (t) \eta_{k'} (t)} =& 2T_\text{noise} \delta_{k,k'} \delta (t - t')
\end{align*}
$$

Within a Monte Carlo optimization, the parameter $T_\text{noise}$ depends on the fact that both $\mathbf{f}$ and $\mathbf{S}$ are evaluated by using a finite sampling with $N$ configurations. We would like to emphasize that the inversion of the matrix $\mathbf{S}$ introduces a controllable bias, which vanishes for $N\to\infty$. This is due to the fact that the matrix elements $S_{k,k'}$, obtained from $\eqref{equation-114}$, are random variables distributed around the corresponding exact values with fluctuations that are $O(1/\sqrt{N})$. Thus, the noise can be tuned to zero by increasing $N$ as $T_\text{noise} \propto 1/N$, for the central limit theorem. The number of iterations necessary to reach convergence is weakly dependent on the "temperature" $T_\text{noise}$. However, it crucially depends on the energy landscape of the system. The optimal value for $N$ is the smallest one that provides the fluctuations of the parameters within the desired accuracy. It should be noted that the variational parameters, averaged over the Langevin simulation, are close to the true energy minimum, but may be affected by a bias that scales to zero with $T_\text{noise}$, due to the presence of non-quadratic terms in the energy landscape. Thus, the best strategy to obtain accurate results is to average the variational parameters by systematically increasing the value of $N$.

### Covariance property

### Scale-invariant regularization

### Optimization using the signal-to-noise ratio
