...
$$
\begin{align*}
  \hat{H}_0 =& \mathbf{d}^\dagger \mathbf{UU}^\dagger \mathbf{TUU}^\dagger \mathbf{d} \\
  =& \boldsymbol{\Phi}^\dagger \mathbf{E} \boldsymbol{\Phi} = \sum_\alpha \varepsilon_\alpha \phi_\alpha^\dagger \phi_\alpha
\end{align*}
$$
...
- The matrix $\mathbf{U}_\text{occ}$ changes in a way that is not a right multiplication by an $N_e \times N_e$ unitary
...
The same kind of formalism can be used also if the non-interacting Hamiltonian contains and electron pairing that couples up and down spin. Let us consider a BCS Hamiltonian by
...
\begin{align*}
  ...
  &+ \sum_{i,j} \Delta_{ij} (\hat{c}_{i,\uparrow}^\dagger \hat{c}_{j,\downarrow} + \hat{c}_{j,\downarrow} \hat{c}_{i,\downarrow})
\end{align*}
$$

where we have included a chemical potential $\mu_0$, which fixes, on average, the number of electrons. In this case, the total number of particles is not conserved and the concept of single-particle orbitals is not defined. The ground state of the BCS Hamiltonian is naturally written in terms of a pairing function. Nevertheless, we can perform a particle-hole transformation on the spin-down electrons

$$
\begin{align*}
  \hat{c}_{i,\uparrow} \to \hat{f}_{i,\uparrow} =: \hat{d}_i \tag{\label{equation-75}} \\
  \hat{c}_{i,\downarrow}\to \hat{f}_{i,\downarrow}^\dagger =: \hat{d}_{i+L}^\dagger \tag{\label{equation-76}}
\end{align*}
$$

Then, apart from constant terms, the transformed BCS Hamiltonian has the form of $\eqref{equation-77}$, where the matrix $\mathbf{T}$ is given by ($i,j \leq L$)
...
Since, after the particle-hole transformation, the number of particles (but not the $z$ component of the spin) is conserved, the eigenstates of the BCS Hamiltonian can be expressed into orbitals, similar to the ones of $\eqref{equation-78}$, but without having a definite spin component along $z$.

Notice that the particle-hole transformations $\eqref{equation-75}$ and $\eqref{equation-76}$ change a spin down into the vacuum and vice versa, so that

$$
\begin{align*}
  \ket{0_i} \to& \hat{f}_{i,\downarrow}^\dagger \ket{\tilde{0}_i} \\
  \hat{c}_{i,\uparrow}^\dagger \ket{0_i} \to& \hat{f}_{i,\uparrow}^\dagger \hat{f}_{i,\downarrow}^\dagger \ket{\tilde{0}_i} \\
  \hat{c}_{i,\downarrow}^\dagger \ket{0_i} \to& \ket{\tilde{0}_i} \\
  \hat{c}_{i,j}^\dagger \hat{c}_{i,\downarrow}^\dagger \ket{0_i}
\end{align*}
$$

where $\ket{\tilde{0}_i}$ is the vacuum of the $f$ electrons, i.e. $\hat{f}_{i,\uparrow} \ket{\tilde{0}} = \hat{f}_{i,\downarrow} \ket{\tilde{0}} = 0$. By using the anticommutation relations of the fermion operators, we have that the occupations $\hat{u}_{i,\sigma}$ of new fermionic operators are related to the original ones by

$$
\begin{align*}
  \hat{u}_{i,\uparrow} =& \hat{f}_{i,\uparrow}^\dagger \hat{f}_{i,\uparrow} = \hat{c}_{i,\uparrow}^\dagger \hat{c}_{i,\dagger} = \hat{n}_{i,\uparrow} \\
  \hat{u}_{i,\downarrow} =& \hat{f}_{i,\downarrow}^\dagger \hat{f}_{i,\downarrow}= 1 - \hat{c}_{i,\downarrow}^\dagger \hat{c}_{i,\downarrow} = 1 - \hat{n}_{i, \downarrow}
\end{align*}
$$

which implies that the relations among denisities and magnetizations on site $i$ are given by

$$
\begin{align*}
  u_i =& u_{i,\uparrow} + u_{i,\downarrow} = n_{i,\uparrow} - n_{i,\downarrow} + 1 = m_z + 1 \\
  w_i^z =& u_{i,\uparrow} - u_{i,\downarrow} = n_{i,\uparrow} + n_{i,\downarrow} - 1 = n_i - 1
\end{align*}
$$

where we have introduced $u_i$ and $w_i^z$, as the density and magnetization for the transformed operators, respectively. The letters $u$ and $w$ are chosen because they look like an upside down $n$ and $m$, respectively. Thus, apart from constant terms, upon the particle-hole transformation of $\eqref{equation-75}$ and $\eqref{equation-76}$, the density changes into the $z$ component of the spin and vice versa. As a corollary, we obtain that, whenever the total magnetization of the original particles is vanishing, the total number of new particles is equal to the number of sites $L$.
