...
using the anticommutation relations $[\hat{c}_i, \hat{c}_i^\dagger]_+ = \delta_{ij}$, the annihilation operato $\hat{c}_{R_l^}$ must be anticommuted through the ordered string until it reaches its matching creation operators. Thus produces a sign factor $(-1)^p$, where $p$ is the number of fermionic operators it passes. After annihilation and reinsertion of $\hat{c}_{R'_l}^\dagger$, the resulting state can again be written in canonical ordered form
...
$$
  \operatorname{perm}(M) = \sum_{\sigm \in S_N} \prod_{i=1}^N M_{i,\sigma(i)} 
$$
...
$$
  \braket{\phi_\alpha |\phi_\alpah} = \braket{0|\hat{\phi}_\alpha \hat{\phi}_\alpha^\dagger | 0} = 1
$$
...
$$
  \math{M}_z = \sum_i \hat{n}_{i,\uparrow} - \hat{n}_{i,\downarrow}
$$
...
$$
\begin{align*}
  \tide{U}'_{j,\alpha} =& \tilde{U}_{j,\alpha} + \sum_{r=1}^m \delta_{j,l_r} A_\alpha^{K_r, l_r} \\
...
$$
  \operatorname{Pf}(\mathbf{A}) = \frac{1}{2^n n!} \sum_{\signa\in S_{2n}} \operatorname{sgn}(\sigma) \prod_{k=1}^n a_{\sigma(2k - 1), \sigma(2k)}
$$
...
\begin{align*}
  ...
  =& \begin{bmatrix} \mathbf{I} & \mathbf{0} \\ (\mathbf{A}^{-1} \mathbf{B})^\top & \mathbf{I} \end{matrix} \begin{bmatrix} \mathbf{A} & \mathbf{0} \\ \mathbf{0} & \mathbf{C}^{-1} + \mathbf{B}^\top \mathbf{A}^{-1} \mathbf{B} \end{bmatrix} \begin{bmatrix} \mathbf{I} & \mathbf{A}^{-1} \mathbf{B} \\ \mathbf{0} & \mathbf{I} \end{bmatrix}
\end{align*}
$$
...
$$
  f_k = -\frac{\partial E_{\boldsymbol{\alpha}}}{\partial\alpha_k}
$$ 
