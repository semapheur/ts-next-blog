...
Let $\mathcal{H}_1, \dots, \mathcal{H}_n$ be Hilbert spaces. The algebraic tensor product

$$
  \bigoplus_{j=1}^n \mathcal{H}_j
$$

is a pre-Hilbert space with respect to the inner product $\braket{\cdot, \cdot}$ determined by inner products $\braket{\cdot, \cdot}_{\mathcal{H}_j}$ in the $\mathcal{H}_j$, by linear extension of

$$
  \braket{\psi_1 \otimes\cdots\otimes \psi_n, \phi_1 \otimes\cdots\times \phi_n} = \pro_{j=1}^n \braket{\psi_j, \phi_j)_{\mathcal{H}_j}
$$

This inner product space can be completed to a Hilbert space, $\mathcal{H} = \bigoplus_{j=1}^n \mathcal{H}_j$, with respect to the induced norm. When all factors coincide, we write

$$
  \mathcal{H}^{\otimes n} := \bigoplus_{j=1}^n \mathcal{H}
$$

## Symmetric and antisymmetric tensor powers

<MathBox title="Symmetric and antisymmetric tensor powers" botType="definition">
Let $V$ be a vector space. 

1. The $n$-fold symmetric tensor power of $V$ is a pair $(V^{\otimes_s n}, m_s)$ where
- $V^{\otimes_s n}$ is a vector space
- $m_s$ is a symmetric $n$-linear map

2. The $n$-fold antisymmetric or exterior power of $V$ is a pair $(V^{\otimes_a n}, m_a)$, where
- $V^{\otimes_a n}$ is a vector space
- $m_a$ is an antisymmetric (alternating) $n$-linear map

The (anti)symmetric $n$-linear map $m_{s/a}$ satisfy the universal property: For every vector space $W'_{s/a}$ and every (anti)symmetric $n$-linear map $m'_{s/a} : V^n \to W'$, there exists a unique linear homomorphism $u_{s/a} : V^{\otimes_{s/n} n} \to W'_{s/a}$ with $m'_{s/a} = u_{s/a} \otimes m_{s/a}$.
</MathBox>

### Construction as quotients

Let $V^{\otimes n}$ be an algebraic tensor power and let $S_n$ be the symmetric group on $\set{1,\dots, n}$. There is a natural linear representation

$$
  U: S_n \to \operatorname{GL}(V^{\otimes n})
$$

defined on elementary tensors by

$$
  U(sigma) \left(\bigotimes_{j=1}^n x_i \right) = \bigotimes_{j=1}^n x_{\sigma^{-1} (j)},\; \sigma\in S_n
$$

and extended lienarly to all of $V^{\otimes n}$. The use of $\sigma^{-1}$ ensures that

$$
  U(\sigma\tau) = U(\sigma) U(\tau)
$$

so $U$ is a representation.

The symmetric and antisymmetric tensor power can be constructed as quotient spaces of $V^{\otimes n}$. For the symmetric case, we define a subspace of relations
$$
  I_s := \operatorname{span}\set{v - U(\sigma)v | v\in V^{\otimes n}, \sigma\in S_n}
$$

The $n$th symmetric tensor power is then defined as the quotient space $V^{\otimes_s n} \cong V^{\otimes n} / I_s$. In this quotient, all tensors that differ by a permutation of factors are equivalent, defining the equivalence relation

$$
  (x_1,\dots,x_n) \sim (x_{\sigma(1)},\dots,x_{\sigma(j)})
$$

For the antisymmetric case, define the subspace of relations

$$
  I_a := \operatorname{span}\set{v - \operatorname{sgn}(\sigma) U(\sigma)v | v\in V^{\otimes n}, \sigma \in S_n }
$$

The $n$th antisymmetric tensor power is then defined as the quotient space $V^{\otimes_a n} \cong V^{\otimes n} / I_a$. In this quotient, permuting factors multiplies the tensor by the sign (parity) of the permution, giving the equivalence relation

$$
  (x_1,\dots, x_n) \sim \operatorname{sgn}(\sigma) (x_{\sigma(1)},\dots,x_{\sigma(n)}
$$

### Realizations as subspaces via symmetrization

The symmetric and antisymmetric tensor products can be realized as subspaces of the ordinary tensor product. In $V^{\otimes n}$ define the linear maps $P_s$ and $P_a$ by linear extension of

$$
\begin{align*}
  P_s =& \frac{1}{n!} \sum_{\sigma\in S_n} U(\sigma) \\
  P_a =& \frac{1}{n!} \sum_{\sigma\in S_n} \operatorname{sgn}(\sigma) U(\sigma)
\end{align*}
$$

The linear maps are projections satisfying

$$
  P_s^2 = P_s,\quad P_a^2 = P_a
$$
...
$$
\begin{align*}
  \operatorname{ran}(P_s) =& \set{v \in V^{\otimes n} | U(\sigma)v = v}
  \operatorname{ran}(P_a) =& \set{v \in V^{\otimes n} | U(\sigma)v = \operatorname{sgn}(\sigma) v}
\end{align*}
$$

The unique linear isomorphism $V^{\otimes_{s/a} n} \cong \operatorname{ran}(P_{s/a}$ is given by

$$
  x_1 \otimes_{s/a} \cdots \otimes_{s/a} x_n \to P_{s/a} (x_1 \otimes\cdots\otimes x_n)
$$

### Symmetric and antisymmetric tensor power of a Hilbert space

Let $\mathcal{H}$ be a Hilbert space with inner product $\braket{\cdot,\cdot}_\mathcal{H}$ and let $S_n$ be the symmetric group on $\set{1,\dots,n}$. The permutation representation $U: S_n \to \mathcal{U}(\mathcal{H}^{\otimes n})$ is unitary. Hence, $P_s^{(n)}$ and $P_a^{(n)}$ are orthogonal projections. The (anti)symmetric tensor powers of $\mathcal{H}$ can be defined by

$$
  \mathcal{H}^{\otimes_{s/a} n} := \operatorname{ran}(P_{s/a}^{(n)})
$$

The tensor product space $\otimes_{s/a}^n \mathcal{H}$ is a pre-Hilbert space with respect to the inner product $\braket{\cdot, \cdot}$, defined by linear extension of

$$
  \braket{\psi_1 \otimes_s \cdots \otimes_s \psi_n, \phi_1 \otimes_s \cdots \otimes_s \phi_n} = \sum_{\sigma\in S_n} \prod_{j=1}^n \braket{\psi_j, \phi_{\sigma(j)}}_\mathcal{H}
$$

for symmetric tensors, and

$$
\begin{align*}
  \braket{\psi_1 \otimes_a \cdots \otimes_a \psi_n, \phi_1 \otimes_a \cdots \otimes_a \phi_n} = \sum_{\sigma\in S_n} \operatorname{sgn}(\sigma) \prod_{j=1}^n \braket{\psi_j, \phi_{\sigma(j)}}_\mathcal{H} \\
  =& \det(\psi_i, \psi_j)_{i,j=1}^n
\end{align*}
$$

for antisymmetric tensors. The pre-Hilbert space $\otimes_{s/a}^n \mathcal{H}$ can be completed to a Hilbert space with respect to the induced norm.
