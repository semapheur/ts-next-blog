...
Let $V$ be a Hilbert space with dual $V^*$.
- A *ket* $\ket{v}\in V$ denotes a vector representing the state of a quantum system
- A *bra* $\bra{f}\in V^*$ denotes a linear form $f : V \mapsto \mathbb{C}$ and is a covector to $\ket{f}$
...
\begin{align*}
  ...
  \ket{v}\braket{v|\lambda} =& \lambda\ket{x} \\
