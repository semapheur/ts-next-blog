...
- A <b>bra-ket</b> $\braket{f | v}$ denotes an innerproduct
...
- Inner product: $\braket{\psi|\hat{A}|\phi} \rangle = \braket{\psi | \phi'} \rangle \in \mathbb{C}$
...
1. Description of the state of a system: The state of an isolated physical system is represented, at a fixed time $t$, by a state vector $\ket{\psi}$ belonging to a Hilbert space $\mathcal{H} = L^2(M)$ of square normalisable functions, called the state space.
...
$$
  \int |\psi(\mathbf{x}, t)|^2 \d^3 x = \int \psi^* \psi \;\d^3x = \braket{\psi | \psi} < \infty
$$
...
The Schrödinger equation describes the time evolution of a state vector $\ket{\psi(\mathbf{x}, t)}$

$$
  i\hbar \frac{\partial}{\partial t}\ket{\psi} = \hat{H}\ket{\psi(t)}
$$
