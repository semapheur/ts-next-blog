...
$$
  O_\text{L} (x) := \braket{\braket{x|\hat{O}|\Psi}}{\braket{x|\Psi}} = \frac{\braket{x|\hat{O}|\Psi}}{\Psi(x)}
$$

expressing $\braket{\hat{O}}$ as

$$
  \braket{\hat{O}} = \frac{\sum_{x\in\Omega} |\Psi(x)|^2 O_\text{L} (x)}{\sum_{x\in\Omega} |\Psi(x)|^2}
$$
...
$$
  \braket{\hat{O}} = \sum_{x\in\Omega} \mathcal{P}(x) O_\text{L} (x)
$$

This is recognized as the expectation value of the random variable $O_\text{L} (x)$ with respect to the probability measure $\mathcal{P}$ over $\Omega$. In particular, for the Hamiltonian $\hat{H}$, the corresponding local estimator
...
$$
  \braket{\hat{H}} = \sum_{x\in\Omega} \mathcal{P}(x) E_\text{L} (x)
$$
...
$$
  \braket{\hat{O}}_N := \frac{1}{N}\sum_{n=1}^N \hat{O}_\text{L} (x_n)
$$
...
A fundamental property of the variational Monte Carlo method is the zero-variance property, which relates statistical fluctuation of the local energy to the spectral properties of the Hamiltonian. Suppose that the variational state $\ket{\Psi}\in\mathcal{H}$ coincides with an exact eigenstate of $\hat{H}$, i.e. $\hat{H}\ket{\Psi} = E\ket{\Psi}$. Then, it follows that the local energy $E_L$ is constant

$$
  E_L (x) = \frac{\braket{x|\hat{H}|\Psi}}{\braket{x|\Psi}} = E\frac{\braket{x|\Psi}}{\braket{x|\Psi}} = E
$$

Thus, the random variable $E_L (x)$ does not depend on $\ket{x}$, implying that its variance is zero, while its mean value $E$ coincides with the exact eigenvalue (in other words, $E_L$ is not a random variable). Clearly, this is an extreme case that is very rare for generic correlated model. However, in general, the variance of $E_L (x)$ will decrease its value whenever the variational state $\ket{\Psi}$ will approach an eigenstate. This fact is very important to reduce the statistical fluctuations and improve the numerical efficiency. The zero-variance property is a feature that exists only for quantum expectation values, whic it is absent in classical calculations, where observables have thermal fluctuations.

The average of the square of the local energy corresponds to the quantum average of the Hamiltonian squared

$$
\begin{align*}
  \frac{\braket{\Psi|\hat{H}^2|\Psi}}{\braket{\Psi|\Psi} =& \frac{\sum_{x\in\Omega} \braket{x|\hat{H}|x}\braket{x|\hat{H}|\Psi}}{\sum_{x\in\Omega} \braket{\Psi|x}\braket{x|\Psi}} \\
  =& \frac{\sum_{x\in\Omega} |\Psi(x)|^2 |E_L (x)|^2}{\sum_{x\in\Omega} |\Psi(x)|^2}
\end{align*}
$$

Thus, the variance of the random variable $E_L (x)$ is exactly equal to the quantum variance of the Hamiltonian over the variational state $\ket{\Psi}$

$$
  \sigma_{E_L}^2 = \frac{\braket{\Psi|(\hat{H} - E)^2|\Psi}}{\braket{\Psi|\Psi}}
$$
