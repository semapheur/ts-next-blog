...
### Two-site model the $\mathrm{H}_2$ molecule

In the simplest approximation to the $\text{H}_2$ molecule, we can take only the four states with $\hat{S}_z = 0$ that have been used in the Heitler-London and Hartree-Fock wave functions
...
within this truncated Hilbert space, the Hamiltonian reads
...
where $\hat{n}_{i,\sigma} = \hat{c}_{i,\sigma}^\dagger \hat{c}_{i,\sigma}$ is the density per spin $\sigma$ on the site $i=1,2$. Here $t$, $U$ and $V$ are parameters that depen on the overlap of orbitals $\phi(\mathbf{r} - \mathbf{R}_i)$ centered around the two atoms
...
In principle, the Hamiltionian $\eqref{equation-50}$ also contains the Coulomb interactio between the protons, however, this is a constant term once their positions are fixed in $\mathbf{R}_1$ and $\mathbf{R}_2$.

Thus, within the truncated bases $\set{\ket{1},\ket{2},\ket{3},\ket{4}}$, the solution of the problem corresponds to the diagonalization of the $4\times 4$ matrix
...
The Hartee-Fock wave function is the exact ground state for $U = V = 0$, given by
...
Here, the ionic configurations $\hat{c}_{1,\uparrow}^\dagger \hat{c}_{1,\downarrow}^\dagger \ket{0}$ and $\hat{c}_{2,\uparrow}^\dagger \hat{c}_{2,\downarrow}^\dagger \ket{0}$ appear with the same weight of the non-ionic configurations $\hat{c}_{1,\uparrow}^\dagger \hat{c}_{2,\downarrow}^\dagger \ket{0}$ and $\hat{c}_{2,\uparrow}^\dagger \hat{c}_{1,\downarrow}^\dagger \ket{0}$. Thus, when the two Hydrogen atoms are pulled apart, this variational wave function does not reproduce the atomic limit. By contrast, this state becomes exact whenever the electron-electron repulsion vanishes.

The Heitler-London wave function is the ground state for $t = 0$ and $U > V$, given by
...
This wave function becomes very accurate in the limit $\mathbf{R}_1 - \mathbf{R}_2| \to \infty$, and becomes exact in the atomic limit, where the distance between the two atoms is infinite.

In the generic case with finite $t$, $U$ and $V$, the exact ground state is a superposition of these two wavefunctions and gives the best variational wave function in this truncated Hilbert space.

The quality of the approximations improves by enlarging the Hilbert space by including more orbitals in the truncated basis. This comes at the cost of increasing complexity. At present, there are no exact methods that allow the solution of a general many-body Hamiltonian with a computational effort that scales polynomially with the system size. The complexity of the many-body problem is generally exponential.

In the notation above, $\ket{0}$ is the vacuum state and $\hat{c}_{i,\sigma}^\dagger$ is the creation operator for an electron with spin $\sigma\in \set{\uparrow, \downarrow}$ in a given orbital $\phi(\mathbf{r} - \mathbf{R}_i)$ centered around the atom $i = 1, 2$ at position $\mathbf{R}_i$
